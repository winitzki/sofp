...
in the infix syntax:{\footnotesize{}he infix syntax:{\footnotesize{}
\[
...
\begin{itemize}
\item {\footnotesize{}$P^{A}\equiv\left(A\Rightarrow R\right)+\left(S\Rightarrow A\right)$};
{\footnotesize{}$\quad P^{A}\equiv\left(A\Rightarrow A\right)\Rightarrow1+A$}{\footnotesize \par}
\end{itemize}
\end{frame}
...
\item Define the applicative instance for $F^{A}=1+A\times A\times A$.
...
functors.
\item For any contrafunctor $H^{A}$, construction 5 shows that $F^{A}\equiv H^{A}\Rightarrow A$
