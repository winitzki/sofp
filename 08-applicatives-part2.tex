...
\vspace{-0.1cm}Transform the result of a \texttt{\textcolor{blue}{\footnotesize{}map2}}:\texttt{\textcolor{blue}{\footnotesize{}\smallskip{}
}}{\footnotesize \par}
...
\textcolor{blue}{\footnotesize{}~~=~map2(cont1,~cont2)(g~andThen~f)}
\[
...
\[
g^{F^{A\Rightarrow B}}\odot h^{F^{B\Rightarrow C}}\equiv\text{map}_{2}\left(g,h\right)(p^{A\Rightarrow B},q^{B\Rightarrow C}\Rightarrow p\circ q)
\]
\end{itemize}
\end{itemize}
...
\left((x^{B\Rightarrow C}\Rightarrow\phi^{A\Rightarrow B}\circ x)^{\uparrow}g^{F^{B\Rightarrow C}}\right)\odot h^{F^{C\Rightarrow D}} & =(x^{B\Rightarrow D}\Rightarrow\phi^{A\Rightarrow B}\circ x)^{\uparrow}\left(g\odot h\right)\\
g^{F^{A\Rightarrow B}}\odot\left((x^{B\Rightarrow C}\Rightarrow x\circ\phi^{C\Rightarrow D})^{\uparrow}h^{F^{B\Rightarrow C}}\right) & =(x^{A\Rightarrow C}\Rightarrow x\circ\phi^{C\Rightarrow D})^{\uparrow}\left(g\odot h\right)
\end{align*}
}{\footnotesize \par}
\begin{itemize}
...
lifting between categories
\end{itemize}
\end{frame}

\begin{frame}{Deriving the category laws for $\left(\text{id}_{\odot},\odot\right)$}
\begin{itemize}
\item \vspace{-0.10cm}Consider the third naturality law for \texttt{\textcolor{blue}{\footnotesize{}map2}}:
\end{itemize}
\end{frame}

\begin{frame}{Deriving the functor laws for \texttt{\textcolor{blue}{\footnotesize{}ap}} }
\begin{itemize}
\item \vspace{-0.10cm}Identity law: 
\[
\text{ap}\left(\text{id}_{\odot}\right)=\text{id}^{F^{A}\Rightarrow F^{A}}
\]
\item Composition law:
\[
\text{ap}\left(g\odot h\right)=\text{ap}\left(g\right)\circ\text{ap}\left(h\right)
\]
\end{itemize}
\end{frame}
...
\texttt{\textcolor{blue}{\footnotesize{}Boolean}} are ``numeric''esize{}Boolean}} are ``numeric''
monoids
...
{\footnotesize{}$\text{Int}+\text{String}\times\text{String}\times\left(\text{Int}\Rightarrow\text{Bool}\right)+\left(\text{Bool}\times\text{String}\Rightarrow1+\text{String}\right)$}{\footnotesize \par}
...
monoidal parameters are applicative: write $F^{A}=Z_{1}+A\times\left(Z_{2}+A\times...\right)$
...
a natural transformation when it is defined using \texttt{\textcolor{blue}{\footnotesize{}unit}}
as shown.
