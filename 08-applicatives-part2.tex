...
\begin{itemize}
\item \vspace{-0.15cm}The monoidal operation $\oplus$ can be \textbf{symmetric}
...
can be \textbf{symmetric} w.r.t.\ its arguments:{\footnotesize{}
\[
...
\item Examples:
\begin{itemize}
\item $\text{List}$gin{itemize}
\item $\text{List}$ is symmetric; applicative parsers are not
\item If defined through the monad instance, \texttt{\textcolor{blue}{\footnotesize{}zip}}
is usually not symmetric
\item All polynomial functors with \emph{symmetric} monoidal coefficients
are symmetric applicative functors
\end{itemize}
\item Most applicative constructions preserve symmetry
\item The same applies to applicative contrafunctors and profunctors
\item Symmetry makes proving associativity easier:{\footnotesize{}
\[
