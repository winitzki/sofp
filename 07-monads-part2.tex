...
\item Implement the monad methods for $F^{A}\equiv\left(Z\Rightarrow1+A\right)\times\text{List}^{A}$quiv\left(Z\Rightarrow1+A\right)\times\text{List}^{A}$
...
m$_{1}$; x $\leftarrow$ m$_{2}$ \} yield x}}{\footnotesize{} and
so on.}{\footnotesize\par}
\end{itemize}
\end{frame}
