...
\begin{frame}{Addendum: Miscellaneous remarks }
\begin{itemize}
\item \vspace*{-0.2cm}A non-empty list $F^{A}\equiv A\times\text{List}^{A}$
is a semigroup but not a monoid.
\item Any polynomial functor $F^{A}\equiv p(A)$ can be made into a monad
when $p(x)$ is a polynomial of the form $p(x)=x^{n_{1}}+x^{n_{2}}+...+x^{n_{k}}$
for some \emph{positive} integers $n_{1}$, ..., $n_{k}$. Indeed,
any $F^{A}$ of this form may be built from the identity monad via
constructions 3 and 5. To illustrate this, denote $E_{1}\equiv1$,
$E_{n+1}\equiv1+E_{n}$. Monoid construction 2 makes $E_{n}$ into
monoids. Then the monads $E_{n}\Rightarrow A$ (reader) and $E_{n}\times A$
(writer) are equivalent to polynomial monads $A\times...\times A$
and $A+...+A$.
\item Contrafunctors cannot be monads or semimonads because if $H^{A}$
is a contrafunctor then $H^{H^{A}}$ is a \emph{functor}, so a natural
transformation between $H^{H^{A}}$ and $H^{A}$ (in either direction)
is impossible.
\item Any exp-poly contrafunctor $H^{A}$ is equivalent to $H^{A}\equiv K^{A}\Rightarrow Z$
for an exp-poly functor $K^{A}$ and a fixed type $Z$. So, construction
9 becomes $F^{A}\equiv\left(K^{A}\Rightarrow Z\right)\Rightarrow A\times G^{A}$
for functors $K^{A}$, $G^{A}$ and a fixed type $Z$.
\item An example of combining natural transformations: Given functors $C$,
