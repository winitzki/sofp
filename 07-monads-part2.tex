...
\vspace{-0.1cm}Mathematicians have generalized this to 
...
there exists the \textbf{composition} morphism $f\circ g$ of type
$A\rightsquigarrow C$
...
\end{itemize}
General \textbf{functor}: a ``lifting'' \textbf{functor}: a ``lifting'' from one category to another
\begin{itemize}
...
theory
\item Our ``functor'' is a lifting from plain functions to $F$-functions
\end{itemize}
\end{itemize}
\end{frame}
