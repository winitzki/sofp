...
\begin{frame}{From Kleisli category back to \texttt{\textcolor{blue}{\footnotesize{}flatMap}} }
...
$\text{ftn}\equiv\text{id}^{S^{S^{A}}\Rightarrow S^{S^{A}}}\diamond\text{id}^{S^{A}\Rightarrow S^{A}}$xt{id}^{S^{A}\Rightarrow S^{A}}$}{\small \par}
