...
\hline 
\textbf{Category} &
\textbf{Type }$A\rightsquigarrow B$xtbf{Type }$A\rightsquigarrow B$ & \textbf{Identity} & \textbf{Composition}\tabularnewline
\hline 
\hline 
...
Mathematicians have generalized this to 

\textbf{Category}: a general class of ``twisted functions'' called
\textbf{morphisms} $A\rightsquigarrow B$
\begin{itemize}
\item For any type $A$, there exists the \textbf{identity} morphism, $\text{id}$,
of type $A\rightsquigarrow A$
\item For any morphisms $f^{A\rightsquigarrow B}$ and $g^{B\rightsquigarrow C}$
there exists the composition morphism $f\circ g$ of type $A\rightsquigarrow C$
\item Composition respects identity: $\text{id}\circ f=f$ and $f\circ\text{id}=f$
\item Composition is associative: $\left(f\circ g\right)\circ h=f\circ\left(g\circ h\right)$
\end{itemize}
General \textbf{functor}: a ``lifting'' from one category to another
\begin{itemize}
\item Lifting must \texttt{\textcolor{blue}{\footnotesize{}fmap}} each morphism
...
The monad laws formulated via Kleisli functions are category axioms
...
in terms of the Kleisli composition $\diamond$:
\begin{itemize}
...
\item Define \texttt{\textcolor{blue}{\footnotesize{}fmap}} through Kleisli:
$\text{fmap}\,f\equiv\text{id}^{S^{A}\Rightarrow S^{A}}\diamond\left(f\circ\text{pure}\right)$
...
\end{itemize}
\end{itemize}
With these laws, the monad laws follow from category axioms for Kleisli
\end{frame}
...
$M^{A}$ is an arbitrary monad) is a semimonad but not a monad.
...
using the known monad constructions (no need to check laws).
\end{enumerate}
\end{frame}
...
\item Implement a monad for $F^{A}\equiv A+\left(R\Rightarrow A\right)$,
where $R$ is a fixed type, and check that all the monad laws hold.
...
were defined as $0+\text{pure}_{G}$.
