...
on for categories and functors}
...
\begin{itemize}
\item A non-empty list $F^{A}\equiv A\times\text{List}^{A}$ is a semigroup
but not a monoid.
...
is impossible.
\item Any exponential-polynomial contrafunctor $H^{A}$ is equivalent to
$H^{A}\equiv K^{A}\Rightarrow Z$ for some exp-poly functor $K^{A}$
and some fixed type $Z$. So, construction 9 can be reformulated as
$F^{A}\equiv\left(K^{A}\Rightarrow Z\right)\Rightarrow A$ being a
monad for any functor $K^{A}$ and any fixed type $Z$.
\item When natural transformations are combined in any way, the result is
another natural transformation. For example, given functors $C$,
$F$, $G$ and natural transformations $C^{A}\Rightarrow F^{A}$ and
$C^{A}\Rightarrow G^{A}$ and taking the product, we get natural transformation
$C^{A}\Rightarrow F^{A}\times G^{A}$.
\end{itemize}
\end{frame}
