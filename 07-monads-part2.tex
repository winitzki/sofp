...
Kleisli: $\text{fmap}\,g^{A\Rightarrow B}\equiv\text{id}^{S^{A}\Rightarrow S^{A}}\diamond\left(g\circ\text{pure}\right)$d\left(g\circ\text{pure}\right)$
\end{itemize}
\end{itemize}
...
\item Show that $P^{A}\equiv Z+W\times A$ is a monad if $W$ is a monoid.
...
\item Check the identity laws for construction 5, $F^{A}\equiv A+G^{A}$,
given that $G^{A}$ is a monad. Show that the identity laws would
fail if $\text{pure}_{F}$ were defined as \texttt{\textcolor{blue}{\footnotesize{}a
$\Rightarrow$ Right(Monad{[}G{]}.pure(a))}} instead of as \texttt{\textcolor{blue}{\footnotesize{}Left(a)}}.
