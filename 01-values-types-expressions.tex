...
\item Find the median element in a sorted array of integers.
\end{itemize}
...
inner sequence.
\item An integer $n$ is called ``two-factor'' if it is divisible by only
two different integers $j$ such that $2\leq j<n$. Compute the set
of all ``two-factor'' integers $n$ among $n\in[1,...,100]$ .
\item Given a function $f$ of type \texttt{\textcolor{blue}{\footnotesize{}Int
=> Boolean}}, an integer $n$ is called ``two-$f$'' if there are
only two different integers $j\in[1,...,n]$ such that $f(j)$ returns
\texttt{\textcolor{blue}{\footnotesize{}true}}. Define a function
that takes $f$ as an argument and returns a sequence of all ``two-$f$''
integers among $n\in[1,...,100]$. What is the type of that function?
