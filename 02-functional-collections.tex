...
\item to remedy this: use \textbf{tail recursion}
\end{itemize}
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}@tailrec~def~fromDigits(...)~=~if~(...)~0~else~fromDigits(...)}{\footnotesize \par}

\textcolor{blue}{\footnotesize{}@tailrec~def~fromDigits(...)~=~if~(...)~0~else~\{}{\footnotesize \par}
...
\begin{itemize}
\item add another argument that accumulates the correct result
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{enumerate}
\item Compute the sum of squared digits of a given integer; e.g., \texttt{\textcolor{blue}{\footnotesize{}dsumsq(123)
= 14}}{\footnotesize \par}
\end{enumerate}
\end{frame}
