...
is called the \textbf{forward composition} of the two functions \lstinline!f!
...
of two functions. Mathematically, this operation is denoted by $\bef$
(pronounced ``before'') and can be defined as
\begin{equation}
...
or else the composition is undefined.
...
side of the law is $f$. We add type annotations to the code as \emph{superscripts},
\[
...
The colon symbol ($:$) in the superscript $x^{:A}$ means the type
annotation, as in Scala code \lstinline!x:A!. A superscript without
the colon symbol, such as $\text{id}^{A}$, means a type parameter,
as in Scala code \lstinline!id[A]!. Since the function \lstinline!id[A]!
...
operation \lstinline!andThen! (denoted by the symbol $\bef$) is
fully parametric. Given a fully parametric function, it is often possible
to derive the most general type signature that matches the body of
the function. The same method will also work for generalizing a given
function to a fully parametric form.
...
$A$, $B$, etc., we need to determine the most general type that
matches the function body. The function body is the expression $x\Rightarrow f(f(x))$.
...
We can now write the type signature of \lstinline!twice! as
\begin{lstlisting}
def twice[A](f: A => A): A => A = { x => f(f(x)) }
\end{lstlisting}
This fully parametric function has only one independent type parameter,
$A$. A mathematical notation for this function can be written as
\[
...
called \textbf{type inference}\index{type inference}. The type signature
$\left(A\Rightarrow A\right)\Rightarrow A\Rightarrow A$ has been
\textbf{infer}red from the code $f\Rightarrow x\Rightarrow f(f(x))$.
...
made clearer if we rename the arguments to eliminate shadowing of
names\index{shadowed name}. To avoid errors, we will rename arguments
one scope at a time:
\begin{align*}
...
type signature of $f$ must be $\text{Int}\Rightarrow A$, where $A$
is the (so far unknown) type of the value $f(2)$. The nameless function
$f\Rightarrow f(2)$ has the argument $f$ of type $\text{Int}\Rightarrow A$
and the result of type $A$, so the type of $p$ must be $\left(\text{Int}\Rightarrow A\right)\Rightarrow A$.
...
To perform the calculations faster, we will write code in a mathematical
...
instead of the Scala code \lstinline!((x:Int) => x + 10)!.
...
Consider the code $g\Rightarrow g(2)$. The body of this function
contains the sub-expression $g(2)$, which makes sense only if $g$
is itself a function with an integer argument. So, types will match
if $g$ has type $\text{Int}\Rightarrow\text{Int}$ or $\text{Int}\Rightarrow\text{String}$
or similar. The most general case is when $g$ has type $\text{Int}\Rightarrow A$,
where $A$ is an arbitrary type (i.e.~a type parameter).

Now we compute an expression that uses $\left(g\Rightarrow g(2)\right)$
as an argument:
\begin{align}
...
$\text{Int}\Rightarrow\text{Int}$. A suitable argument is a function
$x\Rightarrow x+4$, and we can apply the previous expression to that
function:
\begin{align*}
...
like this: We know that the function $f\Rightarrow p\Rightarrow f(p)$
...
$\text{Int}\Rightarrow A$ with a new type parameter $A$. It follows
that the function $g\Rightarrow g(2)$ has type $\left(\text{Int}\Rightarrow A\right)\Rightarrow A$.
This must also be the type of the argument $f$ in $f\Rightarrow p\Rightarrow f(p)$. 
...
argument has type $\text{Int}\Rightarrow A$. This will be the same
as the type of $p:\text{Int}\Rightarrow\text{Int}$ only if we set
...
\item Infer types from code of fully parametric functions.
\end{itemize}
...
We create an iterator that keeps applying the function $f$, and determine
when the condition holds with \lstinline!.find!:
\begin{lstlisting}
def converge[X](f: X => X, x0: X, cond: X => Boolean): X = 
...
if the condition never holds, the program will either run out of memory
(since \lstinline!Stream.iterate! keeps all computed values in memory)
or the user runs out of time.

An alternative implementation that operates in constant memory is
...
To test, let us compute an approximation to the square root of a given
number $q$ by Newton's method. The iteration function $f$ is 
\[
f(x)=\frac{x+q/x}{2}\quad.
\]
We iterate $f(x)$ starting with $x_{0}=q/2$ until we obtain a required
precision:
\begin{lstlisting}
...
are values, we can directly return new functions by writing a nameless
function expression:
\begin{lstlisting}
...
and \lstinline!z! because we already fully specified the type $\text{Int}\Rightarrow\text{Int}\Rightarrow\text{Int}$
of the value \lstinline!add_x_v!. 
...
$x$, and returns \lstinline!true! iff $f(x)$ is a prime. Use the
function \lstinline!is_prime! defined in Section~\ref{subsec:Nameless-functions}. 

\subparagraph{Solution}

First, we need to determine the required type signature of \lstinline!prime_f!.
The value $f(x)$ must have integer type, or else we cannot check
...
Let us now implement the same function as a \lstinline!val!. We have
to rewrite the type signature as
\begin{lstlisting}
...
\lstinline!f! and \lstinline!is_prime!, so we can rewrite it as
\begin{lstlisting}
...
value must be $g(x)$. Infer the most general type of this function.

\subparagraph{Solution}

We begin by assuming that $x$ has type $A$, where $A$ is a type
parameter. Then predicate $p$ must have type \lstinline!A => Boolean!.
Since we are returning $f(x)$ or $g(x)$, the type $A$ must also
be the argument type of both $f$ and $g$. So we may assume $f^{:A\Rightarrow B}$
and $g^{:A\Rightarrow C}$. The type signature of the function is
(so far)
\[
...
Now let us infer the most general type signature for this code. What
could be the return type of \lstinline!choice(x,p,f,g)!? Since $p$
is an arbitrary predicate, the value $p(x)$ will be sometimes \lstinline!true!
and sometimes \lstinline!false!. If $p(x)$ returns \lstinline!true!,
the function \lstinline!choice! returns $f(x)$, which is of type
$B$. Otherwise, \lstinline!choice! returns $g(x)$, which is of
type $C$. However, the type signature of \lstinline!choice! must
be fixed in advance (at compile time) and cannot depend on values
of $p(x)$ passed to the function at run time. So, the types of $f(x)$
and the type of $g(x)$ must be the same, $B=C$. The type signature
of \lstinline!choice! will thus have only two type parameters, $A$
and $B$:
\begin{lstlisting}
...
is $A$, must match the type of \lstinline!q[C, D]!, so we must set
\[
...
\text{where}\quad & A\triangleq\left(C\Rightarrow\left(C\Rightarrow D\right)\Rightarrow D\right)\quad.
\end{align*}
...
\text{where}\quad & A\triangleq\left(C\Rightarrow\left(C\Rightarrow D\right)\Rightarrow D\right)\\
...
For the function \lstinline!id! defined above, what is \lstinline!id(id)!
and what is its type? Same question for \lstinline!id(const)!. Are
\lstinline!id(id)(id)! or \lstinline!id(id(id))! well-typed? What
are they simplified to?

\subsubsection{Exercise \label{subsec:Exercise-hof-simple-2}\ref{subsec:Exercise-hof-simple-2}}

For the function \lstinline!const! defined above, what is \lstinline!const(const)!
simplified to, and what is its most general type?
...
what does \lstinline!twice(twice(twice)))! do? Write out the type
parameters. Test your answer on an example.
...
except it should apply a given function $3$ times. What does \lstinline!thrice(thrice(thrice)))!
do?
...
except it should apply a given function $n$ times.
...
Implement a function of type \texttt{}\lstinline!Int => List[List[Int]] => List[List[Int]]!
...
Implement a function \lstinline!curry2! that takes an uncurried function
...
\end{lstlisting}
The function \texttt{}\lstinline!f3! has type signature \texttt{$(\text{Int}\Rightarrow\text{Int})\Rightarrow\text{Int}$}
and is a higher-order function of order $2$.
...
has an argument of a function type, rather than a return value of
a function type. Converting to an uncurried form cannot eliminate
...
is a syntax where \lstinline!f x! means the same as \lstinline!f(x)!.
...
in Unix shell commands, for example \lstinline!cp file1 file2!, as
well as in the programming language Tcl/Tk. In LISP and Scheme, each
function application is enclosed in parentheses but the arguments
are separated by spaces, for example \lstinline!(+ 1 2 3)!.}
...
arguments. (Consider e.g.~the expression \lstinline!f2 (g 10) (h 20) + 30!.)
...
have values \lstinline!x: A!, \lstinline!y: B!, and \lstinline!f:(A, B)=>C!.
Since the type \lstinline!C! is arbitrary, a fully parametric function
cannot produce any values of type \lstinline!C! from scratch. The
only way of producing any values of type \lstinline!C! is by applying
the function \lstinline!f! to arguments of types \lstinline!A! and
\lstinline!B!. Since the types \lstinline!A! and \lstinline!B!
are arbitrary, we cannot obtain any values of these types other than
\lstinline!x: A! and \lstinline!y: B!. So, the only way of obtaining
a value of type \lstinline!C! is to compute \lstinline!f(x, y)!.
Thus, the body of \lstinline!pa! must be
\begin{lstlisting}
