...
\item Implement scan on a list, as \texttt{\textcolor{blue}{\footnotesize{}traverse}}
with a state monad
...
implementation of $\text{seq}:L^{F^{A}}\Rightarrow F^{L^{A}}$ that
always returns an empty option when $F^{A}\equiv1+A$.}{\footnotesize\par}
\item {\footnotesize{}Show that any profunctor $P^{A}$ admits a method
$A\Rightarrow P^{B}\Rightarrow P^{A\times B}$.}{\footnotesize\par}
