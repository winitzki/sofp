...
\begin{frame}{Recipe for encoding free type constructor typeclasses}
\begin{itemize}
\item To build a ``free typeclass $C$ over $F^{\bullet}$'' as a type
constructor $P^{\bullet}$ 
\begin{itemize}
...
\end{itemize}
What typeclasses cannot be encoded in this way?
\begin{itemize}
...
\begin{itemize}
\item Free pointed functor over a functor $F^{\bullet}$ is just $A+F^{A}$
...
\begin{frame}{Worked examples VI: free filterable functor}
\begin{itemize}
...
\item Reduced encoding: $\text{FreeFi}^{F^{\bullet},A}\equiv\exists Z.F^{Z}\times\left(Z\Rightarrow1+A\right)$quiv\exists Z.F^{Z}\times\left(Z\Rightarrow1+A\right)$,
non-recursive
...
\end{align*}
\item {\footnotesize{}\vspace{-0.15cm}}Can recover \texttt{\textcolor{blue}{\footnotesize{}map}}
