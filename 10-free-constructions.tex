...
is not equivalent to let}$ over a monad $M^{\bullet}$
...
\begin{itemize}
\item \vspace{-0.15cm}To combine different free typeclasses $C_{1}$ and
$C_{2}$:
\item Option 1: functor composition, $\text{FreeC}_{12}^{Z}\equiv\text{FreeC}_{1}^{\text{FreeC}_{2}^{Z}}$
\begin{itemize}
\item Order of composition matters!
\item Operations need to be lifted
\end{itemize}
\item Option 2: use disjunction of induction constructors, $S^{X}\equiv S_{1}^{X}+S_{2}^{X}$,
and build the free typeclass instance using $S^{\bullet}$
\end{itemize}
\end{frame}
