...
$A+F^{A}$
\item If $F^{\bullet}$llet}$ is simplified:
$A+F^{A}$
...
\end{itemize}
%
\end{minipage}\hfill{}{\footnotesize{}}%
\begin{minipage}[t]{0.25\columnwidth}%
{\footnotesize{}
\[
\xymatrix{\xyScaleY{1.5pc}\xyScaleX{3pc} & \text{FreeC}^{Z}\ar[ld]\sb(0.6){\text{run}^{P}(g^{:Z\Rightarrow P})}\ar[d]\sp(0.45){\text{fmap}\,g^{:Z\Rightarrow P}}\\
P\ar[r]\sp(0.5){\text{wrap}} & \text{FreeC}^{P}
}
\]
}%
\end{minipage}\hfill{}
...
Law 2: for all $g^{:Z\Rightarrow P}$, {\footnotesize{}$\text{wrap}\circ\text{run}(g)=\text{fmap}_{\text{FreeC}}g$}.
\begin{itemize}
