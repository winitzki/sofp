...
encoding}'' (of the free semigroup)
...
\begin{frame}{Recipes for encoding free typeclasses}
\begin{itemize}
\item {\footnotesize{}\vspace{-0.15cm}}To build a free typeclass $C$ over
$F^{\bullet}$, as a type constructor $P^{\bullet}$ 
\begin{itemize}
...
\begin{itemize}
\item \textbf{Inductive type class} defined via an induction constructor
$S^{P^{\bullet}}\leadsto P^{\bullet}$
\end{itemize}
\item The tree encoded $\text{FC}^{A}$ is a disjunction defined recursively
by{\footnotesize{}\vspace{-0.15cm}
\[
...
\end{itemize}
\begin{lyxcode}
{\footnotesize{}\vspace{-0.15cm}}\textcolor{blue}{\footnotesize{}sealed~trait~FC{[}A{]};~case~class~Wrap{[}A{]}(fa:~F{[}A{]})~extends~FC{[}A{]}}{\footnotesize\par}
...
\end{lyxcode}
\begin{itemize}
\item {\footnotesize{}\vspace{-0.15cm}}Any type parameters within $Q_{i}$
...
\end{itemize}
{\footnotesize{}\vspace{-0.1cm}}What typeclasses \emph{cannot} be
encoded like this (or have no ``free'' instances)?
\begin{itemize}
...
by $\exists Z.F^{Z}\times...$F^{\bullet},Z}\times...$
...
\begin{frame}{Combining free monad and free applicative functor}
\begin{itemize}
\item {\footnotesize{}\vspace{-0.15cm}}Met
\end{itemize}
\end{frame}
...
\item For any $P^{:C}$ we can implement the functions 
\begin{align*}
\text{run} & :\left(Z\Rightarrow P\right)\Rightarrow\text{FreeC}^{Z}\Rightarrow P\\
...
\begin{itemize}
\item Can use disjunction but then need to inject parts, cumbersome
\item Church encoding makes this easier
\end{itemize}
\end{frame}
