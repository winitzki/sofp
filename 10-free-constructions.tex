...
= f(c2a(ch)) = f(ch.run(a$\Rightarrow$c2a(ch)).run(f)
...
\begin{itemize}
\item Tree encoding: $\text{FreeF}^{F^{\bullet},A}\equiv F^{A}+\exists B.\text{FreeF}^{F^{\bullet},B}\times\left(B\Rightarrow A\right)$
\begin{itemize}
\item The existential quantifier is represented by an extra type parameter
\item It is not $\forall B$ because a specific $B$ is used when building
up a value
\end{itemize}
\item Reduced encoding: $\text{FreeF}^{F^{\bullet},A}\equiv\exists B.F^{B}\times\left(B\Rightarrow A\right)$
\begin{itemize}
\item Substituted $F^{B}$ instead of $\text{FreeF}^{F^{\bullet},B}$ and
eliminated one case
\item The reduced encoding is non-recursive
\item Requires a proof that this encoding is equivalent to the tree encoding
\begin{itemize}
\item Can show that this is equivalent to $F^{A}$ if $F^{\bullet}$ is
already a functor
\end{itemize}
\end{itemize}
\item Church encoding: $\text{FreeF}^{F^{\bullet},A}\equiv\forall P^{\bullet}.\left(\forall C.\big(F^{C}+\exists B.P^{B}\times\left(B\Rightarrow C\right)\big)\leadsto P^{C}\right)\Rightarrow P^{A}$
\end{itemize}
\end{frame}
