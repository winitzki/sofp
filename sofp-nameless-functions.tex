...
\prod_{k=1}^{10}k\quad.
\]
We can then write Scala code in a way that resembles this formula:\inputencoding{latin9}
\begin{lstlisting}
...
To define a name for this value, we use the ``\inputencoding{latin9}\lstinline!val!\inputencoding{utf8}''
syntax:\inputencoding{latin9}
\begin{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}
The code \texttt{}\inputencoding{latin9}\lstinline!(1 to 10).product!\inputencoding{utf8}
is an \textbf{expression}\index{expression}, which means that (1)
...
expression. For example, we could write\inputencoding{latin9}
\begin{lstlisting}
...
\]
The corresponding Scala code is\inputencoding{latin9}
\begin{lstlisting}
def f(n:Int) = (1 to n).product
\end{lstlisting}
\inputencoding{utf8}
In Scala's \texttt{}\inputencoding{latin9}\lstinline!def!\inputencoding{utf8}
syntax, we need to specify the type of a function's argument; in this
case, we write \inputencoding{latin9}\lstinline!n:Int!\inputencoding{utf8}.
...
the type \texttt{}\inputencoding{latin9}\lstinline!Int!\inputencoding{utf8}
in the Scala code. So, the argument's type in the code specifies the
\emph{domain} of a function.

Having defined the function \inputencoding{latin9}\lstinline!f!\inputencoding{utf8},
we can now apply it to an integer argument: \inputencoding{latin9}
\begin{lstlisting}
scala> f(10)
res6: Int = 3628800
\end{lstlisting}
\inputencoding{utf8}It is an error to apply \inputencoding{latin9}\lstinline!f!\inputencoding{utf8}
to a non-integer value, e.g.\ to a string:\inputencoding{latin9}
\begin{lstlisting}
scala> f("abc")
...
this mathematical formula is\inputencoding{latin9}
\begin{lstlisting}
(n: Int) => (1 to n).product
\end{lstlisting}
\inputencoding{utf8}This expression shows Scala's syntax for a \textbf{nameless}\index{nameless function}
function. Here, \inputencoding{latin9}
\begin{lstlisting}
n: Int
\end{lstlisting}
\inputencoding{utf8}is the function's \textbf{argument}, while\inputencoding{latin9}
\begin{lstlisting}
(1 to n).product
\end{lstlisting}
\inputencoding{utf8}is the function's \textbf{body}. The arrow symbol \inputencoding{latin9}\lstinline!=>!\inputencoding{utf8}
separates the argument from the body.\footnote{In Scala, the two ASCII characters \inputencoding{latin9}\lstinline!=>!\inputencoding{utf8}
...
\inputencoding{latin9}\lstinline!->!\inputencoding{utf8} or the Unicode
...
which means that we can also define a Scala value as\inputencoding{latin9}
\begin{lstlisting}
scala> val fac = (n: Int) => (1 to n).product
fac: Int => Int = <function1>
\end{lstlisting}
\inputencoding{utf8}We see that the value \inputencoding{latin9}\lstinline!fac!\inputencoding{utf8}
has the type \inputencoding{latin9}\lstinline!Int => Int!\inputencoding{utf8},
which means that the function takes an integer (\inputencoding{latin9}\lstinline!Int!\inputencoding{utf8})
argument and returns an integer result value. What is the value of
the function \inputencoding{latin9}\lstinline!fac!\inputencoding{utf8}
\emph{itself}? As we have just seen, the Scala interpreter prints
\inputencoding{latin9}\lstinline!<function1>!\inputencoding{utf8}
as the ``value'' of \inputencoding{latin9}\lstinline!fac!\inputencoding{utf8}.
An alternative Scala interpreter\footnote{\texttt{\href{https://ammonite.io/}{https://ammonite.io/}}}
called \texttt{ammonite} prints something like this,\inputencoding{latin9}
\begin{lstlisting}
scala@ val fac = (n: Int) => (1 to n).product
fac: Int => Int = ammonite.$sess.cmd0$$$Lambda$1675/2107543287@1e44b638
\end{lstlisting}
\inputencoding{utf8}This seems to indicate some identifying number, or perhaps a memory
location.
...
Once defined, a function can be applied to an argument like this:\inputencoding{latin9}
\begin{lstlisting}
scala> fac(10)
res1: Int = 3628800
\end{lstlisting}
...
apply a nameless factorial function to an integer argument \inputencoding{latin9}\lstinline!10!\inputencoding{utf8}
instead of writing \inputencoding{latin9}\lstinline!fac(10)!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
scala> ((n: Int) => (1 to n).product)(10)
res2: Int = 3628800
\end{lstlisting}
\inputencoding{utf8}One would not often write code like this because there is no advantage
in creating a nameless function and then applying it right away to
an argument. This is so because we can evaluate the expression\inputencoding{latin9}
\begin{lstlisting}
((n: Int) => (1 to n).product)(10)
\end{lstlisting}
\inputencoding{utf8}by substituting \inputencoding{latin9}\lstinline!10!\inputencoding{utf8}
instead of \inputencoding{latin9}\lstinline!n!\inputencoding{utf8}
in the function body, which gives us\inputencoding{latin9}
\begin{lstlisting}
(1 to 10).product
\end{lstlisting}
\inputencoding{utf8}
If a nameless function uses the argument several times, for example\inputencoding{latin9}
\begin{lstlisting}
((n: Int) => n*n*n + n*n)(12345)
\end{lstlisting}
\inputencoding{utf8}it is still better to substitute the argument and to eliminate the
nameless function. We could have written\inputencoding{latin9}
\begin{lstlisting}
12345*12345*12345 + 12345*12345
\end{lstlisting}
\inputencoding{utf8}but, of course, we want to avoid repeating the value \inputencoding{latin9}\lstinline!12345!\inputencoding{utf8}.
To achieve that, we may define \texttt{}\inputencoding{latin9}\lstinline!n!\inputencoding{utf8}
as a value in an \textbf{expression block\index{expression block}}
like this: \inputencoding{latin9}
\begin{lstlisting}
scala> { val n = 12345; n*n*n + n*n }
res3: Int = 322687002
\end{lstlisting}
\inputencoding{utf8}Defined in this way, the value \inputencoding{latin9}\lstinline!n!\inputencoding{utf8}
is visible only within the expression block. Outside the block, another
value named \inputencoding{latin9}\lstinline!n!\inputencoding{utf8}
could be defined independently of this \inputencoding{latin9}\lstinline!n!\inputencoding{utf8}.
For this reason, the definition of \inputencoding{latin9}\lstinline!n!\inputencoding{utf8}
is called a \textbf{locally}\index{local scope} \textbf{scoped} definition.
...
is translated into Scala code as\inputencoding{latin9}
\begin{lstlisting}
def is_prime(n: Int) = (2 to n-1).forall(k => n % k != 0)
\end{lstlisting}
\inputencoding{utf8}In this code, the two parts of the mathematical formula are implemented
in a way that is closely similar to the mathematical notation, except
for the arrow after $k$.

We can now apply the function \inputencoding{latin9}\lstinline!is_prime!\inputencoding{utf8}
to some integer values:\inputencoding{latin9}
\begin{lstlisting}
scala> is_prime(12)
res3: Boolean = false
...
res4: Boolean = true
\end{lstlisting}
\inputencoding{utf8}As we can see from the output above, the function \inputencoding{latin9}\lstinline!is_prime!\inputencoding{utf8}
returns a value of type \inputencoding{latin9}\lstinline!Boolean!\inputencoding{utf8}.
Therefore, the function \inputencoding{latin9}\lstinline!is_prime!\inputencoding{utf8}
has type \inputencoding{latin9}\lstinline!Int => Boolean!\inputencoding{utf8}.

A function that returns a \inputencoding{latin9}\lstinline!Boolean!\inputencoding{utf8}
value is called a \textbf{predicate}\index{predicate}.

In Scala, it is optional – but strongly recommended – to specify the
return type of named functions. The required syntax looks like this,\inputencoding{latin9}
\begin{lstlisting}
def is_prime(n: Int): Boolean =
  (2 to n-1).forall(k => n % k != 0)
\end{lstlisting}
\inputencoding{utf8}
However, we do not need to specify the type \texttt{}\inputencoding{latin9}\lstinline!Int!\inputencoding{utf8}
for the argument \texttt{}\inputencoding{latin9}\lstinline!k!\inputencoding{utf8}
of the nameless function \inputencoding{latin9}\lstinline*k => n % k != 0*\inputencoding{utf8}.
This is because the Scala compiler knows that \texttt{}\inputencoding{latin9}\lstinline!k!\inputencoding{utf8}
is going to iterate over the \emph{integer} elements of the range
\inputencoding{latin9}\lstinline!(2 to n-1)!\inputencoding{utf8},
which effectively forces \texttt{}\inputencoding{latin9}\lstinline!k!\inputencoding{utf8}
to be of type \inputencoding{latin9}\lstinline!Int!\inputencoding{utf8}.

\subsection{Nameless functions and bound variables}

The code for \texttt{}\inputencoding{latin9}\lstinline!is_prime!\inputencoding{utf8}
differs from the mathematical formula\ (\ref{eq:is_prime_def}) in
two ways.

One difference is that the interval $\left[2,n-1\right]$ is in front
of \inputencoding{latin9}\lstinline!forall!\inputencoding{utf8}.
To understand this, look at the ways Scala allows programmers to define
syntax.

The Scala syntax such as \texttt{}\inputencoding{latin9}\lstinline!(2 to n-1).forall(k => ...)!\inputencoding{utf8}
means to apply a function called \texttt{}\inputencoding{latin9}\lstinline!forall!\inputencoding{utf8}
to \emph{two} arguments: the first argument is the range \texttt{}\inputencoding{latin9}\lstinline!(2 to n-1)!\inputencoding{utf8}\texttt{,}
and the second argument is the nameless function \inputencoding{latin9}\lstinline!(k => ...)!\inputencoding{utf8}.
In Scala, the \textbf{infix} syntax\index{infix syntax} \inputencoding{latin9}\lstinline!x.f(z)!\inputencoding{utf8},
or equivalently \inputencoding{latin9}\lstinline!x f z!\inputencoding{utf8},
means that a function \texttt{}\inputencoding{latin9}\lstinline!f!\inputencoding{utf8}
is applied to its \emph{two} arguments, \texttt{}\inputencoding{latin9}\lstinline!x!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!z!\inputencoding{utf8}. In the
ordinary mathematical notation, this would be $f(x,z)$. Infix notation
is often easier to read and is also widely used in mathematics, for
instance when we write $x+y$ rather than something like $plus(x,y)$.

A single-argument function could be also defined with infix notation,
and then the syntax is \inputencoding{latin9}\lstinline!x.f!\inputencoding{utf8},
as in the expression \texttt{}\inputencoding{latin9}\lstinline!(1 to n).product!\inputencoding{utf8}
we have seen before.

The infix methods \texttt{}\inputencoding{latin9}\lstinline!.product!\inputencoding{utf8}
and \texttt{}\inputencoding{latin9}\lstinline!.forall!\inputencoding{utf8}
...
a function \texttt{}\inputencoding{latin9}\lstinline!for_all!\inputencoding{utf8}
with two arguments and write code like this,

\inputencoding{latin9}\begin{lstlisting}
for_all(2 to n-1, k => n % k != 0)
\end{lstlisting}
\inputencoding{utf8}This would have brought the syntax somewhat closer to the formula\ (\ref{eq:is_prime_def}). 

However, there still remains the second difference: The symbol $k$
is used as an \emph{argument} of a nameless function \texttt{}\inputencoding{latin9}\lstinline*(k => n % k != 0)*\inputencoding{utf8}
...
\emph{of} $k$ that returns a \inputencoding{latin9}\lstinline!Boolean!\inputencoding{utf8}
...
all result values be \inputencoding{latin9}\lstinline!true!\inputencoding{utf8}:
...
\inputencoding{latin9}\lstinline!k!\inputencoding{utf8} of the nameless
Scala function \inputencoding{latin9}\lstinline*k => n % k != 0*\inputencoding{utf8}
is defined only within that function's body and cannot be used in
any code outside the expression \inputencoding{latin9}\lstinline*n % k != 0*\inputencoding{utf8}.
...
\]
or in Scala code, \inputencoding{latin9}
\begin{lstlisting}
(2 to n-1).forall(z => n % z != 0)
\end{lstlisting}
\inputencoding{utf8}In the nameless function \inputencoding{latin9}\lstinline*k => n % k != 0*\inputencoding{utf8},
the argument \inputencoding{latin9}\lstinline!k!\inputencoding{utf8}
may be renamed to \inputencoding{latin9}\lstinline!z!\inputencoding{utf8}
...
\inputencoding{latin9}\lstinline!k!\inputencoding{utf8} to \inputencoding{latin9}\lstinline!z!\inputencoding{utf8}.
But the value \inputencoding{latin9}\lstinline!n!\inputencoding{utf8}
...
\inputencoding{latin9}\lstinline!n!\inputencoding{utf8} in the sub-expression
\inputencoding{latin9}\lstinline*k => n % k != 0*\inputencoding{utf8},
we would also need to change every place in the code that defines
and uses \inputencoding{latin9}\lstinline!n!\inputencoding{utf8}
\emph{outside} that expression, or else the program would become incorrect.
...
(1 to n).forall(k => p(k))
\end{lstlisting}
\inputencoding{utf8}At this point we can apply the following simplification trick to this
...
like \inputencoding{latin9}\lstinline!f(x)!\inputencoding{utf8} but
...
or an implicit conversion for its argument \inputencoding{latin9}\lstinline!x!\inputencoding{utf8}.
In those cases, replacing the code \inputencoding{latin9}\lstinline!x => f(x)!\inputencoding{utf8}
by just \inputencoding{latin9}\lstinline!f!\inputencoding{utf8} may
...
\end{cases}\quad.
\end{align*}
Here we defined a helper function \texttt{}\inputencoding{latin9}\lstinline!is_even!\inputencoding{utf8}
in order to write more easily a formula for \inputencoding{latin9}\lstinline!count_even!\inputencoding{utf8}.
...
function \inputencoding{latin9}\lstinline!is_even!\inputencoding{utf8};
...
the list $L$ using the data type \inputencoding{latin9}\lstinline!List[Int]!\inputencoding{utf8}
from the Scala standard library.

To compute $\sum_{k\in L}\text{is\_even}\left(k\right)$, we must
apply the function \texttt{}\inputencoding{latin9}\lstinline!is_even!\inputencoding{utf8}
...
This can be done with the functions \texttt{}\inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8}, defined
in the Scala standard library as infix methods for the data type \inputencoding{latin9}\lstinline!List!\inputencoding{utf8}.

The method \texttt{}\inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8}
is similar to \inputencoding{latin9}\lstinline!.product!\inputencoding{utf8}
and is defined for any \inputencoding{latin9}\lstinline!List!\inputencoding{utf8}
of numerical types (\inputencoding{latin9}\lstinline!Int!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!Float!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!Double!\inputencoding{utf8},
etc.). It computes the sum of all numbers in the list:\inputencoding{latin9}
\begin{lstlisting}
scala> List(1, 2, 3).sum
res0: Int = 6
\end{lstlisting}
\inputencoding{utf8}
The method \texttt{}\inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}
...
\end{lstlisting}
\inputencoding{utf8}
In this example, the argument of \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}
is the nameless function $x\Rightarrow x^{2}+100x$. This function
is repeatedly applied by \texttt{}\inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}
...
give it a name, and then pass it as the argument to \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}Usually, short and simple functions are defined inline, while longer
functions are given a name and defined separately.

An infix method, such as \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8},
can be also used with a ``dotless'' syntax:\inputencoding{latin9}
\begin{lstlisting}
scala> List(1, 2, 3) map func1
res3: List[Int] = List(101, 204, 309)
\end{lstlisting}
\inputencoding{utf8}
If the transforming function \inputencoding{latin9}\lstinline!func1!\inputencoding{utf8}
...
We can now combine the methods \texttt{}\inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}
and \texttt{}\inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8}
to define \inputencoding{latin9}\lstinline!count_even!\inputencoding{utf8}:
...
\inputencoding{latin9}\lstinline!is_even!\inputencoding{utf8}:
...
operations. For instance \inputencoding{latin9}\lstinline!s.map(...).sum!\inputencoding{utf8}
means first apply \inputencoding{latin9}\lstinline!s.map(...)!\inputencoding{utf8},
which returns a \emph{new} list, and then apply \texttt{}\inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8}
...
the compiler reads the entire code at once.)\inputencoding{latin9}
\begin{lstlisting}
...
res1: Int = 5
\end{lstlisting}
\inputencoding{utf8}Note that the Scala interpreter prints the types differently for functions
defined using \inputencoding{latin9}\lstinline!def!\inputencoding{utf8}.
It prints \inputencoding{latin9}\lstinline!(s: List[Int])Int!\inputencoding{utf8}
for the function type that one would normally write as \inputencoding{latin9}\lstinline!List[Int] => Int!\inputencoding{utf8}.

\section{Filtering and truncating sequences}

In addition to the methods \inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.product!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8},
\texttt{}\inputencoding{latin9}\lstinline!.forall!\inputencoding{utf8}
...
\inputencoding{latin9}\lstinline!.max!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!.min!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.exists!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!.size!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}, and
\inputencoding{latin9}\lstinline!.takeWhile!\inputencoding{utf8}. 

The methods \inputencoding{latin9}\lstinline!.max!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.min!\inputencoding{utf8}, and \texttt{}\inputencoding{latin9}\lstinline!.size!\inputencoding{utf8}
are self-explanatory:\inputencoding{latin9}
\begin{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}
The methods \inputencoding{latin9}\lstinline!.forall!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.exists!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8},
and \texttt{}\inputencoding{latin9}\lstinline!.takeWhile!\inputencoding{utf8}
require a predicate as an argument. The \texttt{}\inputencoding{latin9}\lstinline!.forall!\inputencoding{utf8}
method returns \texttt{}\inputencoding{latin9}\lstinline!true!\inputencoding{utf8}
iff the predicate is true on all values in the list; the \texttt{}\inputencoding{latin9}\lstinline!.exists!\inputencoding{utf8}
method returns \texttt{}\inputencoding{latin9}\lstinline!true!\inputencoding{utf8}
iff the predicate holds (returns \inputencoding{latin9}\lstinline!true!\inputencoding{utf8})
...
The \inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}
method returns a \emph{new list} that contains only the values for
which the predicate returns \texttt{}\inputencoding{latin9}\lstinline!true!\inputencoding{utf8}:
...
\end{lstlisting}
\inputencoding{utf8}
The \texttt{}\inputencoding{latin9}\lstinline!.takeWhile!\inputencoding{utf8}
...
keeps being \inputencoding{latin9}\lstinline!true!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
scala> List(1, 2, 3, 4, 5).takeWhile(k => k % 3 != 0)
res6: List[Int] = List(1, 2)
\end{lstlisting}
\inputencoding{utf8}
In all these cases, the predicate's argument \inputencoding{latin9}\lstinline!k!\inputencoding{utf8}
...
\inputencoding{latin9}\lstinline!List[Int]!\inputencoding{utf8}),
therefore \texttt{}\inputencoding{latin9}\lstinline!k!\inputencoding{utf8}
must be of type \inputencoding{latin9}\lstinline!Int!\inputencoding{utf8}.

The methods \inputencoding{latin9}\lstinline!.max!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.min!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8},
and \texttt{}\inputencoding{latin9}\lstinline!.product!\inputencoding{utf8}
are defined on lists of \emph{numeric types}, such as \inputencoding{latin9}\lstinline!Int!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!Double!\inputencoding{utf8}, and
\inputencoding{latin9}\lstinline!Long!\inputencoding{utf8}. The other
...
of transformation functions are \texttt{}\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}. An
\textbf{aggregation\index{aggregation}} is a function from a list
of values to a \emph{single} value; examples of aggregation functions
are \texttt{}\inputencoding{latin9}\lstinline!.max!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8}.
...
Improve the code for \inputencoding{latin9}\lstinline!is_prime!\inputencoding{utf8}
...
to truncate the initial list when $k^{2}\leq n$ becomes false:\inputencoding{latin9}
\begin{lstlisting}
def is_prime(n: Int): Boolean =
  (2 to n-1)
    .takeWhile(k => k*k + 1 < n)
...
(1 to 10)
  ...
  .sum
\end{lstlisting}
\inputencoding{utf8}It is safe to compute $\sqrt{\cos k}$, because we have first filtered
the list by keeping only values $k$ for which $\cos k>0$:\inputencoding{latin9}
\begin{lstlisting}
...
Compute the average of a non-empty list of type \inputencoding{latin9}\lstinline!List[Double]!\inputencoding{utf8},
\[
...
We need to divide the sum by the length of the list:\inputencoding{latin9}
\begin{lstlisting}
...
We will define the helper function \inputencoding{latin9}\lstinline!wallis_frac(i)!\inputencoding{utf8}
that computes the $i^{\text{th}}$ fraction. The method \texttt{}\inputencoding{latin9}\lstinline!.toDouble!\inputencoding{utf8}
converts integers to \texttt{}\inputencoding{latin9}\lstinline!Double!\inputencoding{utf8}
...
\end{lstlisting}
\inputencoding{utf8}The limit of the Wallis product is $\frac{\pi}{2}$, so the cosine
of \inputencoding{latin9}\lstinline!wallis(n)!\inputencoding{utf8}
...
value of $n$, say $n=10^{5}$, and compare with the right-hand side:\inputencoding{latin9}
\begin{lstlisting}
...
\inputencoding{latin9}\lstinline!f: Int => Int!\inputencoding{utf8},
...
\end{lstlisting}
\inputencoding{utf8}Here is an example test for this function:\inputencoding{latin9}
\begin{lstlisting}
...
Given a list of lists, \inputencoding{latin9}\lstinline!s: List[List[Int]]!\inputencoding{utf8},
select the inner lists of size at least $3$. The result must be again
of type \inputencoding{latin9}\lstinline!List[List[Int]]!\inputencoding{utf8}. 

\subparagraph{Solution:}

To ``select the inner lists'' means to compute a \emph{new} list
containing only the desired inner lists. We use \texttt{}\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}
on the outer list \inputencoding{latin9}\lstinline!s!\inputencoding{utf8}.
The predicate for the filter is a function that takes an inner list
and returns \texttt{}\inputencoding{latin9}\lstinline!true!\inputencoding{utf8}
if the size of that list is at least $3$. Write the predicate as
a nameless function, \inputencoding{latin9}\lstinline!t => t.size >= 3!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}The predicate in the argument of \texttt{}\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}
is a nameless function \texttt{}\inputencoding{latin9}\lstinline!t => t.size >= 3!\inputencoding{utf8}
whose argument \texttt{}\inputencoding{latin9}\lstinline!t!\inputencoding{utf8}
is of type \inputencoding{latin9}\lstinline!List[Int]!\inputencoding{utf8}.
The Scala compiler deduces the type of \inputencoding{latin9}\lstinline!t!\inputencoding{utf8}
from the code; no other type would work with the way we use \inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}
...
\end{lstlisting}
\inputencoding{utf8}The argument of the outer \inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}
is a nameless function that itself uses another \inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}.
The inner expression\inputencoding{latin9}
\begin{lstlisting}
(1 to k).filter(j => j*j > 2*k).size >= 3
\end{lstlisting}
\inputencoding{utf8}computes the list of $j$'s that satisfy the condition $j^{2}>2k$,
...
\item Transform and aggregate data from lists using \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8},\textbf{
}\inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8}, and
...
Library functions we have seen so far, such as \texttt{}\inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}
...
and accumulate results. For instance, when computing \inputencoding{latin9}\lstinline!s.map(f)!\inputencoding{utf8},
...
containing \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!.takeWhile!\inputencoding{utf8},
...
\]
However, operations such as \texttt{}\inputencoding{latin9}\lstinline!.map!\inputencoding{utf8}
and \texttt{}\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}
...
already sufficient for a full-precision \inputencoding{latin9}\lstinline!Double!\inputencoding{utf8}
approximation of $\pi$.%
\begin{comment}
...
Using the function \inputencoding{latin9}\lstinline!is_prime!\inputencoding{utf8},
...
Define a function \texttt{}\inputencoding{latin9}\lstinline!add_20!\inputencoding{utf8}
of type \texttt{}\inputencoding{latin9}\lstinline!List[List[Int]] => List[List[Int]]!\inputencoding{utf8}
that adds $20$ to every element of every inner list. A sample test:\inputencoding{latin9}
\begin{lstlisting}
...
Given a function \inputencoding{latin9}\lstinline!f: Int => Boolean!\inputencoding{utf8},
an integer $n$ is called a ``$3$-$f$'' if there are only three
different integers $j\in[1,...,n]$ such that $f(j)$ returns \inputencoding{latin9}\lstinline!true!\inputencoding{utf8}.
...
Define a function \inputencoding{latin9}\lstinline!see100!\inputencoding{utf8}
of type \texttt{}\inputencoding{latin9}\lstinline!List[List[Int]] => List[List[Int]]!\inputencoding{utf8}
that selects only those inner lists whose largest value is at least
$100$. Test with:\inputencoding{latin9}
\begin{lstlisting}
...
Define a function of type \texttt{}\inputencoding{latin9}\lstinline!List[Double] => List[Double]!\inputencoding{utf8}
...
of functions (such as \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8},
\texttt{}\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8},
...
In Scala, function arguments are immutable within the function body:\inputencoding{latin9}
\begin{lstlisting}
...
is written with the ``\inputencoding{latin9}\lstinline!val!\inputencoding{utf8}''
syntax. Each variable defined using ``\inputencoding{latin9}\lstinline!val!\inputencoding{utf8}''
is a named constant, and its type and value are fixed at the time
of definition. Type annotations for ``\inputencoding{latin9}\lstinline!val!\inputencoding{utf8}''s
are optional in Scala: for instance we could write \inputencoding{latin9}
\begin{lstlisting}
val x: Int = 123
\end{lstlisting}
\inputencoding{utf8}or we could omit the type annotation \inputencoding{latin9}\lstinline!:Int!\inputencoding{utf8}
and write more concisely\inputencoding{latin9}
\begin{lstlisting}
val x = 123
\end{lstlisting}
\inputencoding{utf8}because it is clear that this \texttt{}\inputencoding{latin9}\lstinline!x!\inputencoding{utf8}
...
correctly and give an error message whenever wrong types are used:\inputencoding{latin9}
\begin{lstlisting}
...
\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}, and
\inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8}, which
...
to the library functions \inputencoding{latin9}\lstinline!.map!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!.filter!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!.sum!\inputencoding{utf8},
...
We could implement \texttt{}\inputencoding{latin9}\lstinline!summation(a,b,g)!\inputencoding{utf8}
...
\end{align*}
 A straightforward translation of this formula into Scala is\inputencoding{latin9}
\begin{lstlisting}
...
expressions such as \texttt{}\inputencoding{latin9}\lstinline!(x+123)*y/(2+x)!\inputencoding{utf8}.
Here, \texttt{}\inputencoding{latin9}\lstinline!x!\inputencoding{utf8}
and \texttt{}\inputencoding{latin9}\lstinline!y!\inputencoding{utf8}
are variables with names. Note, however, that the entire expression
does not need to have a name. Parts of that expression (such as \texttt{}\inputencoding{latin9}\lstinline!x+123!\inputencoding{utf8}
or \inputencoding{latin9}\lstinline!2+x!\inputencoding{utf8}) also
...
The code for \inputencoding{latin9}\lstinline!(x+123)*y/(2+x)!\inputencoding{utf8}
...
called a ``hashmap\index{hashmap}'') is declared in Scala as\inputencoding{latin9}
\begin{lstlisting}
Map("a" -> 1, "b" -> 2, "c" -> 3)
\end{lstlisting}
\inputencoding{utf8}This is a nameless expression representing a dictionary. Without this
...
by step with values:\inputencoding{latin9}
