...
of type \inputencoding{latin9}\lstinline!Either[Z, R => A]!\inputencoding{utf8}
from a value of type \inputencoding{latin9}\lstinline!R => Either[Z, A]!\inputencoding{utf8}''
and ``Is the type constructor \inputencoding{latin9}\lstinline!F[A] = Option[(A, A, A)]!\inputencoding{utf8}
...
important operations as \inputencoding{latin9}\lstinline!filter!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!zip!\inputencoding{utf8}, and \inputencoding{latin9}\lstinline!traverse!\inputencoding{utf8}
...
type \inputencoding{latin9}\lstinline!(A, Either[B, C])!\inputencoding{utf8}
is equivalent to \inputencoding{latin9}\lstinline!Either[(A, B), (A, C)]!\inputencoding{utf8} 
...
a useless recursive type definition in Scala is \inputencoding{latin9}\lstinline!case class Bad(x: Bad)!\inputencoding{utf8} 
\item deriving an implementation of a function from its type signature and
required laws; for example, deriving the \inputencoding{latin9}\lstinline!flatMap!\inputencoding{utf8}
method for the \inputencoding{latin9}\lstinline!Reader!\inputencoding{utf8}
monad from the type signature \inputencoding{latin9}
\begin{lstlisting}
def flatMap[Z, A, B](r: Z => A)(f: A => Z => B): Z => B
\end{lstlisting}
\inputencoding{utf8} and verifying that the monad laws hold
\item deciding whether a generic pure function with a given signature can
be implemented; for example, \inputencoding{latin9}\lstinline!def f[A, B]: (A => B) => A!\inputencoding{utf8}
cannot be implemented but \inputencoding{latin9}\lstinline!def g[A, B]: A => (B => A)!\inputencoding{utf8}
can be implemented 
\end{itemize}
...
On the other hand, books such as \href{https://underscore.io/books/scala-with-cats/}{“Scala with Cats”}
