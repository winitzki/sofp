...
unlike \lstinline!mrun!. So the non-degeneracy law is not an automatic
consequence of other laws.
\end{enumerate}
...
Since $\text{ftn}_{\text{Id}}=\text{id}$ and $f^{\uparrow\text{Id}}=f$
for any function $f$, we simplify both sides of the composition law
to the same expression $\text{pu}_{M}$:
\begin{align*}
...
\subsubsection{Statement \label{subsec:Statement-monadic-morphism-is-natural-transformation-1}\ref{subsec:Statement-monadic-morphism-is-natural-transformation-1}}

If $\phi:M^{\bullet}\leadsto N^{\bullet}$, equivalently written as
$\phi:M^{A}\Rightarrow N^{A}$, is a monadic morphism between monads
$M$ and $N$, then $\phi$ is also a natural transformation between
functors $M$ and $N$.
...
and $\chi:L^{\bullet}\leadsto N^{\bullet}$, the componentwise function
product $\phi\boxtimes\chi:K^{\bullet}\times L^{\bullet}\leadsto M^{\bullet}\times N^{\bullet}$
...
The code for $\sigma$ can be written as $\sigma=\Delta\bef\left(\nabla_{2}\boxtimes\nabla_{1}\right)$.
When $\sigma$ is written in this way, it follows that $\sigma$ is
a composition of $\Delta$, which is a monadic morphism by Statement~\ref{subsec:Statement-M-to-M-times-M-is-monadic-morphism},
and $\nabla_{2}\boxtimes\nabla_{1}$, which is a monadic morphism
by Statement~\ref{subsec:Statement-product-of-monadic-morphisms}.
A composition of monadic morphisms is a monadic morphism by Statement~\ref{subsec:Statement-monadic-morphism-composition},
so $\sigma$ is a monadic morphism.
...
to convert each layer of $L$ or $M$ to a layer of $T$, raising
...
\subsection{Monad transformers are not unique}

Suppose $T_{L}^{M}$ is a lawful monad transformer for the base monad
$L$ and a foreign monad $M$. Can we construct another monad that
still satisfies all transformer laws?

One possibility is to compose $T$ with the monad $M$. Define $U$
and $V$ by
\[
U^{A}\triangleq M^{T_{L}^{M,A}}\quad,\quad\quad V^{A}\triangleq T_{L}^{M,M^{A}}\quad.
\]
In a shorter notation, $U\triangleq M\circ T_{L}^{M}$ and $V\triangleq T_{L}^{M}\circ M$.
It turns out that both $U$ and $V$ are lawful monad transformers
for $L$.
...
\big(\text{flm}_{M}f^{:A\Rightarrow M^{B}}\big)^{\uparrow L} & =\text{pure}_{L}^{\uparrow T}\bef\text{flm}_{T}\left(f^{\uparrow L}\right)\quad.
\end{align*}


\subsection{Monad transformer identity law: Proof}

The identity law requires that $T_{L}^{M}\cong L$ if $M=\text{Id}$.
We will now prove this law, assuming that $\text{ftn}_{T}$ is defined
via \lstinline!swap! by Eq.~(\ref{eq:def-flatten-T-via-sw-and-flatten-L-M}).

Setting $M=\text{Id}$ in the composed-outside transformer $T_{L}^{M}=L\circ M$,
we find that the type signature of \lstinline!swap! becomes
\[
\text{sw}_{L,\text{Id}}:\text{Id}\circ L\leadsto L\circ\text{Id}\cong L\leadsto L\quad.
\]
It is reasonable to assume that a function with the type signature
$L^{A}\Rightarrow L^{A}$ is the identity function,
\[
\text{sw}_{L,\text{Id}}=\text{id}^{:L^{A}\Rightarrow L^{A}}\quad.
\]
 This assumption becomes the \textbf{identity law\index{identity law of swap}}
of \lstinline!swap! for composed-outside transformers $T_{L}^{M}=L\circ M$
where $M$ is the foreign monad. For composed-\emph{inside} transformers
$T_{L}^{M}=L\circ M$ where $L$ is the foreign monad, the \lstinline!swap!
identity law is 
\[
\text{sw}_{\text{Id},M}=\text{id}^{:M^{A}\Rightarrow M^{A}}\quad.
\]

For the monad transformer identity law, we need a monadic isomorphism
between the monads $T_{L}^{\text{Id}}$ and $L$. Since $T_{L}^{M}=L\circ M$,
the monad $T_{L}^{\text{Id}}=L\circ\text{Id}=L$ is the same type
constructor as $L$. So, the isomorphism between $T_{L}^{\text{Id}}$
and $L$ is simply the identity maps in both directions, $\text{id}:T_{L}^{\text{Id},A}\Rightarrow L^{A}$
and $\text{id}:L^{A}\Rightarrow T_{L}^{\text{Id},A}$. 

However, we still need to verify that the monad structure of $T_{L}^{\text{Id}}$
is the same as that of $L$; otherwise the identity map would not
be a \emph{monadic} isomorphism (i.e.~an isomorphism that preserves
...
$L$. If the monad's methods are the same functions, the identity
maps will satisfy the laws of the monadic morphism in both directions,
e.g.
\[
\text{pu}_{T}\bef\text{id}=\text{pu}_{L}\quad,\quad\quad\text{ftn}_{T}\bef\text{id}=\text{id}^{\uparrow T}\bef\text{id}\bef\text{ftn}_{L}\quad.
\]

To show that $\text{pu}_{L}=\text{pu}_{T}$, use Eq.~(\ref{eq:def-pure-T-via-pure-L-and-pure-M})
with $M=\text{Id}$:
\begin{align*}
 & \text{pu}_{T}\\
{\color{greenunder}\text{use Eq.~(\ref{eq:def-pure-T-via-pure-L-and-pure-M})}:}\quad & =\text{pu}_{\text{Id}}\bef\text{pu}_{L}=\text{id}\bef\text{pu}_{L}=\text{pu}_{L}\quad.
\end{align*}
...
the laws of the \lstinline!swap! function than to verify the monad
transformer laws directly, because the laws of \lstinline!swap! work
with a simpler type constructors than the monad transformer laws.
...
{\color{greenunder}\text{definition of }\text{sw}:}\quad & =\gunderline{a\Rightarrow}(x\Rightarrow\bbnum 0^{:P}+q\times x)^{\uparrow L}\gunderline a\nonumber \\
...
{\color{greenunder}\text{matrix product}:}\quad & =x^{:P}\Rightarrow(x+\bbnum 0^{:Q\times L^{A}})\triangleright\theta\quad.
\end{align*}
...
and $M^{A}=Q\times\left(P+A\right)$, which I call ``linear''. It
turns out that the composed-\emph{outside} transformers are available
for a significantly wider range of monads. I call those monads ``rigid'',
because one of their general properties is having a single ``shape''
(Section~???).\index{monads!rigid} (There does not seem to be an
already accepted name for monads of this kind.)

\paragraph{Definition:}

A monad $R$ is \textbf{rigid} if it has a composed-outside monad
transformer, $T_{R}^{M}=R\circ M$, where $M$ is a foreign monad. 

This definition does not explain what monads are rigid or how to recognize
a non-rigid monad. Two simple examples of rigid monads are the \lstinline!Reader!
...
four constructions give rigid monads:
\begin{enumerate}
...
The construction I call \textbf{\index{monads!choice monad}}the \textbf{choice}
...
monad\index{monads!search monad}\index{search monad@\textbf{search monad}}}
$R^{A}\triangleq\left(A\Rightarrow Q\right)\Rightarrow A$. 
...
It turns out that we need yet another law for \lstinline!swap! if
we wish to prove that the composed monad also has a lawful \lstinline!swap!.
Before proving that, we will need the following lemma about ``runners''
for composed monads.
...
and $S$ is any monad (not necessarily rigid) then any monadic morphism
...
\subsection{Rigid functors}

The properties of rigid monads can be extended to a slightly larger
class of rigid functors.
...
\]
Not all functors admit a natural transformation with this type signature.
For example, $F^{A}\triangleq Z+A$ is not rigid because the type
signature 
\[
\left(A\Rightarrow Z+B\right)\Rightarrow Z+\left(A\Rightarrow B\right)
\]
...
monad,
\[
\text{sw}_{R,M}:M^{R^{A}}\Rightarrow R^{M^{A}}\cong(Z\Rightarrow R^{A})\Rightarrow R^{Z\Rightarrow A}\text{ where }M^{A}\triangleq Z\Rightarrow A\quad.
\]
So it appears that any rigid monad having a \lstinline!swap! method
will also have \lstinline!fuseIn!. It turns out that all rigid monads
are also rigid functors. In proving that statement, however, we want
to avoid assuming the existence of \lstinline!swap!, which is not
automatically guaranteed for any rigid monad. Instead, we use directly
...
(Note that, in general, $S$ will not be a monad.)
...
A rigid functor $R$ is always pointed; the method \lstinline!pure!
can be defined as
\[
...
flatMap that can handle multiple $M$-effects at once; more precisely,
an $R$-container of $M$-effects.
