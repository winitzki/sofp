...
and ``unplanned\index{unplanned exception}'' exceptions. 
...
It is reasonable that the program evaluation should immediately stop
...
Scala library defines the \lstinline!.reverse! method for this task,
but we will show an implementation using \lstinline!foldLeft!. The
updater function \emph{prepends} an element to a previous list:
\begin{lstlisting}
...
and the types will not match in the later code.
...
guarantee that the result is a non-empty list, we give it two arguments:
\begin{lstlisting}
...
The \lstinline!head! method is safe for non-empty lists, unlike ordinary
\lstinline!List!s:
\begin{lstlisting}
def head[A]: NEL[A] => A = {
  case Last(x)        => x
...
for reversing the ordinary list,
\begin{lstlisting}
def reverse[A](xs: List[A]): List[A] =
  xs.foldLeft(Nil: List[A])((prev,x) => x :: prev)
\end{lstlisting}
However, we cannot reuse the same code for non-empty lists by using
...
scala> reverse1(toNEL(1,List(2,3)))
...
\end{lstlisting}
We can visualize \lstinline!x! as an abstract syntax tree{\tiny{} \Tree[.\texttt{Mul} [.\texttt{Sqrt} [.\texttt{Num} $16$ ] ] [.\texttt{Add} [ [.\texttt{Num} [ $1$ ] ] [.\texttt{Num} [ $2$ ] ] ] ] ] }. 
...
To implement the \lstinline!run! function, we need to define the
``safe arithmetic'' operations for the result type \lstinline!Either[String, Int]!.
...
In the second case, we need to perform pattern matching on the value
\lstinline!eab!:
\begin{lstlisting}
  ...
...
value of \lstinline!b:B!. Since we want to preserve information as
much as possible, we make the second choice. The final code is
\begin{lstlisting}
def f1[A, B]: Option[Either[A, B]] => Either[A, Option[B]] = {
...
is the main motivation for choosing the name ``disjunctive types''.\footnote{These types are also called ``variants'', ``sum types'', and ``union
types''.}
...
function type \lstinline!A => B!. In the next chapter, we will study
