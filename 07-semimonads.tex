...
Typical tasks solved with tree-like monads:
\begin{itemize}
...
Typical tasks:
\begin{itemize}
\item Collect extra information about computations along the way
\item Chain of computations with a nonstandard evaluation strategy
\end{itemize}
Examples: see code
\begin{enumerate}
...
\item {\footnotesize{}$\text{flm}\,\big(f^{A\Rightarrow S^{B}}\circ h^{S^{B}\Rightarrow S^{C}}\big)=\text{flm}\,f\circ h$}
{\footnotesize{}(naturality in $B$)
\[
\xymatrix{\xyScaleY{0.2pc}\xyScaleX{3pc} & S^{B}\ar[rd]\sp(0.5){\ h^{S^{B}\Rightarrow S^{C}}}\\
S^{A}\ar[ru]\sp(0.5){\text{flm}\,f^{A\Rightarrow S^{B}}\ }\ar[rr]\sb(0.5){\text{flm}\,(f^{A\Rightarrow S^{B}}\circ h^{S^{B}\Rightarrow S^{C}})\,} &  & S^{C}
}
\]
}{\footnotesize \par}
\end{enumerate}
{\footnotesize{}$\text{flm}\,\big(f^{A\Rightarrow S^{B}}\circ\text{flm}\,g^{B\Rightarrow S^{C}}\big)=\text{flm}\,f\circ\text{flm}\,g$}
{\footnotesize{}(composition) \textendash{} this is law 2 with $h=\text{flm}\,g$
\[
\xymatrix{\xyScaleY{0.2pc}\xyScaleX{3pc} & S^{B}\ar[rd]\sp(0.5){\ \text{flm}\,g^{B\Rightarrow S^{C}}}\\
...
has \emph{only one} law:
\begin{enumerate}
\item {\footnotesize{}$\text{fmap}\big(h^{S^{A}\Rightarrow S^{B}}\big)\circ\text{ftn}^{\left[S,B\right]}=\text{ftn}^{\left[S,A\right]}\circ\text{fmap}\,f$}
{\footnotesize{}(naturality)
\[
\xymatrix{\xyScaleY{0.2pc}\xyScaleX{5pc} & S^{S^{B}}\ar[rd]\sp(0.5){\ \text{ftn}^{\left[S,B\right]}}\\
S^{S^{A}}\ar[ru]\sp(0.5){\text{fmap}\,\big(h^{S^{A}\Rightarrow S^{B}}\big)\ \ }\ar[rd]\sb(0.5){\text{ftn}^{\left[S,A\right]}\,} &  & S^{B}\\
 & S^{A}\ar[ru]\sb(0.5){h^{S^{A}\Rightarrow S^{B}}}
}
\]
}{\footnotesize \par}
\end{enumerate}
{\footnotesize{}$\text{fmap}\,(\text{ftn}^{\left[S\right]})\circ\text{ftn}^{\left[S\right]}=\text{ftn}^{\left[S^{S}\right]}\circ\text{ftn}^{\left[S\right]}$}
{\footnotesize{}(associativity) \textendash{} this is law 1 with $h=\text{ftn}$
\[
...
Denote for brevity $f_{\uparrow}\equiv\text{fmap}^{\left[S\right]}\,f$ 

Express $\text{flm}\,f=f_{\uparrow}\circ\text{ftn}$ and substitute
into $\text{flm}$'s 2 laws:
\begin{enumerate}
...
law
\item $\text{flm}\left(f\circ h\right)=\text{flm}\,f\circ h$ gives $\left(f\circ h\right)_{\uparrow}\circ\text{ftn}=f_{\uparrow}\circ\text{ftn}\circ h$;\\
...
type signature and just one law (naturality)
\item It is usually easy to check naturality!
\end{itemize}
\end{frame}

\begin{frame}{Semimonad laws IV: Checking naturality of \texttt{\textcolor{blue}{\footnotesize{}flatten}} }

Implement \texttt{\textcolor{blue}{\footnotesize{}flatten}} for various
standard monads and check naturality
\begin{itemize}
\item $F^{A}\equiv1+A$; $\text{ftn}:1+1+A\Rightarrow1+A$
\item $F^{A}\equiv Z+A$; $\text{ftn}:Z+Z+A\Rightarrow Z+A$
\item $F^{A}\equiv\text{List}^{A}$; $\text{ftn}:\text{List}^{\text{List}^{A}}\Rightarrow\text{List}^{A}$
\item $F^{A}\equiv A\times W$; $\text{ftn}:A\times W\times W\Rightarrow A\times W$
\item $F^{A}\equiv R\Rightarrow A$; $\text{ftn}:\left(R\Rightarrow R\Rightarrow A\right)\Rightarrow R\Rightarrow A$
\item $F^{A}\equiv S\Rightarrow A\times S$; $\text{ftn}:\left(S\Rightarrow\left(S\Rightarrow A\times S\right)\times S\right)\Rightarrow S\Rightarrow A\times S$
\item $F^{A}\equiv\left(A\Rightarrow R\right)\Rightarrow R$; $\text{ftn}:\left(\left(\left(\left(A\Rightarrow R\right)\Rightarrow R\right)\Rightarrow R\right)\Rightarrow R\right)\Rightarrow\left(A\Rightarrow R\right)\Rightarrow R$
\end{itemize}
Naturality of all these \texttt{\textcolor{blue}{\footnotesize{}flatten}}
functions is easy to verify
\begin{itemize}
\item Code implementing \texttt{\textcolor{blue}{\footnotesize{}flatten}}
is fully parametric in $A$
\end{itemize}
\end{frame}
...
\item cannot map $F^{1+A}\Rightarrow F^{A}$ because we do not have $1+A\rightarrow A$e do not have $1+A\rightarrow A$
\end{itemize}
\end{itemize}
