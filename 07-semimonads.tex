...
\framesubtitle{Intuitions behind adding more ``left arrows''}
...
\begin{itemize}
\item The computation processes all elements from each collection
\begin{itemize}
\item The number of resulting data items is $\leq m*n*p$\texttt{\textcolor{blue}{\footnotesize{} }}{\footnotesize \par}
\begin{itemize}
\item All the resulting data items must fit within \emph{the same} container
type!
\item The set of \emph{container capacity counts} is closed under multiplication
\end{itemize}
\end{itemize}
...
\item A chain of asynchronous function calls
\item A sequence of steps that update state while returning results
\end{enumerate}
\end{frame}

\begin{frame}{Laws for \texttt{\textcolor{blue}{\footnotesize{}flatMap}} I: The
intuitions}
...
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}~x~$\leftarrow$~makeContainer()}{\footnotesize \par}

\textcolor{blue}{\footnotesize{}~~~.map(a~$\Rightarrow$~f(a))}{\footnotesize \par}
\end{lyxcode}
%
\end{minipage}\texttt{\textcolor{blue}{\footnotesize{}\hfill{}}}%
\begin{minipage}[c][1\totalheight][t]{0.4\columnwidth}%
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}z~$\leftarrow$~makeContainer()}{\footnotesize \par}

\textcolor{blue}{\footnotesize{}x~=~f(z)}{\footnotesize \par}
\end{lyxcode}
%
\end{minipage}\texttt{\textcolor{blue}{\footnotesize{}\hfill{}\medskip{}
container.map(f).flatMap(x $\Rightarrow$ g(x)) = container.flatMap(z
$\Rightarrow$ g(f(z)))}}{\footnotesize \par}
\begin{itemize}
\item In \texttt{\textcolor{blue}{\footnotesize{}y $\leftarrow$ c(x)}},
the right-hand side can use \emph{the previously computed} \texttt{\textcolor{blue}{\footnotesize{}x}} 
\end{itemize}
\texttt{\textcolor{blue}{\footnotesize{}}}%
\begin{minipage}[c][1\totalheight][t]{0.49\columnwidth}%
\begin{lyxcode}
...
\textcolor{blue}{\footnotesize{}y~$\leftarrow$~makeContainer(x)}{\footnotesize \par}
\end{lyxcode}
%
\end{minipage}\texttt{\textcolor{blue}{\footnotesize{}\hfill{}}}%
\begin{minipage}[c][1\totalheight][t]{0.49\columnwidth}%
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}y~$\leftarrow$~makeContainer(f(z))}
\end{lyxcode}
%
\end{minipage}\texttt{\textcolor{blue}{\footnotesize{}\hfill{}\medskip{}
container.flatMap(makeC).map(f) = container.flatMap(z $\Rightarrow$
makeC(f(z)))}}{\footnotesize \par}
\begin{itemize}
...
\begin{minipage}[c][1\totalheight][t]{0.49\columnwidth}%
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}x~$\leftarrow$~makeC1()}{\footnotesize \par}

\textcolor{blue}{\footnotesize{}y~$\leftarrow$~makeC2(x)}{\footnotesize \par}
\end{lyxcode}
%
\end{minipage}\texttt{\textcolor{blue}{\footnotesize{}\hfill{}}}%
\begin{minipage}[c][1\totalheight][t]{0.49\columnwidth}%
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}y~$\leftarrow$~makeC1().flatMap~\{}{\footnotesize \par}

\textcolor{blue}{\footnotesize{}~~z~$\Rightarrow$~makeC2(z)~\}}{\footnotesize \par}
\end{lyxcode}
%
\end{minipage}\texttt{\textcolor{blue}{\footnotesize{}\hfill{}\medskip{}
contr.flatMap(p).flatMap(q) = contr.flatMap(x $\Rightarrow$ p(x).flatMap(q))}}{\footnotesize \par}
\begin{enumerate}
\item {\footnotesize{}$\text{flm}\,f^{A\Rightarrow B}\circ\text{filter}\,p^{B\Rightarrow\text{Boolean}}=\text{filter}\left(f\circ p\right)\circ\text{fmap}\,f^{A\Rightarrow B}$}{\footnotesize \par}
