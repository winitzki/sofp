...
In this way, any type $T$ can be made into a ``trivial'' semigroup
in one of these two ways. (\index{``trivial'' semigroup}``Trivial''
semigroups are rarely useful.)
...
instances have several inequivalent implementations, automatic derivation
of \lstinline!Semigroup! instances is not often useful.
...
\end{lstlisting}
Similarly, the choice $e_{A+B}\triangleq\bbnum 0^{:A}+e_{B}$ forces
us to choose the version ``take $A$'' of the $\oplus$ operation.

\paragraph{Function types}

The semigroup construction for function types works for monoids. Exercise~\ref{subsec:tc-Exercise-3}
is to show that for any type $R$ and any monoid $A$, the function
type $R\Rightarrow A$ is a lawful monoid.
...
\caption{Using a \lstinline!trait! for implementing the \lstinline!Monoid!
...
The special Scala plugin called the ``kind projector''\footnote{\texttt{\href{https://github.com/typelevel/kind-projector}{https://github.com/typelevel/kind-projector}}}
...
syntax in the following code examples.
...
when $F$ is a functor. For co-pointed functors, there is no simpler
form of the \lstinline!extract! method. If we set $A=\bbnum 1$ and
$f^{:\bbnum 1\Rightarrow B}\triangleq(1\Rightarrow b)$ in the naturality
law, both sides will become functions of type $F^{\bbnum 1}\Rightarrow B$.
...
implicit val functorF: Functor[F] = implement   // Automatically implement typeclass instances for F.
implicit val pointedF: Pointed[F] = implement
\end{lstlisting}
...
In some applications, it is important to be able to add new types
to the type domain whenever necessary. For instance, suppose we would
like to add support for the \lstinline!Boolean! type so that \lstinline!bitsize[Boolean]!
returns \lstinline!1!. To add \lstinline!Boolean! to the type domain
of \lstinline!HasBitsize!, we need to add this code,
\begin{lstlisting}
implicit val bitsizeBoolean = HasBitsize[Boolean](1)
\end{lstlisting}
In other applications, it is important that a PTVF should remain undefined
for types other than the initially supported ones. To prevent the
programmer from creating any further values of type \lstinline!HasBitsize!,
we make it a non-case class whose constructor is declared as a \lstinline!private!
function,
\begin{lstlisting}
...
An alternative implementation via a \lstinline!trait! requires longer
code but brings no advantages:
\begin{lstlisting}
...
\subsubsection{Example \label{subsec:tc-Example-4}\ref{subsec:tc-Example-4}}

Using the \texttt{cats} library, define a \lstinline!Functor! instance
for \lstinline!type F[T] = Seq[Try[T]]!.
...
Define a \lstinline!Contrafunctor! type class having the method \lstinline!contramap!:
\begin{lstlisting}
...
Define \lstinline!Functor! instance for recursive type $Q^{A}\triangleq\left(\text{Int}\Rightarrow A\right)+\text{Int}+Q^{A}$.
...
If $A$ is a monoid and $R$ any type, implement a \lstinline!Monoid!
instance for $R\Rightarrow A$:
\begin{lstlisting}
def monoidFunc[A: Monoid, R]: Monoid[R => A] = ???
\end{lstlisting}
With \lstinline!R = Boolean!, use the type equivalence $(R\Rightarrow A)=(\bbnum 2\Rightarrow A)\cong A\times A$
and verify that \lstinline!monoidFunc[A, Boolean]! is the same as
the monoid instance for $A\times A$ computed by \lstinline!monoidPair[A, A]!
(see Section~\ref{subsec:Monoids-constructions}).%
\begin{comment}
\begin{lstlisting}
def monoidFunc[A: Monoid, R] = Monoid[R => A](
  { (x, y) => r => x(r) |+| y(r) }, r => implicitly[Monoid[A]].empty
)
\end{lstlisting}
\end{comment}
...
library's \lstinline!Contravariant! type class for $F^{A}$.
