...
\chapter{Typeclasses and functions of types}
...
it is a function of the \emph{value} \lstinline!x!. For the value
\lstinline!x! defined in this code, the type \lstinline!x.T! evaluates
...
\end{enumerate}
Now we notice that it is not necessary to define the type constructor
\lstinline!Frac[T]! via an unfunctor. The only purpose of \lstinline!Frac[T]!
is to define the type domain $\{\text{BigDecimal},\text{Double}\}$.
The same type domain can be also defined via a case class:
...
\noindent This code creates values of type \lstinline!Frac[T]! with
the correctly chosen type parameters \lstinline!T!, but no other
such values.
...
An equivalent implementation of the \lstinline!Frac! typeclass using
a \lstinline!trait! with methods looks like this:
...
\noindent The code of \lstinline!avg[T]! will work unchanged with
this implementation of the \lstinline!Frac! typeclass.

The implementation via a \lstinline!trait! is significantly longer
than the code shown previously that used values of a case class. The
only advantage of the longer code is the ability to combine different
typeclasses by \lstinline!trait! mixing. We will consider that in
more detail below. For now, we note that both our typeclass implementations
impose significant burdens on programmers:
\begin{itemize}
\item All calls to \lstinline!func[T](args)! need to be rewritten as \lstinline!func[T](args, tci)!,
with typeclass instances \lstinline!tci!.
...
we need to use them.
\end{itemize}
These costs can be reduced (and, in some cases, removed) by using
Scala's ``implicit value'' feature. 
...
\end{lstlisting}
After this declaration, an implicit value of type \lstinline!Int!
becomes available in the current scope. That value will be automatically
passed as an argument to any function declaring an ``\lstinline!implicit!''
...
\end{lstlisting}
What does \lstinline!implicitly! do? Since its only argument is declared
as \lstinline!implicit!, we can simply write \lstinline!implicitly[T]!
...
code. The rules for this search are complicated; consult the Scala
documentation for details. If implicit values are declared in another
module, they can be made available in a given scope by using an \lstinline!import!
declaration. In many cases, explicit \lstinline!import! declarations
can be avoided. One way to avoid them is to declare typeclass instances
as implicit values within the \textbf{companion object}\index{companion object}
of the typeclass (i.e.~the Scala \lstinline!object! with the same
name as the typeclass):
\begin{lstlisting}
final case class Frac[T](add: (T, T) => T, intdiv: (T, Int) => T)

object Frac {  // The companion object for `Frac[T]` defines some typeclass instances.
  implicit val fracBD = Frac[BigDecimal]( (x, y) => x + y,  (x, n) => x / n )
  implicit val fracD = Frac[Double]( (x, y) => x + y,  (x, n) => x / n )
}
\end{lstlisting}
Whenever a function needs a typeclass instance of type \lstinline!Frac[T]!
for a specific type \lstinline!T!, the Scala compiler will automatically
look into the companion object and find the instances declared there.
So, the programmer's code will not need to \lstinline!import! these
typeclass instance values.
\begin{lstlisting}
scala> avg(Seq(1.0, 2.0, 3.0))
res0: Double = 2.0
\end{lstlisting}
...
\subsection{Solved examples: Using typeclasses in practice\index{solved examples}}

We will now see some practical examples of programming tasks implemented
via typeclasses.

\subsubsection{Example \label{subsec:tc-Example-Data-structures}\ref{subsec:tc-Example-Data-structures}:
Known metadata}

The business logic of an application requires to work with data structures
implemented in various third-party libraries. All those data structures
are case classes that must contain certain metadata: ``name'' (a
\lstinline!String!) and ``count'' (a \lstinline!Long! integer).
However, the specific types of these data structures declare the metadata
differently, each in its own way:
\begin{lstlisting}
...
A certain business application needs to count the number of processed
files and to expose this number to external services for observability.
...
function \lstinline!default[T]!, defined for those types \lstinline!T!
...
We need to define a partial type-to-value function \lstinline!default[T]!
with a type domain that contains (at least) the types \lstinline!Int!
and \lstinline!Double!. For every supported type \lstinline!T!,
we need to store the known default value of that type. So, the typeclass
instance can be defined as a simple wrapper for values of type \lstinline!T!:
\begin{lstlisting}
...
\begin{lstlisting}
implicit val defaultList = HasDefault[List[A]](List()) // Error.
\end{lstlisting}
because the type parameter \lstinline!A! needs to be defined in the
left-hand side. Since Scala 2 does not support val declarations with
type parameters (Scala 3 will), we need to write the typeclass instance
as a function:
\begin{lstlisting}
...
\subsubsection{Example \label{subsec:tc-Example-Semigroups-1}\ref{subsec:tc-Example-Semigroups-1}:
...
\end{equation}
This associativity\index{associativity of semigroup} law makes parentheses
in the expression $x\oplus y\oplus z$ unnecessary.

A type \lstinline!T! with an associative binary operation is called
a \textbf{semigroup}\index{semigroup}. The task in this example is
...
\end{lstlisting}
The equivalent code in the short typeclass syntax can be written as
\begin{lstlisting}
def combine[T: Semigroup]: (T, T) => T = implicitly[Semigroup[T]].combine
\end{lstlisting}

As another example of using the semigroup, consider the task of aggregating
a sequence whose elements are of a semigroup type. We may compute
the aggregate value for the sequence by passing the \lstinline!combine!
operation into the standard method \lstinline!.reduce!:
\begin{lstlisting}
def aggregate[T: Semigroup](ts: Seq[T]): T = ts.reduce(combine[T])
\end{lstlisting}
This function assumes a non-empty input sequence \lstinline!ts! whose
elements are of a semigroup type \lstinline!T!. With the previous
definitions, we can now evaluate expressions such as
\begin{lstlisting}
scala> aggregate(Seq(1,2,3))
res0: Int = 6

scala> aggregate(Seq(List(), List(true), List(), List(true, false)))
res1: List[Boolean] = List(true, true, false)
\end{lstlisting}
...
with \lstinline!aggregate! may not work as expected. However, the
...
proof of the associativity law for lists, starting with the recursive
definition of \lstinline!concat!, will require significant work (see
Section~\ref{subsec:Semigroups-constructions}). 

\subsubsection{Example \label{subsec:tc-Example-Semigroups-1-1}\ref{subsec:tc-Example-Semigroups-1-1}:
Mergeable data with default value (monoid)}

When a data type forms a semigroup, it is often possible to find a
special value that acts as an ``default'' value with respect to
the semigroup operation. Merging with the default value will not change
any other value. For instance, concatenating with an empty list does
not change any other list; so the empty list plays the role of the
default value for lists. Merging an empty set and any other set does
not change the other set.
...
Section~\ref{subsec:Monoids-constructions} will study monoids in
more detail and show further examples.
...
def inject[F[_]: Functor, A, B](a: A, f: F[B]): F[(A, B)] = ???
\end{lstlisting}
The intention is to require the type parameter \lstinline!F! to be
a functor. We can use a suitable typeclass to implement this constraint.
Since the type parameter \lstinline!F! is itself a type constructor,
the typeclass will be of the form \lstinline!Functor[F[_]]!. 

What would be the necessary information carried by a typeclass instance?
A functor \lstinline!F! must have an \lstinline!fmap! function with
type signature
\begin{lstlisting}
def fmap[A, B](f: A => B): F[A] => F[B]
\end{lstlisting}
So, a typeclass instance of the \lstinline!Functor! typeclass must
contain this function as a value.{*}{*}{*}

\section{Constructing typeclass instances for composite types}
