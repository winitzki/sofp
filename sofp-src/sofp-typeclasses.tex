...
that declares an \textsf{``}implicit argument\textsf{''} of the same type. Scala's
...
\end{lstlisting}
Although the \textbf{expanded form} of the function\index{expanded form of function!use in recursive values}\textsf{''}
$t\rightarrow f(t)$ is equivalent to just $f$, there is an important
difference: using \textsf{``}expanded functions\textsf{''} in the code will make recursive
...
Can we decide whether such a type equation has a \textsf{``}solution\textsf{''}, i.e.~a
...
an \textsf{``}evidence value\textsf{''}\index{evidence value}\index{typeclass!evidence value}
...
\textsf{``}evidence argument\textsf{''} of type \lstinline!Rel[A, B]!. This will
