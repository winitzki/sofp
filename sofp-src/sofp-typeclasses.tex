...
\end{lstlisting}
may be applied only to values of the form \lstinline!Left(...)!.
...
\noindent The function \lstinline!avg[T]! works unchanged with this
...
\begin{lstlisting}[mathescape=true]
type S[A] = Either[Either[Int, A], (Int, (A, A))]  // Define $\color{dkgreen} S^{\scriptscriptstyle A} \triangleq\text{Int}+A+\text{Int}\times A\times A $.
...
\begin{lstlisting}[mathescape=true]
val t = T(Left(Right(T(Left(Left(10))))))          // $\color{dkgreen}t$ has type $\color{dkgreen} \bbnum 0+(10^{\scriptscriptstyle:\text{Int}}+\bbnum 0+\bbnum 0)^{\scriptscriptstyle:T}+\bbnum 0$.
...
has a \lstinline!Semigroup! instance. 
...
code is shown in Figure~\ref{fig:Implementation-of-Monoid-typeclass-with-trait}.
...
val t1: T = T(() => Right((1, t1)))                   // Unbounded stream [1, 1, 1, ...].
def t2(n: Int): T = T(() => Right((n, t2(n+1))))      // Unbounded stream [n, n+1, n+2, ...].
\end{lstlisting}
The type $T\triangleq\text{Fix}^{S}$ also admits bounded streams
defined without recursion, for example:
\begin{lstlisting}
val t0 = T(() => Right((10, T(() => Left("stop")))))  // Bounded stream [10, "stop"].
\end{lstlisting}
...
\lstinline!M = Int! and \lstinline!N = Double!? (Later we may need
...
code must be reorganized to convert all compatible quantities to a
single chosen unit (e.g., all lengths to kilometers and all masses
...
The resulting code is shown in Figure~\ref{fig:Full-code-implementing-units-length-mass}.
...
\hline 
\textbf{\small{}Typeclass} & \textbf{\small{}Type of instance values} & \textbf{\small{}Inductive form} & \textbf{\small{}Structure functor}\tabularnewline
\hline 
\hline 
{\small{}pointed type} & {\small{}$\bbnum 1\rightarrow A$} & {\small{}$P^{A}\rightarrow A$} & {\small{}$P^{A}\triangleq\bbnum 1$}\tabularnewline
\hline 
...
\hline 
{\small{}functor} & {\small{}$F^{A}\times\left(A\rightarrow B\right)\rightarrow F^{B}$} & {\small{}$\forall(A,B).\,P^{A,B,F}\rightarrow F^{B}$} & {\small{}$P^{A,B,F}\triangleq F^{A}\times\left(A\rightarrow B\right)$}\tabularnewline
\hline 
{\small{}pointed functor} & {\small{}$B+F^{A}\times\left(A\rightarrow B\right)\rightarrow F^{B}$} & {\small{}$\forall(A,B).\,P^{A,B,F}\rightarrow F^{B}$} & {\small{}$P^{A,B,F}\triangleq B+F^{A}\times\left(A\rightarrow B\right)$}\tabularnewline
\hline 
{\small{}contrafunctor} & {\small{}$F^{A}\times(B\rightarrow A)\rightarrow F^{B}$} & {\small{}$\forall(A,B).\,P^{A,B,F}\rightarrow F^{B}$} & {\small{}$P^{A,B,F}\triangleq F^{A}\times\left(B\rightarrow A\right)$}\tabularnewline
\hline 
{\small{}pointed contrafunctor} & {\small{}$\bbnum 1+F^{A}\times\left(B\rightarrow A\right)\rightarrow F^{B}$} & {\small{}$\forall(A,B).\,P^{A,B,F}\rightarrow F^{B}$} & {\small{}$P^{A,B,F}\triangleq\bbnum 1+F^{A}\times\left(B\rightarrow A\right)$}\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
...
parameters. To simplify the presentation in this section, we will
only consider typeclasses for simple types, such as \lstinline!Semigroup!
or \lstinline!Monoid!.
...
hold for $A\times B$. Some advanced reasoning techniques required
for proving that statement will be developed in Chapter~\ref{chap:Free-type-constructions}.

We note that the co-product construction \emph{cannot} be derived
in general for arbitrary $P$-typeclasses: given evidence values of
types $P^{A}\rightarrow A$ and $P^{B}\rightarrow B$, it is not guaranteed
that we can compute a value of type $P^{A+B}\rightarrow A+B$. Indeed,
it turns out that some $P$-typeclasses do not support the co-product
construction (although some do).
...
prove that the laws will hold for the new typeclass instances.
...
any $P$-typeclass. (The typeclass laws still need to be verified
for the new typeclass instances.)
