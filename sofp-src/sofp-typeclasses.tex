...
and additionally the default value of type $T$. So, we write
\begin{lstlisting}
...
What information needs to beeiejccfnjtugggvngudlglfvfduehetjuurlkejldr
wrapped by a typeclass instance? A functor \lstinline!F! must have
a \lstinline!map! function with the standard type signature
...
\hline 
{\footnotesize{}Recursive types} & {\footnotesize{}$\text{Eq}^{T}\Rightarrow\text{Eq}^{S^{T}}$ where
...
\hline 
{\footnotesize{}Recursive types} & {\footnotesize{}$\text{Semigroup}^{T}\Rightarrow\text{Semigroup}^{S^{T}}$
...
\hline 
{\footnotesize{}Recursive types} & {\footnotesize{}$\text{Monoid}^{T}\Rightarrow\text{Monoid}^{S^{T}}$
...
to Section~\ref{subsec:f-Contrafunctor-constructions}. The results
are shown in Table~\ref{tab:Type-constructions-for-pointed-contrafunctor}
below.
...
for arbitrary types $A$, $B$. Show that the inverse type signature,
$(A\Rightarrow F^{B})\Rightarrow F^{A\Rightarrow B}$, cannot be implemented
for some functors $F$. (Functors admitting a function with that type
signature are called ``rigid''; see Section~\ref{subsec:Rigid-functors}.)
...
For example, \lstinline!Semigroup!:
\begin{lstlisting}
