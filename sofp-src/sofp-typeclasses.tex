...
need to be explicitly imported intod every scope where extension methods
...
in $A$ and a functor in $R$. A value of type $C^{\bbnum 1}$ will
...
expression, e.g.
\[
C^{A,B}\triangleq\left(\bbnum 1+A\Rightarrow B\right)+(\text{String}\times A\times B\Rightarrow\text{String})
\]
with respect to $A$? We need to set $A=\bbnum 1$ and try to create
a value of type $C^{\bbnum 1,B}$. In this example, $C^{\bbnum 1,B}=\left(\bbnum 1+\bbnum 1\Rightarrow B\right)+(\text{String}\times\bbnum 1\times B\Rightarrow\text{String})$.
A value of this type is e.g.~$\text{wu}_{C}\triangleq\bbnum 0+(s^{:\text{String}}\times1\times b^{:B}\Rightarrow s)$.
