...
that declares an ``implicit argument'' of the same type. Scala's
...
\end{lstlisting}
We need to declare the arguments as \lstinline!implicit! in the function's
...
We may call this typeclass a ``$Z$-extractor\index{extractor typeclass}\index{typeclass!Extractor@\texttt{Extractor}}''
...
in one of these two ways. (\index{trivial@``trivial'' semigroup}``Trivial''
...
The special Scala plugin called the \index{kind@``kind projector'' plugin}``kind
...
e.g., \lstinline![X] => (F[X], G[X])!. We will use the ``kind projector''
...
and obtain\footnote{The symbol $\overset{!}{=}$ means ``must be equal, according to
what we know''.}
\[
...
apply \lstinline!Seq!'s built-in \lstinline!map! method rather than
...
projector\index{kind@``kind projector'' plugin}'' plugin and replace
...
A framework implements a ``route'' type $R$ as $R\triangleq Q\rightarrow(E+S)$,
...
(dual O'Connor\index{Russell O'Connor})}
...
Can we decide whether such a type equation has a ``solution'', i.e.,
...
The \lstinline!Eq! typeclass is ``uninductive'' (neither inductive
...
is implemented by requiring an ``evidence value''\index{evidence value}\index{typeclass!evidence value}
