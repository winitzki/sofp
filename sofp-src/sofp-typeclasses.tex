...
res1: Double = 6.0
\end{lstlisting}
Computing the average value of a sequence of numbers can be written
as
\begin{lstlisting}
def avg(s: Seq[Double]): Double = s.sum / s.length
\end{lstlisting}
We would like to generalize the averaging function from \lstinline!Double!
to other numeric types, e.g.
\begin{lstlisting}
def avg[T](s: Seq[T]): T = ???
\end{lstlisting}
But this is impossible because averaging works only for certain types
\lstinline!T!, not for arbitrary types \lstinline!T! as would be
implied by that type signature. We will be able to define \lstinline!avg[T]!
only if we constrain the type parameter \lstinline!T! to be a type
representing a suitable numeric value (e.g.~\lstinline!BigDecimal!
or \lstinline!Double!).
...
that those types must satisfy, is called a \textbf{typeclass}\index{typeclass}.
...
\hline 
\hline 
\textbf{\small{}to value} & {\small{}(VVF)~ }\lstinline!def f(x:Int):Int! & {\small{}(TVF)~ }\lstinline!def point[A]: A => List[A]!\tabularnewline
\hline 
...
to a \textbf{partial function}\index{partial function}, i.e.~a function
...
functions \textbf{partial type-to-value}\index{partial type-to-value function}
functions (PTVFs\index{PTVF}), to distinguish them from partial value-to-value
...
to that type domain. The type constraint $T\in\,$\{\lstinline!BigDecimal!,
\lstinline!Double!\} is equivalent to the requirement that a value
of type $\text{Frac}^{T}$ should exist. So, we will implement the
type constraint by including an \emph{additional argument} of type
...
add an extra argument of type \lstinline!Frac[T]! to the function.
...
automatically. This makes typeclasses easier to use because typeclass
instance values will need to be written out much less often.
...
function applications, such as \lstinline!xs.map(f).filter(g)! or
\lstinline!x plus y plus z!, because that code is easier to read
than \lstinline!filter(map(xs, f), g)!. 
...
This code defines partial type-to-value functions (PTVFs) \lstinline!getName!
and \lstinline!getCount! with the type domain that contains the three
types \lstinline!Data1!, \lstinline!Data2!, \lstinline!Data3!.
In order to add a new type, say \lstinline!Data4!, to the type domain,
we will need to declare the appropriate typeclass instance as an implicit
value of type \lstinline!HasMetadata[Data4]!. New implicit values
can be defined anywhere in the code, not necessarily within the companion
object \lstinline!HasMetadata!. To avoid extra \lstinline!import!
statements, the implicit value may be defined within the companion
object of \lstinline!Data4!:
\begin{lstlisting}
...
Types that have default values are also called \textbf{pointed}\index{pointed type}
...
proof of the associativity law for lists, starting from a recursive
definition of the \lstinline!concat! function, requires significant
work (see Section~\ref{subsec:Recursive-types-and}). 
...
It is also convenient to define \lstinline!.map! as an infix method
...
ones. We will now apply the same \index{types!structural analysis}\textbf{structural
...
Example~\ref{subsec:tc-Example-metadata-extractors} showed a typeclass
...
\hline 
{\footnotesize{}Recursive types} & {\footnotesize{}$\text{Extractor}^{T}\Rightarrow\text{Extractor}^{S^{T}}$
...
The results of the analysis will show which types can be meaningfully
...
\end{lstlisting}
The laws hold for the new operation because the code makes sure that
values of type \lstinline!Either[A, B]! can be equal only when they
are all of type \lstinline!Left! or all of type \lstinline!Right!.
If the comparisons of types $A$ and $B$ satisfy the laws separately,
the laws for $A+B$ will be satisfied separately for values of type
\lstinline!Left! and of type \lstinline!Right!.
...
\end{lstlisting}
That would define a comparison operation that violates the identity
law: there are many functions \lstinline!f! and \lstinline!g! that
will give different results for arguments not equal to $r_{1}$. Another
way to see the problem is to write the type equivalence
\[
...
\]
i.e.~if $R$ has a known and finite number of distinct values. In
that case, we can apply a function of type $R\Rightarrow A$ to every
possible value of its argument $R$ and compare all the resulting
values of type $A$. This argument shows that functions e.g.~$f_{1}^{:R\Rightarrow A}$
and $f_{2}^{:R\Rightarrow A}$ will be found to be equal only if we
are able to check that $f_{1}(r)=f_{2}(r)$ for \emph{every} possible
value of type $R$. We cannot implement such a comparison for a general
type $R$.

So, functions of type $R\Rightarrow A$ cannot have an \lstinline!Eq!
instance for a general type $R$. A similar argument shows that functions
of type $A\Rightarrow R$ also cannot have a useful \lstinline!Eq!
instance. The only exceptions are types $R\Rightarrow A$ or $A\Rightarrow R$
...
for $T$ recursively:
\[
\text{eqT}:\text{Eq}^{T}\quad,\quad\quad\text{eqT}\triangleq\text{eqS}\left(\text{eqT}\right)\quad.
\]
The recursive equation for \lstinline!eqT! needs to be implemented
as an expanded function, 
\[
\text{eqT}\triangleq t^{:T}\times t^{:T}\Rightarrow\text{eqS}\left(\text{eqT}\right)(t\times t)\quad,
\]
...
\lstinline!eqS! preserves them. This corresponds to the induction
step.
...
We conclude that the recursively defined typeclass instance will satisfy
the laws. This argument is general and will apply to all cases when
a typeclass instance $t$ is defined by recursion, $t\triangleq s(t)$,
via a function $s$ that preserves the laws.
...
\begin{enumerate}
\item (``Take $A$'') Discard all data of type $B$: $(a^{:A}+\bbnum 0)\oplus(\bbnum 0+b^{:B})=a$
and $(\bbnum 0+b^{:B})\oplus(a^{:A}+\bbnum 0)=a$.
\item (``Take $B$'') Discard all data of type $A$: $(a^{:A}+\bbnum 0)\oplus(\bbnum 0+b^{:B})=b$
and $(\bbnum 0+b^{:B})\oplus(a^{:A}+\bbnum 0)=b$.
\item (``Take first'') $x\oplus y$ discards $y$: $(a^{:A}+\bbnum 0)\oplus(\bbnum 0+b^{:B})=a$
and $(\bbnum 0+b^{:B})\oplus(a^{:A}+\bbnum 0)=b$.
\item (``Take last'') $x\oplus y$ discards $x$: $(a^{:A}+\bbnum 0)\oplus(\bbnum 0+b^{:B})=b$
and $(\bbnum 0+b^{:B})\oplus(a^{:A}+\bbnum 0)=a$.
\end{enumerate}
Does the semigroup law~(\ref{eq:associativity-law-semigroup}) hold
for the new typeclass instance with any of these implementations?
...
can have eight possible combinations of \lstinline!Left! and \lstinline!Right!
types. Each of them needs to be checked against each of the four implementations
of $\oplus$. Instead of doing $32$ separate derivations, let us
perform a quicker and intuitively clearer proof via case by case reasoning
in Scala syntax.
...
The same argument applies to the second implementation (``take $B$'').
Thus, both implementations are associative.
...
The associativity law holds for this operation because
\begin{align*}
 & e\triangleright\left(\left(f\oplus g\right)\oplus h\right)=\left(e\triangleright(f\oplus g)\right)\oplus_{A}h(e)=f(e)\oplus_{A}g(e)\oplus_{A}h(e)\quad.\\
 & e\triangleright\left(f\oplus\left(g\oplus h\right)\right)=f(e)\oplus_{A}\left(e\triangleright(g\oplus h)\right)=f(e)\oplus_{A}g(e)\oplus_{A}h(e)\quad.
\end{align*}
...
expression built up from primitive types and existing semigroups is
again a semigroup.
...
\[
\text{semigroupT}\triangleq\text{semigroupS}\left(\text{semigroupT}\right)\quad.
\]
...
semigroup $T$ by induction, in the same way as shown at the end of
Section~\ref{subsec:The-Eq-typeclass}.

\paragraph{Summary}

All type constructions preserve semigroups, so any type expression
whatsoever can have a \lstinline!Semigroup! instance. Since the typeclass
instances may have several inequivalent implementations, automatic
...
\]
The empty value for the monoid $A\times B$ is the pair of empty values
from the monoids $A$ and $B$,
\[
e_{A\times B}\triangleq e_{A}\times e_{B}\quad.
\]
The new binary operation is defined by Eq.~(\ref{eq:semigroup-product-operation-def})
...
with the appopriate changes to the code of \lstinline!monoidPair!,
...
A \lstinline!Monoid! instance can be implemented for \emph{any} type
expression built from primitive types, products, co-products, and
function types. For recursive types $T$ defined by a type equation
$T\triangleq S^{T}$, a \lstinline!Monoid! instance can be implemented
for any exponential-polynomial type constructor $S^{A}$.
...
\end{lstlisting}
Now we can implement instances of \lstinline!Pointed! for some functors:
\begin{lstlisting}
...
the functor's \lstinline!map! method and the \lstinline!pure! method.
Do these two methods need to be compatible in some way? If we ``wrap''
a value $123$ in a \lstinline!List! and then apply a \lstinline!.map(x => x + 1)!,
we expect to obtain a list containing $124$; any other result would
break our intuition about ``wrapping''. We can generalize this situation
to an arbitrary value $x^{:A}$ wrapped using \lstinline!pure! and
an arbitrary function $f^{:A\Rightarrow B}$ applied to the wrapped
value via \lstinline!map!: 
\begin{lstlisting}[mathescape=true]
pure(x).map(f)  // $\text{pu}_{F}(x)\triangleright \scriptstyle{f^{\uparrow F}}$
\end{lstlisting}
We now expect that the result should be the same as a wrapped $f(x)$.
This expectation can be formulated as a law,
\[
...
\end{lstlisting}
This law is called the \textbf{naturality law}\index{naturality law!of pure@of \texttt{pure}}
of \lstinline!pure!; it must hold for any $f^{:A\Rightarrow B}$.
Using the $\triangleright$-notation, we reformulate this law as $x\triangleright\text{pu}_{F}\triangleright f^{\uparrow F}=x\triangleright f\triangleright\text{pu}_{F}$
...
$x$, we can omit $x$ and write 
\begin{equation}
\text{pu}_{F}\bef f^{\uparrow F}=f\bef\text{pu}_{F}\quad.\label{eq:naturality-law-of-pure}
\end{equation}
\[
\xymatrix{\xyScaleY{2.0pc}\xyScaleX{5.0pc}A\ar[r]\sp(0.5){\text{pu}_{F}}\ar[d]\sb(0.45){f} & F^{A}\ar[d]\sp(0.45){\text{fmap}_{F}(f)}\\
B\ar[r]\sb(0.5){\text{pu}_{F}} & F^{B}
}
\]

This motivates the rigorous definition: A functor $F^{\bullet}$ is
\index{pointed functor}\textbf{pointed} if there exists a fully parametric
function $\text{pu}_{T}:\forall A.\,A\Rightarrow F^{A}$ satisfying
...
for any type $B$. That formula expresses the entire function $\text{pu}_{F}$
...
So far we have seen that if $\text{pu}_{F}$ satisfies the naturality
law then the single ``wrapped unit'' value of that function, $\text{pu}_{F}(1)$,
is sufficient to recover the entire function as
\[
\text{pu}_{F}=b\Rightarrow\text{pu}_{F}(1)\triangleright(\_\Rightarrow b)^{\uparrow F}\quad
\]
Now, if someone gives us just the ``wrapped unit'' value (denoted
$\text{wu}_{F}$) of type $F^{\bbnum 1}$, we can define a new function
$\text{pu}_{F}$ by
\begin{equation}
...
\end{align*}
This new function will satisfy $\text{pu}_{F}(1)=\text{wu}_{F}$ since
\begin{align*}
...
type L[A] = (F[A], G[A]) // Defined a type constructor L, now use it:
Pointed[L] }    // Not a valid Scala syntax.
\end{lstlisting}
...
to defining a nameless function: for instance, the pair functor is
defined as \lstinline!Lambda[X => (F[X], G[X])]!. Such type expressions
can be understood as nameless type-to-type functions. When using the
``kind projector'' plugin, the syntax for defining \lstinline!pointedPair!
is
\begin{lstlisting}
...
are two solutions: $\text{wu}_{L}=\text{wu}_{F}+\bbnum 0^{:G^{\bbnum 1}}$
and $\text{wu}_{L}=\bbnum 0^{:F^{\bbnum 1}}+\text{wu}_{G}$. Both
choices define $L^{\bullet}$ as a pointed functor. 
...
In general, when the type $F^{\bbnum 1}$ has several distinct values,
the choice of $\text{wu}_{F}$ is application-dependent. For example,
if $F^{A}=\text{List}^{A}$, the type $\text{List}^{\bbnum 1}$ has
values such as an empty list \lstinline!List[Unit]()!, a list with
a single \lstinline!Unit! value, a list with two \lstinline!Unit!
values, etc. All of these choices will give a \lstinline!Pointed!
typeclass instance to the \lstinline!List! functor. It is up to the
programmer to choose the \lstinline!Pointed! instance that will be
useful for a specific application. In the case of \lstinline!List!,
the standard choice \lstinline!wu == List(())! and correspondingly
\lstinline!pure(x) = List(x)! is motivated by the usage of the \lstinline!List!
type constructor to represent several possibilities, e.g.~in a search
problem; then the ``pure'' list represents the situation with only
one possibility.
...
no way we could have used a value of type $C^{\bbnum 1}$ because
we know nothing about the contrafunctor $C$. So, we have to set $\text{wu}_{L}\triangleq(\_\Rightarrow\text{wu}_{F})$,
...
used kind of bifunctors), a necessary and sufficient condition is
that the type $S^{\bbnum 1,\bbnum 0}$ should be non-void. If we can
create a value of type $S^{\bbnum 1,\bbnum 0}$ , constructions shown
in Section~\ref{subsec:Recursive-types-and} will guarantee that
we can also create a value of type $F^{\bbnum 1}$ and so the recursive
functor $F$ will be pointed.
...
\]
So, the \lstinline!pure! method of $F$ creates a non-empty list
with a single element $a^{:A}$.
...
whether $F$ is pointed with respect to $A$, and/or with respect
to $B$.
...
construct values of type $B$ using a given argument of type $\bbnum 2\Rightarrow\text{Int}$.
...
We may define the \lstinline!Copointed! typeclass as
\begin{lstlisting}
trait Copointed[F[_]] { def ex[A]: F[A] => A }
def extract[F[_]: Copointed, A](f: F[A]): A = implicitly[Copointed[F]].ex(f)
\end{lstlisting}
The \lstinline!extract! operation must be a fully parametric function
that obeys the \index{naturality law!of extract@of \texttt{extract}}naturality
law,
\begin{equation}
\text{ex}_{F}\bef f=f^{\uparrow F}\bef\text{ex}_{F}\quad.\label{eq:naturality-law-of-extract}
\end{equation}
\[
\xymatrix{\xyScaleY{2.0pc}\xyScaleX{5.0pc}F^{A}\ar[r]\sp(0.5){\text{ex}_{F}}\ar[d]\sb(0.45){\text{fmap}_{F}(f)} & A\ar[d]\sp(0.45){~f}\\
F^{B}\ar[r]\sb(0.5){\text{ex}_{F}} & B
}
\]
The naturality law formulates our expectation that the extractor function
somehow ``selects'' a value of type $A$ among all the values wrapped
by $F^{A}$, and the ``selection'' works independently of the values.
...
$A$ and then transformed it with $f$.
...
call a contrafunctor $C$ with a method \lstinline!pure! of type
$\forall A.\,C^{A}$ a \textbf{pointed contrafunctor}\index{pointed contrafunctor}.
Assuming that $C$ is pointed and denoting its \lstinline!pure! method
by $\text{pu}_{C}$, we can thus compute a value of type $P^{A}$
...
def copointedFunc[C[_]: Pointed, P[_]: Copointed]: Copointed[Lambda[X => C[X] => P[X]]] =
  new Copointed[Lambda[X => C[X] => P[X]]] {
    def ex[A]: (C[A] => P[A]) => A = h => extract[P, A](h(purec[C, A]))
  }      // In Scala 2.13: h => purec[C, A] pipe h pipe extract[P, A] as in $ h\Rightarrow\text{pu}_{C}\triangleright h\bef\text{ex}_{P} $
\end{lstlisting}
...
\begin{equation}
\text{pu}_{C}\triangleright f^{\downarrow C}=\text{pu}_{C}\quad\text{ or equivalently }\quad\text{contramap}_{C}(f^{:A\Rightarrow B})(\text{pu}_{C}^{:C^{B}})=\text{pu}_{C}^{:C^{A}}\quad.\label{eq:naturality-law-for-pure-for-contrafunctors}
\end{equation}
\[
\xymatrix{\xyScaleY{2.0pc}\xyScaleX{3.0pc}\text{pu}_{C}:C^{B}\ar[d]\sb(0.45){\text{contramap}_{C}(f)} & B\\
\text{pu}_{C}:C^{A} & A\ar[u]\sp(0.45){~f}
}
\]
...
We now apply structural analysis to pointed contrafunctors, similarly
to Section~\ref{subsec:f-Contrafunctor-constructions}.
...
\begin{lstlisting}
def pointedFuncFC[C[_]: Pointed, F[_]]: Pointed[Lambda[X => F[X] => C[X]]] =       Pointed[Lambda[X => F[X] => C[X]]](_ => implicitly[Pointed[C]].wu)
\end{lstlisting}
...
seen in several cases (e.g.~the coproduct construction for monoids
or pointed functors).
...
In this way, we have reduced the monoid instance defined by \lstinline!emptyRoute!
and \lstinline!combineRoutes! to a trivial semigroup, a co-product
construction, and a function-type construction for monoids. Since
all those constructions are guaranteed to produce lawful monoids,
...
Using the \texttt{cats} library, define a \lstinline!Bifunctor! instance
for $Q^{X,Y}\triangleq X+X\times Y$.
...
Since the typeclass method has type parameters, it means that the
instance value will have type
\[
...
Define \lstinline!Functor! instance for recursive type constructor
...
Assuming that $F^{A}$ and $G^{A}$ are functors, define a \lstinline!Functor!
instance for $F^{A}+G^{A}$ using a function parameterized by the
type constructors $F$ and $G$.
...
\textbf{(a)} Use a function parameterized by a type constructor $C$,
assumed to be a contrafunctor, that computes a value of type $C^{A\times B}$
given a value of type $C^{A}+C^{B}$, for arbitrary types $A$, $B$.
Show that the inverse transformation, $C^{A\times B}\Rightarrow C^{A}+C^{B}$,
is not implementable for some contrafunctors $C$. (Find an explicit
example of such a contrafunctor $C$.)

\textbf{(b)} Use a function parameterized by a type constructor $F$,
assumed to be a functor, that computes a value of type $F^{A}\times F^{B}$
given a value of type $F^{A\times B}$, for arbitrary types $A$,
$B$. Show that the inverse transformation, $F^{A}\times F^{B}\Rightarrow F^{A\times B}$,
is not implementable for some functors $F$.
...
If we lift these functions to the contrafunctor $C$, we will obtain
the functions $\nabla_{1}^{\downarrow C}:C^{A}\Rightarrow C^{A\times B}$
and $\nabla_{2}^{\downarrow C}:C^{B}\Rightarrow C^{A\times B}$. The
required type signature is then implemented using a \lstinline!match!
expression,
\begin{lstlisting}
def f[C[_]: Contrafunctor, A, B]: Either[C[A], C[B]] => C[(A, B)] = {
...
Let us now try implementing a function $g$ with the converse type
signature,
\[
g:\forall(A,B).\,C^{A\times B}\Rightarrow C^{A}+C^{B}\quad,
\]
for the simple contrafunctor $C^{A}\triangleq A\Rightarrow R$, where
$R$ is a fixed type:
\[
\forall(A,B).\,(A\times B\Rightarrow R)\Rightarrow(A\Rightarrow R)+(B\Rightarrow R)\quad.
\]
We need to choose whether this function returns values of type $A\Rightarrow R$
or $B\Rightarrow R$. Can we compute a value of type $A\Rightarrow R$
given a value of type $A\times B\Rightarrow R$?
\[
...
a value of type $A$. So, we find that the typed hole $\text{???}^{:R}$
cannot be filled. 
...
\]
We need to relate the types $A\times B$, $A$, and $B$ by the functions
...
If we try implementing a function $g$ with the converse type signature,
\begin{equation}
...
and trying various combinations, we find the functor $F^{A}\triangleq(P\Rightarrow A)+(Q\Rightarrow A)$
as a co-product of two function-type functors. For this functor, the
...
\]
which can be transformed equivalently to a disjunction of four cases,
\[
...
can be implemented are called ``applicative\index{applicative functor}''
(see Chapter~\ref{chap:8-Applicative-functors,-contrafunc}). The
functor $F$ is an example of a functor that is not applicative.
...
def p[F[_]: Functor : Pointed, A, B]: Either[A, F[B]] => F[Either[A, B]] = {
  case Left(a)     => implicitly[Pointed[F]].pure(Left[A, B](a))
...
\end{array}\quad.
\]
The only remaining difference is in the second line of the matrices.
...
Define a \lstinline!Functor! instance for the recursive type $Q^{A}\triangleq\text{String}+A\times A\times Q^{A}$.
...
equivalent to a value $\text{pu}_{C}:\forall A.\,C^{A}$ satisfying
...
Assuming that $F^{A}$ and $G^{A}$ are functors, define a \lstinline!Functor!
instance for $F^{A}\times G^{A}$. Use a function parameterized by
the type constructors $F$ and $G$.

\subsubsection{Exercise \label{subsec:tc-Exercise-10}\ref{subsec:tc-Exercise-10}}

Define a \lstinline!Functor! instance for $F^{A}\Rightarrow G^{A}$
where $F^{A}$ is a contrafunctor and $G^{A}$ is a functor. Implement
that as a function parameterized by the type constructors $F$ and
$G$. For the contrafunctor $F$, use either the \lstinline!Contrafunctor!
...
\textbf{(a)} Use a function parameterized by a type constructor $F$,
assumed to be a functor, that computes a value of type $F^{A+B}$
given values of type $F^{A}$ and $F^{B}$, for arbitrary types $A$,
$B$. Show that the inverse transformation, $F^{A+B}\Rightarrow F^{A}\times F^{B}$,
is not implementable for some functors $F$. (Find an explicit example
of such a functor $F$.)

\textbf{(b)} Use a function parameterized by a type constructor $C$,
assumed to be a contrafunctor, that computes a value of type $C^{A}\times C^{B}$
given a value of type $C^{A+B}$, for arbitrary types $A$, $B$.
Show that the inverse transformation, $C^{A}\times C^{B}\Rightarrow C^{A+B}$,
is not implementable for some contrafunctors $C$.
...
Use a function parameterized by a functor $F$ to compute a value
of type $A\Rightarrow F^{B}$ from a given value of type $F^{A\Rightarrow B}$,
for arbitrary types $A$, $B$. Show that the inverse transformation,
$(A\Rightarrow F^{B})\Rightarrow F^{A\Rightarrow B}$, cannot be implemented
for some functors $F$. (Functors supporting that transformation are
called ``rigid''; see Section~\ref{subsec:Rigid-functors}.)
...
\begin{lstlisting}
      // For any value f: F[(Unit, B)], first compute
val (a1, b1) = q[Unit, B, F](f)._1
      // Then we must have:    a1 == ()   and   b1 == f.map { case (_, b) => b }

      // For any value g: F[(A, B, C)], first compute
...
\subsection{Recursive types and recursive values\label{subsec:Recursive-types-and}}

A recursive type is defined by a type equation such as $T\triangleq\bbnum 1+\text{Int}+T\times T$.
A recursive value (usually a function) is defined by a formula {*}{*}{*}

There must be a clearly defined base case and inductive step must
be clearly defined.
...
We have seen examples of typeclasses that have a similar form: {*}{*}{*}
and similar properties.

A typeclass is called \textbf{inductive}\index{inductive typeclass}
if instances for a type $A$ are values of type $P^{A}\Rightarrow A$
with some functor $P$. The value of type $P^{A}\Rightarrow A$ represents
all the methods of the typeclass. We can implement this definition
by the Scala code
\begin{lstlisting}
...
instances have type $\bbnum 1+A\times A\Rightarrow A$ {*}{*}{*}can
be 

Product of two inductive typeclasses always has an instance:
\[
(P^{A}\Rightarrow A)\times(P^{B}\Rightarrow B)\Rightarrow P^{A\times B}\Rightarrow A\times B\quad.
\]

Recursive type construction works for inductive typeclasses. If $T\triangleq S^{T}$
then assuming we can implement $f:(P^{A}\Rightarrow A)\Rightarrow P^{S^{A}}\Rightarrow S^{A}$,
we define 
\[
x^{:P^{T}\Rightarrow T}\triangleq p^{:P^{T}}\Rightarrow f(x)(p)\quad.
\]
This recursive definition is valid because we can convert between
the equivalent types $T$ and $S^{T}$ whenever necessary, so $p^{:P^{T}}$
can be converted to a value of type $P^{S^{T}}$, while the value
$f(x)(p)$ can be converted from type $S^{T}$ to type $T$. In Scala
code, the conversions between $T$ and $S^{T}$ are implemented by
the constructor and accessor methods of the case class that wraps
the type $T$.

The typeclasses \lstinline!HasDefault!, \lstinline!Semigroup!, and
\lstinline!Monoid! are inductive because they can be written as
\[
\text{HasDefault}^{A}\cong\bbnum 1\Rightarrow A\quad,\quad\quad\text{Semigroup}^{A}\cong A\times A\Rightarrow A\quad,\quad\quad\text{Monoid}^{A}\cong\left(\bbnum 1+A\times A\right)\Rightarrow A\quad.
\]
We can implement these typeclasses via the general \lstinline!InductiveTypeclass!.
For example, to implement \lstinline!Semigroup!:
\begin{lstlisting}
...
{*}{*}{*}The \lstinline!Functor! and \lstinline!Pointed! typeclasses
are inductive if we look at the level of type constructors (higher-order
functions on types). 

The \lstinline!Extractor! and \lstinline!Copointed! typeclasses
are \textbf{co-inductive}, because they have the form $A\Rightarrow S^{A}$.
Co-product works for co-inductive typeclasses, rather than the product
construction. The \lstinline!Eq! typeclass is neither inductive nor
co-inductive.
