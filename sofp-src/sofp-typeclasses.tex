...
\[
\forall(A,B,Z).\,\text{Extractor}^{A}\Rightarrow\text{Extractor}^{A\times B}\quad.
\]
\begin{lstlisting}
...
\[
f^{:A\Rightarrow Z}\times g^{:B\Rightarrow Z}\Rightarrow\begin{array}{|c||c|}
 & Z\\
...
for $T$ by a recursive function,
\[
\text{extractorT}\triangleq\text{extractorS}(\text{extractorT})\quad.
\]
\begin{lstlisting}
...
scala> extractorT.extract(t) // The recursion terminates.
res0: String = abc
\end{lstlisting}
...
\lstinline!Eq!. We also define \lstinline!===! as an extension method:
\begin{lstlisting}
...
the laws of reflexivity and transitivity. The law of \textbf{transitivity}\index{transitivity law}
states that $x=y$ and $y=z$ entails $x=z$. The law of \textbf{reflexivity}\index{reflexivity law}
...
violates this law when we choose $x\triangleq n^{:\text{Int}}\Rightarrow n$.

Let us perform structural analysis for the \lstinline!Eq! typeclass.
The results will show which types can be meaningfully compared for
equality.
...
\end{lstlisting}
It is easy to check that the reflexivity and transitivity laws hold
for the new comparison operation if they hold for comparisons of $A$
and $B$ separately.
...
\[
f^{:A\times A\Rightarrow A}\times g^{:B\times B\Rightarrow B}\Rightarrow a_{1}^{:A}\times b_{1}^{:B}\times a_{2}^{:A}\times b_{2}^{:B}\Rightarrow???^{:A}\times???^{:B}\quad.
\]
...
\[
f^{:A\times A\Rightarrow A}\times g^{:B\times B\Rightarrow B}\Rightarrow c^{:\left(A+B\right)\times\left(A+B\right)}\Rightarrow\text{???}^{:A+B}\quad.
\]
...
the last rows of the matrix leave us no choice and can be filled first,
\[
...
The difference is in how the binary operation works on two different
parts of the disjunction, $a^{:A}+\bbnum 0$ and $\bbnum 0+b^{:B}$.
The possibilities are:
\begin{enumerate}
\item (``Take $A$'') Result is always of type $A$: $(a^{:A}+\bbnum 0)\oplus(\bbnum 0+b^{:B})=a$
and $(\bbnum 0+b^{:B})\oplus(a^{:A}+\bbnum 0)=a$.
\item (``Take left'') $x\oplus y$ discards $y$: $(a^{:A}+\bbnum 0)\oplus(\bbnum 0+b^{:B})=a$
and $(\bbnum 0+b^{:B})\oplus(a^{:A}+\bbnum 0)=b$.
\item (``Take right'') $x\oplus y$ discards $x$: $(a^{:A}+\bbnum 0)\oplus(\bbnum 0+b^{:B})=b$
and $(\bbnum 0+b^{:B})\oplus(a^{:A}+\bbnum 0)=a$.
\item (``Take $B$'') Result is always of type $B$: $(a^{:A}+\bbnum 0)\oplus(\bbnum 0+b^{:B})=b$
and $(\bbnum 0+b^{:B})\oplus(a^{:A}+\bbnum 0)=b$.
\end{enumerate}
To verify the associativity law, we need to show that the values of
expressions such as $(a_{1}+\bbnum 0)\oplus(a_{2}+\bbnum 0)\oplus(\bbnum 0+b_{1})$
or $(a_{1}+\bbnum 0)\oplus(\bbnum 0+b_{1})\oplus(\bbnum 0+b_{2})$,
etc., does not depend on the order of inserted parentheses.

First consider the case when all three values are of type $A+\bbnum 0$;
then the binary operation reduces to the binary operation of the semigroup
$A$:
\[
(a_{1}+\bbnum 0)\oplus(a_{2}+\bbnum 0)\oplus(a_{3}+\bbnum 0)=(a_{1}\oplus_{A}a_{2}\oplus_{A}a_{3})+\bbnum 0\quad,
\]
which is associative since we assume that $A$ is a lawful semigroup.
The same argument applies to three values of type $\bbnum 0+B$. It
remains to consider the ``mixed'' cases. {*}{*}{*}
...
Define \lstinline!Functor! instance for recursive type $Q^{A}\equiv\left(\text{Int}\Rightarrow A\right)+\text{Int}+Q^{A}$
...
class having \texttt{\textcolor{blue}{\footnotesize{}xmap}}:

\texttt{\textcolor{blue}{\footnotesize{}def xmap{[}A, B{]}(f: A $\Rightarrow$
...
where $F^{A}$ is a contrafunctor (use Cats' \texttt{\textcolor{blue}{\footnotesize{}Contravariant}}
