...
constraint}\index{typeclass!constraint}. 
...
functions (VTFs) are known as \textbf{dependent} types\index{dependent type}
...
types. This set is called the \textbf{type domain}\index{type domain}
...
\end{lstlisting}
The value \lstinline!frac:Frac[T]! is called a \index{typeclass!instance}\textbf{typeclass
...
In this way, we implemented the typeclass constraint in the PTVF \lstinline!avg[T]!.
The main steps were:
\begin{enumerate}
...
types. However, this book defines the typeclass \lstinline!Pointed!
for pointed \emph{functors} (Section~\ref{subsec:Pointed-functors:-motivation})
...
of this construction are $Z\times B$ and $P\times Q\times Z$ (where,
as we know, the type $Z$ has an \lstinline!Extractor! instance).
...
\subsection{The \texttt{Eq} typeclass}
...
of type $R$. Could we define comparisons of functions only via their
values at $r_{1}$?
\begin{lstlisting}
...
in code as an expanded function.
...
is to show that the function type $R\Rightarrow A$ is a lawful monoid
...
It is sufficient for only $F^{\bullet}$ to be a pointed functor;
$\text{wu}_{L}\triangleq\text{wu}_{F}+\bbnum 0^{:G^{\bbnum 1}}$ then
provides a \lstinline!Pointed! typeclass instance for $F^{\bullet}+G^{\bullet}$,
...
must be defined as a class or a trait. Defining \lstinline!F! as
a type alias would make the Scala compiler confused: if the type \lstinline!F[T]!
were \emph{the same} as \lstinline!Seq[Try[T]]!, the expression \lstinline!Seq(Try(1)).map(_ * 10)!
would mean to apply \lstinline!Seq!'s predefined \lstinline!.map!
method rather than the extension method \lstinline!.map! of the type
constructor \lstinline!F! defined via the Functor typeclass.
...
(a) Use $C^{A}\triangleq A\Rightarrow R$ where $R$ is a fixed type,
to show impossibility. 

(b) Use $F^{A}\triangleq(P\Rightarrow A)+(Q\Rightarrow A)$ to show
impossibility. Note that $F^{A}\triangleq P\Rightarrow A$ does not
provide a counterexample.

{*}{*}{*}

\begin{comment}
Sorry, I made several mistakes in my examples, it\textquoteright s
hard to get right. So I repeated all calculations again, and now I
think that actually neither of the two identities holds. We do not
have identities but only maps going in one direction, namely, we can
implement C{[}A{]} | C{[}B{]} => C{[}A \& B{]} and C{[}A | B{]} =>
C{[}A{]} \& C{[}B{]}. The opposite directions cannot be implemented.
Here are counterexamples for the impossibility to implement these
maps. The map C{[}A \& B{]} => C{[}A{]} | C{[}B{]} cannot be implemented
for C{[}A{]} = A => R. The required type signature is ((A, B) => R)
=> Either{[}A => R, B => R{]}. We cannot implement this signature,
because we are given a function of type (A, B) => R, which requires
both arguments A and B, and we cannot produce a function A => R that
accepts just one argument. The map C{[}A{]} \& C{[}B{]} => C{[}A |
B{]} cannot be implemented for C{[}A{]} = Either{[}A => R, A => S{]}.
(This more complicated contravariant functor C is necessary because
the simpler example C{[}A{]} = A => R does not provide a counterexample.)
The required type signature is(Either{[}A => R, A => S{]}, Either{[}B
=> R, B => S{]}) => Either{[}Either{[}A, B{]} => R, Either{[}A, B{]}
=> S{]}. This type signature cannot be implemented because we are
required to accept any combination of Either values, for example Left{[}A
=> R{]} and Right{[}B => S{]}. If we are given these values, we cannot
produce Either{[}A, B{]} => R because there is no way of converting
B to R, we only have a function B => S. For the same reason, we cannot
produce Either{[}A, B{]} => S.
\end{comment}
...
\begin{comment}
The map C{[}A{]} \& C{[}B{]} => C{[}A | B{]} cannot be implemented
for C{[}A{]} = Either{[}A => R, A => S{]}. (This more complicated
contravariant functor C is necessary because the simpler example C{[}A{]}
= A => R does not provide a counterexample.) 
\end{comment}
