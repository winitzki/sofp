...
functional\index{purely functional!program} programs. We will now
...
We may also use a ``named unit\index{unit type!named}'', e.g.,
...
as a ``row vector'' $\,\begin{array}{|cc|}
...
A useful tool for calculations is the pipe\index{pipe notation} operation,
...
composition operation ($\bef$).\index{pipe notation!operator precedence}.
...
The ``diagonal'' function $\Delta$ is a right inverse for $\pi_{1}$
and $\pi_{2}$:
...
\begin{align*}
\text{expect to equal }\text{id}:\quad & \text{id}^{\uparrow L}=a^{:A}\times p^{:F^{A}}\rightarrow\text{id}\,(a)\times(p\triangleright\text{id}^{\uparrow F})\quad.
\end{align*}
