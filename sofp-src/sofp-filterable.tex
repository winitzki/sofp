...
operation simply removes all data that fails the filtering condition.
What about the custom data type \lstinline!Orders! from Example~\ref{subsec:Example-filtering-orders-tue-fri}?
...
despite the filtering condition, ``\lstinline!if x < 1000!''. This
happened because the code of \lstinline!filt! did not remove the
value \lstinline!x = 2000! from the data structure even though it
...
function in a special way when the type parameter $A$ is set to,
...
\end{lstlisting}
This implementation can be reduced to general type constructions for
filterable functors, so the filtering laws hold. Intuitively, we expect
this to be the case because the \lstinline!filter! function removes
values that fail the predicate \lstinline!p! from the sequence. The
filtering function also removes other values that may or may not fail
the predicate, but the filtering laws allow removing \emph{more} values.
...
\begin{lstlisting}
type F[T] = (Option[T], Option[(T, T)])
def filter[T](p: T => Boolean): F[T] => F[T] = {
  case (t, None)             => (t.filter(p), None)
  case (t, Some((t1, t2)))   => (t.filter(p), if (p(t1) && p(t2)) Some((t1, t2)) else None)
}
\end{lstlisting}
...
\lstinline!n! for counting the number of data items \emph{removed}
by filtering:
\begin{lstlisting}
...
We will study filterable contrafunctors in more detail later in this
chapter. For now, we simply ignore the fact that $C^{Z,A}$ is a contrafunctor
in $A$, and try implementing a \lstinline!filter! function with
the standard type signature
\begin{lstlisting}
type C[A] = A => Option[Z]       // The type `Z` must be already defined.
def filter[A](p: A => Boolean)(c: C[A]): C[A] = { a: A => ??? }
\end{lstlisting}
We need to implement a function of type $A\rightarrow\bbnum 1+Z$,
trying to preserve information as much as possible. We could return
\lstinline!None! for all arguments, but that implementation would
lose information. Instead, we may compute \lstinline!c(a)! and obtain
a value of type $\bbnum 1+Z$. If the predicate fails on the input,
we should not use the value $A$; the function of type $A\rightarrow\bbnum 1+Z$
that does not use its argument must always return $1+\bbnum 0^{:Z}$
since $Z$ is a type parameter. So, the implementation is
\begin{lstlisting}
def filter[A](p: A => Boolean)(c: C[A]): C[A] = { a: A => if (p(a)) c(a) else None }
\end{lstlisting}
...
\subsection{Simplifying the filtering laws: Motivation for \texttt{deflate}}
...
\begin{empheq}[box=\mymathbgbox]{align*}
 ...
 & \xymatrix{\xyScaleX{5pc}F^{A}\ar[r]\sp(0.45){\text{inflate}} & F^{1+A}\ar[r]\sp(0.55){\left(\text{filt}_{\text{Opt}}(p)\right)^{\uparrow F}} & F^{1+A}\ar[r]\sp(0.55){\text{deflate}} & F^{A}}
\end{empheq}
...
\begin{empheq}[box=\mymathbgbox]{align}
 & \xymatrix{\xyScaleX{4pc}F^{A}\ar[r]\sp(0.5){\psi_{p}^{\uparrow F}} & F^{1+A}\ar[r]\sp(0.55){\text{deflate}} & F^{A}}
...
def deflate[F[_]: Filterable : Functor,A]:
  ...
  // _.get is a partial function $\scriptstyle{\bbnum1+x^{:A}\rightarrow x}$
\end{lstlisting}
...
\noindent \vspace{-1.6\baselineskip}
\begin{empheq}[box=\mymathbgbox]{align}
 & \text{deflate}:\xymatrix{\xyScaleX{4.5pc}F^{1+A}\ar[r]\sb(0.5){\text{filt}_{F}(\text{nonEmpty)}} & F^{1+A}\ar[r]\sb(0.5){\text{get}^{\uparrow F}} & F^{A}}
\nonumber \\
...
being applied to an empty \lstinline!Option! value. Because of this,
...
into the equation illustrated by the following diagram: 
\begin{empheq}[box=\mymathbgbox]{equation*}
...
{\color{greenunder}\text{naturality law~(\ref{eq:naturality-law-of-filter}) of }\text{filt}_{F}:}\quad & =\text{filt}_{F}(f^{\uparrow\text{Opt}}\bef\text{nonEmpty})\bef f^{\uparrow\text{Opt}\uparrow F}\bef\text{get}^{\uparrow F}\quad.\\
...
\begin{comment}
Remove this page break if necessary.
\end{comment}
\pagebreak{}
...
initial \lstinline!liftOpt!.

Proof for \textbf{(a)} directly derives the formula \lstinline!deflate!$^{\prime}=$\lstinline!deflate!
by this calculation:
\begin{empheq}[box=\mymathbgbox]{align*}
...
let us derive a suitable law for \lstinline!liftOpt! for any $h^{:A\rightarrow B}$,
...
\vspace{-2\baselineskip}
\begin{empheq}[box=\mymathbgbox]{equation*}
\xymatrix{\xyScaleY{1.4pc}\xyScaleX{4.5pc}F^{A}\ar[d]\sb(0.5){h^{\uparrow F}}\ar[rd]\sp(0.5){~~\text{ liftOpt}\,(h\bef f)}\\
...
\noindent It follows that \emph{in case} \lstinline!liftOpt! were
defined via \lstinline!deflate! then \lstinline!liftOpt! would automatically
...
In other words, \lstinline!deflate! has only $3$ laws while \lstinline!filter!
...
Begin by expressing \lstinline!filter! through \lstinline!deflate!
and then finally through \lstinline!liftOpt!:
\begin{empheq}[box=\mymathbgbox]{equation}
\text{filt}_{F}(p^{:A\rightarrow\bbnum 2})=\psi_{p}^{\uparrow F}\bef\text{deflate}=\text{liftOpt}\,(\psi_{p})\quad.\label{eq:filter-through-liftOpt}
\end{empheq}

{*}{*}{*}The logic is wrong here: we cannot assume that the naturality
law holds for filter and then derive that same assumption.

{*}{*}{*} Plan: 1) derive a second naturality law for liftOpt from
naturality of deflate. 2) derive the naturality law of filter from
naturality of liftopt. 3) statement 8.2.3.3. 4) derive the naturality
law of liftopt from composition law of liftopt.

Translate the naturality law~(\ref{eq:naturality-law-of-filter})
for \lstinline!filter! into the corresponding law for \lstinline!liftOpt!:
\begin{empheq}[box=\mymathbgbox]{align*}
{\color{greenunder}\text{left-hand side}:}\quad & f^{\uparrow F}\bef\text{filt}_{F}(p)=f^{\uparrow F}\bef\psi_{p}^{\uparrow F}\bef\text{deflate}=\text{liftOpt}\,(f\bef\psi_{p})\quad.\\
{\color{greenunder}\text{right-hand side}:}\quad & \text{filt}_{F}(f\bef p)\bef f^{\uparrow F}=\psi_{f\bef p}^{\uparrow F}\bef\text{deflate}\bef f^{\uparrow F}\\
{\color{greenunder}\text{naturality law~(\ref{eq:naturality-law-of-deflate}) of }\text{deflate}:}\quad & \quad=\psi_{f\bef p}^{\uparrow F}\bef f^{\uparrow\text{Opt}\uparrow F}\bef\text{deflate}\\
 & \quad=\big(\psi_{f\bef p}\bef f^{\uparrow\text{Opt}}\big)^{\uparrow F}\bef\text{deflate}=\text{liftOpt}\,(\psi_{f\bef p}\bef f^{\uparrow\text{Opt}})\quad.
\end{empheq}
We are allowed to use the naturality law~(\ref{eq:naturality-law-of-deflate})
for \lstinline!deflate! because we assume that the naturality law
holds for \lstinline!filter! (Statement~\ref{subsec:Statement-naturality-law-of-deflate-from-filter}).

It remains to show that, for any $f^{:A\rightarrow B}$ and $p^{:B\rightarrow\bbnum 2}$,
\begin{empheq}[box=\mymathbgbox]{equation}
f\bef\psi_{p}=\psi_{f\bef p}\bef f^{\uparrow\text{Opt}}\quad.\label{eq:naturality-law-of-psi}
\end{empheq}
To derive this equation, which can be viewed as a naturality law specific
to the function $\psi$, it is convenient to use the definition of
$\psi$ that represents $p$ as a function $A\rightarrow\bbnum 1+\bbnum 1\cong\text{Opt}^{\bbnum 1}$,
\begin{empheq}[box=\mymathbgbox]{equation}
...
\end{empheq}
Using this formula, we can derive Eq.~(\ref{eq:naturality-law-of-psi})
...
\end{empheq}
The two sides of Eq.~(\ref{eq:naturality-law-of-psi}) are equal
to the same expression. This proves the following statement:

\subsubsection{Statement \label{subsec:Statement-naturality-of-liftOpt}\ref{subsec:Statement-naturality-of-liftOpt}}

If \lstinline!liftOpt! obeys its naturality law~(\ref{eq:naturality-law-of-liftOpt})
and \lstinline!filter! is defined via Eq.~(\ref{eq:filter-through-liftOpt})
then \lstinline!filter! also obeys its naturality law~(\ref{eq:naturality-law-of-filter}).

The converse statement also holds if we define \lstinline!liftOpt!
through \lstinline!filter! by
\begin{empheq}[box=\mymathbgbox]{equation}
...
\subsubsection{Statement \label{subsec:Statement-naturality-of-liftOpt-1}\ref{subsec:Statement-naturality-of-liftOpt-1}}

If \lstinline!liftOpt! is defined via Eq.~(\ref{eq:def-liftOpt-via-filter})
then \lstinline!liftOpt! obeys its naturality law~(\ref{eq:naturality-law-of-liftOpt}).

\subparagraph{Proof}

To show that the two sides of the naturality law~(\ref{eq:naturality-law-of-liftOpt})
are equal, compute:
\begin{empheq}[box=\mymathbgbox]{align*}
{\color{greenunder}\text{expect to equal }\text{liftOpt}\,(h\bef f):}\quad & h^{\uparrow F}\bef\text{liftOpt}\,(f)=\gunderline{h^{\uparrow F}\bef f^{\uparrow F}}\bef\text{filt}_{F}(\psi_{\text{nonEmpty}})\\
{\color{greenunder}\text{composition law of }F:}\quad & =(h\bef f)^{\uparrow F}\bef\text{filt}_{F}(\psi_{\text{nonEmpty}})=\text{liftOpt}\,(h\bef f)\quad.
\end{empheq}
\begin{comment}
\hfill{}\rule{0.33\columnwidth}{0.33pt}\hfill{} This (as a QED
symbol) doesn't seem to work well visually.
\end{comment}

Let us now translate \lstinline!filter!'s identity law~(\ref{eq:identity-law-of-filter})
into the corresponding identity law for \lstinline!liftOpt!. Begin
by expressing $\text{filt}_{F}(\_\rightarrow\text{true})$ via \lstinline!liftOpt!
using Eq.~(\ref{eq:filter-through-liftOpt}):
\begin{empheq}[box=\mymathbgbox]{equation*}
...
\subsubsection{Statement \label{subsec:Statement-composition-law-of-liftOpt}\ref{subsec:Statement-composition-law-of-liftOpt}}
...
also obeys its identity law~(\ref{eq:identity-law-of-liftOpt}).

\textbf{(b)} If \lstinline!liftOpt! obeys its identity law~(\ref{eq:identity-law-of-liftOpt})
and \lstinline!filter! is defined via Eq.~(\ref{eq:filter-through-liftOpt})
then \lstinline!filter! also obeys its identity law~(\ref{eq:identity-law-of-filter})
and naturality law~(\ref{eq:naturality-law-of-filter}).
...
\begin{empheq}[box=\mymathbgbox]{align*}
{\color{greenunder}\text{use Eq.~(\ref{eq:filter-through-liftOpt})}:}\quad & \text{filt}_{F}(\_\rightarrow\text{true})=\text{liftOpt}\,(\psi_{(\_\rightarrow\text{true})})\\
...
\end{empheq}
The naturality law~(\ref{eq:naturality-law-of-filter}) holds for
\lstinline!filter! because it is defined via \lstinline!liftOpt!
(Statement~\ref{subsec:Statement-naturality-of-liftOpt}).

It remains to translate \lstinline!filter!'s composition law~(\ref{eq:composition-law-of-filter})
into a corresponding law for \lstinline!liftOpt!. That law needs
to combine two predicates $p_{1}$ and $p_{2}$ into a new predicate
$p(x)\triangleq p_{1}(x)\wedge p_{2}(x)$. As Eq.~(\ref{eq:filter-through-liftOpt})
shows, \lstinline!liftOpt! uses a predicate $p$ only through the
function $\psi_{p}$. So, we will derive the composition law for \lstinline!liftOpt!
if we somehow express $\psi_{p}$ as a combination of $\psi_{p_{1}}$
and $\psi_{p_{2}}$. Begin by writing
\begin{lstlisting}
...
psi(p) == x => Some(x).filter(p1).map { y => Some(y).filter(p2) }.flatten
       ...
       == psi(p1) andThen (_.flatMap(psi(p2)))
\end{lstlisting}
Denote this combination of the functions $\psi_{p_{1}}$ and $\psi_{p_{2}}$
by the symbol $\diamond_{\text{Opt}}$, so that we may write
\begin{empheq}[box=\mymathbgbox]{equation*}
\psi_{p}=\psi_{p_{1}}\diamond_{\text{Opt}}\psi_{p_{2}}\triangleq x^{:A}\rightarrow x\triangleright\psi_{p_{1}}\triangleright\text{flatMap}\,(\psi_{p_{2}})=\psi_{p_{1}}\bef(y\rightarrow y\triangleright\text{flatMap}\,(\psi_{p_{2}}))\quad.
\end{empheq}
The operation $\diamond_{\text{Opt}}$ is a special kind of composition
...
and cannot be composed with the ordinary function composition ($\psi_{p_{1}}\bef\psi_{p_{2}}$
does not type-check). It is straightforward to extend the operation
$\diamond_{\text{Opt}}$ from functions of type $A\rightarrow\bbnum 1+A$
to functions with more general types $A\rightarrow\bbnum 1+B$ and
$B\rightarrow\bbnum 1+C$:
\begin{lstlisting}
def kleisliOpt[A, B](f: A => Option[B], g: B => Option[C]): A => Option[C] =
  { x: A => f(x).flatMap(g) }
\end{lstlisting}
\begin{empheq}[box=\mymathbgbox]{equation*}
f^{:A\rightarrow\bbnum 1+B}\diamond_{\text{Opt}}g^{:B\rightarrow\bbnum 1+C}\triangleq x^{:A}\rightarrow f(x)\triangleright\text{flatMap}\,(g)\quad.
\end{empheq}
The Kleisli composition $f\diamond_{\text{Opt}}g$ gives a function
of type $A\rightarrow\bbnum 1+C$ and is similar to the ordinary composition
$f\bef g$ except for using functions with ``twisted'' types, $A\rightarrow\bbnum 1+B$
instead of $A\rightarrow B$. (The composition $f\bef g$ is not defined
for the ``twisted'' functions because their types do not match.)

We can now proceed to derive the composition law for \lstinline!liftOpt!
beginning from Eq.~(\ref{eq:composition-law-of-filter}):
\begin{empheq}[box=\mymathbgbox]{align*}
...
{\color{greenunder}\text{right-hand side}:}\quad & \text{filt}_{F}(p)=\text{liftOpt}\,(\psi_{p})=\text{liftOpt}\,(\psi_{p_{1}}\diamond_{\text{Opt}}\psi_{p_{2}})\quad.
\end{empheq}
If \lstinline!filter!'s composition law holds, we obtain (without
any additional assumptions) the equation
\begin{empheq}[box=\mymathbgbox]{equation*}
\text{liftOpt}\,(\psi_{p_{1}})\bef\text{liftOpt}\,(\psi_{p_{2}})=\text{liftOpt}\,(\psi_{p_{1}}\diamond_{\text{Opt}}\psi_{p_{2}})\quad.
\end{empheq}
This looks like a composition law typical for a ``lifting'' transformation:
the composition of lifted functions is equal to the lifted Kleisli
composition. So, it appears useful to formulate the composition law
of \lstinline!liftOpt! in a more general way by allowing arbitrary
functions $f^{:A\rightarrow\bbnum 1+B}$ and $g^{:B\rightarrow\bbnum 1+C}$
instead of specific functions $\psi_{p_{1}}$ and $\psi_{p_{2}}$.
The composition law of \lstinline!liftOpt! is then written as\index{composition law!of liftOpt@of \texttt{liftOpt}}
\begin{empheq}[box=\mymathbgbox]{equation}
\text{liftOpt}\,(f^{:A\rightarrow\bbnum 1+B})\bef\text{liftOpt}\,(g^{:B\rightarrow\bbnum 1+C})=\text{liftOpt}\,(f\diamond_{\text{Opt}}g)\quad.\label{eq:composition-law-of-liftOpt}
\end{empheq}
Because this law allows arbitrary functions $f$ and $g$, it is stronger
than \lstinline!filter!'s composition law.

\subsubsection{Statement \label{subsec:Statement-composition-law-of-liftOpt-covers-two-laws-of-filter}\ref{subsec:Statement-composition-law-of-liftOpt-covers-two-laws-of-filter}}

If the composition law~(\ref{eq:composition-law-of-liftOpt}) holds
for \lstinline!liftOpt!, the 
...
\textbf{Identity law} (covers old law 3): 
\begin{empheq}[box=\mymathbgbox]{equation*}
\text{fmapOpt}\,(\text{id}_{\diamond_{\text{Opt}}}^{A\rightarrow1+A})=\text{id}^{F^{A}\rightarrow F^{A}}
\end{empheq}

\textbf{Composition law} (covers old laws 1 and 2): 
\begin{empheq}[box=\mymathbgbox]{equation*}
...
\paragraph{Summary: The methods and the laws}
...
\texttt{\textcolor{blue}{\footnotesize{}fmapOpt}} 
...
\subsubsection{Constructions of filterable functors}
...
\paragraph{{*} Worked examples II: Constructions of filterable functors I}
...
\paragraph{Worked examples II: Constructions of filterable functors IV}
...
\paragraph{{*} Bonus slide I: Definition of filterable contrafunctors}
...
$\text{filter}(p^{A\rightarrow\text{Boolean}})=\text{inflate}^{C^{A}\rightarrow C^{1+A}}\bef\text{contrafmap}(\text{bop}\,p)$

$\text{inflate}^{C^{A}\rightarrow C^{1+A}}=\text{contrafmap}\left(0+x^{A}\rightarrow x\right)\bef\text{filter}\left(\_\rightarrow\text{true}\right)$

$\text{contrafmapOpt}\:f^{B\rightarrow1+A}=\text{inflate}\bef\text{contrafmap}\,f$

$\text{inflate}=\text{contrafmapOpt}\,(\text{id}^{1+A\rightarrow1+A})$
...
\paragraph{{*} Bonus slide II: Structure of filterable contrafunctors}
...
be expressed (isomorphically) as $F^{A}\triangleq C\times\left(\bbnum 1+...\right)$
where $C$ is a constant type. An example of a polynomial functor
not of this form is $F^{A}=P+Q\times A$, where $P\neq Q$ are constant
types and $P$ is not pointed (not known to be isomorphic to $\bbnum 1+Z$
for some type $Z$).
