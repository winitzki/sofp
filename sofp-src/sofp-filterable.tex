...
a precise definition of type constructors (\textsf{``}filterable functors\textsf{''})
...
The name \textsf{``}functor block\textsf{''} is chosen because it is a single expression
(a \textsf{``}block of code\textsf{''}) whose type is required to be a functor. The
...
called \index{Kleisli composition}\textbf{Kleisli}\footnote{The Swiss-German name Kleisli is pronounced \textsf{``}cli-slee\textsf{''} (\textsf{``}cli\textsf{''}
as in \textsf{``}climb\textsf{''}).}\textbf{ composition}.\label{kleisli-composition} It applies to functions
...
is to \textsf{``}unroll\textsf{''} the recursive\index{recursive type!unrolling trick}
...
This is a consequence of the \textsf{``}parametricity theorem\textsf{''}\index{parametricity theorem},
...
twice, a mathematician would not be satisfied without defining a \textsf{``}generalized
lifting\textsf{''} that replaces $\text{Opt}$ by a functor $M$,
\begin{align*}
...
We have seen four examples of operations that have the form of a \textsf{``}lifting\textsf{''}:
\begin{align*}
...
the ordinary function $A\rightarrow B$, the \textsf{``}reversed\textsf{''} type $B\rightarrow A$,
...
that lifts more general or \textsf{``}twisted\textsf{''} function types instead of
...
of \index{category theory!objects} \textbf{objects}\footnote{The \textsf{``}objects\textsf{''} in category theory are not related to \textsf{``}object-oriented
programming\textsf{''}.} $\left\{ A,B,...\right\} $ and a collection of morphisms $\left\{ f_{1},f_{2},...\right\} $,
...
to another}. We will use the phrase \textsf{``}categorical functor\textsf{''} to distinguish
...
starting from heuristic considerations.\footnote{E.g., with an additional \textsf{``}empty-value\textsf{''} law here: \texttt{\href{https://github.com/fantasyland/fantasy-land\#filterable}{https://github.com/fantasyland/fantasy-land\#filterable}}}
...
new monads $M$, we can look for \textsf{``}$M$-filterable\textsf{''} functors $F$
