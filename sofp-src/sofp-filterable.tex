...
\begin{align*}
 ...
 & \xymatrix{\xyScaleX{5pc}F^{A}\ar[r]\sp(0.45){\text{inflate}} & F^{\bbnum 1+A}\ar[r]\sp(0.55){\left(\text{filt}_{\text{Opt}}(p)\right)^{\uparrow F}} & F^{\bbnum 1+A}\ar[r]\sp(0.55){\text{deflate}} & F^{A}}
\end{align*}
...
as
\[
\text{liftOpt}\,(\text{pu}_{\text{Opt}})=\text{id}\quad.
\]

We can combine Eq.~(\ref{eq:identity-law-of-liftOpt}) and naturality
law~(\ref{eq:naturality-law-of-liftOpt}) into a single law if we
compose the identity law with an arbitrary function $f^{:A\rightarrow B}$:
\[
...
is
\begin{equation}
\text{liftOpt}\,(f^{:A\rightarrow B}\bef\text{pu}_{\text{Opt}}^{B})=f^{\uparrow F}\quad.\label{eq:combined-identity-naturality-of-liftOpt}
\end{equation}
...
\[
\psi_{p}=\psi_{p_{1}}\diamond_{_{\text{Opt}}}\psi_{p_{2}}\triangleq x^{:A}\rightarrow x\triangleright\psi_{p_{1}}\triangleright\text{flatMap}_{\text{Opt}}(\psi_{p_{2}})=\psi_{p_{1}}\bef(y\rightarrow y\triangleright\text{flatMap}_{\text{Opt}}(\psi_{p_{2}}))\quad.
\]
...
\vspace{-2\baselineskip}
\[
\xymatrix{\xyScaleY{2pc}\xyScaleX{3pc} & F^{B}\ar[rd]\sp(0.55){\ \text{liftOpt}\,(g^{:B\rightarrow\bbnum 1+C})}\\
F^{A}\ar[ru]\sp(0.45){\text{liftOpt}\,(f^{:A\rightarrow\bbnum 1+B})\ }\ar[rr]\sb(0.5){\text{liftOpt}\,(f\diamond_{_{\text{Opt}}}g)} &  & F^{C}
}
\]
\vspace{-2\baselineskip}
\end{wrapfigure}%

\noindent Because this law holds for $f$ and $g$ involving arbitrary
types $A$, $B$, $C$, it is stronger than \lstinline!filter!'s
composition law. We will now show that \lstinline!deflate!'s naturality
law~(\ref{eq:naturality-law-of-deflate}) and hence \lstinline!filter!'s
naturality law~(\ref{eq:naturality-law-of-filter}) can be derived
from Eqs.~(\ref{eq:combined-identity-naturality-of-liftOpt})\textendash (\ref{eq:composition-law-of-liftOpt})
...
of \lstinline!liftOpt! follows from Eqs.~(\ref{eq:combined-identity-naturality-of-liftOpt})\textendash (\ref{eq:composition-law-of-liftOpt}).
...
{\color{greenunder}\text{use Eq.~(\ref{eq:combined-identity-naturality-of-liftOpt})}:}\quad & \quad=h^{\uparrow F}\bef\text{liftOpt}\,(g)\quad.
\end{align*}
...
function of the \lstinline!Option! type. For symbolic computations,
it is convenient to rewrite \lstinline!flatMap! as a curried function
denoted $\text{flm}_{\text{Opt}}$, having the type signature
\[
\text{flm}_{\text{Opt}}:(A\rightarrow\text{Opt}^{B})\rightarrow\text{Opt}^{A}\rightarrow\text{Opt}^{B}\quad,\quad\quad\text{flm}_{\text{Opt}}(g)\triangleq p^{:\bbnum 1+A}\rightarrow p\triangleright\text{flatMap}_{\text{Opt}}(g)\quad.
\]
With this notation, the definition of the Kleisli product $\diamond_{_{\text{Opt}}}$
is simplified to
\begin{equation}
...
\end{equation}
Then we compute $f\diamond_{_{\text{Opt}}}g$ as
\begin{align*}
 & f\diamond_{_{\text{Opt}}}g=\\
{\color{greenunder}\text{definition of }\diamond_{_{\text{Opt}}}:}\quad & =h\bef\gunderline{\text{pu}_{\text{Opt}}\bef\text{flm}_{\text{Opt}}}(g)\\
...
{\color{greenunder}\text{use Eq.~(\ref{eq:combined-identity-naturality-of-liftOpt})}:}\quad & \quad=\text{liftOpt}\,(f)\bef h^{\uparrow F}\quad.
\end{align*}
...
{\color{greenunder}\text{definition~(\ref{eq:def-of-Kleisli-product}) of }\diamond_{_{\text{Opt}}}:}\quad & =f\bef\text{flm}_{\text{Opt}}(h\bef\text{pu}_{\text{Opt}})\\
...
\end{align*}
This yields the ``right naturality'' law\index{naturality law!of liftOpt@of \texttt{liftOpt}}
...
two laws~(\ref{eq:combined-identity-naturality-of-liftOpt})\textendash (\ref{eq:composition-law-of-liftOpt}).
...
\begin{align*}
{\color{greenunder}\text{law~(\ref{eq:combined-identity-naturality-of-liftOpt}) of }\text{liftOpt}:}\quad & \text{liftOpt}\,(f\bef\text{pu}_{\text{Opt}})=\text{id}=f^{\uparrow\text{Const}}\quad,\\
...
{\color{greenunder}\text{law~(\ref{eq:combined-identity-naturality-of-liftOpt}) of }\text{liftOpt}_{H}:}\quad & \quad=\big(f^{\uparrow H}\big)^{\uparrow G}=f^{\uparrow F}\quad.
\end{align*}
...
{\color{greenunder}\text{law~(\ref{eq:combined-identity-naturality-of-liftOpt}) for }G\text{ and }H:}\quad & =f^{\uparrow G}\boxtimes f^{\uparrow H}=f^{\uparrow F}\quad.
\end{align*}
...
To verify the naturality-identity law~(\ref{eq:combined-identity-naturality-of-liftOpt}):
\begin{align*}
...
\end{array}\\
{\color{greenunder}\text{law~(\ref{eq:combined-identity-naturality-of-liftOpt}) for }G\text{ and }H:}\quad & =\,\begin{array}{||cc|}
f^{\uparrow G} & \bbnum 0\\
\bbnum 0 & f^{\uparrow H}
...
def liftOpt_F[A, B](f: A => Option[B]): Option[(A, G[A])] => Option[(B, G[B])] = {
  case None          => None
  case Some((a, ga))   => f(a) match {
       case None    => None
       case Some(b) => Some((b, liftOpt_G(f)(ga)))
  }
}
\end{lstlisting}
\[
...
\end{array}\quad.
\]
This definition will delete all data in $G^{A}$ and return the empty
wrapper, $1+\bbnum 0^{:B\times G^{B}}$, if the filtering predicate
returns \lstinline!false! on the given value $a^{:A}$ in the pair
$A\times G^{A}$. 

Before we start calculations, we note that the matrix for $\text{liftOpt}_{F}$
has a non-split output column (representing the disjunctive type $\bbnum 1+B\times G^{B}$)
because we need to pattern match on $f(a)$ in order to determine
...
conventions of matrix products do not apply when some parts of a disjunctive
type are not represented by separate columns. So, we need to rewrite
...
To verify the laws, we need to define the lifting to $F$:
\begin{lstlisting}
...
\end{lstlisting}
Again, it is convenient to rewrite this code using the standard \lstinline!.map!
...
\]
For brevity, we omit type annotations. The naturality-identity law~(\ref{eq:combined-identity-naturality-of-liftOpt})
for $F$:
\begin{align*}
...
{\color{greenunder}\text{law~(\ref{eq:combined-identity-naturality-of-liftOpt}) for }G:}\quad & =\text{flm}_{\text{Opt}}\big(a\times g\rightarrow a\triangleright f\bef\gunderline{\text{pu}_{\text{Opt}}\bef\big(b\rightarrow b\times f^{\uparrow G}(g)\big)^{\uparrow\text{Opt}}}\big)\\
...
{\color{greenunder}\text{function composition}:}\quad & =\text{flm}_{\text{Opt}}\big(\gunderline{a\times g\rightarrow\big(f(a)\times f^{\uparrow G}(g)\big)}\triangleright\text{pu}_{\text{Opt}}\big)=\text{flm}_{\text{Opt}}\big(\gunderline{(f\boxtimes f^{\uparrow G})\bef\text{pu}_{\text{Opt}}}\big)\\
...
\end{equation}
because $\text{liftOpt}_{F}(f)$ is equal to $\text{flm}_{\text{Opt}}(...)$,
so we need to transform $\text{flm}_{\text{Opt}}(...)\bef\text{flm}_{\text{Opt}}(...)$
...
law holds for $\text{liftOpt}_{\text{Opt}}$, 
\[
...
is the standard method \lstinline!.flatten! (denoted by $\text{ftn}_{\text{Opt}}$
for brevity):
\[
...
law} for filterable contrafunctors\index{identity laws!for filterable contrafunctor}
to be 
\begin{equation}
...
\]
Clearly, we need to require the \textbf{composition law} for filterable\index{composition law!for filterable contrafunctor}
...
\]
and the recursive construction for ordinary functors (Statement~\ref{subsec:functor-Statement-functor-recursive-1})
that requires a bifunctor $S^{\bullet,\bullet}$,
\[
F^{A}\triangleq S^{A,F^{A}}\quad.
\]
...
produces the standard filtering operation of \lstinline!List!.
...
because the columns cannot be split in the matrix shown above. Instead,
we apply both sides of laws to arbitrary values of type $F^{A}$.
There are two cases, $G^{A}+\bbnum 0^{:A\times F^{A}}$ and $\bbnum 0^{:G^{A}}+A\times F^{A}$.
When applied to the first case, the function $\text{liftOpt}_{F}$
...
It remains to apply the laws to a value $\bbnum 0^{:G^{A}}+a^{:A}\times r^{:F^{A}}$.
...
The fmapOpt laws hold for $F^{A}\equiv G^{A}+A\times F^{A}$ if they
hold for $G^{A}$

For $f^{A\rightarrow1+B}$, we have $\text{fmapOpt}_{G}(f):G^{A}\rightarrow G^{B}$
and $\text{fmapOpt}_{F}^{\prime}(f):F^{A}\rightarrow F^{B}$ (for
use in recursive arguments as the inductive assumption)

Define $\text{fmapOpt}_{F}(f)(q^{G^{A}}+a^{A}\times p^{F^{A}})$ by
returning $0+\text{fmapOpt}_{F}^{\prime}(f)(p)$ if $f(a)=1+0$, and
$\text{fmapOpt}_{G}(f)(q)+b\times\text{fmapOpt}_{F}^{\prime}(f)(p)$
otherwise

Identity law: $\text{id}_{\diamond_{\text{Opt}}}(x)\neq1+0$, so $\text{fmapOpt}_{F}(\text{id}_{\diamond_{\text{Opt}}})(q+a\times p)=q+a\times p$ 

Composition law: $(\text{fmapOpt}_{F}(f_{1})\bef\text{fmapOpt}_{F}(f_{2}))(q+a\times p)=\text{fmapOpt}_{F}(f_{1}\diamond_{\text{Opt}}f_{2})(q+a\times p)$

For arguments $q+0$, the laws for $G^{A}$ hold; so assume arguments
$0+a\times p$. When $f_{1}(a)=0+b$ and $f_{2}(b)=0+c$, the proof
of the previous example will go through. So we need to consider the
two cases $f_{1}(a)=1+0$ and $f_{1}(a)=0+b$, $f_{2}(b)=1+0$ 

If $f_{1}(a)=1+0$ then $(f_{1}\diamond_{\text{Opt}}f_{2})(a)=1+0$;
...
If $f_{1}(a)=0+b$ and $f_{2}(b)=1+0$ then $(f_{1}\diamond_{\text{Opt}}f_{2})(a)=1+0$;
...
2 laws (identity, composition) for \texttt{\textcolor{blue}{\footnotesize{}contrafmapOpt}} 
...
\paragraph{Structure analysis of filterable contrafunctors}
...
\subsubsection{Example \label{subsec:Example-filterable-laws-1-1}\ref{subsec:Example-filterable-laws-1-1}}

Solved example: the functor $F^{A}\equiv G^{A}\rightarrow A$ is not
filterable
...
assuming that the contrafunctor \texttt{\textcolor{blue}{\footnotesize{}H{[}T{]}}}
already has a \texttt{\textcolor{blue}{\footnotesize{}Filterable}}
instance and $G$ is an arbitrary contrafunctor. Verify the laws rigorously
(i.e.\ by calculations, not tests).
...
Show that $\text{flm}_{\text{Opt}}(\text{pu}_{\text{Opt}})=\text{id}$.
