...
\section{Practical uses of filtering}
...
are placed together and the functor composition law can be applied.
...
$f$. The only way to obtain some $g^{:\bbnum 1+A\rightarrow\bbnum 1+B}$
...
assuming that a suitable naturality law, Eq.~(\ref{eq:left-naturality-law-of-liftOpt})
...
initial \lstinline!liftOpt!, assuming Eq.~(\ref{eq:left-naturality-law-of-liftOpt}).
...
(see diagram at left)
\begin{equation}
(h^{:A\rightarrow B})^{\uparrow F}\bef\text{liftOpt}^{B,C}(f^{:B\rightarrow\bbnum 1+C})=\text{liftOpt}^{A,C}(h^{:A\rightarrow B}\bef f^{:B\rightarrow\bbnum 1+C})\quad.\label{eq:left-naturality-law-of-liftOpt}
\end{equation}
...
Denote that function for brevity by $\text{pu}_{\text{Opt}}$:
\[
...
a single law if we compose the identity law with an arbitrary function
$f^{:A\rightarrow B}$:
\[
...
\end{align*}
In this calculation, we used the composition property of the function
product,
\begin{equation}
...
\end{equation}
It is convenient to call that law also the \index{composition law!of flatMap for Option@of \texttt{flatMap} for \texttt{Option}}\textbf{composition
...
{\color{greenunder}\text{use Eq.~(\ref{eq:associativity-law-of-flatMap-for-Option})}:}\quad & =\text{flm}_{\text{Opt}}\big(\big(a\times g\rightarrow a\triangleright f\triangleright r_{f,g}^{\uparrow\text{Opt}}\big)\bef\text{flm}_{\text{Opt}}\big(a^{\prime}\times g^{\prime}\rightarrow a^{\prime}\triangleright f^{\prime}\bef r_{f^{\prime},g^{\prime}}^{\uparrow\text{Opt}}\big)\big)\nonumber \\
...
\end{align}
With these equations, which are the \textbf{naturality laws}\index{naturality laws!of flatMap for Option@of \texttt{flatMap} for \texttt{Option}}
...
{\color{greenunder}\text{inductive assumption}:}\quad & =\bbnum 0+f(a)\times r\triangleright\gunderline{f^{\uparrow F}}\quad.
\end{align*}
...
b^{:B}\rightarrow\big((\bbnum 0+b\times r\triangleright\overline{\text{liftOpt}_{F}}(f)\big)\triangleright\text{liftOpt}_{F}(g)
\end{array}\nonumber \\
...
c^{:C}\rightarrow\bbnum 0+c\times r\triangleright\overline{\text{liftOpt}_{F}}(f)\bef\overline{\text{liftOpt}_{F}}(g)
\end{array}
\end{array}\nonumber \\
...
and preserves all other values. So, it has the same filtering logic
as the standard \lstinline!.filter! method for sequences.
...
\]
Because this law interchanges $\text{liftOpt}_{S}$ with a lifting
that affects the \emph{other} type parameter of $S^{\bullet,\bullet}$,
all fully parametric functions will obey this law (see a discussion
...
\subsubsection{Example \label{subsec:Example-first-filterable-contrafunctor}\ref{subsec:Example-first-filterable-contrafunctor}\index{solved examples}}
...
\text{inflate}=\text{liftOpt}_{C}(\text{id}^{:\bbnum 1+A\rightarrow\bbnum 1+A})\quad, & \quad\quad\text{liftOpt}_{C}(f^{:A\rightarrow\bbnum 1+B})=\text{inflate}^{:C^{B}\rightarrow C^{\bbnum 1+B}}\bef f^{\downarrow C}\quad.\label{eq:express-liftOpt-via-inflate-for-contrafunctors}
\end{align}
\[
\xymatrix{C^{\bbnum 1+A}\ar[rd]\sb(0.4){\text{filt}_{C}^{\bbnum 1+A}(\text{nonEmpty}^{:\bbnum 1+A\rightarrow\bbnum 2})~~~~~~~} & C^{A}\ar[l]\sb(0.4){\text{get}^{\downarrow C}}\ar[d]\sp(0.4){\text{inflate}^{A}}\ar[r]\sp(0.5){\text{filt}_{C}^{A}(p^{:A\rightarrow\bbnum 2})} & C^{A} & C^{B}\ar[r]\sp(0.5){\text{inflate}^{B}}\ar[rd]\sb(0.4){\text{liftOpt}_{C}(f)\triangleq~~~} & C^{\bbnum 1+B}\ar[d]\sp(0.45){(f^{:A\rightarrow B})^{\downarrow C}}\\
...
function for proving laws, while \lstinline!inflate! is the easiest
...
For $C^{A}\triangleq A\rightarrow\bbnum 1+Z$, implement \lstinline!inflate!
and \lstinline!liftOpt!, and directly verify Eq.~(\ref{eq:express-filter-via-inflate-for-contrafunctor}).
...
a filter (which makes it a total function due to filter's partial
function law). For this reason, we are allowed to apply lifted partial
functions before filtering in contrafunctors.
...
First part: starting from a given $\text{filt}_{C}$, we compute $\text{inflate}_{C}$
and then use that to define a new $\text{filt}_{C}^{\prime}$; we
must show that $\text{filt}_{C}=\text{filt}_{C}^{\prime}$.
\begin{align*}
...
of $\psi_{p}$ and $\text{get}$, if only we could move these functions
next to each other. It is clear that we need a law that exchanges
the order of compositions of $\text{filt}_{C}$ with lifted functions.
Typically, that is done by naturality laws. By analogy with Eq.~(\ref{eq:naturality-law-of-filter})
and making sure types match, we write a \textbf{naturality law} of\index{naturality law!of filter for contrafunctors@of \texttt{filter} for contrafunctors}
$\text{filt}_{C}$ as
\begin{equation}
...
\begin{align*}
{\color{greenunder}\text{use naturality law of }\text{filt}_{C}:}\quad & \text{get}^{\downarrow C}\bef\gunderline{\text{filt}_{C}(\text{nonEmpty})\bef\psi_{p}^{\downarrow C}}=\gunderline{\text{get}^{\downarrow C}\bef\psi_{p}^{\downarrow C}}\bef\text{filt}_{C}(\psi_{p}\bef\text{nonEmpty})\\
{\color{greenunder}\text{use Eqs.~(\ref{eq:composition-of-psi-p-and-nonEmpty-simplified})--(\ref{eq:composition-of-psi-p-and-get-simplified})}:}\quad & =\gunderline{\text{id}_{|p}^{\downarrow C}}\bef\text{filt}_{C}(p)\\
...
\]
The lifted partial function is applied \emph{before} filtering, as
appropriate for filterable contrafunctors.

Second part: starting from a given $\text{inflate}_{C}$, we compute
...
\vspace{-2\baselineskip}
\[
\xymatrix{C^{A}\ar[r]\sp(0.5){\text{inflate}_{C}^{A}}\ar[d]\sb(0.45){(f^{:B\rightarrow A})^{\downarrow C}} & C^{\bbnum 1+A}\ar[d]\sp(0.45){(f^{:B\rightarrow A})^{\uparrow\text{Opt}\downarrow C}}\\
\xyScaleY{1.8pc}\xyScaleX{4.5pc}C^{B}\ar[r]\sp(0.5){\text{inflate}_{C}^{B}} & C^{\bbnum 1+B}
}
\]
...
\noindent \lstinline!inflate! function:
\begin{equation}
...
$\text{liftOpt}\left(g\right)\bef\text{liftOpt}\left(f\bef\text{pu}_{\text{Opt}}\right)=\text{liftOpt}(f\bef\text{pu}_{\text{Opt}}\diamond_{_{\text{Opt}}}g)=\text{liftOpt}(f\bef g)\quad.$Now
use the naturality-identity law and get $\text{liftOpt}\left(f\bef\text{pu}_{\text{Opt}}\right)=f^{\downarrow C}$.
So the result is the naturality law $\text{liftOpt}\left(f\bef g\right)=\text{liftOpt}\left(g\right)\bef f^{\downarrow C}$.
\end{comment}


\subsection{Constructions of filterable contrafunctors}

How to build up a filterable contrafunctor from parts? Structural
analysis discovers systematically a number of type constructions for
contrafunctors that are guaranteed to have filterable properties.
...
Constant contrafunctors $C^{A}\triangleq Z$ are ``trivially'' filterable:
...
a filterable contrafunctor and then $P^{Q^{\bullet}}$ is filterable.
...
by $\text{liftOpt}_{P\circ Q}(f^{:A\rightarrow\bbnum 1+B})\triangleq\big(\text{liftOpt}_{Q}(f)\big)^{\downarrow P}$.
\begin{lstlisting}
...
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{liftOpt}_{P\circ Q}(f\diamond_{_{\text{Opt}}}g):}\quad & \text{liftOpt}_{P\circ Q}(g)\bef\text{liftOpt}_{P\circ Q}(f)=\big(\text{liftOpt}_{Q}(g)\big)^{\downarrow P}\bef\big(\text{liftOpt}_{Q}(f)\big)^{\downarrow P}\\
...
by $\text{liftOpt}_{P\circ Q}(f^{:A\rightarrow\bbnum 1+B})\triangleq\big(\text{liftOpt}_{Q}(f)\big)^{\uparrow P}$.
\begin{lstlisting}
...
To verify the composition law~(\ref{eq:composition-law-filterable-contrafunctor}):
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{liftOpt}_{P\circ Q}(f\diamond_{_{\text{Opt}}}g):}\quad & \text{liftOpt}_{P\circ Q}(g)\bef\text{liftOpt}_{P\circ Q}(f)=\big(\text{liftOpt}_{Q}(g)\big)^{\uparrow P}\bef\big(\text{liftOpt}_{Q}(f)\big)^{\uparrow P}\\
...
(Exercise~\ref{subsec:Exercise-filterable-laws-6-2}). 
...
\subsubsection{Statement \label{subsec:Statement-function-type-exponential-filterable-contrafunctor}}
...
\end{lstlisting}
To obtain a clearer code formula, first rewrite the Scala code using
the $\triangleright$-notation and then simplify:
\begin{align*}
...
{\color{greenunder}\text{simplify }(x\rightarrow x\triangleright f)=f:}\quad & \quad=p^{:G^{B}\rightarrow H^{B}}\rightarrow\text{liftOpt}_{G}(f)\bef p\bef\text{liftOpt}_{H}(f)\quad.
\end{align*}

To verify the naturality-identity law~(\ref{eq:naturality-identity-law-filterable-contrafunctor}),
apply to an arbitrary $p^{:G^{B}\rightarrow H^{B}}$:
\begin{align*}
...
To verify the composition law~(\ref{eq:composition-law-filterable-contrafunctor}):
\begin{align*}
{\color{greenunder}\text{expect to equal }p\triangleright\text{liftOpt}_{F}(f\diamond_{_{\text{Opt}}}g):}\quad & p\triangleright\text{liftOpt}_{F}(g)\bef\text{liftOpt}_{F}(f)\\
...
{\color{greenunder}\text{definition of }\text{liftOpt}_{F}(g):}\quad & =\text{liftOpt}_{G}(f)\bef\text{liftOpt}_{G}(g)\bef p\bef\text{liftOpt}_{H}(g)\bef\text{liftOpt}_{H}(f)\\
...
defined by $S_{Z}^{A}\triangleq\left(A\rightarrow\bbnum 1+Z\right)\rightarrow\bbnum 1+A$,
where $Z$ is a fixed type. This functor is the function from the
filterable contrafunctor $A\rightarrow\bbnum 1+Z$ to the filterable
functor $\bbnum 1+A$, and so it is filterable. The simplest case
of the search functor is found by setting $Z\triangleq\bbnum 1$,
which gives the type constructor
\[
S_{\bbnum 1}^{A}\triangleq\left(A\rightarrow\bbnum 2\right)\rightarrow\bbnum 1+A\quad.
\]
Values of type $S_{\bbnum 1}^{A}$ may be ``search functions'' taking
a predicate $q^{:A\rightarrow\bbnum 2}$ and looking for a value of
type $A$ that satisfies the predicate. A search function will return
either a suitable value of type $\bbnum 0+A$, or an empty value $1+\bbnum 0$
(``not found''). Applying a filter with an a predicate $p^{:A\rightarrow\bbnum 2}$
to a search function will exclude values of type $A$ from the search
unless they also satisfy $p$.
...
\end{lstlisting}
\begin{align*}
 & \text{liftOpt}_{F}(f^{:A\rightarrow\bbnum 1+B})\triangleq p^{:B\rightarrow\bbnum 1+H^{B}}\rightarrow a^{:A}\rightarrow a\triangleright f\triangleright\text{flm}_{\text{Opt}}(p)\triangleright\big(\text{liftOpt}_{H}(f)\big)^{\uparrow\text{Opt}}\\
 & =p\rightarrow f\bef\text{flm}_{\text{Opt}}(p)\bef\big(\text{liftOpt}_{H}(f)\big)^{\uparrow\text{Opt}}\quad.
\end{align*}
To verify the naturality-identity law~(\ref{eq:naturality-identity-law-filterable-contrafunctor}),
apply to an arbitrary $p^{:B\rightarrow\bbnum 1+H^{B}}$:
\begin{align*}
...
with functions $f^{:A\rightarrow\bbnum 1+B}$, $g^{:B\rightarrow\bbnum 1+C}$,
and transform the result until it is equal to $p\triangleright\text{liftOpt}_{F}(f\diamond_{_{\text{Opt}}}g)$:
\begin{align*}
{\color{greenunder}\triangleright\text{-notation}:}\quad & p\triangleright\text{liftOpt}_{F}(g)\bef\text{liftOpt}_{F}(f)=\big(p\triangleright\text{liftOpt}_{F}(g)\big)\triangleright\gunderline{\text{liftOpt}_{F}(f)}\\
...
{\color{greenunder}\text{Eqs.~(\ref{eq:associativity-law-of-flatMap-for-Option}) and~(\ref{eq:right-naturality-flatmap-option}) of }\text{flm}_{\text{Opt}}:}\quad & \gunderline{f\bef\text{flm}_{\text{Opt}}(g)}\bef\text{flm}_{\text{Opt}}(p)\bef\big(\gunderline{\text{liftOpt}_{H}(g)\bef\text{liftOpt}_{H}(f)}\big)^{\uparrow\text{Opt}}\\
...
To define a contrafunctor via type recursion, we need to use a profunctor
$S^{A,R}$ (see Section~\ref{subsec:f-Profunctors}) that is contravariant
in $A$ and covariant in $R$.
...
\end{lstlisting}
 The code of the function \lstinline!liftOpt! for $F^{\bullet}$
is recursive and uses the \lstinline!xmap! method of the profunctor
$S$,
\begin{lstlisting}
...
\[
\xymatrix{S^{B,F^{B}}\ar[r]\sp(0.5){\text{liftOpt}_{S}(f^{:A\rightarrow\bbnum 1+B})}\ar[rd]\sb(0.45){\text{liftOpt}_{F}(f)\triangleq~~~} & S^{A,F^{B}}\ar[d]\sp(0.45){\big(\overline{\text{liftOpt}_{F}}(f)\big)^{\uparrow S^{A,\bullet}}}\\
\xyScaleY{1.8pc}\xyScaleX{7pc} & S^{A,F^{A}}
}
\]
...
\noindent As before, we use an overline to mark recursive calls to
the same function:
\begin{align*}
...
with respect to lifting in the other type parameter of $S^{A,R}$:

\begin{wrapfigure}{l}{0.3\columnwidth}%
\vspace{-2\baselineskip}
\[
\xymatrix{S^{B,R}\ar[r]\sp(0.5){\text{liftOpt}_{S}(f^{:A\rightarrow\bbnum 1+B})}\ar[d]\sp(0.4){h^{\uparrow S^{B,\bullet}}} & S^{A,R}\ar[d]\sb(0.4){h^{\uparrow S^{A,\bullet}}}\\
...
\end{itemize}
Most non-trivial type constructors have many lawful but inequivalent
implementations of \lstinline!filter!. It is not possible to choose
a ``preferred'' implementation automatically, since different applications
may need different filtering behavior. While in most cases the standard
library provides sufficient implementations of filtering (e.g.~as
\lstinline!.filter! or \lstinline!.takeWhile! methods on sequences),
in some situations the programmer will need to provide a custom implementation
of \lstinline!.filter! for a custom data type. The programmer must
examine the given business requirements and decide whether they can
be implemented as a lawful \lstinline!filter! function.
...
\end{equation}
Also show that it is not a full inverse: $\text{deflate}_{F}\bef\text{inflate}_{F}\ne\text{id}$
for some functors $F$.
...
is to ``unroll'' the recursive equation and to reduce $F^{A}$ to
...
form $F^{A}\triangleq P^{A}+Q^{A}\times F^{A}$:
\begin{equation}
\text{define}\quad F^{A}\triangleq P^{A}+Q^{A}\times F^{A}\quad,\quad\text{then}\quad F^{A}\cong P^{A}\times\text{List}^{Q^{A}}\quad,\label{eq:def-recursive-functor-unrolling}
\end{equation}
...
of $F^{A}$ can be ``unrolled'' $n$ times as
\begin{align*}
...
on the number of ``unrolled'' functors $F$. We can now use the
...
to be filterable is that one can implement a function of type $C^{\bbnum 0}\rightarrow C^{\bbnum 1}$
(i.e.~the type $C^{\bbnum 0}\rightarrow C^{\bbnum 1}$ is not void).
...
to move $\psi_{p}^{\downarrow C}$ in the expression $\text{get}^{\downarrow C}\bef\text{filt}_{C}(\text{nonEmpty})\bef\psi_{p}^{\downarrow C}$
to the left of $\text{filt}_{C}$, or else we could make no progress
with the calculations. The required interchange was possible by using
the law~(\ref{eq:naturality-for-filter-for-contrafunctors}), 
\[
\text{filt}_{C}(p)\bef f^{\downarrow C}=f^{\downarrow C}\bef\text{filt}_{C}(f\bef p)\quad.
\]
In this and previous chapters, we discovered a large number of laws
of that form, e.g.:
\begin{align*}
{\color{greenunder}\text{Eq.~(\ref{eq:naturality-law-of-pure})}:}\quad & \text{pu}_{F}:A\rightarrow F^{A} &  & \text{pu}_{F}\bef f^{\uparrow F}=f\bef\text{pu}_{F}\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:naturality-law-of-extract})}:}\quad & \text{ex}_{F}:F^{A}\rightarrow A &  & \text{ex}_{F}\bef f=f^{\uparrow F}\bef\text{ex}_{F}\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:copointed-bifunctor-naturality-law})}:}\quad & \text{ex}_{S}:S^{A,A}\rightarrow A &  & \text{ex}_{S}\bef f=\text{bimap}_{S}(f)(f)\bef\text{ex}_{S}\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:naturality-law-of-filter})}:}\quad & \text{filt}_{F}:\left(A\rightarrow\bbnum 2\right)\rightarrow F^{A}\rightarrow F^{A} &  & f^{\uparrow F}\bef\text{filt}_{F}(p)=\text{filt}_{F}(f\bef p)\bef f^{\uparrow F}\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:naturality-law-of-deflate})}:}\quad & \text{deflate}_{F}:F^{\bbnum 1+A}\rightarrow F^{A} &  & \text{deflate}\bef f^{\uparrow F}=f^{\uparrow\text{Opt}\uparrow F}\bef\text{deflate}\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:naturality-law-of-psi})}:}\quad & \psi:\left(A\rightarrow\bbnum 2\right)\rightarrow A\rightarrow\bbnum 1+A &  & f\bef\psi_{p}=\psi_{f\bef p}\bef f^{\uparrow\text{Opt}}\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:left-naturality-law-of-liftOpt})}:}\quad & \text{liftOpt}_{F}:\left(A\rightarrow\text{Opt}^{B}\right)\rightarrow F^{A}\rightarrow F^{B} &  & h^{\uparrow F}\bef\text{liftOpt}_{F}(f)=\text{liftOpt}_{F}(h\bef f)\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:right-naturality-law-of-liftOpt})}:}\quad & \text{liftOpt}_{F}:\left(A\rightarrow\text{Opt}^{B}\right)\rightarrow F^{A}\rightarrow F^{B} &  & \text{liftOpt}_{F}(f)\bef h^{\uparrow F}=\text{liftOpt}_{F}(f\bef h^{\uparrow\text{Opt}})\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:naturality-for-filter-for-contrafunctors})}:}\quad & \text{filt}_{C}:\left(A\rightarrow\bbnum 2\right)\rightarrow C^{A}\rightarrow C^{A} &  & \text{filt}_{C}(p)\bef f^{\downarrow C}=f^{\downarrow C}\bef\text{filt}_{C}(f\bef p)\quad,\\
{\color{greenunder}\text{Eq.~(\ref{eq:naturality-law-of-inflate-for-filterable-contrafunctor})}:}\quad & \text{inflate}_{C}:C^{A}\rightarrow C^{\bbnum 1+A} &  & f^{\downarrow C}\bef\text{inflate}_{C}=\text{inflate}_{C}\bef f^{\uparrow\text{Opt}\downarrow C}\quad.
\end{align*}
We called all these laws ``naturality laws'', although they were
...
\paragraph{Natural transformation}
...
Functions of that kind are called \textbf{natural transformations}
between $F$ and $G$:\index{natural transformation} a natural transformation
is a fully parametric function $t^{A}:F^{A}\rightarrow G^{A}$. Heuristically,
we may view $t^{A}$ as a function that copies data of type $A$ from
one ``wrapper'' to another and may rearrange that data in a way
that does not depend on $A$. An example is the \lstinline!.headOpt!
method defined in Scala on various sequence types such as \lstinline!List!,
\[
...
This property is formulated mathematically as the naturality law of
\lstinline!headOption!,

\begin{wrapfigure}{l}{0.27\columnwidth}%
\vspace{-2.1\baselineskip}
\[
\xymatrix{\text{List}^{A}\ar[r]\sp(0.55){\text{headOpt}^{A}}\ar[d]\sp(0.4){(f^{:A\rightarrow B})^{\uparrow\text{Listt}}} & \text{Opt}^{A}\ar[d]\sb(0.4){f^{\uparrow\text{Opt}}}\\
...
\]
It is important to keep in mind that this law in no way reflects the
fact that \lstinline!headOption! examines the \emph{first} element
of a list. The same naturality law will hold for any fully parametric
function of type $\text{List}^{A}\rightarrow\text{Opt}^{A}$, e.g.~the
functions \lstinline!_.lastOption! or \lstinline!_.drop(2).headOptionk!.
The naturality law only expresses the property that \lstinline!headOption!
works in the same way for all types and does not depend on having
any specific values in the list. This property is mathematically formulated
as the requirement that we may first transform the list with a lifted
function $(f^{:A\rightarrow B})^{\uparrow\text{List}}$ and then apply
\lstinline!headOption!; or we may first apply \lstinline!headOption!
and then transform the data with a lifted function $f$ (and we will
now need to lift to the \lstinline!Option! functor rather than to
the \lstinline!List! functor).
...
``natural transformation'' pattern, which we can formulate for arbitrary
functors $F^{\bullet}$ and $G^{\bullet}$ as the following law:

\begin{wrapfigure}{l}{0.18\columnwidth}%
\vspace{-2.3\baselineskip}
\[
\xymatrix{F^{A}\ar[r]\sp(0.55){t^{A}}\ar[d]\sp(0.4){(f^{:A\rightarrow B})^{\uparrow F}} & G^{A}\ar[d]\sb(0.4){f^{\uparrow G}}\\
...
\end{equation}
If we recognize that a given function $t^{A}:F^{A}\rightarrow G^{A}$
has the type signature of a natural transformation, how can we formulate
a naturality law for $t$? A naturality law always involves an arbitrary
function $f^{:A\rightarrow B}$ with type parameters $A$, $B$. Types
will match only if the function $f$ is lifted to the functor $F$
when $f$ is applied before $t$, and to the functor $G$ when $f$
is applied after $t$. So, the naturality law is $f^{\uparrow F}\bef t=t\bef f^{\uparrow G}$
with appropriate type parameters.

The analogous naturality law for natural transformations $t:C^{A}\rightarrow D^{A}$
between contrafunctors $C$, $D$ has exactly the same form, but the
order of type parameters must be swapped:

\begin{wrapfigure}{l}{0.18\columnwidth}%
\vspace{-2.3\baselineskip}
\[
\xymatrix{C^{A}\ar[r]\sp(0.55){t^{A}}\ar[d]\sp(0.4){(f^{:B\rightarrow A})^{\downarrow C}} & D^{A}\ar[d]\sb(0.4){f^{\downarrow D}}\\
...
\paragraph{Parameterized transformation }

The second pattern, which may be called ``parameterized transformation'',
...
\]
The argument of $t$ must be of type $C^{A}$, which we obtain by
applying $f^{\downarrow C}$ to $c$. So, the law is
\begin{equation}
...
swapped, we can write the naturality law for $\tilde{t}$ as 
\[
...
\begin{align*}
{\color{greenunder}\text{left-hand side applied to }p^{:F^{A}}:}\quad & p\triangleright f^{\uparrow F}\bef\gunderline{\tilde{t}}=p\triangleright f^{\uparrow F}\bef\big(p\rightarrow c\rightarrow p\triangleright t(c)\big)\\
{\color{greenunder}\text{compute composition}:}\quad & \quad=c\rightarrow p\triangleright f^{\uparrow F}\triangleright t(c)=c\rightarrow p\triangleright f^{\uparrow F}\bef t(c)\quad,\\
...
{\color{greenunder}\text{compute composition}:}\quad & \quad=c\rightarrow p\triangleright t(c\triangleright f^{\downarrow C})\bef f^{\uparrow G}\quad.
\end{align*}
So, we have reduced both sides of the law for $\tilde{t}$ to the
two sides of the law~(\ref{eq:naturality-law-general-parameterized-transformation}),
when applied to an arbitrary $p^{:F^{A}}$and considered as functions
of $c^{:C^{B}}$.
...
naturality law for $t:C^{A}\rightarrow F^{A}\rightarrow G^{A}$ can
then be derived from $t$'s type signature.

\paragraph{Lifting}

A ``lifted'' function $f^{\uparrow F}$ is the result of applying
a functor $F$'s \lstinline!map! operation to a function $f$. The
type signatures of \lstinline!map!, $\text{flm}_{\text{Opt}}$, and
\lstinline!liftOpt! are similar except for using the function type
$A\rightarrow\text{Opt}^{B}$ instead of $A\rightarrow B$:
\begin{align*}
...
type signature of a ``generalized lifting'',
\[
\text{lift}_{G,F}^{A,B}:(A\rightarrow G^{B})\rightarrow F^{A}\rightarrow F^{B}\quad,
\]
which we can view as a lifting of functions with a ``twisted'' type
$A\rightarrow G^{B}$ (which we may call ``Kleisli functions'')
to functions of type $F^{A}\rightarrow F^{B}$. We will look at properties
of generalized liftings in the next subsection. Here we focus on the
naturality laws for generalized liftings.

A generalized lifting has two type parameters and two naturality laws.
Looking at the two naturality laws~(\ref{eq:left-naturality-law-of-liftOpt})\textendash (\ref{eq:right-naturality-law-of-liftOpt})
for \lstinline!liftOpt! or at the two laws~(\ref{eq:left-naturality-flatmap-option})\textendash (\ref{eq:right-naturality-flatmap-option})
for $\text{flm}_{\text{Opt}}$, we notice that each naturality law
replaces one of the type parameters and keeps the other type parameter
...
For fixed $B$, the function $\text{lift}_{G,F}^{A,B}$ is a natural
...
will \emph{always hold} when the code of the function $t$ is fully
...
This is a consequence of the ``parametricity theorem''\index{parametricity theorem},
which is beyond the scope of this chapter.\footnote{See the paper ``Theorems for free'' by P.~Wadler, \texttt{\href{https://homepages.inf.ed.ac.uk/wadler/papers/free/free.ps}{https://homepages.inf.ed.ac.uk/wadler/papers/free/free.ps}}
and some further explanations in the posts \texttt{\href{https://reasonablypolymorphic.com/blog/theorems-for-free/}{https://reasonablypolymorphic.com/blog/theorems-for-free/}}
and \texttt{\href{https://bartoszmilewski.com/2014/09/22/}{https://bartoszmilewski.com/2014/09/22/}}} Due to the parametricity theorem, we do \emph{not} need to verify
naturality laws for functions whose code is known to be fully parametric.
This saves a significant amount of work, since every method of every
typeclass will have one naturality law per type parameter. Until now,
we have been systematically deriving and checking all naturality laws;
but we will not check those laws in the rest of the book.

While we may not need to verify naturality laws, it is important to
be able to recognize their form and to use them in derivations, where
...
is that an arbitrary function $f$ must be lifted to the functor $F$
to the left of $t:F^{A}\rightarrow G^{A}$ and to the functor $G$
to the right of $t$, matching with the two sides of the type signature
$F^{A}\rightarrow G^{A}$. Naturality laws for more complicated type
signatures are derived similarly.

All methods of typeclasses considered in this book are covered by
the natural tranformation recipe. However, not all type signatures
of fully parametric functions can be reduced to natural transformations;
for instance, $t^{A}:(A\rightarrow A)\rightarrow A$ is not of the
form $F^{A}\rightarrow G^{A}$ where $F,G$ are either functors or
contrafunctors. The parametricity theorem will still give laws for
such functions; but we will not be able to derive those laws via the
natural transformation recipe.

\subsection{Generalizing the laws of liftings}
...
\lstinline!liftOpt! method. These methods and their laws are equivalent
...
laws and is most convenient for general proofs of type constructions.

If we put aside the naturality law, the two laws of \lstinline!liftOpt!
are the laws of identity~(\ref{eq:identity-law-of-liftOpt}) and
composition~(\ref{eq:composition-law-of-liftOpt}). It is notable
how these two laws are similar to the identity and composition laws
for functors, Eqs.~(\ref{eq:f-identity-law-functor-fmap})\textendash (\ref{eq:f-composition-law-functor-fmap}):
\begin{align*}
...
We will now focus on the similarities between these laws, which go
far beyond the superficial similarity of form.

Kleisli functions of types $f^{:A\rightarrow\bbnum 1+B}$ and $g^{:B\rightarrow\bbnum 1+C}$
cannot be composed with the ordinary function composition ($f\bef g$).
...
The function \lstinline!liftOpt! can be viewed as a ``generalized
lifting'' from Kleisli functions $A\rightarrow\bbnum 1+B$ into functions
$F^{A}\rightarrow F^{B}$, just as \lstinline!fmap! is a lifting
from ordinary functions $A\rightarrow B$ into functions $F^{A}\rightarrow F^{B}$;
...
that any \emph{proofs of properties} of ordinary liftings can be mechanically
...
(functor product), written using the function production operation
$\boxtimes$, into the proof of Statement~\ref{subsec:Statement-filterable-functor-product-1}
(filterable functor product). The same holds for the proofs of functor
co-product and functor composition constructions, for ordinary functors
and for filterable functors.
...
did not have the appropriate level of abstraction to see that. While
functional programmers may accept the work of writing these proofs
twice, a mathematician would not be satisfied without defining a ``generalized''
lifting and replacing $\text{Opt}$ by a functor $M$:
\begin{align*}
...
formulated for an ``$M$\textbf{-filterable}''\index{$M$-filterable functor}
functor $F$, where the functor $M$ could be later set to the identity
functor or the $\text{Opt}$ functor.
...
known as \textbf{monads}\index{monad}, in Chapter~\ref{chap:Semimonads-and-monads}. 

\subsection{Motivation and first notions of category theory}

So far, we have seen four examples of operations that resemble liftings:
\begin{align*}
...
\end{align*}
All these operations obey analogous laws of naturality, identity,
and composition, and differ only in the type of functions being lifted:
the ordinary function $A\rightarrow B$, the ``reversed'' type $B\rightarrow A$,
the Kleisli function $A\rightarrow M^{B}$, and the ``reversed''
Kleisli function $B\rightarrow M^{A}$. In turn, all these function
types obey the laws of identity and composition (the composition of
reversed functions needs to be performed in the reverse order for
the types to match).

In order to avoid writing essentially the same proofs multiple times,
we need to use a more abstract view of this situation: a new notion
of a functor that lifts more general or ``twisted'' function types
instead of the ordinary function types ($A\rightarrow B$). This notion
(and its further generalizations) is provided by\index{category theory}
...
\textbf{morphisms} $A\leadsto B$, which\index{morphism} can be in
any relation to $A$ and $B$ as long as the identity and composition
laws hold. The morphism types, the ``\index{identity morphism}identity
morphism'' of type $A\leadsto A$, and the composition operation
...
\hline 
\hline 
``plain'' & $\text{Int},\text{String},...$ & $f:A\rightarrow B$ & $\text{id}^{:A\rightarrow A}$ & $f\bef g$\tabularnewline
\hline 
``opposite'' & $\text{Int},\text{String},...$ & $f:B\rightarrow A$ & $\text{id}^{:A\rightarrow A}$ & $g\bef f$\tabularnewline
\hline 
``$M$-Kleisli'' & $\text{Int},\text{String},...$ & $f:A\rightarrow M^{B}$ & $\text{pu}_{M}:A\rightarrow M^{A}$ & $f\diamond_{_{M}}g$\tabularnewline
\hline 
``opposite $M$-Kleisli'' & $\text{Int},\text{String},...$ & $f:B\rightarrow M^{A}$ & $\text{pu}_{M}:A\rightarrow M^{A}$ & $g\diamond_{_{M}}f$\tabularnewline
\hline 
``$F$-lifted'' & $F^{\text{Int}},F^{\text{String}},...$ & $f:F^{A}\rightarrow F^{B}$ & $\text{id}^{:F^{A}\rightarrow F^{A}}$ & $f\bef g$\tabularnewline
\hline 
\end{tabular}
\par\end{center}

In functional programming, a functor is a type constructor $F^{\bullet}$
with a lawful lifting of functions $A\rightarrow B$ to functions
$F^{A}\rightarrow F^{B}$. Category theory\index{category theory!functor}
defines a \textbf{functor} much more\index{functor!in category theory}
generally \textemdash{} as a lawful lifting \emph{from one category
to another}. We will use the longer phrase ``categorical functor''
to distinguish the categorical notion of a functor from the programmer's
notion of type constructor with \lstinline!map!. 

Defining a categorical functor requires us to specify how types and
the morphisms from one category are mapped to types and morphisms
of another. The laws of identity and composition must hold according
to the rules of each category. That is, identity morphisms from the
first category must be mapped to those of the second category; and
the composition of any two morphisms from the first category must
be mapped to the composition of morphisms in the second category.
...
To formulate this categorically, we say that there must be a categorical
functor from the opposite category to the $C$-lifted category. To
define that functor, we need to specify how types and morphisms are
mapped from the first category to the second. Each type $A$ of the
opposite category is mapped to the corresponding type $C^{A}$ of
the $C$-lifted category. Each $(A\leadsto B)$-morphism $f^{:B\rightarrow A}$
of the opposite category is mapped to the $(A\leadsto B)$-morphism
...
the composition operation for each category:
\begin{align*}
{\color{greenunder}\text{for the opposite category}:}\quad & \text{id}^{:A\rightarrow A}\quad\text{and}\quad g\bef f\quad,\\
{\color{greenunder}\text{for the }C\text{-lifted category}:}\quad & \text{id}^{:C^{A}\rightarrow C^{A}}\quad\text{and}\quad f\bef g\quad.
\end{align*}
Now we require that the first category's identity morphism is mapped
to the second category's identity morphism, and that a composition
of any two morphisms (defined in the first category) is also mapped
...
To formulate the categorical definition for this situation, assume
that there is a functor from the $\text{Opt}$-Kleisli category to
the $F$-lifted category.
...
is mapped to the $(A\leadsto B)$-morphism $\text{liftOpt}_{F}(f):F^{A}\rightarrow F^{B}$
...
In both examples, the general laws of functor are equivalent to the
laws we derived previously. This gives us assurance that we have correctly
guessed the relevant laws, which is not always self-evident. For example,
we formulated the four laws of \lstinline!filter! in Section~\ref{subsec:Motivation-for-and-derivation-of-laws-of-filtering},
based on heuristic ideas. However, the laws of the \lstinline!filter!
function have been formulated differently by different people, also
starting from heuristic considerations. \footnote{E.g., with an additional ``empty-value'' law here: \texttt{\href{https://github.com/fantasyland/fantasy-land\#filterable}{https://github.com/fantasyland/fantasy-land\#filterable}}}.
It is not clear whether we correctly guessed all the relevant laws
of \lstinline!filter!, or introduced more laws than necessary. In
contrast, the two functor laws are very general and turn up time and
again in all kinds of different calculations. This gives us confidence
that these laws are correctly chosen and are useful in a wide range
of contexts. Proving that the four laws of \lstinline!filter! from
Section~\ref{subsec:Motivation-for-and-derivation-of-laws-of-filtering}
are equivalent to the two laws of a functor gives assurance that the
choice of filtering laws is reasonable and is likely to prove useful.

Having looked at the laws of \lstinline!liftOpt!, we also noticed
that we can reduce the number of different proofs if we generalize
from the $\text{Opt}$-Kleisli category to an $M$-Kleisli category
with a functor $M$ to be chosen later. One can prove general theorems
about products, co-products, and composition of (categorical) functors
that map between two fixed categories. The proofs are very similar
to the proofs of our type constructions. In this way, we can replace
four derivations (say, for the co-product of functors, contrafunctors,
filterable functors, and filterable contrafunctors) by a single but
more abstract proof about the co-product of (categorical) functors.

The categorical view shows us two directions for developing the theory
further, hoping to find useful applications. First, we can look for
functors $M$ (called ``monads'') that admit the Kleisli composition
with the properties (the identity and the associativity laws) required
by an $M$-Kleisli category. Second, having found some new monads
$M$, we can look for ``$M$-filterable'' functors $F$ that admit
an operation $\text{lift}_{M,F}$ that replaces $\text{liftOpt}_{F}$
and satisfies the analogous laws. We will see examples of $M$-filterable
functors later in this book.

To summarize, using category theory's notion of functor brings several
important advantages:
\begin{itemize}
\item We have assurance that we formulated the laws correctly.
\item Several directions for generalization are suggested.
\item Many proofs can be reduced to a single proof for properties of (categorical)
functors.
\end{itemize}
In this way, category theory serves as a useful tool for general reasoning
tasks, especially at the level of abstraction where we need to work
with laws of typeclasses for type constructors.

What does category theory \emph{not} do for functional programming? 
\begin{itemize}
\item It does not directly provide any proofs of specific laws for a given
function, and does not give any guidance towards finding such proofs
or derivations.
\item It does not help in determining whether a given type constructor is,
say, a filterable functor, or a pointed functor, or a monad.
\item It does not help in implementing typeclasses or instances that obey
the required laws, when that \emph{can} be done. 
\item It does not say whether there exists a natural transformation between
two given functors, or how to implement one.
\item It suggests reasonable and rigorous ways of defining many abstract
constructions, but does not say which of these constructions will
be useful in practical programming.
\end{itemize}
These questions may be \emph{motivated} by category theory, but answering
them requires symbolic derivations using special techniques that category
theory does not provide. Developing these techniques (while avoiding
unnecessary generalizations) is one of the main themes of this book.
