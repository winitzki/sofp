...
later in this chapter. Running the \texttt{scalacheck} tests does
...
values that violate the predicate \lstinline!p! from the sequence.
The filtering function also removes other values that may or may not
violate the predicate, but removing additional values is compatible
with the filtering laws.
...
So we can implement \lstinline!filter! as e.g.~this code,
\begin{lstlisting}
...
lose information. Instead, we may apply \lstinline!c(a)! and obtain
a value of type $\bbnum 1+Z$.
...
Implement a \lstinline!filter! function for a regular-shaped tree
$R^{A}$ defined by
\[
