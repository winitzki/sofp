...
function \lstinline!filter! (code notation $\text{filt}_{F}$) with
type signature
\[
\text{filt}_{F}:(A\Rightarrow\bbnum 2)\Rightarrow F^{A}\Rightarrow F^{A}\quad.
\]
The laws are called the naturality, composition, identity, and partial
function laws of \lstinline!filter!:\index{composition law!of filter@of \texttt{filter}}\index{naturality law!of filter@of \texttt{filter}}\index{identity law!of filter@of \texttt{filter}}
\begin{align*}
...
to write randomized tests for the four laws of filterable functors:
\begin{lstlisting}
def checkFilterableLaws[F[_] : Filterable : Functor, A, B](implicit
...
checkFilterableLaws[Orders, Int, String] // Need to set type parameters to specific types.
\end{lstlisting}
...
checkFilterableLaws[Orders, Int, String] // Tests pass.
\end{lstlisting}
However, implementing the extra business rule \textbf{(b)} will violate
some laws:
\begin{lstlisting}
implicit val filterableOrdersRuleB = new Filterable[Orders] {
  def filt[A](p: A => Boolean)(fa: F[A]): F[A] =
    if (fa.tue.exists(p) || fa.fri.exists(p))) fa
    else Orders(None, None) // Rule (b): Both orders are approved if at least one is approved.
}
checkFilterableLawsWithFilter[Orders, Boolean, Boolean]() // Tests fail!

scala> Orders(Some(500), Some(2000)).filter(x => x < 1000).filter(x => x > 1000)
res0: Orders[Int] = Orders(Some(500),Some(2000))

scala> Orders(Some(500), Some(2000)).filter(x => x < 1000 && x > 1000) // Breaks composition law.
...
with the business rule \textbf{(b)} is an example of a filtering operation
that does not correspond to our intuitions and, as a consequence,
violates the filtering laws. However, the same functor \lstinline!Orders!
with business rule \textbf{(a)} is filterable. It is up to the programmer
to verify that the implementation of the \lstinline!filter! function
satisfies the laws.

What are other examples of functors that are \emph{not} filterable?
One way of implementing the type signature of the \lstinline!filter!
function is to use the identity function,
\begin{lstlisting}
def filt[A](p: A => Boolean)(fa: F[A]): F[A] = fa  // Ignore the predicate `p`; always return `fa`.
\end{lstlisting}
There are some functors $F$ (e.g.~the identity functor $F^{A}\triangleq A$
or the exponential functor $F^{A}\triangleq E\Rightarrow A$) for
which this implementation is the only one possible. However, this
implementation never removes any data and so will violate the partial
function law if the functor $F^{A}$ wraps a value of type $A$ that
should not pass the filter. So, functors of the form $F^{A}\triangleq E\Rightarrow A$
...
are filterable if the function $\text{filt}_{F}$ is defined correctly;
but incorrect implementations are also possible. For example, one
could imagine defining \lstinline!filter! for an \lstinline!Option!
type as
\begin{lstlisting}
...
\end{lstlisting}
This definition discards information and so violates the identity
law: the result of filtering with an identically \lstinline!true!
predicate is not the identity function of type \lstinline!Option[A] => Option[A]!.
...
function must be fully parametric and should not use hard-coded values
...
Is the functor $F^{A}\triangleq\bbnum 1+A\times A$ filterable?
...
\textbf{(a)} The functor $P^{T}$ defined by this Scala code,
\begin{lstlisting}
...
\textbf{(b)} The functor $R^{A}$ defined by this Scala code,
\begin{lstlisting}
