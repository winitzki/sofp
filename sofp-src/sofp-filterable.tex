...
Is the simplest regular-shaped tree $R^{A}$ defined by $R^{A}\triangleq A+R^{A\times A}$
filterable?
...
Are \lstinline!deflate! and \lstinline!filter! computationally equivalent\index{computational equivalence!example}?
...
and thus Eq.~(\ref{eq:deflate-derivation-3}) gives \lstinline!deflate!$^{\prime}=$\lstinline!deflate!.
...
Are these two functions computationally equivalent?\index{computational equivalence!example}
...
of the law~(\ref{eq:naturality-law-general-parameterized-transformation})
 applied to an arbitrary $p^{:F^{A}}$and considered as functions
of $c^{:C^{B}}$.
