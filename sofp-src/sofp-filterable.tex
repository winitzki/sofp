...
so that the function $c$ is applied only to values that pass the
...
\]
whose role is similar to that of \lstinline!deflate! for filterable
functors. We can relate these functions by
\begin{align*}
...
As an illustration, we implement these functions for the two examples
...
\end{equation}
We have not yet seen the \textbf{identity law} for \lstinline!deflate!;
it is quick to derive it\index{identity laws!for deflate@for \texttt{deflate}}
from Eq.~(\ref{eq:identity-law-of-filter}),
\begin{equation}
\text{filt}_{H}(\_\rightarrow\text{true})=\psi_{(\_\rightarrow\text{true})}^{\uparrow H}\bef\text{deflate}_{H}=\text{inflate}_{H}\bef\text{deflate}_{H}=\text{id}\quad.\label{eq:identity-law-of-deflate}
\end{equation}
Since Eq.~(\ref{eq:emptyable-wrapper-psi-p-filter-derivation1})
...
\end{equation}
(See definition of \lstinline!inflate! in Section~\ref{subsec:Simplifying-the-filtering-laws-deflate}.)
...
\begin{align*}
 & \text{inflate}_{C}:(A\rightarrow Z)\rightarrow\bbnum 1+A\rightarrow Z\quad,\\
 & \text{inflate}_{C}=c^{:A\rightarrow Z}\rightarrow p^{\bbnum 1+A}\rightarrow\text{???}^{:Z}\quad.
\end{align*}
...
Prove rigorously (not via tests) that $\text{flm}_{\text{Opt}}(\text{pu}_{\text{Opt}})=\text{id}$.
...
for any functor $F^{A}$ (not necessarily filterable). Prove that
in case $C^{\bullet}$ and $F^{\bullet}$ \emph{are} filterable, the
\textbf{identity laws}\index{identity laws!of deflate@of \texttt{deflate}}\index{identity laws!of inflate@of \texttt{inflate}}
will hold:
\[
\text{inflate}_{C}\bef\text{deflate}_{C}=\text{id}^{:C^{A}\rightarrow C^{A}}\quad,\quad\quad\text{inflate}_{F}\bef\text{deflate}_{F}=\text{id}^{:F^{A}\rightarrow F^{A}}\quad.
\]
Show on examples that the inverse equations do not hold in general:
$\text{deflate}\bef\text{inflate}\neq\text{id}$.
...
prove that $C^{H^{A}}$ is a filterable \emph{functor}.
