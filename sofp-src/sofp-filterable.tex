...
function laws of filtering:\index{composition law!of filter@of \texttt{filter}}\index{naturality law!of filter@of \texttt{filter}}\index{identity laws!of filter@of \texttt{filter}}
\begin{align}
{\color{greenunder}\text{naturality law of }\text{filt}_{F}:}\quad & f^{\uparrow F}\bef\text{filt}_{F}(p)=\text{filt}_{F}(f\bef p)\bef f^{\uparrow F}\quad\text{for any }f^{:A\rightarrow B},~p^{:B\rightarrow\bbnum 2}\quad.\label{eq:naturality-law-of-filter}\\
...
{\color{greenunder}\text{partial function law of }\text{filt}_{F}:}\quad & \text{filt}_{F}(p)\bef f^{\uparrow F}=\text{filt}_{F}(p)\bef f_{|p}^{\uparrow F}\quad\text{for any }f^{:A\rightarrow B},~p^{:A\rightarrow\bbnum 2}\quad.\label{eq:partial-function-law-of-filter}
\end{align}
...
Below we will need to use this function often, so let us call it $\psi_{p}$
for brevity:
...
\lstinline!filter! automatically like this:
\begin{lstlisting}
...
for all sequences, similarly to the \lstinline!.flatten! method with
type \lstinline!Seq[Seq[A]] => Seq[A]!.
...
and then define a new function \lstinline!filter_prime! via Eq.~(\ref{eq:def-filter-through-deflate}):
\[
...
\]
Then \lstinline!filter_prime! the same function as \lstinline!filter!,
...
\end{align}
To proceed with the calculation, we need to simplify expressions $\psi_{p}\bef\text{nonEmpty}$
and $\psi_{p}\bef\text{get}$. Begin with writing the code notation
for the methods \lstinline!.nonEmpty! and \lstinline!.get!, defined
by the Scala library for the \lstinline!Option! type, but using the
equivalent type \lstinline!Option[Unit]! (i.e.~$\bbnum 1+\bbnum 1$)
instead of \lstinline!Boolean! (i.e.~$\bbnum 2$):
\begin{lstlisting}
...
\end{lstlisting}
\begin{align*}
{\color{greenunder}\text{view }p^{:A\rightarrow\bbnum 2}(x^{:A})\text{ as having type }\bbnum 1+\bbnum 1:}\quad & \psi_{p}\triangleq x^{:A}\rightarrow p(x)\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
...
\begin{align*}
{\color{greenunder}\text{expect to equal }x\triangleright p:}\quad & x^{:A}\triangleright\psi_{p}\bef\text{nonEmpty}=x\triangleright p\bef(\_^{:\bbnum 1}\rightarrow x)^{\uparrow\text{Opt}}\bef(\_\rightarrow1)^{\uparrow\text{Opt}}\\
{\color{greenunder}\text{functor composition law of }\text{Opt}:}\quad & =x\triangleright p\bef(\gunderline{\_^{:\bbnum 1}\rightarrow1})^{\uparrow\text{Opt}}=x\triangleright p\bef\big(\text{id}^{:\bbnum 1\rightarrow\bbnum 1}\gunderline{\big)^{\uparrow\text{Opt}}}=x\triangleright p\quad.
\end{align*}
The composition $\psi_{p}\bef\text{get}$ is simplified to the partial
function $\text{id}_{|p}$:
\begin{align*}
 & x^{:A}\triangleright\psi_{p}\bef\text{get}=p(x)\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
\bbnum 1 & \bbnum 0 & (\_\rightarrow x)
...
{\color{greenunder}\text{partial function law~(\ref{eq:partial-function-law-of-filter}) of }\text{filt}_{F}:}\quad & =\text{filt}_{F}(p)\bef\gunderline{(\text{id})^{\uparrow F}}=\text{filt}_{F}(p)\bef\text{id}=\text{filt}_{F}(p)\quad.
\end{align*}
So, the new \lstinline!filter_prime! function is equal to the original
...
Beginning with a given \lstinline!deflate! function, we first define
the corresponding \lstinline!filter! function via Eq.~(\ref{eq:def-filter-through-deflate})
and then use Eq.~(\ref{eq:def-deflate-via-filter}) to define a new
function \lstinline!deflate_prime!:
\[
...
\]
Then \lstinline!deflate_prime! is the same function as \lstinline!deflate!,
assuming that the naturality law holds.
...
unless we can somehow permute the order of function compositions so
that $\psi^{\uparrow F}$ and $\text{get}^{\uparrow F}$ are composed
together. To achieve that, we need to assume a law that switches the
order of lifted function compositions around \lstinline!deflate!.
The naturality law~(\ref{eq:naturality-law-of-filter}) of \lstinline!filter!
is of that form; so let us derive the naturality law required for
\lstinline!deflate!.

\begin{wrapfigure}{l}{0.35\columnwidth}%
\vspace{-2\baselineskip}
\[
\xymatrix{\xyScaleY{2.0pc}\xyScaleX{5.0pc}F^{\bbnum 1+A}\ar[r]\sp(0.5){\text{deflate}} & F^{A}\ar[d]\sp(0.45){(f^{:A\rightarrow B})^{\uparrow F}}\\
...
\noindent In order to allow us to switch the order of composition
of \lstinline!deflate! with a lifted function, the required law must
be an equation of the form
\[
\text{deflate}\bef f^{\uparrow F}=(\text{???})^{\uparrow F}\bef\text{deflate}\quad,
\]
which is illustrated in the type diagram above. The types match in
the right-hand side only if the argument of deflate is of type $F^{\bbnum 1+B}$.
So, the law must have the form

\begin{wrapfigure}{l}{0.35\columnwidth}%
\vspace{-2\baselineskip}
\[
\xymatrix{\xyScaleY{2.0pc}\xyScaleX{5.0pc}F^{\bbnum 1+A}\ar[r]\sp(0.5){\text{deflate}}\ar[d]\sp(0.5){(g^{:\bbnum 1+A\rightarrow\bbnum 1+B})^{\uparrow F}} & F^{A}\ar[d]\sp(0.45){(f^{:A\rightarrow B})^{\uparrow F}}\\
...
\noindent 
\[
\text{deflate}\bef f^{\uparrow F}=(g)^{\uparrow F}\bef\text{deflate}\quad,
\]
where $g^{:\bbnum 1+A\rightarrow\bbnum 1+B}$ is a suitable function.
To compute $g$, note that we can lift $f^{:A\rightarrow B}$ to the
\lstinline!Option! functor $\text{Opt}^{A}\triangleq\bbnum 1+A$
and obtain $g\triangleq f^{\uparrow\text{Opt}}$. So, the final form
of the naturality law for \lstinline!deflate! is

\begin{wrapfigure}{l}{0.35\columnwidth}%
\vspace{-2\baselineskip}
\[
\xymatrix{\xyScaleY{2.0pc}\xyScaleX{5.0pc}F^{\bbnum 1+A}\ar[r]\sp(0.5){\text{deflate}}\ar[d]\sp(0.5){(f^{\uparrow\text{Opt}})^{\uparrow F}} & F^{A}\ar[d]\sp(0.45){(f^{:A\rightarrow B})^{\uparrow F}}\\
F^{\bbnum 1+B}\ar[r]\sp(0.5){\text{deflate}} & F^{B}
}
\]
\vspace{-1\baselineskip}
...
towards showing that \lstinline!deflate_prime! equals \lstinline!deflate!:
\begin{align}
...
\end{align}
To simplify the function $\psi_{\text{nonEmpty}}\bef\text{get}^{\uparrow\text{Opt}}$,
which must be a function of type $\bbnum 1+A\rightarrow\bbnum 1+A$
for the types to match, we perform a separate calculation. The lifting
operation $^{\uparrow\text{Opt}}$ for the \lstinline!Option! functor
is written as
\[
...
\hline \bbnum 1 & \text{id} & \bbnum 0\\
C & \bbnum 0 & h
\end{array}\quad,\quad\quad\text{get}^{\uparrow\text{Opt}}=\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
...
Then we compute
\begin{align*}
 & x^{:\bbnum 1+A}\triangleright\psi_{\text{nonEmpty}}\bef\text{get}^{\uparrow\text{Opt}}=\text{nonEmpty}(x)\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & \bbnum 1+A\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
\bbnum 1 & \bbnum 0 & (\_\rightarrow x)
...
\end{array}\,\bef\,\text{get}^{\uparrow\text{Opt}}=x\triangleright\begin{array}{|c||ccc|}
 & \bbnum 1 & \bbnum 1 & A\\
\hline \bbnum 1 & (\_\rightarrow1) & \bbnum 0 & \bbnum 0\\
A & \bbnum 0 & \bbnum 0 & (\_\rightarrow x)
\end{array}\,\bef\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
\bbnum 1 & \bbnum 0 & \bbnum 0\\
...
through Eq.~(\ref{eq:def-filter-through-deflate}). Then compute:
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{filt}_{F}(p)\bef f^{\uparrow F}:}\quad & \gunderline{\text{filt}_{F}(p)}\bef f_{|p}^{\uparrow F}\\
{\color{greenunder}\text{use Eq.~(\ref{eq:def-filter-through-deflate}), define }\text{filt}_{F}\text{ via }\text{deflate}:}\quad & =\psi_{p}^{\uparrow F}\bef\gunderline{\text{deflate}\bef f_{|p}^{\uparrow F}}\\
\text{naturality law~(\ref{eq:naturality-law-of-deflate}) of }\text{deflate}: & =\gunderline{\psi_{p}^{\uparrow F}\bef f_{|p}^{\uparrow\text{Opt}\uparrow F}}\bef\text{deflate}=\big(\psi_{p}\bef f_{|p}^{\uparrow\text{Opt}}\big)^{\uparrow F}\bef\text{deflate}\quad.
\end{align*}
A similar calculation with $f$ instead of $f_{|p}$ gives 
\[
\text{filt}_{F}(p)\bef f^{\uparrow F}=\big(\psi_{p}\bef f^{\uparrow\text{Opt}}\big)^{\uparrow F}\bef\text{deflate}\quad.
\]
...
\end{array}\quad.
\end{align*}
In the last expression, the function $f$ is used only in the case
$p(x)=\text{true}$. So, the result is the same if we replace $f$
by the partial function $f_{|p}$ defined by Eq.~(\ref{eq:def-partial-f})
...
which can be viewed as two naturality laws specific to these functions.
...
\subsection{Motivation for the \texttt{liftOpt} function}
...
\begin{align*}
{\color{greenunder}\text{since }\bbnum 2\cong\bbnum 1+\bbnum 1\text{, view }p^{:A\rightarrow\bbnum 2}(x^{:A})\text{ as having type }\bbnum 1+\bbnum 1:}\quad & \psi_{p}=x^{:A}\rightarrow p(x)\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
\bbnum 1 & \bbnum 0 & (\_\rightarrow x)
\end{array}\quad.
...
\paragraph{Structure of filterable functors}
