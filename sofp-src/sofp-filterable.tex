...
{\color{greenunder}\text{Eqs.~(\ref{eq:composition-law-of-flatMap-for-Option}) and~(\ref{eq:right-naturality-flatmap-option}) of }\text{flm}_{\text{Opt}}:}\quad & \gunderline{f\bef\text{flm}_{\text{Opt}}(g)}\bef\text{flm}_{\text{Opt}}(p)\bef\big(\gunderline{\text{liftOpt}_{H}(g)\bef\text{liftOpt}_{H}(f)}\big)^{\uparrow\text{Opt}}\\
...
\end{align*}
Some of these naturality laws came from intuition about fully parametric
functions that rearrange data inside wrappers without knowing the
type of that data. Other naturality laws followed directly from code
of specific functions such as $\psi$, or were postulated in order
to prove other necessary laws. It is remarkable that all naturality
laws, derived for different functions and from different premises,
are similar to each other and can be categorized into three patterns
shown in the following table, where we use arbitrary functors $F$,
$G$, $H$ and contrafunctors $C$:
\begin{center}
\begin{tabular}{|c|c|c|}
\hline 
...
\hline 
{\small{}generalized lifting; $A$-naturality } & {\small{}$t^{A,B}:(G^{A}\rightarrow H^{B})\rightarrow F^{A}\rightarrow F^{B}$} & {\small{}$(f^{:A\rightarrow B})^{\uparrow F}\bef t^{B,C}(g)=t^{A,C}(f^{\uparrow G}\bef g)$}\tabularnewline
\hline 
{\small{}generalized lifting; $B$-naturality} & {\small{}$t^{A,B}:(G^{A}\rightarrow H^{B})\rightarrow F^{A}\rightarrow F^{B}$} & {\small{}$t^{A,B}(g)\bef(h^{:B\rightarrow C})^{\uparrow F}=t^{A,C}(g\bef h^{\uparrow H})$}\tabularnewline
\hline 
\end{tabular}
\par\end{center}
...
The first pattern covers functions with type signatures of the form
$F^{A}\rightarrow G^{A}$ where $F$ and $G$ must be both functors
or both contrafunctors. Functions of that kind are called \textbf{natural
transformations} between $F$ and $G$.\index{natural transformation}
A natural transformation is a fully parametric function $t^{A}:F^{A}\rightarrow G^{A}$.
Heuristically, we may view $t^{A}$ as a function that copies data
of type $A$ from one ``wrapper'' to another and may rearrange that
data in a way that does not depend on $A$. An example is \lstinline!List!'s
\lstinline!.headOpt! method,
\[
...
function of type $\text{List}^{A}\rightarrow\text{Opt}^{A}$, such
as the methods \lstinline!.lastOption! or \lstinline!.find(p)!.
...
``natural transformation'' pattern, which we can formulate generally
for arbitrary functors $F$ and $G$:
...
\xyScaleY{1.5pc}\xyScaleX{3.5pc}F^{B}\ar[r]\sp(0.55){t^{B}} & G^{B}
}
\]
...
~\vspace{-0.5\baselineskip}
\[
(f^{:A\rightarrow B})^{\uparrow F}\bef t^{:F^{B}\rightarrow G^{B}}=t^{:F^{A}\rightarrow G^{A}}\bef f^{\uparrow G}\quad.
\]
...
function $f^{:A\rightarrow B}$ with arbitrary types $A$, $B$. Types
...
\xyScaleY{1.5pc}\xyScaleX{3.5pc}C^{B}\ar[r]\sp(0.55){t^{B}} & D^{B}
}
\]
...
~\vspace{-0.5\baselineskip}
\[
(f^{:B\rightarrow A})^{\downarrow C}\bef t^{:C^{B}\rightarrow D^{B}}=t^{:C^{A}\rightarrow D^{A}}\bef f^{\downarrow D}\quad.
\]
...
The second pattern is a ``parameterized transformation''; it is
a curried function that takes a first argument of type $C^{A}$ and
returns a natural transformation of type $F^{A}\rightarrow G^{A}$.
An example of this function is $\text{filt}_{F}$. The naturality
law for functions of this kind must modify the argument of type $C^{A}$
when changing the order of lifted functions. In order to formulate
the naturality law for a parameterized transformation $t^{A}:C^{A}\rightarrow F^{A}\rightarrow G^{A}$,
...
\[
f^{\uparrow F}\bef t(c)=t(c\triangleright f^{\downarrow C})\bef f^{\uparrow G}\quad.
\]

The third pattern is a ``lifting''.{*}{*}{*}
...
{*} Category theory\index{category theory} accommodates this via
a generalized definition of functors as liftings between ``twisted''
types. Compare:
