...
before a ``source\index{functor block!source}'', i.e.~a value
...
intermediate value of the same ``source'' functor type. The result
...
of type $A$ from one ``wrapper'' to another and may rearrange that
data in a way that does not depend on $A$. An example is the \lstinline!.headOpt!
...
of type constructor with \lstinline!map!.\footnote{In category theory, programmer's functors are called \textbf{endofunctors}\index{endofunctor}\textemdash{}
categorical functors from a category $\mathcal{C}$ to itself.}
