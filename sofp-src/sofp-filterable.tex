...
be written using Scala's \lstinline!for! / \lstinline!yield! block,
...
code written via method chains:
...
into mathematical laws for the function \lstinline!withFilter!. To
look systematically for data types that support a filtering operation
with these properties, we will then perform structural analysis.
...
val result = for {  // Some computations in the context of the `List` functor.
  ...
  k(x, y, z)     // compute the new values and put them into the `result` list.
\end{lstlisting}
{\footnotesize\par}
...
the filtering operation as the equivalence of the expressions \lstinline!result1!
...
despite filtering with the predicate ``\lstinline!x < 1000!''.
This happened because the code of \lstinline!filter! does not remove
the value \lstinline!x = 2000! from the data structure%
\begin{comment}
...
To motivate such as reformulation, begin by considering a heuristic
