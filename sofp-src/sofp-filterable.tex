...
a precise definition of ``filterable'' type constructors \textemdash{}
...
using Scala's \lstinline!for!/\lstinline!yield! syntax, which is
...
The name ``functor block'' is chosen because the code is a single
expression (a ``block'') whose type is required to be a functor.
...
(meaning ``\lstinline!false!'' + ``\lstinline!true!''), we can
...
\begin{align}
\text{naturality law}:\quad & f^{\uparrow F}\bef\text{filt}_{F}(p)=\text{filt}_{F}(f\bef p)\bef f^{\uparrow F}\quad\text{for }\forall(f^{:A\rightarrow B},~p^{:B\rightarrow\bbnum 2})\quad.\label{eq:naturality-law-of-filter}\\
\text{identity law}:\quad & \text{filt}_{F}(\_\rightarrow\text{true})=\text{id}^{:F^{A}\rightarrow F^{A}}\quad.\label{eq:identity-law-of-filter}\\
\text{composition law}:\quad & \text{filt}_{F}(p_{1})\bef\text{filt}_{F}(p_{2})=\text{filt}_{F}(x\rightarrow p_{1}(x)\wedge p_{2}(x))\quad\text{for }\forall(p_{1}^{:A\rightarrow\bbnum 2},~p_{2}^{:A\rightarrow\bbnum 2})\quad.\label{eq:composition-law-of-filter}\\
\text{partial function law}:\quad & \text{filt}_{F}(p)\bef f^{\uparrow F}=\text{filt}_{F}(p)\bef f_{|p}^{\uparrow F}\quad\text{for }\forall(f^{:A\rightarrow B},~p^{:A\rightarrow\bbnum 2})\quad.\label{eq:partial-function-law-of-filter}
\end{align}
...
pass the filtering condition. In examples of non-filterable functors
$F$, such as $F^{A}\triangleq A\times\left(\bbnum 1+A\right)$, the
type expression for $F^{A}$ cannot accommodate replacing the type
$A$ by $\bbnum 1$. 
...
empty \lstinline!Option! values (\lstinline!None!). However, it
is safe to use $\text{get}^{\uparrow F}$ here, because the partial
function law of filtering guarantees that \lstinline!.filter(_.nonEmpty)!
...
$F$'s \lstinline!filter! obeys its partial function law.
...
\end{array}\\
\text{definition~(\ref{eq:def-of-nonEmpty-and-get}) of }\text{nonEmpty}:\quad & =x\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & \bbnum 1\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
...
is pronounced ``cli-slee'' (``cli'' as in ``climb'').}\textbf{ composition}.\label{kleisli-composition} It applies to functions
...
To define a contrafunctor via type recursion, we need to use a profunctor\index{profunctor}
...
library provides the generally most useful implementations of filtering
(e.g., the \lstinline!filter! or \lstinline!takeWhile! methods on
sequences), in some situations the programmer will need to provide
a custom implementation of \lstinline!filter! for a custom data type.
The programmer must examine the given requirements and decide whether
those requirements can be implemented as a lawful \lstinline!filter!
function.
...
The \index{unrolling trick for recursive types}\textbf{unrolling
trick} works for any recursive definition of the form $F^{A}\triangleq P^{A}+Q^{A}\times F^{A}$:
\begin{equation}
...
together with recursion. This is a consequence of the ``parametricity
theorem''\index{parametricity theorem}, which is beyond the scope
...
laws. The choice of laws is not self-evident. For example, Section~\ref{subsec:Motivation-for-and-derivation-of-laws-of-filtering}
...
by different people, also starting from heuristic considerations.\footnote{E.g., with an additional ``empty-value'' law here: \texttt{\href{https://github.com/fantasyland/fantasy-land\#filterable}{https://github.com/fantasyland/fantasy-land\#filterable}}}
...
new monads $M$, we can look for ``$M$-filterable'' functors\index{$M$-filterable functor}
...
contrafunctors later in this book.

To summarize, using the category theory's notion of functor brings
the following advantages:
\begin{itemize}
\item We are assured that we found a correct formulation of the laws of
a typeclass. Instead of using heuristic considerations, we deduce
those laws from the two laws of (categorical) functors. 
\item We can find some promising directions for finding more general type
constructions.
\item Several proofs can be replaced by a single proof for properties of
...
What does category theory (CT) \emph{not} do for functional programming? 
\begin{itemize}
...
\item For checking laws of a specific function (e.g., the laws of the \lstinline!Filterable!
typeclass for the \lstinline!takeWhile! function), CT provides neither
any proofs nor any guidance towards finding such proofs.
\item CT does not help in determining whether a given type constructor is,
say, a filterable functor, or a pointed functor, or a monad.
\item If we know that, say, a lawful \lstinline!Filterable! instance can
be implemented for a given type constructor, CT does not help in writing
correct code for that typeclass instance.
\item CT does not say whether there exists a natural transformation between
two given functors, or how to implement one if it exists.
\end{itemize}
Performing these tasks requires symbolic derivations using techniques
