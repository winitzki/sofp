...
can be derived from Eqs.~(\ref{eq:combined-identity-naturality-of-liftOpt})\textendash (\ref{eq:composition-law-of-liftOpt})
...
\end{equation}
To check the naturality-identity law~(\ref{eq:combined-identity-naturality-of-liftOpt})
...
\end{align*}
 The same expression is found by applying the right-hand side of Eq.~(\ref{eq:combined-identity-naturality-of-liftOpt})
to $\bbnum 0+a\times r$:
\[
...
{\color{greenunder}\text{law~(\ref{eq:combined-identity-naturality-of-liftOpt}) of }\text{liftOpt}_{S}:}\quad & =f^{\uparrow S^{\bullet,R}}\bef\big(\gunderline{\overline{\text{liftOpt}_{F}}(f\bef\text{pu}_{\text{Opt}})}\big)^{\uparrow S^{B,\bullet}}\\
...
\subsection{Filterable contrafunctors: definition and examples}
...
Consider the contrafunctor $C^{A}\triangleq A\rightarrow\bbnum 1+Z$,
where $Z$ is a constant type. This contrafunctor ``consumes'' data
of type $A$ and computes values of type $\bbnum 1+Z$. Given a value
$c$ of type $C^{A}$,
\begin{lstlisting}
val c: A => Option[Z] = ???
\end{lstlisting}
we would like to impose a filter predicate $p^{:A\rightarrow\bbnum 2}$
so that the function $c$ is only applied to values that pass the
predicate. The result is a new function $d^{:C^{A}}$ defined by the
code
\begin{lstlisting}
...
Another example is the contrafunctor $C^{A}\triangleq\bbnum 1+A\rightarrow Z$.
The filtering transformation is
\begin{lstlisting}
...
\end{lstlisting}
\[
\text{filt}_{C}(p^{:A\rightarrow\bbnum 2})\triangleq c^{:\bbnum 1+A\rightarrow Z}\rightarrow\text{filt}_{\text{Opt}}(p)\bef c\quad.
\]

We have seen two definitions of a filterable contrafunctor $C^{\bullet}$:
one through the \lstinline!filter! method,
\[
\text{filt}_{C}(p^{:A\rightarrow\bbnum 2}):C^{A}\rightarrow C^{A}\quad,
\]
and another through \lstinline!liftOpt! as shown in Eq.~(\ref{eq:type-signature-liftOpt-contrafunctors}).
These two definitions are equivalent when suitable laws are assumed
to hold, similarly to the situation with filterable functors. 

We can also consider the function \lstinline!inflate! with the type
signature
\[
\text{inflate}:C^{A}\rightarrow C^{\bbnum 1+A}\quad,
\]
which is similar to \lstinline!deflate! for filterable functors.
We can relate these functions as
\begin{align*}
\text{filt}_{C}(p)=\text{inflate}^{:C^{A}\rightarrow C^{\bbnum 1+A}}\bef(\psi_{p}^{:A\rightarrow\bbnum 1+A})^{\downarrow C}\quad, & \quad\quad\text{inflate}^{:C^{A}\rightarrow C^{\bbnum 1+A}}=\text{filt}_{C}(\_\rightarrow\text{true})\bef\big(\text{get}^{:\bbnum 1+A\rightarrow A}\big)^{\downarrow C}\quad,\\
\text{liftOpt}_{C}(f^{:A\rightarrow\bbnum 1+B})=\text{inflate}^{:C^{B}\rightarrow C^{\bbnum 1+B}}\bef f^{\downarrow C}\quad, & \quad\quad\text{inflate}^{:C^{A}\rightarrow C^{\bbnum 1+A}}=\text{liftOpt}_{C}(\text{id}^{:\bbnum 1+A\rightarrow\bbnum 1+A})\quad.
\end{align*}
...
We analyze the structure of this data type to decide which constructions
we may use.
...
Now we can rewrite $F^{A}=R_{1}\left[G^{A}+R_{2}\left[H^{A}\right]\right]$
...
filterable by constructions 1 and 3 with the Option functor $1+A$
...
Show rigorously that the functor $F^{A}\triangleq\bbnum 1+A\times A+A\times\left(\bbnum 1+A\right)\times F^{A}$
is filterable.
...
Show rigorously (not by tests) that $\text{flm}_{\text{Opt}}(\text{pu}_{\text{Opt}})=\text{id}$.
...
to how one can define \lstinline!inflate!$:F^{A}\rightarrow F^{\bbnum 1+A}$
for any functor $F^{A}$ (not necessarily filterable). Show that if
$C^{\bullet}$ and $F^{\bullet}$ \emph{are} filterable, the \textbf{identity
laws}\index{identity laws!of deflate@of \texttt{deflate}} will hold:
\[
\text{deflate}_{C}\bef\text{inflate}_{C}=\text{id}^{:C^{\bbnum 1+A}\rightarrow C^{\bbnum 1+A}}\quad,\quad\quad\text{inflate}_{F}\bef\text{deflate}_{F}=\text{id}^{:F^{A}\rightarrow F^{A}}\quad.
\]


\subsubsection{Exercise \label{subsec:Exercise-filterable-laws-3}\ref{subsec:Exercise-filterable-laws-3}}

Assuming that $G^{\bullet}$ is a filterable functor, show rigorously
