...
\begin{empheq}[box=\mymathbgbox]{align*}
{\color{greenunder}\text{left-hand side of Eq.~(???)}:}\quad & f^{\uparrow F}\bef\text{filt}_{F}(p)=f^{\uparrow F}\bef\psi_{p}^{\uparrow F}\bef\text{deflate}=\text{liftOpt}\,(f\bef\psi_{p})\quad.\\
...
$A\rightarrow\bbnum 1+\bbnum 1\cong\text{Opt}^{\bbnum 1}$,
\begin{empheq}[box=\mymathbgbox]{equation}
