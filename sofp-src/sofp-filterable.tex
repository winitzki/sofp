...
in the type $C^{A}$. In other words, we consider the contrafunctor
$C^{A}\triangleq Z\times\left(A\rightarrow Z\right)$ instead of simply
$A\rightarrow Z$. Second, by using a type $\bbnum 1+Z$ instead of
$Z$, since the type $\bbnum 1+Z$ has a default value $1+\bbnum 0^{:Z}$.
We will now implement filtering for these contrafunctors.
