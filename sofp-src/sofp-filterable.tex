...
Two simple examples of filterable contrafunctors are $C^{A}\triangleq A\rightarrow\bbnum 1+Z$
and $C^{A}\triangleq\bbnum 1+A\rightarrow Z$, where $Z$ is a fixed
type.

An example of a non-filterable contrafunctor is $C^{A}\triangleq A\rightarrow Z$.
...
Show that one can define deflate$:C^{1+A}\rightarrow C^{A}$ for any
contrafunctor $C^{A}$ (not necessarily filterable), similarly to
how one can define inflate$:F^{A}\rightarrow F^{1+A}$ for any functor
$F^{A}$ (not necessarily filterable). 
