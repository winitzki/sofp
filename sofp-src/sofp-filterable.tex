...
a precise definition of type constructors (``filterable functors'')
...
only the positive values of $\cos(x)$. It is safe to apply the square
root function to positive values, so the code will work correctly.
...
this book. The \lstinline!.filter! operation is represented by an
\lstinline!if! keyword embedded in a functor block. Compare each
line of the functor block code with the corresponding line of the
code written via method chains with some more verbosity:
...
The name ``functor block'' is chosen because it is a single expression
(a ``block of code'') whose type is required to be a functor. The
functor's type is selected in the first line by specifying a source.
...
viewed as an ``infinite disjunction'',
\begin{equation}
...
and implement it as a case class having methods \lstinline!.map!
and \lstinline!.withFilter!:
\begin{lstlisting}
...
val result = for {  // Some computations in the context of the `List` functor.
  ...
  z = g(x, y)    // ... compute z
  if q(x, y, z)  // ... and impose another condition.
...
should always be equal to each other:
...
``\lstinline!if true!'' from a functor block program. The corresponding
...
``\lstinline!if p(x)!''. In particular, we should be able to use
...
in code, first define a general ``factory'' for partial functions,
\begin{lstlisting}
...
\end{equation}
The top row contains the void type $\bbnum 0$ for which no values
can be computed, indicating that the partial function $f_{|p}$ will
crash if applied to a value $x$ for which $p(x)=\text{false}$.
...
``laws'' (i.e.~equations) that any reasonable \lstinline!filter!
...
quickly. For now, we can use the \texttt{scalacheck} library to implement
randomized tests for the four filtering laws:
\begin{lstlisting}
...
filtering with the predicate ``\lstinline!x < 1000!''. This happened
...
means the first server's credentials expired. In that case, if \lstinline!p(a2) == true!,
the first server copies the second server's valid credentials, \lstinline!a2!.
...
law for a method $q$ often has the form $f^{\uparrow}\bef q=\text{<modified }q\text{>}\bef f^{\uparrow}$,
...
remove the value \lstinline!a1!, or \lstinline!a2!, or both of them
...
sealed trait JohnsCoupons[A]           // This represents the type $\bbnum 1+A+A\times A+A\times A\times A$.
...
\begin{lstlisting}[mathescape=true]
sealed trait JillsCoupons[A]    // This represents the type $\bbnum 1+A+A\times A$.
...
\lstinline!JohnsCoupons! and for \lstinline!JillsCoupons!:$\text{\emph{all}}$
\begin{lstlisting}[mathescape=true]
...
\lstinline!p! first returns \lstinline!false!:
\begin{lstlisting}
...
general constructions for filterable functors (see page~\pageref{proof-that-takeWhile-is-a-lawful-filter}
...
days of the week. Is this situation described by a filterable functor?
...
\textbf{(d)} The functor $\text{MyTree}^{A}$ defined recursively
...
data set may be discarded. Is this situation described by a filterable
...
of the laws that is easier to remember and to verify?

To motivate such a reformulation, begin by considering a heuristic
picture of a filter operation $\text{filt}_{F}(p):F^{A}\rightarrow F^{A}$
that may remove some values of type $A$ from a wrapper $F^{A}$.
For example, if $F^{A}\triangleq\bbnum 1+A$, the filtering operation
may remove the value of type $A$ and return the result $1+\bbnum 0$.
...
\begin{align*}
 ...
 & \xymatrix{\xyScaleX{5pc}\xyScaleY{1pc}F^{A}\ar[r]\sp(0.45){\text{inflate}} & F^{\bbnum 1+A}\ar[r]\sp(0.55){\big(\text{filt}_{\text{Opt}}(p)\big)^{\uparrow F}} & F^{\bbnum 1+A}\ar[r]\sp(0.55){\text{deflate}} & F^{A}}
\end{align*}
Here $\text{filt}_{\text{Opt}}$ is the standard \lstinline!filter!
function for the \lstinline!Option! type. 
...
method for any filterable $F$: use $F$'s \lstinline!filter! to
...
means that a functor's filtering logic is fully described as long
...
Are \lstinline!deflate! and \lstinline!filter! computationally equivalent?
...
Begin with a filterable functor $F$'s function \lstinline!filter!,
...
\[
\xymatrix{\xyScaleY{1.5pc}\xyScaleX{5.0pc}F^{\bbnum 1+A}\ar[r]\sp(0.5){\text{deflate}} & F^{A}\ar[d]\sb(0.45){f^{\uparrow F}}\\
...
lift the function $f$ to the $\text{Opt}$ functor, i.e.~to define
$g\triangleq f^{\uparrow\text{Opt}}$. 

\newpage So, the \textbf{naturality law}\index{naturality law!of deflate@of \texttt{deflate}}
...
a formula for the lifting operation $^{\uparrow\text{Opt}}$ for the
...
\subsection{Motivation and laws for \texttt{liftOpt}}
...
\end{align}
So, we expect the \textbf{identity law} for\index{identity laws!of liftOpt@of \texttt{liftOpt}}
...
\]
Then \lstinline!liftOpt!'s \index{identity laws!of liftOpt@of \texttt{liftOpt}}identity
...
\]
So the ``combined'' \textbf{naturality-identity law} of\index{naturality law!combined with identity law}
...
(Statement~\ref{subsec:functor-Statement-functor-product}).
...
\end{equation}
This property is derived by applying the code for \lstinline!Option!'s
...
\end{align}
Here we used a convenient ``parenthesis-canceling'' property of
...
\lstinline!.takeWhile! method defined on sequences.\index{filterable!using takeWhile@using \lstinline!takeWhile!}
...
below) similar to the properties of filterable functors. We will call
...
{\color{greenunder}\text{naturality-identity law for }\text{liftOpt}_{H}:}\quad & =\text{\text{liftOpt}}_{G}(f\bef\text{pu}_{\text{Opt}})\bef p\bef\gunderline{f^{\uparrow H}}\quad.
\end{align*}
...
{\color{greenunder}\text{composition law~(\ref{eq:composition-law-of-liftOpt}) for }\text{liftOpt}_{H}:}\quad & =\text{\text{liftOpt}}_{G}(g)\bef\text{\text{liftOpt}}_{G}(f)\bef p\bef\text{liftOpt}_{H}(f\diamond_{_{\text{Opt}}}g)\quad.
\end{align*}
...
all fully parametric functions will obey this law (see the discussion
at the end of Section~\ref{subsec:Bifunctors}).

\subsection{Filterable contrafunctors: motivation and examples}

An intuitive view is that functors are ``wrappers'' of data, while
...
$Z$ is a constant type. This is a general form of an ``extractor''
...
a filter $\psi$ (which makes it a total function due to \lstinline!filter!'s
...
analysis produces a number of type constructions for contrafunctors
that are guaranteed to have filterable properties.
...
to the filterable functor $\bbnum 1+A$, and so it is filterable.
The simplest case of the search functor is found by setting $Z\triangleq\bbnum 1$,
which gives the type constructor
\[
S_{\bbnum 1}^{A}\triangleq(A\rightarrow\bbnum 2)\rightarrow\bbnum 1+A\quad.
\]
Values of type $S_{\bbnum 1}^{A}$ may be viewed as ``searchers''
...
library provides sufficient implementations of filtering (e.g.~the
...
is to ``unroll'' the recursive\index{recursive type!unrolling trick}
equation and to reduce $F^{A}$ to the \lstinline!List! functor,
which has a standard filtering operation.

The ``unrolling'' trick works for any recursive definition of the
...
the laws of filterable functor rigorously (i.e.\ by calculations,
not tests).
...
to be filterable.%
\begin{comment}
$\bbnum 1+A\times\left(Z\rightarrow A\right)$ \textemdash{} can implement
\texttt{deflate} but cannot obey the identity law, must return $1+\bbnum 0$.
\end{comment}
...
{\color{greenunder}\text{Eq.~(\ref{eq:left-naturality-law-of-liftOpt})}:}\quad & h^{\uparrow F}\bef\text{liftOpt}_{F}(f)=\text{liftOpt}_{F}(h\bef f) &  & \text{for }\quad\text{liftOpt}_{F}:\left(A\rightarrow\text{Opt}^{B}\right)\rightarrow F^{A}\rightarrow F^{B}\\
{\color{greenunder}\text{Eq.~(\ref{eq:right-naturality-law-of-liftOpt})}:}\quad & \text{liftOpt}_{F}(f)\bef h^{\uparrow F}=\text{liftOpt}_{F}(f\bef h^{\uparrow\text{Opt}}) &  & \text{for }\quad\text{liftOpt}_{F}:\left(A\rightarrow\text{Opt}^{B}\right)\rightarrow F^{A}\rightarrow F^{B}\\
...
values in the list. This property is mathematically formulated as
the requirement that we may first transform the list with a lifted
...
then need to lift to the \lstinline!Option! functor rather than to
the \lstinline!List! functor): the results will be equal.
...
We may not need to verify naturality laws, but it is important to
...
If we put aside the naturality laws, \lstinline!liftOpt! has the
...
\end{align*}
This calculation motivates the name ``associativity law'' for Eq.~(\ref{eq:associativity-law-of-flatMap-for-Option}).

The function \lstinline!liftOpt! can be viewed as a ``generalized
lifting'' from Kleisli functions $A\rightarrow\bbnum 1+B$ to functions
...
(functor product), written using the function product operation $\boxtimes$,
...
lifting'' that replaces $\text{Opt}$ by a functor $M$,
\begin{align*}
...
\subsection{Motivation for using category theory}
...
that lifts more general or ``twisted'' function types instead of
the ordinary function types ($A\rightarrow B$). This notion (and
its further generalizations) is provided by\index{category theory}
...
composition laws hold. The morphism types, the ``\index{identity morphism}identity
...
\hline 
\hline 
{\small{}``plain''} & {\small{}$\text{Int},\text{String},...$} & {\small{}$f:A\rightarrow B$} & {\small{}$\text{id}^{:A\rightarrow A}$} & {\small{}$f\bef g$}\tabularnewline
\hline 
{\small{}``reversed''} & {\small{}$\text{Int},\text{String},...$} & {\small{}$f:B\rightarrow A$} & {\small{}$\text{id}^{:A\rightarrow A}$} & {\small{}$g\bef f$}\tabularnewline
\hline 
{\small{}``$M$-Kleisli''} & {\small{}$\text{Int},\text{String},...$} & {\small{}$f:A\rightarrow M^{B}$} & {\small{}$\text{pu}_{M}:A\rightarrow M^{A}$} & {\small{}$f\diamond_{_{M}}g$}\tabularnewline
\hline 
{\small{}``reversed $M$-Kleisli''} & {\small{}$\text{Int},\text{String},...$} & {\small{}$f:B\rightarrow M^{A}$} & {\small{}$\text{pu}_{M}:A\rightarrow M^{A}$} & {\small{}$g\diamond_{_{M}}f$}\tabularnewline
\hline 
{\small{}``$F$-lifted''} & {\small{}$F^{\text{Int}},F^{\text{String}},...$} & {\small{}$f:F^{A}\rightarrow F^{B}$} & {\small{}$\text{id}^{:F^{A}\rightarrow F^{A}}$} & {\small{}$f\bef g$}\tabularnewline
\hline 
\end{tabular}
\par\end{center}

\paragraph{Definition of category}

A category\index{category theory!category} $\mathcal{C}$ is a collection
of \index{category theory!objects} \textbf{objects}\footnote{The ``objects'' in category theory are not related to ``object-oriented
programming''.} $\left\{ A,B,...\right\} $ and a collection of morphisms $\left\{ f_{1},f_{2},...\right\} $,
where each morphism is labeled by two objects as $f^{:A\leadsto B}$
($A$ and $B$ do not need to be different). For any object $A$,
the identity morphism $\text{id}_{\mathcal{C}}^{:A\leadsto A}$ must
exist. For any two morphisms $f^{:A\leadsto B}$ and $g^{:B\leadsto C}$,
the composition $f\bef_{_{\mathcal{C}}}g$ must exist and be equal
to a morphism labeled $A\leadsto C$. Additionally, the identity and
the associativity laws must hold:
\[
...
Most applications of category theory to functional programming will
use categories whose objects are the types (\lstinline!Int!, \lstinline!String!,
...) of the programming language (or, sometimes, type constructors),
and whose morphisms are functions with type signatures defined in
a specific way, e.g.~$A\rightarrow M^{B}$ or $F^{A}\rightarrow F^{B}$
as we have seen. However, the definition of category is general and
does not assume that morphisms are functions between objects.
...
to another}. We will use the phrase ``categorical functor'' to distinguish
the notion of functor in category theory from the programmer's notion
of type constructor with \lstinline!map!.

\paragraph{Definition of categorical functor}

Given two categories ${\cal C}$ and ${\cal D}$, a functor $\mathcal{F}:{\cal C}\leadsto{\cal D}$
...
morphisms from the first category must be mapped to those of the second
category; and the composition of any two morphisms from the first
category must be mapped to the composition of morphisms in the second
category.
\begin{align*}
...
$f^{:B\rightarrow A}$ of the reversed category is mapped to the $(A\leadsto B)$-morphism
...
In both examples, laws of the appropriately formulated categorical
functors turned out to be equivalent to laws we derived previously.
This gives us assurance that we have correctly guessed the relevant
laws. The choice of laws is not self-evident. For example, our four
laws of \lstinline!filter! in Section~\ref{subsec:Motivation-for-and-derivation-of-laws-of-filtering}
were based on heuristic ideas. The laws of the \lstinline!filter!
...
contrast, the two laws of the categorical functor are very general
and appear time and again in different areas of mathematics. This
gives us confidence that these laws are correctly chosen and will
be useful in a wide range of contexts. Proving that the four laws
of \lstinline!filter! from Section~\ref{subsec:Motivation-for-and-derivation-of-laws-of-filtering}
...
we can reduce the number of different proofs if we generalize from
the $\text{Opt}$-Kleisli category to an $M$-Kleisli category with
a functor $M$ to be chosen later. One can prove general theorems
...
four derivations (say, for the product of functors, contrafunctors,
filterable functors, and filterable contrafunctors) by a single but
more abstract proof about the product of (categorical) functors. We
will not look at these proofs further, since this chapter already
worked through several almost identical proofs that follow from this
technique.
...
look for functors $M$ (called ``monads'') that admit the Kleisli
...
new monads $M$, we can look for ``$M$-filterable'' functors $F$
that admit an operation $\text{lift}_{M,F}$ that replaces $\text{liftOpt}_{F}$
and satisfies analogous laws. We will see examples of $M$-filterable
functors later in this book.

To summarize, using category theory's notion of functor brings important
advantages:
\begin{itemize}
\item We have assurance that we formulated the laws correctly.
\item We can find some promising directions for generalizing the constructions.
...
(functor, contrafunctor, filterable, etc.) in a similar way.
...
\item It does not help in implementing typeclass instances that obey the
required laws. 
...
them requires symbolic derivations using techniques specific to functional
programming. Developing these techniques (while avoiding unnecessary
generalizations) is one of the main themes of this book.
