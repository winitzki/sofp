...
functor\textsf{'}s type is selected in the first line by specifying a source.
...
server\textsf{'}s credentials if needed, but the cluster will go down whenever
the first server\textsf{'}s credentials expire. This corresponds to the code
...
as long as $F$\textsf{'}s \lstinline!filter! obeys its partial function law.
