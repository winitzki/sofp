...
\subsection{Using \texttt{deflate} to define filterable functors}
...
equivalent. To verify that rigorously, we will now find the precise
...
\]
We would like to show that \lstinline!filter_prime! the same function
as \lstinline!filter!:
\begin{align}
...
by the Scala library for the \lstinline!Option! type:
\begin{align*}
...
The function $\psi$ is also fully parametric because its code is
\begin{align*}
...
\end{array}\quad.
\end{align*}
Now we compute the function compositions we need. First, we simplify
$\psi_{p}\bef\text{nonEmpty}=p$:
\begin{align*}
{\color{greenunder}\text{expect to equal }x\triangleright p:}\quad & x^{:A}\triangleright\psi_{p}\bef\text{nonEmpty}=p(x)\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
\bbnum 1 & \bbnum 0 & (\_\rightarrow x)
...
{\color{greenunder}\text{matrix composition}:}\quad & =p(x)\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & \bbnum 1\\
\hline \bbnum 1 & (\_\rightarrow1) & \bbnum 0\\
\bbnum 1 & \bbnum 0 & (\_\rightarrow1)
\end{array}=x\triangleright p\triangleright\begin{array}{|c||cc|}
 & \bbnum 1 & \bbnum 1\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
\bbnum 1 & \bbnum 0 & \text{id}
\end{array}=x\triangleright p\triangleright\text{id}=x\triangleright p\quad.
\end{align*}
...
{\color{greenunder}\text{partial function law of }\text{filt}_{F}:}\quad & =\text{filt}_{F}(p)\bef\gunderline{(\text{id})^{\uparrow F}}=\text{filt}_{F}(p)\bef\text{id}=\text{filt}_{F}(p)\quad.
\end{align*}
So, the new \lstinline!filter_prime! function is equal to the original
\lstinline!filter! function.

Beginning now with a given \lstinline!deflate! function, we first
define the corresponding \lstinline!filter! function via Eq.~(\ref{eq:def-filter-through-deflate})
...
\]
We would like to show that \lstinline!deflate_prime! is the same
function as \lstinline!deflate!, so we write
\begin{equation}
...
\noindent 
\[
\text{deflate}\bef f^{\uparrow F}=f^{\uparrow\text{Opt}\uparrow F}\bef\text{deflate}\quad,
\]
...
{\color{greenunder}\text{naturality law of }\text{deflate}:}\quad & =\gunderline{\psi_{\text{nonEmpty}}^{\uparrow F}\bef\text{get}^{\uparrow\text{Opt}\uparrow F}}\bef\text{deflate}\nonumber \\
...
to the \lstinline!Option! functor is expressed as
\[
...
computationally equivalent as long as the partial function law holds
for \lstinline!filter! and the naturality law holds for \lstinline!deflate!.
