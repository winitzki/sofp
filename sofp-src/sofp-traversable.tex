...
as:
\[
\text{seq}_{L}:M^{F^{A}}\times N^{F^{A}}\rightarrow F^{M^{A}\times N^{A}}\quad,\quad\quad\text{seq}_{L}\triangleq\quad.
\]
...
as:
\[
\text{seq}_{L}:M^{F^{A}}+N^{F^{A}}\rightarrow F^{M^{A}+N^{A}}\quad,\quad\quad\text{seq}_{L}\triangleq\quad.
\]
...
Here $L^{A}\triangleq S^{A,L^{A}}$, with the recursion scheme given
by a bifunctor $S^{A,R}$. In order to obtain the traversable property
of $L$, we need to assume that $S$ is traversable with respect to
both its type parameters. Such bifunctors $S$ are called bitraversable.
A bifunctor $S^{A,B}$ is bitraversable if it has \lstinline!bisequence!
method:
\[
\text{biseq}_{S}^{F,A,B}:S^{F^{A},F^{B}}\rightarrow F^{S^{A,B}}\quad,
\]
parameterized by an arbitrary applicative functor $F$. The analogous
laws must hold for \lstinline!bisequence!. We assume that a lawful
$\text{biseq}_{S}$ is available and define $\text{seq}_{L}$ as:
\[
\text{seq}_{L}:S^{F^{A},L^{F^{A}}}\rightarrow F^{S^{A,L^{A}}}\quad,\quad\quad\text{seq}_{L}\triangleq\quad.
\]
...
order over all values stored in the polynomial functor. However, in
some applications the traversal order may need to be chosen differently,
which will require a custom implementation of a \lstinline!Traversable!
instance.
...
Generalize from the example $L^{A}\triangleq A\times A\times A$ to
other polynomials
