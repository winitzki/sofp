...
In addition to the naturality law, we would like to impose a law that
holds only when \lstinline!toList! has extracted \emph{every} value
of type \lstinline!A! stored in \lstinline!p!. Unfortunately, it
seems to be impossible to express this property in the form of an
equation satisfied by \lstinline!toList!. Usually, the loss of information
is prevented by identity laws. Can we formulate an identity law for
\lstinline!toList!? Such a law could state that \lstinline!toList(p)!
should extract some known values contained in \lstinline!p!. If $L$
were a pointed functor, we could use its \lstinline!pure! method
to inject a known value \lstinline!x! into the container \lstinline!p!
and then require \lstinline!toList! to extract the same value \lstinline!x!.
But \lstinline!toList! works on values \lstinline!p! of type \lstinline!L[A]!
with a functor $L$ that is not known to be pointed or co-pointed.
So, in general we can neither inject values into $L^{A}$ nor extract
values from $L^{A}$ in a way that is guaranteed to preserve information.
This prevents us from finding an identity law for \lstinline!toList!.
...
to functions of type $L^{A}\rightarrow M$. However, we will not be
able to impose the functor laws on these liftings. For instance, the
functor composition law involves applying \lstinline!foldMap! to
a composition of arguments. But the type signature $A\rightarrow M$
(where $M$ is a fixed type) does not support composition, as we cannot
compose $A\rightarrow M$ with $B\rightarrow M$.
...
is that folding operations are available only for polynomial and recursive
polynomial functors (e.g., \lstinline!Option[A]! and \lstinline!List[A]!).
These functors represent finite containers storing values of type
\lstinline!A!. So, it is clear what it means to extract \textsf{``}all values
of type $A$\textsf{''} from such containers. To make this property precise,
we will define a \lstinline!length! function that counts the number
of values of type \lstinline!A! in a container of type \lstinline!L[A]!,
where \lstinline!L! is a (recursive) polynomial functor. We will
require that the operation \lstinline!toList! should extract exactly
that many values.
...
Since all folding operations are equivalent, it is convenient to use
the \lstinline!toList! operation to define the \textsf{``}\index{foldable functor}foldable
functor\textsf{''} typeclass:
\begin{lstlisting}
trait Foldable[L[_]] {
  def toList[A]: L[A] => List[A]
}
...
\lstinline!reduce!) can be derived from \lstinline!toList! (see
Section~\ref{subsec:Equivalence-of-foldLeft,foldMap,reduce,and-toList}).

Polynomial functors are built via the six standard type constructions
(Table~\ref{subsec:Type-notation-and-standard-type-constructions}),
except that the function types may not be used. Let us show how to
define \lstinline!toList! for the remaining five type constructions.
This will provide an implementation of folding operations for all
polynomial functors.
...
type constructions that build up polynomial functors. In principle,
this means one can write a code generator to provide a \lstinline!Foldable!
instance for any polynomial functor automatically. When writing code
by hand, it is more convenient to implement folding operations directly,
as we have been doing at the beginning of this chapter.
...
\subsection{Laws of \texttt{traverse} and \texttt{sequence}}
...
these expectations as equations that \lstinline!traverse! must satisfy?

Naturality laws formalize the requirement that a function with a type
parameter should work in the same way for all types. So, we impose
the naturality laws with respect to type parameters \lstinline!A!
and \lstinline!B!. The form of the naturality laws follows from the
type signature of \lstinline!traverse!:
\[
...
\[
\text{trav}_{L}^{F,X,B}(f\bef g)=f^{\uparrow L}\bef\text{trav}_{L}^{F,A,B}(g)\quad,\quad\quad\text{trav}_{L}^{F,A,C}(g\bef h^{\uparrow F})=\text{trav}_{L}^{F,A,B}(g)\bef h^{\uparrow L\uparrow F}\quad.
\]
\vspace{-1\baselineskip}
\[
...
which is a type constructor assumed to be an applicative functor.
...
To formulate this \textbf{applicative naturality law}\index{applicative naturality law},
we write an equation similar to the second naturality law, except
that the arbitrary function $f$ should now map the applicative functor
$F$ to another arbitrary applicative functor $G$:%
\begin{comment}
precarious formatting
\end{comment}

\begin{wrapfigure}{i}{0.28\columnwidth}%
...
$\forall X.\,F^{X}\rightarrow G^{X}$. In addition, the function $f$
must preserve the applicative typeclass, in the sense that $f$ should
map $F$\textsf{'}s applicative methods (\lstinline!wu! and \lstinline!zip!)
...
morphisms} (compare to monad morphisms defined in Section~\ref{subsec:Monads-in-category-theory-monad-morphisms}).
...
{*}{*}{*} morphisms $a\rightarrow a\times a$ and $a\times b\rightarrow a$.
Illustrate naturality of traverse for $1+a$ mapped to $1+a\times a$. 
...
To express the expectation that \lstinline!p.traverse(f)! goes over
\emph{every} value of type \lstinline!A! stored inside \lstinline!p!,
consider for simplicity a function $f$ of type $A\rightarrow F^{A}$.
Then the result of \lstinline!p.traverse(f)! has type $F^{L^{A}}$.
We may require that the structure of \lstinline!p! (of type $L^{A}$)
should be preserved when it is wrapped under $F$. In this way, we
will ensure that \lstinline!p.traverse(f)! does not skip any data
contained in \lstinline!p!. 

To formulate this condition as a law, we choose functions $f$ that
always return an \emph{empty} $F$-effect (that is, a result of applying
$F$\textsf{'}s \lstinline!pure! method to some value). So, we choose $f\triangleq\text{pu}_{F}$.
We expect that traversing \lstinline!p! with this function $f$ should
reproduce the same value \lstinline!p! wrapped in an empty $F$-effect.
We write the law as:
\begin{equation}
...
$L$ such that a natural transformation with type signature $F^{L^{A}}\rightarrow L^{F^{A}}$
...
are also traversable. However, this is not automatic (we cannot implement
traverse via foldLeft). Foldable functors are traversable because
all polynomial functors (including recursively defined polynomial
functors) are foldable and no other functors are. {*}{*}{*}
