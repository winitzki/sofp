...
is a \textbf{monoid morphism}\index{monoid morphism|textit} if $\phi$
satisfies the following laws:
\begin{align*}
{\color{greenunder}\text{identity law}:}\quad & \phi(e_{M})=e_{N}\quad,\\
{\color{greenunder}\text{composition law}:}\quad & \phi(x^{:M}\oplus_{M}y^{:M})=\phi(x)\oplus_{N}\phi(y)\quad.
\end{align*}
...
except \lstinline!foldFn! uses the type $B\rightarrow B$ where \lstinline!reduceE!
uses an arbitrary monoid $M$ (having an empty value $e_{M}$ and
a binary operation $\oplus_{M}$). The type of functions $B\rightarrow B$
is itself a monoid that we here denote by $\text{MF}^{B}\triangleq B\rightarrow B$.
The empty value of that monoid is the identity function ($e_{_{\text{MF}^{B}}}=\text{id}^{:B\rightarrow B}$),
...
\end{align*}
if \lstinline!reduceE! obeys the monoidal naturality law~(\ref{eq:monoidal-naturality-law-of-reduceE}).
Here, $\text{reduceE}^{\text{List}^{A}}$ is \lstinline!reduceE!
applied to the monoidal type \lstinline!List[A]!, with the list concatenation
as the monoidal operation. The function \lstinline!reduceList[M]!
is defined for monoids $M$ by:
\begin{lstlisting}
...
\]
If we keep \lstinline!B! fixed and let \lstinline!A! vary, the type
signature is of the form $J^{A}\rightarrow K^{A}$, a natural transformation
between two contrafunctors. If we keep \lstinline!A! fixed and let
\lstinline!B! vary, this type signature has the form $G^{A}\rightarrow H^{A}$,
a natural transformation between two functors. So, we can formulate
the two \index{naturality law!of traverse@of \texttt{traverse}}naturality
...
$\xymatrix{\xyScaleY{3.0pc}\xyScaleX{5.0pc}L^{A}\ar[rd]\sb(0.4){\text{trav}_{L}^{G,A,B}(g\bef f)~~}\ar[r]\sp(0.5){\text{trav}_{L}^{F,A,B}(g)} & F^{L^{B}}\ar[d]\sp(0.45){f^{\uparrow L\uparrow F}}\\
 & G^{L^{B}}
}
$\vspace{0\baselineskip}
\end{wrapfigure}%
...
\]
is required for $L$ to be traversable. That function must be applicatively
natural in $F$, working in the same way for any applicative functor
$F$. We note that $\text{seq}_{L}$ is covariant in $F$ because
$L^{\bullet}$ is contravariant. Since $\text{pu}_{F}:\text{Id}^{A}\rightarrow F^{A}$
is an applicative morphism (Example~\ref{subsec:Example-pure-is-applicative-morphism}),
...
(a perfect-shaped tree, see Sections~\ref{subsec:Perfect-shaped-trees}
...
via a recursion scheme, we may write:
\[
...
\[
\text{seq}_{L}^{\text{Id},A}=\big(\text{seq}_{P}^{\text{Id},A}\big)^{\uparrow L\uparrow S^{F^{A},\bullet}}\bef\big(\overline{\text{seq}}_{L}^{\text{Id},P^{A}}\big)^{\uparrow S^{F^{A},\bullet}}\bef\text{seq2}_{S}^{\text{Id},A,L^{P^{A}}}=\text{id}\bef\text{id}\bef\text{id}=\text{id}\quad.
\]
To verify the composition law~(\ref{eq:composition-law-of-sequence}),
...
side so that each of the three \lstinline!sequence! methods are composed
next to each other (as in the right-hand side). This can be done using
various naturality laws, which allow us to change the order of composition
when functions are lifted to certain functors:
\begin{align*}
...
As an advanced example of a nested traversable functor, we will show
a type that enforces a square shape on matrices represented by nested
lists\footnote{This and other advanced examples of nested recursive types are explained
...
implemented some operations for square matrices encoded via nested
lists. However, a value of type \lstinline!List[List[A]]! is not
guaranteed to represent a matrix of a consistent shape. We would like
to ensure that there are always $n$ nested lists of length $n$ (for
$n=1,2,...$) and disallow irregular nested lists such as $\left[\left[1,2\right],\left[\right],\left[3\right]\right]$
that do not represent the rows of a square matrix.
