...
\begin{align*}
{\color{greenunder}\text{identity law of applicative morphism}:}\quad & f^{:F^{\bbnum 1}\rightarrow G^{\bbnum 1}}(\text{wu}_{F})=\text{wu}_{G}\quad,\\
{\color{greenunder}\text{composition law of applicative morphism}:}\quad & \text{zip}_{G}\big(f(p^{:F^{A}})\times f(q^{:F^{B}})\big)=f(\text{zip}_{F}(p\times q))\quad.
\end{align*}
...
\subsubsection{Example \label{subsec:Example-pure-is-applicative-morphism-1}\ref{subsec:Example-pure-is-applicative-morphism-1}}

Consider applicative functors $\text{Id}^{A}\triangleq A$ and $P^{A}\triangleq A\times A$. 

\textbf{(a)} Show that the standard function $\Delta:\text{Id}^{A}\rightarrow P^{A}$
is an applicative morphism.

\textbf{(b)} Show that the standard function $\pi_{1}:P^{A}\rightarrow\text{Id}^{A}$
is an applicative morphism. 
...
\subsubsection{Example \label{subsec:Example-pure-is-applicative-morphism-2}\ref{subsec:Example-pure-is-applicative-morphism-2}}

Verify the naturality law of \lstinline!traverse! for $L^{A}\triangleq A\times A$
using the applicative functor $F^{A}\triangleq\bbnum 1+A$ mapped
to $G^{A}\triangleq\bbnum 1+A\times A$ by an applicative morphism. 
...
For an applicative functor $F$, show that the \lstinline!pure! method
($\text{pu}_{F}:A\rightarrow F^{A}$) is an applicative morphism between
the identity functor $\text{Id}^{A}\triangleq A$ and $F$.
...
\]
To check the identity law, write:
\begin{align*}
...
\end{align*}
To check the composition law, write:
\begin{align*}
...
values are visited exactly once. The remaining expected property of
\lstinline!traverse! is that all $F$-effects returned by $f$ are
collected exactly once. We do not expect \lstinline!traverse! to
copy some of the $F$-effects multiple times. As an example of violating
that expectation, consider a simple traversable functor $L^{A}=A\times A$
and implement the \lstinline!traverse! method like this:
\begin{lstlisting}
...
The code is supposed to \lstinline!zip! the two $F$-effects together,
but we copy the first effect twice, discarding the first of the values
of type \lstinline!B!. 
...
\]
The second \lstinline!traverse! operation needs to be lifted to \lstinline!F!
for the types to match. The result looks like a \lstinline!traverse!
operation with respect to the functor $F\circ G$. Since the functor
composition of two applicative functors is again applicative (Statement~\ref{subsec:Statement-applicative-composition}),
the functor $F\circ G$ is applicative. So, we may also write a single
\lstinline!traverse! operation using that functor and obtain a value
of the same type $F^{G^{L^{C}}}$:
\[
...
To make this heuristic explanation more rigorous, let us show a counterexample
...
effects do not commute. A suitable example is found by choosing both
$F$ and $G$ to be the \lstinline!State! monad.\footnote{The paper \texttt{\href{https://arxiv.org/abs/1202.2919}{https://arxiv.org/abs/1202.2919}}
...
guarantees that the traverse function evaluates each $F$-effect exactly
once.
...
via \lstinline!sequence! and substitute in both sides of Eq.~(\ref{eq:traverse-applicative-naturality-law}):
\begin{align*}
...
\end{align*}
The common function $g^{\uparrow L}$ can be omitted from the equality
(see Exercise~\ref{subsec:Exercise-simplify-law-omit-lifted-function}
...
\]
So, the identity law of sequence\index{identity laws!of sequence@of \texttt{sequence}}
is:
\[
\text{seq}_{L}^{\text{Id},A}=\text{id}\quad.
\]
...
and renaming $C$ to $A$, we obtain the \index{composition law!of sequence@of \texttt{sequence}}composition
law of \lstinline!sequence!:
\[
\text{seq}_{L}^{F,G^{A}}\bef(\text{seq}_{L}^{G,A})^{\uparrow F}=\text{seq}_{L}^{F\circ G,A}\quad.
\]
...
\subsection{All polynomial functors are traversable}
...
two functions (\lstinline!tr! and \lstinline!ftr!) are equivalent
if a naturality law holds for \lstinline!ftr! with respect to the
type parameter $A$. Under the same assumption, show that the naturality
...
\paragraph{Motivation for the laws of traverse }
...
Laws of traversables allow ignoring the effects of $F$ ! Not sure
how to specify that the effects cannot be ignored. Does the applicative
naturality in $F$ help?
...
Given a lawful traversable functor $L$, show that $\text{seq}_{L}^{:L^{F^{A}}\rightarrow F^{L^{A}}}=\text{id}$
