...
and sequence-like data types \lstinline!L[A]!, such as \lstinline!List[A]!.
To generalize the \lstinline!sequence! operation to other data types,
we replace a sequence-like type \lstinline!L[A]! by an arbitrary
traversable functor. We also replace the \lstinline!Future! type
constructor by an arbitrary applicative functor (since the traverse
...
holds under the assumption that traverse obeys a naturality law with
respect to the type parameter $A$. This follows from Statement~\ref{subsec:Statement-tr-equivalent-to-ftr}
if we set $\text{tr}=\text{seq}$, $\text{ftr}=\text{trav}$, $F\triangleq L$,
$G\triangleq F$, and $H\triangleq F\circ L$. 

Because of this equivalence, we may define the Traversable typeclass
in terms of \lstinline!sequence! instead of using \lstinline!traverse!.
...
So, we will use \lstinline!sequence! to prove the type constructions
...
should evaluate the function \lstinline!f: A => F[B]! on all values
...
of the value \lstinline!p: L[A]! must be preserved, as much as possible,
in the resulting values of type \lstinline!L[B]! wrapped under \lstinline!F!.
How can we express these expectations as laws of \lstinline!traverse!?
...
To derive it, ask: what is missing from \lstinline!fmap! to do the
job of traverse?
\[
