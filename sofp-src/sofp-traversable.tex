...
\begin{lstlisting}
final case class St[A](run: Int => (A, Int))  { // A State monad with internal state of type Int.
  import io.chymyst.ch.implement                // Derive the monad\textsf{'}s methods automatically from types.
...
function collects each $F$-effect exactly once.{*}{*}{*}Mention the
paper about the rewinding law.{*}{*}{*}
...
We define a \textsf{``}constant-function \lstinline!State! monad\textsf{''}, which
we denote by $\text{CFState}^{S,A}$, like this: Write the type of
a \lstinline!State! monad equivalently as $\text{State}^{S,A}\triangleq S\rightarrow A\times S\cong(S\rightarrow A)\times(S\rightarrow S)$
...
Here is the complete code of \lstinline!sequence! for \lstinline!Sq!:
