...
and \texttt{toList}. Monoid morphisms}
...
are requivalent. The type signatures of these functions are similar
...
The functions \lstinline!reduceE! and \lstinline!toList! are equivalent:
\begin{align*}
...
\end{align*}
 as long as \lstinline!reduceE! obeys the monoidal naturality law~(\ref{eq:monoidal-naturality-law-of-reduceE}).
The function \lstinline!reduceList! is defined by:
\[
\text{reduceList}:\text{List}^{M}\rightarrow M\quad,\quad\quad\text{reduceList}\triangleq\,\begin{array}{|c||c|}
 & M\\
\hline \bbnum 1 & e_{M}\\
M\times\text{List}^{M} & m^{:M}\times t^{:\text{List}^{M}}\rightarrow m\oplus_{M}\overline{\text{reduceList}}\left(t\right)
\end{array}\quad.
\]
...
\subsection{The difficulty with the laws of \texttt{foldMap} and \texttt{reduce}}
...
which entails various naturality laws. However, these laws do not
fully describe the programmer\textsf{'}s expectations about the behavior of
folding operations. 
...
\]
which describes the property that \lstinline!toList[A]! works in
the same way for all types \lstinline!A!. This is certainly one of
the programmers\textsf{'} expectations about the behavior of \lstinline!toList!.
But naturality does not express the main intent of \lstinline!toList!,
which is extracting values of type $A$ out of $L^{A}$ and storing
them in a list. More precisely, programmers expect that for any \textsf{``}container\textsf{''}
value \lstinline!p! of type \lstinline!L[A]!, the value \lstinline!toList(p)!
should be a list of \emph{all} values of type \lstinline!A! stored
in \lstinline!p!. It would be unacceptable to have an implementation
of \lstinline!toList! that always returns an empty list.

Unfortunately, it seems to be impossible to express this property
in the form of an equation or a law. The naturality law does not describe
this property. The difficulty is that we cannot check whether we have
listed \textsf{``}all values\textsf{''} of type \lstinline!A! stored in \lstinline!p!.
But actually the 

The type signature of the \lstinline!foldMap! function resembles
that of a lifting but the type of arrows $A\rightarrow Z$ does not
support composition. (We would need to compose $A\rightarrow Z$ with
$B\rightarrow Z$ to obtain some $C\rightarrow Z$, which does not
make sense in terms of category theory.)

We do have the laws of naturality with respect to $A$ and $Z$. The
naturality laws must respect the constraint that $Z$ is a monoid.
To express this rigorously, we define \textsf{``}monoidal naturality\textsf{''}. 
...
\subsection{Constructions of traversable functors}
