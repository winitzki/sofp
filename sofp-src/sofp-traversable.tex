...
custom code for each new data type. However, that code will contain
a certain common pattern: it will use recursive calls at the points
where the recursive data type refers to itself. It turns out that
we can implement this common pattern separately from the custom code,
...
of \lstinline!fold!, we can define a general \textsf{``}unfolding\textsf{''} operation
...
operation generalizes this computation to an arbitrary recursive type
$L^{A}$ whose recursion scheme $S^{A,R}$ is given. 

To get more intuition, we look at examples using \lstinline!unfold!
...
the recursion scheme for aFor a list, we set $L^{A}\triangleq\text{List}^{A}$
and the corresponding recursion scheme $L^{A}\triangleq\text{T2}^{A}$:
creating a list of a List{[}A{]} {*}{*}{*} Depending on $f:Z\rightarrow S^{A,Z}$
and an initial value, we can now generate a value of an arbitrary
recursive type $L^{A}$. 
...
For that data type, define a recursion scheme and implement \lstinline!unfold!.
Using unfold, write a function that generates trees
