...
def trav[A, B, F[_]: Functor : Applicative](f: A => F[B])(t: Tree2[A]): F[Tree2[B]] = t match {
  ...
  case Branch(t1, t2)   =>
    ...
    F.zip(r1, f2).map { case (b1, b2)  => Branch(b1, b2) } // Reproduce the Branch structure under F.
}
\end{lstlisting}
...
the breadth-first traversal of a binary tree. The idea is to prepare
...
Implementing breadth-first \lstinline!traverse! for a tree is more
...
$F$. The next section shows an example implementation of the breadth-first
tree traversal.

\subsection{Decorating a tree with depth-first and breadth-first traversals }

Once we implement \lstinline!traverse! for a tree-like data structure,
we may use it to perform various operations on trees, as long as those
operations are described by the effect of some applicative functor
$F$. An example

\subsection{Implement \texttt{scanLeft} and \texttt{scanMap} via \texttt{traverse}
and the \texttt{State} monad}
...
\emph{can} be implemented as a traversal. 

Depth labeling of a tree as  \Tree[ [ 2 [ 3 3 ] ] 1 ]  is not possible
with a traversal because we cannot distinguish between nodes at the
same depth and nodes at different depth. A traversal simply accumulates
the effects for all tree nodes. We cannot label a certain pair of
nodes with the same depth value but other nodes with a different depth
value; either the state value is updated at \emph{each} node or not.
Depth labeling must be implemented as a special operation such as
zipWithDepth.
