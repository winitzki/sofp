...
Non-traversable: $L^{A}\equiv R\rightarrow A$; lazy list (\textsf{``}infinite
product\textsf{''})
...
\textsf{``}Identity\textsf{''} as $\text{id}^{\underline{A\rightarrow A}}$ with $F^{A}\equiv A$
...
Traversal for a non-monadic \textsf{``}rigid\textsf{''} tree $T^{A}\equiv A+T^{A\times A}$
...
we choose $F^{A}\equiv A$ as the identity functor. 
...
$\text{seq}:L^{F^{A}}\rightarrow F^{L^{A}}$ for $F^{A}\equiv1+A$
...
Show that $K^{A}\equiv G^{H^{A}}$ is traversable if $G$ and $H$
are traversable.
...
For the tree-like type defined as $T^{A}\equiv1+A\times T^{A}\times T^{A}$,
...
Implement a traversable instance for a \textsf{``}rose tree\textsf{''} $T^{A}\equiv A+\text{List}^{T^{A}}$.
...
Is the recursive type constructor $L^{A}\equiv A+L^{\text{List}^{A}}$
...
due to Exercise~\ref{subsec:Exercise-1-monads}).
...
and $N^{R}$ are monoids due to Exercise~\ref{subsec:Exercise-1-monads}).
