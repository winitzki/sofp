...
into {\scriptsize{}\Tree[ 28 [ [ 23 25 ] 24 ] ]}. However, the \lstinline!map!
...
the tree {\scriptsize{}\Tree[ 8 [ [ 3 5 ] 4 ] ] } into {\tiny{}\Tree[ (8,0) [ [ (3,1) (5,2) ] (4,3) ] ]}
with added indices ($0$, $1$, $2$, $3$). For such tasks, we need
the \lstinline!traverse! operation.
...
goal is not to achieve high efficiency but to obtain simple working
code.
...
traversal order. As a result, we gain flexibility: an arbitrary implementation
of \lstinline!traverse! may be used with an arbitrary \textsf{``}decoration\textsf{''}
function. 
...
In this way, \lstinline!scanLeft! is generalized to a wide class
...
information that \lstinline!traverse! cannot have. We will look at
...
This cannot be implemented via \lstinline!traverse! because the code
of \lstinline!traverse! will always collect and merge all the effects
of a given functor \lstinline!F!. If the effect of \lstinline!F!
describes incrementing a counter (as in our examples involving the
\lstinline!State! monad), the code of \lstinline!traverse! will
increment that counter at \emph{each} leaf. We may certainly traverse
the tree so that leafs at the same depth are traversed next to each
other (as in the breadth-first traversal). But the code cannot skip
the incrementing when the leaf is at the same depth since \lstinline!traverse!
...
{\scriptsize{} \Tree[ 8 [ [ 3 5 ] 4 ] ] } is represented by the text
...
or \lstinline!traverse! methods and are implemented via custom recursive
code. If we needed to implement the same operations for trees of different
shapes or for other recursive data types, we would need to write new
custom code for each new data type. That code will contain a certain
common pattern: it will use recursive calls at the points where the
recursive data type refers to itself. It turns out that we can implement
this common pattern separately from the custom code, reducing the
effort required for implementing the folding and traversing operations
...
equation, $\text{T2}^{A}\triangleq S^{A,\text{T2}^{A}}$. The corresponding
Scala code is:
\begin{lstlisting}
...
refers to itself in its definition: all the recursive references correspond
...
function in the code above is the lifting $\overline{\text{pls}}^{\uparrow S^{A,\bullet}}$
...
\end{equation}
We can then define $\text{pls}\triangleq\text{fold}_{S}(\text{toLaTeX})$.
...
It is important that the generalized \lstinline!fold! function may
be implemented in the same way for all recursion schemes $S^{\bullet,\bullet}$
...
To illustrate the general applicability of \lstinline!fold! to different
...
cause an infinite loop in \lstinline!fold!. A simple example where
\lstinline!fold! enters an infinite loop with the recursion scheme
...
Rather than working with the general \lstinline!fold! function and
redefine all recursive types via \lstinline!Fix!, it is more convenient
to implement and use specialized versions of \lstinline!fold! for
already defined recursive types. The general implementation of \lstinline!fold!
in Eq.~(\ref{eq:fold-via-recursion-scheme-1}) can be translated
mechanically (e.g., using macros or code generators) into a specialized
implementation for a given data type and recursion scheme.

For instance, while the type \lstinline!TreeN[A]! is equivalent to
\lstinline!Fix[S3[A, *]]! shown above, it is easier to work with
\lstinline!TreeN[A]!. The specialized version of \lstinline!fold!
for \lstinline!TreeN[A]! has the type signature:
\begin{lstlisting}
def foldTreeN[A, Z](f: S3[A, Z] => Z): TreeN[A] => Z = ???
\end{lstlisting}
The general definition of \lstinline!fold! in Eq.~(\ref{eq:fold-via-recursion-scheme-1})
...
By reversing the direction of function arrows in the type signature
of \lstinline!fold!, we can define a general \textsf{``}unfolding\textsf{''} method
that depends on an arbitrary recursion scheme $S^{A,R}$ and an arbitrary
...
track of the current initial power of $2$. So, let us choose $Z\triangleq$\lstinline!Long!
to represent that value; the type $A$ will be also \lstinline!Long!. 
...
in the middle of generating the list, the function $f$ should return
$\bbnum 0+a\times z$ with some values $a$ and $z$. The value $a$
must be the new element of the list; the value $z$ will be passed
to the next call of $f$. Since the next element must be twice the
previous one, we must return $z=2*a$. So, the code of $f$ is:
\begin{lstlisting}
...
create \textsf{``}full\textsf{''} binary trees of given depth, e.g., {\scriptsize{}\Tree[ [ 0 1 ] [ 2 3 ] ]}
...
\end{lstlisting}
The next step is to choose a suitable type $Z$ such that the unfolding
procedure can generate trees of the required shape. To figure out
what $Z$ must be, we need to consider an intermediate step that generates
a subtree of this tree at some point in the middle of \textsf{``}unfolding\textsf{''}.
For the tree of depth $3$ as shown above, an example of a subtree
in the middle is {\tiny{}\Tree[ 4 5 ]}. This subtree must be computed
as \lstinline!unfoldT2(f)(z)! with some value \lstinline!z: Z!.
The information needed for this computation is the initial value ($4$)
and the total number ($2$) of the subtree\textsf{'}s leaves. So, the type
$Z$ needs to contain two integers, the value of the first leaf and
the size of the remaining subtree (which will always be a power of
$2$):
\begin{lstlisting}
type Z = (Int, Int)        // (k, m) where k is the first leaf\textsf{'}s value and m is the subtree size.
...
to some intermediate value \lstinline!z: Z!. If \lstinline!f(z) == Left(k)!,
...
between the subtrees must come entirely from the difference between
...
odd or even. The function \lstinline!f! cannot know that its application
\lstinline!f(z1)! or \lstinline!f(z2)! is used to generate the left
or the right subtree, or that it must create a \lstinline!Leaf! in
some cases. This information must come from the values \lstinline!z1!
and \lstinline!z2!. So, the type \lstinline!Z! must contain a \lstinline!Boolean!
flag indicating whether it is necessary to create a \lstinline!Leaf!
at the current place. For clarity, let us define the type \lstinline!Z!
as a case class:
\begin{lstlisting}
...
either \lstinline!z1! or \lstinline!z2! should have \lstinline!makeLeaf = true!
according to the number \lstinline!startAt! being odd or even. Looking
at the examples, we find that \lstinline!f! should also return a
\lstinline!Left()! when \lstinline!startAt == 0!. So, the complete
...
\end{lstlisting}
The implementation of \lstinline!evenOdd! is now complete. Let us
test it:
\begin{lstlisting}
scala> evenOdd(3)
...
further parts of a recursive structure are evaluated only when requested.
...
\end{lstlisting}
The value \lstinline!tree1toInf! is itself finite but contains an
unbounded number of unevaluated parts. To visualize that value, let
us implement a function that converts \lstinline!UT[A]! to \lstinline!T2[A]!
by truncating at a given maximum depth. The unevaluated parts of the
tree are marked with a value called \lstinline!default!:
\begin{lstlisting}
...
by an arbitrary recursion scheme $S^{A,R}$.

To figure out the type signature of $\text{trav}_{S}$, consider the
relationship between ordinary \lstinline!traverse! and \lstinline!foldMap!
methods:
\begin{align*}
...
\end{align*}
We recover \lstinline!foldMap! from \lstinline!traverse! by using
a constant applicative functor $F^{B}\triangleq Z$. So, we expect
...
$L^{A}$ stored inside $S^{A,L^{A}}$:
\[
\big(s^{:S^{A,L^{A}}}\triangleright\big(\overline{\text{trav}_{S}(f)}\big)^{\uparrow S^{A,\bullet}}\big):S^{A,F^{L^{B}}}\quad.
\]
It remains to apply $f$ to the last obtained value. This completes
the code of $\text{trav}_{S}$: 
\[
\text{trav}_{S}\big(f^{:S^{A,F^{L^{B}}}\rightarrow F^{L^{B}}}\big)\triangleq\big(\overline{\text{trav}_{S}(f)}\big)^{\uparrow S^{A,\bullet}}\bef f\quad.
\]
...
at hand, as we did in the previous sections for folding and unfolding. 
...
\subsection{Laws of traverse}

\subsection{Equivalence of traverse and sequence}

\subsection{Foldable and traversable functors are polynomial}
...
Use the specialized version of \lstinline!fold! for the binary tree
...
\]
for any applicative functor $F$. Show that \lstinline!traverse!
and \lstinline!consume! are equivalent.

Solution:

First, we express consume via traverse and back.
\[
\text{consume}\,(f^{:L^{A}\rightarrow B})(p^{:L^{F^{A}}})\triangleq
\]
...
we choose $F^{A}\triangleq A$ as the identity functor. {*}{*}{*}From
what premises?
