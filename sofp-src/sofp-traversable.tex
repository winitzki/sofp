...
itself under the delayed recursive evaluation:
\begin{lstlisting}
...
\lstinline!Fix[S3[A, *]]! shown above, it is easier to work directly
with values of type \lstinline!TreeN[A]!. The specialized version
of \lstinline!fold! for \lstinline!TreeN[A]! has the type signature:
\begin{lstlisting}
...
number of a given rose tree. The \lstinline!foldTreeN! function allows
...
procedure can generate a tree such as {\tiny{}\Tree[ [ [ 0 1 ] [ 2 3 ] ] [ [ 4 5 ] [ 6 7 ] ] ]}.
To figure out what $Z$ must be, we need to consider an intermediate
step that generates a subtree of this tree at some point in the middle
of \textsf{``}unfolding\textsf{''}. An example of a subtree in the middle is {\tiny{}\Tree[ 4 5 ]}.
This subtree must be computed as \lstinline!unfoldT2(f)(z)! with
some value \lstinline!z: Z!. The information needed for this computation
is the initial value ($4$) and the total number ($2$) of the subtree\textsf{'}s
leaves. So, the type $Z$ needs to contain two integers, the value
of the first leaf and the size of the remaining subtree (which will
...
\lstinline!m! is $1$, the return value needs to be \lstinline!Left(k)!.
Otherwise, the return value needs to contain two new values of type
\lstinline!Z! corresponding to the two next subtrees at one level
deeper. These two subtrees have sizes \lstinline!m / 2! and initial
values \lstinline!k! and \lstinline!k + m / 2!. So, the code of
\lstinline!f! is:
\begin{lstlisting}
val f: Z => Either[Int, (Z, Z)] = {
...
For additional clarity, let us define the type \lstinline!Z! as a
case class:
\begin{lstlisting}
