...
\textbf{(a)} Each monad is at the same time an applicative functor.
Given a monad morphism $\phi:M\leadsto N$ between two monads, show
that $\phi$ is also an applicative morphism between applicative functors
$M$ and $N$.
...
necessarily a monad morphism. One example is where $M$ is a \lstinline!State!
monad and $N$ is the identity monad. Show that the function $\text{run}_{\text{State}}(s_{0}):\text{State}^{S,A}\rightarrow A$
...
So, every contrafunctor has a \lstinline!sequence! method that satisfies
the laws of traversables. However, the code of \lstinline!sequence!
always ignores all $F$-effects in its arguments and instead produces
values with empty $F$-effects.  We conclude that contrafunctors
are traversable in a way that is not practically useful.
