...
functor\index{polynomial functor!recursive}). Those functors represent
...
law}\index{applicative naturality law!of traverse@of \texttt{traverse}},
...
The first property is that the value $p:L^{A}$ must be restored if
...
One year later, R.~Bird et al.~showed that the properties of \lstinline!zipWithIndex!
...
theory is complicated but has limited practical use. In practice,
there is no uncertainty about how to implement a law-abiding \lstinline!traverse!
...
We will use the results of that theory to prove a statement that we
will need later:

\subsubsection{Statement \label{subsec:Statement-polynomial-functors-Int-A}\ref{subsec:Statement-polynomial-functors-Int-A}}

For any polynomial functor $L$ and any type $A$, there exists an
injective function of type $L^{A}\rightarrow(\text{Int}\rightarrow A)\times L^{\text{Int}}$.

\subparagraph{Proof}

We assume the results of Bird et al.~and accept without proof that,
for any $p^{:L^{A}}$, there exists a function $f_{p}:\text{Int}\rightarrow A$
such that:
\[
p\overset{!}{=}p\triangleright\text{zwi}_{L}\triangleright\pi_{2}^{\uparrow L}\triangleright f_{p}^{\uparrow L}\quad.
\]
The function $f_{p}$ depends on $p$; the type of $f$ in standard
notation is written as $f:L^{A}\rightarrow\text{Int}\rightarrow A$.
Note that $p\triangleright\text{zwi}_{L}\triangleright\pi_{2}^{\uparrow L}$
is a function of type $L^{A}\rightarrow L^{\text{Int}}$. We can combine
that function with $f$ and define a \textsf{``}tabulating\textsf{''} function (denoted
by $\text{tab}_{L}$):
\begin{align*}
 & \text{tab}_{L}^{A}:L^{A}\rightarrow(\text{Int}\rightarrow A)\times L^{\text{Int}}\quad,\\
 & \text{tab}_{L}^{A}\triangleq\Delta\bef\big(f\boxtimes(\text{zwi}_{L}\bef\pi_{2}^{\uparrow L}\big)=p^{:L^{A}}\rightarrow f(p)\times(p\triangleright\text{zwi}_{L}\triangleright\pi_{2}^{\uparrow L})\quad.
\end{align*}
Then the above property of $\text{zwi}_{L}$ is rewritten as:
\[
