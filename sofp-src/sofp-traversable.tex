...
\end{equation}
Since we have already assumed that both laws~(\ref{eq:foldFn-first-special-law})
...
\begin{align*}
{\color{greenunder}\text{left-hand side}:}\quad & f\big(e_{M}\triangleright(p\triangleright\gunderline{\text{inMF}^{\uparrow L}\triangleright\text{foldFn}})\big)=f\big(e_{M}\triangleright(p\triangleright\gunderline{\text{inMF}^{\uparrow L}\bef\text{reduceE}})\big)\\
{\color{greenunder}\text{use Eq.~(\ref{eq:monoidal-naturality-law-of-reduceE-inMF})}:}\quad & \quad=f\big(e_{M}\triangleright(p\triangleright\text{reduceE}\bef\text{inMF})\big)=f\big(\gunderline{\text{inMF}}\,(p\triangleright\text{reduceE})\gunderline{(e_{M})}\big)\\
{\color{greenunder}\text{use Eq.~(\ref{eq:identity-law-of-inMF})}:}\quad & \quad=f\big(p\triangleright\text{reduceE}\big)=p\triangleright\text{reduceE}\bef f\quad,\\
...
{\color{greenunder}\text{use Eq.~(\ref{eq:identity-law-of-inMF})}:}\quad & \quad=p\triangleright\text{reduceE}\bef f\quad.
\end{align*}
...
satisfies the law~(\ref{eq:foldFn-second-special-law}):
...
\begin{equation}
\text{if }x\triangleright(h^{:A\rightarrow A}\rightarrow f\bef h)^{\uparrow L}=y\triangleright(g^{:B\rightarrow B}\rightarrow g\bef f)^{\uparrow L}\text{ then }f\bef\phi(x)=\phi(y)\bef f\quad.\label{eq:strong-dinaturality-law-of-phi-with-f}
\end{equation}
...
able to derive Eq.~(\ref{eq:first-special-law-of-phi}). After trying
...
\end{align*}
This shows that Eq.~(\ref{eq:first-special-law-of-phi}) holds.

\textbf{(b)} We already showed in part \textbf{(a)} that $\phi$ satisfies
Eq.~(\ref{eq:strong-dinaturality-law-of-phi-with-f}).
...
constructor covariant in each of its three type parameters). To show
that $S$ is bitraversable, we need to assume that $T$ is traversable
with respect to all its type parameters at once, which we call \textsf{``}3-traversable\textsf{''}.
We say that a 3-functor $T^{A,B,R}$ is \textbf{3-traversable}\index{3-traversable 3-functor}
