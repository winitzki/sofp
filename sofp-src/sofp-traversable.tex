...
be \emph{polynomial} bifunctors. Such $S^{A,R}$ will define recursive
...
already defined types. The general implementation of \lstinline!fold!
...
Folding with a recursion scheme allows us to implement operations
such as \lstinline!printLaTeXSubtree! (Section~\ref{subsec:Recursion-schemes.-folding})
...
through custom code. We will now figure out how to implement \lstinline!zipWithDepth!
as a more general traversal operation that uses a recursion scheme.
...
To illustrate this, let us implement \lstinline!zipWithDepth! as
a traversal with a tree recursion scheme.{*}{*}{*}
...
\subsection{Perfect-shaped binary tree}
...
Show that the identity law is not satisfied by the implementation
of $\text{seq}:L^{F^{A}}\rightarrow F^{L^{A}}$ for $F^{A}\triangleq\bbnum 1+A$
if \lstinline!seq! always returns an empty option ($1+\bbnum 0$,
...
\subsection{Recursion schemes for perfect-shaped trees}
