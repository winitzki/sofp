...
\end{lstlisting}
This transforms the tree \Tree[ 8 [ [ 3 5 ] 4 ] ]  into \Tree[ 28 [ [ 23 25 ] 24 ] ] \relax.
However, the \lstinline!map! method works separately with each leaf
and cannot use any information from previously computed values. We
cannot use \lstinline!map! to, e.g., count the leaves and to label
each leaf with its index value, similarly to the \lstinline!zipWithIndex!
function for sequences, transforming the tree \Tree[ 8 [ [ 3 5 ] 4 ] ] 
into \Tree[ (8,0) [ [ (3,1) (5,2) ] (4,3) ] ] \relax. For such tasks,
we need the \lstinline!traverse! operation.
...
and for breadth-first tree traversals. The implementation of \lstinline!zipWithIndexDFS!
will have this form:
\begin{lstlisting}
...
val f: A => St[(A, Int)] = ???     // Define the "decoration" function.
...
to define the function \lstinline!f! describing the \textsf{``}decoration
logic\textsf{''}. Begin writing the code of \lstinline!f!:
\begin{lstlisting}
val f: A => St[(A, Int)] = a => St { i => ???: ((A, Int), Int) }
\end{lstlisting}
...
the internal state to keep the number of leaves seen so far. We just
need to increment the internal state after copying it to the leaf:
\begin{lstlisting}
val f: A => St[(A, Int)] = a => St { i => ((a, i), i + 1) }
\end{lstlisting}
...
Note that the depth-first code differs from the breadth-first code
only in the implementation of \lstinline!traverse!. The \textsf{``}decoration\textsf{''}
logic is completely separate from the traversal code. As a result,
we gain flexibility: an arbitrary implementation of \lstinline!traverse!
may be used with an arbitrary \textsf{``}decoration\textsf{''} function.
...
Depth labeling can be implemented as a special operation such as zipWithDepth.

More generally, traversals cannot perform aggregations that depend
on depth or on the left or right position of data in the tree. An
example is pretty-printing where the tree  \Tree[ [ 1 [ 2 3 ] ] 4 ] 
is printed as the string \lstinline!"[ [ 1 [ 2 3 ] ] 4 ]"!. 
