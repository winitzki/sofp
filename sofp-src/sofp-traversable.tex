...
the \lstinline!reduce! method. In this way, we obtain a complete
...
to a new function called \lstinline!foldMap! and then to a function
called \lstinline!traverse!. 
...
The type signature of \lstinline!foldMap! suggests that we replace
\lstinline!Seq[_]! by an arbitrary type constructor, denoted by \lstinline!L[_]!.
...
\begin{lstlisting}
def traverse[F[_]: Applicative: Functor, A, B](f: A => F[B]): L[A] => F[L[B]]
\end{lstlisting}
\[
...
For the given type constructor $L^{\bullet}$, write the type signature
of \lstinline!trav! as:
\begin{lstlisting}
...
def trav[A, B, F[_]: Applicative : Functor](f: A => F[B]): Option[(A, A)] => F[Option[(B, B)]] = {
  ...
  case Some((a1, a2))   => ???                        // Expect a value of type F[Some[(B, B)]] here.
}
\end{lstlisting}
...
two values of type $F^{B}$. We may combine them in a single value
of type $F^{B\times B}$ via $F$\textsf{'}s \lstinline!zip! method. It remains
to use $F$\textsf{'}s \lstinline!map! and transform $F^{B\times B}$ to the
required type $F^{\bbnum 1+B\times B}$:
\begin{lstlisting}
def trav[A, B, F[_]: Applicative : Functor](f: A => F[B]): Option[(A, A)] => F[Option[(B, B)]] = {
  case None             => Applicative[F].pure(None)          // Here we must return an empty value.
...
of type $A$. Each stored value of type $A$ will be processed using
the given function $f:A\rightarrow F^{B}$. All of the resulting $F$-effects
...
type $L^{B}$. That value should reproduce the shape of the data structure
that was present in the input value of type $L^{A}$. 

In all examples above, we merge all $F$-effects together, collecting
each $F$-effect exactly once. It would be strange if the \lstinline!traverse!
operation were to repeat some $F$-effects, as in the following code:
\begin{lstlisting}
...
get after applying \lstinline!addLeft! and \lstinline!addRight!.
...
of type \lstinline!TD[A] => T2[A]! that will only work with valid
tree descriptors, that is, values of type \lstinline!TD[A]! that
were obtained by applying \lstinline!t2ToTd! to some trees of type
\lstinline!T2[A]!. The composition \lstinline!t2ToTD andThen tdToT2!
...
\lstinline!Left()! wrapper by using the helper functions \lstinline!removeLeft!
and \lstinline!filterLeft!:
\begin{lstlisting}
...
with an arbitrary traversal order may be used with an arbitrary \textsf{``}decoration
logic\textsf{''}. 
...
\subsection{Tasks that \emph{cannot} be performed via \texttt{traverse}\label{subsec:Tasks-not-implementable-via-traverse}}
...
into a text form. In the \LaTeX{} format used to typeset this book,
the tree {\tiny{} \Tree[ 8 [ [ 3 5 ] 4 ] ] } is represented by the
text string \lstinline!"\Tree[ 8 [ [ 3 5 ] 4 ] ]"!. We may write
the following code for converting trees to this format:
\begin{lstlisting}
...
is directly modeled on induction). The main difference is that in
co-induction, the base cases are not at the beginning of the computation
but \textsf{``}in the future\textsf{''}. So, the programmer must figure out both a
suitable type $Z$ and a suitable function $f$ so that \lstinline!unfold(f)(z)!
stops the recursion at the correct places.
...
However, the type signature $f:Z\rightarrow S^{A,Z}$ itself does
not guarantee that $f(z)$ will return values that stop the unfolding
at the right places. It is the programmer\textsf{'}s responsibility to choose
functions $f$ that do not cause infinite loops during unfolding.
If $S^{A,Z}\triangleq A+Z\times Z$ and $f(z)$ always returns values
of type $\bbnum 0+Z\times Z$ (for example, $f(z)\triangleq\bbnum 0+z\times z$),
the unfolding operation will enter an infinite loop. This choice of
$f$ will force \lstinline!unfold(f)! to try constructing a tree
of infinite size. 
...
can imitate infinite lists or trees by evaluating further data items
only on demand.
