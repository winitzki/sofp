...
The \lstinline!map!/\lstinline!reduce! programming style\index{map/reduce programming style@\texttt{map}/\texttt{reduce} programming style}
was perhaps the first significant industrial use of functional programming.
The methods \lstinline!filter!, \lstinline!flatMap!, and \lstinline!zip!
were added later to increase the expressive power of that programming
style. Previous chapters examined systematically the properties of
those methods and generalized them to many different type constructors.
This chapter uses the same approach to study and generalize the \lstinline!reduce!
method. In this way, we will obtain a complete understanding of the
\lstinline!map!/\lstinline!reduce! programming style.

\subsection{\label{subsec:From-reduce-and-foldleft-to-foldmap}From \texttt{reduce}
and \texttt{foldLeft} to \texttt{foldMap}}
...
is a generalized version of \lstinline!reduce!. We can implement
...
\lstinline!foldLeft! further to obtain the \lstinline!traverse!
method, for which we can motivate non-trivial laws. We will achieve
this generalization in two steps, first transforming \lstinline!foldLeft!
to a new function called \lstinline!foldMap! and then to \lstinline!traverse!. 

The way from \lstinline!foldLeft! to \lstinline!foldMap! starts
by changing the type of the argument \lstinline!update: (B, A) => B!
...
is that all functions of type $B\rightarrow B$ form a \emph{monoid}
...
\lstinline!Seq[_]! by another type constructor, denoted by \lstinline!L[_]!.
...
The second step towards the \lstinline!traverse! method replaces
the arbitrary monoid $M$ by an arbitrary applicative functor $F^{B}$
with a type parameter $B$. Since this step is not straightforward,
...
need to apply the processing function to all items from the list in
...
by recalling the example with $F=$ \lstinline!Future! and $L=$
\lstinline!List!.
...
Implement a \lstinline!traverse! operation for the perfect-shaped
tree \lstinline!PTree! (Section~\ref{subsec:Perfect-shaped-trees}).
...
def trav[A, B, F[_]: Functor : Applicative](f: A => F[B])(t: PTree[A]): F[PTree[B]] = t match {
  case Leaf(a)     => f(a).map(b => Leaf(b))           // Reproduce the Leaf structure under F.
...
is equivalent to \lstinline!foldMap!. So, a foldable functor supports
applying any aggregation operation to the functor\textsf{'}s data. 
...
a list of all values at level $1$, and so on. At each level, values
must be collected left to right. The result must be a list of lists.
For instance, the tree \lstinline!t2! has no values at level $0$,
...
work correctly since \lstinline!zip! will truncate the longer list.
...
$F$. Section~\ref{subsec:Decorating-a-tree-breadth-first-traversal}
will show an example implementation of a breadth-first \lstinline!traverse!.
...
method works separately with each leaf and cannot use any information
from previously computed values. We cannot use \lstinline!map! to,
e.g., implement a \lstinline!zipWithIndex! function for trees: to
count the leaves and to label each leaf with its index value, transforming
the tree {\tiny{}\Tree[ 8 [ [ 3 5 ] 4 ] ] } into {\tiny{}\Tree[ (8,0) [ [ (3,1) (5,2) ] (4,3) ] ]}
...
depth-first tree traversal; let us call that function \lstinline!zipWithIndexDF!.
...
just need to increment the internal state after copying it to a leaf:
\begin{lstlisting}
...
What additional code does a \lstinline!traverse! need? Folding over
...
(\lstinline!addLeft! and \lstinline!addRight!) for this transformation:
\begin{lstlisting}
...
implement \lstinline!travTD! as follows:
\begin{lstlisting}
...
traversal order. As a result, we gain flexibility: a \lstinline!traverse!
with an arbitrary traversal order may be used with an arbitrary \textsf{``}decoration\textsf{''}
logic. 
...
as having a \lstinline!traverse! operation, which is convenient to
...
monad and compute a value of type \lstinline!State[Z, Z]!. The code
of \lstinline!scanLeft! calls \lstinline!traverse! with a suitable
function \lstinline!accum! that creates the required value of type
\lstinline!State[Z, Z]!:
\begin{lstlisting}
...
\end{lstlisting}
In this way, \lstinline!scanLeft! is made available for a wide class
of data types (traversable functors).
...
\subsection{Tasks \emph{not} implementable\label{subsec:Tasks-not-implementable-via-traverse}
via \texttt{traverse}}
...
a value equal to its depth. For instance, a depth-labeled tree {\tiny{} \Tree[ 8 [ [ 3 5 ] 4 ] ] }
becomes {\tiny{}}{\tiny{} \Tree[ (8,1) [ [ (3,3) (5,3) ] (4,2) ] ] }.
This cannot be implemented via \lstinline!traverse! because it cannot
detect nodes that have the same depth in the tree. To see this in
more detail, recall that the code of \lstinline!traverse! always
collects and merges all the effects of a given functor \lstinline!F!.
...
is in the left or in the right branch and at what depth). An example
of such a computation is \textsf{``}pretty-printing\textsf{''} that converts trees
into a text form. In the \LaTeX{} format used in this book, the tree
{\tiny{} \Tree[ 8 [ [ 3 5 ] 4 ] ] } is represented by the text \lstinline!\Tree[ 8 [ [ 3 5 ] 4 ] ]!.
...
that we can implement this common pattern separately from the custom
code, reducing the effort required for implementing the folding and
traversing operations for different recursive data types.
...
matrix in the code above is the lifting $\overline{\text{pls}}^{\uparrow S^{A,\bullet}}$
with respect to the type parameter $R$ of $S^{A,R}$. So, we may
express the custom string manipulations specific to \lstinline!printLaTeXSubtree!
...
It is important that the function $\text{fold}_{S}$ may be implemented
in the same way for all recursion schemes $S^{\bullet,\bullet}$ and
for all result types $Z$ (which are no longer required to be monoids!).
Each recursion scheme $S^{\bullet,\bullet}$ defines a certain recursive
type constructor $L^{\bullet}$. In previous chapters, we have seen
examples where different recursion schemes $S^{\bullet,\bullet}$
were used to define lists, trees, and other recursive data types.
So, the same code of $\text{fold}_{S}$ will work for all those data
...
cause an infinite loop in $\text{fold}_{S}$. A simple example where
$\text{fold}_{S}$ enters an infinite loop with the recursion scheme
$S^{A,R}\triangleq A+(\bbnum 1\rightarrow R)$. Note that $S^{A,R}$
...
\end{comment}
the evaluation of the value of type $R$. This allows us to implement
a well-defined, finite value \lstinline!x: L[A]! which refers to
itself under the delayed evaluation:
\begin{lstlisting}
...
number of a given rose tree. The function \lstinline!foldTreeN! allows
us to implement that computation:
\begin{lstlisting}
...
The recursion scheme for a \lstinline!List[A]! is $S^{A,R}\triangleq\bbnum 1+A\times R$.
...
$f:Z\rightarrow\bbnum 1+A\times Z$ that we can \textsf{``}unfold\textsf{''} into
...
given only a current value of type $Z$. We can do that if we keep
track of the current initial power of $2$. So, let us choose $Z\triangleq$
\lstinline!Long! to represent that value; the type $A$ will be also
\lstinline!Long!. 
...
\end{lstlisting}
The code of $f$ is \emph{not} recursive; the value $n$ is stored
inside the nameless function returned by $f(n)$. We can now complete
the solution:
\begin{lstlisting}
...
intermediate step that generates a subtree of this tree at some point
in the middle of \textsf{``}unfolding\textsf{''}. For the tree of depth $3$ as shown
above, an example of a subtree in the middle is {\tiny{}\Tree[ 4 5 ]}.
This subtree must be computed as \lstinline!unfoldT2(f)(z)! with
some \lstinline!z! of type \lstinline!Z!. The information needed
for this computation is the initial value ($4$) and the total number
...
corresponding to the two next subtrees at one level deeper. These
two subtrees have sizes \lstinline!m / 2! and initial values \lstinline!k!
and \lstinline!k + m / 2!. So, the code of \lstinline!f! is:
\begin{lstlisting}
val f: Z => Either[Int, (Z, Z)] = {
...
\subsubsection{Example \label{subsec:Example-unfold-tree-1}\ref{subsec:Example-unfold-tree-1}}
...
are on the left and even numbers on the right. For example, \lstinline!evenOdd(3)!
should generate the tree {\tiny{}\Tree[ 3 [ [ 1 0 ] 2 ] ]}, while
\lstinline!evenOdd(4)! should give {\tiny{}\Tree[ [ 3 [ [ 1 0 ] 2 ] ] 4 ]}.
...
to some intermediate value \lstinline!z! of type \lstinline!Z!.
If \lstinline!f(z) == Left(k)!, we will get a \lstinline!Leaf! with
value \lstinline!k!. The other possibility is \lstinline!f(z) == Right((z1, z2))!,
where \lstinline!z1! and \lstinline!z2! are some values of type
\lstinline!Z!. This will generate a \lstinline!Branch! with two
subtrees. The function \lstinline!unfoldT2! will then apply \lstinline!f!
to \lstinline!z1! and \lstinline!z2! in order to create the left
and the right subtrees. The difference between those subtrees must
come entirely from the difference between the values \lstinline!z1!
and \lstinline!z2!. 

In our case, we need to make either the left or the right subtree
a \lstinline!Leaf! depending on whether the initial leaf value is
odd or even. The function \lstinline!f! cannot know that a call \lstinline!f(z1)!
or \lstinline!f(z2)! is used to generate a left or a right subtree,
or that sometimes a \lstinline!Leaf! must be made. This information
can only come from the values \lstinline!z1! and \lstinline!z2!.
So, the type \lstinline!Z! must contain a \lstinline!Boolean! flag
indicating whether it is necessary to create a \lstinline!Leaf! at
the current place. For clarity, let us define the type \lstinline!Z!
as a case class:
\begin{lstlisting}
final case class Z(startAt: Int, makeLeaf: Boolean)
val init = Z(startAt = n, makeLeaf = false)
\end{lstlisting}
When \lstinline!makeLeaf == true!, we should create a \lstinline!Leaf!,
so \lstinline!f! should return a \lstinline!Left()!. Otherwise,
\lstinline!f! should return a \lstinline!Right((z1, z2))!, where
\lstinline!z1! and \lstinline!z2! should set \lstinline!makeLeaf!
appropriately depending on \lstinline!startAt! being odd or even.
Looking at the examples, we find that \lstinline!f! should also return
a \lstinline!Left()! when \lstinline!startAt == 0!. So, the complete
code of \lstinline!f! is:
\begin{lstlisting}
val f: Z => Either[Int, (Z, Z)] = {
...
During evaluation, \lstinline!unfold(f)(z)! will recursively call
$f$ whenever the value $f(z)$ of type $S^{A,Z}$ contains additional
values of type $Z$. In the examples just shown, we used \lstinline!unfold(f)!
with a function $f$ that was specially designed to stop the recursion
at the required places. For instance, if $S^{A,Z}\triangleq A+Z\times Z$,
the function $f$ can return a value of type $A+\bbnum 0$ to stop
the unfolding.

However, the type signature $f:Z\rightarrow S^{A,Z}$ does not guarantee
that $f(z)$ will return values that stop the unfolding at the right
place. It is the programmer\textsf{'}s responsibility to choose functions $f$
that do not cause infinite loops during unfolding. If $S^{A,Z}\triangleq A+Z\times Z$
and $f(z)$ always returns values of type $\bbnum 0+Z\times Z$ (for
example, $f(z)\triangleq\bbnum 0+z\times z$), the unfolding operation
will enter an infinite loop trying to construct a tree of infinite
size. 

Data structures in a computer\textsf{'}s memory cannot have infinite size.
To ensure that unfolding always terminates, we could use a data structure
that delays the evaluation of some of its parts. Such data structures
can imitate infinite lists or trees where further data items are evaluated
on demand.
...
tree with a delayed evaluation of branches (but eager evaluation of
leaves). This data structure supports unfolding with \emph{any} function
...
def unfoldUT[A, Z](f: Z => S[A, Z])(init: Z): UT[A] = f(init) match {
  ...
  case Right(func)   => UBranch { _ =>     // It is important to delay the evaluation of func(()).
    val (z1, z2) = func(())
    (unfoldUT(f)(z1), unfoldUT(f)(z2))     // Unfolding of further branches is also delayed.
  }
}
...
\end{lstlisting}
The value \lstinline!tree1toInf! is finite but contains a potentially
unbounded number of leaves computed on demand. To visualize \lstinline!tree1toInf!,
w write a function that converts \lstinline!UT[A]! to \lstinline!T2[A]!
...
relationship between \lstinline!foldMap!, the ordinary \lstinline!traverse!
(denoted $\text{trav}_{L}$), and $\text{fold}_{S}$:
\begin{align*}
...
powerful but hard to use because we need to work always with data
structures defined via the \lstinline!Fix! type constructor. It is
easier to use a specialized version of $\text{trav}_{S}$ for the
data structure at hand, similarly to what we did in the previous sections
for folding and unfolding. 

To illustrate this, let us implement \lstinline!zipWithDepth! via
a traversal with a binary tree recursion scheme. As in Section~\ref{subsec:Recursion-schemes.-folding},
we will use the \lstinline!State! monad as the functor $F$ when
...
\end{lstlisting}
To test this code, apply \lstinline!zipWithDepth! to a sample tree
...
\begin{lstlisting}
scala> zipWithDepth(t2)
res0: T2[(Int, Int)] = Branch(Branch(Leaf((8,2)), Branch(Leaf((3,3)), Leaf((5,3)))), Leaf((4,1)))
\end{lstlisting}

\vspace{-0.5\baselineskip}
\end{wrapfigure}%

\noindent ~{\tiny{}\Tree[ [ (8,2) [ (3,3) (5,3) ] ] (4,1) ]}

\noindent ~

Note that we used the \lstinline!State! monad\textsf{'}s method \lstinline!set(s)!
...
effect. We can see that the recursion scheme-based traversal ($\text{trav}_{S}$)
is more powerful than the plain traversal, $\text{trav}_{L}(f^{:A\rightarrow F^{B}})$,
that only uses the applicative functor\textsf{'}s methods of $F$.
...
\textbf{(c)} $F^{A}\triangleq\bbnum 1+A+A\times A\times F^{A}$ (defined
recursively).
...
implement a \lstinline!Traversable! instance for right-to-left depth-first
traversal order. Use that to implement \lstinline!zipWithIndex!,
and test that the tree {\tiny{} \Tree[ [ 8 [ 3 5 ] ] 4 ] } is decorated
with index as {\tiny{} \Tree[ [ (8,3) [ (3,2) (5,1) ] ] (4,0) ] }.
...
To study the laws of the operations of folding and traversing, it
helps to choose simpler but equivalent versions of these operations.
We have seen four methods that implement folding operations: \lstinline!foldLeft!,
\lstinline!foldMap!, \lstinline!reduce!, and \lstinline!toList!.
With suitable naturality laws, these methods are equivalent.

\subsection{Equivalence of \texttt{foldLeft}, \texttt{foldMap}, \texttt{reduce},
...
So, we rewrite the type signature of \lstinline!reduceE! using a
...
\end{align*}
While \lstinline!toList! has the simplest type signature, \lstinline!foldLeft!
is used most often. We will now show that these four functions are
equivalent, assuming certain naturality laws.
...
\]
Does this law hold with $\phi=$ \lstinline!reduceE!? Since we would
need to use \lstinline!reduceE! with type parameters $M$ and $N$,
those types have to be monoids for the law to make sense. It turns
out that the law must be used only with functions $f$ that satisfy
certain conditions with respect to the monoids $M$ and $N$. 
...
\[
\xymatrix{\xyScaleY{1.4pc}\xyScaleX{2.8pc}L^{M}\ar[r]\sp(0.5){\ \text{reduceE}^{M}\ }\ar[d]\sp(0.45){\,f^{\uparrow L}} & M\ar[d]\sp(0.45){\,f}\\
...
\end{equation}
where the types $M$, $N$ must be monoids and $f:M\rightarrow N$
...
but may not inspect the type parameter \lstinline!M! via run-time
reflection or make any decisions based on that. 
...
\textbf{(b)} The functions \lstinline!foldLeft! and \lstinline!reduceE!
are equivalent as long as the law~(\ref{eq:foldFn-transformation-law})
shown below holds and \lstinline!reduceE! satisfies the monoidal
naturality law with respect to its type parameter $M$. 

We will show in Example~\ref{subsec:relational-property-for-foldFn}
(Appendix~\ref{app:Proofs-of-naturality-parametricity}) that Eq.~(\ref{eq:foldFn-transformation-law})
is a consequence of parametricity. So, the law~(\ref{eq:foldFn-transformation-law})
will hold automatically as long as the code of \lstinline!foldLeft!
is fully parametric.
...
with respect to the type parameter $A$. To get that equivalence,
we need to set $F^{A}\triangleq L^{A}$, $G^{B}\triangleq M$, and
$H^{B}\triangleq M$ in Statement~\ref{subsec:Statement-tr-equivalent-to-ftr}.

\textbf{(b)} A similar equivalence holds for \lstinline!foldLeft!
if we reformulate its type signature in a form more similar to that
of \lstinline!foldMap!. We change the order of curried arguments
in \lstinline!foldLeft! and also replace the updater function of
type $B\times A\rightarrow B$ by an equivalent curried function of
type $A\rightarrow B\rightarrow B$ (similarly to what was done in
Section~\ref{subsec:From-reduce-and-foldleft-to-foldmap}). The result
is a function we denote by $\text{fldl}_{L}$:
\begin{align*}
...
\end{align*}
Setting $F^{A}\triangleq L^{A}$, $G^{A}\triangleq B\rightarrow B$,
and $H^{A}\triangleq B\rightarrow B$ in Statement~\ref{subsec:Statement-tr-equivalent-to-ftr},
we obtain an equivalence between $\text{fldl}_{L}$ and a function
with type signature $L^{B\rightarrow B}\rightarrow B\rightarrow B$
that we will temporarily denote by \lstinline!foldFn! (\textsf{``}fold with
function\textsf{''}):
\[
...
We will now prove that \lstinline!foldFn! and \lstinline!reduceE!
are equivalent. The type signatures of these functions are similar
except that \lstinline!foldFn! uses the type $B\rightarrow B$ where
\lstinline!reduceE! uses an arbitrary monoid $M$ (having an empty
value $e_{M}$ and a binary operation $\oplus_{M}$). The type of
functions $B\rightarrow B$ is itself a monoid that we here denote
...
\end{align*}
Because of this, we may use $f\triangleq\text{inMF}$ in the monoidal
naturality law~(\ref{eq:monoidal-naturality-law-of-reduceE}) of
\lstinline!reduceE! with monoids $M$ and $N\triangleq\text{MF}^{M}$:
\begin{equation}
...
It remains to demonstrate the isomorphism between \lstinline!foldFn!
...
We find that $\text{foldFn}^{\prime}(p)=\text{foldFn}\,(p)$ if we
assume that the following law holds:
\begin{equation}
...
\end{equation}
This special law is a consequence of parametricity, as shown in Example~\ref{subsec:relational-property-for-foldFn}.
...
{\color{greenunder}\text{use Eq.~(\ref{eq:monoidal-naturality-law-of-reduceE-applied-to-inMF})}:}\quad & =e_{M}\triangleright\big(p\triangleright\text{reduceE}\bef\text{inMF}\big)=\gunderline{\text{inMF}}\,(p\triangleright\text{reduceE})\gunderline{(e_{M})}\\
...
will always satisfy the transformation law~(\ref{eq:foldFn-transformation-law}):
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{foldFn}\left(p\right):}\quad & \text{foldFn}\,(p\triangleright\text{inMF}^{\uparrow L})(e_{\text{MF}})=\text{reduceE}\,(p\triangleright\text{inMF}^{\uparrow L})(e_{\text{MF}})\\
 & =e_{\text{MF}}\triangleright(p\triangleright\gunderline{\text{inMF}^{\uparrow L}\bef\text{reduceE}})\\
{\color{greenunder}\text{use Eq.~(\ref{eq:monoidal-naturality-law-of-reduceE-applied-to-inMF})}:}\quad & =e_{\text{MF}}\triangleright\big(p\triangleright\text{reduceE}\bef\text{inMF}\big)=\gunderline{\text{inMF}}\,(p\triangleright\text{reduceE})\gunderline{(e_{\text{MF}})}\\
{\color{greenunder}\text{use Eq.~(\ref{eq:identity-law-of-inMF})}:}\quad & =p\triangleright\text{reduceE}=\text{reduceE}\,(p)=\text{foldFn}\,(p)\quad.
\end{align*}

Let us clarify the meaning of the special law~(\ref{eq:foldFn-transformation-law}).
This law is a consequence of parametricity, so it ensures that \lstinline!foldFn(p)!
uses the functions of type $B\rightarrow B$ contained in $p:L^{B\rightarrow B}$
in a fully parametric manner. In other words, the code of \lstinline!foldFn!
must work in the same way for all types $B$; it cannot inspect the
type $B$ or the code of the functions of type $B\rightarrow B$.
The only possibility of using those functions is to compose them with
each other, obtaining again a function of type $B\rightarrow B$.
The law~(\ref{eq:foldFn-transformation-law}) expresses this property
by writing the function composition explicitly as part of the code
of the function \lstinline!inMF!.
...
whose binary operation is the list concatenation. The (monoidally
natural) function \lstinline!reduceList[M]! is defined for monoids
$M$ by:
\begin{lstlisting}
...
\[
t\triangleright f^{\uparrow\text{List}}\bef\overline{\text{reduceList}}=t\triangleright\overline{\text{reduceList}}\bef f=f(\overline{\text{reduceList}}(t))\quad.
\]
The bottom-row expression in the last matrix is then simplified to:
\begin{align*}
 & f(h)\oplus_{N}(t\triangleright f^{\uparrow\text{List}}\bef\overline{\text{reduceList}})=f(h)\oplus_{N}f(\overline{\text{reduceList}}(t))\\
{\color{greenunder}\text{monad morphism law}:}\quad & =f\big(h\oplus_{M}\overline{\text{reduceList}}(t)\big)\quad.
\end{align*}
...
M\times\text{List}^{M} & h\times t\rightarrow f\big(h\oplus_{M}\overline{\text{reduceList}}(t)\big)
\end{array}\\
 & =\,\begin{array}{|c||c|}
 & N\\
\hline \bbnum 1 & \_\rightarrow e_{M}\\
M\times\text{List}^{M} & h\times t\rightarrow h\oplus_{M}\overline{\text{reduceList}}(t)
\end{array}\bef f=\text{reduceList}\bef f\quad.
\end{align*}
 This is equal to the right-hand side of the law.

We are ready to show the isomorphism between \lstinline!reduceE!
and \lstinline!toList! in both directions:

\textbf{(1)} Given a function \lstinline!reduceE!, define \lstinline!toList!
...
matrix composition:
\begin{align*}
 & \text{pu}_{\text{List}}\bef\text{reduceList}\\
 & =\,\begin{array}{|c||cc|}
...
\end{array}\\
 & =\,\,\begin{array}{|c||c|}
 & M\\
\hline M & m\rightarrow m\oplus_{M}\gunderline{\overline{\text{reduceList}}(\left[\right])}
\end{array}\,=m\rightarrow\gunderline{m\oplus e_{M}}=m\rightarrow m=\text{id}\quad.
\end{align*}
In the last line, we used the fact that \lstinline!reduceList! applied
to an empty list gives $e_{M}$.
...
and \lstinline!toList! was ensured by assuming full parametricity,
which entails various naturality laws. However, those naturality laws
do not fully describe the programmer\textsf{'}s expectations about the behavior
...
of type \lstinline!A!, this implementation would be unacceptable
since it loses information (the code always ignores the argument and
returns a fixed value).

To prohibit such implementations, we would like to impose a law that
holds only when \lstinline!toList! has extracted every value of type
\lstinline!A! stored in \lstinline!p!. Unfortunately, it seems to
be impossible to express this property in the form of an equation
satisfied by \lstinline!toList!. Often, a loss of information is
prevented by imposing an identity law. Can we formulate an identity
law for \lstinline!toList!? Such a law could state that \lstinline!toList(p)!
should extract some known values contained in \lstinline!p!. If $L$
were a pointed functor\index{pointed functor}, we could use its \lstinline!pure!
method to inject a known value \lstinline!x! into the container \lstinline!p!
and then require \lstinline!toList! to extract the same value \lstinline!x!.
But the definition of \lstinline!toList! does not require the functor
...
the form of a \textsf{``}lifting\textsf{''}, where a function of one type is transformed
...
to impose the functor laws on these liftings. For instance, the functor
...
is a \emph{recursive} polynomial functor). These functors represent
finite containers storing values of type \lstinline!A!. So, it is
clear what it means to extract \textsf{``}all values of type $A$\textsf{''} from
such containers. To show that all polynomial functors are foldable,
we will define \lstinline!toList! inductively via structural analysis.
The definition will ensure that \lstinline!toList! extracts exactly
as many values as stored in the functor.
...
all values of a given type (as an example, consider the type $R=\text{String}\rightarrow\text{String}$).
...
monad ($\text{Reader}^{R,A}$). The only exceptions are types $R$
that are known to have a fixed and finite number of distinct values,
such as $R=\text{Boolean}$. However, in these cases the type $R\rightarrow A$
...
If $K$ and $L$ are foldable functors (so $\text{toList}_{K}$ and
$\text{toList}_{L}$ are available), we define the product $M^{A}\triangleq K^{A}\times L^{A}$
and the corresponding $\text{toList}_{M}$ as:
\begin{lstlisting}
def toList_M[A]: ((K[A], L[A])) => List[A] = { case (p, q) => toList_K(p) ++ toList_L(q) }
\end{lstlisting}
\[
\text{toList}_{M}:K^{A}\times L^{A}\rightarrow\text{List}^{A}\quad,\quad\quad\text{toList}_{M}\triangleq p^{:K^{A}}\times q^{:L^{A}}\triangleq\text{toList}_{K}(p)\,\pplus\,\text{toList}_{L}(q)\quad.
\]
This implementation contains an arbitrary choice: the values stored
in $p$ are taken first and the values stored in $q$ are taken last.
We could equally well write $\text{toList}_{L}(q)\,\pplus\,\text{toList}_{K}(p)$
in the function body. This arbitrary choice corresponds to the arbitrariness
of the order in which a folding operation traverses the values stored
a container. Different instances of the \lstinline!Foldable! typeclass
may define different traversal orders, as we have seen in the code
...
and the corresponding $\text{toList}_{M}:K^{A}+L^{A}\rightarrow\text{List}^{A}$
as:
...
\noindent ~\vspace{-2.25\baselineskip}
\[
...
recursively by $L^{A}\triangleq S^{A,L^{A}}$, where $S^{\bullet,\bullet}$
is a bifunctor that is itself foldable. A bifunctor $S^{\bullet,\bullet}$
...
\]
Given $\text{toList}_{S}$, we define $\text{toList}_{L}$ by:
\begin{lstlisting}
def toList_L[A]: S[A, L[A]] => List[A] = _.map2(toList_L).map1(List(_)).toList.flatten
\end{lstlisting}
\[
...
of $S^{\bullet,\bullet}$. We also convert $S^{A,\text{List}^{A}}$
...
The final flatten operation reduces that to a $\text{List}^{A}$.

This completes the implementation of \lstinline!toList! for the five
type constructions that build up polynomial functors. In each case,
it is clear that \lstinline!toList! extracts all stored values of
type $A$. In principle, one can write a code generator to provide
a \lstinline!Foldable! instance for any polynomial functor automatically.
When writing code by hand, it is more convenient to implement folding
operations directly, as we have been doing at the beginning of this
chapter.
...
in addition to \lstinline!Future.traverse!. While full type signature
of \lstinline!Future.sequence! is complicated due to the implementation
details, the main use of \lstinline!sequence! is for transforming
a sequence of \lstinline!Future! values that may run in parallel.
The result is a single \lstinline!Future! value that waits until
all parallel computations are finished. We may simplify the type signature
of \lstinline!Future.sequence! to:
\begin{lstlisting}
def sequence[A, L[X]: TraversableOnce[X]]: L[Future[A]] => Future[L[A]]
\end{lstlisting}

The function \lstinline!Future.sequence! is limited to sequences
and sequence-like data types such as \lstinline!L[A] = List[A]!.
To generalize the \lstinline!sequence! operation to other data types,
we replace a sequence-like type by an arbitrary traversable functor
\lstinline!L[A]!. We also replace the \lstinline!Future! type constructor
by an arbitrary applicative functor (since the traverse operation
accepts one). It turns out that a function called \lstinline!sequence!
can then be defined via the \lstinline!traverse! operation, and vice
versa:
\begin{lstlisting}
...
if we set $\text{tr}\triangleq\text{seq}$, $\text{ftr}\triangleq\text{trav}$,
$F\triangleq L$, $G\triangleq F$, and $H\triangleq F\circ L$. 

Because of this equivalence, we may define the \lstinline!Traversable!
typeclass in terms of \lstinline!sequence! instead of using \lstinline!traverse!.
...
to prove that the laws hold for an implementation of \lstinline!sequence!.
So, we will use \lstinline!sequence! when proving the type constructions
...
work in the same way for all types \lstinline!A! and \lstinline!B!,
and for all applicative functors \lstinline!F!. How can we express
these expectations as equations that \lstinline!traverse! must satisfy?\footnote{Those equations were given in the paper by M.~Jaskelioff\index{Mauro Jaskelioff}
...
\vspace{-1\baselineskip}
\[
\xymatrix{\xyScaleY{3.0pc}\xyScaleX{5.0pc}L^{X}\ar[rd]\sp(0.6){~~\text{trav}_{L}^{F,X,B}(f\bef g)}\ar[d]\sp(0.45){f^{\uparrow L}} &  & L^{A}\ar[rd]\sb(0.4){\text{trav}_{L}^{F,A,C}(g\bef h)~~}\ar[r]\sp(0.5){\text{trav}_{L}^{F,A,B}(g)} & F^{L^{B}}\ar[d]\sp(0.45){h^{\uparrow L\uparrow F}}\\
...
$\xymatrix{\xyScaleY{2.0pc}\xyScaleX{5.0pc}L^{A}\ar[rd]\sb(0.4){\text{trav}_{L}^{G,A,B}(g\bef f)~~}\ar[r]\sp(0.5){\text{trav}_{L}^{F,A,B}(g)} & F^{L^{B}}\ar[d]\sp(0.45){f^{\uparrow L\uparrow F}}\\
 & G^{L^{B}}
}
$\vspace{0.5\baselineskip}
\end{wrapfigure}%
...
In addition, the function $f$ must preserve the applicative typeclass,
in the sense that $f$ should map $F$\textsf{'}s applicative methods (\lstinline!wu!
and \lstinline!zip!) to the corresponding methods of $G$. Otherwise,
the code of $\text{trav}_{L}^{F,A,B}$ that uses those methods of
$F$ will not be mapped via $f$ to the same code of $\text{trav}_{L}^{G,A,B}$
that uses $G$\textsf{'}s applicative methods. 
...
\begin{align*}
{\color{greenunder}\text{identity law of applicative morphism }f:}\quad & f^{:F^{\bbnum 1}\rightarrow G^{\bbnum 1}}(\text{wu}_{F})=\text{wu}_{G}\quad,\\
{\color{greenunder}\text{composition law of applicative morphism }f:}\quad & \text{zip}_{G}\big(f(p^{:F^{A}})\times f(q^{:F^{B}})\big)=f(\text{zip}_{F}(p\times q))\quad.
\end{align*}
...
\textbf{(b)} To verify the identity law:
\[
...
\end{array}\quad.
\]
The function $\text{zip}_{L}$ is defined as in the solution to Example~\ref{subsec:Example-some-applicative-morphisms}(a). 
...
$L$ satisfies the applicative naturality law~(\ref{eq:traverse-applicative-naturality-law}),
where we swap $f$ with $g$ and use applicative functors $G$, $H$
instead of $F$, G. Write the two sides of the law separately:
\begin{align*}
...
\end{align*}
To verify the composition law, write:
\begin{align*}
...
wrapped in an empty $F$-effect. So, we write the law as:
\begin{equation}
...
and the law~(\ref{eq:traverse-applicative-naturality-law}) then
it also obeys Eq.~(\ref{eq:traverse-identity-law-with-pure}).
...
\end{align*}
The left-hand side is $\text{trav}_{L}(\text{id}\bef f)=\text{trav}_{L}(f)=\text{trav}_{L}(\text{pu}_{F})$.
...
As an example where that expectation fails, consider a simple traversable
functor $L^{A}=A\times A$ and write the \lstinline!traverse! method
like this:
\begin{lstlisting}
...
to combine the three $F$-effects together. The first of the values
of type \lstinline!B! is discarded.
...
the functor $F\circ G$ is applicative. So, we may write a single
...
\]
The \textbf{composition law}\index{composition law!of traverse@of \texttt{traverse}}
...
effects do not commute. A suitable choice is when both $F$ and $G$
are the \lstinline!State! monad.\footnote{The paper \texttt{\href{https://arxiv.org/abs/1202.2919}{https://arxiv.org/abs/1202.2919}}
shows another example using the \lstinline!List! monad.} For simplicity, we set all types to \lstinline!Int! and choose the
functions $f$ and $g$ that add their arguments to the internal state.
The complete code (using the \lstinline!Zippable! typeclass defined
in Section~\ref{subsec:The-Zippable-and-Applicative-typeclass})
...
The counterexamples show that \emph{some} incorrect implementations
of \lstinline!traverse! are excluded by the composition laws. But
it remains unknown (Problem~\ref{par:Problem-traverse-law}) whether
the composition law~(\ref{eq:composition-law-of-traverse}) always
guarantees that the traverse function collects each $F$-effect exactly
once.
...
\noindent Renaming $B$ to $A$, we write the applicative naturality
...
\noindent Here we wrote the full type parameters in $f^{X}$ and $\text{seq}_{L}^{F,A}$
for clarity.
...
\vspace{-1.5\baselineskip}
$\xymatrix{\xyScaleY{1.5pc}\xyScaleX{3.0pc}L^{F^{G^{A}}}\ar[r]\sp(0.55){\text{seq}_{L}^{F,G^{A}}}\ar[rd]\sb(0.45){\text{seq}_{L}^{F\circ G,A}} & F^{L^{G^{A}}}\ar[d]\sp(0.45){(\text{seq}_{L}^{G,A})^{\uparrow F}}\\
 & F^{G^{L^{A}}}
}
...
\]
In the point-free style, we can write:
\[
...
{\color{greenunder}\text{identity laws of }\text{seq}_{M}\text{ and }\text{seq}_{N}:}\quad & =(\text{id}\boxtimes\text{id})=\text{id}\quad.
\end{align*}
...
\[
\text{seq}_{L}^{F,G^{A}}\bef(\text{seq}_{L}^{G,A})^{\uparrow F}=\big(\text{seq}_{M}^{F\circ G,A}\boxtimes\text{seq}_{N}^{F\circ G,A}\big)\bef\text{zip}_{F}\bef\text{zip}_{G}^{\uparrow F}\quad.
\]
...
would still be lawful (see Exercise~\ref{subsec:Exercise-traversables-3-1}
...
\end{align*}
Now we can simplify the left-hand side using the composition laws
of $\text{seq}_{M}$ and $\text{seq}_{N}$:
\begin{align*}
...
traversable property of $L$, we need to assume that $S$ is traversable
with respect to both its type parameters. Such recursion schemes $S$
are called bitraversable. More rigorously, a bifunctor $S^{A,B}$
is \textbf{bitraversable}\index{bitraversable bifunctor} if it has
a \lstinline!bisequence! method (denoted by $\text{seq2}_{S}$):
\[
...
lifted with respect to to the type parameter $R$ of $S^{A,R}$. The
...
order over all values stored in the polynomial functor. We can visualize
the way \lstinline!traverse! works by writing a polynomial functor
$L$ as:
\[
L^{A}=Z_{0}+Z_{1}\times A+Z_{2}\times A\times A+...\quad,
\]
where $Z_{0}$, $Z_{1}$, etc.~are fixed types. Any polynomial functor
may be equivalently rewritten in this way, perhaps with an infinite
number of parts in the disjunction. Each part of the disjunction contains
a fixed number $k$ of values of type $A$ and a value of a fixed
type $Z_{k}$. The computation \lstinline!traverse(f)!, where $f$
is a function of type $A\rightarrow F^{B}$, iterates over the $k$
values of type $A$, merging the $F$-effects in the natural order.
The constant value of type $Z_{k}$ is kept unchanged and lifted into
the final value of type $F^{Z_{k}\times B\times B\times...\times B}$.

If the traversal order needs to be chosen differently, which will
require a custom implementation of a \lstinline!Traversable! instance.
...
\emph{any} traversable functor must be polynomial, one needs advanced
...
{\color{greenunder}\text{naturality law of }\text{zip}_{F}:}\quad & =\gunderline{(\text{seq2}_{M}^{F,G^{A},G^{B}}\boxtimes\text{seq2}_{N}^{F,G^{A},G^{B}})\bef\big((\text{seq2}_{M}^{G,A,B})^{\uparrow F}\boxtimes(\text{seq2}_{N}^{G,A,B})^{\uparrow F}\big)}\bef\text{zip}_{F}\bef\text{zip}_{G}^{\uparrow F}\\
{\color{greenunder}\text{composition law~(\ref{eq:pair-product-composition-law})}:}\quad & =\big((\text{seq2}_{M}^{F,G^{A},G^{B}}\bef(\text{seq2}_{M}^{G,A,B})^{\uparrow F})\boxtimes(\text{seq2}_{N}^{F,G^{A},G^{B}}\bef(\text{seq2}_{N}^{G,A,B})^{\uparrow F})\big)\bef\text{zip}_{F}\bef\text{zip}_{G}^{\uparrow F}\quad.
\end{align*}
...
as:
\[
\text{seq2}_{S}:M^{F^{A},F^{B}}+N^{F^{A},F^{B}}\rightarrow F^{M^{A,B}+N^{A,B}}\quad,\quad\quad\text{seq2}_{S}\triangleq\,\begin{array}{|c||c|}
 & F^{M^{A,B}+N^{A,B}}\\
\hline M^{F^{A},F^{B}} & \text{seq2}_{M}^{F,A,B}\bef(m^{:M^{A,B}}\rightarrow m+\bbnum 0^{:N^{A,B}})^{\uparrow F}\\
N^{F^{A},F^{B}} & \text{seq2}_{N}^{F,A,B}\bef(n^{:N^{A,B}}\rightarrow\bbnum 0^{:M^{A,B}}+n)^{\uparrow F}
\end{array}\quad.
\]
...
\end{align*}
Now simplify the left-hand side using the composition laws of $\text{seq2}_{M}$
and $\text{seq2}_{N}$:
\begin{align*}
...
given by a \textbf{3-functor} \index{3-functor}$T^{A,B,R}$ (a type
constructor covariant in each of its three type parameters). In order
to obtain the traversable property of $S$, we need to assume that
$T$ is traversable with respect to its three type parameters. Such
recursion schemes $T$ are called 3-traversable. More rigorously,
a 3-functor $T^{A,B,R}$ is \textbf{3-traversable}\index{3-traversable 3-functor}
if it has a \lstinline!sequence3! method:
\[
...
become longer but will be the same as for bitraversable bifunctors,
...
Given a lawful traversable functor $L$, show that $\text{seq}_{L}^{F,A}=\text{id}$
if we choose $F^{A}\triangleq A$ (the identity functor).
...
\begin{lstlisting}
def seq[F[_]: Zippable : Functor, A]: ((F[A], F[A])) => F[(A, A)] = {
  case (fa1, fa2) => (fa2 zip fa1) map { case (x, y) => (y, x) }
}
\end{lstlisting}
...
Show that the identity law is \emph{not} satisfied by the implementation
...
Prove that $L^{A}\triangleq M^{N^{A}}$ is traversable if $M$ and
$N$ are traversable.
...
Given a monad $M$ and a monoid morphism $\phi:R\rightarrow S$ between
some monoid types $R$ and $S$, prove that $\phi^{\uparrow M}:M^{R}\rightarrow M^{S}$
...
found some uses for applicative profunctors. We will now briefly investigate
whether contrafunctors or profunctors may be traversable. To answer
that question, we will try implementing a lawful \lstinline!sequence!
...
we may lift this morphism to transform $\text{seq}_{L}^{\text{Id},A}$
into $\text{seq}_{L}^{F,A}$ for \emph{any} applicative functor $F$:
\[
...
So, we have found the code for $\text{seq}_{L}$:
\[
\text{seq}_{L}^{F,A}=\text{pu}_{F}^{\downarrow L}\bef\text{pu}_{F}\quad.
\]
In this way, the implementation of $\text{seq}_{L}$ follows unambiguously
from applicative naturality.
...
However, the code of $\text{seq}_{L}$ always produces values with
empty $F$-effects. Any information about nontrivial $F$-effects
...
nested type expression ($A\times A$) instead of just $A$. To express
the type equation~(\ref{eq:perfect-shaped-binary-tree-type-equation})
...
various naturality laws, which allow us to change the order of composition
of lifted functions:
\begin{align*}
...
{\color{greenunder}\text{naturality law of }\text{seq2}_{S}:}\quad & =\big(\overline{\text{seq}}_{L}^{F,P^{G^{A}}}\big)^{\uparrow S}\bef\big((\text{seq}_{P}^{G,A})^{\uparrow L}\bef\overline{\text{seq}}_{L}^{G,P^{A}}\big)^{\uparrow F\uparrow S}\bef\text{seq2}_{S}^{F,G^{A},G^{L^{P^{A}}}}\\
{\color{greenunder}\text{function composition}:}\quad & =\big(\gunderline{\overline{\text{seq}}_{L}^{F,P^{G^{A}}}\bef(\text{seq}_{P}^{G,A})^{\uparrow L\uparrow F}}\big)^{\uparrow S}\bef\big(\overline{\text{seq}}_{L}^{G,P^{A}}\big)^{\uparrow F\uparrow S}\bef\text{seq2}_{S}^{F,G^{A},G^{L^{P^{A}}}}\quad.\\
{\color{greenunder}\text{naturality law of }\overline{\text{seq}}_{L}:}\quad & =\big(\gunderline{\overline{\text{seq}}_{L}^{F,P^{G^{A}}}\bef(\text{seq}_{P}^{G,A})^{\uparrow L\uparrow F}}\big)^{\uparrow S}\bef\big(\overline{\text{seq}}_{L}^{G,P^{A}}\big)^{\uparrow F\uparrow S}\bef\text{seq2}_{S}^{F,G^{A},G^{L^{P^{A}}}}
\end{align*}
...
see \texttt{\href{https://www.cs.ox.ac.uk/ralf.hinze/publications/WAAAPL99a.ps.gz}{https://www.cs.ox.ac.uk/ralf.hinze/publications/WAAAPL99a.ps.gz}}} with elements of type $A$. For motivation, recall that Example~\ref{subsec:Example-matrix-products}
...
as the type of matrices of size \emph{at least} $N$. We intend the
type parameter $N$ to be one of the types $\bbnum 1$, $\bbnum 2$,
etc., although this will not be enforced by the type \lstinline!SqSize!.
The base case and the inductive step are written as:
\[
...
constructors in front of a \lstinline!Matrix! constructor.

Let us now implement a \lstinline!sequence! function for \lstinline!Sq!.
Since \lstinline!Sq! is defined by induction using \lstinline!SqSize[N, A]!,
we must first implement sequence for \lstinline!SqSize!:
\begin{lstlisting}
...
we can do these computations is by enumerating all possible values
of type $N$.
...
constructors to be set to \lstinline!Unit! wrapped in a number of
\lstinline!Option! constructors. All values of a type of this form
can be enumerated explicitly. However, the type \lstinline!SqSize[N, A]!
does not speficy that the type parameter \lstinline!N! will be an
enumerable type. This is a problem our current code for \lstinline!Sq!,
which prevents us from implementing the \lstinline!sequence! method.

To resolve this problem, we add a typeclass constraint on the type
parameter \lstinline!N!. Let us call the typeclass \textsf{``}\lstinline!Finite!\textsf{''}.
A suitable typeclass instance of type \lstinline!Finite[N]! simply
contains a list of all values of type \lstinline!N!:
\begin{lstlisting}
type Finite[N] = List[N] // A list of all possible values of type N.
\end{lstlisting}
Typeclass instances are created automatically for \lstinline!Finite[Unit]!,
\lstinline!Finite[Option[Unit]]!, \lstinline!Finite[Option[Option[Unit]]]!
and so on:
\begin{lstlisting}
implicit val finiteUnit: Finite[Unit] = List(())
implicit def finiteOUnit[N: Finite]: Finite[Option[N]] = None +: Finite[N].map(Some(_))
\end{lstlisting}
Using these typeclass instances, we can now extract all values of
type $A$ from a value of type $N\times N\rightarrow A$:
\begin{lstlisting}
...
// We use this function to avoid requiring F\textsf{'}s `pure` method, which we do not need below.
...
def sequence[N: Finite, F[_] : Zippable : Functor, A](sq: SqSize[N, F[A]]): F[SqSize[N, A]] =
  sq match {
    case Matrix(byIndex)   =>
...
Figures~\ref{fig:Full-code-implementing-traverse-for-square-matrix}
and~\ref{fig:Full-code-implementing-traverse-for-square-matrix-tests}
show the complete code of \lstinline!sequence! for \lstinline!Sq!.
To test this code, we define a value \lstinline!matrix2x2List! that
represents a square matrix of lists:
\[
\text{matrix2x2List}\triangleq\left|\begin{array}{cc}
...
\end{lstlisting}
\par\end{centering}
\caption{Tests for the \lstinline!sequence! method for square matrices.\label{fig:Full-code-implementing-traverse-for-square-matrix-tests}}
\end{figure}
