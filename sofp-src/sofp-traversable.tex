...
To guarantee that unfolding always terminates, we could use a data
structure that delays the full evaluation of some of its parts. Such
data structures can simulate \textsf{``}infinite\textsf{''} lists or trees because
further data items are evaluated only when requested.
...
sealed trait UT[A]        // A tree of unbounded size.
...
let us write a function that converts \lstinline!UT[A]! to \lstinline!T2[A]!
...
scala> toT2(maxDepth = 3, default = -1)
res0: T2[Int] = Branch(Leaf(1), Branch(Leaf(2), Branch(Leaf(3), Leaf(-1))))
\end{lstlisting}
...
\lstinline!t2! defined earlier in Sections~\ref{subsec:Decorating-a-tree-breadth-first-traversal}
...
\subsection{Equivalence of \texttt{foldMap} and \texttt{reduce}}

\subsection{Equivalence of \texttt{reduce} and \texttt{toList}}

\subsection{The difficulty with laws of \texttt{foldMap} and \texttt{reduce}}
...
Consider the data type \lstinline!T3[A]! defined in Exercise~\ref{subsec:Exercise-applicative-I-1-1}.
For that data type, define a recursion scheme and implement a specialized
version of \lstinline!unfold! as \lstinline!unfoldT3!. Using \lstinline!unfoldT3!,
write a function that generates ternary trees of the form {\tiny{}}{\tiny{} \Tree[.3  0  [.2 0   1   0  ]   0  ] }
...
Implement a \lstinline!Traversable! instance for a \textsf{``}rose tree\textsf{''}
$T^{A}\triangleq A+\text{List}^{T^{A}}$. Represent $T^{A}$ via a
suitable bifunctor $S^{X,Y}$ and show that $S$ is bitraversable
(use constructions). {*}{*}{*} Did we already have an Example with
this solved?
