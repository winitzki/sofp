...
does not specify that the type parameter \lstinline!N! will be a
type with a known finite number of values. This is a problem our current
code for \lstinline!Sq!, which prevents us from implementing the
\lstinline!sequence! method.
...
suitable typeclass instance of type \lstinline!Finite[N]! contains
a list of all values of type \lstinline!N!:
\begin{lstlisting}
type Finite[N] = List[N] // A list of all possible values of type N.
\end{lstlisting}
...
\lstinline!N!: namely, the types \lstinline!Finite[Unit]!, \lstinline!Finite[Option[Unit]]!,
\lstinline!Finite[Option[Option[Unit]]]! and so on:
\begin{lstlisting}
implicit val finiteUnit: Finite[Unit] = List(())
implicit def finiteOUnit[N: Finite]: Finite[Option[N]] = None +: Finite[N].map(Some(_))
\end{lstlisting}
