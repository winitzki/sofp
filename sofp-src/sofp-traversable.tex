...
given only a current value of type $Z$. It is clear that we need
to keep track of the last used power of $2$. So, let us choose $Z\triangleq\text{Long}$
for that value; the type $A$ is the same. The function $f$ should
return \lstinline!None! when the list is finished:
\begin{lstlisting}
...
The code of \lstinline!unfoldT2! is an adaptation of the general
code in Eq.~(\ref{eq:unfold-via-recursion-scheme}):
\begin{lstlisting}
type S[A, R] = Either[A, (R, R)]
...
procedure can create the required tree. 
