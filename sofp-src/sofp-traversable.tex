...
leaf values of type \lstinline!A! in the breadth-first order, a traversal
requires to sequence the effects of an arbitrary applicative functor
\lstinline!F[_]! in that order while reproducing the original tree
structure wrapped under \lstinline!F!. The function \lstinline!toListBFS!
shown in the previous section is not sufficient for that purpose,
because the tree structure cannot be reproduced if we only have a
list of leaf values. We need to keep additional information about
the tree position of each leaf value.

The plan of {*}{*}{*}

Note that the implementation of \lstinline!traverse! is the only
difference between the breadth-first code and the depth-first code
shown in the previous section. The \textsf{``}decoration logic\textsf{''} is described
by the function \lstinline!computeIndex! and does not depend on the
...
\subsection{Implementing \texttt{scanLeft} and \texttt{scanMap} via \texttt{traverse}}
