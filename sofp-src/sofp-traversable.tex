...
We have seen many examples of using \lstinline!foldLeft! and \lstinline!reduce!
in Chapter~\ref{chap:2-Mathematical-induction}. Now we would like
to generalize these functions to data structures other than sequences.
Such a generalization must be based on requiring suitable laws, but
it is not obvious what laws \lstinline!foldLeft! and \lstinline!reduce!
must satisfy (other than naturality laws that will hold automatically
for any fully parametric code). To make progress, we need to simplify
these type signatures and to reformulate them in a more suggestive
form. We will achieve this reformulation in two steps, generalizing
\lstinline!foldLeft! first to a new function called \lstinline!foldMap!
and then to \lstinline!traverse!. 
...
The standard Scala method to use in this situation is called \lstinline!Future.traverse!:
\begin{lstlisting}
val data: List[A] = ...
...
which is specific to the \lstinline!Future! data type, and if we
flip the first two curried arguments, we will arrive at the following
type signature of \lstinline!traverse! (short notation \textsf{``}$\text{trav}$\textsf{''}):
\[
...
as appropriate. To see that \lstinline!foldMap! is a special case
of \lstinline!traverse!, consider a constant functor $F^{A}\triangleq M$,
...
\section{Practical use of \texttt{traverse}}
...
Depth labeling of a tree as {\footnotesize{}} \Tree[ [ 2 [ 3 3 ] ] 1 ] {\footnotesize\par}
