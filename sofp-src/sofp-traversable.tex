...
new function we call \lstinline!foldFn! (\textsf{``}fold with function\textsf{''}):
\[
\text{foldFn}:L^{B\rightarrow B}\rightarrow B\rightarrow B\quad,
\]
assuming that \lstinline!foldLeft! satisfies the naturality law with
respect to its type parameter $A$.

\textbf{(c)} The functions \lstinline!foldFn! and \lstinline!reduceE!
are equivalent as long as \lstinline!foldFn! obeys the laws~(\ref{eq:foldFn-transformation-law})
...
below that the laws~(\ref{eq:foldFn-transformation-law}) and~(\ref{eq:foldFn-second-special-law})
follow from parametricity. So, these laws will hold automatically
as long as the code of \lstinline!foldFn! is fully parametric. However,
formulating these laws allows us to prove the equivalence of \lstinline!foldFn!
and \lstinline!reduceE! without assuming parametricity.
...
law~(\ref{eq:monoidal-naturality-law-of-reduceE}) of \lstinline!reduceE!
with monoids $M$ and $N\triangleq\text{MF}^{M}$:
\begin{equation}
...
\begin{align*}
{\color{greenunder}\text{left-hand side}:}\quad & p^{:L^{M}}\triangleright\text{reduceE}^{M}\bef f^{:M\rightarrow N}=f(\text{foldFn}\,(p\triangleright\text{inMF}^{\uparrow L})(e_{M}))\quad,\\
...
\begin{equation}
f\big(e_{M}\triangleright(p\triangleright\text{inMF}^{\uparrow L}\triangleright\text{foldFn})\big)=e_{N}\triangleright\big(p\triangleright f^{\uparrow L}\triangleright\text{inMF}^{\uparrow L}\triangleright\text{foldFn}\big)\quad.\label{eq:foldFn-second-special-law}
\end{equation}
Since we have already assumed that both laws~(\ref{eq:foldFn-transformation-law})
and~(\ref{eq:foldFn-second-special-law}) hold, we have demonstrated
that the equivalence between \lstinline!reduceE! and \lstinline!foldFn!
...
will always satisfy the laws~(\ref{eq:foldFn-transformation-law})
and~(\ref{eq:foldFn-second-special-law}). Begin with the first law:
\begin{align*}
...
Let us clarify the meaning of the special laws~(\ref{eq:foldFn-transformation-law})
...
The laws~(\ref{eq:foldFn-transformation-law}) and~\ref{eq:foldFn-second-special-law}
...
\begin{align}
 & \phi\,(p^{:L^{E^{A}}})=\phi\,(p\triangleright\text{inF}^{\uparrow L})(\text{id}^{:A\rightarrow A})\quad,\label{eq:transformation-law-of-g}\\
...
\textbf{(b)} Any fully parametric function $\phi:\forall A.\,L^{E^{A}}\rightarrow E^{A}$
{*}{*}{*}
...
\begin{comment}
{*}{*}{*}Begin by formulating the relational naturality law of $\phi$:
for all $r^{:A\leftrightarrow B}$,
\[
...
\begin{equation}
\text{if }x\triangleright(h^{:A\rightarrow A}\rightarrow f\bef h)^{\uparrow L}=y\triangleright(g^{:B\rightarrow B}\rightarrow g\bef f)^{\uparrow L}\text{ then }f\bef\phi(x)=\phi(y)\bef f\quad.\label{eq:strong-dinaturality-law-of-g-with-f}
\end{equation}
...
able to derive Eq.~(\ref{eq:transformation-law-of-g}). After trying
out a few possibilities, one finds that a good choice is $B\triangleq A\rightarrow A$
...
for which the law~(\ref{eq:strong-dinaturality-law-of-g-with-f})
holds. 

We will now show that Eq.~(\ref{eq:transformation-law-of-g}) follows
from Eq.~(\ref{eq:strong-dinaturality-law-of-g-with-f}) with this
choice of $f$. The value $a_{0}$ will be held fixed throughout most
of this proof, so we will write just $f$ instead of a more detailed
notation such as $f_{a_{0}}$.

In order to use Eq.~(\ref{eq:strong-dinaturality-law-of-g-with-f})
with this choice of $f$, we need to find suitable values $x$ and
$y$. Note that the conclusion of Eq.~(\ref{eq:strong-dinaturality-law-of-g-with-f})
...
This choice would work with Eq.~(\ref{eq:strong-dinaturality-law-of-g-with-f})
...
Since the precondition of Eq.~(\ref{eq:strong-dinaturality-law-of-g-with-f})
...
\]
In Eq.~(\ref{eq:transformation-law-of-g}), the function $\phi(p\triangleright\text{inF}^{\uparrow L})$
...
{\color{greenunder}\text{since this holds for any }a_{0}:}\quad & \phi(p)\overset{!}{=}\phi(p\triangleright\text{inF}^{\uparrow L})(\text{id})\quad.
\end{align*}
This shows that Eq.~(\ref{eq:transformation-law-of-g}) holds.
