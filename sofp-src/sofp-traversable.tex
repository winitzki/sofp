...
we obtain a complete understanding of the \lstinline!map!/\lstinline!reduce!
programming.
...
\subsection{The \texttt{traverse} operation}

It is often necessary to wait for several concurrent operations to
finish. As a simple example, assume that we are given a list of type
\lstinline!List[A]! and a processing function of type \lstinline!A => Future[B]!.
We are required to apply the processing function to all items from
the list in parallel and wait until the entire data set is processed.
...
is called \index{traversable}\textbf{traversable}.
...
which is applicative if $M$ is a monoid (see Section~\ref{subsec:Constructions-of-applicative-functors}).
...
\section{Practical use of \texttt{foldMap} and \texttt{traverse}}
...
Implement a \lstinline!traverse! operation for the binary tree \lstinline!Tree2!
(Section~\ref{subsec:Binary-trees}).
...
\begin{lstlisting}
def trav[A, B, F[_]: Functor : Applicative](f: A => F[B])(t: Tree2[A]): F[Tree2[B]] = t match {
  case Leaf(a)          => f(a).map(b => Leaf(b))                    // Reproduce the Leaf structure.
  case Branch(t1, t2)   =>
    val (r1, r2) = (trav(f)(t1), trav(f)(t2))    // Traverse the two branches and obtain two results.
    F.zip(r1, f2).map { case (b1, b2)   => Branch(b1, b2) }        // Reproduce the Branch structure.
}
\end{lstlisting}
...
this implementation represents a depth-first traversal of the tree.
$\square$

These examples show how one would implement the \lstinline!traverse!
operation for a data structure of type $L^{A}$. Each element of type
$A$ contained in the data structure should be processed using the
given function $f:A\rightarrow F^{B}$. All of the resulting $F$-effects
need to be collected and merged (using $F$\textsf{'}s \lstinline!zip!) into
a single $F$-effect that wraps a value of type $L^{B}$. That value
should (as closely as possible) repeat the data structure that was
present in the input value of type $L^{A}$. 

In all examples above, we all $F$-effects together, making sure that
each $F$-effect is collected exactly once. It would be strange if
the \lstinline!traverse! operation were to repeat some $F$-effects,
...
\end{lstlisting}
Although this implementation fits the right type signature, it contradicts
the intuition of traversing the sequence only once. When $F=$\lstinline!Future!,
the code \lstinline!badtrav(f)(xs)! would start \emph{two} parallel
computations for each element of the sequence \lstinline!xs! and
ignore one of the results. When $F$\textsf{'}s effect describes parsing (see
...
\subsection{Aggregating tree-like data by folding. Breadth-first traversal}

Although \lstinline!foldMap! (and, equivalently, \lstinline!foldLeft!)
are special cases of \lstinline!traverse!, it is often easier to
implement \lstinline!foldMap! directly. Let us look at some examples
of \lstinline!foldMap! that implements different kinds of tree traversals.

We will use the binary tree data type denoted by \lstinline!T2! for
brevity:
\begin{lstlisting}
sealed trait T2[A]
...
One implementation of \lstinline!foldMap! corresponds to the traverse
...
the data items stored inside it. Once the data is extracted into a
list, we may use standard aggregation operations on lists (\lstinline!foldLeft!,
...
It is important that the function \lstinline!toList! is free to choose
the order in which it writes the data from \lstinline!L! to a list.
Some data structures have a \textsf{``}natural\textsf{''} order of traversal that
is easiest to implement, but this is by no means the only possible
traversal order. Let us see some examples of implementing different
traversal orders for lists and trees.
...
to a tree of the form  \Tree[ 8 [ [ 3 5 ] 4 ] ] :
\begin{lstlisting}
...
a breadth-first traversal of binary trees. The idea is to prepare
a list of all values at level $0$ in the tree, then a list of all
values at level $1$, and so on. For each level, values must be collected
left to right. For instance, a breadth-first traversal of the tree
\lstinline!t2! has no values at level $0$, value $8$ at level $1$,
value $4$ at level $2$, and values $3$ and $5$ at level $3$.
So, the first step is to implement a function (say, \lstinline!toList2!)
to compute the list of nested lists of the form $\left[\left[\right],\left[8\right],\left[4\right],\left[3,5\right]\right]$
for this tree, enumerating all leaf values level by level. Then we
can simply \lstinline!flatten! that list and obtain the breadth-first
traversal $\left[8,4,3,5\right]$. 
...
\subsection{Decorating a tree with depth-first and breadth-first traversals{*}{*}{*}}

\subsection{Using \texttt{traverse} and the \texttt{State} monad}

Implement scanMap and scanLeft

\subsection{Traversing a regular-shaped binary tree}
...
\subsection{Examples of tasks that \emph{cannot} be implemented as a traversal}

Breadth-first traversal for a tree as  \Tree[ [ 2 [ 3 4 ] ] 1 ] 
- is this correct, and if so, how can we see that?

Depth labeling of a tree as  \Tree[ [ 2 [ 3 3 ] ] 1 ]  - can we still
do this using some kind of trick?
...
by M.~Jaskelioff\index{Mauro Jaskelioff} (2012) argues that \texttt{\textcolor{blue}{\footnotesize{}traverse}}
...
is called a \textbf{monoid morphism}\index{monoid morphism} if $\phi$
