...
values of type $Z$. The programmer must use \lstinline!unfold(f)!
with a function $f$ that is carefully designed to stop the recursion
...
of infinite size, which will fail. 
...
\begin{align*}
 & p^{:L^{M}}\triangleright\text{reduceE}^{M}\bef f^{:M\rightarrow N}=f(\text{foldFn}\,(p\triangleright\text{inMF}^{\uparrow L})(e_{M}))\\
 & \text{reduceE}^{M}\bef f^{:M\rightarrow N}=f^{\uparrow L}\bef\text{reduceE}^{N}***\\
 & p^{:L^{M}}\triangleright f^{\uparrow L}\bef\text{reduceE}^{N}=\text{foldFn}\,(p\triangleright f^{\uparrow L}\triangleright\text{inMF}^{\uparrow L})(e_{N})
\end{align*}
...
$T$ is traversable with respect to all its type parameters at once,
which we call \textsf{``}3-traversable\textsf{''}. More rigorously, a 3-functor $T^{A,B,R}$
is \textbf{3-traversable}\index{3-traversable 3-functor} if it has
a \lstinline!sequence3! method:
\[
