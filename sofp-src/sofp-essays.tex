...
writing equations in a mathematical notation. Once the applicative
...
for practical programming. Those courses answer questions that computer
science theorists have, not questions that practicing functional programmers
have.
...
as reasoning about and implementing quantified types, types parameterized
by type constructors, or partial type-level functions (known as \textsf{``}typeclasses\textsf{''}).
...
recursive type definition is \lstinline!class Bad(x: Bad)!. A small
change transforms this example into a \textsf{``}valid\textsf{''} recursive type:
\lstinline!class Good(x: Option[Good])!.
