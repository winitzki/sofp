...
\chapter{Essay: Why category theory is useful in computer programming}
...
a subcategory of these categories.
...
\section{The meaning of endofunctors}
...
(usually called \lstinline!fmap! in functional programming libraries):
\begin{lstlisting}
...
Why does \lstinline!fmap! satisfy the laws of endofunctors? The properties
of function composition are preserved if we apply functions element
by element in an array and collect the results \emph{in the same order}.
So, for instance, an identity function applied to every element will
not modify the array.

The same construction can be applied to several data structures other
...
are implemented in a standard library. The programmer\textsf{'}s intuition
about \textsf{``}applying functions to every value held within a data structure\textsf{''}
is then directly represented by the formal laws of endofunctors. Once
these laws are verified, the programmer is assured that the code written
via \lstinline!fmap! will work according to the programmer\textsf{'}s intuitive
expectations.

\section{The meaning of natural transformations between endofunctors}
...
Generally, naturality laws are satisfied for transformations that
may rearrange data items in a data structure but do not examine any
values or any specific types. In this way, programmer\textsf{'}s intuitions
about \textsf{``}code that works in the same way for all types\textsf{''} are directly
...
Finally, disjunctive types in the programming language correspond
to co-products in the \textsf{``}types/functions\textsf{''} category.

In this way, we find that various useful properties of the programming
language\textsf{'}s type system correspond to certain well-known properties
of the \textsf{``}types/functions\textsf{''} category. Most modern FP languages correspond
to categories that support initial and terminal objects, finite products
and co-products, and exponentials.
