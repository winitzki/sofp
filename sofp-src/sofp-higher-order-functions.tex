...
\index{saturated application}\textbf{saturated} application. A saturated
application returns a value that is not a function and cannot be applied
to more arguments.
...
\lstinline!c! of type \lstinline!C! and returns \emph{a new function}
that always returns \lstinline!c!:
\begin{lstlisting}
...
To make the backward notation easier to work with, one could write\footnote{As in the book \textsf{``}Program design by calculation\textsf{''}, see \texttt{\href{http://www4.di.uminho.pt/~jno/ps/pdbc.pdf}{http://www4.di.uminho.pt/$\sim$jno/ps/pdbc.pdf}}}
...
holds, without need to examine the code of \lstinline!fid!. In this
book, we will always formulate the desired properties through equations
...
values for these symbols but using only the general properties of
functions. This is similar to symbolic calculations in polynomial
