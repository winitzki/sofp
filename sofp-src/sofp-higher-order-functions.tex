...
def p[...]:... = { f => f(2) }
\end{lstlisting}
\textbf{(b)} Could we choose the type parameter(s) in the expression
...
\textbf{(b)} The expression \lstinline!p(p)! uses \lstinline!p!
as an argument for itself. We begin by writing that expression with
unknown type parameters: \lstinline!p[A](p[B])!. Then we try to choose
\lstinline!A! and \lstinline!B! so that the types match in that
expression. Does the type of \lstinline!p[B]!, which is \lstinline!(Int => B) => B!,
