...
of a function by type parameters. The result is a ``fully parametric''
function.
...
is applied and then $f$. Using the symbol $\circ$ (pronounced ``after'')
...
parametric}'' to designate a certain restricted kind of generic functions.
...
To make the backward composition easier to work with, one could write\footnote{As in the book ``Program design by calculation'', see \texttt{\href{http://www4.di.uminho.pt/~jno/ps/pdbc.pdf}{http://www4.di.uminho.pt/$\sim$jno/ps/pdbc.pdf}}}
...
equations or ``laws''. To verify that a law holds, we will perform
...
type annotations when writing code; it makes the code longer but may
be helpful.
...
method\index{Newton's method} with the iteration function $f(x)=\frac{1}{2}\left(x+\frac{q}{x}\right)$.
...
treat these two $x$'s as two \emph{completely} \emph{different} variables
...
\item Function applications group stronger than infix operations, so $f\,x+y$
means $(f\,x)+y$, just as in mathematics ``$\cos x+y$'' groups
``$\cos x$'' stronger than the infix ``$+$'' operation.
\end{itemize}
...
This book will not use the ``operator syntax'' when reasoning about
...
\subsection{Deriving a function's code from its type\label{subsec:Deriving-a-function-s-code}}
...
It is remarkable that one can sometimes perform ``\index{code inference}code
inference'': derive a function's \emph{code} from the function's
