...
values it uses. One sometimes says that the function's body \textsf{``}closes
...
argument types, other than assuming that the arguments' types correctly
...
$\circ$ (pronounced \textsf{``}after\textsf{''}) for this operation, we can write
\begin{equation}
...
parametric}\textsf{''} to refer to a certain restricted kind of generic functions.
...
To make the backward composition easier to work with, people sometimes
write the function types in reverse as e.g.~$g^{:C\leftarrow B}\circ f^{:B\leftarrow A}$.
...
It is important that we are able to detect that \lstinline!fakeId!
...
this book, we will always formulate the desired properties of functions
through equations or \textsf{``}laws\textsf{''}. To prove the laws, we will perform
...
\textsf{``}higher-orderness\textsf{''} comes from the fact that the return value has
a function type. An equivalent computation can be performed by an
uncurried function that is not higher-order:
\begin{lstlisting}
...
a special \lstinline!@infix! annotation. Do not confuse Scala's \textsf{``}dotless\textsf{''}
...
inference\textsf{''}: derive a function's \emph{code} from the function's
