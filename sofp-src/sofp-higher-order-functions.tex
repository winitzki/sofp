...
This book uses the term ``fully parametric'' to refer to a certain
restricted kind of generic functions.
...
is an expanded form of the same function $f$.
...
notation $g\circ f$. If necessary, all equations can be converted
from one notation to the other by reversing the order of compositions,
\[
f\bef g=g\circ f
\]
...
the type signature of \lstinline!add_x! to the equivalent curried
type expression $\text{Int}\rightarrow\text{Int}\rightarrow\text{Int}$.
