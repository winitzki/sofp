...
a name for that value, we use the ``\lstinline!val!'' syntax:
\begin{lstlisting}
...
involve \emph{naming} the function as ``$f$''. Sometimes a function
does not really need a name, \textemdash{} say, if the function is
used only once. ``Nameless'' mathematical functions may be denoted
using the symbol $\rightarrow$ (pronounced ``maps to'') like this:
\[
...
\end{lstlisting}
This expression shows Scala's syntax for a \textbf{nameless function}\index{nameless function}.
Here, 
\begin{lstlisting}
n: Int
\end{lstlisting}
...
(1 to n).product
\end{lstlisting}
is the function's \textbf{body}. The function arrow (\lstinline!=>!)
separates the argument from the body.\footnote{In mathematics, the ``maps to'' symbol is $\mapsto$, but this book
...
}of compiled code that will actually run and evaluate the function's
...
and then used in the sub-expression ``$n\%k$''.
...
A method, such as \lstinline!map!, can be also used with a ``dotless''
(infix) syntax:
\begin{lstlisting}
scala> List(1, 2, 3) map func1
...
In all these cases, the predicate's argument, \lstinline!k!, must
...
for the Riemann's zeta function\index{Riemann's zeta function} $\zeta\left(4\right)$.
...
that ``normalizes'' the list: it finds the element having the largest
...
systematically, without guessing or ``debugging.'' Like mathematicians
...
rules of calculation. Converting formulas into code, functional programming
capitalizes on the power of these reasoning tools.
...
``translated into code'', building up the specific kind of applied
mathematical intuition, and getting familiar with mathematical concepts
adapted to a programmer's needs. The FP community has discovered a
number of specific programing idioms founded on mathematical principles
but driven by practical necessities of writing software. This book
explains the idioms of FP in detail, starting from code examples and
heuristic ideas, and gradually building up the techniques of rigorous
reasoning.
...
expressions. This technique can be productively used in any programming
...
$i$. And yet, no mathematics textbook uses loops or says ``now repeat
this formula ten times''. Indeed, it would be pointless to evaluate
a formula such as $x^{2}+x$ ten times, because the result of $x^{2}+x$
remains the same every time. It is also meaningless to ``repeat''
...
the \textbf{local scope}\index{local scope} of the function (that
is, within the function's body); the code contains no loops. This
is similar to the way mathematical texts would define Simpson's rule.
In other words, this code is written in the FP paradigm. Similar code
can be written in any programming language that supports nameless
...
In today's programming languages, we may directly write expressions
...
a ``map'') is created in Scala with this code:
\begin{lstlisting}
Map("a" -> 1, "b" -> 2, "c" -> 3)
\end{lstlisting}
...
Nameless functions are useful for the same reason as nameless values
of other types: they allow us to build larger programs from simpler
parts in a uniform way.
...
language called ``$\lambda$-calculus\index{\$@$\lambda$-calculus}''.
...
function,\index{anonymous function!see ``nameless functions''} function
expression, function literal, closure, \index{lambda-function!see ``nameless function''}lambda
function, lambda expression, or just a ``lambda''.
