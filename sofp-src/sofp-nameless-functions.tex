...
uses the symbol $\Rightarrow$ for symbolic calculations and \lstinline!=>!
for Scala code. However, when doing calculations \emph{by hand}, one
could write $\rightarrow$ instead of $\Rightarrow$ since it is faster.
Several programming languages, such as OCaml and Haskell, use the
symbols \lstinline!->! or the Unicode equivalent, $\rightarrow$,
...
\end{lstlisting}
Usually, short functions are defined inline, and longer functions
