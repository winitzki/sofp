...
using the symbol $\rightarrow$ (pronounced \textsf{``}maps to\textsf{''}) like this:\footnote{In mathematics, the standard symbol for \textsf{``}maps to\textsf{''} is $\mapsto$,
...
that function. But \lstinline!n! is a free variable (it is not defined
inside the function). If we wanted to rename \lstinline!n! in the
sub-expression \lstinline*z => n % z != 0*, we would also need to
change all the code that involves the variable \lstinline!n! \emph{outside}
that sub-expression, or else the program would become incorrect.
...
the argument \lstinline!x! or additional hidden arguments of the
function \lstinline!f!. In those cases, replacing the code \lstinline!x => f(x)!
...
and \texttt{}\lstinline!filter! because they cannot compute the
next element of a sequence depending on previous values.
...
\end{align*}
In this notation, the summation symbol $\sum_{0}^{x}$ does not mention
the name \textsf{``}$k$\textsf{''} but takes a function as an argument. Similarly,
...
\hline 
{\footnotesize{}ALGOL68} & {\footnotesize{}1968} & \texttt{\footnotesize{}}\lstinline!(INT k) INT: k + 1!\tabularnewline
\hline 
...
the nameless function $x\rightarrow x+1$ would be written as $\lambda x.~add~x~1$
