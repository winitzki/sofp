...
If a nameless function uses the argument several times, e.g., as in
this code:
\begin{lstlisting}
...
for the arrow after $k$. We do not need to specify the type \texttt{}\lstinline!Int!
...
like \lstinline!f(x)! but actually involves additional hidden arguments
of the function \lstinline!f!, or an implicit type conversion for
its argument \lstinline!x!. In those cases, replacing the code \lstinline!x => f(x)!
by \lstinline!f! will fail to compile. But these complications do
not arise when working with simple functions.}
...
code, the dots \emph{may} be at the beginning of line since the compiler
...
is, within the function\textsf{'}s body); the code contains no loops. This
is similar to the way mathematical texts would define Simpson\textsf{'}s rule.
...
integral calculus. Also, the letter $\lambda$ has no particular significance;
it plays a purely syntactic role in the $\lambda$-calculus. Practitioners
...
will be explained in Chapter~\ref{chap:Higher-order-functions}.} all functions are nameless and have a single argument. The letter
...
if it had a function $add$ for adding integers (but it does not).
