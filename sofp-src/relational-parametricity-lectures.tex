...
\item A complete proof of ``theorems for free'' in 7 steps
\begin{itemize}
\item Step 1: Deriving \texttt{\textcolor{blue}{\small{}fmap}} and \texttt{\textcolor{blue}{\small{}cmap}}
methods from types
\item Step 2: The commutativity law for bifunctors and profunctors
\item Step 3: Motivation for the relational formulation of naturality laws
\item Step 4: Definition of relational lifting (\texttt{\textcolor{blue}{\small{}rmap}})
\item Step 5: Properties of relational lifting
\item Step 6: Proof of the relational naturality law
\item Step 7: Deriving the wedge law from the relational naturality law
\end{itemize}
\item Applications of the parametricity theorem
\begin{itemize}
\item Yoneda identities
\end{itemize}
\item Advanced applications of the parametricity theorem
\begin{itemize}
\item Church encoding of recursive types
\item Simplifying types with universal quantifiers
\item Equivalence of \texttt{\textcolor{blue}{\small{}foldMap}} and \texttt{\textcolor{blue}{\small{}foldLeft}}
...
\begin{itemize}
\item Naturality laws for code that works in the same way for all types
\end{itemize}
\begin{lyxcode}
...
\end{lyxcode}
\begin{itemize}
\item Uniqueness properties for fully parametric functions
\begin{itemize}
...
A)}}{\small\par}
\end{itemize}
\item Type equivalence for universally quantified types
\begin{itemize}
...
=> A => F{[}A{]}}}{\small\par}
\end{itemize}
\item The type \texttt{\textcolor{blue}{\small{}{[}A{]} => (Option{[}(R,
A){]} => A) => A}} is equivalent to \texttt{\textcolor{blue}{\small{}List{[}R{]}}}{\small\par}
...
no external libraries or built-in types (so, no \texttt{\textcolor{blue}{\small{}IO}}-like
monads)
...
\begin{frame}{Step 1. Naturality laws require \texttt{map}}
...
\begin{itemize}
\item What is the \texttt{\textcolor{blue}{\small{}fmap}} function for a
...
=> G{[}A{]}}}~\\
\texttt{\textcolor{blue}{\small{}fmap\_F(f) = (k:~G{[}A{]} => H{[}A{]})
=> (gb:~G{[}B{]}) => fmap\_H(f)(k(cmap\_G(f)(gb))}}{\small\par}
...
\texttt{\textcolor{blue}{\small{}fmap\_F(f) = k => {[}X{]} => fmap\_G1(f)(k{[}X{]}))}} 
\end{enumerate}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item When \texttt{\textcolor{blue}{\small{}F{[}\_{]}}} is contravariant,
...
\texttt{\textcolor{blue}{\small{}cmap\_F(f) = (k:~G{[}B{]} => H{[}B{]})
=> (ga:~G{[}A{]}) => cmap\_H(f)(k(fmap\_G(f)(ga))}}{\small\par}
\item If \texttt{\textcolor{blue}{\small{}F{[}A{]} = G{[}F{[}A{]}, A{]}}}
...
\begin{itemize}
\item The type constructions for \texttt{\textcolor{blue}{\small{}fmap}}
and \texttt{\textcolor{blue}{\small{}cmap}} are the same except for
function types
...
\end{itemize}
\begin{lyxcode}
\textcolor{blue}{\small{}type~F{[}A,~B{]}~=~(A~=>~Either{[}A,~B{]},~(B~=>~A)~=>~A~=>~(A,~B))}{\small\par}
...
positions
\begin{itemize}
\item We can recognize this just by counting the function arrows
\end{itemize}
\item We can generate the code for \texttt{\textcolor{blue}{\small{}fmap}}
and \texttt{\textcolor{blue}{\small{}cmap}} mechanically, from types
...
\begin{frame}{Step 1. ``Invariant'' types and profunctors}
...
=> (A, A), (A, A) => A{]}}}{\small\par}
\item Define \texttt{\textcolor{blue}{\small{}type G{[}X, A{]} = Either{[}X
=> (A, A), (X, X) => A{]}}}{\small\par}
\item Then \texttt{\textcolor{blue}{\small{}F{[}A{]} = G{[}A, A{]}}} while
\texttt{\textcolor{blue}{\small{}G{[}X, A{]}}} is contravariant in
\texttt{\textcolor{blue}{\small{}X}} and covariant in \texttt{\textcolor{blue}{\small{}A}}.
Such \texttt{\textcolor{blue}{\small{}G{[}X, A{]}}} are called \textbf{profunctors}
...
\end{itemize}
\begin{lyxcode}
\textcolor{blue}{\small{}def~fmapG{[}X,~A,~B{]}:~(A~=>~B)~=>~G{[}X,~A{]}~=>~G{[}X,~B{]}}{\small\par}

\textcolor{blue}{\small{}def~cmapG{[}X,~Y,~A{]}:~(Y~=>~X)~=>~G{[}X,~A{]}~=>~G{[}Y,~A{]}}{\small\par}
\end{lyxcode}
\begin{itemize}
\item Then we can compose \texttt{\textcolor{blue}{\small{}cmapG}} and \texttt{\textcolor{blue}{\small{}fmapG}}
to get \texttt{\textcolor{blue}{\small{}xmapF}}:

\texttt{\textcolor{blue}{\small{}def xmapF{[}A, B{]}: (A => B) =>
(B => A) => G{[}A, A{]} => G{[}B, B{]} =}}{\small\par}

\texttt{\textcolor{blue}{\small{}~ ~f => g => cmapG{[}A, B, A{]}(g)
andThen fmapG{[}B, A, B{]}(f)}}{\small\par}
\item What if we compose in another order? Need a commutativity law:
\end{itemize}
\[
\xymatrix{\xyScaleY{2.8pc}\xyScaleX{5.5pc}\mathtt{G[A,A]}\ar[d]\sb(0.5){~\mathtt{fmapG(f)}}\ar[r]\sp(0.5){\mathtt{~\mathtt{cmapG(g)}}} & \mathtt{G[B,A]}\ar[d]\sp(0.5){\mathtt{~\mathtt{fmapG(f)}}}\\
\mathtt{G[A,B]}\ar[r]\sp(0.5){~\mathtt{cmapG(g)}} & \mathtt{G[B,B]}
}
\]
...
\begin{itemize}
\item \texttt{\textcolor{blue}{\small{}fmap}} and \texttt{\textcolor{blue}{\small{}cmap}}
need to satisfy two functor laws
\item Identity law: \texttt{\textcolor{blue}{\small{}fmap(identity) = identity}},
...
and lawful
\item It is precisely that code that we need to use in naturality laws
\item Consistency of \texttt{\textcolor{blue}{\small{}xmap}} requires to
have a commutativity law
\begin{itemize}
\item Commutativity laws are the subject of Step 2
\end{itemize}
\end{itemize}
\end{frame}
...
(1989)
\begin{itemize}
\item Those papers are outdated and also hard to understand 
\end{itemize}
\item There are few pedagogical tutorials on relational parametricity
\begin{itemize}
