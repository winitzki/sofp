...
\item For purely covariant or contravariant \texttt{\textcolor{blue}{\small{}F{[}A{]}}}
...
and \texttt{\textcolor{blue}{\small{}H}} are already defined
\end{itemize}
Actually, we need to define \texttt{\textcolor{blue}{\small{}rmap2}},
\texttt{\textcolor{blue}{\small{}rmap3}}, \texttt{\textcolor{blue}{\small{}rmap4}},
..., all at once
\end{frame}
...
for function types (\texttt{\textcolor{blue}{\small{}G{[}A{]} => H{[}A{]}}})
...
Some examples of using \texttt{\textcolor{blue}{\small{}rmap}} when
lifting relations to type constructors

Two main examples of relations generated by functions are the function
graph relation \texttt{\textcolor{blue}{\small{}graph(f)}} and the
pullback relation \texttt{\textcolor{blue}{\small{}pull(f, g)}} 

Consider the following type constructors of different complexity:
\begin{itemize}
...
\texttt{\textcolor{blue}{\small{}(fa, fb) in rmap\_H(graph(f))}} is:
\end{itemize}
when \texttt{\textcolor{blue}{\small{}(p, q) in rmap\_G(graph(f))}}
then \texttt{\textcolor{blue}{\small{}(fa(p), fb(q)) in graph(f)}}{\small\par}

equivalently: if \texttt{\textcolor{blue}{\small{}p andThen f == f
andThen q}} then \texttt{\textcolor{blue}{\small{}f(fa(p)) == fb(q)}}{\small\par}
...
\begin{itemize}
\item This happens when lifting to a sufficiently complicated type constructor
\end{itemize}
\end{itemize}
\end{frame}
...
q(z))}} and{*}{*}{*}:
\begin{itemize}
\item We have \texttt{\textcolor{blue}{\small{}rmap\_P(r) = pprod({*}{*}{*})}}{\small\par}
\end{itemize}
...
f)(t{[}B{]})}}{\small\par}
\begin{itemize}
\item We do not need to assume the commutativity law for \texttt{\textcolor{blue}{\small{}xmapP}}{\small\par}
\end{itemize}
\end{itemize}
\end{frame}
...
=> Y'}}, \texttt{\textcolor{blue}{\small{}t:~P{[}Y, A{]}}}, \texttt{\textcolor{blue}{\small{}t':~P{[}Y',
A'{]}}} such that \texttt{\textcolor{blue}{\small{}(f, f') in pfun(p,
q)}} and \texttt{\textcolor{blue}{\small{}(g, g') in pfun(r, s)}}
and

\texttt{\textcolor{blue}{\small{}(t, t') in rmap2\_P(s, p)}} we will
have:

\texttt{\textcolor{blue}{\small{}(xmapP(f, g)(t), xmapP(f', g')(t'))
in rmap2\_P(r, q)}}{\small\par}

We need to get an equation \texttt{\textcolor{blue}{\small{}xmapP(f,
...
\item For the case \texttt{\textcolor{blue}{\small{}P{[}X, A{]} = {[}Q{]}
=> G{[}X, A, Q{]}}}we need to assume full parametricity for values
...
With The relational naturality law holds for \texttt{\textcolor{blue}{\small{}xmapP}}:
...
\begin{itemize}
\item For type signatures \texttt{\textcolor{blue}{\small{}G{[}A{]} => H{[}A{]}}}where
both \texttt{\textcolor{blue}{\small{}G}} and \texttt{\textcolor{blue}{\small{}H}}
are covariant:
\end{itemize}
...
\begin{itemize}
\item This and related results are shown in ``\href{https://homepages.inf.ed.ac.uk/wadler/papers/free-rectypes/free-rectypes.txt}{Recursive types for free}''
\end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Advanced applications. II. Example of a quantified type}
\begin{itemize}
