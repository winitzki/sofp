...
This problem contains two open questions concerning the \lstinline!List!
monad\textsf{'}s transformer, 
\[
T_{\text{List}}^{M,A}\triangleq M^{L^{A}}\quad,\quad\quad L^{A}\triangleq\bbnum 1+A\times M^{L^{A}}\quad,
\]
restricted only to using \emph{monoid} types $A$. Normally, we cannot
implement fully parametric base runner $T_{\text{List}}^{M,A}\rightarrow M^{A}$
because we cannot have a runner $\text{List}^{A}\rightarrow A$. However,
for a monoid type $R$ with binary operation $\oplus_{R}$ and empty
element $e_{R}$, the standard \textsf{``}reduce\textsf{''} operation for a list of
type $\text{List}^{R}$ has the type signature $\text{List}^{R}\rightarrow R$:
\[
...
transformer $T_{\text{List}}$. The function \lstinline!brun! with
the type signature $M^{L^{R}}\rightarrow M^{R}$ aggregates all elements
...
\lstinline!brun! is also a \emph{monoid} morphism $M^{L^{A}}\rightarrow M^{A}$.) 
