...
implement computations that require selecting some elements from a
sequence and truncating a sequence once a certain condition is achieved.
These methods are examples of \textsf{``}transformations\textsf{''}.
...
such as $\sum$. So, functional programming enables us to code transformations
and aggregations without loops by using suitable operations.
...
\subsection{Proofs of program correctness via dependent types}

\subsection{Linear types and session types}
...
\item Implement a function \lstinline!norepeat! that removes consecutive
repetitions from sequences. For example:
\begin{lstlisting}
...
times $F$-effects have been collected?)

\subsubsection{Problem \label{par:Problem-monads-2}\ref{par:Problem-monads-2}}

Monad transformers are defined by different methods for each type
of monad. If someone comes up with a new type of monad, it is not
certain that the new monad\textsf{'}s transformer will be obtained through
one of the known methods. Can we prove that a monad transformer will
exist for every monad whose \lstinline!pure! and \lstinline!flatMap!
methods are implemented via fully parametric code? Given the code
for an arbitrary monad, can we derive an implementation of the corresponding
monad transformer?
...
a \lstinline!swap! function for certain monads $L$. Is this also
the case for any monad stacks built out of such monads? If each of
the monads $L_{1}$, $L_{2}$, ..., $L_{k}$ admits a transformer
defined via a lawful \lstinline!swap! function, will the monad $L_{1}\varangle L_{2}\varangle...\varangle L_{k}$
...
\textsf{``}Rigid functors\textsf{''} are\index{rigid functor} defined in Section~\ref{subsec:Rigid-functors}.
