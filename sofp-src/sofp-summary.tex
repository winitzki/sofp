...
Given a fully parametric contrafunctor $F^{A}$ that is not a constant
contrafunctor and does not explicitly use the void type\index{void type}
($\bbnum 0$) in its type expression, the type $\forall A.\,F^{A}\rightarrow A$
is void. Without the last condition, the contrafunctor $F^{A}\triangleq(\bbnum 0\rightarrow A)\rightarrow\bbnum 0$
...
($F^{A}$) \textsf{``}does not contain\textsf{''} values of type $A$.
