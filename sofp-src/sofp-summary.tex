...
\end{lstlisting}
\item Consider a typeclass called ``\lstinline!Splittable!'' for functors
...
for monad transformers' base runners (see page~\pageref{par:Open-question-monad-id-trans}). 
...
\end{align*}
where $T_{L}^{M}$ is the monad $L$'s transformer applied to the
foreign monad $M$.

The problem is to implement the monad $F$'s transformer for an arbitrary
...
is the \lstinline!List! monad's transformer without the outer layer
...
The continuation monad's operations \lstinline!reset! and \lstinline!shift!
are defined by
\begin{align*}
