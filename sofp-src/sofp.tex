...
\geometry{verbose,tmargin=1.15cm,bmargin=1.3cm,lmargin=2.2cm,rmargin=1.3cm,headsep=0.4cm,footskip=0.72cm}
...
\titlehead{{\footnotesize{}Source hash: 1e97089b41f33319f70a5ac53c3d4c43a93d991acc778265672c74132f8820f9}\\
{\footnotesize{}Git commit: 3a4adccfcc8daab46699e7ad8cb5a73dbfbacdfa}}
...
The source code may be also included as a ``file attachment'' \inputencoding{latin9}\lstinline!sofp-src.tar.bz2!\inputencoding{utf8}
within the PDF file. To extract, run the command \inputencoding{latin9}\lstinline!`pdftk sofp.pdf unpack_files output .`!\inputencoding{utf8}
and then \inputencoding{latin9}\lstinline!`tar jxvf sofp-src.tar.bz2`!\inputencoding{utf8}.
See the file \inputencoding{latin9}\lstinline!README.md!\inputencoding{utf8}
for compilation instructions.}
\lowertitleback{This book is a pedagogical in-depth tutorial and reference on the
functional programming paradigm as it is understood in the early 21$^{\text{st}}$
century. Starting from problems encountered in practical coding, the
book builds up the theoretical intuition, knowledge, and techniques
that functional programmers use for rigorous and formal reasoning
about types and code.\\
\\
The book's topics include working with collections; recursive functions
and types; the type and code constructions used in purely functional
programming; the Curry-Howard correspondence; laws, structural analysis,
and code for functors, monads, and other typeclasses; parametricity
theorems; and free type constructions.\\
\\
Long and difficult, yet boring explanations are logically developed
in excruciating detail and are accompanied by full step-by-step derivations
and solved examples tested in the Scala interpreter, as well as exercises
and discussions that further build upon each chapter's material.\\
\\
Beginners interested in functional programming will find tutorials
about working with the \inputencoding{latin9}\lstinline!map!\inputencoding{utf8}
/ \inputencoding{latin9}\lstinline!reduce!\inputencoding{utf8} programming
style, type parameters, disjunctive types, and higher-order functions.
For advanced readers, the book shows  the practical use of the Curry-Howard
correspondence; proves that the implementations of all the standard
monads (such as \inputencoding{latin9}\lstinline!State!\inputencoding{utf8}
or \inputencoding{latin9}\lstinline!Continuation!\inputencoding{utf8})
actually satisfy the monad laws; derives lawful instances of \inputencoding{latin9}\lstinline!Functor!\inputencoding{utf8}
and other typeclasses from types; defines monad transformers and the
free monad and proves that their implementations satisfy the necessary
properties (when they do so); explains the use of Yoneda identities
for reasoning about the “tagless final” or the “free functor” constructions;
and proves the parametricity theorems without unnecessary jargon.\\
\\
Readers should have a working knowledge of programming; e.g.\ be
able to write code that prints the number of words in a small text
file. The difficulty of mathematical derivations in this book is at
the level of high-school mathematics, similar to simplifying the expressions
\[
\frac{1}{x-2}-\frac{1}{x+2}\quad\text{ and }\quad\frac{d}{dx}\left((x+1)e^{-x}\right)\quad.
\]
\\
Sergei Winitzki received a Ph.D. in theoretical physics and, after
a career in academic research, currently works as a software engineer.
This is the fourth book written by him.}
...
be no space left, assuming the 720 page limit.
