...
chapter. For now, let us look at some possible uses of a general \lstinline!zip!
operation.

As a first example, consider the type \lstinline!Either[E, A]! where
the type \lstinline!E! is a semigroup with a combine operation (\lstinline!|+|!),
representing error messages that can be combined. We may implement
\lstinline!zip! like this:
\begin{lstlisting}
...
Sometimes, the \lstinline!zip! operation is defined even for type
constructors that are not covariant. Consider, for example, the following
type constructor:
...
\noindent The type $L^{A}$ represents a monoid typeclass instance
(Section~\ref{subsec:tc-Example-Monoids}), and its \lstinline!zip!
operation constructs a monoid instance for the $A\times B$ if $A$
and $B$ are monoids (the product construction in Section~\ref{subsec:Monoids-constructions}).
...
\noindent Note that Scala\textsf{'}s \lstinline!Future! will automatically
schedule and start the computations inside it. So, this program will
schedule the computation func2() only after first run the computation
\lstinline!func1()! on a separate thread,. The computations in func1(),
func2(), and func3() do not depend on the previously computed values
x and y. Nevertheless, a monadic program 

{*}{*}{*} independent effects.

\subsection{Programs that accumulate errors}
