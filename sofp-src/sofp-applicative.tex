...
constructors that are \emph{not} functors (i.e., not covariant in
the type parameter).

\subsubsection{Example \label{subsec:Example-applicative-profunctor}\ref{subsec:Example-applicative-profunctor}}

A \lstinline!zip! method can be implemented for the following type
constructor:
...
The type $L^{A}$ represents a monoid typeclass instance (Example~\ref{subsec:tc-Example-Monoids})
whose \lstinline!zip! operation constructs a monoid instance for
the $A\times B$ if $A$ and $B$ are monoids (the \textsf{``}products\textsf{''}
construction of Section~\ref{subsec:Monoids-constructions}). We
usually do not view typeclass instances as data to be manipulated.
...
\end{lstlisting}
Here, the result of \lstinline!map2! is used in further monadic computations.
...
\begin{lstlisting}
def zip[A, B, E: Semigroup](x: Either[E, A], y: Either[E, B]): Either[E, (A, B)] = (x, y) match {
  case (Left(e1), Left(e2))   => Left(e1 |+| e2)
...
\end{lstlisting}
In the code notation, this \lstinline!zip! function is written like
this:
\[
...
When both arguments of \lstinline!zip! have error messages, the code
could have dropped one of the errors and returned \lstinline!Left(e1)!,
...
and so on. For the purposes of illustration, let us make the types
of all elements the same, so that the type signatures become:
\[
...
def zipN[A](xs: List[Either[String, A]]): Either[String, List[A]] = xs match {
  case Nil            => Right(Nil)
  case head :: tail   => zip(head, zipN(tail)).map { case (h, t) => h :: t }
}
\end{lstlisting}
...
This expression can be made clearer if we implement infix syntax for
\lstinline!fmap! and \lstinline!ap!:
\begin{lstlisting}
...
def fmap4[A, B, C, D, E](f: A => B => C => D => E)(a: L[A])(b: L[B])(c: L[C])(d: L[D]): L[E] =
  f <@> a <*> b <*> c <*> d
\end{lstlisting}
Now it becomes clear how to implement the functions $\text{fmap}_{N}$
...
\subsection{Transposing a matrix in parallel via \lstinline!map2!}

$F^{A}\triangleq\text{List}^{A}$: transposing a matrix by using \texttt{\textcolor{blue}{\footnotesize{}map2}} 

\subsection{Applicative contrafunctors and applicative profunctors}
...
\subsection{Merging several \texttt{fold} operations into one}
...
\subsection{Parsing with applicative functors}
...
Write a zip function that defines an instance of the \texttt{\textcolor{blue}{\footnotesize{}Monoid}}
...
\texttt{\textcolor{blue}{\footnotesize{}Option{[}A{]}}}. Implement
\texttt{\textcolor{blue}{\footnotesize{}zip}} and \texttt{\textcolor{blue}{\footnotesize{}map2}}
for $R^{A}$.
...
The map2 and zip methods defined in this way will have the right type
signature and will satisfy the laws of applicative functors. However,
we may need to define the \lstinline!map2! method in a different
way, because defining \lstinline!map2! via \lstinline!flatMap! would
not give us the required functionality.{*}{*}{*} Examples are lists,
trees, and Either.
