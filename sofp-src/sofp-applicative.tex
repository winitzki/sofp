...
\section{Motivation for applicative functors}

Applicative functors are less often used than monads or filterable
functors, and are less well known. One motivation for applicative
functors comes from studying the properties of the \lstinline!zip!
operation.

\subsection{Generalizing the \texttt{zip} method to other types}
...
\]
Using this type signature, the \lstinline!zip! operation may be defined
for types that have no resemblance to collections. In order to ensure
that this generalization is meaningful and safe, we will establish
and verify the laws of the \lstinline!zip! operation later in this
chapter. For now, let us look at some examples of using a general
\lstinline!zip! operation.
...
\end{lstlisting}
Note that the type constructor $L$ is \emph{not} a monad (see Exercise~\ref{subsec:Exercise-1-monads-7-not-a-monad}).
$\square$
...
the $A\times B$ if $A$ and $B$ are monoids (the product construction
of Section~\ref{subsec:Monoids-constructions}).

In a programmer\textsf{'}s view, \textbf{applicative} type constructors are
those having a suitable \lstinline!zip! method.
...
\noindent Note that Scala\textsf{'}s \lstinline!Future! values represent computations
that have been already scheduled (on other threads). In this example,
the computations in \lstinline!func1()!, \lstinline!func2()!, and
\lstinline!func3()! do not depend on the previously computed values
(\lstinline!x! and \lstinline!y!). So, it would be desirable to
run those computations in parallel. However, the monadic program will
create a second \lstinline!Future! value and schedule the computation
\lstinline!func2()! only after the computation \lstinline!func1()!
is finished. Similarly, \lstinline!func3()! will not start until
\lstinline!func2()! is finished. Indeed, the functor block is translated
into \lstinline!flatMap! and \lstinline!map! methods:
...
\noindent The problem is that the \lstinline!flatMap! and \lstinline!map!
methods are general and must wait until previous values are computed.
The code of \lstinline!flatMap! and \lstinline!map! cannot use the
fact that the next computations are independent of the previous values.
We need a new function that will explicitly assume independence of
effects. A possible type signature for that function (combining three
computations that return \lstinline!Future[Int]!) is
\begin{lstlisting}
...
map3 is called, and evaluating a \lstinline!Future! will schedule
the computation immediately. For other type constructors, we will
implement \lstinline!map3! (and similar functions such as \lstinline!map2!,
\lstinline!map4!, etc.) later in this chapter.

It is not obvious that \lstinline!map2! is in any way related to
\lstinline!zip!. Once we study the properties of these functions
in more detail, we will find that defining \lstinline!map! and \lstinline!map2!
is equivalent to defining \lstinline!zip!, assuming that certain
laws hold for these functions.
...
To be specific, let us implement the computation \lstinline!(1 / 0) + (2 / 0)!
in the \textsf{``}safe arithmetic\textsf{''}: {*}{*}{*}

A monadic implementation (see Section~\ref{subsec:Pass/fail-monads})
will stop the computation after the first error. , the two errors
implement A monadic We have seen implementations of this 

{*}{*}{*}As a further example, consider the type \lstinline!Either[E, A]!
...
We may implement the \lstinline!zip! method that gathers all error
messages:
\begin{lstlisting}
def zip[A, B](x: Either[String, A], y: Either[String, B]): Either[String, (A, B)] = (x, y) match {
...
\end{lstlisting}
To write this function in the code notation, let us replace \lstinline!String!
by a monoid type $E$:
\[
...
Monads are inconvenient for expressing \emph{independent} effects
...
and / or do not depend on previous results. flatMap cannot express
independent effects even if we use flatMap(\_ => ...)
...
%
\end{minipage}\texttt{\textcolor{blue}{\footnotesize{}\hfill{}\medskip{}
}}\hrule Note that the map2 function defined via flatMap will have
the right type signature and will work. But sometimes we need a different
implementation, not equivalent to an implementation via flatMap.\texttt{\textcolor{blue}{\footnotesize{}\medskip{}
}}{\footnotesize\par}
