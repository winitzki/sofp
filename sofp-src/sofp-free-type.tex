...
quantifier}\index{existential quantifier}, which is denoted by $\exists$
...
with a specific (but arbitrarily chosen) type \inputencoding{latin9}\lstinline!A!\inputencoding{utf8}.
...
can determine if the type \inputencoding{latin9}\lstinline!A!\inputencoding{utf8}
...
via case classes with an extra type parameter, and via traits with
...
Note that the type parameter $C$ is \emph{bound} by the quantifier
and will not be visible outside the type expression $\exists C.\,L^{A,B,C}$.
The type parameters $A$ and $B$ remain free in $\exists C.\,L^{A,B,C}$.
An example of such a type is $\exists C.\,A\times C\times\left(C\rightarrow B\right)$.

To implement the type $\exists C.\,L^{A,B,C}$ via case classes, we
write code similar to this:\inputencoding{latin9}
\begin{lstlisting}
sealed trait L[A, B]
final case class InternalL[A, B, C](...) extends L[A, B]
\end{lstlisting}
...
\begin{lstlisting}
val p: L[Int, Int] = InternalL[Int, Int, String](...)
\end{lstlisting}
\inputencoding{utf8}
To implement the type $\exists C.\,L^{A,B,C}$ via a trait with a
type member, we write:\inputencoding{latin9}
\begin{lstlisting}
trait L[A, B] {
  type C.
  // Add any other code needed in L.
}
\end{lstlisting}
\inputencoding{utf8} Each time we create a value of type \inputencoding{latin9}\lstinline!L[A, B]!\inputencoding{utf8},
we will have to specify some type for \inputencoding{latin9}\lstinline!C!\inputencoding{utf8}.
However, that type will be hidden inside the definition of \inputencoding{latin9}\lstinline!L!\inputencoding{utf8}
and will not be accessible to external code:\inputencoding{latin9}
\begin{lstlisting}
val p: L[Int, Int] = new L[Int, Int] { type C = String }
