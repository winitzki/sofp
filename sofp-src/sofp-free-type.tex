...
with $P=F$. Such functions $f$ are called $F$-\textbf{algebra morphisms}.\index{$F$-algebra morphism}

The following statement shows that $F$-algebra morphisms form a category:

\subsubsection{Statement \label{subsec:Statement-category-of-P-algebras}}
...
\textbf{(c)} For any $P$-algebra $C$ that obeys all the $P$-typeclass
...
\textbf{(a)} If $T$ is a $P$-algebra that obeys all the laws of
$E^{\bullet}$ then: 
\[
...
all). We begin by generalizing the raw tree encoding from \inputencoding{latin9}\lstinline!Monoid!\inputencoding{utf8}
...
\textbf{(d)} It remains to prove the uniqueness property of the runner.
...
and values of type $T$. The monadic operation \inputencoding{latin9}\lstinline!flatten!\inputencoding{utf8}
...
As we will now show, the function \inputencoding{latin9}\lstinline!flatten!\inputencoding{utf8}
...
of \inputencoding{latin9}\lstinline!FPR[T]!\inputencoding{utf8}.
...
proved that the type \inputencoding{latin9}\lstinline!FPR!\inputencoding{utf8}
(the free monad on $P$) is a free $P$-typeclass encoding. It is
not accidental that \inputencoding{latin9}\lstinline!FPR!\inputencoding{utf8}
...
Setting $f=\text{pu}_{E}$ and $S=A$ in Eq.~(\ref{eq:naturality-law-of-runE-derivation2})
...
{\color{greenunder}\text{left identity law of }E:}\quad & =\text{ftn}_{E}\quad.
\end{align*}
$\square$
...
is based on the category theory notion of a \textsf{``}\index{monad algebra}monad
algebra\textsf{''}:

\subsubsection{Definition \label{subsec:Definition-monad-algebra}\ref{subsec:Definition-monad-algebra}}

A type $C$ is an $E$-\textbf{monad algebra} for a monad $E$ if
$C$ is an $E$-algebra with a structure map $s:E^{C}\rightarrow C$
...
\textbf{(b)} If $C$ is a $P$-algebra, we define $s:E^{C}\rightarrow C$
by:
\[
s:E^{C}\rightarrow C\quad,\quad\quad s\triangleq\text{run}_{E}(\text{id}^{:C\rightarrow C})\quad.
\]
This definition is valid because $C$ satisfies all the laws of $E$,
and so we obtain $s$ from Definition~\ref{subsec:Definition-free-P-typeclass-encoding}(c).
By the same definition, $s$ is a $P$-algebra morphism. It remains
to verify that $s$ obeys the monad algebra\textsf{'}s laws:
\begin{align*}
...
The identity law is verified by using Eq.~(\ref{eq:free-typeclass-encoding-identity-law})
...
and back. Now we need to show that the mapping is an isomorphism in
both directions.
...
In category theory, an $F$-algebra $X$ is called \textbf{initial}\index{$F$-algebra!initial}
...
that the type \inputencoding{latin9}\lstinline!FPR!\inputencoding{utf8}
plays an important role in the description of $P$-typeclasses with
laws. Since \inputencoding{latin9}\lstinline!FPR!\inputencoding{utf8}
...
satisfy some laws will be, in a precise sense, subsets of \inputencoding{latin9}\lstinline!FPR!\inputencoding{utf8}.
...
??? Not sure this can be proved as stated.
...
although it may \emph{not} be a $Q_{T}$-algebra morphism.
...
In that case, we find that the functions $\text{in}_{E}^{T}$ and
$r_{\text{FPR}}^{T,E^{T}}$ are inverses of each other and give an
\emph{isomorphism} between $E^{T}$ and $\text{FPR}^{T}$.

We will now show that $E$ satisfies the conditions of Statement~\ref{subsec:Definition-free-P-typeclass-encoding-simplified}.
