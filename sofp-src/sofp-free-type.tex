...
(and obeys the relevant laws) and for any type $T$ and any function
$r:T\rightarrow C$, there exists a function $\text{run}_{E}^{T,C}(r):E^{T}\rightarrow C$.
The \textsf{``}universal runner\textsf{''} ($\text{run}_{E}^{T,C}$) satisfies:
\begin{align*}
...
{\color{greenunder}P\text{-typeclass morphism law}:}\quad & p_{E}^{T}\bef\text{run}_{E}^{T,C}(r)=\big(\text{run}_{E}^{T,C}(r)\big)^{\uparrow P}\bef p_{C}^{T}\quad,\\
{\color{greenunder}P\text{-typeclass naturality law in }C:}\quad & \text{for any }P\text{-typeclass morphism }g^{:C\rightarrow D}:\\
 & \quad\quad\text{run}_{E}^{T,D}(r\bef g)=\text{run}_{E}^{T,C}(r)\bef g\quad.
\end{align*}
Additionally, the following uniqueness property must hold: any $P$-typeclass
morphism $f:E^{T}\rightarrow C$ satisfying the identity law ($\text{pu}_{E}\bef f=r$)
will be expressed as $f=\text{run}_{E}^{C}(r)$. $\square$

The uniqueness property reflects a programmer\textsf{'}s expectation that there
is only one \textsf{``}correct\textsf{''} way of running a free $P$-typeclass program
while preserving the typeclass\textsf{'}s operations.
...
we find that there is a unique $P$-typeclass morphism $\text{run}_{E}^{T,E^{T}}(\text{pu}_{E})$
...
To verify the $P$-typeclass morphism law for the function $\text{run}_{\text{FPR}}^{T,C}(r)$:
\[
...
To verify the $P$-typeclass naturality law, assume $g^{:C\rightarrow D}$
is a $P$-typeclass morphism ($p_{C}\bef g=g^{\uparrow P}\bef p_{D}$)
...
\end{array}\quad.
\end{align*}
{*}{*}{*} run\textasciicircum T,C {*}{*}{*} The two code matrices
are now equal due to the assumed law $p_{C}\bef g=g^{\uparrow P}\bef p_{D}$.
...
have the function $\text{run}_{\text{FPR}}^{T,C}$ in a matrix form,
