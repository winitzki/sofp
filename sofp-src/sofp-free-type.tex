...
type constructor that has monad operations. If $F$ is already itself
a monad, constructing the free monad on $F$ seems to be unnecessary.
Indeed, a value of type $\text{Free}^{F,A}$ can be always mapped
back to $F^{A}$ while preserving the monad operations:
...
also simplify certain expressions containing quantified types where
the Yoneda identities cannot be applied.
