...
\addto\captionsrussian{\renewcommand{\lstlistingname}{\inputencoding{koi8-r}Листинг}}
\renewcommand{\lstlistingname}{Listing}

\begin{document}

\chapter{``Free'' type constructions\label{chap:Free-type-constructions}}
...
An \emph{interpreter} will ``run'' the program and return a complex
number
...
{*}{*}{*}\footnote{``A function \lstinline!launch()! that launches real-world missiles
can run out of missiles.'' (A quote attributed to \index{Simon Peyton Jones}Simon
Peyton Jones.)}
...
\inputencoding{latin9}Consider the Russian letter \inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}
(ts\`{e}) and the Chinese word \shui~(shu\textipa{\v i})

\inputencoding{koi8-r}%
\inputencoding{latin9}We want to \emph{multiply} \inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}
by \shui. Multiply how?

Say, we want an associative (but noncommutative) product of them

\inputencoding{koi8-r}%
\inputencoding{latin9}So we want to define a \emph{semigroup} that
\emph{contains} \inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}
and \shui~as elements

\inputencoding{koi8-r}%
\inputencoding{latin9}while we still know nothing about \inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}
and \shui

\inputencoding{koi8-r}%
\inputencoding{latin9}Consider the set of all \emph{unevaluated expressions}
such as \inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}$\cdot$\shui$\cdot$\shui$\cdot$\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}$\cdot$\shui

\inputencoding{koi8-r}%
\inputencoding{latin9}Here \inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}$\cdot$\shui~is
different from \shui$\cdot$\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}
but $\left(a\cdot b\right)\cdot c=a\cdot\left(b\cdot c\right)$

\inputencoding{koi8-r}%
\inputencoding{latin9}All these expressions form a \textbf{free semigroup}
generated by \inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}
and \shui

\inputencoding{koi8-r}%
\inputencoding{latin9}This is the most unrestricted semigroup that
contains \inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}
and \shui

Example calculation: (\shui$\cdot$\shui)$\cdot$(\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}$\cdot$\shui)$\cdot$\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}
$=$ \shui$\cdot$\shui$\cdot$\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9}$\cdot$\shui$\cdot$\inputencoding{koi8-r}\foreignlanguage{russian}{ц}

\inputencoding{latin9}How to represent this as a data type:

\textbf{Tree encoding}: the full expression tree: (((\shui,\shui),(\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9},\shui)),\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9})

Implement the operation $a\cdot b$ as pair constructor (easy)

\textbf{Reduced encoding}, as a ``smart'' structure: List(\shui,\shui,\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9},\shui,\inputencoding{koi8-r}\foreignlanguage{russian}{ц}\inputencoding{latin9})
...
Church encoding is then a particular case of the Yoneda identity where
the functor $F$ is chosen as the identity functor.
...
where the function is required to be a natural transformation between
the functors $P\rightarrow\bullet$ and $F^{\bullet}$.

\subparagraph{Proof}

The isomorphism between the two types is implemented as two functions,
\begin{align*}
\text{toC}:F^{A}\rightarrow\forall X.\,(A\rightarrow X)\rightarrow F^{X}\quad, & \quad\quad\text{toC}\triangleq\forall A.\,p^{:F^{A}}\rightarrow\forall X.\,g^{:A\rightarrow X}\rightarrow p\triangleright g^{\uparrow F}\quad,\\
\text{fromC}:(\forall X.\,(A\rightarrow X)\rightarrow F^{X})\rightarrow F^{A}
\end{align*}
...
an extra type parameter $Z$, which is ``hidden''
...
We ``enriched'' $Z$ to a monoid $\text{FM}^{Z}$, and $F^{A}$
...
$\text{FM}^{Z}$ wraps $Z$ in ``just enough'' stuff to make it
look like a monoid

$\text{FreeF}^{F^{\bullet},A}$ wraps $F^{A}$ in ``just enough''
stuff to make it look like a functor

A value of a free construction can be ``run'' to yield non-free
values 
...
Church encoding is ``automatically reduced'', but performance may
differ
...
To combine generators, use $\text{FreeC}^{Z_{1}+Z_{2}}$; an ``instance
over $Z_{1}$ and $Z_{2}$''
...
Implement a ``free pointed filterable'' typeclass (combining pointed
...
The ``logarithm'' $\text{Lg}\,(F^{\bullet})\triangleq\forall A.\,F^{A}\rightarrow A$
...
name ``logarithm''. But $\text{Lg}\,(F^{\bullet}+G^{\bullet})=\text{Lg}\,(F)\times\text{Lg}\,(G)$,
which resembles the distributive law for the \emph{exponential} function
rather than for the logarithm. Also, $\forall A.\,(Z\times A\times A)\rightarrow A\cong Z\times\bbnum2$,
so $\text{Lg}\,(Z\times(\bbnum2\rightarrow\bullet))=Z\times\bbnum2$.
However, for a constant functor, $\text{Lg}\,(Z)=\bbnum0$. This shows
that $\text{Lg}\,(F^{\bullet}\times G^{\bullet})\not\cong\text{Lg}\,(F)\times\text{Lg}\,(G)$.
We also have $\text{Lg}\,(\text{Opt})=\bbnum0$.
...
The Yoneda identity
\[
\forall A.(R\rightarrow A)\rightarrow F^{A}\cong F^{R}
\]
 holds for fully parametric code. If we assume parametricity, we can
simplify certain expressions containing quantified types where the
Yoneda identities cannot be applied.
...
now so let's implement instead of niЯo Java file types let's just
...
complex as parameters it's up to us we could have programmers parameters
it's a matter of convenience what important in it but whatever that
is it's going to be encapsulated now in the case class ops so this
...
and so we define a new typeclass let's call it X Z for extracting
...
typeclass constraints so here's what will happen if we do that we
...
so here's a computation we wrap one wrap to wrap three add them that's
...
these are already interpreters let's see how that works we will interpret
this threesome in two string which would have a standard seven group
instance so let's define are those standard semi group instance for
string now in order to extract into string we need to have an extractor
so let's make it available so now string has a typeclass instance
of the extractor typeclass and we can run that's it that's how we
run we do need to specify the type parameter but that's it so we don't
...
on X and we have the two possible cases and that's how it works so
now we have seen the encoding coatings of three typeclasses let's
...
so there is one method let's look at this method so the tree encoding
of a free factor would have directly encoded this as a case class
let's call it f map and it will also have a case class for wrapping
...
a but a is not known is an argument so it's a type function really
so I'm trying to find notation for type function so in Scala it'll
be like this and I don't like this notation so much but it's okay
...
of F of N and the other case class raps this so it's it's going to
...
of the case class have a parameter Z it's an arbitrary FZ which is
...
a type parameter let's consider a simpler example simpler than all
...
key of it so it hides the Z from the outside type let's look at how
this works what if I wanted to construct a value of type K of F how
would I do that well here's here's how there's only this case class
so I have to use this case class and I have to specify some other
type for Z let's specify string and then I would have a value which
has visible type Q int but actually inside it's hiding a string type
and it knows that it's hiding experiment right so it could have been
...
but we hide it so other outside we don't see that Z it's exists inside
...
and precede the role for Z is existentially quantified because it's
...
it is not universally quantified so even though it's a type parameter
here it's not universally quantified with respect to Z and this is
...
Z baked in the value Q once you construct it so that's why it is an
...
so until now it's very similar but case class is not method quite
rate so this is the crucial difference so if it's a method of a trait
...
and this would be given already as an implicit argument let's omit
...
X which must be of this typeclass and then I have this so it's a much
...
it's perhaps more confusing than suggested so let's just not use that
terminology instead let's concentrate on what these things actually
...
is functions with some type signatures like this and all these Q's
...
that were given if that is so we can put all of these Q's together
...
because these queues will have FC inside them perhaps but that that's
fine now the queues might contain existential type primer so if if
these methods contain more type parameters on the left hand side it's
...
we can just use those methods and that's how the run function works
so this is very easy to encode the tree with coding it's very straightforward
almost mechanical so you have a definition of a typeclass you can
mechanically generate this and it's run method there's no problem
...
done or you can do a church encoding of the reduced encoding so that's
...
be trained included and they're not inductive well one typeclass that's
...
any typeclass that has a method that's not returning a value of that
type that's not inductive the reason is methods must be of this sort
so if you have methods not of the sort that did not return values
of this type then it's not an inductive typeclass so here's an example
...
for not just some so this is still not non good enough there's some
...
free factor which I have not yet shown but actually here's the code
what I was just talking about I can define free instance of any inductive
typeclass I can write code for this generically so it's a free instance
...
basically that's it we can show that it has methods of the typeclass
...
all the required methods so that's why I called P methods C so P has
all the methods of C and this is a very easy code that just checks
whether it's wrapping then it runs the extractor or if it's the operation
...
that's how it works and there's an example I use this for a generic
...
a free semi-group that's it generate it by screen now this is just
to be clear this is my own definition here it's not the cats library
definition or any other or scalzi definition I believe in those libraries
what is called free is actually the free Mona it's a free Mona not
just freaking structure any type cons so in my definition here it's
of generic free instance of a given typeclass for inductive typeclasses
and here's code that creates some values and runs them so I create
...
it's slightly more type definitions but it's exactly similar the church
...
that it's a five class now the showing that it is a typeclass is actually
...
typeclass constraint on G and to declare this as an argument so that's
...
this type signature we realize that it's inductive because this method
returns again the value of the type we're constraining and also we
realize that it has an existential type inside because it's parameterize
with this be there for the three encoding looks like this it's again
...
can compose them after doing Countryman that's a composition law there
...
dropped since they are not used so that's how we derive the reduced
encoding we need to figure out what are the possible expressions and
how they can be simplified using the laws of the typeclass there's
...
control factor which is prepending prefix to the log message and that's
I'm going to show the code in a few minutes that's how we would use
...
does not produce a new in equivalent type it's the result is equivalent
the free country function of over F is equivalent to F you just like
it is the case for the pre factor or the look at example code so here's
...
it's just putting it into the wrap is constantly the country funky
...
single function that is left so that's the example I just described
...
so that's an that's an interesting property so the result is is a
contractor but if you look at the country factories type signature
it is say this it's a contractor in being but it is no longer a functor
so even if the constructor that we used was a functor it is no longer
function and indeed so it's a contractor indeed so we have we if we
do this we take a function like the right here function we wrap it
into American structure and we lose the factor for it's a little bit
you know it is a country function just to make it clear what's going
...
so what's the tree encoding of this well it's just a so we follow
...
so that's how we encode so we have a disjunction with three parts
...
this wrapped constructor times the function and it's not recursive
so that's very nice note that this is exactly the same as a free function
...
a three-pointed so that becomes appointed and it's actually free pointed
...
let's look at the code it's very easy to implement this we need three
...
they return the type of know actually it's sufficient to keep just
map upped because we can restore map from it so let's not overdo things
and let's just implement one method in the free construction since
...
visualized as an infinite disjunction like this so it's F a F 1 plus
...
so on so it's this in this way it's implementing the free filterable
...
like this so we can still encode it so that's going to be reduced
...
f co-option of a is filterable and that's a free filterable over a
...
will control factors because that is completely analogous here's the
...
as well and for the implicit if there's the different ways but what
...
given for example so that's why would she was pure and flat map here
...
encoding needs to be derived let's derive it so first of all let's
...
is still recursive it is somewhat shorter but it's still recursive
...
mind I'm not going to talk about final tagless because it's not really
something specific or or special to jamuna's or put the portal DSL
it's just the church encoding of a free movement and you can choose
...
most likely it's similar with monads and because the three encoding
...
as an option there's almost never advantages in using a non reduced
...
also provide this as an option in the library because it's so much
...
one and the result is going to be this and that's it we cannot really
simplify this because we don't have a pure encoded so we cannot say
oh let's only have this case because we cannot encode a fade so we
...
FA plus this so that's reduced encoding here is a code for the free
moment so you spend a lot of time on this let's consider the free
...
the typeclass so it's not just an arbitrary transformation from P
...
the methods frontier of the typeclass and here's the method for P
...
free so that's this function and that that's the first one that I
...
number of generators and for any typeclass C it's a general formula
...
of typeclasses and any combination of generators it's a very powerful
...
that is at once a function and the control factor and that's possible
...
have the typeclass instances for functor monad it's more or less boilerplate
I just do a little bit of simplification it's not important most so
...
make it quicker to lift things it's a wrapper so basically I generate
...
a pure value doesn't do anything it's a pure data structure that describes
...
context it's fine it's all working it all works in arbitrary order
