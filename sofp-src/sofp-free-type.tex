...
\subsection{Free monoids\label{subsec:Free-monoids}}
...
\]
We can see that \inputencoding{latin9}\lstinline!FSR[T]!\inputencoding{utf8}
...
$\text{FMR}^{T}$ represents an unevaluated expression tree built
from the monoid operations and from values of type $T$, for example:
...
like this:\inputencoding{latin9}
\begin{lstlisting}
implicit def monoidFMR[T]: Monoid[FMR[T]] = Monoid((l, r) => Combine(l, r), Empty()) *** verify code
\end{lstlisting}
...
As usual with raw tree encodings, the free monoid\textsf{'}s operations not
...
The next step is to implement a universal runner that will evaluate
that expression tree, mapping all values into a chosen monoid \inputencoding{latin9}\lstinline!M!\inputencoding{utf8}.
Given a function \inputencoding{latin9}\lstinline!T => M!\inputencoding{utf8},
we can run any \inputencoding{latin9}\lstinline!FMR!\inputencoding{utf8}-program
(a value of type \inputencoding{latin9}\lstinline!FMR[T]!\inputencoding{utf8})
into \inputencoding{latin9}\lstinline!M!\inputencoding{utf8} like
this:\inputencoding{latin9}
\begin{lstlisting}
def runner[M: Monoid, T](runT: T => M): FMR[T] => M = {
  case Combine(left, right)   => runner(runT)(left) |+| runner(runT)(right)
...
does not satisfy the monoid laws (the associativity law and the two
identity laws). This is not a problem in practice, since the laws
will hold after running an \inputencoding{latin9}\lstinline!FMR!\inputencoding{utf8}-program
...
As in the case of the free semigroup, the associativity law will hold
if we replace a binary tree by a non-empty list. The result is a non-empty
...
\begin{lstlisting}
def runner[M: Monoid, T](runT: T => M): List[T] => M = foldMap[M, T](runT)
\end{lstlisting}
\inputencoding{utf8}
We may also define a reduced encoding of the free monoid that satisfies
the identity laws but not the associativity law. To impose the identity
laws on $\text{Tree2}^{\bbnum 1+T}$, we note that expression trees
creating \textsf{``}empty\textsf{''} leaf values ($1+\bbnum 0^{:T}$) must be reduced
by the identity laws to expression trees that do not contain any \textsf{``}empty\textsf{''}
values. For instance, the value \inputencoding{latin9}\lstinline!exampleFMR!\inputencoding{utf8}
needs to be simplified to If the expression is itself equal to the
empty element
...
let us examine the definition of the free monad encoding \inputencoding{latin9}\lstinline!Free1!\inputencoding{utf8}.
A monad may be defined as a functor that additionally has the \inputencoding{latin9}\lstinline!pure!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!flatten!\inputencoding{utf8}
...
The free typeclass recipe applied to a monad\textsf{'}s definition via \inputencoding{latin9}\lstinline!pure!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!flatten!\inputencoding{utf8}
will give:\inputencoding{latin9}
\begin{lstlisting}
...
in two ways: the presence of \inputencoding{latin9}\lstinline!Wrap!\inputencoding{utf8}
and the definition of the form \inputencoding{latin9}\lstinline!Free5[F, Free5[F, A]]!\inputencoding{utf8}
instead of \inputencoding{latin9}\lstinline!F[Free1[F, A]]!\inputencoding{utf8}
within \inputencoding{latin9}\lstinline!Flatten!\inputencoding{utf8}.{*}{*}{*}
We avoid the simplification Free5{[}F, F{[}A{]}{]} because this creates
a recursive definition of Free5{[}F, A{]} whose recursive use modifies
the type parameter A of Free5
...
\subsection{The Church encodings of free typeclasses}
...
different performance trade-offs. Free constructions of other typeclasses
are motivated by analogy with the free monad and its various encodings.
Can we formulate any properties or laws that validate the correctness
of those constructions? Are all the different encodings equally safe
to use? We will now develop the necessary theory for answering these
questions.

\subsection{Free constructions for inductive typeclasses\label{subsec:Free-constructions-for-inductive-typeclasses}}
...
construction to other inductive typeclasses. We will then extend the
...
will hold for the values of type $M$ obtained by running a \textsf{``}free
monoid program\textsf{''}, even when the chosen encoding $\text{FM}^{T}$
violates some of the monoid laws.

To generalize from free monoids to other typeclasses, it helps to
use the notion of an \textsf{``}inductive typeclass\textsf{''}\index{inductive typeclass}\index{typeclass!inductive}
introduced in Section~\ref{subsec:Inductive-typeclasses}. For a
given (covariant) functor $P$, we define a $P$\textbf{-typeclass}
as\index{$P$-typeclass} an inductive typeclass with structure functor
$P$. So, all the data in a $P$-typeclass instance for a type $A$
is equivalent to a value of type $P^{A}\rightarrow A$. For the \inputencoding{latin9}\lstinline!Monoid!\inputencoding{utf8}
typeclass, the instance data has type $A\times\left(A\times A\rightarrow A\right)$,
which is equivalent to to $P^{A}\rightarrow A$ if the structure functor
is $P^{A}\triangleq\bbnum 1+A\times A$. The two parts of the disjunctive
type $\bbnum 1+A\times A$ correspond to the \emph{arguments} of the
monoid\textsf{'}s two operations: the empty value ($e_{A}$) and the binary
operation ($\oplus_{A}$).
...
the laws in Definition~\ref{subsec:Definition-monoid-morphism} in
terms of the monoid\textsf{'}s structure functor $P^{A}=\bbnum 1+A\times A$?
...
\begin{equation}
p_{M}\bef f\overset{!}{=}f^{\uparrow P}\bef p_{N}\quad.\label{eq:p-typeclass-morphism-law}
\end{equation}
We simplify both sides of Eq.~(\ref{eq:p-typeclass-morphism-law})
...
\end{array}\quad.
\end{align*}
Then Eq.~(\ref{eq:p-typeclass-morphism-law}) is rewritten as:
\[
\begin{array}{||c|}
1\rightarrow f(e_{M})\\
a\times b\rightarrow f(a\oplus_{M}b)
...
for an arbitrary $P$-typeclass:

\subsubsection{Definition \label{subsec:Definition-P-typeclass-morphism}\ref{subsec:Definition-P-typeclass-morphism}}

For any two types $M$ and $N$ with $P$-typeclass instances $p_{M}:P^{M}\rightarrow M$
and $p_{N}:P^{N}\rightarrow N$, a function $f:M\rightarrow N$ is
a \textbf{$P$-typeclass morphism}\index{$P$-typeclass morphism|textit}
if Eq.~(\ref{eq:p-typeclass-morphism-law}) holds. 

We say that $f$ \textsf{``}preserves the $P$-typeclass operations\textsf{''} if
$f$ is a $P$-typeclass morphism. $\square$

We will now generalize the free monoid construction to the \textsf{``}free
$P$-typeclass\textsf{''} construction, with an arbitrary structure functor
$P$. By analogy with the free monoid, we formulate the required properties
of an encoding of a free $P$-typeclass:
...
in $T$. So, for any $f:T\rightarrow U$ the function $f^{\uparrow E}:E^{T}\rightarrow E^{U}$
is a $P$-typeclass morphism.

\textbf{(b)} The functor $E$ is pointed: there exists a fully parametric
function $\text{pu}_{E}:T\rightarrow E^{T}$.

\textbf{(c)} For any other type $C$ that already has a $P$-typeclass
instance ($p_{C}:P^{C}\rightarrow C$) and for any function $r:T\rightarrow C$,
there exists a unique runner function ($\text{run}_{E}$) that satisfies:
\begin{align*}
 & \text{run}_{E}:(T\rightarrow C)\rightarrow E^{T}\rightarrow C\quad,\\
{\color{greenunder}\text{identity law}:}\quad & \text{pu}_{E}\bef\text{run}_{E}(r)=r\quad,\\
{\color{greenunder}P\text{-typeclass morphism law}:}\quad & p_{E}\bef\text{run}_{E}(r)=\big(\text{run}_{E}(r)\big)^{\uparrow P}\bef p_{C}\quad,\\
{\color{greenunder}P\text{-typeclass naturality law}:}\quad & \text{for any }P\text{-typeclass morphism }f^{:C\rightarrow D}:\\
 & \quad\quad\text{run}_{E}(r\bef f)=\text{run}_{E}(r)\bef f\quad.
\end{align*}
Uniqueness of the runner $\text{run}$ means that any $P$-typeclass
morphism $f:E^{T}\rightarrow C$ satisfying the identity law ($\text{pu}_{E}\bef f=r$)
will be expressed as $f=\text{run}_{E}(r)$. $\square$

The requirement of having a unique runner reflects our expectation
that there is only one \textsf{``}correct\textsf{''} way of running a free $P$-typeclass
program while preserving the typeclass\textsf{'}s operations. This was the
case in several examples seen so far.

If $T$ already has a $P$-typeclass instance then the function $\text{run}_{E}(\text{id}^{:T\rightarrow T})$
of type $E^{T}\rightarrow T$ satisfies the identity law:
\[
\text{pu}_{E}\bef\text{run}_{E}(\text{id}^{:T\rightarrow T})=\text{id}^{:T\rightarrow T}\quad.
\]
...
we find that there is a unique $P$-typeclass morphism $\text{run}_{E}(\text{pu}_{E})$
of type $E^{T}\rightarrow E^{T}$. That $P$-typeclass morphism must
be equal to the identity function, $\text{id}:E^{T}\rightarrow E^{T}$,
because the identity function is also a $P$-typeclass morphism satisfying
the identity law ($\text{pu}_{E}\bef\text{id}=\text{pu}_{E}$). So,
we must have: 
\[
\text{run}_{E}(\text{pu}_{E})=\text{id}\quad.
\]

Let us now find out how to generalize a raw tree encoding to an arbitrary
$P$-typeclass. The raw tree encoding of the free monoid (see Section~\ref{subsec:Free-monoids})
has the type: 
\[
\text{FMR}^{T}\triangleq\bbnum 1+T+\text{FMR}^{T}\times\text{FMR}^{T}\quad.
\]
...
two of these three ways. So, we are motivated to rewrite the definition
of $\text{FMR}^{T}$ as $\text{FMR}^{T}\triangleq T+P^{\text{FMR}^{T}}$.
This can be directly generalized to an arbitrary $P$-typeclass. So,
we define the raw tree encoding of a free $P$-typeclass (denoted
by $\text{FPR}^{T}$) as:
\[
...
\]
This equation is the same as the $P$-typeclass morphism law for the
function $f^{\uparrow\text{FPR}}$.

\textbf{(b)} The function $\text{pu}_{\text{FPR}}:T\rightarrow\text{FPR}^{T}$
is defined as $\text{in}_{\text{FPR}}(t)\triangleq t+\bbnum 0$.

\textbf{(c)} Given $p_{C}:P^{C}\rightarrow C$, the runner function
$\text{run}_{\text{FPR}}$ is defined recursively by:
\[
\text{run}_{\text{FPR}}:(T\rightarrow C)\rightarrow\text{FPR}^{T}\rightarrow C\quad,\quad\quad\text{run}_{\text{FPR}}(r^{:T\rightarrow C})\triangleq\,\begin{array}{|c||c|}
 & C\\
\hline T & r\\
P^{\text{FPR}^{T}} & \overline{\text{run}_{\text{FPR}}(r)}^{\uparrow P}\bef p_{C}
\end{array}\quad.
\]

To verify the identity law:
\[
\text{pu}_{\text{FPR}}\bef\text{run}_{\text{FPR}}(r)=\,\begin{array}{|c||cc|}
 & T & P^{\text{FPR}^{T}}\\
...
To verify the $P$-typeclass morphism law for the function $\text{run}_{\text{FPR}}(r)$:
\[
p_{\text{FPR}}\bef\text{run}_{\text{FPR}}(r)=\,\begin{array}{|c||cc|}
 & T & P^{\text{FPR}^{T}}\\
...
\hline T & r\\
P^{\text{FPR}^{T}} & \overline{\text{run}_{\text{FPR}}(r)}^{\uparrow P}\bef p_{C}
\end{array}\,=\overline{\text{run}_{\text{FPR}}(r)}^{\uparrow P}\bef p_{C}\quad.
\]

To verify the $P$-typeclass naturality law, assume $f^{:C\rightarrow D}$
is a $P$-typeclass morphism ($p_{C}\bef f=f^{\uparrow P}\bef p_{D}$)
and use the inductive assumption that $\text{run}_{\text{FPR}}(r\bef f)=\text{run}_{\text{FPR}}(r)\bef f$
for recursive calls to $\text{run}_{\text{FPR}}$:
\begin{align*}
{\color{greenunder}\text{left-hand side}:}\quad & \text{run}_{\text{FPR}}(r\bef f)=\,\begin{array}{|c||c|}
 & D\\
\hline T & r\bef f\\
P^{\text{FPR}^{T}} & \overline{\text{run}_{\text{FPR}}(r\bef f)}^{\uparrow P}\bef p_{D}
\end{array}\\
{\color{greenunder}\text{inductive assumption}:}\quad & \quad\quad=\,\begin{array}{||c|}
r\bef f\\
\big(\overline{\text{run}_{\text{FPR}}(r)}\bef f\big)^{\uparrow P}\bef p_{D}
\end{array}\,=\,\begin{array}{||c|}
r\bef f\\
\overline{\text{run}_{\text{FPR}}(r)}^{\uparrow P}\bef f^{\uparrow P}\bef p_{D}
\end{array}\quad;\\
{\color{greenunder}\text{right-hand side}:}\quad & \text{run}_{\text{FPR}}(r)\bef f=\,\begin{array}{|c||c|}
 & C\\
\hline T & r\\
P^{\text{FPR}^{T}} & \overline{\text{run}_{\text{FPR}}(r)}^{\uparrow P}\bef p_{C}
\end{array}\,\bef f=\,\begin{array}{||c|}
r\bef f\\
\overline{\text{run}_{\text{FPR}}(r)}^{\uparrow P}\bef p_{C}\bef f
\end{array}\quad.
\end{align*}
The two code matrices are now equal due to the assumed law $p_{C}\bef f=f^{\uparrow P}\bef p_{D}$.

It remains to prove the uniqueness of the runner function. For a fixed
type $C$ with $P$-typeclass instance data $p_{C}:P^{C}\rightarrow C$
and a fixed $r:T\rightarrow C$, suppose a function $f:\text{FPR}^{T}\rightarrow C$
satisfies the identity law and the $P$-typeclass morphism law:
\[
\text{pu}_{\text{FPR}}\bef f=r\quad,\quad\quad p_{\text{FPR}}\bef f=f^{\uparrow P}\bef p_{C}\quad.
\]
We need to show that $f=\text{run}_{\text{FPR}}(r)$. Since the function
$\text{run}_{\text{FPR}}$ is in a matrix form, it is convenient to
rewrite $f$ also in that form:
\[
...
the $P$-typeclass morphism law gives:
\[
p_{\text{FPR}}\bef f=h\overset{!}{=}f^{\uparrow P}\bef p_{C}\quad.
\]
It remains to show that the following code matrices are equal:
\[
\text{run}_{\text{FPR}}(r)=\,\begin{array}{||c|}
r\\
\overline{\text{run}_{\text{FPR}}(r)}^{\uparrow P}\bef p_{C}
...
\end{array}\quad.
\]
By the inductive assumption, the recursive call $\overline{\text{run}_{\text{FPR}}(r)}$
already satisfies the equation we need to prove: $f=\overline{\text{run}_{\text{FPR}}(r)}$.
So, the code matrices for $f$ and for $\text{run}_{\text{FPR}}$
are equal. $\square$

We notice that $\text{FPR}^{T}$ is the same as the free monad on
$P$ (first defined in Statement~\ref{subsec:Statement-monad-construction-4-free-monad}).
...
monad on $P$ represents an unevaluated expression tree whose operations
have arguments described by $P$. The monadic operation \inputencoding{latin9}\lstinline!flatten!\inputencoding{utf8}
...
As we will now show, that transformation is a $P$-typeclass morphism;
in other words, it preserves the typeclass operations. So, it is not
useful to apply the free $P$-typeclass construction twice, as the
result will be reducible to a single layer of \inputencoding{latin9}\lstinline!FPR[T]!\inputencoding{utf8}.
...
Write the $P$-typeclass morphism law of $\text{ftn}_{\text{FPR}}$:
...
\[
p_{\text{FPR}}^{\text{FPR}^{T}}\bef\text{ftn}_{\text{FPR}}=\big(\text{ftn}_{\text{FPR}}\big)^{\uparrow P}\bef p_{\text{FPR}}^{T}\quad.
\]
The function $p_{\text{FPR}}^{A}$ can be expressed through $\text{ftn}_{\text{FPR}}$
as:
\[
p_{\text{FPR}}^{A}:P^{\text{FPR}^{A}}\rightarrow\text{FPR}^{A}\quad,\quad\quad p_{\text{FPR}}^{A}=\big(x^{:P^{\text{FPR}^{A}}}\rightarrow(\bbnum 0^{:A}+x)^{:\text{FPR}^{A}}+\bbnum 0^{:P^{\text{FPR}^{\text{FPR}^{A}}}}\big)\bef\text{ftn}_{\text{FPR}}^{A}\quad.
\]
...
functions of type $T$. This follows from Example~\ref{subsec:Example-strong-dinaturality-for-some-type-signatures}(c)
