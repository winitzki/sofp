...
That type constructor is known as the \textsf{``}free monad\textsf{''}, for reasons
we will discuss later. Let us now summarize the recipe for using \inputencoding{latin9}\lstinline!MonadDSL!\inputencoding{utf8}
when we would like to add the monadic variable-binding functionality
to a given DSL.
...
Operations may be also functions parameterized by one or more arbitrary
types, for example, $A\times\text{Int}\rightarrow\text{String}$ where
$A$ is a type parameter. In every case, we need to write down the
types in the form of a simple (non-curried) function, such as $P\rightarrow Q$,
where $P$ and $Q$ are type expressions possibly depending on some
type parameters.
...
operations. For every operation with type $P\rightarrow Q$, define
the corresponding case class as:\inputencoding{latin9}
\begin{lstlisting}
final case class SomeOperation(x: P) extends F[Q]
\end{lstlisting}
\inputencoding{utf8}The name \inputencoding{latin9}\lstinline!SomeOperation!\inputencoding{utf8}
...
is not. For example, the \inputencoding{latin9}\lstinline!PrgFile!\inputencoding{utf8}
...
example shown above, \inputencoding{latin9}\lstinline!PrgFileC!\inputencoding{utf8}
...
\inputencoding{latin9}\lstinline!PrgFileC[JPath]!\inputencoding{utf8}
...
with an arbitrary unknown \inputencoding{latin9}\lstinline!X!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
val runFileDSL = new Runner[PrgFileC] { def run[X]: PrgFileC[X] => X = runFileC } ***check if this works
\end{lstlisting}
\inputencoding{utf8}Nevertheless, the runner\textsf{'}s type signature (\inputencoding{latin9}\lstinline!def run[X]: F[X] => X!\inputencoding{utf8})
keeps a general type parameter \inputencoding{latin9}\lstinline!X!\inputencoding{utf8}.
...
as a wrapper that adds the monad\textsf{'}s functionality to any effect constructor
\inputencoding{latin9}\lstinline!F!\inputencoding{utf8}. The monad\textsf{'}s
methods (\inputencoding{latin9}\lstinline!pure!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!flatMap!\inputencoding{utf8})
merely create some data structures (case classes) in memory and do
not cause any domain-specific effects or actions to be performed.
The effects will be performed at a later time by an effect runner
(\inputencoding{latin9}\lstinline!runF!\inputencoding{utf8}) when
we apply \inputencoding{latin9}\lstinline!MonadDSL.run!\inputencoding{utf8}
to a DSL program.
...
often does not support these methods. To see how \inputencoding{latin9}\lstinline!MonadDSL!\inputencoding{utf8}
