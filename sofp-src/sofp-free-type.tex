...
(the free monad on $P$) is a free $P$-typeclass encoding. We have
seen in Section~\ref{subsec:Describing-laws-of-P-typeclasses-as-values}
...
Suppose $E^{T}$ is a $Q_{T}$-algebra with $q_{E}^{T}:Q_{T}^{E^{T}}\rightarrow E^{T}$
natural in $T$. Suppose there exists a function $\text{in}_{E}^{T}:E^{T}\rightarrow\text{FPR}^{T}$
(also natural in $T$) such that:
\[
\text{in}_{E}^{T}\bef r_{\text{FPR}}^{T,E^{T}}=\text{id}\quad.
\]
(The function $\text{in}_{E}^{T}$ is injective but \emph{not} a $Q_{T}$-algebra
morphism.) Then $E$ is a free $P$-typeclass encoding.
