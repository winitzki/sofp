...
where \index{Gabriella Gonzalez (known as Gabriel Gonzalez prior to 2021)}G.~Gonzalez
...
We call these implementations \textbf{encodings}\index{free monad!different encodings}
...
Are these encodings equivalent? What properties do they need to have
in order to be considered \textsf{``}correct\textsf{''}? Are there any other encodings
of the free monad? Can we find constructions that convert any type
into a functor, a monoid, or into another typeclass? Can we find those
constructions in a systematic way, without guessing? The rest of this
chapter will develop the required theory and show the derivations
...
\subsection{Deriving a reduced encoding of the free monad}
...
These observations suggest that we might find a simpler encoding if
we use the monad laws when designing the free monad\textsf{'}s type constructor.
To see how this works, we will first reduce \inputencoding{latin9}\lstinline!Free4!\inputencoding{utf8}
...
\emph{assumes} that the effect constructor \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}
is a functor with a \inputencoding{latin9}\lstinline!map!\inputencoding{utf8}
...
is equivalent to the other constructions when \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}
is a functor. We will also show more examples of free typeclass constructions
that build upon another typeclass.}
...
be implemented by translating all case classes identically. That code,
however, will never create values of type \inputencoding{latin9}\lstinline!FMap!\inputencoding{utf8}
since \inputencoding{latin9}\lstinline!Free3!\inputencoding{utf8}
does not have that case class. It is impossible to convert any values
...
After running the DSL programs, the results will be the same:\inputencoding{latin9}
\begin{lstlisting}
