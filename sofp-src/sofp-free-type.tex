...
that are based on another typeclass.}
...
in the ordinary way. So, we write the definition of \inputencoding{latin9}\lstinline!Bind[F, T, A]!\inputencoding{utf8}
in the type notation as: 
\[
\exists A.\,\text{MonadDSL}^{F,A}\times(A\rightarrow\text{MonadDSL}^{F,T})\lesssim\text{MonadDSL}^{F,T}\quad.
\]
