...
\end{lstlisting}
\inputencoding{utf8}
An important property of the universal runner is that it maps \inputencoding{latin9}\lstinline!MonadDSL!\inputencoding{utf8}\textsf{'}s
...
\end{lstlisting}
\inputencoding{utf8}Conversely, if we require that a runner \inputencoding{latin9}\lstinline!run[A]: MonadDSL[A] => M[A]!\inputencoding{utf8}
should preserve \inputencoding{latin9}\lstinline!pure!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!flatMap!\inputencoding{utf8},
we have no choice but transform \inputencoding{latin9}\lstinline!Val!\inputencoding{utf8}
into \inputencoding{latin9}\lstinline!M!\inputencoding{utf8}\textsf{'}s pure
and \inputencoding{latin9}\lstinline!Bind!\inputencoding{utf8} into
\inputencoding{latin9}\lstinline!M!\inputencoding{utf8}\textsf{'}s \inputencoding{latin9}\lstinline!flatMap!\inputencoding{utf8}.
Under the requirement of he code of the universal runner is unique.
...
perform any computations but only create more nested case classes.
Those nested case classes represent an unevaluated expression tree
of an \textsf{``}\inputencoding{latin9}\lstinline!FMR!\inputencoding{utf8}-program\textsf{''}.
The next step is to implement a universal runner that will evaluate
that expression tree, mapping all values into a given monoid \inputencoding{latin9}\lstinline!M!\inputencoding{utf8}.
...
into a lawful monoid \inputencoding{latin9}\lstinline!M!\inputencoding{utf8}.
Imposing the monoid laws merely helps us find a reduced encoding of
the free monoid.
...
is a possible encoding of the free monoid, but it does not satisfy
the identity laws. The empty value of the free monoid is represented
by a single-element list containing \inputencoding{latin9}\lstinline!None!\inputencoding{utf8}
...
The conclusion is that \inputencoding{latin9}\lstinline!List[T]!\inputencoding{utf8}
is the reduced encoding for the free monoid on $T$. The runner is
the same as the \inputencoding{latin9}\lstinline!foldMap!\inputencoding{utf8}
...
simple type-safe DSL. One then discovers different encodings of the
free monad (the raw tree encoding and the various reduced encodings)
that have different performance trade-offs. Free constructions of
...
encodings equally safe to use?

\subsection{Free constructions for inductive typeclasses\label{subsec:Free-constructions-for-inductive-typeclasses}}

Let us look at the features common to all the free typeclass constructions
we have seen. For simplicity, we first consider the free monoid construction
and generalize its features to other inductive typeclasses. We will
then extend the results to typeclasses for type constructors (such
as \inputencoding{latin9}\lstinline!Functor!\inputencoding{utf8}
...
is equivalent to a value of type $P^{A}\rightarrow A$. The typeclass
instance data for \inputencoding{latin9}\lstinline!Monoid!\inputencoding{utf8}
has type $A\times\left(A\times A\rightarrow A\right)$, which is equivalent
to to $P^{A}\rightarrow A$ if we define the structure functor as
$P^{A}\triangleq\bbnum 1+A\times A$. The two parts of the disjunctive
type $\bbnum 1+A\times A$ correspond to the \emph{arguments} of the
monoid\textsf{'}s two operations: the empty value and the binary operation.

What does it mean for a function to \textsf{``}preserve the monoid\textsf{'}s operations\textsf{''}?
{*}{*}{*}
...
\begin{itemize}
\item The free $P$-typeclass is a covariant type constructor denoted by
$\text{FPTC}^{\bullet}$.
\item For any type $T$, the type $\text{FPTC}^{T}$ has a $P$-typeclass
instance (i.e., there exists a value of type $P^{\text{FPTC}^{T}}\rightarrow\text{FPTC}^{T}$).
\item There exists a function \inputencoding{latin9}\lstinline!in!\inputencoding{utf8}
$:T\rightarrow\text{FPTC}^{T}$.
\item For any $P$-typeclass instance $C$ and any function $T\rightarrow C$,
there exists a unique runner $\text{FPTC}^{T}\rightarrow C$ that
preserves the operations of the $P$-typeclass:
\[
\text{run}:(T\rightarrow C)\rightarrow\text{FPTC}^{T}\rightarrow C\quad.
\]
Here, the requirement of a \emph{unique} runner reflects our expectation
...
\item If $T$ already has a $P$-typeclass instance then the corresponding
unique runner $\text{run}\,(\text{id}^{:T\rightarrow T})$ of type
$\text{FPTC}^{T}\rightarrow T$ satisfies the identity law involving
the injection function (\inputencoding{latin9}\lstinline!in!\inputencoding{utf8}):
\[
\text{in}^{:T\rightarrow\text{FPTC}^{T}}\bef\text{run}\,(\text{id}^{:T\rightarrow T})=\text{id}^{T\rightarrow T}\quad.
\]
\end{itemize}
The raw tree encoding of the free monoid (see Section~\ref{subsec:Free-monoids})
has the type: 
\[
\text{FMR}^{T}\triangleq\bbnum 1+T+\text{FMR}^{T}\times\text{FMR}^{T}\quad.
\]
...
\]
Let us verify that $\text{FPR}^{T}$ has the required properties of
a free $P$-typeclass.

{*}{*}{*}

The It turns out that the property of runner
