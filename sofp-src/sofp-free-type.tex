...
and $L_{\text{inf}}^{A}$. To see this formally, we can find injective
functions $f_{1}:L_{\text{fin}}^{A}\rightarrow L^{A}$ and $f_{2}:L_{\text{inf}}^{A}\rightarrow L^{A}$.
...
it is the function \lstinline!fix!. If we apply a value of type $T$
to the function \lstinline!fix!, we will again get a value of type
$T$. This gives us a function of type $T\rightarrow T$; but this
function is equal to identity, as the next statement shows.
