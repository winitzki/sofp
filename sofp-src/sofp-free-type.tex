...
case class, we need to use the inductive assumption that \inputencoding{latin9}\lstinline!runFree4!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!runFree3!\inputencoding{utf8}
...
\end{lstlisting}
\inputencoding{utf8}Nevertheless, we may write valid code that uses data of type $C$
without knowing what $C$ is:\inputencoding{latin9}
\begin{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}
We will use this notation later for proofs of properties of free monads
and other constructions. With the new notation for existential types,
...
that adds monadic features to an arbitrary type constructor. Are there
...
Looking at the raw tree encoding of the free monad, we find a pattern:
the raw tree encoding contains a case class for each of the monad\textsf{'}s
methods (\inputencoding{latin9}\lstinline!map!\inputencoding{utf8},
...
The same pattern can give a raw tree encoding for many other free
typeclasses. This section will look at several examples.

\subsection{Free functor}

\subsection{Free filterable functor}

\subsection{Free applicative functor}
