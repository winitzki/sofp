...
a new feature into the notation, namely a type with an \textbf{existential
quantifier}\index{existential quantifier} (denoted by $\exists$).
...
To clarify the usage of $\exists$ in these examples, begin with \inputencoding{latin9}\lstinline!Free3!\inputencoding{utf8}
(which is equivalent to \inputencoding{latin9}\lstinline!MonadDSL!\inputencoding{utf8}):\inputencoding{latin9}
\begin{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}The definition of the case class \inputencoding{latin9}\lstinline!Bind[F, T, A]!\inputencoding{utf8}
...
will be set to a specific type. However, the result will be a value
...
from the rest of the code, although that type parameter is necessary
...
\inputencoding{latin9}\lstinline!T!\inputencoding{utf8}) are used
in the ordinary way: they are used both in the case class and in the
parent trait. So, we write the definition of \inputencoding{latin9}\lstinline!Bind[F, T, A]!\inputencoding{utf8}
in the type notation as: 
\[
\exists A.\,\text{MonadDSL}^{F,A}\times(A\rightarrow\text{MonadDSL}^{F,T})\quad.
\]
...
\[
\text{MonadDSL}^{F,T}\triangleq T+F^{T}+\exists A.\,\text{MonadDSL}^{F,A}\times(A\rightarrow\text{MonadDSL}^{F,T})\quad.
\]

To understand the word \textsf{``}existential\textsf{''}, we may imagine that a type
...
since \inputencoding{latin9}\lstinline!q!\inputencoding{utf8} could
have been created as \inputencoding{latin9}\lstinline!Bind[F, T, A]!\inputencoding{utf8}
with any type \inputencoding{latin9}\lstinline!A!\inputencoding{utf8}.
...
will be able to determine if the type \inputencoding{latin9}\lstinline!A!\inputencoding{utf8}
is \inputencoding{latin9}\lstinline!Int!\inputencoding{utf8} or not.
...
Consider a type expression $\exists C.\,L^{A,B,C}$ where $L$ is
a type constructor with type parameters $A$, $B$, $C$ and an existential
quantifier is set on $C$. {*}{*}{*}
