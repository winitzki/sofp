...
Free typeclass constructions such as free monoids, free functors,
free monads, free applicative functors, and so on, are commonly used
to implement the DSL (domain-specific language\index{domain-specific language (DSL)})
...
The DSL pattern consists of separating the description of computations
from the process of their evaluation. One defines a \textbf{DSL program}\index{DSL program}
as a value that fully describes what computations and side effects
need to be run but does not actually perform those computations and
side-effects. The programmer begins by defining the type of DSL programs.
Helper functions are implemented for creating the DSL program values
and for combining several DSL programs into a larger one. Finally,
one or more functions are written to interpret (or \textsf{``}run\textsf{''}) the
DSL programs.
...
Scala), rather than as a separate new language. This book only considers
embedded DSLs and calls them just \textsf{``}DSL'\textsf{'}s for brevity.
...
we will obtain the construction known as the free monad.
...
Instead of running the complex-number programs directly, the DSL pattern
tells us first to create data structures that contain an \emph{unevaluated}
expression tree of the program. Separate case classes are used for
addition, multiplication, and conjugation, as well as for inserting
a literal \lstinline!Complex! value into the program:
\begin{lstlisting}
...
By representing complex-number calculations as DSL programs, which
are values of type \lstinline!PrgComplex!, we can compose, optimize,
or transform those DSL programs before running them. We may also implement
several runners in order to run the same computation in different
environments (e.g., on GPUs or on a remote computer cluster) or instrumented
in different ways (e.g., with logging, debugging, or benchmarking).
...
the operations that need to be run. A runner will perform those operations
and extract the final \lstinline!String! value:
\begin{lstlisting}
...
\end{lstlisting}
To test this code, prepare a file \lstinline!config.txt! containing
...
to a \lstinline!Path!, but this is not enforced by the DSL. When
combining a larger DSL program from separately defined parts, one
could by mistake create an invalid DSL program such as \lstinline!Read(Read(Literal("file")))!
...
Second, we should not use \lstinline!run! in the middle of the program.
This will prevent us from running the same code through different
\lstinline!run! functions. DSL programs should consist only of the
case classes representing the DSL operations.
...
\end{lstlisting}
and replace \lstinline!Read(p)! by \lstinline!Read(Literal(p))!
in the code above. The code of \lstinline!run! is unchanged.

To solve the second problem, we need to introduce a new DSL operation
...
we can avoid the \lstinline!run! call in the code \lstinline!val p = run(...)!
and instead introduce a new DSL operation that we will call \lstinline!Bind!.
The above code that computes \lstinline!p! and \lstinline!prg! is
replaced by:
\begin{lstlisting}
val prg2: PrgFile[JPath] = Path(Literal("config.txt"))
val prg: PrgFile[String] = Bind(prg2, p => if (Files.exists(p)) Read(p) else Literal("File not found"))
\end{lstlisting}
...
methods of a monad. For convenience, let us define those methods on
...
\end{lstlisting}
Here we defined \lstinline!map! through \lstinline!flatMap! by analogy
with the right identity law of monads.
