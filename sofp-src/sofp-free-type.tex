...
\chapter{\textquotedblleft Free\textquotedblright{} type constructions\label{chap:Free-type-constructions}}

\section{Motivation for the free monad: implement a DSL in five steps}

A domain-specific language (DSL) is a design pattern that separates
the computations from the process of their evaluation. A simple way
of implementing a DSL is to write a library of functions in an already
existing programming language (e.g., Scala). First, the programmer
defines a data structure that represents a DSL program and helper
functions for creating such programs. Then, a separate method is used
to interpret (or \textsf{``}run\textsf{''}) DSL programs. The result is known as an
\textsf{``}embedded\textsf{''} DSL.

As first examples, we will consider a DSL for complex numbers and
a DSL for file operations. We will refine and refactor our DSLs step
by step to make them more powerful and safer to use. In this way,
we will obtain the construction known as the \textsf{``}free monad\textsf{''}.

\subsection{Step 1: unevaluated expression trees}

Suppose we need to perform calculations with complex numbers. We could
begin by implementing a simple data structure (\lstinline!case class Complex!)
...
Instead of running the complex-number programs directly, a DSL will
first create data structures that contain an \emph{unevaluated} expression
tree of the program. Separate case classes are used for addition,
multiplication, and conjugation, as well as for inserting a literal
\lstinline!Complex! value into the program:
\begin{lstlisting}
...
A simple interpreter for the complex-number DSL programs can be written
like this:
\begin{lstlisting}
...
\end{lstlisting}
We can now use \lstinline!run! to perform the computation \lstinline!prgComplex!
and get the result as a complex number:
\begin{lstlisting}
...
By representing complex-number calculations as DSL program data, which
are values of type \lstinline!PrgComplex!, we can freely compose,
optimize, or transform those DSL programs before running them. We
may also implement several runners in order to run the same computation
in different ways (say, with logging, debugging, benchmarking, etc.)
or in different environments (say, on GPUs or on a remote machine).

As a second example, consider a DSL for file operations, such as reading
and writing files at a given file path. Direct Scala code for such
...
\end{lstlisting}
The DSL will represent these operations by the case classes \lstinline!Literal!,
...
val prgFile: PrgFile = Read(Path(Read(Path(Literal("config_location.txt")))))
\end{lstlisting}
As before, a DSL program contains an unevaluated expression tree describing
the operations that need to be run. A separate interpreter (or \textsf{``}runner\textsf{''})
is needed to extract values from a DSL program:
\begin{lstlisting}
def run: PrgFile => String = {
  case Literal(s)      => s
  case Path(p)         => "path=" + run(p)                // For debugging.
  case Read(Path(p))   => new String(Files.readAllBytes(Paths.get(run(p))))
  case x => throw new Exception(s"Illegal PrgFile operation: $x")
}
\end{lstlisting}
As a test, prepare a file \lstinline!config.txt! containing the text
\lstinline!version = 1! and a file \lstinline!config_location.txt!
...
\subsection{Step 2: implementing type safety in the DSLs}

The two DSLs defined in the previous section have an important shortcoming:
they are not type-checked at compile time. This becomes clear by looking
at the DSL program \lstinline!prgFile! shown above. The \lstinline!run!
function assumes that the \lstinline!Read! operation is always applied
to a \lstinline!Path!, but this is not enforced by the DSL. By mistake,
one could create an invalid DSL program such as \lstinline!Read(Read(Literal("file")))!
where the \lstinline!Read! operation is used incorrectly. Such mistakes
could be made when combining a larger DSL program from separately
defined parts. Running this program causes a run-time error:
\begin{lstlisting}
scala> run(Read(Read(Literal("file"))))
java.lang.Exception: Illegal PrgFile operation: Read(Read(Literal(file)))
\end{lstlisting}
The Scala compiler cannot check that we are using the DSL correctly,
because all DSL values are of the same type (\lstinline!PrgFile!)
regardless of their intended meaning. It would be better if the DSL
program \lstinline!Read(Read(Literal("file")))! failed to compile.
...
replace the type \lstinline!PrgFile! by the type constructor \lstinline!PrgFile[A]!,
representing a DSL program that returns a value of type \lstinline!A!
when we run it:
\begin{lstlisting}
...
\end{lstlisting}
Our example DSL is type-safe now, and invalid programs are rejected
at compile time:
\begin{lstlisting}
val prgFile: PrgFile[String] = Read(Path(Read(Path(Literal("config_location.txt")))))
...
\subsection{Step 3: implementing bound variables}
...
code (such as a numerical algorithms library for complex numbers,
a library for file manipulation, or any other Scala code). 

Both of these limitations would be removed if the DSL could support
defining a \emph{new Scala variable} and setting it to a value computed
within the DSL. Let us see how this feature may be implemented.

As an example, consider the task of reading a file if it exists, and
reporting an error otherwise. A direct code for this computation would
look like this:
\begin{lstlisting}
val p = Paths.get("config.txt")
val result = if (Files.exists(p)) new String(Files.readAllBytes(p)) else "File not found"
\end{lstlisting}
...
\begin{lstlisting}
val p: JPath = run(Path(Literal("config.txt")))
val prg: PrgFile[String] = if (Files.exists(p)) Read(p) else Literal("File not found")
\end{lstlisting}
There are two problems with this code. First, the code \lstinline!Read(p)!
...
Second, we use \lstinline!run! in the middle of the program. We cannot
translate that into the DSL because the \lstinline!run! function
needs to be kept out of the DSL. (For instance, we should be able
to run the same code through different \lstinline!run! functions.)

The first problem is solved if we generalize the \lstinline!Literal!
class to hold values of arbitrary type:
\begin{lstlisting}
final case class Literal[A](a: A) extends PrgFile[A]
\end{lstlisting}
We replace \lstinline!Read(p)! by \lstinline!Read(Literal(p))! in
the code above. The code of \lstinline!run! remains unchanged.

To solve the second problem, we need to introduce a new DSL command
that binds Scala variables. A standard way of creating \index{bound variable}bound
variables is by using those variables as arguments of nameless functions.
So, let us consider the nameless function
\begin{lstlisting}
p => if (Files.exists(p)) Read(Literal(p)) else Literal("File not found")
\end{lstlisting}
This function binds the variable \lstinline!p! and uses it directly
in an expression of type \lstinline!PrgFile[String]!. We can call
this function at the time when the entire DSL program is run. So,
we can postpone the \lstinline!run! call in the code \lstinline!val p = run(...)!
and instead introduce a new DSL operation that we will call \lstinline!Bind!.
The above code (that computed \lstinline!p! and \lstinline!prg!)
is replaced by:
\begin{lstlisting}
val q: PrgFile[JPath] = Path(Literal("config.txt"))
val prg: PrgFile[String] = Bind(q, p => if (Files.exists(p)) Read(p) else Literal("File not found"))
\end{lstlisting}
Here, the case class \lstinline!Bind! has two parts: a value of type
\lstinline!PrgFile[JPath]! and a function of type \lstinline!JPath => PrgFile[String]!.
In a different DSL program, we may need different types instead of
\lstinline!JPath! and \lstinline!String!. So, let us replace those
types by type parameters \lstinline!A! and \lstinline!B!. Note that
the resulting type is \lstinline!PrgFile[B]!:
\begin{lstlisting}
final case class Bind[A, B](pa: PrgFile[A], f: A => PrgFile[B]) extends PrgFile[B]
\end{lstlisting}

After adding some code to the runner to handle \lstinline!Bind!,
the implementation of the DSL becomes:
\begin{lstlisting}
sealed trait PrgFile[A]
final case class Literal[A](a: A) extends PrgFile[A]
final case class Bind[A, B](pa: PrgFile[A], f: A => PrgFile[B]) extends PrgFile[B]
...
The new \lstinline!Bind! operation allows us to implement a part
of the DSL program as a Scala function that depends on the result
computed by another part of the DSL program. Since \lstinline!Bind!
contains an arbitrary Scala function (of type \lstinline!A => PrgFile[B]!),
we can now use non-DSL code as part of a DSL program, as long as it
is enclosed inside a \lstinline!Bind!.

\subsection{Step 4: arriving at a monadic DSL}

The type signatures of \lstinline!Bind! and \lstinline!Literal!
are the same as those of \lstinline!flatMap! and \lstinline!pure!
methods of a monad. For convenience, let us define those methods for
the \lstinline!PrgFile! class:
\begin{lstlisting}
sealed trait PrgFile[A] {
...
object PrgFile {
  def pure[A]: A => PrgFile[A] = Literal.apply 
}
\end{lstlisting}
Here, we defined the \lstinline!map! method through \lstinline!flatMap!
by analogy with the right identity law of monads.

Just like other DSL operations, the \lstinline!flatMap! and \lstinline!pure!
methods do not run or evaluate any parts of the DSL program. They
merely create some nested data structures containing not-yet-called
functions. The necessary logic will be executed only when the \lstinline!run!
...
val prg: PrgFile[String] = for {
  p <- Path(Literal("config.txt"))
  r <- if (Files.exists(p)) Read(p) else Literal("File not found")
} yield r
\end{lstlisting}
...
def readPath(filename: String): PrgFile[String] = for {
  ...
  text <- if (Files.exists(path)) Read(path) else Literal("File not found")
} yield text

val prg2: PrgFile[String] = for {
  str <- readPath("config.txt")
  result <- if (str.nonEmpty) readPath(str) else PrgFile.pure("Error: empty filename")
} yield result
\end{lstlisting}


\subsection{Step 5: refactoring to reuse common code}

Recall our first example (a DSL for complex numbers) and rewrite it
in a similar way:
\begin{lstlisting}
sealed trait PrgComplex[A]
final case class Literal[A](a: A) extends PrgComplex[A]
final case class Bind[A, B](pa: PrgComplex[A], f: A => PrgComplex[B]) extends PrgComplex[B]
...
Comparing the code of \lstinline!PrgFile! and \lstinline!PrgComplex!,
we notice that the the \lstinline!Literal! and \lstinline!Bind!
operations are implemented in the same way. The differences are the
code for operations specific to file handling or to complex numbers.
So, let us refactor the code so that the domain-specific operations
are separated and the common code is reused. This can be achieved
if we gather the domain-specific operations in a separate type constructor
that we call the \index{effect constructor}\textbf{effect constructor}.
The common code will then use the effect constructor as a type parameter.
Let us now refactor the two examples in this way.

{*}{*}{*}is it Ops(f: F{[}A{]}) and why not Ops(f: F{[}Prg{[}A{]}{]})???
What about the complex-numbers example?
...
\subsection{Interpreting a DSL: handling errors}
...
$T$ to a free monad construction. Implement a monad instance for
\lstinline!TIO[A]!.
...
The following statement\footnote{See also the paper \textsf{``}A note on strong dinaturality\textsf{''}: \texttt{\href{https://web.archive.org/web/20110601105059/http://www.cs.ioc.ee/~tarmo/papers/fics10.pdf}{http://www.cs.ioc.ee/$\sim$tarmo/papers/fics10.pdf}}}
...
\end{equation}
(Strong dinaturality holds for all functions of type $T$ implemented
via fully parametric code; see Example~\ref{subsec:Example-strong-dinaturality-for-Church-encoding}
in Appendix~\ref{app:Proofs-of-naturality-parametricity}.) Defined
in this way, the type $T$ is a solution of the fixpoint equation
$T\cong F^{T}$.
...
type $\exists X.\,X$, which is observationally equivalent to \lstinline!Unit!
. The advantage of using \lstinline!Any! instead of \lstinline!Unit!
is that \lstinline!Any! is a supertype of \emph{all} Scala types.
Indeed, for any type $T$ there is an injective function $T\rightarrow\exists X.\:X$,
This function corresponds to a function of type \lstinline!T => Any!
in Scala:
\begin{lstlisting}
