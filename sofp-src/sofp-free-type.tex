...
for an arbitrary $P$-typeclass: we just need to impose Eq.~(\ref{eq:p-typeclass-morphism-law}).
...
as functions $f^{:M\rightarrow N}$ satisfying Eq.~(\ref{eq:p-typeclass-morphism-law}).
Such functions $f$ are then called $F$-\textbf{algebra morphisms}.\index{$F$-algebra morphism}
...
plays the role of an evidence value showing that a type $M$ belongs
to the $P$-typeclass. If $M$ and $N$ are two $P$-algebras and
a function $f^{:M\rightarrow N}$ is a $P$-algebra morphism then
...
and obeys the relevant laws, and for any type $T$ (not necessarily
...
Suppose we need to verify whether a type $T$ obeys the \inputencoding{latin9}\lstinline!et!\inputencoding{utf8}-law.
...
\end{lstlisting}
\inputencoding{utf8}Then we evaluate both expression trees using $T$\textsf{'}s operations:\inputencoding{latin9}
\begin{lstlisting}
...
and \inputencoding{latin9}\lstinline!LawET!\inputencoding{utf8} are
equivalent, we may use either of them when it is more convenient within
...
\inputencoding{latin9}\lstinline!F2!\inputencoding{utf8}, \inputencoding{latin9}\lstinline!F3!\inputencoding{utf8},
and \inputencoding{latin9}\lstinline!F4!\inputencoding{utf8}). In
fact, all those encodings satisfy the requirements of Definition~\ref{subsec:Definition-free-P-typeclass-encoding}
