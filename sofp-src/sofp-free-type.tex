...
\end{lstlisting}
A DSL program of type \lstinline!PrgFile! is an unevaluated expression
tree describing the operations that need to be performed. A runner
will perform those operations and extract the final \lstinline!String!
value:
\begin{lstlisting}
...
different operations as well as for inserting a Val \lstinline!Complex!
value:
\begin{lstlisting}
...
becomes clear by looking at the DSL program \lstinline!prgFile! shown
in the previous section. The code of \lstinline!runFile! function
...
DSL. When combining a larger DSL program from separately defined parts,
...
if the DSL program \lstinline!Read(Read(Val("file")))! failed to
compile. To achieve that, we will use different Scala types for DSL
programs returning a \lstinline!String! and a \lstinline!Path!.
So, let us replace the type \lstinline!PrgFile! by the type constructor
\lstinline!PrgFile[A]!, representing a DSL program that will return
...
\end{lstlisting}
Our example DSL is type-safe now.\index{type safety} Invalid programs
...
is always applied to a phase, but the DSL does not enforce that assumption.
Both arguments of the \lstinline!Rotate! operation are values of
type \lstinline!PrgComplex!, and the Scala compiler is unable to
verify that the DSL is being used correctly. If the programmer uses
an arbitrary complex number instead of a phase, the program will return
an incorrect result without any indication of error:
\begin{lstlisting}
...
of type \lstinline!A! when run. The new definitions of the case classes
are:
\begin{lstlisting}
...
\end{lstlisting}
The revised code of \lstinline!runComplex! is simpler because hacks
...
middle of a DSL program. This will break the assumption that a DSL
program merely describes the computations without executing them.
The use of a specific \lstinline!run! function inside a DSL program
will also prevent us from being able to apply a different runner to
...
at the time we call \lstinline!runFile!. At that time, the argument
\lstinline!p! will be set to the value obtained by running \lstinline!prg2!.
This is implemented in the new code for \lstinline!runFile! shown
below.
...
the resulting type is \lstinline!PrgFile[A]!:
\begin{lstlisting}
...
the arguments of the \lstinline!Bind[A, B]! constructor are curried,
...
braces:
\begin{lstlisting}
val prg: PrgFile[String] = Bind(prg2){ p =>  ... }
\end{lstlisting}
...
it would be \lstinline!p: JPath!). This makes the DSL easier to use.
However, the underlyng implementation becomes more complicated. Scala\textsf{'}s
...
contains arbitrary Scala code (a function of type \lstinline!B => PrgFile[A]!),
we can now use non-DSL code as part of a DSL program, as long as it
is enclosed inside a \lstinline!Bind!. 

The body of the function inside \lstinline!Bind! will be evaluated
only when the DSL program is run. So, the DSL program is still only
a value that describes the computation but does not execute it.
\begin{lstlisting}
scala> runFile(prg)
...
Like other DSL operations, the \lstinline!map!, \lstinline!flatMap!,
and \lstinline!pure! methods do not run or evaluate any parts of
a DSL program. They just create some nested data structures containing
not-yet-called functions. The necessary logic will be executed only
when the \lstinline!run! method is invoked. At that time, the entire
DSL program will be converted into a result value.

Nevertheless, DSL programs can now be written as functor blocks:
\begin{lstlisting}
val prg: PrgFile[String] = for {
  p <- Path(Val("config.txt"))
  r <- if (Files.exists(p)) Read(p) else Val("No file.")
} yield r
\end{lstlisting}
...
val prg2: PrgFile[String] = for {
  ...
  result <- if (str.nonEmpty) readFileContents(str) else PrgFile.pure("No filename.")
} yield result
...
Recall our second example (a DSL for complex numbers) and rewrite
it in a similar way:
\begin{lstlisting}
sealed trait PrgComplex[A] {
  def flatMap[B](f: A => Prg[B]): Prg[B] = Bind(this)(f)
...
we notice that the the \lstinline!Val! and \lstinline!Bind! operations
...
us refactor these data structures so that the domain-specific operations
are separated and the common code may be reused. We can gather all
the domain-specific operations in a separate type constructor that
we call the \index{effect constructor}\textbf{effect constructor}.
...
\texttt{\textcolor{blue}{\footnotesize{}case class Val{[}A{]}(a: A)
