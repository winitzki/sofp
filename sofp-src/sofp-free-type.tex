...
is a binary tree with leaf values of type $\bbnum 1+T$:
\[
...
if Eq.~(\ref{eq:p-typeclass-morphism-law}) holds.\footnote{In \index{category theory}category theory, this is called a $P$-algebra\index{$F$-algebra}
morphism. However, a $P$-typeclass is not the same as a $P$-algebra
because it usually needs to satisfy additional laws. Below we will
develop a more detailed description of $P$-typeclass laws.}
...
in $T$. For any $f:T\rightarrow U$ the function $f^{\uparrow E}:E^{T}\rightarrow E^{U}$
...
\textbf{(c)} For any other type $C$ that belongs to the $P$-typeclass
(and obeys the relevant laws) and for any function $r:T\rightarrow C$,
there exists a unique function $\text{run}_{E}^{C}(r):E^{T}\rightarrow C$.
The \textsf{``}universal runner\textsf{''} ($\text{run}_{E}^{C}$) satisfies:
\begin{align*}
 & \text{run}_{E}^{C}:(T\rightarrow C)\rightarrow E^{T}\rightarrow C\quad,\\
{\color{greenunder}\text{identity law}:}\quad & \text{pu}_{E}\bef\text{run}_{E}^{C}(r)=r\quad,\\
{\color{greenunder}P\text{-typeclass morphism law}:}\quad & p_{E}^{T}\bef\text{run}_{E}^{C}(r)=\big(\text{run}_{E}^{C}(r)\big)^{\uparrow P}\bef p_{C}^{T}\quad,\\
{\color{greenunder}P\text{-typeclass naturality law}:}\quad & \text{for any }P\text{-typeclass morphism }f^{:C\rightarrow D}:\\
 & \quad\quad\text{run}_{E}^{D}(r\bef f)=\text{run}_{E}^{C}(r)\bef f\quad.
\end{align*}
Uniqueness of the runner means that any $P$-typeclass morphism $f:E^{T}\rightarrow C$
satisfying the identity law ($\text{pu}_{E}\bef f=r$) will be expressed
as $f=\text{run}_{E}^{C}(r)$. $\square$
...
If $T$ itself has a $P$-typeclass instance then the function $\text{run}_{E}^{T}(\text{id}^{:T\rightarrow T})$
of type $E^{T}\rightarrow T$ satisfies the identity law:
\[
\text{pu}_{E}\bef\text{run}_{E}^{T}(\text{id}^{:T\rightarrow T})=\text{id}^{:T\rightarrow T}\quad.
\]
...
we find that there is a unique $P$-typeclass morphism $\text{run}_{E}^{E^{T}}(\text{pu}_{E})$
...
that: 
\[
\text{run}_{E}^{E^{T}}(\text{pu}_{E})=\text{id}^{:E^{T}\rightarrow E^{T}}\quad.
\]

Let us now find out how to generalize the raw tree encoding from \inputencoding{latin9}\lstinline!Monoid!\inputencoding{utf8}
to an arbitrary $P$-typeclass. The raw tree encoding of a free monoid
(see Section~\ref{subsec:Free-monoids}) has the type: 
\[
\text{FMR}^{T}\triangleq\bbnum 1+T+\text{FMR}^{T}\times\text{FMR}^{T}\quad.
\]
...
$\text{run}_{\text{FPR}}^{C}$ is defined recursively by:
\[
\text{run}_{\text{FPR}}^{C}:(T\rightarrow C)\rightarrow\text{FPR}^{T}\rightarrow C\quad,\quad\quad\text{run}_{\text{FPR}}^{C}(r^{:T\rightarrow C})\triangleq\,\begin{array}{|c||c|}
 & C\\
\hline T & r\\
P^{\text{FPR}^{T}} & \overline{\text{run}_{\text{FPR}}^{C}(r)}^{\uparrow P}\bef p_{C}
\end{array}\quad.
\]

To verify the identity law:
\[
\text{pu}_{\text{FPR}}\bef\text{run}_{\text{FPR}}^{C}(r)=\,\begin{array}{|c||cc|}
 & T & P^{\text{FPR}^{T}}\\
...
To verify the $P$-typeclass morphism law for the function $\text{run}_{\text{FPR}}^{C}(r)$:
\[
p_{\text{FPR}}\bef\text{run}_{\text{FPR}}^{C}(r)=\,\begin{array}{|c||cc|}
 & T & P^{\text{FPR}^{T}}\\
...
\hline T & r\\
P^{\text{FPR}^{T}} & \overline{\text{run}_{\text{FPR}}^{C}(r)}^{\uparrow P}\bef p_{C}
\end{array}\,=\overline{\text{run}_{\text{FPR}}^{C}(r)}^{\uparrow P}\bef p_{C}\quad.
\]
...
and use the inductive assumption that $\text{run}_{\text{FPR}}^{D}(r\bef f)=\text{run}_{\text{FPR}}^{C}(r)\bef f$
for recursive calls to $\text{run}_{\text{FPR}}$:
\begin{align*}
{\color{greenunder}\text{left-hand side}:}\quad & \text{run}_{\text{FPR}}^{D}(r\bef f)=\,\begin{array}{|c||c|}
 & D\\
\hline T & r\bef f\\
P^{\text{FPR}^{T}} & \overline{\text{run}_{\text{FPR}}^{D}(r\bef f)}^{\uparrow P}\bef p_{D}
\end{array}\\
{\color{greenunder}\text{inductive assumption}:}\quad & \quad\quad=\,\begin{array}{||c|}
r\bef f\\
\big(\overline{\text{run}_{\text{FPR}}^{C}(r)}\bef f\big)^{\uparrow P}\bef p_{D}
\end{array}\,=\,\begin{array}{||c|}
r\bef f\\
\overline{\text{run}_{\text{FPR}}^{C}(r)}^{\uparrow P}\bef f^{\uparrow P}\bef p_{D}
\end{array}\quad;\\
{\color{greenunder}\text{right-hand side}:}\quad & \text{run}_{\text{FPR}}^{C}(r)\bef f=\,\begin{array}{|c||c|}
 & C\\
\hline T & r\\
P^{\text{FPR}^{T}} & \overline{\text{run}_{\text{FPR}}^{C}(r)}^{\uparrow P}\bef p_{C}
\end{array}\,\bef f=\,\begin{array}{||c|}
r\bef f\\
\overline{\text{run}_{\text{FPR}}^{C}(r)}^{\uparrow P}\bef p_{C}\bef f
\end{array}\quad.
\end{align*}
{*}{*}{*} run\textasciicircum C {*}{*}{*} The two code matrices are
now equal due to the assumed law $p_{C}\bef f=f^{\uparrow P}\bef p_{D}$.
...
\]
We need to show that $f=\text{run}_{\text{FPR}}^{C}(r)$. Since we
have the function $\text{run}_{\text{FPR}}^{C}$ in a matrix form,
...
\[
\text{run}_{\text{FPR}}^{C}(r)=\,\begin{array}{||c|}
r\\
\overline{\text{run}_{\text{FPR}}^{C}(r)}^{\uparrow P}\bef p_{C}
...
\end{array}\quad.
\]
By the inductive assumption, the recursive call $\overline{\text{run}_{\text{FPR}}^{C}(r)}$
already satisfies the equation we need to prove: $f=\overline{\text{run}_{\text{FPR}}^{C}(r)}$.
...
operations described by $P$. The monadic operation \inputencoding{latin9}\lstinline!flatten!\inputencoding{utf8}
...
\subsection{Describing laws of $P$-typeclasses via law functions}

Usually, typeclasses impose some laws on their methods. We will now
develop a rigorous description of $P$-typeclass laws.
...
Laws for other typeclasses may require more than three arbitrary values
of type $A$. To make the definition of a law function more general,
we replace an arbitrary value of type $A\times A\times A$ by an arbitrary
function $f$ of type $\text{Int}\rightarrow A$. Instead of arbitrary
values $x$, $y$, $z$ we can then use the values $f(1)$, $f(2)$,
$f(3)$, etc. This allows us to generalize the definition of the \inputencoding{latin9}\lstinline!Monoid!\inputencoding{utf8}
...
A \textbf{law function} for a $P$-typeclass \index{$P$-typeclass!law functions}
with a \index{law function of a $P$-typeclass}structure functor $P$
is a value $l$ of type: 
\begin{equation}
\text{LawF}\triangleq\forall A.\,(P^{A}\rightarrow A)\times(\text{Int}\rightarrow A)\rightarrow A\times A\quad.\label{eq:P-typeclass-law-type}
\end{equation}
Given a law function $l$ of type \inputencoding{latin9}\lstinline!LawF!\inputencoding{utf8},
we say that an evidence value $w:P^{T}\rightarrow T$ \textbf{satisfies
the $l$-law} if:
\begin{align}
{\color{greenunder}w\text{ satisfies the }l\text{-law}:}\quad & \text{for all }f^{:\text{Int}\rightarrow T}\quad:\quad\pi_{1}\big(l^{T}(w,f)\big)=\pi_{2}\big(l^{T}(w,f)\big)\quad.\label{eq:P-typeclass-law}
\end{align}
...
typeclass if there exists an evidence value $w:P^{T}\rightarrow T$
that satisfies the $l_{i}$-law~(\ref{eq:P-typeclass-law}) for each
of the specified law functions $l_{i}$. $\square$

Another approach to typeclass laws is by describing the two sides
of a law via expression trees. The the raw tree encoding of a free
$P$-typeclass ($\text{FPR}^{T}$) is a type that represents arbitrary
expression trees with leaf values of type $T$. We can use a pair
of values of type $\text{FPR}^{T}$ as two sides of the law. The two
sides can be evaluated as expression trees using the $P$-operations
of a specific type $T$. The result will be two values of type $T$;
the law holds if these two values are equal.
...
arbitrary $P$-typeclass. The two sides of the law will be functions
that take as arguments one or more values of type $T$ and return
a value of type $\text{FPR}^{T}$. In order to be able to describe
...
we need to add a universal quantifier ($\forall T$). The type of
\textsf{``}pairs of law expression trees\textsf{''} becomes:
\begin{equation}
...
specify a law of a $P$-typeclass? Suppose we need to verify whether
a type $T$ obeys the \inputencoding{latin9}\lstinline!et!\inputencoding{utf8}-law.
...
\end{lstlisting}
\inputencoding{utf8}Then we need to evaluate both esxpression trees using $T$\textsf{'}s operations:\inputencoding{latin9}
\begin{lstlisting}
...
is a Church encoding (see {*}{*}{*}) of the recursive type $U$ that
is the least fixpoint of the type equation $U\cong P^{U}+\text{Int}$.
The least fixpoint $U$ is the same as the type we denote by $\text{FPR}^{\text{Int}}$
according to the definition of $\text{FPR}^{T}$.
...
\subsection{Free $P$-typeclasses that satisfy laws}
