...
\index{structure map of $F$-algebra}\textbf{structure map} of the
$F$-algebra. \index{$F$-algebra!structure map} All $F$-algebras
...
and values of type $T$. The monad\textsf{'}s \inputencoding{latin9}\lstinline!flatten!\inputencoding{utf8}
method will transform \inputencoding{latin9}\lstinline!FPR[FPR[T]]!\inputencoding{utf8}
into \inputencoding{latin9}\lstinline!FPR[T]!\inputencoding{utf8}.
As we will now show, \inputencoding{latin9}\lstinline!flatten!\inputencoding{utf8}
...
Assume that the following three conditions hold for a functor $E$:
...
The second equation holds because of the assumption $\text{pu}_{E}\bef\text{in}_{E}^{T}=\text{pu}_{\text{FPR}}$:
\[
...
is based on the notion of a \textsf{``}\index{monad algebra}monad algebra\textsf{''}:
