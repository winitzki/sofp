...
Church encoding is then regarded as a particular case of the Yoneda
identity where the functor $F$ is chosen as the identity functor.
...
between functors $A\rightarrow X$ and $F^{X}$, where $X$ is a type
parameter of $q$. To apply $q$ to an argument $\text{id}$ of type
$A\rightarrow A$ in the last line, we set $q$'s type parameter $X$
as $X=A$. This is indicated by the superscript $A$ in $q^{A}$. 
...
\end{align*}
Since by assumption $q$ is a natural transformation, it satisfies
the naturality law: for any $r^{:A\rightarrow B}$ and $f^{:B\rightarrow C}$,
\begin{equation}
q^{B}(r)\bef f^{\uparrow F}=q^{C}(r\bef f)\quad.\label{eq:assumed-naturality-of-argument-q}
\end{equation}
Applying this law with $B=A$, $C=X$, $f=g$, and $r=\text{id}$,
we get
\[
q^{A}(\text{id})\bef g^{\uparrow F}=q^{X}(\text{id}\bef g)=q^{X}(g)\quad.
\]
It follows that the function $q\triangleright\text{fromC}\bef\text{toC}$
is the same as $q$:
\[
q\triangleright\text{fromC}\bef\text{toC}=\forall X.\,g^{:A\rightarrow X}\rightarrow q^{X}(g)=\forall X.\,q^{X}=q\quad.
\]
...
\begin{align*}
\text{expect to equal }q^{C}(r\bef f):\quad & q^{B}(r)\bef f^{\uparrow F}=p\triangleright r^{\uparrow F}\bef f^{\uparrow F}=p\triangleright(r\bef f)^{\uparrow F}=q^{C}(r\bef f)\quad.
\end{align*}
...
Denote $T\triangleq\text{Free}^{F}$. When $F$ is itself a monad,
the map $p:T^{A}\rightarrow F^{A}$ defined by
\[
...
\]
This equality is the inductive assumption that the recursive calls
to $\overline{p}$ already satisfy the composition law.
