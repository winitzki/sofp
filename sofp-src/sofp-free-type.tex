...
monad\index{free monad} emerges from elaborating the DSL pattern
in the functional programming paradigm.

\section{Motivation for the free monad: make a DSL in five stages}

The main point of using a DSL is to separate the description of computations
from the process of their evaluation. One writes a \textbf{DSL program}\index{DSL program}
...
This design pattern is known as an \textsf{``}embedded\textsf{''} DSL because it is
implemented as a library in an existing programming language (e.g.,
Scala), rather than as a separate new language. This chapter only
considers embedded DSLs and calls them just \textsf{``}DSL'\textsf{'}s for brevity.

We will look at two examples: a DSL for file operations and a DSL
for complex numbers. We will refine and refactor our DSLs step by
step to make them more powerful and safer to use. In this way, we
will obtain the construction known as the \textsf{``}free monad\textsf{''}.
...
is an unevaluated expression tree describing the operations that need
to be performed. A runner will perform those operations and extract
the final \inputencoding{latin9}\lstinline!String!\inputencoding{utf8}
...
\end{lstlisting}
\inputencoding{utf8}To test this code, prepare a file \inputencoding{latin9}\lstinline!config.txt!\inputencoding{utf8}
...
We could begin by implementing a data structure (\inputencoding{latin9}\lstinline!case class Complex!\inputencoding{utf8})
...
first creates a data structure that contains an \emph{unevaluated}
expression tree for a program. Separate case classes are used for
different operations as well as for inserting a Val \inputencoding{latin9}\lstinline!Complex!\inputencoding{utf8}
value:\inputencoding{latin9}
\begin{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}
A simple runner for complex-number DSL programs can be written like
...
By representing file operations as values of type \inputencoding{latin9}\lstinline!PrgFile!\inputencoding{utf8}
and complex-number calculations as values of type \inputencoding{latin9}\lstinline!PrgComplex!\inputencoding{utf8},
we can compose, verify, optimize, or transform those DSL programs
before running them. We may also implement special runners in order
to execute the same DSL program in different ways: on a remote computer
cluster, on a distributed file system, by running numerical computations
on GPUs instead of CPUs, or with logging, debugging, benchmarking,
and so on.
...
they are not type-checked\index{type checking} at compile time. This
becomes clear by looking at the DSL program \inputencoding{latin9}\lstinline!prgFile!\inputencoding{utf8}
shown in the previous section. The code of \inputencoding{latin9}\lstinline!runFile!\inputencoding{utf8}
function assumes that the \inputencoding{latin9}\lstinline!Read!\inputencoding{utf8}
operation is always applied to a \inputencoding{latin9}\lstinline!Path!\inputencoding{utf8}.
However, this assumption is not enforced by the DSL. When combining
...
by the type constructor \inputencoding{latin9}\lstinline!PrgFile[A]!\inputencoding{utf8},
...
\end{lstlisting}
\inputencoding{utf8}Our example DSL is type-safe now.\index{type safety} Invalid programs
are rejected at compile time:\inputencoding{latin9}
\begin{lstlisting}
...
operation is always applied to a phase, but the DSL does not enforce
that assumption. Both arguments of the \inputencoding{latin9}\lstinline!Rotate!\inputencoding{utf8}
operation are values of type \inputencoding{latin9}\lstinline!PrgComplex!\inputencoding{utf8},
and the Scala compiler is unable to verify that the DSL is being used
correctly. If the programmer uses an arbitrary complex number instead
of a phase, the program will return an incorrect result without any
indication of error:\inputencoding{latin9}
\begin{lstlisting}
val prgComplex3: PrgComplex = Rotate(prgComplex1, Val(Complex(0, 1))) // Forgot Phase().

scala> runComplex(prgComplex3)
...
when run. (So, the new DSL program will be able to compute values
...
is simpler:\inputencoding{latin9}
\begin{lstlisting}
...
Another limitation of our DSLs is that they cannot define and use
new variables within a DSL program. We also cannot use any non-DSL
code, such as a library for file manipulation, a numerical algorithms
library for complex numbers, or any other Scala code. 

Both of these limitations would be removed if the DSL could support
defining \emph{new Scala variables} and setting them to values computed
within the DSL. We will then be able to add arbitrary Scala code working
with those variables. Let us see how this feature may be implemented.

As an example, consider the task of reading a file if it exists and
reporting an error otherwise. A direct (non-DSL) code for this computation
...
in the middle of a DSL program. This will break the assumption that
a DSL program merely describes the computations without executing
them. The use of a specific \inputencoding{latin9}\lstinline!run!\inputencoding{utf8}
function inside a DSL program will also prevent us from being able
to apply a different runner to the entire DSL program.
...
That function will be called at the time we call \inputencoding{latin9}\lstinline!runFile!\inputencoding{utf8}.
At that time, the argument \inputencoding{latin9}\lstinline!p!\inputencoding{utf8}
will be set to the value obtained by running \inputencoding{latin9}\lstinline!prg2!\inputencoding{utf8}.
This is implemented in the new code for \inputencoding{latin9}\lstinline!runFile!\inputencoding{utf8}
shown below.

In a different DSL program, we may need different types instead of
\inputencoding{latin9}\lstinline!JPath!\inputencoding{utf8} and \inputencoding{latin9}\lstinline!String!\inputencoding{utf8}.
So, let us replace those types by type parameters \inputencoding{latin9}\lstinline!A!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!B!\inputencoding{utf8}. Note
that the resulting type is \inputencoding{latin9}\lstinline!PrgFile[A]!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
...
code to the runner, the implementation of the DSL becomes:\inputencoding{latin9}
\begin{lstlisting}
sealed trait PrgFile[A]
final case class Val[A](a: A)                                       extends PrgFile[A]
...
\end{lstlisting}
\inputencoding{utf8}
Let us now explain certain features of Scala used in this DSL. Since
the arguments of the \inputencoding{latin9}\lstinline!Bind[A, B]!\inputencoding{utf8}
constructor are curried, the first curried argument (of type \inputencoding{latin9}\lstinline!PrgFile[B]!\inputencoding{utf8})
...
may be written without a type annotation (in this case, it would be
\inputencoding{latin9}\lstinline!p: JPath!\inputencoding{utf8}).
...
operation allows us to implement a part of the DSL program as a \emph{Scala
function} that depends on the result computed by another part of the
DSL program. Since \inputencoding{latin9}\lstinline!Bind!\inputencoding{utf8}
contains arbitrary Scala code (a function of type \inputencoding{latin9}\lstinline!B => PrgFile[A]!\inputencoding{utf8}),
we can now use non-DSL code as part of a DSL program, as long as it
is enclosed inside a \inputencoding{latin9}\lstinline!Bind!\inputencoding{utf8}. 

The body of the function inside \inputencoding{latin9}\lstinline!Bind!\inputencoding{utf8}
will be evaluated only when the DSL program is run:\inputencoding{latin9}
\begin{lstlisting}
scala> runFile(prg)
res8: String = "version = 1"
\end{lstlisting}
\inputencoding{utf8} So, a value of type \inputencoding{latin9}\lstinline!PrgFile[A]!\inputencoding{utf8}
...
\subsection{Stage 5: refactoring to reuse common code}
...
needs to \textsf{``}run the effects\textsf{''} from the effect constructor \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}.
...
To be able to use the effect runner with arbitrary types, the \inputencoding{latin9}\lstinline!runner!\inputencoding{utf8}
argument must be of type \inputencoding{latin9}\lstinline!F[X] => X!\inputencoding{utf8}
where \inputencoding{latin9}\lstinline!X!\inputencoding{utf8} is
not known in advance. We notice that the type signatures of the effect
runners \inputencoding{latin9}\lstinline!runComplexC!\inputencoding{utf8}
and \inputencoding{latin9}\lstinline!runFileC!\inputencoding{utf8}
already have that form:\inputencoding{latin9}
\begin{lstlisting}
def runComplexC[X]: PrgComplexC[X] => X    // PrgComplexC[X] => X with arbitrary X
def runFileC[X]: PrgFileC[X] => X          // PrgFileC[X] => X with arbitrary X
\end{lstlisting}
\inputencoding{utf8}We need to pass those functions as arguments to the \inputencoding{latin9}\lstinline!run!\inputencoding{utf8}
function and keep the freedom provided by the type parameter \inputencoding{latin9}\lstinline!X!\inputencoding{utf8}. 

To that end, we define a new type \inputencoding{latin9}\lstinline!Runner!\inputencoding{utf8}
as a Scala trait encapsulating the type signature of the effect runners.
The effect runners can then be declared as values of type \inputencoding{latin9}\lstinline!Runner!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
...
still has the freedom to use any type \inputencoding{latin9}\lstinline!X!\inputencoding{utf8}
...
and run some tests:\inputencoding{latin9}
\begin{lstlisting}
...
\subsection{The recipe for monadic DSLs}

The previous section explained the motivation for defining the type
constructor \inputencoding{latin9}\lstinline!MonadDSL!\inputencoding{utf8}.
Let us now summarize the recipe for using \inputencoding{latin9}\lstinline!MonadDSL!\inputencoding{utf8}
when we need to add the monadic functionality to a given DSL.

Begin by writing down the types of available operations in the DSL.
The operations may be functions between fixed known types, such as
$\text{Int}\rightarrow\text{Int}\rightarrow\text{String}$ or $\text{String}\rightarrow\bbnum 1$.
Operations may be also parameterized by one or more arbitrary types,
for example, $A\times\text{Int}\rightarrow\text{String}$ where $A$
is a type parameter. In every case, we need to write down the types
in the form of a function, such as $P\rightarrow Q$, where $P$ and
$Q$ are type expressions possibly depending on some type parameters.

The next step is to define a new type constructor \inputencoding{latin9}\lstinline!F[_]!\inputencoding{utf8}
that will be the effect constructor\index{effect constructor} of
the DSL:\inputencoding{latin9}
\begin{lstlisting}
sealed trait F[_]
\end{lstlisting}
\inputencoding{utf8} The effect constructor \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}
will be a disjunctive type whose parts correspond to each of the DSL
operations. For an operation with type $P\rightarrow Q$, the corresponding
case class will be defined as:\inputencoding{latin9}
\begin{lstlisting}
final case class SomeOperation(x: P) extends F[Q]
\end{lstlisting}
\inputencoding{utf8} It is an important part of the recipe that the result type (\inputencoding{latin9}\lstinline!Q!\inputencoding{utf8})
...
is not. The name \inputencoding{latin9}\lstinline!SomeOperation!\inputencoding{utf8}
should be chosen for clarity.

For example, the \inputencoding{latin9}\lstinline!PrgFile!\inputencoding{utf8}
DSL described in the previous section has two operations of types
$\text{String}\rightarrow\text{JPath}$ and $\text{JPath}\rightarrow\text{String}$.
The corresponding effect constructor is defined by:\inputencoding{latin9}
\begin{lstlisting}
sealed trait F[_]
final case class Path(p: String) extends F[JPath]
final case class Read(p: JPath) extends F[String]
\end{lstlisting}
\inputencoding{utf8}
Typically, a type constructor \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}
defined in this way is an \textsf{``}unfunctor\textsf{''}\index{unfunctor}. In the
example shown above, \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}
cannot be a functor because it is impossible to create values of type
\inputencoding{latin9}\lstinline!F[A]!\inputencoding{utf8} with an
arbitrary type \inputencoding{latin9}\lstinline!A!\inputencoding{utf8}
...
Next, we implement an \textsf{``}effect runner\textsf{''} for \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}.
The effect runner must be a polymorphic function\index{polymorphic function}
with the type signature $\forall X.\,F^{X}\rightarrow X$:\inputencoding{latin9}
\begin{lstlisting}
...
then the runner only needs to work for those types. In the example
shown above, the runner only needs to work with arguments of types
\inputencoding{latin9}\lstinline!F[JPath]!\inputencoding{utf8} and
\inputencoding{latin9}\lstinline!F[String]!\inputencoding{utf8},
not with arguments of type \inputencoding{latin9}\lstinline!F[X]!\inputencoding{utf8}
with an arbitrary unknown \inputencoding{latin9}\lstinline!X!\inputencoding{utf8}.
Nevertheless, the runner\textsf{'}s type signature (\inputencoding{latin9}\lstinline!def run[X]: F[X] => X!\inputencoding{utf8})
remains general, as if \inputencoding{latin9}\lstinline!X!\inputencoding{utf8}
could be any type

We can now define the monadic DSL as:\inputencoding{latin9}
\begin{lstlisting}
type MyDSL[A] = MonadDSL[F, A]
def runMyDSL[A]: MyDSL[A] => A = MonadDSL.run(runF)
\end{lstlisting}
\inputencoding{utf8}DSL programs may be written using functor blocks. Each DSL operation
needs to be wrapped in \inputencoding{latin9}\lstinline!Op!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
...
\subsection{Preparing to handle errors after running a DSL program}
...
\texttt{\textcolor{blue}{\footnotesize{}    case Val(a) => Right(a)}}\textcolor{darkgray}{\footnotesize{}
...
So, we can generalize to any other monad $M^{A}$ instead of \texttt{\textcolor{blue}{\footnotesize{}Either{[}Err,
A{]}}} 

The resulting construction:

Start with an \textsf{``}effects constructor\textsf{''} $F^{A}$ (often an unfunctor)

Use $\text{DSL}^{F,A}$ and interpreter $\text{run}^{M,A}:\left(\forall X.F^{X}\rightarrow M^{X}\right)\rightarrow\text{DSL}^{F,A}\rightarrow M^{A}$

Note: the function of type $\forall X.\,F^{X}\rightarrow M^{X}$ does
not need to handle all possible types $X$ but only those for which
values of type $F^{X}$ can exist; so, unfunctors are not a problem
...
\subsection{The free monad in the raw tree encoding}
...
\textsf{``}free\textsf{''} indicates that the type \inputencoding{latin9}\lstinline!MonadDSL!\inputencoding{utf8}
is free of any domain-specific code and works equally well with every
DSL. We may view \inputencoding{latin9}\lstinline!MonadDSL[F, A]!\inputencoding{utf8}
as a wrapper that somehow provides the functionality of a monad to
any type constructor \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}.

A monad must support the methods \inputencoding{latin9}\lstinline!pure!\inputencoding{utf8},
\inputencoding{latin9}\lstinline!flatMap!\inputencoding{utf8}, and
\inputencoding{latin9}\lstinline!map!\inputencoding{utf8}. The type
constructor \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}
does not support these methods. In many cases, \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}
is an unfunctor and cannot possibly support the monad\textsf{'}s methods. To
see how \inputencoding{latin9}\lstinline!MonadDSL!\inputencoding{utf8}
is able to convert \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}
into a monad, let us look at the types used by \inputencoding{latin9}\lstinline!MonadDSL[F, A]!\inputencoding{utf8}.
...
Monad laws hold for DSL programs only after evaluating them; when
DSL is defined as an unevaluated tree, monad laws do not hold for
that tree
...
a $\rightarrow$ Val(a))}}{\footnotesize\par}
...
Val(a)) andThen run(ex)) }}{\footnotesize\par}

\texttt{\textcolor{blue}{\footnotesize{}  == run(ex)(prg).flatMap(a
$\rightarrow$ run(ex)(Val(a)) }}{\footnotesize\par}
...
{*}{*}{*}\footnote{\textsf{``}A function \inputencoding{latin9}\lstinline!launch()!\inputencoding{utf8}
that launches real-world missiles can run out of missiles.\textsf{''} (A quote
attributed to \index{Simon Peyton Jones}Simon Peyton Jones.)}
