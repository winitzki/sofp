...
def runComplex: PrgComplex => Complex = {
  ...
  case Phase(p)           => Complex(runComplex(p).phase, 0) // Hack: pretend the phase is a Complex.
...
make sure that we are using the DSL correctly. It would be better
if the DSL program \inputencoding{latin9}\lstinline!Read(Read(Val("file")))!\inputencoding{utf8}
failed to compile. To achieve that, we will use different Scala types
for DSL programs returning a \inputencoding{latin9}\lstinline!String!\inputencoding{utf8}
...
final case class Rotate(p: PrgComplex[Complex], a: PrgComplex[Double]) extends PrgComplex[Complex]
\end{lstlisting}
\inputencoding{utf8}The revised code of \inputencoding{latin9}\lstinline!runComplex!\inputencoding{utf8}
is simpler because hacks are no longer needed:\inputencoding{latin9}
\begin{lstlisting}
...
final case class Rotate(p: Complex, alpha: Double) extends PrgComplex[Complex]
...
def runComplex[A]: PrgComplex[A] => A = {
  ...
  case Rotate(p, alpha)   => p.rotate(alpha)
}
\end{lstlisting}
\inputencoding{utf8}
...
operations in a new type constructor that we call the \index{effect constructor}\textbf{effect
...
into a new function, \inputencoding{latin9}\lstinline!runFileC!\inputencoding{utf8}:\inputencoding{latin9}
\begin{lstlisting}
...
def runFile[A]: PrgFile[A] => A = {
  case Val(a) => a
  case bind@Bind(pa) => runFile(bind.f(runFile(pa)))
  case Op(op) => runFileC(op)     // Run the custom operation and get the result value.
}
\end{lstlisting}
\inputencoding{utf8}
