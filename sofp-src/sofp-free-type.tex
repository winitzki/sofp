...
\subsection{Fixpoint equations for recursive types}

A recursive type is usually defined by a fixpoint type equation\index{fixpoint type equation}
of the form $T\cong F^{T}$, where $F$ is a \textsf{``}structure functor\textsf{''}
that describes the structure of the type recursion. A \textsf{``}solution\textsf{''}
of the fixpoint equation $T\cong F^{T}$ is a type $T$ that is equivalent
to $F^{T}$; more precisely, there must exist two isomorphisms:
\[
\text{wrap}:T\rightarrow F^{T}\quad,\quad\quad\text{unwrap}:F^{T}\rightarrow T\quad,\quad\quad\text{wrap}\bef\text{unwrap}=\text{id}\quad,\quad\text{unwrap}\bef\text{wrap}=\text{id}\quad.
\]

A fixpoint equation may have several solutions, i.e., \emph{inequivalent}
types $T_{1}$, $T_{2}$, etc., each separately satisfying the fixpoint
equation. An example is the fixpoint equation for the \textsf{``}lazy list\textsf{''}:
\begin{equation}
...
we can implement the corresponding isomorphisms \lstinline!wrap!
and \lstinline!unwrap!. Each of these functions will either add or
remove one element at the beginning of the list. These operations
keep finite lists finite and infinite lists infinite. So, a composition
(such as, $\text{wrap}\bef\text{unwrap}$) of these isomorphisms will
act as an identity function on $L_{\text{fin}}^{A}$ or on $L_{\text{inf}}^{A}$.
