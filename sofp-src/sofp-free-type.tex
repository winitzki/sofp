...
use the notion of a \textsf{``}$P$-typeclass\textsf{''}\index{inductive typeclass}\index{typeclass!inductive}
introduced in Section~\ref{subsec:P-algebraic-typeclasses}. For
a given (covariant) functor $P$, a $P$\textbf{-typeclass} \index{$P$-typeclass}
has the instance data equivalent to a value of type $P^{A}\rightarrow A$.
...
and the instance data has type $A\times\left(A\times A\rightarrow A\right)$,
...
Usually, $P$-typeclasses impose some laws on their methods. Laws
are formulated via one or more law functions whose types are of the
form $A\times...\times A\times(P^{A}\rightarrow A)\rightarrow A\times A$.
