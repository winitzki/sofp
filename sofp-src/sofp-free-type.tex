...
meaning that it is implemented as a library in an existing programming
language (e.g., Scala), rather than as a separate new language. This
...
and extract the final \inputencoding{latin9}\lstinline!String!\inputencoding{utf8}
...
A simple runner for the DSL programs of type \inputencoding{latin9}\lstinline!PrgComplex!\inputencoding{utf8}
...
\end{lstlisting}
\inputencoding{utf8}We need to pass those functions as arguments to \inputencoding{latin9}\lstinline!run!\inputencoding{utf8}
...
\subsection{A recipe for monadic DSLs}

The previous section explained the motivation for defining the type
constructor \inputencoding{latin9}\lstinline!MonadDSL[F[_], A]!\inputencoding{utf8}.
That type constructor is called the \textsf{``}free monad on \inputencoding{latin9}\lstinline!F!\inputencoding{utf8}\textsf{''},
for reasons explained later in this chapter. The type \inputencoding{latin9}\lstinline!MonadDSL[F[_], A]!\inputencoding{utf8}
...
that will serve as the effect constructor\index{effect constructor}
of the DSL:\inputencoding{latin9}
\begin{lstlisting}
sealed trait F[_]
\end{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}The names of case classes (\inputencoding{latin9}\lstinline!SomeOperation!\inputencoding{utf8}
and so on) should be chosen for clarity.
...
\subsection{Handling errors after running a DSL program}
...
where $M$ is some monad, we hope to obtain a general runner of type
\inputencoding{latin9}\lstinline!MonadDSL[F, A] => M[A]!\inputencoding{utf8}.

To see an example of using such general runners in practice, notice
that the execution of $F$-effects may produce errors. For instance,
the runner of the \inputencoding{latin9}\lstinline!PrgFile!\inputencoding{utf8}
...
class. However, this means replacing the runner of type \inputencoding{latin9}\lstinline!PrgFileC[X] => X!\inputencoding{utf8}
by a polymorphic function of type \inputencoding{latin9}\lstinline!PrgFileC[X] => Try[X]!\inputencoding{utf8}.
For the \inputencoding{latin9}\lstinline!PrgFile!\inputencoding{utf8}
DSL, the new code looks like this:\inputencoding{latin9}
\begin{lstlisting}
...
\end{lstlisting}
\inputencoding{utf8}
Let us now generalize this code to an arbitrary monad $M$ instead
of \inputencoding{latin9}\lstinline!Try!\inputencoding{utf8}. The
code of \inputencoding{latin9}\lstinline!runTry!\inputencoding{utf8}
uses the \inputencoding{latin9}\lstinline!flatMap!\inputencoding{utf8}
method of the \inputencoding{latin9}\lstinline!Try!\inputencoding{utf8}
monad. The \inputencoding{latin9}\lstinline!Try!\inputencoding{utf8}
monad\textsf{'}s \inputencoding{latin9}\lstinline!pure!\inputencoding{utf8}
method is implemented by the \inputencoding{latin9}\lstinline!Success!\inputencoding{utf8}
case class. So, we can generalize to another monad $M$ instead of
\inputencoding{latin9}\lstinline!Try!\inputencoding{utf8} if we use
$M$\textsf{'}s \inputencoding{latin9}\lstinline!flatMap!\inputencoding{utf8}
and replace \inputencoding{latin9}\lstinline!Success!\inputencoding{utf8}
...
and \inputencoding{latin9}\lstinline!flatMap!\inputencoding{utf8})
will create some data structures (nested case classes) in memory and
will not perform any domain-specific effects or actions. The effects
will be performed at a later time by an effect runner (\inputencoding{latin9}\lstinline!runF!\inputencoding{utf8})
