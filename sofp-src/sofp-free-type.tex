...
Interpreter is parameterized by a ``value extractor'' {\footnotesize{}$\text{Ex}^{F}\triangleq\forall A.\left(F^{A}\rightarrow A\right)$}{\footnotesize\par}
...
\textbf{Reduced encoding}, as a ``smart'' structure: List(\shui,\shui,\foreignlanguage{russian}{\textcyr{\cyrc}},\shui,\foreignlanguage{russian}{\textcyr{\cyrc}})
...
The ``induction functor'' for ``semigroup over $Z$'' is {\footnotesize{}$\text{SemiG}^{X}\triangleq Z+X\times X$}{\footnotesize\par}
...
Tree encoding of ``free $C$ over $Z$'' is recursive, $\text{FreeC}^{Z}\triangleq Z+C^{\text{FreeC}^{Z}}$
...
Example: $F^{A}\triangleq A$, ``interpret'' into the contrafunctor
...
``$L$-module''). Type $P$ is an $L$-module if the monoid $L$
``acts'' on $P$ via a function act$:L\rightarrow P\rightarrow P$,
...
of type $A\rightarrow A$ in the last line, we set $q$'s type parameter
...
\emph{not} required to be a natural transformation since $P$ and
$F$ are not required to be functors). The corresponding naturality
...
A recursive type is usually defined as a fixpoint of a functor; that
is, a solution of a type equation\index{recursive type equation}
of the form $T\cong F^{T}$, where $F$ is a ``structure functor''
...
\]
representing an ``on-call'' list of values of type $A$. To get
...
Let us denote by $L_{\text{fin}}^{A}$ the type corresponding to ``finite''
...
to ``infinite'' on-call lists, i.e., those that \emph{never} stop
...
Some fixpoints represent ``larger'' types than other fixpoints.
...
but these encodings would give us no advantages. It turns out that
another, more useful Church encoding for $T$ is
\begin{equation}
...
where the type $A$ must not depend on the quantified type $X$. 

The following statement\footnote{See the paper ``A note on strong dinaturality'': \texttt{\href{https://web.archive.org/web/20110601105059/http://www.cs.ioc.ee/~tarmo/papers/fics10.pdf}{http://www.cs.ioc.ee/$\sim$tarmo/papers/fics10.pdf}}}
provides a form of the Yoneda lemma for Church encodings:
...
\end{equation}
(Strong dinaturality holds for all functions implemented via fully
parametric code; see Appendix~\ref{app:Proofs-of-naturality-parametricity}.)
...
a catamorphism\index{catamorphism}. To define that map, assume that
the fixpoint $R$ has a known function $\text{fix}_{R}:F^{R}\rightarrow R$
and write
\[
...
hold:
\begin{align*}
\text{covariant co-Yoneda identity}:\quad & \exists R.\,F^{R}\times\left(R\rightarrow A\right)\cong F^{A}\quad,\\
\text{contravariant co-Yoneda identity}:\quad & \exists R.\,H^{R}\times\left(A\rightarrow R\right)\cong H^{A}\quad.
\end{align*}
...
Note that the Scala type \lstinline!Any! closely corresponds to the
type $\exists X.\,X$, which is observationally (but not computationally)
equivalent to \lstinline!Unit! . The type \lstinline!Any! is a supertype
of all Scala types. Also, for any type $T$ there is an injective
map $T\rightarrow\exists X.\:X$, just as there is an injective map
\lstinline!T => Any! for any Scala type \lstinline!T!. This map
establishes the subtyping relation \lstinline!T <: Any!.
...
now so let's implement instead of niño Java file types let's just
