...
\end{itemize}
\begin{center}
\includegraphics[width=0.4\columnwidth]{\string"Winitzki - physics paper - sample page\string".png}\vspace{-0.6\baselineskip}
\par\end{center}
\begin{itemize}
...
\lstinline!found   : Seq[Some[V]]!\\
 \lstinline!required: Seq[Option[?V8]] where type ?V8 <: V (this is a GADT skolem)!
\end{itemize}
\end{itemize}
\end{frame}
...
\item The answers will help the programmer in designing the code
\end{itemize}
\end{frame}
...
be implemented, while \texttt{\textcolor{blue}{\footnotesize{}g}}
can be:
\end{itemize}
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}def~g{[}Z,~R,~A{]}(e:~Either{[}Z,~R~=>~A{]}):~R~=>~Either{[}Z,~A{]}~=}{\footnotesize\par}
...
\textcolor{blue}{\footnotesize{}~~c~<-~Future(...)~//~a~computation~that~is~run~asynchronously.}{\footnotesize\par}
...
\item Programmers need to develop intuition about how to implement them
\item This is a rigorous result (programmers do not need to test it)
\begin{itemize}
...
to software resemble \emph{artisanship}
\begin{center}
...
A systematic approach to understanding \texttt{\textcolor{blue}{\footnotesize{}map}}
...
\end{itemize}
\item Develop proof techniques that save time when proving the laws
\end{itemize}
\end{frame}
...
etc.
\item in some cases, we can generate code automatically
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Then the logic of types is \emph{incomplete} in those languages
\end{itemize}
...
\begin{itemize}
\item My approach forced me to formulate and prove every statement
\item Each chapter gave me at least one surprise
\begin{itemize}
...
\par\end{center}
\begin{itemize}
\item Variable shadowing and lexical scoping are the same in math usage
...
\begin{itemize}
\item Typeclasses can be viewed as partial functions from types to values
...
\item Almost all monads are not commutative, but almost all applicative
...
\item One \emph{can} define \texttt{\textcolor{blue}{\footnotesize{}fold}}
and \texttt{\textcolor{blue}{\footnotesize{}traverse}} for a breadth-first
traversal of trees
\end{itemize}
\end{frame}
...
functor, free contrafunctor, free filterable, free monad, free applicative
functor, but not free foldable or free traversable
\item ``Tagless final'' is just a Church encoding of the free monad
\item A complete proof of the correctness of the Church encoding is \emph{hard}
\begin{itemize}
\item My proof uses relational parametricity together with some obscure
results from \href{https://www.ioc.ee/~tarmo/tday-voore/vene-slides.pdf}{unpublished talk slides}
\begin{itemize}
...
\item The difficulty is at the level of undergraduate calculus
\end{itemize}
