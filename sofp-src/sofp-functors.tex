...
$B$. The \textbf{image} of $f$ (the subset of all values of type
$B$ that can be obtained as $f(a)$ for some $a^{:A}$) thus contains
a distinct value of type $B$ for each distinct value of type $A$.
So, there must exist a function that maps any $b$ from the image
of $f$ back to a value $a^{:A}$ it came from; call that function
$g^{:B\rightarrow A}$. The function $g$ must satisfy the following
law:
\[
...
\[
g\circ f=\text{id}\quad,\quad\quad\text{or equivalently}:\quad\quad f\bef g=\text{id}\quad.
\]
...
$f\bef h=\text{id}$ and $h\bef f=\text{id}$. The function $g$ is
called a \textbf{left inverse}\index{left inverse} for $f$.
