...
are ``wrappers'' because they have a suitable \lstinline!map! method.
...
(or ``sums'') are often used to represent application-specific data.
Consider the code:
\begin{lstlisting}
...
Another important example where the \lstinline!map!'s type signature
...
\lstinline!Vec3[_]! ``is'' a functor, meaning that a suitable lawful
...
\begin{enumerate}
\item Declaring a class that ``\lstinline!extends!'' another class (as
we have just seen).
...
\end{enumerate}
Since parts of disjunctive types are delared via the ``\lstinline!extends!''
keyword, subtyping works automatically for them. A\index{subtyping!for disjunctive types}
...
of $\bbnum 0$. The reason is that the type $\bbnum 0$ has no values,
...
$\text{fmap}_{L}(\text{id})=\text{id}$ (the functor $L$'s identity
law).
...
type constructors.\footnote{The name ``\textbf{contrafunctor}''\index{contrafunctor} is used
in this book as a shortened form of ``\index{contravariant functor!see ``contrafunctor''}contravariant
...
notation} with the symbol $\triangleright$ (``pipe''). The notation
...
(pronounced ``lifted to $L$''). For any functtion $f$ of type
$A\rightarrow B$, we define the function $f^{\uparrow L}$ of type
$L^{A}\rightarrow L^{B}$:
\[
...
at the same vertex (as mathematicians say, ``the diagram is \index{commutative diagram}commutative'').
...
\text{composition law}:\quad & \text{\ensuremath{\text{fmap}_{\text{Id}}}}(f\bef g)=f\bef g=\text{fmap}_{\text{Id}}(f)\bef\text{fmap}_{\text{Id}}(g)\quad.
\end{align*}
...
\text{composition law}:\quad & \text{\ensuremath{\text{fmap}_{\text{Const}}}}(f\bef g)=\text{id}=\text{fmap}_{\text{Const}}(f)\bef\text{fmap}_{\text{Const}}(g)=\text{id}\bef\text{id}\quad.
\end{align*}
The corresponding Scala code is:
\begin{lstlisting}
type Const[Z, A] = Z
...
``type equation\index{type equation}'':
\[
...
constructions for ``co-product'', ``exponential'', and ``product''.
