...
type \lstinline!A!. These data structures are parametric: they work
in the same way for every type \lstinline!A!. Working with parametric
``data wrappers'' turns out to be a powerful design pattern of functional
programming. To fully realize its benefits, we need to formulate rigorously
the concept of ``wrapping'' a value of an arbitrary type. Once the
concept of ``data wrapper'' is formalized through a set of mathematical
laws, we can extend that design pattern to many other data structures.

\section{Practical use}

\subsection{Motivation: Type constructors that wrap data}

How to formalize the idea of ``parametrically wrapping data''? An
intuition behind ``wrapping'' is that the wrapped data is ``still
there'', i.e.~we should be able to \emph{manipulate} the data held
within the wrapper. In functional programming, to ``manipulate''
means to apply functions to data.

Let us look at some type constructors in the Scala library, such as
...
For example, the Scala code \lstinline!List.fill(10)(0)! creates
a sequence with ten values of type \lstinline!Int!, each value equal
to \lstinline!0!.
...
returns a non-empty option when the first element exist.
...
features \textendash{} reading data out of the wrapper, inserting
or deleting data, waiting until data becomes available etc., \textendash{}
will be implemented by methods specific to each wrapper type.
...
Note that the type signature of \lstinline!fmap!,
\[
...
is called \textbf{lifting}\index{lifting} because it ``lifts''
a function $f^{:A\Rightarrow B}$ operating on simple values into
a function operating on \lstinline!Option!-wrapped values. 
...
$f$ can be applied to wrapped data after lifting; the corresponding
...
\end{wrapfigure}%
It would be confusing and counter-intuitive if the expression \lstinline!c.map(x => x + 2)!
...
and write the Scala expressions:\begin{wrapfigure}{l}{0.33\columnwidth}%
\vspace{-0.8\baselineskip}
\begin{lstlisting}
...
intuitions about data transformations would be incorrect. In other
words, violations of the composition law prevent us from analyzing
the code via mathematical reasoning about the transformation of data
values.
...
\begin{itemize}
\item a data type with a type parameter, e.g.~\lstinline!L[A]!, which
we may also denote by $L^{\bullet}$ (in Scala, \lstinline!L[_]!)
if we do not need to specify the name of its type parameter,
\item having a fully parametric function \lstinline!fmap! with type signature
\[
...
a path in the diagram. For example, a type diagram for the composition
law~(\ref{eq:f-composition-law-functor-fmap}) is
\[
...
\]
Type diagrams are easier to read when using the forward composition
$f\bef g$ because the order of edges is the same as the order of
functions in the composition. To see this, compare Eq.~(\ref{eq:f-composition-law-functor-fmap})
...
defined in the application code, such as case classes with type parameters,
...
Polynomial type\index{types!polynomial} constructors (whose type
expressions are built with primitive types, type parameters, products,
and disjunctions or ``sums'') are often used to represent application-specific
...
method, rather than defiing \lstinline!map! as a function, which
...
that \lstinline!Counted[_]! is a functor. To be sure, we need to
...
In Scala, the identity law means that for all \lstinline!n:Int! and
...
We will prove later that all polynomial type constructors have a unique
...
Define a list of odd length as a recursive type $\text{LO}^{\bullet}$,
\begin{align}
...
laws), but the error can be fixed: a different implementation of \lstinline!map!
...
type signature cannot be implemented are \index{generalized algebraic data types}\textbf{generalized
...
for us to produce such a function because the type \lstinline!B!
is unknown and no values of type \lstinline!B! are given. 
...
function. However, we will say ``the type constructor $L^{\bullet}$
...
The reason for this failure is complicated.\footnote{The code was published by Pawel Szulc at \texttt{\href{https://gist.github.com/tpolecat/7401433}{https://gist.github.com/tpolecat/7401433}}}
...
will have a mathematically correct \lstinline!equals! operator. Here
is an artificial example of a type whose \lstinline!.equals! method
...
operation (or of any other operation). The \lstinline!map! operation
...
implementation of certain operations violate some other laws. For
...
\end{align}
The type signature of \lstinline!contrafmap! has the form of a ``lifting'':
functions of type \lstinline!B => A! are lifted into the type \lstinline!C[A] => C[B]!.
The Scala code for \lstinline!contrafmap! is
\begin{lstlisting}
...
is called a \textbf{contrafunctor}\index{contrafunctor} if athe identity
...
as a subtype of \lstinline!Counted[AtMostTwo]!:
\begin{lstlisting}
...
to $R$? How can we recognize quickly that a given type constructor
is a functor or a contrafunctor? To answer these questions, we will
systematically build up various type expressions for which the functor
or contrafunctor laws hold. 
...
\end{lstlisting}
To take advantage of this syntax, we can use the \index{forward notation}forward
...
{\color{greenunder}\text{compute composition}:}\quad & =(a^{:A}\times b_{1}^{:B}\times b_{2}^{:B}\Rightarrow f(a)\times b_{1}\times b_{2})\quad.
\end{align*}
...
\end{equation}
The following diagram shows the relationships between various \lstinline!bimap!
and \lstinline!fmap! functions:
\[
...
{\color{greenunder}\text{use Eq.~(\ref{eq:f-definition-of-bimap})}:}\quad & =\text{fmap}_{F^{\bullet,B}}(\text{id})\bef\text{fmap}_{F^{C,\bullet}}(\text{id})\\
...
hold automatically? The calculation for the example $F^{A,B}\triangleq A\times B\times B$
shows how the two \lstinline!fmap! functions work on different parts
of the data structure of $F^{A,B}$. It turns out\footnote{The proof is beyond the scope of this chapter; see \texttt{\href{https://byorgey.wordpress.com/2018/03/30/}{https://byorgey.wordpress.com/2018/03/30/}}}
...
to verify the \lstinline!bimap! laws: it is sufficient to know that
$F^{\bullet,B}$ and $F^{A,\bullet}$ are lawful functors.
...
Functional programming languages support the five fundamental type
constructions shown in Table~\ref{tab:ch-correspondence-type-notation-CH-propositions}
...
We have seen that type constructors such as $L^{A}\triangleq A\times A\times A$
are functors. The next construction (the \index{product functor}\textbf{product
functor}) shows that this is a general property.
...
\]
or equivalently using the pipe ($\triangleright$) operation,
\begin{equation}
...
\begin{align*}
 ...
 & (a\times c)\triangleright p\boxtimes q\triangleq\left(a\triangleright p\right)\times\left(b\triangleright q\right)\quad.
\end{align*}
...
\end{align*}
If we use the function product notation, the calculations are shorter:
\begin{align*}
...
{\color{greenunder}\text{composition of functions under }\boxtimes:}\quad & =(f^{\uparrow L}\bef g^{\uparrow L})\boxtimes(f^{\uparrow M}\bef g^{\uparrow M})\\
{\color{greenunder}\text{composition laws for }L,M:}\quad & =(f\bef g)^{\uparrow L}\boxtimes(f\bef g)^{\uparrow M}=(f\bef g)^{\uparrow P}\quad.
\end{align*}
For comparison, the same derivation using the Scala code syntax will
look like this,
\begin{lstlisting}
...
\]
By the convention of the piping notation, we have
\[
x\triangleright f\triangleright g=x\triangleright f\bef g=x\triangleright(f\bef g)=(f\bef g)(x)=g(f(x))\quad.
\]
We will often use this convention in code derivations.
...
Omitting the type annotations, we write the code of \lstinline!fmap(f)!
as
\begin{equation}
f^{\uparrow L}\triangleq\begin{array}{||cc|}
f^{\uparrow P} & \\
 & f^{\uparrow Q}
...
\end{array}\\
{\color{greenunder}\text{matrix composition}:}\quad & =\begin{array}{||cc|}
f^{\uparrow P}\bef g^{\uparrow P} & \\
 & f^{\uparrow Q}\bef g^{\uparrow Q}
\end{array}\\
...
The last two statements show that any type constructor is a functor
if it is built up only using primitive types, type parameters, products
and co-products, such as $L^{A}\triangleq\bbnum 1+(\text{String }+A)\times A\times\text{Int}+A$.
Functors of this kind are called \index{polynomial functors}\textbf{polynomial
functors} because they correspond to ordinary algebra's notion of
polynomial functions of the type parameter $A$. The type notation
with its symbols ($+$, $\times$) makes this analogy visually clear.
...
generalizes this to arbitrary contrafunctors $C^{A}$ used as arguments
of function types.
...
Since both List and Option have a .map method, we may write code such
as
\begin{lstlisting}
...
\end{lstlisting}
The code \lstinline!p.map(_.map(f))! lifts a function $f^{:A\Rightarrow B}$
...
\lstinline!.map! function. So, the composition of functors is always
a functor.
...
\begin{align*}
 ...
 & \text{fmap}_{L}\triangleq\text{fmap}_{Q}\bef\text{fmap}_{P}\quad,\\
{\color{greenunder}\text{in an equivalent shorter notation}:}\quad & f^{\uparrow L}\triangleq(f^{\uparrow Q})^{\uparrow P}\triangleq f^{\uparrow Q\uparrow P}\quad.
\end{align*}
...
For the regular-shaped tree, the construction~(\ref{eq:f-def-recursive-functor})
is insufficient: no bifunctor $S^{A,L^{A}}$ can replace the type
argument in $L^{A}$ by $L^{A\times A}$. To see that, consider that
$S^{A,L^{A}}$ is an application of a type-level function $S^{\bullet,\bullet}$
to its two type parameters, which are set to $A$ and $L^{A}$. In
Scala syntax, $S^{A,L^{A}}$ is written as \lstinline!S[A,L[A]]!.
...
by adding another arbitrary functor, $P$, in the type argument of
$L^{\bullet}$:
\begin{equation}
...
If $S^{A,B}$ is a contrafunctor with respect to $A$ and a functor
with respect to $B$ then the recursively defined type constructor
$C^{A}$ is a contrafunctor,
\[
C^{A}\triangleq S^{A,C^{A}}\quad.
\]
Given the functions \lstinline!contrafmap!$_{S^{\bullet,R}}$ and
\lstinline!fmap!$_{S^{A,\bullet}}$ for $S$, we implement \lstinline!contrafmacp!$_{C}$
as
\begin{align*}
...
def xmap_S[A,B,Q,R](f: B => A)(g: Q => R): S[A, C[A]] => S[B, C[B]] = ???    // Must be defined.

def contrafmap[A, B](f: B => A): C[A] => C[B] = { ca: C[A] =>
  val sbcb: S[B, C[B]] = xmap_S(f)(contrafmap(f))(ca.x)      // Recursive call to contrafmap(f).
...
To verify the composition law:
\begin{align*}
{\color{greenunder}\text{expect to equal }(g^{\downarrow C}\bef f^{\downarrow C}):}\quad & (f^{:D\Rightarrow B}\bef g^{:B\Rightarrow A})^{\downarrow C}=\text{xmap}_{S}(f\bef g)(\gunderline{\overline{\text{contrafmap}_{C}}\gunderline{(f\bef g)}})\\
...
{\color{greenunder}\text{composition laws for }\text{xmap}_{S}:}\quad & =\gunderline{\text{xmap}_{S}(g)(\overline{\text{contrafmap}_{C}}(g))}\bef\gunderline{\text{xmap}_{S}(f)(\overline{\text{contrafmap}_{C}}(f))}\\
{\color{greenunder}\text{definition of }^{\downarrow C}:}\quad & =g^{\downarrow C}\bef f^{\downarrow C}\quad.
\end{align*}


\subsection{Solved examples: How to recognize functors and contrafunctors}
...
expressions. We can see from Tables~\ref{tab:f-Functor-constructions}\textendash \ref{tab:f-Contrafunctor-constructions}
that \emph{every} one of the five basic type constructions (Section~\ref{subsec:Type-notation-and-standard-type-constructions})
...
in Tables~\ref{tab:f-Functor-constructions}\textendash \ref{tab:f-Contrafunctor-constructions}:
\begin{itemize}
\item Primitive types $\bbnum 1$, \lstinline!Int!, \lstinline!String!,
etc.~can be viewed as constant functors and as constant contrafunctors
...
$F^{A}\triangleq A\times A\times A$. Since $F^{A}$ is covariant
in $A$, $F^{A}\Rightarrow\text{Int}$ is contravariant in $A$.
\item Nested type constructors flip each other's variance: if we know that
$F^{A}$ is contravariant in $A$ then $F^{A\Rightarrow\text{Int}}$
is covariant in $A$.
\end{itemize}
...
function arrows:
\[
\left(\left(\underset{+}{A}\Rightarrow\underset{+}{A}\Rightarrow\underset{-}{R}\right)\Rightarrow\underset{+}{R}\right)\times\underset{+}{A}+\left(\bbnum 1+\underset{-}{R}\Rightarrow\underset{+}{A}+\text{Int}\right)+\underset{+}{A}\times\underset{+}{A}\times\text{Int}\times\text{Int}\quad.
\]
...
the techniques explained in this and the previous chapters: from the
type signature of \lstinline!map!,
\[
\text{map}_{Z}:Z^{A,R}\Rightarrow\left(A\Rightarrow B\right)\Rightarrow Z^{B,R}\quad,
\]
we would derive a fully parametric, information-preserving implementation
...
This requires a lot of work for a complicated type constructor such
as $Z^{A,R}$.
...
Each step provides both a fragment of the implementation of \lstinline!map!
and a proofs that the functor laws hold up to that step. In this way,
...
\]
Mark the covariant and the contravariant positions in the type expression:
\[
...
\]
In the type expression $Z\Rightarrow\text{Int}\Rightarrow Z\Rightarrow\text{Int}\times A$,
both $Z$ positions are contravariant since the function arrows are
curried rather than nested. We see that $A$ is always in covariant
positions ($+$) while $Z$ is always in contravariant positions ($-$).
It follows that $G^{A,Z}$ is covariant in $A$ and contravariant
in $Z$. 
...
\text{where } & G_{1}^{A}\triangleq\text{Int}+A\quad\text{ and }G_{2}^{A,Z}\triangleq\bbnum 1+(Z\Rightarrow\text{Int}\Rightarrow Z\Rightarrow\text{Int}\times A)\quad.
\end{align*}
...
disjunctive type containing a curried function type, ultimately returning
a product $\text{Int}\times A$. This tells us to use the ``co-product
functor'', the ``exponential functor'', and the ``product functor''
constructions. So, we note which functor constructions are needed
at each step as we decompose $G_{2}^{A,Z}$:
\begin{align*}
...
{\color{greenunder}\text{product functor}:}\quad & G_{6}^{A}\cong\text{Int}\times A\quad.
\end{align*}
...
It remains to derive the code for the \lstinline!map! method of $G$.
Each of the functor constructions will combine the \lstinline!map!
implementations from previously defined functors into a new \lstinline!map!
implementation, so we just need to substitute the codes in the order
of constructions. The code is shorter if we define \lstinline!fmap!
rather than \lstinline!map! and use the notations $f^{\uparrow L}\triangleq\text{fmap}_{L}(f)$
...
In each case, assume that the given type constructors $F$ and $G$
already satisfy the appropriate laws, implement the required \lstinline!fmap!
or \lstinline!contrafmap! function, and prove that the appropriate
laws hold for the new type constructor $L$. Write the implementations
both in Scala and in the code notation.
...
\textbf{(e)} $L^{A}\triangleq F^{A}\Rightarrow G^{A}$ is a contrafunctor
when $F$ is a functor and $G$ is a contrafunctor.
...
\textbf{(a)} $F^{A}\triangleq\text{Int}\times A\times A+(\text{String}\Rightarrow A)\times A$.
...
\subsection{Profunctors\index{profunctors}}
...
Setting the type parameter $Z=A$, we will obtain the \lstinline!xmap!
...
parameters:
\begin{align*}
{\color{greenunder}\text{identity law for }P:}\quad & \text{xmap}_{P}(\text{id}^{:A\Rightarrow A})(\text{id}^{:A\Rightarrow A})=\text{id}\quad,\\
{\color{greenunder}\text{composition law for }P:}\quad & \text{xmap}_{P}(f_{1}^{:B\Rightarrow A})(g_{1}^{:A\Rightarrow B})\bef\text{xmap}_{S}(f_{2}^{:C\Rightarrow B})(g_{2}^{:B\Rightarrow C})=\text{xmap}_{S}(f_{2}\bef f_{1})(g_{1}\bef g_{2})\quad.
\end{align*}
...
\[
P^{A}\triangleq\left(\left(\bbnum 1+A\times A\right)\Rightarrow A\right)\times A\Rightarrow\bbnum 1+\left(A\Rightarrow A+\text{Int}\right)\quad.
\]
...
which will be covariant in $A$ and contravariant in $Z$. parameter.
Since $\tilde{P}^{A,Z}$ will be a functor in $A$ and a contrafunctor
in $Z$, we will be able to define a function \lstinline!xmap!$_{\tilde{P}}$
satisfying the identity and composition laws. Setting $Z=A$, we will
obtain a lawful \lstinline!xmap!$_{P}$, which makes $P$ a profunctor.
Thus, any exponential-polynomial type constructor is a profunctor.
...
function is injective. An example of a subtyping relation with a surjective
...
are not necessarily injective or surjective; they can be anything
``in between''.
