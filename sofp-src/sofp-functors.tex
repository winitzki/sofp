...
code even if it is not tail-recursive. Once the laws are shown to
hold for that code, the programmer is free to use an equivalent but
more efficient implementation of the same function.
...
data type (see Section~\ref{subsec:The-recursive-type-List}). We
view \lstinline!List! as a recursive polynomial functor whose \lstinline!map!
...
Both sides of the law are functions, and we need to apply them to
...
are the only ways of creating a \lstinline!List! value. The base
case is an empty list (\lstinline!s == Nil!). To prove the base case,
we apply the code of \lstinline!fmap! to \lstinline!Nil!. The code
goes into the first line: \lstinline!case Nil => Nil!. So, we have
proved that \lstinline!fmap(identity)(Nil) == Nil!.
...
in its right-hand side. For the same reason, lists and trees are recursive
polynomial functors.
...
\end{lstlisting}
However, the Scala compiler will admit this kind of code when $P$
is a \textsf{``}subtype\textsf{''} of $Q$. We may imagine that the compiler automatically
converts values of type $P$ into values of type $Q$ using a fixed
\textsf{``}conversion function\textsf{''} (of type $P\rightarrow Q$) that is somehow
already available.

It is convenient to \emph{define} subtyping through the existence
...
of a type $Q$ (denoted $P\lesssim Q$) if there exists a designated\index{subtyping!subtype conversion function|textit}
...
a value of type $P$ is used instead of a required value of type $Q$. 

Programming languages that support subtyping usually do not define
the subtype conversion functions explicitly. Nevertheless, whenever
$P\lesssim Q$ we can always recover the underlying conversion function
$c^{:P\rightarrow Q}$. We just apply an identity function $\text{id}^{:Q\rightarrow Q}$
to a value of type $P$ (since we may use values of type $P$ instead
...
are chosen to be \emph{identity} \emph{functions} that only reassign
the types. Let us look at some examples of subtyping of that kind. 
...
whose conversion function is an identity function. To see that, let
us implement the conversion functions from each of the three case
classes to \lstinline!AtMostTwo!. The required functions reassign
the types but perform no transformations on the data:
\begin{lstlisting}
def f0: Zero => AtMostTwo  = { case Zero()      => Zero()    }
def f1: One  => AtMostTwo  = { case One(x)      => One(x)    }
def f2: Two  => AtMostTwo  = { case Two(x, y)   => Two(x, y) }
\end{lstlisting}
The implementations of these subtype conversions indeed look like
the code of identity functions. In the matrix code notation, we write:
\begin{align*}
...
\end{align*}
The matrix notation emphasizes that the code consists of identity
functions with reassigned types.
...
of type \lstinline!Two!. So, the implementation of \lstinline!p2q!
...
machine code, and the subtype conversion does not lead to any decrease
in performance.

The void type (\lstinline!Nothing!, denoted by $\bbnum 0$ in the
type notation) is special:\index{void type} any type $A$ is a subtype
of $\bbnum 0$. The reason is that the type $\bbnum 0$ has \emph{no}
values, so a conversion function \lstinline!absurd! $:\bbnum 0\rightarrow A$
(shown in Example~\ref{subsec:ch-Example-type-identity-0-to-A})
never actually needs to be applied. The Scala compiler recognizes
this automatically.
...
between the types $L^{P}$ and $L^{Q}$. If $f$ is an identity  function
then $\text{fmap}_{L}(f)$ is also an identity function because $\text{fmap}_{L}(\text{id})=\text{id}$
...
type constructors.\footnote{The name \textsf{``}\textbf{contrafunctor}\textsf{''}\index{contrafunctor} is used
...
\end{lstlisting}
The code will be clearer if we implement \lstinline!fmap! separately
...
Sometimes when $P$ is a subtype of $Q$ (denoted as $P\lesssim Q$),
...
$P\rightarrow Q$ is injective and embeds all information from a value
of type $P$ into a value of type $Q$. This kind of subtyping works
for parts of disjunctive types, such as \lstinline!Some[A] <: Option[A]!
(in the type notation, $\bbnum 0+A\lesssim\bbnum 1+A$). The set of
all values of type \lstinline!Some[A]! is a subset of the set of
values of type \lstinline!Option[A]!. Then the conversion function
is injective because it is an identity function, $\bbnum 0+x^{:A}\rightarrow\bbnum 0+x$,
that merely reassigns types.

However, subtyping does not always mean that the conversion function
is injective. An example of subtyping with a \emph{surjective} conversion
function is between the function types $P\triangleq\bbnum 1+A\rightarrow\text{Int}$
...
\end{lstlisting}
In the code notation, $p\rightarrow x\rightarrow p(x)$ is just an
expanded form of the identity function $p\rightarrow p$. 

Note that functions of type $P$ accept \lstinline!None! as an argument,
while functions of type $Q$ do not. So, there are \emph{more values}
of type $P$ than values of type $Q$. The conversion function $\text{p2q}:P\rightarrow Q$
...
Another example of subtyping with a surjective subtype conversion
function is found when using object-oriented inheritance.\index{object-oriented inheritance}
...
but $r_{2}:P_{2}\rightarrow Q_{2}$ is surjective (as we have seen,
this can happen). Now consider the product types $P_{1}\times P_{2}$
and $Q_{1}\times Q_{2}$. The product type $A\times B$ is covariant
in both type parameters, so $P_{1}\times P_{2}\lesssim Q_{1}\times Q_{2}$.
The conversion function is the pair product $r_{1}\boxtimes r_{2}:P_{1}\times P_{2}\rightarrow Q_{1}\times Q_{2}$
...
$B$. The\index{image of a function} \textbf{image} of $f$ (the
...
\]
We note that $g$ may be a partial function:\index{partial function}
$g$ needs to be defined only for a subset of values of type $B$,
namely the values within the image of $f$. Despite the equation $f\bef g=\text{id}$,
the function $g$ is not an inverse of $f$. (An inverse\index{inverse function}
function of $f$ must be a \emph{total} (not a partial) function $h$
satisfying $f\bef h=\text{id}$ and $h\bef f=\text{id}$.) The function
$g$ is called a \textbf{left inverse}\index{left inverse} of $f$.
...
Now we apply this trick to functions lifted to the functor $L$. To
...
\]
So, $g$ is a \index{right inverse}\textbf{right inverse} for $f$.
