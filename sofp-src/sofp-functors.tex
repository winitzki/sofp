...
are chosen to be \emph{identity} functions that merely reassign the
types. Let us look at some examples of type conversion functions of
that kind. 

Within the focus of this book, the main example of subtyping is with
disjunctive types (some other examples are shown in Section~\ref{subsec:Subtyping-with-injective}).
Consider this definition of a disjunctive type:
\begin{lstlisting}
...
A\index{subtyping!for disjunctive types} function with argument of
type \lstinline!AtMostTwo! can be applied to a value of type \lstinline!Two!
...
$B$ are equivalent then the types $L^{A}$and $L^{B}$ are also equivalent.
...
form of identity function $p\rightarrow p$. 

However, functions of type $P$ must accept \lstinline!None! as an
argument, while functions of type $Q$ do not. So, there are more
values of type $P$ than values of type $Q$. The conversion function
$\text{p2q}:P\rightarrow Q$ is surjective: for any function $f$
of type \lstinline!Some[A] => Int!, we can find a function $g$ of
type \lstinline!Option[A] => Int! such that $\text{p2q}\,(g)=f$.
(The function $g$ is defined as \lstinline!g(Some(x)) = f(x)! and
\lstinline!g(None) = 123!, say.)
...
A useful property of functor liftings is that they preserve injectivity
...
of $f$. (An inverse\index{inverse function} function of $f$ must
...
inverse}\index{left inverse} of $f$.
