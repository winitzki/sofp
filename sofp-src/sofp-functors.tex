...
of a type $Q$ if there exists a designated\index{subtyping!type conversion function}
\textbf{type conversion} function of type $P\rightarrow Q$. The compiler
will automatically insert that conversion function whenever a value
of type $P$ is used instead of type $Q$. 
...
will automatically use the designated type conversion function (of
type $P\rightarrow Q$) before applying the function \lstinline!h!.
...
$Q$ to which type conversions may apply. Often, the type conversion
functions are chosen to be \emph{identity} \emph{functions} that only
reassign the types. Then the programmer does not need to declare the
type conversion functions explicitly. Let us look at some examples
of type conversions of that kind. 
...
\end{lstlisting}
The implementations of these type conversions indeed look like the
code of identity functions. In the matrix notation, we can write:
\begin{align*}
...
Another example is a subtyping relation between \emph{function} types.
Consider these types $P$ and $Q$:
\begin{lstlisting}
...
\begin{enumerate}
\item Declaring a class that \textsf{``}\lstinline!extends!\textsf{''} another class.
...
type conversion function \lstinline!f2! shown above to convert a
value of the type \lstinline!Two! into a value of the type \lstinline!AtMostTwo!.
...
types. So, the compiler does not need to insert any additional machine
code, and the type conversion does not lead to any decrease in performance.
...
\subsection{Functor laws in the short code notation}
...
\]
In the lifting notation, the identity and composition laws for a functor
...
\end{align*}
Statement~\ref{subsec:functor-Statement-functor-exponential} proves
that placing a contravariant functor behind a function arrow creates
a covariant functor. One can prove in a similar way that a function
arrow turns covariance into contravariance (Exercise~\ref{subsec:functor-Exercise-contrafunctor-exponential}).
...
However, subtyping does not necessarily mean that the conversion function
...
is found when using object-oriented inheritance.\index{object-oriented inheritance}
...
The type conversion function is $\pi_{2}:\text{String}\times\text{Long}\rightarrow\text{Long}$,
