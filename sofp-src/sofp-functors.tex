...
expected only if that operation obeys the laws of \textbf{identity}\index{identity laws!of equality}
(if $x=y$ then $f(x)=f(y)$ for any $f$), \textbf{reflexivity}\index{reflexivity law|textit}
...
We will give two non-trivial examples of types that violate the laws
of equality. The first example\footnote{This example is based on a comment by \index{Pawel@Pawe\l{} Szulc}Pawe\l{}
