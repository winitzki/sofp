...
\]
This is called the \textbf{composition law}\index{composition law}.
...
Applications often use polynomial type\index{types!polynomial} constructors
...
\]
showing that \lstinline!Counted! is a polynomial\index{types!polynomial}
...
The type notation for the type constructor \lstinline!Vec3[A]! is
\[
...
The type constructor \lstinline!LO[A]! is a \textbf{recursive} \index{polynomial functors!recursive}\textbf{polynomial}
...
Similarly, one could define the map method for the List{[}A{]} type
...
\end{lstlisting}
The law does not hold because \lstinline!Set[Weird]! fails to see
...
be \textbf{lawful}\index{lawful functor}, i.e.~must satisfy the
...
A type constructor is called a \textbf{contrafunctor}\index{contrafunctor}
...
a type $Q$ if there exists a\index{type conversion function} \textbf{type
...
Consider this implementation of \lstinline!map! for the type \lstinline!Option[A]!:
\begin{lstlisting}
...
To derive the code of \lstinline!fmap! for \lstinline!Q!, we begin
...
\subsection{Formulation of laws}
...
The type\index{identity functor} constructor $\text{Id}^{A}\triangleq A$
...
To verify the identity law for $P$:
\begin{align*}
