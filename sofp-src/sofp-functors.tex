...
\vspace{-0.5\baselineskip}
\end{wrapfigure}%
To take advantage of this syntax, we can use the \index{pipe notation}\textbf{pipe
...
scheme}) with two type parameters $A,R$. If the type constructor
$S^{\bullet,\bullet}$ is given, the Scala code defining $L^{\bullet}$
can be written as:
\begin{lstlisting}
...
\end{lstlisting}
Different choices of $P^{\bullet}$ will define regular-shaped trees
