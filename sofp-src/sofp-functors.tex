...
\begin{align*}
 & \quad{\color{greenunder}\quad\text{expect }\text{fmap}_{F^{A,\bullet}}(g)\bef\text{fmap}_{F^{\bullet,D}}(f):}\quad\\
 & \text{fmap}_{F^{\bullet,B}}(f^{:A\rightarrow C})\bef\text{fmap}_{F^{C,\bullet}}(g^{:B\rightarrow D})\\
...
def fmap_L[A, B](f: A => B): L[A] => L[B] = { case L(x) =>
    ...
    L(newX)   // Need to wrap the value of type S[B, L[B]] in the type constructor L.
}
\end{lstlisting}
...
def cmap_C[A, B](f: B => A): C[A] => C[B] = { case C(x) =>
  ...
  C(sbcb) // Need to wrap the value of type S[B, C[B]] in the type constructor C.
}
\end{lstlisting}
