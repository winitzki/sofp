...
type notation) is special:\index{void type} any other type $A$ is
...
\end{lstlisting}
if we assume that the pipe symbol $\left(\triangleright\right)$ groups
weaker than the composition symbol $\left(\bef\right)$.
...
\end{equation}
where $S^{A,R}$ is a suitably chosen type constructor (called a \index{recursion scheme}\textbf{recursion
