...
of run-time errors (as seen in some programming languages: \textsf{``}\emph{object
}\lstinline!x!\emph{ does not have method }\lstinline!y!\textsf{''} or
\textsf{``}\emph{undefined is not an object}\textsf{''}) that occur due to wrong types. 
...
code is an example of a destructuring definition:
\begin{lstlisting}
case class MySock(size: Double, color: String)
case class BagOfSocks(sock: MySock, count: Int)
...
for sequences, and \lstinline!get! for dictionaries.
...
ignore the given value \lstinline!a: A!, which loses information.\index{information loss}
...
this book calls them \textsf{``}named unit\textsf{''} types.\index{unit type!named}
