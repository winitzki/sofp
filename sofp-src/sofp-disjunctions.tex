...
Scala's \textbf{case classes}\index{case class} can be seen as \textsf{``}tuples
...
contains \textsf{``}no data\textsf{''} \textemdash{} may appear to be useless. It
...
Scala's \textsf{``}case classes\textsf{''} got their name from their use in \lstinline!case!
...
to \lstinline!.map! in this code, namely \textsf{``}if \lstinline!None!
then return \lstinline!None!, else do a computation\textsf{''}. So we may
