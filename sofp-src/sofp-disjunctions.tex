...
scala> val poincar$\text{\'e}$ = Person(firstName = "Henri", lastName = "Poincar$\text{\'e}$", 165)
...
lists. Type signature and a sample test:
\begin{lstlisting}
...
ignore the given value \lstinline!a:A!, i.e.~we will lose information.
...
\lstinline!None! would unnecessarily lose information\index{information loss},
so we write
\begin{lstlisting}
def f2[A, B]: (Option[A], Option[B]) => Option[(A, B)] = {
...
expression or function in our program. So, the proposition ``the
function \emph{can} return a value of type \lstinline!RootsOfQ!''
