...
example: monad \textsf{``}interpreters\textsf{''} $M^{A}\rightarrow N^{A}$ are monadic
morphisms
...
\textsf{``}Lifting\textsf{''} \textendash{} a monadic morphism $\text{lift}_{L}^{M}:M^{A}\leadsto T_{L}^{M,A}$

\textsf{``}Base lifting\textsf{''} \textendash{} a monadic morphism $\text{blift}:L^{A}\leadsto T_{L}^{M,A}$
...
\textsf{``}Base runner\textsf{''}: lifts $L^{A}\leadsto A$ into a monadic morphism
...
$Q^{A}\triangleq Z\rightarrow M^{A}$ to $M^{A}$.
...
{\color{greenunder}\text{definition of }\text{ftn}_{M\times M}:}\quad & =\Delta\bef\left(\Delta^{\uparrow M}\boxtimes\Delta^{\uparrow M}\right)\bef\big(\pi_{1}^{\uparrow M}\bef\text{ftn}_{M}\big)\boxtimes\big(\pi_{2}^{\uparrow M}\bef\text{ftn}_{M}\big)\\
...
is a monadic morphism. Same for $\pi_{2}:M^{\bullet}\times N^{\bullet}\leadsto N^{\bullet}$.

\subparagraph{Proof}

It is sufficient to verify the laws for $\pi_{1}$; the proof for
$\pi_{2}$ will be analogous. The identity law:
\begin{align*}
...
$\Delta$, $\pi_{1}$, and $\pi_{2}$ are monadic morphisms by Statements~\ref{subsec:Statement-M-to-M-times-M-is-monadic-morphism}
and~\ref{subsec:Statement-projection-is-monadic-morphism}. The function
product $\pi_{1}\boxtimes\pi_{2}$ is a monadic morphism by Statement~\ref{subsec:Statement-product-of-monadic-morphisms}.
So $\sigma$ is a composition of monadic morphisms; by Statement~\ref{subsec:Statement-monadic-morphism-composition},
...
\]
are monadic morphisms. In other words, all \textsf{``}runners\textsf{''} $\theta$
...
map to flatten ax is monadic morphism so it\textsf{'}s not just the function
or morphism but it\textsf{'}s a monadic morphism in other words it\textsf{'}s a morphism
