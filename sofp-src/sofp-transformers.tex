...
the stack changes. It would be convenient if the programmer could
merely indicate which monad\textsf{'}s effects to run, without writing the
code of the runner by hand.  However, it does not seem possible to
define a \textsf{``}universal runner\textsf{''} that is fully parametric in the chosen
monad. 

As an example, consider a monad stack of type \lstinline!EitherT[E, ReaderT[R, Option, *], A]!.
...
$L\leadsto K\varangle M$ if there is already a lift relation $L\leadsto K$:
\begin{lstlisting}
...
relation. Those instances can be defined as implicit values in a monad
transformer library. Then the user may declare an arbitrary monad
stack and use automatic lifts between any levels in a stack.
...
The MTL-style\footnote{The name MTL stands for \textsf{``}monad transformer library\textsf{''}; see \texttt{\href{http://book.realworldhaskell.org/read/monad-transformers.html}{http://book.realworldhaskell.org/read/monad-transformers.html}}}
monadic\index{monadic program!MTL-style} programming is another way
of achieving automatic lifting of monadic programs to arbitrary monad
...
monad having the type $\text{Reader}^{R,A}\triangleq R\rightarrow A$,
the \lstinline!State! monad having the type $\text{State}^{S,A}\triangleq S\rightarrow A\times S$,
and so on), define some typeclasses (\lstinline!ReaderMonadOps!,
\lstinline!StateMonadOps!, etc.) that encapsulate all the \textsf{``}effectful
operations\textsf{''} specific to each monad. Refactor all monadic code to
use only the operations from the provided typeclasses. 
...
\end{align*}
A functor block using the \lstinline!State! monad can then be written
without referring to the type signature $S\rightarrow A\times S$:
\begin{lstlisting}
...
\end{lstlisting}
The two functions \lstinline!get! and \lstinline!set! are sufficient
to manipulate the internal state in an arbitrary way. So, these two
functions are the \textsf{``}effectful operations\index{monad!effectful operations}\textsf{''}
...
\]
These two functions show how the result value is computed and how
the state is updated. Now we can express $q$ equivalently as a monadic
program involving only \lstinline!map!, \lstinline!flatMap!, and
the operations \lstinline!get! and \lstinline!set!:
\begin{lstlisting}
q == for {
  s1 <- get
  s2 = f2(s1)
...
\begin{lstlisting}
implicit def stateTMonadOps[S, Q[_]](implicit monadQ: Monad[Q]): StateMonadOps[StateT[Q, S, *], S] = new StateMonadOps[StateT[Q, S, *], S] {
...
monad $N$. By the inductive assumption, we already have a typeclass
instance of \lstinline!StateMonadOps! for the monad \lstinline!Q!.
Using the \lstinline!flift! method of the transformer \lstinline!OtherT!,
we write:
\begin{lstlisting}
...
To use the MTL style in practice, one needs a library (such as \texttt{cats-mtl}\footnote{See \texttt{\href{https://typelevel.org/blog/2018/10/06/intro-to-mtl.html}{https://typelevel.org/blog/2018/10/06/intro-to-mtl.html}}})
that: 
\begin{itemize}
\item defines sufficiently many operations for each standard monad, so that
any monadic program may be expressed without using the monad types
explicitly;
\item defines the operation typeclasses for each standard monad;
\item implements instances of all monad operation typeclasses for \emph{all}
possible monad stacks.
\end{itemize}
While the MTL style provides automatic lifting with somewhat less
code than the direct approach of lifting relation (as developed in
the previous section), two problems remain. The first problem is the
large number of typeclass instances that the libraries need to maintain.
As we have seen in the example with \lstinline!StateMonadOps!, just
two implicit functions per monad type are in principle sufficient
to define the operation typeclasses for all monad stacks. However,
for reasons of run-time efficiency, libraries sometimes define a large
number of operation typeclass instances for specific combinations
...
of typeclasses quadratic in the number of supported monads. So, libraries
...
operations. We have seen that two operations (\lstinline!get! and
\lstinline!set!) are sufficient, in principle, to express any \lstinline!State!-monadic
programs. However, it may not be adequate to write code just with
those two operations. For example, suppose a program needs to \emph{update}
the state using a given function of type $S\rightarrow S$. An update
is equivalent to \lstinline!get!ting the previous value and immediately
...
implemented through \lstinline!get! and \lstinline!set!; a separate
effectful operation (\lstinline!update!) must be provided to the
programmer. 

It is not obvious in advance what effectful operations will be necessary
in practical tasks involving a given monad. Different libraries may
choose different sets of supported operations. However, adding a new
operation to a monad in an MTL-style library requires a rewrite of
the typeclasses involving that monad. Users of the library will be
unable to add any new operations.

Here is a summary of various operations that may be defined for well-known
monads.
...
\textbf{(a)} Implement a monad morphism $\text{Reader}^{R,\bullet}\leadsto\text{State}^{R,\bullet}$
and prove that the laws hold.

\textbf{(b)} Show that a function of type $\text{State}^{R,A}\rightarrow\text{Reader}^{R,A}$
cannot be a monad morphism.
