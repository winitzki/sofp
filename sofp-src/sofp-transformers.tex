...
\[
m\triangleright\text{flift}\triangleright\text{brun}_{L}\overset{!}{=}m\quad,\quad\quad\text{flift}\bef\text{brun}\overset{!}{=}\text{id}\quad,
\]
...
We have derived the numerous laws of lifts and runners from heuristic
considerations. Let us turn to category theory for guidance about
creating a simplified and coherent picture of those laws.
...
may view the functor $T_{L}^{\bullet}$ as the monad transformer \textsf{``}itself\textsf{''}.
...
All these monad morphisms can be drawn as a square diagram.\begin{wrapfigure}{l}{0.25\columnwidth}%
\vspace{-1.75\baselineskip}
\[
\xymatrix{\xyScaleY{2.0pc}\xyScaleX{3.0pc}M^{A}\ar[d]\sb(0.45){\phi}\ar[r]\sp(0.5){\text{flift}} & T_{L}^{M,A}\ar[d]\sp(0.45){\text{frun}_{L}\left(\phi\right)}\\
...
\end{wrapfigure}%
The diagram will be commutative if the following equation holds:
\[
\text{flift}_{L}^{M,A}\bef\text{frun}_{L}(\phi^{:M^{A}\rightarrow N^{A}})=\phi\bef\text{flift}_{L}^{N,A}\quad.
\]
...
When $M=\text{Id}$, we always have a monad morphism $\phi\triangleq\text{pu}_{N}:\text{Id}\leadsto N$.
The monad stack $T_{L}^{M}$ is simplified to $T_{L}^{\text{Id}}\cong L$.
...
When $N=\text{Id}$, we need to choose $\phi=\theta_{M}$, assuming
that $M$ has a runner. The monad stack $T_{L}^{N}$ is simplified
to $T_{L}^{\text{Id}}\cong L$. The function $\text{flift}_{L}^{N}$
has type signature $A\rightarrow L^{A}$ and is equal to $\text{pu}_{L}$.
So, we get:
\[
\text{flift}_{L}^{M}\bef\text{frun}_{L}(\theta_{M})=\theta_{M}\bef\text{pu}_{L}\quad.
\]
This is one of the laws of runners we have derived earlier.

When $M=N$, we may choose $\phi=\text{id}$. The equation becomes
\[
