...
$M$. For any monad morphism $\phi:M\leadsto M^{\prime}$, we denote
by $\psi$ the corresponding monad morphism between $P\leadsto P^{\prime}$
(see Exercise~\ref{par:Exercise-mt-3}):
\[
...
functor laws hold for $\text{frun}_{K}$. The identity law:
\[
...
$T_{K}$, we define the base runner for $T_{L}$ through the runner
$\theta_{P}:P^{A}\rightarrow A$ that we can implement through $\text{brun}_{K}$:
\[
\text{brun}_{L}:M^{A+T_{K}^{M,A}}\rightarrow M^{A}\quad,\quad\quad\text{brun}_{L}\triangleq\theta_{P}^{\uparrow M}\quad,\quad\quad\theta_{P}\triangleq\,\begin{array}{|c||c|}
 & A\\
\hline A & \text{id}\\
...
We can now use Statement~\ref{subsec:Statement-monad-transformer-runner-laws-from-swap}
(see part (b) of the proof) to show that $\text{brun}_{L}$ satisfies
the laws of monad morphisms and the non-degeneracy law. The proofs
of those laws of the proof do not use the monadic naturality of \lstinline!swap!
and thus are sufficient in the present case.
