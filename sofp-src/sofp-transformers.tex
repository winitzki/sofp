...
and has a unit (the monad $\text{Id}$). However, the monad $L\varangle M$
...
side effects may change the state of the external world irreversibly.\footnote{See \texttt{\href{https://stackoverflow.com/questions/11792275/}{https://stackoverflow.com/questions/11792275/}}
for a discussion.} 
...
means being able to use the effects of the monads $K$, $L$, $M$,
$N$. The code should appear to be able to compose Kleisli functions
...
\]
where the \textsf{``}universal\textsf{''} operation $\diamond$ should somehow apply
to Kleisli functions involving any of the given monads or their combinations.
We also expect to be able to run the effects of any one of these monads
while keeping the effects of others. To achieve all that, we need
to define the lifts and the runners for the monad stack.
...
methods \lstinline!flift! and \lstinline!frun!. 

Transformers for certain monads (the continuation and the codensity
monad, the task-like monads such as \lstinline!Future!, and the external
effect monads such as \lstinline!IO!) have neither the \lstinline!frun!
nor the \lstinline!blift! methods. So, values of these monads can
be lifted to a monad stack only when the problematic monads are located
at the deep end of the stack. For example, the monad $N$ at the end
of the stack $K\varangle L\varangle M\varangle N$ can be lifted to
the stack without using any \lstinline!blift! methods:
\[
...
\]
For this reason, it is necessary to put the \lstinline!IO! or \lstinline!Future!
monad at the deep end of the stack.

While we have solved the lifting problem in principle, it is impractical
for the programmer to have to write manually a composition of several
lifts for every monad value in a functor block. Even if we implement
all the lifting functions as extension methods for brevity, the code
would have to be written like this:
\begin{lstlisting}
for {
  x <- valueM.blift.flift.flift
  y <- valueK.blift
  z <- valueL.blift.flift
} yield ...
\end{lstlisting}
If a monad stack needs to be changed from, say, $K\varangle L\varangle M\varangle N$
to $M\varangle L\varangle N\varangle K\varangle P$, all functor blocks
will need to be revised. Productive practical use of a deep monad
stack needs better techniques.{*}{*}{*}

\paragraph{Runners}

{*}{*}{*}

\subsection{Combining monads via typeclasses for monad operations (\textsf{``}MTL style\textsf{''})}
...
of monads. To show that this natural transformation is the identity
function, we need to prove that the identity transformations are the
only natural transformations between identity functors in the category
of monads. A natural transformation of type $\text{Id}^{\bullet}\leadsto\text{Id}^{\bullet}$
...
\]
For brevity, we omit the notation of the functor $F$, since that
functor will be fixed throughout the derivations. When the foreign
monad $M$ is also fixed, we will denote $T_{\text{Free}}^{M,A}$
by simply $T^{A}$.

In the rest of this subsection, we will prove that $T_{\text{Free}}$
...
described as pointed and co-pointed functors in the category of monads
(see Section~\ref{subsec:Category-theoretic-properties-of-lifts-and-runners-functors-in-category-of-monads}).
We now consider certain monads whose monad transformers are not functors
in the category of monads because they are deficient in one or another
way.
...
monads. Instead, \lstinline!StateT! needs a custom definition (Section~\ref{subsec:Monad-transformers-for-standard-monads}).
