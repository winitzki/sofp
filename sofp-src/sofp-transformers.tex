...
\[
L^{A}\triangleq A+F^{M^{A+F^{M^{A+...}}}}=A+F^{M^{L^{A}}}\quad,
\]
...
The base runner for the \lstinline!List! transformer cannot be implemented
as a function of type $T_{\text{List}}^{M,A}\rightarrow M^{A}$: Since
a list may be empty, it is not always possible to extract a value
of type $M^{A}$ out of a given value of type $T_{\text{List}}^{M,A}$.
...
runner $\text{List}^{A}\rightarrow A$. However, for a monoid type
