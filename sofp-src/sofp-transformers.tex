...
the functor $T_{L}$ will be also co-pointed, with $\text{brun}_{L}:T_{L}^{\bullet}\leadsto\text{Id}^{\bullet}$
...
In the category of monads, we have not shown that the non-degeneracy
law is an automatic consequence of monadic naturality. So, we will
impose this law separately, in case the functor $T_{L}$ is co-pointed
(which is not the case for all base monads $L$):
\[
\text{flift}_{T}\bef\text{brun}_{T}=\text{id}\quad.
\]

While this formulation of monad transformers is short, it does not
help {*}{*}{*}
...
is a \textsf{``}foreign runner\textsf{''} function, $\text{frun}_{L}(\phi)$, such
that for any monads $M$, $N$ and any monad morphism $\phi:M\leadsto N$,
the function $\text{frun}\left(\phi\right):T_{L}^{M}\leadsto T_{L}^{N}$
...
\begin{align*}
{\color{greenunder}\text{monadic naturality of }\text{flift}_{L}:}\quad & \text{flift}_{L}^{M}\bef\text{frun}_{L}(\phi^{:M\leadsto N})=\phi\bef\text{flift}_{L}^{N}\quad,\\
{\color{greenunder}\text{monadic naturality of }\text{brun}_{L}:}\quad & \text{brun}_{L}^{M}(\theta)\bef\phi^{:M\leadsto N}=\text{frun}_{L}(\phi)\bef\text{brun}_{L}^{N}(\theta)\quad.
\end{align*}
\end{enumerate}
...
\]
It is a monad morphism because it is a composition (Statement~\ref{subsec:Statement-monadic-morphism-composition})
of a projection (Statement~\ref{subsec:Statement-projection-is-monadic-morphism})
and a monad morphism $\text{brun}_{G}$.
...
\]
In other words, show that any such $\phi$ must be equal to $M$\textsf{'}s
\lstinline!pure! method: $\phi=\text{pu}_{M}$.%
\begin{comment}
Solution:
...
There are no other monad morphisms.
\end{comment}
