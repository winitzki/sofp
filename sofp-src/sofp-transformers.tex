...
\end{lstlisting}
To shorten the code, we define conversions (``lifts'') from \lstinline!Future!
...
(``appending a log message'') and the \lstinline!Either! monad
...
the two \lstinline!up! functions (the ``lifts'') as extension methods:
\begin{lstlisting}
...
\lstinline!flatMap! must discard the second monad's effect. This
...
supported monads. ``Monad transformers'' is an approach for organizing
the monad-combining code without incurring the quadratic growth of
the code size.

In this approach, we fix the first monad $L$ (the ``\textbf{base}''
monad) and write code that combines $L$ with an \emph{arbitrary}
second monad $M$ (the ``\textbf{foreign}'' monad), implementing
...
and \lstinline!flatMap! methods of $T_{L}^{M}$ will use only $M$'s
monad methods ($\text{pu}_{M}$ and $\text{flm}_{M}$) and will work
in the same way for all foreign monads $M$. The monad $T_{L}^{M}$
is called ``the monad $L$'s transformer applied to $M$''. So,
the \textbf{monad} $L$'s\textbf{ transformer} \index{monad transformers}is
...
to indicate the usage of the \lstinline!List! monad's methods \lstinline!pure!
...
$L$. We expect the code for $T_{L}^{M}$'s methods to be parametric
in $M$ but not in $L$ (this is reflected in the notation: $L$ is
a \emph{subscript} in $T_{L}^{M}$). So, changing the base monad $L$
to another monad forces a complete rewrite of $T_{L}^{M}$'s code.
...
to run the $M$'s effects and extract values from a \lstinline!ListT!
into an ordinary \lstinline!List!. This is accomplished by a runner
(call it \lstinline!runListT!) that applies $M$'s runner ($\theta_{M}$)
...
of an ``infinite'' collection that uses the \lstinline!Reader!
monad as $M$:
\begin{lstlisting}
...
passed via the \lstinline!Reader! monad's dependency injection. It
...
monad's effects while keeping the effects of the foreign monad $M$:
\begin{lstlisting}
...
where the foreign monad's constructor $M$ wraps the type $R$ but
...
is not covariant in the type parameter $M$. The base runner's type
...
is \lstinline!Eval!'s runner. A similar function (\lstinline!brun!)
plays the role of the ``base runner'' for \lstinline!EvalT!: it
runs \lstinline!Eval!'s effect (converting lazy values into eager
ones) but keeps $M$'s effects intact. We use \lstinline!brun! to
...
we could use $K$'s transformer and obtain the monad $T_{K}^{T_{L}^{M}}$
...
use the ``kind projector\index{kind@``kind projector'' plugin}''
...
dependency (of type $R$) is used to produce an ``eager'' list.
...
a ``lazy'' (on-call) sequence: getting the value of each next element
...
monad and other ``task-like'' monads that create new threads. Transformers
...
\end{lstlisting}
The transformer's code is parametric in the foreign monad $M$ (but
...
better techniques. One such technique, the ``lift relation'', is
described in Section~\ref{subsec:Constructing-lifts-via-type-relations}.
Another technique is an ``MTL-style'' monad stack (Section~\ref{subsec:Combining-monads-via-mtl-style}).
...
the stack changes. It does not seem possible to define a ``stack
runner'' parametric in the chosen monad, allowing the programmer
simply to indicate which monad's effects need to be run. To see why,
...
The \index{monad transformers!lift@``lift relation'' technique}``lift
relation'' technique creates a type relation that describes all the
...
for the monads' type constructors (\lstinline!P!, \lstinline!Q!).
...
derived from the transformer's type. To make the base monad known
...
The MTL-style\footnote{The name MTL stands for ``monad transformer library''.}
...
\begin{itemize}
\item For a chosen set of ``standard'' monads (such as the \lstinline!Reader!
...
etc.) encapsulating all the ``effectful operations'' specific to
...
typeclass instance. After that, all code written using \lstinline!StateMonadOps!'s
methods will automatically work under any monad stack that contains
the \lstinline!State! monad. 
\end{itemize}
First, we need to define the ``effectful operations'' corresponding
...
So, we regard these two functions as the ``effectful operations\index{monads!effectful operations}''
...
The second ingredient of the MTL-style programming involves ``lifting''
...
to define the ``lifted'' operations $\text{get}_{P}:P^{S}$ and
...
the \lstinline!State! monad's type signature, \lstinline!S => (A, S)!.
...
$P$ ``contains'' some monad $L$. Intuitively, the stack $P$ should
...
where ``\lstinline!OtherT!'' is a monad transformer for the base
...
and the papers ``\emph{Monad transformers and modular interpreters}''
(see \texttt{\href{http://web.cecs.pdx.edu/~mpj/pubs/modinterp.html}{http://web.cecs.pdx.edu/$\sim$mpj/pubs/modinterp.html}})
and ``\emph{Modular monad transformers}'' (see \texttt{\href{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}}).} The choice of operations seems to be heuristic and not following
...
Another example is the \lstinline!Writer! monad's \lstinline!clear!
...
\]
A value of type $A$ can be now extracted by using $L$'s runner.
...
the continuation monad's runner (Section~\ref{subsec:The-continuation-monad})
...
becomes clear from viewing \lstinline!frun! as a lifting (``\lstinline!fmap!'')
...
way for all foreign monads $M$. We call it a ``\textbf{monadic}''
...
the monad $L$'s transformer has a base runner, $\text{brun}_{L}:T_{L}^{M}\leadsto M$.
...
The monadic naturality law of \lstinline!frun! is equivalent to \lstinline!frun!'s
...
are ``pointed''\index{pointed functor!in the category of monads};
those with a natural transformation $\text{ex}_{F}:F^{A}\rightarrow\text{Id}^{A}$
are ``co-pointed''\index{co-pointed functor!in the category of monads}.
...
of monads}.\footnote{``\emph{What's the big deal?}'' as the joke would go.}
...
law},\index{nondegeneracy law!of base lift}
\[
...
satisfy a \textbf{nondegeneracy law}:\index{nondegeneracy law!of base runner}
\[
...
for any monads $L$ and $M$. This ``fake transformer'' cannot possibly
...
is always a monad morphism. So, the ``fake transformer'' actually
...
represent properly the effects of the monads, even though the transformer's
...
type), instead of $P\times Q$. The result is a different ``monoidal
convolution'' ($\star_{Z}$):
\[
...
on a functor $F$. The codensity monad provides yet another way of
making a monad out of any functor. Can we then combine arbitrary monads
$L$, $M$ into a new monad? It turns out that the codensity construction
...
are required for the \texttt{}\lstinline!swap! operation.\footnote{The \lstinline!swap! operation was studied in the paper ``\emph{Composing
monads}'': \texttt{\href{http://web.cecs.pdx.edu/~mpj/pubs/RR-1004.pdf}{http://web.cecs.pdx.edu/$\sim$mpj/pubs/RR-1004.pdf}}}
...
\end{equation}
This law requires that a ``pure'' layer of the functor $L$ should
remain ``pure'' after swapping the order of layers.
...
\paragraph{Intuition behind \texttt{swap}'s interchange laws}
...
the ``compatibility laws''~(\ref{eq:ftn-first-compatibility-law})\textendash (\ref{eq:ftn-second-compatibility-law}),
...
\]
In the same way, the laws of the monad morphisms will hold for ``$\text{id}$''
...
of this monad is $Q\times\left(P+A\right)$. This monad's effect is
...
``linear''. No other examples of composed-inside transformers are
...
are called ``rigid'' in this book\footnote{Monads of this kind do not seem to have an already accepted name.}
because one of their general properties is having a single, fixed
``data shape'' (Statement~\ref{subsec:Statement-rigid-functor-wrapped-unit-is-unit}
...
\begin{enumerate}
\item ``Product'': $P^{A}\times R^{A}$ is a rigid monad if $P$ and $R$
are rigid monads.
\item ``Composition'': $P\circ R$ is a rigid monad if $P$ and $R$ are
rigid monads.
\item ``Choice'': $C^{A}\triangleq H^{A}\rightarrow A$ is a rigid monad
...
that the functor $P$ is a rigid monad; so $P$'s transformer is of
...
$R$ for any given contrafunctor $H$. This monad somehow ``chooses''
...
The selector monad represents the effect of somehow ``selecting''
...
\end{align}
The monad $R$'s methods may be defined by
\begin{align}
...
The monad $T$'s methods depend on the monad $R$, whose code we will
...
We now derive the code for the monad $R$'s methods. That monad's
...
This concludes the proof of the laws for the ``choice'' monad's
transformer.
...
which requires a suitable \lstinline!swap! function (denoted by ``$\text{sw}$''):
\begin{equation}
...
\end{array}\,\bigg)\quad.
\]
Here we use the monoid $M^{R}$'s binary operation $\oplus_{M}$ defined
by
\begin{equation}
...
This completes the proof of the laws for the \lstinline!List! monad's
transformer.
...
The \lstinline!State! monad's transformer requires custom code (see
...
The continuation monad transformer's type constructor \lstinline!ContT[R, M[_], *]!
is defined by
\[
...
\]
where $L$ is any fixed monad and $T_{L}$ is the monad $L$'s transformer.
Like the continuation monad, the \lstinline!Search! monad's transformer
...
To verify the composition law, we use the flipped Kleisli\index{flipped@``flipped Kleisli'' technique}
...
\]
Now we can show that \lstinline!flift!'s composition law holds:
\begin{align*}
...
``return type'' is $F^{R}$. The difference is that $\text{Cod}^{F,A}$
...
\textbf{(a)} We need to define the monad $L$'s methods and verify
the laws. The idea is to add the quantifier ($\forall R$) to monad
$K$'s methods. (For brevity, we will sometimes write $K$ to mean
...
\end{align*}
Note that \lstinline!inC! is the same as $F$'s \lstinline!flatMap!,
...
monad's runner. That runner returns the result value of type $A$
but discards the possibly changed state (of type $R$). This makes
the \lstinline!State! monad's runner fail the composition law, as
...
Similarly to the continuation monad's transformer, we only prove three
groups of laws.
...
To verify $\Delta$'s composition law, 
\[
...
\]
and a new ``double-valued \lstinline!flatMap!'' as 
\[
...
of any number of $M$'s. The general version of the ``multi-valued
\lstinline!flatMap!'' is a situation when the collection of $M$-effects
...
For any rigid functor $R$ and a monad $M$, there exists an ``$R$-valued
\lstinline!flatMap!'':
\[
...
Using $R$'s \lstinline!fuseIn! method, we can convert a value of
type $A\rightarrow R^{M^{B}}$ into a value of type $R^{A\rightarrow M^{B}}$.
After that, we can use $M$'s \lstinline!flatMap!:
\[
...
We use $R$'s method \lstinline!fuseIn! to convert a value of type
...
This is the motivation for the name ``rigid'': the data structure
$R^{A}$ must have a fixed ``shape'' and cannot be a disjunction
of several variants. 
...
However, the right-hand side of the nondegeneracy law applied to $\text{id}$
...
\subsection{Monadically natural lifting of MTL operations (Jaskelioff's ``\texttt{Monatron}'')\label{subsec:Monatron}}
...
to all supported monad stacks.\index{Mauro Jaskelioff} M.~Jaskelioff's
``\texttt{Monatron}'' library\footnote{See \texttt{\href{https://www.fceia.unr.edu.ar/~mauro/pubs/monatron.pdf}{https://www.fceia.unr.edu.ar/$\sim$mauro/pubs/monatron.pdf}}}\index{Monatron library@\texttt{Monatron} library}
...
based on the methods and results explained in a paper by M.~Jaskelioff,\footnote{``Modular monad transformers'', see \texttt{\href{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}}}
will show how \texttt{Monatron}'s liftings work. 
...
A \textbf{``fixed-type'' operation} is a function of type $P\rightarrow M^{Q}$,
where $P$ and $Q$ are fixed types.

A \textbf{``simple'' operation} is a natural transformation with
type signature $\forall A.\,F^{A}\rightarrow M^{A}$.

A \textbf{``standard'' operation} is a natural transformation with
type signature $\forall A.\,F^{M^{A}}\rightarrow M^{A}$.\\
$\square$

According to this definition, the \lstinline!State! monad's \lstinline!get!
and \lstinline!set! operations are ``fixed-type'', the \lstinline!Either!
monad's \lstinline!raise! operation is ``simple'' (with the functor
$F$ defined as the constant functor $F^{A}\triangleq E$), and the
\lstinline!List! monad's \lstinline!append! operation is ``standard''
(with the functor $F^{A}\triangleq A\times A$). 

The following statement shows that the ``fixed-type'' operations
are a subset of ``simple'' operations, while the ``simple'' operations
are a subset of ``standard'' operations.
...
\textbf{(a)} Any ``fixed-type'' operation $\text{fop}:P\rightarrow M^{Q}$
is  equivalent to a ``simple'' operation \lstinline!sop! with functor
...
\textbf{(b)} Any ``standard'' operation $\text{op}:F^{M^{A}}\rightarrow M^{A}$
...
\end{equation}
is equivalent to a ``simple'' operation \lstinline!sop! defined
by
\[
...
Direction $\text{op}\rightarrow\text{sop}$: Given that a ``standard''
...
and then compute a new ``standard'' operation $\text{op}^{\prime}\triangleq\text{sop}\bef\text{ftn}_{M}$.
...
Direction $\text{sop}\rightarrow\text{op}$: Given a ``simple''
operation $\text{sop}:F^{A}\rightarrow M^{A}$, we first define $\text{op}\triangleq\text{sop}\bef\text{ftn}_{M}$
and then compute a new ``simple'' operation $\text{sop}^{\prime}\triangleq\text{pu}^{\uparrow F}\bef\text{op}$.
...
and \lstinline!filter!) are either ``fixed-type'', ``simple'',
or ``standard'', and so (by Statement~\ref{subsec:Statement-simple-standard-operations-monatron})
may be equivalently represented by some ``standard'' operations.
Many operations with a ``standard'' type signature turn out to be
equivalent to ``simple'' operations because they satisfy the compatibility
law. 
...
The \lstinline!List! monad's \lstinline!append! operation has the
type signature of the ``standard'' form $F^{\text{List}^{A}}\rightarrow\text{List}^{A}$
...
\lstinline!append! is equivalent to a ``simple'' operation \lstinline!sappend!
defined by
\begin{align*}
...
Certain ``standard'' operations are \emph{not} equivalent to ``simple''
ones:

\subsubsection{Example \label{subsec:Example-monatron-flush-operation}\ref{subsec:Example-monatron-flush-operation}}

The \lstinline!Writer! monad's \lstinline!clear! operation (erasing
...
\]
The type signature $M^{A}\rightarrow M^{A}$ is of the ``standard''
...
and is not equivalent to a ``simple'' operation. If it were, the
corresponding ``simple'' operation would be a natural transformation
...
corresponding ``standard'' operation is 
\[
...
To build an intuition for the meaning of ``simple'' and ``standard'',
consider a ``standard'' operation $\text{op}:F^{M^{A}}\rightarrow M^{A}$,
where $F^{A}$ contains a few values of type $A$ (such as $F^{A}\triangleq A\times A$
as in the \lstinline!List! monad's \lstinline!append! operation).
A ``simple'' operation $\text{sop}:F^{A}\rightarrow M^{A}$ computes
a new $M$-effect value from a few values of type $A$ without using
any previously given $M$-effect values. A ``standard'' operation
\lstinline!op! transforms a few effectful values of type $M^{A}$
into a new effectful value. If \lstinline!op! were equivalent to
a ``simple'' operation \lstinline!sop!, we would have $\text{op}=\text{sop}\bef\text{ftn}_{M}$.
...
As a heuristic summary, we may say that a ``simple'' operation creates
new effect values from ordinary, non-effectful values. ``Standard''
operations that are equivalent to ``simple''operations cannot perform
...
any other way. A ``standard'' operation could transform effects
in a way that is \emph{not} equivalent to flattening (as the example
with \lstinline!clear! shows).

Putting the ``non-standard'' operations \lstinline!callcc!, \lstinline!collect!,
\lstinline!process!, and \lstinline!filter! temporarily aside, we
now focus on the methods for lifting ``simple'' and ``standard''
operations to monad stacks.

The goal is to lift a ``standard'' operation $\text{op}_{M}:F^{M}\leadsto M$
...
For example, since the \lstinline!List! monad's \lstinline!append!
...
within the stack $P$. The \lstinline!Writer! monad's \lstinline!clear!
...
\paragraph{Lifting ``simple'' operations}

Section~\ref{subsec:Combining-monads-via-mtl-style} showed that
the \lstinline!State! monad's \lstinline!get! and \lstinline!set!
operations are lifted to any monad stack by composing with a lift
function. This method works %
\begin{comment}
equally well 
\end{comment}
for any ``simple'' operation $\text{sop}_{M}:F^{A}\rightarrow M^{A}$
and an arbitrary monad stack $P$ that has a lift function $p:M^{A}\rightarrow P^{A}$.
The result of lifting is a ``simple'' operation ($\text{sop}_{P}$)
defined by
\begin{equation}
...
of ``standard'' operations. It also proves that the ``standard''
...
Given a ``simple'' $M$-operation $\text{sop}_{M}:F^{A}\rightarrow M^{A}$
...
Then the ``standard'' operation $\text{op}_{P}\triangleq\text{sop}_{P}\bef\text{ftn}_{P}$
...
allows us to lift a ``simple'' operation from a monad $M$ to any
monad stack $P$ containing $M$, as long as we have a lift function
$p:M\leadsto P$. The lift $p$ will exist if the monad $M$ is at
the deep end of the stack, or if $M$'s transformer supports a base
...
The same method allows us to lift a ``standard'' operation if it
is equivalent to a ``simple'' operation. The condition for that
equivalence is the compatibility law~(\ref{eq:monatron-standard-operation-compatibility-law}).
If a ``standard'' operation \lstinline!op! satisfies that law,
the corresponding lifted operation $\text{op}_{P}$ is expressed by
Eq.~(\ref{eq:monatron-lifting-simple-op}).

\paragraph{Lifting ``standard'' operations}

Some ``standard'' operations fail the compatibility law~(\ref{eq:monatron-standard-operation-compatibility-law}).
So, they cannot be reduced to a ``simple'' operation and require
a different method of lifting.

We begin with the task of lifting a ``standard'' operation $\text{op}:F^{M^{A}}\rightarrow M^{A}$
...
\begin{enumerate}
\item Convert \lstinline!op! into an equivalent ``simple'' operation
...
(the type of ``standard'' operations in the monad $M$) is \emph{equivalent}
to the type of natural transformations $F^{A}\rightarrow\text{Cod}^{M,A}$
(the type of ``simple'' operations in the codensity monad on $M$).
...
\begin{align*}
\text{expect to equal }\text{op}:\quad & \text{op}\triangleright\text{inK}\triangleright\text{outK}=(f\rightarrow k\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op}\gunderline{)\triangleright\big(}\text{cop}\rightarrow g\rightarrow\text{cop}\,(g)(\text{id})\big)\\
\text{apply function}:\quad & =g\rightarrow(\gunderline{f\rightarrow k\rightarrow}\,f\triangleright k^{\uparrow F}\triangleright\text{op})\gunderline{(g)(\text{id})}\\
\text{apply function}:\quad & =g\rightarrow g\triangleright\text{id}^{\uparrow F}\triangleright\text{op}=\gunderline{g\rightarrow g\,\triangleright}\,\text{op}=\text{op}\quad.
\end{align*}

Proving that $\text{outK}\bef\text{inK}=\text{id}$ needs a naturality
...
of the isomorphism:
\begin{align*}
\text{expect to equal }\text{cop}:\quad & \text{cop}\triangleright\text{outK}\triangleright\text{inK}=\big(g\rightarrow\text{cop}\,(g)(\text{id})\big)\triangleright(\text{op}\rightarrow f\rightarrow k\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op})\\
...
\text{naturality law of }\text{cop}:\quad & =f\rightarrow k\rightarrow\text{cop}(f)(k\bef\text{id})=\gunderline{f\rightarrow k\rightarrow}\,\text{cop}\,(f)(k)=\text{cop}\quad.
\end{align*}
...
\begin{align*}
\text{left-hand side}:\quad & x^{:F^{K^{A}}}\triangleright f^{\uparrow K\uparrow F}\bef\text{outK}\,(\text{cop})=x\triangleright f^{\uparrow K\uparrow F}\triangleright\big(g\rightarrow\text{cop}\,(g)(\text{id})\big)=\text{cop}\,(x\triangleright f^{\uparrow K\uparrow F})(\text{id})\\
\text{use Eq.~(\ref{eq:monatron-naturality-law-of-cop})}:\quad & \quad=\text{cop}\,(x)(f^{\uparrow K}\bef\text{id})=\text{cop}\,(x)(f^{\uparrow K})\quad,\\
\text{right-hand side}:\quad & x\triangleright\text{outK}\,(\text{cop})\triangleright f^{\uparrow L}=x\triangleright\big(g\rightarrow\text{cop}\,(g)(\text{id})\big)\triangleright f^{\uparrow L}=\text{cop}\,(x)(\text{id})\triangleright f^{\uparrow L}\quad.
\end{align*}
...
with $K=L=M$, we find that a ``standard'' operation \lstinline!op!
in a monad $M$ is equivalent to a ``simple'' operation \lstinline!scop!
...
``simple'' operation ($\text{scop}_{P^{\prime}}$) defined by
\[
...
we may use $T_{L}$'s \lstinline!frun! to transform between $P$
...
a ``simple'' operation $\text{sop}_{P}$ of type $F^{A}\rightarrow P^{A}$.
However, the resulting operation would not correctly extend \lstinline!op!'s
...
which is the ``simple'' counterpart of \lstinline!op!. However,
...
Given a ``standard'' operation \lstinline!op! in a monad $M$,
define \lstinline!scop! by Eq.~(\ref{eq:monatron-scop-via-op}).
Also, define the corresponding ``standard'' operation \lstinline!cop!
...
``standard'' operation ($\text{cop}_{P^{\prime}}$):
\[
...
What if we use this method to lift a ``standard'' operation that
is actually equivalent to a ``simple'' one (and could be lifted
...
and additionally assume that the ``standard'' operation \lstinline!op!
is equivalent to a ``simple'' operation \lstinline!sop! (so that
$\text{op}=\text{sop}\bef\text{ftn}_{M}$). Then the lifted ``standard''
operation ($\text{op}_{P}$) is equivalent to the lifted ``simple''
...
The method just shown lifts ``standard'' $M$-operations to monad
...
All ``standard'' $M$-operations that satisfy the compatibility
law~(\ref{eq:monatron-standard-operation-compatibility-law}) are
equivalent to ``simple'' operations and are lifted to the stack
$Q\triangleq M\varangle N$ using $Q$'s \lstinline!blift! function.
(For monads whose transformers do not support base lifts, operations
must be lifted by custom code.) It remains to consider ``standard''
...
out of a given non-``simple'' operation \lstinline!op!.

If $M$'s transformer is composed-inside (so that $Q^{A}\triangleq M^{N^{A}}$),
...
non-``simple'' \lstinline!local! operation) but does not apply,
e.g., to the \lstinline!Writer! monad's \lstinline!clear! operation
or to other non-rigid monads. The \lstinline!Writer! monad's transformer
...
monad's \lstinline!clear! operation to any stack $Q\triangleq M\varangle N\cong N\circ M$
as follows:
...
of the \lstinline!Writer! monad's \lstinline!clear! operation to
...
A similar case is the \lstinline!Either! monad's \lstinline!handle!
operation,
\[
...
\end{array}\quad.
\]
This operation is of the ``standard'' form with the functor $F^{B}\triangleq\left(E\rightarrow B\right)\times B$
but is not equivalent to a ``simple'' operation. Custom code is
...
Then a monadically natural lifting of the \lstinline!Either! monad's
...
After writing custom code for lifting each ``standard'' but non-``simple''
...
We could also combine all ``standard'' operations ($\text{op}_{1}:F_{1}^{M^{A}}\rightarrow M^{A}$,
...
\]
The resulting ``standard'' operation corresponds to the product
of all ``standard'' operations of the monad $M$. It is easier to
...
\paragraph{``Flipped-standard'' operations}

It remains to lift operations that have neither a ``fixed-type'',
nor ``simple'', nor a ``standard'' type signature. Four such operations
...
That type is of ``flipped-standard'' form, $M^{F^{A}}\rightarrow M^{A}$,
where $M^{A}=\text{Writer}^{W,A}$ and $F^{A}\triangleq W\rightarrow A$.

If an operation of ``flipped-standard'' form ($M^{F^{A}}\rightarrow M^{A}$)
satisfies a suitable compatibility law, it is equivalent to a ``simple''
...
The ``flipped-standard'' operations that are equivalent to ``simple''
operations may be lifted to monad stacks similarly to the ``standard''
operations that are equivalent to ``simple'' operations. An example
...
the \index{compatibility law!of deflate (filterable monad)@of \texttt{deflate} (filterable monad)}\textbf{compatibility
law}:
\[
...
is equivalent to a ``simple'' operation that we will call \lstinline!defl!:
\[
...
However, the other two operations of ``flipped-standard'' signature
(\lstinline!collect! and \lstinline!process!) are \emph{not} equivalent
to ``simple'' operations. Let us see how to lift those operations
...
general (for arbitrary ``flipped-standard'' operations), this lifting
...
has a type signature of neither ``standard'' nor ``flipped-standard''
form:
\[
...
through two ``simple'' operations: one denoted by \lstinline!jump!
...
The functor $F$ for operation \lstinline!scc! is the \index{monads!Sel (selector) monad@\texttt{Sel} (selector) monad}``selector
monad'': $F^{A}\triangleq\text{Sel}^{R,A}=(A\rightarrow R)\rightarrow A$.
The function \lstinline!scc! is studied in Exercise~\ref{subsec:Exercise-selector-and-continuation-monads}.
The two ``simple'' operations \lstinline!scc! and \lstinline!jump!
are lifted straightforwardly to any monad stack. 

To express \lstinline!callcc! via \lstinline!jump! and \lstinline!scc!,
we first convert \lstinline!scc! to an equivalent ``standard''
...
\]
To arrive at a systematic way of transforming functions of \lstinline!cc!'s
type signature into functions of \lstinline!callcc!'s type signature,
...
is equivalent to a ``simple'' operation \lstinline!scc!, the lifted
...
\end{align*}
We may lift the ``simple'' operation \lstinline!jump! to the stack
$P$ as
\[
...
\end{align*}
The type signatures of these operations are neither ``standard''
nor of ``flipped-standard''. Lifting these operations in a systematic
...
a monad's type given its operations? This appears to be impossible
in general. It is also unknown how to obtain a ``complete'' set
...
Show that any monad $M$ admits a ``standard'' operation with $F=M$.
Is that ``standard'' operation equivalent to a ``simple'' operation?

\subparagraph{Solution}

The type signature of a ``standard'' operation with $F=M$ is $\text{op}:M^{M^{A}}\rightarrow M^{A}$.
...
a ``standard'' operation $\text{op}:F^{M}\leadsto M$. Is that ``standard''
operation equivalent to a ``simple'' operation?
...
\end{array}\quad.
\end{align*}
The equivalent ``simple'' operation is the injection of $F$ into
the free monad:
\[
...
So, if we have some ``simple'' effectful operations with functors
...
describe any operations that are not equivalent to ``simple'' ones.
...
Heuristically, this function ``doubles'' the $M$-effect in a given
...
\begin{align*}
\text{inner pure compatibility law}:\quad & \text{ftn}_{L}=\text{pu}_{M}^{\uparrow L}\bef\text{ftn}_{T}\quad:L^{L^{M^{A}}}\rightarrow L^{M^{A}}\quad,\\
\text{outer pure compatibility law}:\quad & \text{ftn}_{M}^{\uparrow L}=\text{pu}_{L}^{\uparrow T}\bef\text{ftn}_{T}\quad:L^{M^{M^{A}}}\rightarrow L^{M^{A}}\quad,
\end{align*}
...
Define a ``search monad'' by $\text{Search}^{P,A}\triangleq\left(A\rightarrow\bbnum 1+P\right)\rightarrow\bbnum 1+A$.
The function 
\begin{lstlisting}
...
for a fixed type $X$ and for some $M^{\bullet}$, the type constructor
$L^{A}\triangleq(A\rightarrow X)\rightarrow M^{X}$ is \emph{not}
a monad.
