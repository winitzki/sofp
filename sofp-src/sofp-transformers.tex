...
The \lstinline!Search! monad\textsf{'}s transformer does not support foreign
runner, base runner, or base lift functions.
...
Similarly, the monad laws hold for $T_{\text{Search}}^{M,A}$ if we
use the $T_{L}^{M}$-filterable contrafunctor $H^{A}\triangleq A\rightarrow T_{L}^{M,A}$.

\paragraph{Identity laws}

\paragraph{Lifting laws}

The \lstinline!lift! function produces only \textsf{``}pure\textsf{''} values of
the \lstinline!Search! monad:
\[
\text{lift}:\quad M^{A}\rightarrow(A\rightarrow T_{L}^{M,Q})\rightarrow T_{L}^{M,A}\quad,\quad\quad\text{lift}\triangleq m^{:M^{A}}\rightarrow\_^{:A\rightarrow T_{L}^{M,Q}}\rightarrow m\triangleright\text{lift}_{L}\quad,
\]
where $\text{lift}_{L}$ is the transformer $T_{L}$\textsf{'}s \lstinline!lift!
function.
...
Consider the generalized \lstinline!Search! monad with $L$ chosen
to be the identity monad.{*}{*}{*}
