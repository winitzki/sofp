...
\subsubsection{Exercise \label{subsec:Statement-search-and-selector-monads}\ref{subsec:Statement-search-and-selector-monads}\index{exercises}}
...
and there is a monad morphism in one direction:
...
To obtain a value $p:H^{M^{B}}$, we use the function $\text{flm}_{M}\left(q\triangleright g\right):M^{B}\rightarrow M^{C}$
