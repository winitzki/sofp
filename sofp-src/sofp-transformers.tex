...
of \lstinline!flatMap! must discard the second monad\textsf{'}s effect. This
...
\lstinline!Success! subtypes). It is not obvious how to define the
type of a monad transformer $T_{L}^{M}$ for an arbitrary given base
monad $L$. We expect the code for $T_{L}^{M}$\textsf{'}s methods to be parametric
in $M$ but not in $L$ (this is reflected in the notation: $L$ is
a \emph{subscript} in $T_{L}^{M}$). So, changing the base monad $L$
to another monad may require a complete rewrite of $T_{L}^{M}$\textsf{'}s
code.
...
in \texttt{\href{https://wiki.haskell.org/index.php?title=ListT_done_right}{https://wiki.haskell.org/index.php?title=ListT\_done\_right}}} A justification for the types in Table~\ref{tab:Known-monad-transformers}
...
do not usually have runners ($\theta_{\text{List}}:\text{List}^{A}\rightarrow A$)
because the \lstinline!List! monad normally represents \emph{several}
useful results rather than a single result. Also, a list may be empty,
which prevents having a fully parametric function of type $\forall A.\,\text{List}^{A}\rightarrow A$.
However, the foreign monad $M$ may come with its runner $\theta_{M}$.
We can use $\theta_{M}$ to run the $M$\textsf{'}s effects and extract values
from a \lstinline!ListT!. This is accomplished by a runner (call
it \lstinline!runListT!) that applies $M$\textsf{'}s runner ($\theta_{M}$)
...
that runs the \lstinline!State! monad\textsf{'}s effects but keeps the effects
of the foreign monad $M$. The type of $T_{\text{State}}^{M}$ does
not allow base runners parametric in $\theta_{\text{State}}$. Instead,
...
may be interfaces to system libraries implemented in non-fully parametric
code. It may be impossible to rewrite that code to return \lstinline!ContT[M, R, A]!
...
represents a choice between eager and lazy evaluation. The corresponding
...
similar function (\lstinline!brun!) plays the role of the \textsf{``}base
runner\textsf{''} for \lstinline!EvalT!: it runs \lstinline!Eval!\textsf{'}s effect
(converting lazy values into eager ones) but keeps $M$\textsf{'}s effects
intact. We use \lstinline!brun! to implement \lstinline!flatMap!:
\begin{lstlisting}
...
to use the \textsf{``}kind projector\index{kind@\textsf{``}kind projector\textsf{''} plugin}\textsf{''}
...
examples of stacking monads in different orders.
...
stack. The only requirement is the need to know the transformers for
each monad (except for the last one).
...
\]
Now translate the type notation into code, assuming that \lstinline!S!
is a fixed type:
\begin{lstlisting}
...
and use its transformer (\lstinline!TryT!) on the foreign monad $M$:
\begin{lstlisting}
type TryM[A] = M[Try[A]]
\end{lstlisting}
...
even if restricted to exponential-polynomial type constructors whose
monad methods are implemented by purely functional code.
...
must allow the foreign monad to cancel the effects of the base monad,
...
monad derived in this book (although some transformers will only partially
...
Transformers for certain monads (such as the continuation and the
\index{monads!codensity monad}codensity monad) have neither the \lstinline!frun!
nor the \lstinline!blift! methods. Also, task-like monads (such as
\lstinline!Future!) and the external side-effect monads (such as
\lstinline!IO! and \lstinline!ZIO!) have no transformers at all.
Values of those monads can be lifted to a monad stack only when the
problematic monad is located at the deep end of the stack, so that
its transformer is not needed. For example, the monad $N$ at the
end of the stack $K\varangle L\varangle M\varangle N$ can be lifted
to the stack without need for $N$\textsf{'}s transformer:
\[
...
\]
For this reason, one must put the \lstinline!IO! and \lstinline!Future!
monads at the deep end of the stack. This limits the use of those
monads in stacks; for instance, a stack cannot contain both an \lstinline!IO!
and a \lstinline!Future! monad. To provide adequate functionality
for applications, the \lstinline!IO! and \lstinline!ZIO! monads
include special methods for running parallel computations on new threads,
which avoids the need to use a \lstinline!Future! monad in the same
monad stack.

While this solves the lifting problem in principle for most monad
stacks, it is impractical for the programmer to have to write manually
a composition of several lifts for every source line in a functor
block. Even if we implement all the lift functions as extension methods
and abbreviate the type annotations as much as possible, the code
would have to be written similarly to this fragment:

\begin{wrapfigure}{l}{0.4\columnwidth}%
\vspace{0.25\baselineskip}
\begin{lstlisting}
for { // Using a stack of monads K, L, M, N.
...
\noindent If a monad stack $K\varangle L\varangle M\varangle N$ needs
to be extended to $M\varangle L\varangle N\varangle K\varangle P$
because of a change in program requirements, all source lines in all
functor blocks will have to be modified to accommodate the required
changes in the lifts. Clearly, productive practical use of deep monad
stacks needs better techniques. One such technique, the \textsf{``}lift relation\textsf{''},
is described in Section~\ref{subsec:Constructing-lifts-via-type-relations}.
...
simply to indicate which monad\textsf{'}s effects need to be run and without
writing the code of the runner by hand. To see why, consider a monad
stack of type \lstinline!EitherT[E, ReaderT[R, Option, *], A]!. If
we wish to run the effects of the \lstinline!Reader! monad alone,
...
is just one of the type parameters. This complication is due to the
fact that the type of the stack is not parametric in the type of the
base monad.
...
monad transformers from that stack. For instance, within the stack
$K\varangle L\varangle M\varangle N$, the monads $L$ and $K\varangle L\varangle M$
will be in a lift relation because we can construct a monad morphism
$L\leadsto K\varangle L\varangle M$ as
\[
...
type constructors. The data carried by the type relation \lstinline!MM[P, Q]!
will be the code for the required monad morphism between \lstinline!P!
and \lstinline!Q!:
\begin{lstlisting}
...
\begin{lstlisting}
implicit val readerBase: BaseMonad[Reader, ReaderT] = BaseMonad(
  new MM[Reader, ReaderT[Id, *]] {
...
instance, and a relation instance for the \lstinline!BaseMonad! type
relation. (So, the length of code is linear in the number of supported
monad types.) Those instances can be defined as implicit values in
a monad transformer library. Then the user may declare an arbitrary
monad stack and use automatic lifts between any levels in a stack.
...
The MTL-style\footnote{See \texttt{\href{http://book.realworldhaskell.org/read/monad-transformers.html}{http://book.realworldhaskell.org/read/monad-transformers.html}}.
The name MTL stands for \textsf{``}monad transformer library\textsf{''}.} monadic\index{monadic program!MTL-style} programming is another
way of achieving automatic lifting of monadic programs to arbitrary
monad stacks. The MTL approach has two main ingredients:
\begin{itemize}
...
($\text{State}^{S,A}$). Its effect allows programs to manipulate
an internal state value (of type $S$): the state can be read, updated,
and automatically passed on to the next computation. We may now define
the functions \lstinline!get! and \lstinline!set! that work with
the internal state:
\[
...
So, we regard these two functions as the \textsf{``}effectful operations\index{monad!effectful operations}\textsf{''}
...
\[
\text{raise}:E\rightarrow\forall A.\,E+A\quad,\quad\quad\text{raise}\triangleq e^{:E}\rightarrow e+\bbnum 0^{:A}\quad.
\]
...
operation but does not refer to the specific type $E+A$ of the \lstinline!Either!
...
the monad operations to arbitrary monad stacks. For example, the operations
\lstinline!get! and \lstinline!set! can be lifted from the \lstinline!State!
monad to any monad stack containing the \lstinline!State! monad.
For any such monad stack $P$, there exists a specific composition
of lifts that creates a monad morphism $p:\text{State}^{S,\bullet}\leadsto P^{\bullet}$.
If $p$ is known, the new operations $\text{get}_{P}$ and $\text{set}_{P}$
are expressed as
\[
...
\[
\text{raise}_{P}:\forall A.\,P^{A}\quad,\quad\quad\text{raise}_{P}\triangleq\forall A.\,\text{raise}^{A}\triangleright p\quad.
\]

Lifting operations from a monad to a stack depends on having a suitable
lift function ($p$) that maps values of the specific monad to the
stack. The lift function $p$ will be different for each monad and
each stack, and we would like to avoid having to implement $p$ by
hand each time. In MTL-style programming, this is achieved by first
refactoring the code to use special typeclasses that encapsulate the
effectful operations of all the required monads. For the \lstinline!State!
monad, the typeclass \lstinline!StateMonadOps! may be defined as
\begin{lstlisting}
...
through appropriate lift functions. Let us see in more detail how
this works.
...
by induction: 
\begin{itemize}
\item Base case: $P$ contains $L$ when $P=L$ or $P=L\varangle Q$ for
...
\]
where $p_{L,Q}$ is the lift function for the stack $Q$, which is
...
with the base case: The \lstinline!State! monad itself already has
...
implicit def stateTMonadOps[S, Q[_]: Monad] = new StateMonadOps[StateT[Q, S, *], S] {
  ...
  def set: S => StateT[Q, S, Unit] = s => StateT(_ => implicitly[Monad[Q]].pure(((), s)))
}
\end{lstlisting}
...
The first problem is the lack of standard conventions or principles
for the choice of effectful operations. We have seen that the operations
\lstinline!get! and \lstinline!set! are sufficient to express any
\lstinline!State!-monadic programs. However, it may not be adequate
to write code just with those two operations. 

\begin{wraptable}{l}{0.62\columnwidth}%
\begin{centering}
\vspace{-0.4\baselineskip}
\begin{tabular}{|>{\centering}m{1.3cm}|>{\centering}m{7.2cm}|}
\hline 
\textbf{\small{}Monad} & \textbf{\small{}Operations}\tabularnewline
\hline 
\hline 
{\small{}$\text{State}^{S,A}$} & {\small{}$\text{get}:\text{State}^{S,S}$}{\small\par}

{\small{}$\text{set}:S\rightarrow\text{State}^{S,\bbnum 1}$}{\small\par}

{\small{}$\text{update}:(S\rightarrow S)\rightarrow\text{State}^{S,\bbnum 1}$}\tabularnewline
\hline 
{\small{}$\text{Reader}^{R,A}$} & {\small{}$\text{ask}:\text{Reader}^{R,R}$}{\small\par}

{\small{}$\text{withEnv}:R\rightarrow\text{Reader}^{R,A}\rightarrow\text{Reader}^{R,A}$}\tabularnewline
\hline 
{\small{}$\text{Either}^{E,A}$} & {\small{}$\text{raise}:E\rightarrow\text{Either}^{E,A}$}{\small\par}

{\small{}$\text{handle}:(E\rightarrow\text{Either}^{E,A})\rightarrow\text{Either}^{E,A}\rightarrow\text{Either}^{E,A}$}\tabularnewline
\hline 
{\small{}$\text{List}^{A}$} & {\small{}$\text{empty}:\text{List}^{A}$}{\small\par}

{\small{}$\text{append}:\text{List}^{A}\times\text{List}^{A}\rightarrow\text{List}^{A}$}{\small\par}

{\small{}$\text{filter}:\text{List}^{A}\rightarrow(A\rightarrow\bbnum 2)\rightarrow\text{List}^{A}$}\tabularnewline
\hline 
{\small{}$\text{Writer}^{W,A}$} & {\small{}$\text{log}:W\rightarrow\text{Writer}^{W,\bbnum 1}$}{\small\par}

{\small{}$\text{clear}:\text{Writer}^{W,A}\rightarrow\text{Writer}^{W,A}$}\tabularnewline
\hline 
{\small{}$\text{Cont}^{R,A}$} & {\small{}$\text{abort}:R\rightarrow\text{Cont}^{R,A}$}{\small\par}

{\small{}$\text{callcc}:((A\rightarrow\text{Cont}^{R,B})\rightarrow\text{Cont}^{R,A})\rightarrow\text{Cont}^{R,A}$}\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
\caption{Known operations for some monads.\label{tab:Known-operations-for-some-monads}}
\vspace{-0.4\baselineskip}
\end{wraptable}%

\noindent As an example, consider \emph{updating} the state using
a given function of type $S\rightarrow S$. In theory, updating is
equivalent to \lstinline!get!ting the previous value and immediately
\lstinline!set!ting a new value. But a program may be required to
handle certain state updates in a special way. Examples are updates
required to be atomic in a concurrent program; updates wrapped in
a database transaction; or updates that must be run on a specially
designated processing thread (such as, the GUI thread that drives
the user interface events). Special handling of updates cannot be
implemented through \lstinline!get! and \lstinline!set!; a new effectful
operation (\lstinline!update!) must be provided to the programmer.

Table~\ref{tab:Known-operations-for-some-monads} shows a selection
of effectful operations for some well-known monads.\footnote{Operations were taken from the \texttt{cats-mtl} documentation and
the papers \textsf{``}\emph{Monad transformers and modular interpreters}\textsf{''}
(see \texttt{\href{http://web.cecs.pdx.edu/~mpj/pubs/modinterp.html}{http://web.cecs.pdx.edu/$\sim$mpj/pubs/modinterp.html}})
and \textsf{``}\emph{Modular monad transformers}\textsf{''} (see \texttt{\href{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}}).} The choice of operations appears to be heuristic, not following any
principle or system. It is not obvious in advance what operations
...
requires rewriting the typeclasses involving that monad. Users who
cannot modify the library code will be unable to add new operations.
...
and cannot be lifted to arbtirary monad stacks by composing with a
lift function. For instance, an operation called \lstinline!withEnv!
(defined for the \lstinline!Reader! monad) has the type signature
$R\rightarrow\text{Reader}^{R,A}\rightarrow\text{Reader}^{R,A}$.
...
one cannot transform \lstinline!clear! into a function of type $P^{A}\rightarrow P^{A}$
with an arbitrary monad stack $P$.
...
\]
the lifted operation $\text{clear}_{P}$ is implemented as
\begin{align*}
 & \text{clear}_{P}:\left((W\times A\rightarrow R)\rightarrow R\right)\rightarrow(W\times A\rightarrow R)\rightarrow R\quad,\\
...
transformer will require custom code adapted to a given combination
of monads and operations. So, the length of required custom code is
quadratic in the number of supported monads and linear in the number
of operations.

The third problem is the large number of typeclass instances that
the libraries need to maintain. As we have seen in the example with
...
linear in the number of supported monads. However, for reasons of
run-time efficiency, libraries sometimes provide typeclass instances
for many specific combination of monads (say, \lstinline!Reader!
with \lstinline!State!, \lstinline!Either! with \lstinline!Reader!,
and so on). This makes the required number of typeclass instances
quadratic in the number of supported monads. For this reason, libraries
such as \texttt{cats-mtl} support only a limited number of most frequently
used monads. Adding a new monad to \texttt{cats-mtl} would require
adding a large number of new typeclass implementations.
...
A similar situation occurs for pass/fail monads $L$ that have composed-inside
transformers $T_{L}^{M}=M\circ L$. We may run just the effects of
one of the monads, or both if needed; the runners commute.
...
expect this morphism to be an identity function\footnote{It is unknown whether there exist non-identity monad morphisms $M^{A}\rightarrow M^{A}$
that work for all monads $M$. See Problem~\ref{par:Problem-identity-natural-monad-morphism}.} because the monad $M$ is arbitrary, and the code of \lstinline!flift!
and \lstinline!brun! must be generic in $M$. This requirement is
written as the law
\[
...
of that category are all monad morphisms between those monad stacks.
(Since monad stacks are themselves monads, the category of monad stacks
is a sub-category of the category of monads\index{monads!category of}.) 
...
$T$ is an arbitrary pointed and co-pointed functor in the category
of monads. Such a functor $T$ maps monads $M$, $N$, etc., to monads
$T^{M}$, $T^{N}$, etc. Suppose we were given such a functor $T$,
but were not told that it is a monad transformer for some base monad.
Still, we can \emph{compute} the base monad $L$ as the image of the
identity monad under $T$:
\[
...
must be equal to an identity function, we need to prove that there
exist no other natural transformations between identity functors in
...
we obtain
\[
\text{frun}\,(\text{pu}_{M}):T_{L}^{\text{Id}}\leadsto T_{L}^{M}=L\leadsto T_{L}^{M}.
\]
This function is called the \index{monad transformers!base lift}\textbf{base
lift}, $\text{blift}\triangleq\text{frun}\,(\text{pu}_{M}):L^{A}\rightarrow T_{L}^{M,A}$.
...
for any monads $L$ and $M$. This \textsf{``}transformer\textsf{''} is useless: it
cannot possibly describe the effects of $L$ and $M$, because the
methods of the unit monad discard \emph{all} information and return
...
represent properly the effects of the monads, even though all the
required methods with the correct type signatures are provided. The
only way for us to detect the problem is to establish a violation
of some of the required laws.
...
One would hope to discover a type construction that works in the same
way for all monads $L$ and $M$ and faithfully represents the effects
of both monads at once (i.e., satisfies the laws of monad transformers).
We will now consider a few different ways of combining the type constructors
$L$ and $M$. In all cases, we will find that some monad transformer
laws are violated.
...
\]
But this construction also fails to combine monads.
...
$M^{L^{M^{B}}}$. There is no way of exchanging the layers of $M$
and $L$ when those monads are arbitrary, so we cannot obtain a value
of type $L^{M^{B}}$. A simple formal counterexample is found with
$L^{A}\triangleq\bbnum 1+A$ and $M^{A}\triangleq Z\rightarrow A$
for a fixed type $Z$: the type signature
\[
...
\begin{align*}
 ...
 & \cong L^{(R\rightarrow A)\times S}=S\rightarrow S\times\left(R\rightarrow A\right)\quad.
\end{align*}
...
Therefore, $T_{P}^{S}=T_{P}^{T_{Q}^{M}}$ is a monad.
...
which can interchange the order of the layers. The type signature
...
is a monad when the monad $L$ is a traversable functor. A counterexample
...
functors), but their composition $Q^{A}\triangleq\bbnum 1+A\times A$
...
are required for the \texttt{}\lstinline!swap! operation.\footnote{The \lstinline!swap! operation was introduced in the paper \textsf{``}Composing
...
{\color{greenunder}\text{right identity law for }L:}\quad & =\text{pu}_{M}^{\uparrow M\uparrow L}\bef\text{ftn}_{M}^{\uparrow L}=\gunderline{(\text{pu}_{M}^{\uparrow M}\bef\text{ftn}_{M})}^{\uparrow L}\\
...
{\color{greenunder}\text{outer-interchange for }\text{sw}:}\quad & =\text{sw}^{\uparrow L}\bef\text{ftn}_{L}\bef\gunderline{(\text{sw}^{\uparrow M}\bef\text{sw}\bef\text{ftn}_{M}^{\uparrow L})}^{\uparrow L}\bef\text{ftn}_{L}\bef\text{ftn}_{M}^{\uparrow L}\\
...
{\color{greenunder}\text{naturality of }\text{ftn}_{L}:}\quad & =\text{sw}^{\uparrow L}\bef\gunderline{\text{ftn}_{L}\bef(\text{sw}^{\uparrow M}\bef\text{sw})^{\uparrow L}}\bef\text{ftn}_{L}\bef\text{ftn}_{M}^{\uparrow L}\bef\text{ftn}_{M}^{\uparrow L}\\
{\color{greenunder}\text{naturality of }\text{ftn}_{L}:}\quad & =\text{sw}^{\uparrow L}\bef\gunderline{(\text{sw}^{\uparrow M}\bef\text{sw})^{\uparrow L\uparrow L}\bef\text{ftn}_{L}}\bef\text{ftn}_{L}\bef\text{ftn}_{M}^{\uparrow L}\bef\text{ftn}_{M}^{\uparrow L}\quad.
\end{align*}
...
{\color{greenunder}\text{naturality of }\text{sw}:}\quad & =(\text{sw}^{\uparrow L}\bef\text{ftn}_{L})^{\uparrow M\uparrow L}\bef\big(\text{sw}\bef\gunderline{\text{ftn}_{M}^{\uparrow M\uparrow L}}\big)^{\uparrow L}\gunderline{\bef\text{ftn}_{L}}\bef\text{ftn}_{M}^{\uparrow L}\\
...
{\color{greenunder}\text{functor composition for }L:}\quad & =\text{sw}\bef\gunderline{(\text{pu}_{M}^{\uparrow M}\bef\text{ftn}_{M})}^{\uparrow L}\\
...
{\color{greenunder}\text{substitute }\text{sw}:}\quad & =\gunderline{(\text{pu}_{M}^{\uparrow L\uparrow M}\bef\text{pu}_{L}\bef\text{ftn}_{T})^{\uparrow L}}\bef\text{ftn}_{L}\bef\text{ftn}_{M}^{\uparrow L}\nonumber \\
...
\]
In the same way, the laws of the monad morphism will hold for \textsf{``}$\text{id}$\textsf{''}
...
{\color{greenunder}\text{definition of }\text{pu}_{T}:}\quad & \quad=\text{pu}_{T}\quad,\\
{\color{greenunder}\text{expect to equal }\text{pu}_{T}:}\quad & \text{pu}_{L}\bef\text{blift}=\text{pu}_{L}\bef\text{pu}_{M}^{\uparrow L}\\
{\color{greenunder}\text{naturality of }\text{pu}_{L}:}\quad & \quad=\text{pu}_{M}\bef\text{pu}_{L}=\text{pu}_{T}\quad.
\end{align*}
...
{\color{greenunder}\text{naturality of }\text{ftn}_{L}:}\quad & =\text{sw}_{L,M}^{\uparrow L}\bef\text{ftn}_{L}\bef\gunderline{(\phi^{\uparrow M}\bef\phi\bef\text{ftn}_{N})}^{\uparrow L}\\
...
The first law to be proved is the identity law for $\text{frun}\,(\phi)$:
\begin{align*}
...
{\color{greenunder}\text{naturality of }\theta:}\quad & =(\theta\bef\theta^{\uparrow L})^{\uparrow L}\bef\text{ftn}_{L}\quad.
\end{align*}
...
are fixed types, and $Q$ is a monoid. (The polynomial functor $P+Q\times A$
...
To show that $T_{M}^{L,A}=L^{M^{A}}$ is the monad transformer for
the base monad $M^{A}\triangleq P+Q\times A$, we will implement a
suitable \lstinline!swap! function having the type signature
\[
...
then to compare with the column in Eq.~(\ref{eq:l-interchange-derivation1}).
...
{\color{greenunder}\text{composition}:}\quad & =a\rightarrow\gunderline{(\bbnum 0^{:P}+q\times a)\triangleright\text{sw}}\nonumber \\
...
{\color{greenunder}\text{naturality of }\text{ftn}_{L}:}\quad & =q\times l\rightarrow l\triangleright\text{ftn}_{L}\bef(x\rightarrow\bbnum 0^{:P}+q\times x)^{\uparrow L}\quad.
\end{align*}
...
No; instead we may use the stacking property of monad transformers
...
transformers are available only for monads\index{monads!linear} of
the form $M^{A}=P+Q\times A$ and $M^{A}=Q\times\left(P+A\right)$,
called \textsf{``}linear\textsf{''}. No other examples of composed-inside transformers
...
because one of their general properties is having a single \textsf{``}data
shape\textsf{''} (Statement~\ref{subsec:Statement-rigid-functor-wrapped-unit-is-unit}
...
monads are given in Problem~\ref{par:Problem-monads-3}.
...
by setting $Q\triangleq\bbnum 2$ (where $\bbnum 2$ denotes the \lstinline!Boolean!
type). One may implement a function of type $\left(A\rightarrow\bbnum 2\right)\rightarrow A$
...
and there is a monad morphism in one direction (see Exercise~\ref{subsec:Exercise-selector-and-continuation-monads}).
...
\]
We now implement a suitable \lstinline!swap! function and verify
its special laws. Then the results of Section~\ref{sec:Monad-transformers-that-use-composition}
...
be written as
\begin{align}
 & \text{sw}_{R,M}:M^{R^{A}}\rightarrow R^{M^{A}}\quad,\quad\quad\text{sw}_{R,M}\triangleq m^{:M^{R^{A}}}\rightarrow q^{:H^{M^{A}}}\rightarrow m\triangleright(r\rightarrow q\triangleright\text{pu}_{M}^{\downarrow H}\bef r)^{\uparrow M}\quad,\label{eq:rigid-monad-choice-swap}\\
...
\]
Rewrite the formula in the $\triangleright$-notation to obtain Eq.~(\ref{eq:rigid-monad-choice-swap}):
\[
...
\end{align*}
This code can be shortened by defining the following helper function
(for fixed $g$ and $k$):
\begin{align*}
 & \gamma_{g,k}:B\rightarrow C\quad,\quad\quad\gamma_{g,k}\triangleq\tilde{g}(k)=b\rightarrow g(b)(k)\quad,\\
 & f\diamond_{_{R}}g=a^{:A}\rightarrow k^{:H^{C}}\rightarrow f(a)(k\triangleright\gamma_{g,k}^{\downarrow H})\triangleright\gamma_{g,k}\quad.
\end{align*}
...
Given the code of \lstinline!swap! defined by Eq.~(\ref{eq:rigid-monad-choice-swap}),
...
{\color{greenunder}\text{naturality of }\text{flm}_{R}:}\quad & =\text{flm}_{R}(\text{sw}\bef\text{ftn}_{M}^{\uparrow R})\quad.\label{eq:rigid-monad-ftn-derivation-4a}
\end{align}
...
{\color{greenunder}\text{use Eq.~(\ref{eq:rigid-monad-flm-R-def})}:}\quad & =t\rightarrow k\rightarrow k\triangleright\big(t\triangleright(a\rightarrow(\gunderline{\text{sw}\bef\text{ftn}_{M}^{\uparrow R}})(a)(k))^{\uparrow R}\big)\\
 & =t\rightarrow k\rightarrow k\triangleright\big(t\triangleright(\gunderline{a\rightarrow a\,\triangleright}\,\text{flm}_{M}(r\rightarrow r(k)))^{\uparrow R}\big)\\
...
{\color{greenunder}\text{left naturality of }\text{flm}_{R}:}\quad & =\text{flm}_{R}\big(\gunderline{g^{\uparrow M}\bef(m}\rightarrow q\rightarrow m\triangleright\text{flm}_{M}\left(r\rightarrow r(q)\right))\big)\\
...
{\color{greenunder}\text{naturality of }\text{flm}_{M}:}\quad & =\text{flm}_{R}\big(m\rightarrow q\rightarrow m\triangleright\text{flm}_{M}\big(g\bef(r\rightarrow r(q))\big)\big)=\text{flm}_{R}\big(m\rightarrow q\rightarrow m\triangleright\text{flm}_{M}\big(r\rightarrow g(r)(q))\big)\big)\\
{\color{greenunder}\text{obtain Eq.~(\ref{eq:rigid-monad-def-flm-t-via-flm-r})}:}\quad & =\text{flm}_{R}\big(m\rightarrow q\rightarrow m\triangleright\text{flm}_{M}\big(\gamma_{g,q}\big)\big)\quad.
\end{align*}
...
defined in Eq.~(\ref{eq:rigid-monad-choice-swap}) satisfies the
four laws of Statement~\ref{sec:Statement-swap-laws-to-monad-transformer-first-law}
and the monadic naturality laws of Statement~\ref{subsec:Statement-outside-transformer-swap-laws}.

\subparagraph{Proof}

We will check each of the laws in turn.
...
and $q^{:H^{M^{A}}}$, then transform both sides to the same expression. 
...
{\color{greenunder}\text{use Eq.~(\ref{eq:rigid-monad-choice-swap-short})}:}\quad & =\gunderline{x\rightarrow x\triangleright\big(}r\rightarrow q\triangleright\text{pu}_{M}^{\downarrow H}\bef r\big)^{\uparrow M}\nonumber \\
...
To argue that the third law holds,\footnote{This book does not have a fully rigorous proof of the third monadic
naturality law. The place where the proof lacks rigor will be shown.} apply the left-hand side to $m$ and $q$:
\begin{align}
...
is equivalent to $\forall A.\,H^{A}$. Later steps in the proof assume
that $\theta$ has the form shown in Eq.~(\ref{eq:rigid-monad-base-runner-1}).
...
This concludes the proof of the t laws for the choice monad\textsf{'}s transformer.

\section{Transformers for some monad constructions}

\subsection{Transformer for products of monads}
...
\subsubsection{Exercise \label{subsec:Exercise-monad-product}\ref{subsec:Exercise-monad-product}\index{exercises}}
...
\subsection{Transformer for free pointed monads}
...
the monad $M$, namely $P^{A}\triangleq A+T_{K}^{M,A}$. Mapping the
monad $M$ to $M^{\prime}$ requires us also to map $P$ to $P^{\prime}\triangleq A+T_{K}^{M^{\prime},A}$.
...
with the two monads denoted by $M$, $N$ instead of $L$, $M$. Let
$\phi:N\leadsto M$ be a monad morphism such that the \lstinline!swap!
function obeys the special law~(\ref{eq:swap-special-law-monad-morphism-composition})
below. That law can be seen as a weaker version of a monadic naturality
law of $\text{sw}_{M,N}$ with respect to the parameter $N$ (where
the monad $N$ is transformed by $\phi$ to the fixed monad $M$ rather
...
\subsection{Transformer for free monads}
...
paper did not correctly define the \lstinline!List! monad\textsf{'}s transformer.}
...
\end{equation}
The function \lstinline!prod! then satisfies certain laws, proved
in Statement~\ref{subsec:Statement-laws-of-prod-for-effectful-list}
...
\[
\xymatrix{\xyScaleY{1.5pc}\xyScaleX{1.6pc}L^{M^{L^{A}}}\ar[rd]\sb(0.45){\text{prod}_{M,L}}\ar[r]\sp(0.5){\psi^{\uparrow M\uparrow L}} & L^{M^{L^{\prime A}}}\ar[r]\sp(0.5){\phi^{\uparrow L}} & L^{M^{\prime L^{\prime A}}}\ar[r]\sp(0.5){\psi} & L^{\prime M^{\prime L^{\prime A}}}\ar[d]\sb(0.45){\,\text{prod}_{M^{\prime},L^{\prime}}}\\
...
This law guarantees that the code of $\text{prod}_{M,L}$ works in
the same way for all foreign monads $M$. We will verify this law
in Statement~\ref{subsec:Statement-laws-of-prod-for-effectful-list}
below.
...
identity law:
\begin{align*}
\text{expect to equal \ensuremath{\text{pu}_{M^{\prime}\circ L^{\prime}}:}} & \text{pu}_{M\circ L}\bef\text{frun}\,(\phi)=\text{pu}_{L}\bef\gunderline{\text{pu}_{M}\bef\psi^{\uparrow M}}\bef\phi=\text{pu}_{L}\bef\psi\bef\gunderline{\text{pu}_{M}\bef\phi}\\
...
\end{align*}
Here we computed the following expression:
\[
...
However, $L$ and $L^{\prime}$ do not have the correct monad structure
that we can use.
...
{\color{greenunder}\text{naturality of }\phi:}\quad & \quad=\phi\bef\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\big)^{\uparrow M^{\prime}}\bef\text{flm}_{M^{\prime}}(\text{prod})=\phi\bef\text{flm}_{M^{\prime}}\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\bef\text{prod}\big)\quad.
\end{align*}
The remaining difference is
\[
\text{prod}\bef\psi^{\uparrow M}\bef\phi\overset{?}{=}(\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\bef\text{prod}\quad.
\]
This equality is the same as the monadic naturality law~(\ref{eq:monadic-naturality-law-of-prod-for-list-transformer})
of \lstinline!prod!.
...
{\color{greenunder}\text{naturality of }\phi_{1}:}\quad & =\psi^{\uparrow M}(\phi_{1})\bef\psi^{\uparrow M}(\phi_{2})\bef\phi_{1}\bef\phi_{2}=\big(\psi(\phi_{1})\bef\psi(\phi_{2})\big)^{\uparrow M}\bef\phi_{1}\bef\phi_{2}\\
...
implemented for arbitrary types $A$, similarly to the unimplementable
runner $\text{List}^{A}\rightarrow A$: Since a list may be empty,
it is not always possible to extract a value of type $M^{A}$ out
of a given value of type $T_{\text{List}}^{M,A}$. However, for a
monoid type $R$ with binary operation $\oplus_{R}$ and empty element
$e_{R}$, the type signature $\text{List}^{R}\rightarrow R$ is implemented
...
\end{array}\,\bigg)\\
{\color{greenunder}\text{naturality of }\text{flm}_{M}:}\quad & =\text{flm}_{M}\bigg(\text{pu}_{L}\bef\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M}(e_{R})\\
r\times t\rightarrow\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}\,(t)
\end{array}\,\bigg)\\
...
r\times t\rightarrow\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}\,(t)
\end{array}\,\bigg)\\
 & =\text{flm}_{M}(a\rightarrow\text{pu}_{M}(a)\oplus_{M}\gunderline{\text{brun}(\text{Nil}\triangleright\text{pu}_{M})})=\text{flm}_{M}(a\rightarrow\gunderline{\text{pu}_{M}(a)\oplus\text{pu}_{M}(e_{R})})\\
 & =\text{flm}_{M}(a\rightarrow\text{pu}_{M}(\gunderline{a\oplus e_{R}}))=\text{flm}_{M}(\gunderline{a\rightarrow\text{pu}_{M}(a)})=\text{flm}_{M}(\text{pu}_{M})=\text{id}\quad.
\end{align*}
Here, we used the property that $\text{pu}_{M}(e_{R})$ is the empty
element for $\oplus_{M}$, and also computed
\begin{equation}
...
However, the monad morphism composition law of \lstinline!brun! may
or may not hold (see Problem~\ref{par:Problem-monads-5-2-1}). If
the composition law does not hold, the base runner \lstinline!brun!
must be applied to an entire monadic program. Running several parts
of a program separately and composing the results as values of type
$M^{A}$ will not give the same value as running the entire program.

\paragraph{Monadic naturality laws}

For \lstinline!flift!, the monadic naturality law is
\[
...
{\color{greenunder}\text{naturality of }\phi:}\quad & =\phi\bef(\text{pu}_{L}\bef\psi)^{\uparrow M}\\
...
For \lstinline!brun!, the monadic naturality law is
\[
...
\]
Using $\text{frun}\,(\phi)=\psi^{\uparrow M}\bef\phi=\phi\bef\psi^{\uparrow M^{\prime}}$,
we write the right-hand side as
\begin{align*}
 & \text{frun}\,(\phi)\bef\text{brun}=\phi\bef\psi^{\uparrow M^{\prime}}\bef\text{flm}_{M^{\prime}}\bigg(\,\begin{array}{||c|}
...
naturality of $\oplus_{M}$ defined by Eq.~(\ref{eq:definition-of-oplus-M-monad-monoid}):
\[
...
{\color{greenunder}\text{naturality of }\phi:}\quad & =\phi(p)\triangleright\text{flm}_{M^{\prime}}(u\rightarrow\phi(q)\triangleright(v\rightarrow u\oplus_{R}v)^{\uparrow M^{\prime}})\\
 & =\phi(p)\oplus_{M}\phi(q)\quad.
\end{align*}
...
type $A$ (not necessarily a monoid).
...
\]
Since we do not have a matrix representation for $\text{flm}_{M}$,
it is inconvenient to use matrices at this step. Instead, we substitute
...
Substituting $\text{Nil}$ into the left-hand side yields:
\begin{align*}
{\color{greenunder}\text{left-hand side}:}\quad & \gunderline{\text{Nil}\triangleright\text{prod}}\bef\text{flm}_{M}(q\triangleright\text{ftn}_{T}\triangleright\xi)=\text{Nil}\triangleright\gunderline{\text{pu}_{M}\bef\text{flm}_{M}}(q\triangleright\text{ftn}_{T}\triangleright\xi)\\
...
In the second case, we substitute an arbitrary value of the form $\bbnum 0+h^{:T^{A}}\times t^{:T^{T^{A}}}$.
...
is proved in Statement~\ref{subsec:Statement-monoid-operation-comb-for-effectful-list}.)
...
{\color{greenunder}\text{naturality of }\phi:}\quad & =p\triangleright\gunderline{\text{flm}_{M}(q\triangleright\xi)\bef\phi}\bef\psi^{\uparrow M^{\prime}}\\
...
{\color{greenunder}\text{naturality of }\text{flm}_{M^{\prime}}:}\quad & =p\triangleright\phi\bef\text{flm}_{M^{\prime}}\big((q\triangleright\xi)\bef\phi\bef\psi^{\uparrow M^{\prime}}\big)\quad.
\end{align*}
...
{\color{greenunder}\text{naturality of }\phi:}\quad & =p\triangleright\phi\bef\gunderline{\psi^{\uparrow M^{\prime}}\bef\text{flm}_{M^{\prime}}}\big(q\triangleright\phi\bef\psi^{\uparrow M^{\prime}}\bef\xi\big)\\
...
\end{align*}
which coincides with the right-han side above. This completes the
proof.
...
will use the definition $\text{ftn}_{T}=\text{prod}^{\uparrow M}\bef\text{ftn}_{M}$
only to make the derivation shorter, but we will not use any laws
of $\text{ftn}_{T}$ in the derivation (since those laws will be established
only after this proof is complete).
...
by definition of $T\triangleq M\circ L$). However, we may not directly
assume that the associativity law holds for $\text{ftn}_{T}$. The
allowed inductive hypothesis is that the law~(\ref{eq:associativity-law-of-prod-for-effectful-list})
holds when it involves any recursive calls to \lstinline!prod!. So,
we rewrite the above equation through $\overline{\text{prod}}$:
\begin{align*}
...
{\color{greenunder}\text{associativity law of }\text{flm}_{M}:}\quad & \quad=\text{flm}_{M}(\overline{\text{prod}}\bef\text{flm}_{M}(\text{prod}))\quad,\\
...
{\color{greenunder}\text{naturality of }\text{flm}_{M}:}\quad & \quad=\text{flm}_{M}(\text{ftn}_{T}^{\overline{\uparrow L}}\bef\overline{\text{prod}})=\text{flm}_{M}(\text{ftn}_{T}^{\overline{\uparrow L}}\bef\overline{\text{prod}})\quad.
\end{align*}
It remains to show that
\[
...
h\times t\rightarrow\text{comb}\,(h)(t\triangleright\overline{\text{ftn}}_{T})\triangleright\psi^{\uparrow M}\bef\phi
\end{array}\,=\,\begin{array}{||c|}
1\rightarrow\text{Nil}\bef\text{pu}_{M^{\prime}}\\
h\times t\rightarrow\text{comb}\,(h)(t\triangleright\overline{\text{ftn}}_{T})\triangleright\psi^{\uparrow M}\bef\phi
\end{array}\quad.
\end{align*}
...
\end{array}\quad.
\end{align*}
The difference between the two sides is now in the last rows of the
matrices:
\[
...
At this point, we need to replace $\text{ftn}_{T}$ with \lstinline!prod!
in both sides above:
\begin{align*}
...
{\color{greenunder}\text{naturality of }\phi:}\quad & \quad=\phi\bef\big((\psi^{\uparrow M}\bef\phi)^{\overline{\uparrow L}}\bef\psi\big)^{\uparrow M^{\prime}}\bef\text{flm}_{M^{\prime}}(\overline{\text{prod}})\\
{\color{greenunder}\text{naturality of }\text{flm}_{M}:}\quad & \quad=\phi\bef\text{flm}_{M^{\prime}}((\psi^{\uparrow M}\bef\phi)^{\overline{\uparrow L}}\bef\psi\bef\overline{\text{prod}})\quad,\\
...
{\color{greenunder}\text{naturality of }\text{flm}_{M}:}\quad & \quad=\text{flm}_{M}(\overline{\text{prod}}\bef\psi^{\uparrow M})\bef\phi\\
...
{\color{greenunder}\text{apply }\text{flm}_{M^{\prime}}:}\quad & \text{flm}_{M^{\prime}}\big(\overline{\text{prod}}_{M,L}\bef\psi^{\uparrow M}\bef\phi\big)=\text{flm}_{M^{\prime}}\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\bef\overline{\text{prod}}_{M^{\prime},L^{\prime}}\big)\quad.
\end{align*}
So, we can transform both sides to the following expressions:
\begin{align*}
...
\subsection{Transformer for the state monad}
...
\subsection{Transformer for the continuation monad}
...
and contravariant positions. (In the language of categories, the transformer
\lstinline!ContT! is not a functor in the category of monads.) As
a result, we cannot define the foreign runner or the base lift functions.
This significantly limits the usability of the continuation monad
transformer. 
...
in applications where the values of type $\text{Cont}^{R,A}$ are
suitably restricted. Accordingly, the transformer \lstinline!ContT!
does not have fully parametric base runners, and it is not possible
to formulate their laws (as well as the foreign runner laws and the
monadic naturality laws). We will now verify the remaining laws of
\lstinline!ContT!.
...
\subsection{Transformer for the generalized \texttt{Search} monad}

The generalized\index{monads!generalized Search monad@generalized \texttt{Search} monad}
\lstinline!Search! monad is defined by
\[
\text{Search}^{L,Q,A}\triangleq(A\rightarrow L^{Q})\rightarrow L^{A}\quad,
\]
where $L$ is a fixed monad and $Q$ is a fixed type. The ordinary
\lstinline!Search! monad is obtained with $L\triangleq\text{Opt}$
...
Similarly to the continuation monad, the \lstinline!Search! monad\textsf{'}s
transformer does not support foreign runner, base runner, or base
lift functions.
...
also write $T_{L}$ instead of $T_{L}^{M}$ and simply $T$ instead
of $T_{\text{Search}}^{M}$ (the foreign monad $M$ will be fixed
...
\begin{align*}
 ...
 & f^{:H^{B}\rightarrow A\rightarrow T_{L}^{M,B}}\tilde{\diamond}_{_{T}}g^{:H^{C}\rightarrow A\rightarrow T_{L}^{M,C}}\triangleq h^{:H^{C}}\rightarrow f\big(h\triangleright\text{lift}_{T_{L},H}(g(h))\big)\diamond_{_{T_{L}}}g(h)\quad,\\
 & \text{lift}_{T_{L},H}\triangleq f^{:A\rightarrow T_{L}^{M,B}}\rightarrow h^{:B\rightarrow T_{L}^{M,Q}}\rightarrow f\diamond_{_{T_{L}}}h\quad.
\end{align*}
...
{\color{greenunder}\text{compute composition}:}\quad & =u\rightarrow w\rightarrow\gunderline{(w\bef r)\triangleright\big(v}\rightarrow(v\bef s)\triangleright u\bef r\big)\bef s\\
 & =u\rightarrow w\rightarrow(w\bef\gunderline{r\bef s})\triangleright u\bef\gunderline{r\bef s}=u\rightarrow(\gunderline{w\rightarrow w\,\triangleright}\,u)=u\rightarrow u=\text{id}\quad.
\end{align*}
...
{\color{greenunder}\text{composition law of }\text{flift}_{L}:}\quad & =\text{flip}\,\big(\gunderline{\_\rightarrow(p\diamond_{_{M}}q)\bef\text{flift}_{L}}\big)=\gunderline{\text{flip}\,\big(\text{flip}}((p\diamond_{_{M}}q)\bef\text{flift})\big)\\
{\color{greenunder}\text{use }\text{flip}\circ\text{flip}=\text{id}:}\quad & =(p\diamond_{_{M}}q)\bef\text{flift}\quad.
\end{align*}


\subsection{The codensity monad and its transformer}

The \index{monads!codensity monad}\textbf{codensity monad}\footnote{See, e.g., the talk \texttt{\href{https://begriffs.com/posts/2016-02-04-difference-lists-and-codennsity.html}{https://begriffs.com/posts/2016-02-04-difference-lists-and-codennsity.html}}
for applications of the codensity monad in functional programming
and for more references.} on a type constructor $F$ is defined by 
\[
...
parameter $A$ with a fixed $R$. Then \textbf{(a)} $L^{A}\triangleq\forall R.\,K^{R,A}$
...
\begin{align*}
{\color{greenunder}\text{right-hand side}:}\quad & k\rightarrow(f\bef\text{inC})^{T}(a)\big(\gunderline{b\rightarrow b\,\triangleright\,}g\bef\text{flm}_{F}(k)\big)=k\rightarrow(f\bef\text{inC})(a)(g\bef\text{flm}_{F}(k))\\
...
in the argument of $f$ because $f$ is an arbitrary function. In
part (c) of this proof, we will see an example that violates the composition
law of \lstinline!outC!.
...
\end{align*}
This shows explicitly that $\text{outC}\bef\text{inC}\neq\text{id}$
at least for some monads $F$. $\square$
...
constructor. The transformer $T_{\text{Cod}}$ does not have foreign
runner, base lift, or base runner methods.
...
\paragraph{The monad construction law}
...
\subsection{Examples of monads with two different transformers}

Most monads have only one monad transformer. This section shows two
exceptional cases where a monad has two \emph{inequivalent} transformers.
As we will see, in both cases the first transformer is complete (has
all required methods) but the second is incomplete.
...
\lstinline!Search! monad with $L$ chosen to be the identity monad:
\[
\text{Sel}^{Q,A}\cong\text{Search}^{\text{Id},Q,A}\quad.
\]
So, the selector monad $\text{Sel}^{Q,A}$ also has a transformer
...
So, the codensity type constructor is a transformer \emph{for the
identity monad}. The identity monad also has a standard transformer,
...
The type constructor $\text{Cod}^{M,\bullet}$ is an alternative transformer
for the identity monad ($\text{Id}$) applied to the foreign monad
$M$. The transformer is incomplete as it lacks a foreign runner,
and its base runner fails the composition law of monad morphisms.

\subparagraph{Proof}

The monad construction laws hold because $\text{Cod}^{M,\bullet}$
is a monad (Statement~\ref{subsec:Statement-codensity-monad-lawful}). 
...
For any monads $M$ and $N$, the swapping function $\sigma:M^{\bullet}\times N^{\bullet}\leadsto N^{\bullet}\times M^{\bullet}$
is a monad morphism.
...
we proved the monad transformer laws for $T_{R}^{M}\triangleq R\circ M$
...
{\color{greenunder}\text{naturality of }\text{sw}_{R,S}:}\quad & =\text{sw}_{R,M}\bef\big(\gunderline{\text{sw}_{S,M}\bef\text{sw}_{R,M}^{\uparrow S}\bef\text{sw}_{R,S}}\bef\text{sw}_{S,M}^{\uparrow S\uparrow R}\big)^{\uparrow R}\bef\text{ftn}_{R}\bef\text{ftn}_{S}^{\uparrow R}\\
...
{\color{greenunder}\text{naturality of }\text{sw}_{R,M}:}\quad & =\text{sw}_{R,S}^{\uparrow R\uparrow M}\bef\gunderline{\text{ftn}_{R}^{\uparrow M}\bef\text{sw}_{R,M}}\bef\text{ftn}_{S}^{\uparrow M\uparrow R}\bef\text{sw}_{S,M}^{\uparrow R}\\
...
{\color{greenunder}\text{naturality of }\text{ftn}_{R}:}\quad & =\text{sw}_{R,M}\bef\gunderline{\text{sw}_{R,S}^{\uparrow M\uparrow R}\bef\text{sw}_{R,M}^{\uparrow R}\bef\big(\text{sw}_{S,M}\bef\text{sw}_{S,M}^{\uparrow S}\big)^{\uparrow R\uparrow R}}\bef\text{ftn}_{R}\bef\text{ftn}_{S}^{\uparrow R}\\
...
we can express $\theta=\theta_{R}\bef\theta_{S}$ with some monad
morphisms $\theta_{R}$ and $\theta_{S}$. So, we may use $\theta_{R}$
...
define $\theta_{R}$ and $\theta_{S}$ from the given runner $\theta$
as
\[
\theta_{R}\triangleq\text{pu}_{S}^{\uparrow R}\bef\theta\quad,\quad\quad\theta_{S}\triangleq\text{pu}_{R}\bef\theta\quad.
\]
Since $\text{pu}_{R}$ and $\text{pu}_{S}^{\uparrow R}$ are the foreign
and base lifts of the monad transformer $T_{R}^{S}=R\circ S$, Statement~\ref{subsec:Statement-outside-transformer-swap-laws}
...
can handle multiple $M$-effects at once. As motivation, consider
...
\[
\text{flm2}_{M}:\left(A\rightarrow M^{B}\times M^{B}\right)\rightarrow M^{A}\rightarrow M^{B}\times M^{B}\quad,\quad\quad\text{flm2}_{M}\triangleq f\rightarrow\Delta\bef\big(\text{flm}_{M}(f\bef\pi_{1})\boxtimes\text{flm}_{M}(f\bef\pi_{2})\big)\quad.
\]
...
For a rigid functor $R$ and a monad $M$, there exists an \textsf{``}$R$-valued
...
Can we obtain a transformed program $q:R^{C}$ that has the same functionality
as $p$ except for also returning a value wrapped in $R$? This is
possible if $R$ is a rigid functor:
...
have a \lstinline!swap! method. It turns out that \emph{all} rigid
monads (regardless of the existence of \lstinline!swap!) are also
rigid functors. While proving that statement, we will directly use
the definition of rigid monads via the composed-outside monad transformer,
without assuming a \lstinline!swap! method.
...
\[
\xymatrix{\xyScaleY{2pc}\xyScaleX{4pc}(A\rightarrow R^{B})\ar[r]\sp(0.5){\text{pu}_{M}^{\uparrow R\uparrow M}}\ar[d]\sb(0.5){\text{fi}_{R}\triangleq} & (A\rightarrow R^{A\rightarrow B})\ar[d]\sp(0.5){\text{pu}_{R}}\\
...
\[
\text{pu}_{R}(x)\triangleq(\text{fi}_{R}(\text{id}))\triangleright(\_\rightarrow x)^{\uparrow R}=\text{id}\triangleright\text{fi}_{R}\triangleright(\_\rightarrow x)^{\uparrow R}\quad.
\]
...
which will give a monad morphism $\bar{\theta}_{i}$ with type signature
\[
...
\]
Prove that any two runners $\bar{\theta}_{i}$ commute: $\bar{\theta}_{i}\bef\bar{\theta}_{j}=\bar{\theta}_{j}\bef\bar{\theta}_{i}$
for any $i\neq j$.
...
(with the same fixed monoid type $W$). 
...
The function 
\begin{lstlisting}
def finder[A, P](s: (A => Option[P]) => A): (A => Option[P]) => Option[A] = { p: (A => Option[P]) =>
  p(s(p)).map { _ => s(p) }
}
\end{lstlisting}
\[
