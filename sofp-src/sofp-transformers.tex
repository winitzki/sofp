...
are not subtypes of each other, so we cannot simply use them together
in a functor block:
\begin{lstlisting}
for {
  x <- Future(1)
...
where the incorrect examples are marked with a strike-through line).
...
$Z\rightarrow A$ always composes on the outside (Statement~\ref{subsec:Statement-monad-construction-1})
but not necessarily on the inside. On the other hand, pass/fail monads
$M^{A}\triangleq Z+A$ composes when it is on the inside.
...
For example, the \lstinline!Writer! monad $W\times A$ (appending
...
We will see throughout this chapter that, even when monads do not
compose, effects can still be combined in different orders. The programmer
needs to choose carefully the desired order of composition, according
to the desired meaning of the effects.

When functor composition works, implementing the \lstinline!lift!
methods is straightforward:

\subsubsection{Example \label{subsec:Example-lift-for-List-Try-composition}\ref{subsec:Example-lift-for-List-Try-composition}\index{solved examples}}
...
The length of the required code would be quadratic in the number of
supported monads. To avoid the code explosion, we will use a different
approach to organizing the code that combines monads.

The main idea of that approach is to fix the first monad $L$ (the
\textsf{``}base\textsf{''} monad) and to write code that combines $L$ with an arbitrary
...
the code of $T_{L}^{M}$ is fully parametric in $M$. In other words,
the code of the \lstinline!pure! and \lstinline!flatMap! methods
for $T_{L}^{M}$ will work in the same way for all foreign monads
$M$ and will only depend on $M$\textsf{'}s monadic methods ($\text{pu}_{M}$
and $\text{flm}_{M}$). The monad $T_{L}^{M}$ is called the \textsf{``}$L$-transformer
applied to $M$\textsf{''} or the \textsf{``}transformed monad $M$\textsf{''}. The \textbf{monad
transformer} of\index{monad transformer} the monad $L$ is the type
constructor $T_{L}^{M}$ viewed as a function of its type parameters
$M$ and $A$.
...
final case class TryT[M[_]: Monad : Functor, A](value: M[Try[A]]) {
  def map[B](f: A => B): TryT[M, B] = TryT(value.map(_.map(f)))     // Using M\textsf{'}s map here.
  def flatMap[B](f: A => TryT[M, B]): TryT[M, B] = TryT(
    value.flatMap {                                                 // Using M\textsf{'}s flatMap here.
...
This example shows how to define the monad transformer $T_{\text{Try}}^{M}$
that works parametrically in the foreign monad $M$. The result is
a type constructor that we denote by $T_{\text{Try}}^{M,A}$. We can
use that type as a monad with respect to the type parameter $A$.
(The foreign monad $M$ is usually the first type parameter in the
transformer.) Similar code is required for $T_{\text{Read}}^{M}$,
$T_{\text{Write}}^{M}$, $T_{\text{State}}^{M}$, and all other monads.
The advantage of the transformer approach is that the code for $T_{L}^{M}$
needs to be written once for every base monad $L$ (and not for every
pair $L$, $M$).
...
so until it was corrected in 2007. See a discussion in \texttt{\href{https://wiki.haskell.org/index.php?title=ListT_done_right}{https://wiki.haskell.org/index.php?title=ListT\_done\_right}}} The only justification for those choices is that they work and obey
the required laws, while no alternative transformers are known. We
will derive and prove the required laws for these and other transformers
...
monad transformers. The first task is to obtain the implementations
of monad methods and lifts for the transformers from Table~\ref{tab:Known-monad-transformers}.
...
{\color{greenunder}\text{base lift}:}\quad & \text{blift}:(R\rightarrow A)\rightarrow T_{\text{Reader}}^{M,A}\quad,\quad\quad\text{blift}\,(t^{:R\rightarrow A})\triangleq r^{:R}\rightarrow\text{pu}_{M}(t(r))=t\bef\text{pu}_{M}\quad.
\end{align*}
...
The only way to get a value of type $A$ wrapped in $M^{A\times W}$
...
\end{lstlisting}
The \lstinline!List! monad was defined using the list concatenation
function \lstinline!concat!; \textsf{``}flattening\textsf{''} a \lstinline!List[List[A]]!
means concatenating all nested lists. If the ordinary \lstinline!List!
were defined via \lstinline!Option!, the code for \lstinline!concat!
would be
\begin{lstlisting}
...
\begin{lstlisting}
final case class ListT[M[_]: Monad : Functor, A](value: M[Option[(A, ListT[M, A])]]) {
  def map[B](f: A => B): ListT[M, B] = ListT(t.map(_.map { // Use M\textsf{'}s and Option\textsf{'}s map methods.
...
because the meaning of the monad\textsf{'}s value is usually the entire set
...
to run the $M$\textsf{'}s effects if we are to extract values from a \lstinline!ListT!.
...
of an \textsf{``}infinite\textsf{''} collection uses the \lstinline!Reader! monad
as $M$:
\begin{lstlisting}
...
final case class StateT[M[_]: Monad : Functor, S, A](run: S => M[(A, S)]) {
  def map[B](f: A => B): StateT[M, S, B] = StateT(s1 => run(s1).map { // Use M\textsf{'}s map method.
    case (a, s2) => (f(a), s2)
  })
...
\end{lstlisting}
\begin{align}
 & T_{\text{State}}^{M,A}\triangleq T^{A}\triangleq S\rightarrow M^{A\times S}\quad,\quad\quad\text{pu}_{T}\triangleq a^{:A}\rightarrow s^{:S}\rightarrow\text{pu}_{M}(a\times s)\quad,\nonumber \\
...
because that transformer is not a functor composition. Instead, we
can implement a special \textsf{``}base runner\textsf{''} that converts $T_{\text{State}}^{M,A}$
into $M^{A}$ by running the \lstinline!State! monad\textsf{'}s effects but
...
while getting a value of type $(A\rightarrow M^{R})\rightarrow M^{R}$
...
as freely as other monads. Namely, we cannot lift an arbitrary given
value of type \lstinline!Cont! (for instance, a value produced by
a third-party library that cannot be modified) into the transformed
monad \lstinline!ContT!. This is a serious deficiency of the continuation
monad that makes it unsuitable for certain applications. Concrete
values of type \lstinline!Cont[R, A]! are often interfaces to system
libraries implemented in non-fully parametric code. It may be impossible
to rewrite that code to return \lstinline!ContT[M, R, A]! with an
arbitrary monad \lstinline!M!. 
...
\begin{lstlisting}
def flift[A](m: M[A]): SelT[M, Z, A] = SelT(_ => m) // This is the Sel monad\textsf{'}s `pure` method.
...
\subsection{Combining more than two monads}
...
The type of $T_{\text{State}}^{\text{Reader}}$ is equivalent to that
of $T_{\text{Reader}}^{\text{State}}$: the only difference is the
flipped order of curried arguments. To verify that the implementations
of \lstinline!pure! and \lstinline!flatMap! in these transformers
are equivalent up to that order, we begin by writing \lstinline!flatMap!
for the \lstinline!Reader! and \lstinline!State! monads:
\begin{align*}
...
An example of inequivalent transformers is the pair $T_{\text{Reader}}^{\text{List}}$
and $T_{\text{List}}^{\text{Reader}}$. These two monads describe
different ways of combining the effects of \lstinline!Reader! and
\lstinline!List!. In a value of type $T_{\text{Reader}}^{\text{List}}$,
the injected dependency (of type $R$) is used to produce an \textsf{``}eager\textsf{''}
list of values. The length of the resulting list, as well as every
value in it, are determined at once depending on the injected value
of type $R$. In contrast, a data structure of type $T_{\text{List}}^{\text{Reader}}$
...
resulting on-call sequence may be unlimited in length.

The types of $T_{\text{State}}^{\text{Either}}$ and $T_{\text{Either}}^{\text{State}}$
are also not equivalent. The effect of $\text{State}^{S,A}=S\rightarrow A\times S$
is to produce an updated internal state alongside with a result value
(of type $A$). Let us look at the types of the transformed monads:
\[
...
description (of type $E$) and produce no new state and no result
value. The second type describes a computation that never fails to
update the state, although it could fail to give a result value. 

This example shows that effects of the two monads are not always independent
within the larger transformed monad. An effect of one monad (\lstinline!Either!)
could \textsf{``}cancel\textsf{''} an effect of the other (\lstinline!State!).

The monad stack $T_{K}^{T_{L}^{M}}$ is a monad whose code is parametric
in $M$ and thus can be viewed as the monad transformer \emph{for
the monad} $T_{K}^{L}$:
\begin{equation}
...
for each monad (except possibly for the last one).

We may also view a monad transformer as a binary operation ($\varangle$)
that combines monads $K$ and $L$ into a new monad $K\varangle L\triangleq T_{K}^{L}$.
This operation is associative \emph{by construction}: the transformer
for the monad $T_{K}^{L}$ is defined via Eq.~(\ref{eq:monad-transformers-associativity-of-combination}),
which we can rewrite as
\[
\left(K\varangle L\right)\varangle M=K\varangle\left(L\varangle M\right)\quad.
\]
 A monad stack is then denoted unambiguously by, e.g., $K\varangle L\varangle M\varangle N$
without need for parentheses.

Is there a e unit element for the binary operation ($\varangle$)?
We notice from Table~\ref{tab:Known-monad-transformers} that each
transformer $T_{L}^{M}$ somehow mixes the foreign monad $M$ with
the type of the base monad $L$. In every example, if we set $M$
to be the identity monad, $\text{Id}^{A}\triangleq A$, the transformed
type will become equivalent to the base monad $L$. Intuitively, the
identity monad is a trivial effect; combining it with another monad
should not change that monad. So, we may expect this to hold for every
transformer:
\[
T_{L}^{\text{Id}}\cong L\quad,\quad\quad L\varangle\text{Id}\cong L\quad.
\]
We also define the monad transformer for the identity monad as equal
to the foreign monad:
\[
T_{\text{Id}}^{M}\triangleq M\quad,\quad\quad\text{Id}\varangle M=M\quad.
\]

Because of these properties of the identity monad, the base monad
$L$ can be recovered as as $T_{L}^{\text{Id}}$ if we know the transformer
$T_{L}^{M}$. So, if we already have the type and an implementation
for a monad\textsf{'}s transformer, we do not need to define the monad\textsf{'}s type
and methods separately. This saves time when implementing monad code
in libraries.

It is unknown\footnote{For a discussion, see \texttt{\href{https://stackoverflow.com/questions/24515876/}{https://stackoverflow.com/questions/24515876/}}}
whether a transformer exists for every monad relevant to functional
programming; say, for every monad defined by purely functional code.\footnote{Monad transformers may certainly fail to exist for monads defined
using low-level, non-purely functional code with side effects. A lawful
transformer must allow the foreign monad to cancel the effect of the
base monad, as we have seen in an example with \lstinline!State!
and \lstinline!Either!. However, non-referentially transparent code
may perform side effects that cannot be undone or canceled (picture
this as \textsf{``}launching missiles\textsf{''}). See \texttt{\href{https://stackoverflow.com/questions/11792275/}{https://stackoverflow.com/questions/11792275/}}} However, this chapter will show that lawful transformers exist for
every monad derived anywhere in this book. 

\subsection{Lift operations for monad stacks}

It remains to see how we could define the lifts and the runners for
a given monad stack.{*}{*}{*}
...
\subsection{Motivation for the laws of monad transformer lifts}
...
The programmer certainly expects all previously accepted properties
...
alone. It follows that $T$ must itself satisfy the monad laws, which
we derived in Chapter~\ref{chap:Semimonads-and-monads} from expected
functor block refactorings. 

There are some additional code refactorings involving lifted monadic
values that occur before or after a source line with an arbitrary
value of $T$. Similarly to the standard monadic refactorings, we
consider three cases: a \lstinline!pure! method before a source line,
a \lstinline!pure! method after a source line, and refactoring two
adjacent source lines into a separate functor block.
...
An associative operation can have only one two-sided unit. So, the
first two laws are equivalent to a single\index{identity laws!of monad transformer\textsf{'}s lift}
\textbf{identity law}, 
\[
...
The function \lstinline!blift! maps values $L^{A}$ to $T^{A}$,
and the identity and composition law enforce its compatibility with
the monad methods of $L$ and $T$. Laws of this form were introduced
in Section~\ref{subsec:Monads-in-category-theory-monad-morphisms}
...
\lstinline!blift! be a monad morphism $L\leadsto T$.

Since we expect to be able to use the monads $L$ and $M$ equally
well within a functor block, the same refactorings must apply when
we use values of type $M$ instead of $L$. Thus, the laws of monad
morphisms must hold also for the foreign lift function ($\text{flift}:M\leadsto T$).
...
\subsection{Motivation for the laws of monad transformer runners}
...
this is the case for $L=\text{Reader}$ and some other monads. Given
a runner $\theta_{M}$, we may first run $M$\textsf{'}s effects by lifting
$\theta_{M}$ to $L$,
\[
...
We may run just the effects of one of the monads, or both if needed,
and the runners are commutative.
...
$\theta_{M}$. The foreign runner must be a function of $M$\textsf{'}s runner
($\theta_{M}$). The effects of the base monad and of the foreign
monad may be run in any order, and the results must be the same.
...
such as \lstinline!Future!; the \lstinline!Option! monad is mapped
to the \lstinline!Either! monad. 

When a foreign monad $M$ requires a \textsf{``}extended\textsf{''} runner of the
form $\phi:M\leadsto N$ with a nontrivial target $N\not\cong\text{Id}$,
we would like to be able to map the monad stacks accordingly as $T_{L}^{M}\leadsto T_{L}^{N}$,
partially running the effects of $M$ but keeping the effects of $L$
...
easily compute a map $\text{frun}_{L}\left(\phi\right):T_{L}^{M}\leadsto T_{L}^{N}$
that runs the stacks as required.

When the base monad $L$ requires a runner with a nontrivial target
$K$, we would similarly need a monad morphism $T_{L}^{M}\leadsto T_{K}^{M}$,
in order to partially run the effects of $L$. Since $T_{K}^{M}$
depends on $K$ non-parametrically, we cannot expect to have a universal
base runner that uses $\phi:L\leadsto K$ as a parameter. The extended
base runner $\text{brun}_{L,K}:T_{L}^{M}\leadsto T_{K}^{M}$ will
need to have custom code specially adapted to the monads $L$ and
$K$.

The pass/fail monads (such as \lstinline!Option! and \lstinline!Try!)
show one example where the extended base runner exists, because the
monad transformers for pass/fail monads work by functor composition.
If $L^{A}=\bbnum 1+A$ and $K^{A}\triangleq E+A$ (where $E$ is a
fixed type representing error information), we can choose an arbitrary
runner $\phi:L\leadsto K$ and compute the corresponding base runner
as 
\[
\text{brun}_{L,K}:(T_{L}^{M}\leadsto T_{K}^{M})\triangleq\phi^{\uparrow M}\quad.
\]
...
\]
We could also use the extended runners to map $T_{L}^{\text{Cont}}\leadsto T_{L}^{\text{Future}}$
...
\[
\text{frun}_{L}(\phi_{1})\bef\text{frun}_{L}(\phi_{2})\overset{!}{=}\text{frun}_{L}(\phi_{1}\bef\phi_{2})\quad.
\]
...
\[
\text{lift}\bef\text{frun}_{L}(\theta_{M}):M^{A}\rightarrow L^{A}\quad.
\]
Note that both \lstinline!lift! and \lstinline!frun! are monad morphisms,
so their composition is again a monad morphism $M\leadsto L$. Since
$M$ is an arbitrary foreign monad, we cannot expect the monad $L$
to describe any effects of the foreign monad $M$. So, the result
...
to the value $m:M^{A}$. We obtain the law 
\[
m\triangleright\text{lift}\triangleright\text{frun}_{L}(\theta_{M})=m\triangleright\theta_{M}\triangleright\text{pu}_{L}\quad,\quad\quad\text{lift}\bef\text{frun}_{L}(\theta_{M})=\theta_{M}\bef\text{pu}_{L}\quad.
\]

The analogous law for \lstinline!blift! and \lstinline!brun! is
\[
\text{blift}\bef\text{brun}=\theta_{L}\bef\text{pu}_{M}\quad.
\]

In the second case, we lift a value $m:M^{A}$ to the stack, obtaining
a value $t\triangleq\text{lift}\left(m\right)$, and then apply the
\emph{base} runner to $t$ (running just the effects of $L$). This
yields a monad morphism $M^{A}\rightarrow M^{A}$. We expect this
to be an identity function because the monad $M$ is arbitrary, and
the code of \lstinline!lift! and \lstinline!brun! must be generic
in $M$. This requirement is written as the law
\[
m\triangleright\text{lift}\triangleright\text{brun}_{L}\overset{!}{=}m\quad,\quad\quad\text{lift}\bef\text{brun}\overset{!}{=}\text{id}\quad,
\]
called the \textbf{non-degeneracy law}\index{monad transformers!non-degeneracy law}
of \lstinline!lift! and \lstinline!brun!. The law requires \lstinline!lift!
...
\subsection{Category theory view of lifts and runners }

The lifting composition law{*}{*}{*} suggests that we should consider
a lifting from monad morphisms $M\leadsto N$ to monad morphisms $T_{L}^{M}\leadsto T_{L}^{N}$.
In the language of category theory, a monad morphism is a morphism
in the category of monads. So, it appears that $\text{frun}_{L}$
is the morphism mapping function for a (categorical) functor from
the category of monads ($M$, $N$, ...) to the category of monad
stacks with base $L$. The objects of that category are monads $T_{L}^{M}$,
$T_{L}^{N}$, etc., all with the same base monad $L$.
...
is a monad morphism. (In a shorter notation, $\text{lift}:M\leadsto T_{L}^{M}$.)
\item \textbf{Runner laws}:\index{monad transformers!runner laws} For any
monads $M$, $N$ and any monad morphism $\phi:M\leadsto N$, the
runner $\text{frun}\left(\phi\right):T_{L}^{M}\leadsto T_{L}^{N}$
is a monad morphism. Moreover, the function \lstinline!frun! lifts
monad morphisms from $M\leadsto N$ to $T_{L}^{M}\leadsto T_{L}^{N}$
and must satisfy the corresponding \textbf{functor laws}\index{monad transformers!functor laws of runner}:
\[
...
\[
\text{blift}\bef\text{frun}_{L}(\phi^{:M\leadsto\text{Id}})=\text{id}\quad,
\]
...
For any monad morphism $\theta_{L}:L\leadsto\text{Id}$ and for any
monad $M$, the base runner $\text{brun}\left(\theta_{L}\right):T_{L}^{M}\leadsto M$
is a monad morphism. The base runner must also satisfy the \textbf{non-degeneracy
...
does not support an arbitrary other base monad, there are no functor
laws for \lstinline!brun!, unlike \lstinline!frun!. The non-degeneracy
law for \lstinline!brun! is not an automatic consequence of other
laws.
\end{enumerate}
In total, we listed 15 laws for monad transformers. Are all these
laws necessary? 

The main use of the laws is to verify that the code is written correctly.
The next section shows some examples of incorrect implementations
of monad transformers and indicates the laws that are violated.
...
for any monads $L$ and $M$. It is clear that this \textsf{``}transformer\textsf{''}
is completely wrong: it cannot possibly keep the information about
the effects of $L$ and $M$, because the methods of the unit monad
discard \emph{all} information and return $1$. However, the type
constructor $T_{L}^{M,A}$ has all the required methods $\text{pu}_{T}$,
$\text{ftn}_{T}$, $\text{lift}_{T}$, $\text{frun}_{T}$, and $\text{brun}_{T}$
with the required type signatures (they are constant functions returning
$1$). All these functions are automatically monad morphisms, since
a function from any monad to the unit monad is always a monad morphism.
So, the fake \textsf{``}transformer\textsf{''} satisfies many of the monad transformer
...
laws are $T_{L}^{M}=L$ (no lifting law) and $T_{L}^{M}=M$ (no identity
law).
...
missing in $T_{L}^{M}$. A potentially working definition of $T_{L}^{M}$
must be a type constructor that somehow combines both $L$ and $M$.
Many such definitions are possible, but few will satisfy the monad
transformer laws.
...
find that there is no naturally defined $\text{lift}:M^{\bullet}\leadsto L^{\bullet}\times M^{\bullet}$
...
In Scala code, this \textsf{``}stacking\textsf{''} composition is written as
\begin{lstlisting}
...
To shorten the notation, we talk about a \textsf{``}monad $T_{P}^{M}$\textsf{''}
...
via a monadic isomorphism. So, we already have a monad morphism $\phi:Q\leadsto T_{Q}^{\text{Id}}$
...
We need to show that there exists a monad morphism $M\leadsto T_{P}^{T_{Q}^{M}}$
...
\]
The composition of this with $\text{lift}_{Q}$ is a monad morphism
of the required type $M\leadsto T_{P}^{T_{Q}^{M}}$. (A composition
of monad morphisms is again a monad morphism by Statement~\ref{subsec:Statement-monadic-morphism-composition}.)
...
monad morphisms, $\text{brun}_{R}\theta$ is a monad morphism by Statement~\ref{subsec:Statement-monadic-morphism-composition}.
...
To avoid confusion, we the function $\text{sw}_{L,M}:M^{L^{\bullet}}\leadsto L^{M^{\bullet}}$
\textsf{``}swap\textsf{''} rather than \textsf{``}sequence\textsf{''} in the context of monad transformers.
Let us now find out what laws are required for the \texttt{}\lstinline!swap!
operation.\footnote{The \lstinline!swap! operation was used in the paper \textsf{``}Composing
...
(the \textsf{``}linear\textsf{''} and the \textsf{``}rigid\textsf{''} monads) are defined via \lstinline!swap!.
...
and the foreign monad: the runner, \lstinline!frun!, is generic in
...
the runners are
\begin{align*}
\text{frun}\,\phi^{:M^{\bullet}\leadsto N^{\bullet}} & :L^{M^{\bullet}}\leadsto L^{N^{\bullet}}\quad,\quad\quad\text{frun}\,\phi=\phi^{\uparrow L}\quad;\\
...
hold for $\text{frun}\,\phi$ and $\text{brun}\,\theta$:
\begin{align*}
...
\end{align*}
The next law to be shown is the composition law for $\text{frun}\,\phi$:
\begin{align*}
...
The lifting laws for $\text{frun}$ are
\[
\text{frun}\left(\text{id}\right)=\text{id}\quad,\quad\quad\text{frun}\left(\phi\right)\bef\text{frun}\left(\chi\right)=\text{frun}\left(\phi\bef\chi\right)\quad.
\]
...
\begin{align*}
\text{frun}\,\phi^{:L^{\bullet}\leadsto N^{\bullet}} & :L^{M^{\bullet}}\leadsto N^{M^{\bullet}}\quad,\quad\quad\text{frun}\,\phi=\phi\quad;\\
...
transformer of a \textsf{``}composed\textsf{''} kind. It is easier to check the 6
...
We have just shown that the \textsf{``}\lstinline!Either!/\lstinline!Writer!\textsf{''}
...
The other type of linear monad is the \textsf{``}\lstinline!Writer!/\lstinline!Either!\textsf{''}
monad $W^{A}\triangleq Q\times\left(P+A\right)$. Do we need to show
...
and $M^{A}=Q\times\left(P+A\right)$, called \textsf{``}linear\textsf{''}. It turns
out that the composed-\emph{outside} transformers are available for
a significantly wider range of monads. Those monads are called \textsf{``}rigid\textsf{''}
because one of their general properties is having a single \textsf{``}shape\textsf{''}
...
(the reader monad), as well as the \textbf{selector}\footnote{See \href{http://math.andrej.com/2008/11/21/}{http://math.andrej.com/2008/11/21/}}\textbf{
monad}\index{monads!Sel (selector monad)@\texttt{Sel} (selector monad)}\index{selector monad}
$\text{Sel}^{Q,A}\triangleq\left(A\rightarrow Q\right)\rightarrow A$. 
...
is found by setting $Q\triangleq\text{Boolean}$. One may implement
a function of type $\left(A\rightarrow\text{Boolean}\right)\rightarrow A$
that \emph{somehow} finds a value of type $A$ that might satisfy
the given predicate of type $A\rightarrow\text{Boolean}$. The intention
...
A closely related monad is the search-with-failure monad, $R^{A}\triangleq\left(A\rightarrow\text{Boolean}\right)\rightarrow\bbnum 1+A$.
...
a natural transformation \lstinline!fuseIn! (short notation \textsf{``}$\text{fi}_{R}$\textsf{''})
...
the identity monad. The corresponding \textsf{``}runner\textsf{''} $\phi_{a}^{:M\leadsto\text{Id}}$
is
\begin{equation}
...
satisfies all laws) says that the lifted \textsf{``}runner\textsf{''} $\phi_{a}^{\uparrow R}$
...
and derive the connection between the \textsf{``}runner\textsf{''} $\phi_{a}$ and
...
such as $\bbnum 1+A+A$ or $\text{List}^{A}$. A rigid functor\textsf{'}s type
definition must have a single constructor. This is one motivation
for the name \textsf{``}rigid\textsf{''}: the container $R^{A}$ has a fixed shape
...
cannot be rigid. This partially explains the choice of the name \textsf{``}rigid\textsf{''}:
a rigid functor has a fixed \textsf{``}shape\textsf{''} of data wrapped in it.
...
The \textsf{``}$R$-valued \lstinline!flatMap!\textsf{''} is a generalization of
...
that $\text{inC}\,\bef\text{outC}=\text{id}$
...
also does not have any of the required \textsf{``}runner\textsf{''} transformations
$\text{frun}$ and $\text{brun}$,
\begin{align*}
\text{frun} & :\left(M^{\bullet}\leadsto N^{\bullet}\right)\rightarrow\left(\forall B.\left(A\rightarrow M^{F^{B}}\right)\rightarrow M^{F^{B}}\right)\rightarrow\forall C.\left(A\rightarrow N^{F^{C}}\right)\rightarrow N^{F^{C}}\quad,\\
...
product $\pi_{1}\boxtimes\pi_{2}$ is a monad morphism by Statement~\ref{subsec:Statement-product-of-monadic-morphisms}.
