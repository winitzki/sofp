...
\subsection{Monad transformers for standard monads}
...
so until it was corrected in 2007. See a discussion in \texttt{\href{https://wiki.haskell.org/index.php?title=ListT_done_right&oldid=10815}{https://wiki.haskell.org/index.php?title=ListT\_done\_right\&oldid=10815}}} The only justification for those choices is that they work and obey
...
or \texttt{\textcolor{blue}{\footnotesize{}mrun}} 
...
\subsection{The laws of monad transformers\label{subsec:Laws-of-monad-transformers}}

A monad transformer $T_{L}^{M,A}$ is a functor with a type parameter
$A$ and a monad parameter $M$, such that the following laws hold:
\begin{enumerate}
...
is a monad morphism. Moreover, the function \lstinline!mrun! lifts
...
\]
This function is called the \index{monad transformers!base lifting}\textbf{base
...
For any monad morphism $\theta:L\leadsto\text{Id}$ and for any monad
$M$, the base runner $\text{brun}\left(\theta\right):T_{L}^{M}\leadsto M$
...
So, the fake \textsf{``}transformer\textsf{''} satisfies almost all of the monad transformer
...
a \textsf{``}transformer\textsf{''} that appears to have all the methods with the
...
Other examples of fake \textsf{``}transformers\textsf{''} violating some of the laws
...
and its inverse, $\chi:T_{Q}^{\text{Id}}\leadsto Q$. The runner $\text{mrun}_{P}$
...
for any two monad morphisms $f$ and $g$. We also have $\text{mrun}_{P}\left(\text{id}\right)=\text{id}$
by the same law. So, 
\begin{align*}
...
hold for $\text{mrun}\,\phi$ and $\text{brun}\,\theta$:
\begin{align*}
