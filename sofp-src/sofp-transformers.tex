...
for $T_{L}^{M}$ uses only $M$\textsf{'}s monadic methods ($\text{pu}_{M}$
...
final case class TryT[M[_]: Monad : Functor, A](value: M[Try[A]]) {
  def map[B](f: A => B): TryT[M, B] = TryT(value.map(_.map(f)))        // Using $M'$s `map` here.
  def flatMap[B](f: A => TryT[M, B]): TryT[M, B] = TryT(
    value.flatMap {                                                // Using $M'$s `flatMap` here.
...
We may also view monad stacking as a binary operation (denoted by
$\varangle$) that combines monads $K$ and $L$ into a new monad
...
Does the stacking operation ($\varangle$) have a unit element? We
notice from Table~\ref{tab:Known-monad-transformers} that each transformer
...
transformed type will become equivalent to the base monad $L$. Intuitively,
the identity monad is a \textsf{``}trivial effect\textsf{''}; combining it with another
monad should not change that monad\textsf{'}s effects. So, we may expect this
property to hold for every transformer:
\[
...
$L$ can be recovered as as $T_{L}^{\text{Id}}$ if we know the formula
for the transformer $T_{L}^{M}$. So, if we already have the type
and an implementation for a monad $L$\textsf{'}s transformer, we do not need
to define the monad $L$\textsf{'}s type and methods separately. This saves
time when implementing monad code in libraries (but may sometimes
...
may perform side effects (picture them as \textsf{``}launching missiles\textsf{''})
that cannot be undone or canceled. See \texttt{\href{https://stackoverflow.com/questions/11792275/}{https://stackoverflow.com/questions/11792275/}}} However, this chapter will show that lawful transformers exist for
every monad derived anywhere in this book (except for the continuation
monad\textsf{'}s transformer, which violates some of the required laws). 
...
\subsection{Combining monads via typeclasses for monad operations }
...
$T$ is an \emph{arbitrary} pointed and optionally co-pointed functor
in the category of monads. Such a functor $T$ maps monads $M$, $N$,
etc., to monads $T^{M}$, $T^{N}$, etc. Given such a $T$, we \emph{define}
the base monad as the image of the identity monad:
\[
...
\]
The functor laws and the monadic naturality laws will then enforce
all the laws of monad transformers. 

Is the non-degeneracy law ($\text{flift}_{T}\bef\text{brun}_{T}=\text{id}$)
a consequence of monadic naturality of \lstinline!brun!? This may
be the case, but this book does not have a proof. The composition
of two natural transformations is again a natural transformation;
this is true in any category, including the category of monads. So,
the composition $\text{flift}\bef\text{brun}$ has to be a natural
transformation of type $\text{Id}^{\bullet}\leadsto\text{Id}^{\bullet}$.
To show that this natural transformation is the identity function,
we need to prove that the category of monads admits no other natural
transformations between identity functors except identity transformations.
A natural transformation of type $\text{Id}^{\bullet}\leadsto\text{Id}^{\bullet}$
means a family of monad morphisms $\varepsilon^{M}:M\leadsto M$ that
work the same way for every monad $M$. One such morphism is the identity
...
Since we have not shown that the non-degeneracy law is an automatic
consequence of monadic naturality, we will verify that law separately
whenever the base runner exists (which is not the case for all base
monads $L$).
...
inner-identity laws (Statement~\ref{sec:Statement-swap-laws-to-monad-transformer-first-law}).
...
a rigid functor cannot be a disjunctive type defined with several
constructors, such as $\bbnum 1+A+A$ or $\text{List}^{A}$. A rigid
functor\textsf{'}s type definition must have a single constructor. This is
one motivation for the name \textsf{``}rigid\textsf{''}: the container $R^{A}$ has
a fixed shape and bears no extra information other than holding some
values of type $A$. 
...
The idea of the proof is to apply both sides of the non-degeneracy
law $\text{fi}_{R}\bef\text{fo}_{R}=\text{id}$ to the identity function
of type $R^{\bbnum 1}\rightarrow R^{\bbnum 1}$. To adapt the type
parameters, consider the type signature of $\text{fi}_{R}\bef\text{fo}_{R}$,
\[
...
It follows from Statement~\ref{subsec:Statement-rigid-functor-wrapped-unit-is-unit}
...
are not rigid. A rigid functor must have a fixed \textsf{``}shape\textsf{''} of data
wrapped in it. This partially explains the choice of the name \textsf{``}rigid\textsf{''}.
...
the collection of $M$-effects is described by a functor $R$.
...
The product of two monad transformers satisfies all monad transformer
laws.

\subparagraph{Proof}

Given two monads $G$ and $H$ whose transformers $T_{G}$ and $T_{H}$
are known, we need to show that the lawful transformer for the monad
$L^{\bullet}\triangleq G^{\bullet}\times H^{\bullet}$ is given by
\[
T_{L}^{M,A}\triangleq T_{G}^{M,A}\times T_{H}^{M,A}\quad.
\]
We may assume that all monad transformer laws hold for $T_{G}$ and
$T_{H}$.
...
We will now show that the free pointed monad has a full-featured monad
transformer satisfying all laws.
...
that
\[
\text{pu}_{P}=a^{:A}\rightarrow a+\bbnum 0^{:N^{A}}\quad,\quad\text{ftn}_{P}=\,\begin{array}{|c||cc|}
 & A & N^{A}\\
...
with a suitable \lstinline!swap! function (denoted by \textsf{``}$\text{sw}$\textsf{''}):
\begin{equation}
...
\end{equation}
where the helper function temporarily denoted by \lstinline!merge!
will combine all effects contained within $N^{M^{A}}$ into an overall
$N$-effect in a value of type $N^{A}$:
\[
\text{merge}:N^{M^{A}}\rightarrow N^{A}\quad,\quad\quad\text{merge}\triangleq\text{flift}_{K}^{\uparrow N}\bef\text{ftn}_{N}\quad.
\]
We will now verify the monad transformer laws for $T_{L}.$

\paragraph{Monad construction law}

The monad methods of $T_{L}$ are defined via $\text{sw}$ as in Statement~\ref{sec:Statement-swap-laws-to-monad-transformer-first-law}:
\[
...
where that function was denoted by $\text{sw}_{M,P}$. Let us verify
those laws.
...
\[
T_{L}^{\text{Id},A}\cong\text{Id}^{A+T_{K}^{\text{Id},A}}\cong A+K^{A}\cong L^{A}\quad.
\]
These isomorphisms are monad morphisms due to Statement~\ref{subsec:Statement-composition-transformer-identity-law}
under the assumption that the identity law already holds for the transformer
$T_{K}$. We also need an additional property that the free pointed
monad construction preserves monad morphisms (Exercise~\ref{par:Exercise-mt-3}).
Then the assumed monad isomorphism $T_{K}^{\text{Id}}\cong K$ extends
to a monad isomorphism $A+T_{K}^{\text{Id},A}\cong A+K^{A}$.

\paragraph{Lifting laws}

The \lstinline!flift! function for $T_{L}$ is defined as
\[
...
To verify the runner laws for $T_{L}$, we cannot use Statement~\ref{subsec:Statement-monad-transformer-runner-laws-from-swap}
because the transformer $T_{L}^{M}=M\circ P$ is not a true compositional
transformer: the monad $P$ contains in its definition the monad $M$,
\[
P^{A}\triangleq A+T_{K}^{M,A}\quad.
\]
Mapping the monad $M$ to $M^{\prime}$ forces us also to map $P$
to $P^{\prime}\triangleq A+T_{K}^{M^{\prime},A}$. So, the runners
(\lstinline!frun! and \lstinline!brun!) must be defined differently
from Statement~\ref{subsec:Statement-monad-transformer-runner-laws-from-swap}:
\begin{align*}
...
Since the monad $P$ depends on $M$, the monadic naturality of $\text{sw}_{M,P}$
with respect to the monad $M$ is not simply the monadic naturality
of the function \lstinline!swap[M[_], P[_], A]! with respect to the
type parameter \lstinline!M[_]!. For any monad morphism $\phi:M\leadsto M^{\prime}$,
...
Let us now show that $\text{frun}_{L}^{M,M^{\prime}}(\phi)$ is a
monad morphism. To verify the identity law:
\begin{align*}
...
\end{array}\quad.
\end{align*}
With these notations, we write the left side of the composition law:
\begin{align*}
 & \text{frun}_{L}^{Q,R}(\phi_{1})\bef\text{frun}_{L}^{R,S}(\phi_{2})=\psi_{1}^{\uparrow Q}\bef\gunderline{\phi_{1}\bef\psi_{2}^{\uparrow R}}\bef\phi_{2}\\
...
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{pu}_{M}:}\quad & \text{pu}_{T}\bef\text{brun}_{L}=\text{pu}_{P}\bef\gunderline{\text{pu}_{M}\bef\theta_{P}^{\uparrow M}}\bef\text{ftn}_{M}=\text{pu}_{P}\bef\theta_{P}\bef\gunderline{\text{pu}_{M}\bef\text{ftn}_{M}}\\
{\color{greenunder}\text{identity law of }M:}\quad & =\text{pu}_{P}\bef\theta_{P}\\
{\color{greenunder}\text{identity law of }\theta_{P}:}\quad & =\text{pu}_{M}\quad.
\end{align*}

To verify the composition law, which is an equation between functions
...
{\color{greenunder}\text{or equivalently}:}\quad & \theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}\overset{?}{=}\text{sw}^{\uparrow M}\bef\text{ftn}_{M}\bef\text{ftn}_{P}^{\uparrow M}\bef\theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\quad.\label{eq:monadic-naturality-brun-free-pointed-derivation1}
\end{align}
Rewrite the right-hand side of the last equation:
\begin{align*}
{\color{greenunder}\text{right-hand side}:}\quad & \text{sw}^{\uparrow M}\bef\text{ftn}_{M}\bef\text{ftn}_{P}^{\uparrow M}\bef\theta_{P}^{\uparrow M}\bef\text{ftn}_{M}=\text{sw}^{\uparrow M}\bef\text{ftn}_{M}\bef(\gunderline{\text{ftn}_{P}\bef\theta_{P}})^{\uparrow M}\bef\text{ftn}_{M}\\
{\color{greenunder}\text{composition law of }\theta_{P}:}\quad & =\text{sw}^{\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef(\theta_{P}}\bef\theta_{P}^{\uparrow M}\bef\text{ftn}_{M})^{\uparrow M}\bef\text{ftn}_{M}\\
{\color{greenunder}\text{naturality of }\text{ftn}_{M}:}\quad & =\text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}\quad.
\end{align*}
...
separately:
\begin{align*}
{\color{greenunder}\text{left-hand side}:}\quad & \theta_{P}^{\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}}\bef\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}=\theta_{P}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M\uparrow M}\bef\text{ftn}_{M}\bef\gunderline{\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}}\\
...
{\color{greenunder}\text{right-hand side}:}\quad & \text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}}\bef\gunderline{\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}}\\
...
\]
The last equation is similar (but not equivalent) to the monadic naturality
law of \lstinline!swap! with respect to the monad parameter $P$.
...
{\color{greenunder}\text{using definitions of }\text{brun}_{L}\text{ and }\text{frun}_{L}:}\quad & \theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\bef\phi\overset{?}{=}\,\begin{array}{|c||cc|}
 & A & T_{K}^{M^{\prime},A}\\
\hline A & \text{id} & \bbnum 0\\
T_{K}^{M,A} & \bbnum 0 & \text{frun}_{K}^{M,M^{\prime}}\!(\phi)
\end{array}^{\uparrow M}\bef\gunderline{\phi\bef\theta_{P^{\prime}}^{\uparrow M^{\prime}}}\bef\text{ftn}_{M^{\prime}}\\
{\color{greenunder}\text{naturality of }\phi:}\quad & \quad=\,\begin{array}{|c||cc|}
 & A & T_{K}^{M^{\prime},A}\\
\hline A & \text{id} & \bbnum 0\\
T_{K}^{M,A} & \bbnum 0 & \text{frun}_{K}^{M,M^{\prime}}\!(\phi)
...
The monad transformers for linear monads, rigid monads, recursive
monads, product monads, and free pointed monads are \textsf{``}regular\textsf{''}
in the sense that they support the most general form of lifts and
runners, for which all laws hold. Those monad transformers are fully
described as pointed functors in the category of monads (see Section~\ref{subsec:Category-theoretic-properties-of-lifts-and-runners-functors-in-category-of-monads}).
We now turn to certain monads (such as \lstinline!State! and \lstinline!Cont!)
whose monad transformers are not functors in the category of monads
because they are deficient in one or another way.

\subsection{The state monad transformer (\texttt{StateT})}

The transformer for the \lstinline!State! monad is not compositional;
as shown in Section~???, the \lstinline!State! monad does not generally
compose with other monads. Instead, \lstinline!StateT! needs a custom
definition (Section~\ref{subsec:Monad-transformers-for-standard-monads}).

\subsubsection{Statement \label{subsec:Statement-state-monad-transformer}\ref{subsec:Statement-state-monad-transformer}}

The monad transformer for the \lstinline!State! monad is 
\[
...
solved by starting the runner from an updated state instead of the
fixed state $s_{0}$. However, that solution cannot be used with the
\lstinline!StateT! transformer whose updated state is wrapped as
$M^{A\times S}$ in an arbitrary foreign monad $M$. The monad $M$\textsf{'}s
effect could wrap several values or no values. The updated state is
...
The transformer\textsf{'}s type is defined by
\[
...
\subsubsection{Problem \label{par:Problem-identity-natural-monad-morphism}\ref{par:Problem-identity-natural-monad-morphism}\index{problems}}

Consider a monad morphism $\varepsilon^{M,A}:M^{A}\rightarrow M^{A}$
that is defined for all monads $M$ and is monadically natural in
the monad parameter $M$. So, $\varepsilon$ must satisfy the laws
listed here:
\begin{align*}
...
morphism. Either prove that any such $\varepsilon$ must be an identity
function, $\varepsilon=\text{id}^{:M^{A}\rightarrow M^{A}}$, or show
an example of such $\varepsilon$ not equal to identity.\footnote{The author of this book does not know how to resolve this question.}
