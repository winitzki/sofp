...
verify all the monad transformer laws. 

\paragraph{Monad construction law}

Begin by noting that the recursively defined type of $T_{\text{Free}}^{M,A}$
contains infinitely many nested layers of the monad $M$ composed
with the functor $F$. Informally, we may write that type constructor
as an \textsf{``}infinite composition\textsf{''},
\[
...
a monad. This resembles the situation with the composed-outside monad
...
law, that difference is unimportant. 

Similarly to the case of composed-outside transformers, we define
a \lstinline!swap! function:
\[
...
which is the \lstinline!pure! method of the monad $L$. The lower
row of the matrix contains a composition of four functions, the first
of which being $\overline{\text{sw}}$, the recursive call to \lstinline!swap!.
The following diagram illustrates the types involved in the composition:
\[
...
The composition $M\circ L$ will be a monad if \lstinline!swap! satisfies
...
\end{align*}
The monad methods \lstinline!pure! and \lstinline!flatten! of $T\triangleq M\circ L$
are defined by
\[
...
Since $L$ is a free monad on $F\circ M$, we have
\begin{align*}
\\
\end{align*}
...
functors in the category of monads.
