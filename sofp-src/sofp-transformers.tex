...
technique, the \textsf{``}lift relation\textsf{''}. Another technique is the \textsf{``}MTL-style\textsf{''}
...
\textbf{in a lift relation} if we can define a monad morphism $P\leadsto Q$
...
$L$ and $K\varangle L\varangle M$ will be in a lift relation because
...
parameter $A$ with a fixed $R$. Then $L^{A}\triangleq\forall R.\,K^{R,A}$
is also a lawful monad.

\subparagraph{Proof}

We need to define the monad $L$\textsf{'}s methods and verify the laws. The
idea is to add the quantifier ($\forall R$) to monad $K$\textsf{'}s methods.
(For brevity, we will sometimes write $K$ to mean $K^{R,\bullet}$.)
Begin with the method \lstinline!pure!:
\[
