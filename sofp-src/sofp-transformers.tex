...
\paragraph{Combining monads}

Programs often need to combine monadic effects (see code)

\textsf{``}Effect\textsf{''} $\triangleq$ what else happens in {\footnotesize{}$A\rightarrow M^{B}$}
...
The code will work if we \textsf{``}unify\textsf{''} all effects in a new, larger
monad
...
{\footnotesize{}\vspace{-0.15cm}}If a \textsf{``}big monad\textsf{''} \texttt{\textcolor{blue}{\footnotesize{}BigM{[}A{]}}}
...
\textrm{\textcolor{darkgray}{\footnotesize{}~//~required~\textsf{``}lifting\textsf{''}~functions:}}{\footnotesize\par}
...
Building a \textsf{``}big monad\textsf{''} out of \textsf{``}smaller\textsf{''} ones, with lawful
liftings
...
\subsection{Combining monadic effects by functor composition}
...
One way of combining two effect is by using functor composition, \lstinline!Future[Option[A]]!.
However, working directly with that data type is inconvenient:
\begin{lstlisting}
...
of previous ones:
\begin{lstlisting}
val r: Future[Option[Int]] = for { // This will not compile.
  x <- Future(Option(1))     // x: Int
  y <- Future(Option(x + 1)) // y: Int
  z <- Future(Option(y + 3)) // z: Int
} yield z
\end{lstlisting}
...
\begin{lstlisting}
case class FutOpt[A](nested: Future[Option[A]]) {
...
val r: FutOpt[Int] = for {           // This code compiles now.
...
a rigid functor has a fixed \textsf{``}shape\textsf{''} of the placement of data in
it.
