...
\subsection{Lifts and runners for monad stacks via type relations}
...
a monad morphism $L\leadsto K$ that comes from another lift relation:
\begin{lstlisting}
...
\begin{lstlisting}
type Stack1[A] = EitherT[ReaderT[Option, *], A]
...
implicit val readerBase: BaseMonad[Reader, ReaderT] = ...

val resultStack: Stack1[Int] = for {
  x <- Option(5).up[Stack1]
  r <- Reader(identity).up[Stack1]
  t <- toEither(x, r).up[Stack1] // toEither() returns Either[E, Int]
} yield t
\end{lstlisting}
...
we may want to run the effects of a selected monad, say $L$, using
a known runner $\theta_{L}:L^{A}\rightarrow A$, but keep the effects
of other monads in the stack. If all the monad transformers in the
stack support the foreign and the base runners, we can always run
the effects of any given monad:
\[
\text{frun}_{K}(\text{brun}_{L}^{M\varangle N}(\theta)):(K\varangle L\varangle M\varangle N\leadsto K\varangle M\varangle N\quad.
\]
However, this implementation depends on the order of monads in the
stack and will need to be changed if the stack changes. It is convenient
if the programmer could merely indicate which monad to run. Given
the code shown above, we would like to write simply \lstinline!resultStack.run[Reader](123)!
and obtain a value of type \lstinline!EitherT[Option, Int]!.
