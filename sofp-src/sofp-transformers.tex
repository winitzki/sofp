...
correctly (Exercise~\ref{subsec:Exercise-effectful-list-not-monad}).
So, we cannot define $T_{\text{List}}=M\circ L$ as a functor composition
of two monads, unlike what we did for the other two transformers just
mentioned. 
...
monad methods of $T_{\text{List}}$ via a special helper function
named \lstinline!prod!, with the type signature\footnote{The function \lstinline!prod! was studied in the paper \textsf{``}Composing
...
one. We will show in Statement\ref{subsec:Statement-monoid-operation-comb-for-effectful-list}
...
later derivations. The method \lstinline!prod! is defined using \lstinline!comb!
as:
\[
...
Throughout this exercise, we assume an arbitrary unknown monad $M$
and define $L$ by
\[
L^{A}\triangleq\bbnum 1+A\times M^{L^{A}}\quad.
\]
...
\textbf{(b){*}} Show that a lawful monad instance \emph{can} be implemented
for $L$. This defines a \textsf{``}non-standard\textsf{''} effectful \lstinline!List!
monad, which has the truncating behavior demonstrated in \textbf{(a)}.\index{monads!non-standard List monad@non-standard \texttt{List} monad}
...
a lawful \lstinline!lift! function with type $M\leadsto L$ cannot
be implemented.
