...
it maps objects of one category to an object of another. It is not
a \emph{function} from values of type $M^{A}$ to values of type $T_{L}^{M,A}$.
...
transformation between identity functors. A natural transformation
of that type means a family of monad morphisms $t^{M}:M\leadsto M$
that work the same way for every monad $M$. One such morphism is
the identity function, $t=\text{id}$. However, it is not clear whether
there exist non-identity monad morphisms $M\leadsto M$ that would
...
monad $M$, there is a monad morphism $\text{flift}:M^{A}\rightarrow T_{L}^{M,A}$
(in a shorter notation, $\text{flift}:M\leadsto T_{L}^{M}$).
...
monads may admit a general base runner, $\text{brun}\,(\theta_{L})$,
parameterized by an arbitrary runner $\theta_{L}:L\leadsto\text{Id}$.
If so, the function $\text{brun}\,(\theta_{L})$ must obey the base
runner laws for any fixed $\theta_{L}$.
...
\begin{align*}
{\color{greenunder}\text{monadic naturality of }\text{flift}:}\quad & \text{flift}^{M}\bef\text{frun}\,(\phi^{:M\leadsto N})=\phi\bef\text{flift}^{N}\quad,\\
{\color{greenunder}\text{monadic naturality of }\text{brun}:}\quad & \text{brun}^{M}\bef\phi^{:M\leadsto N}=\text{frun}\,(\phi)\bef\text{brun}^{N}\quad.
\end{align*}
\end{enumerate}
...
other. Are these functions always equivalent? 
...
Statement~\ref{subsec:Statement-outside-transformer-swap-laws}\textendash \ref{subsec:Statement-inside-transformer-swap-laws}
enable us to check more easily whether a given base monad admits a
monad transformer via functor composition. It is easier to check the
6 laws for the \lstinline!swap! function than to verify the 18 monad
transformer laws directly. Also, the laws of \lstinline!swap! use
simpler types than the monad transformer laws.
...
\end{align*}
The runner into $M\times M$ also obeys a monadic naturality law involving
$\phi\boxtimes\phi$ instead of $\phi$:
\begin{align*}
...
For an arbitrary monad $K$, the \textbf{free pointed monad} on $K$
the monad $L^{A}\triangleq A+K^{A}$ (see Statement~\ref{subsec:Statement-co-product-with-identity-monad}).
The new monad\textsf{'}s transformer is defined by the type formula
\[
...
The monad methods of $T_{L}$ are defined as usual for compositional
transformers:
\[
...
\end{align*}
We will prove the last equality if we show that \lstinline!merge!
satisfies the law
...
\[
\xymatrix{\xyScaleY{1.8pc}\xyScaleX{3.5pc}N^{M^{M^{A}}}\ar[r]\sp(0.5){\text{merge}}\ar[d]\sb(0.45){\text{ftn}_{M}^{\uparrow N}} & N^{M^{A}}\ar[d]\sb(0.5){\text{merge}}\\
N^{M^{A}}\ar[r]\sp(0.5){\text{merge}} & N^{A}
}
\]
\vspace{0.1\baselineskip}
\end{wrapfigure}%
...
\]
We use the laws of lifts for $K$:
\begin{align*}
 & \text{merge}\bef\text{merge}=\text{flift}_{K}^{\uparrow N}\bef\text{ftn}_{N}\bef\text{flift}_{K}^{\uparrow N}\bef\text{ftn}_{N}\\
 & =\text{flift}_{K}^{\uparrow N}\bef\text{flift}_{K}^{\uparrow N\uparrow N}\bef\text{ftn}_{N}^{\uparrow N}\bef\text{ftn}_{N}\\
\end{align*}
...
as shown in Section~???, the State monad does not generally compose
with other monads. Instead, \lstinline!StateT! needs a custom definition
...
\begin{align*}
 ...
 & =(a\rightarrow a\times s_{0})\bef\big(\text{uncu}\,(f)\diamond_{_{M}}\text{uncu}\,(g)\gunderline{\big)\bef\pi_{1}^{\uparrow M}}\\
...
For an arbitrary fixed monad $M$, consider a function $\text{dbl}:M^{A}\rightarrow M^{A}$
defined as
...
$m$. Show that (at least for some monads $M$) the function \lstinline!double!
is \emph{not} a monad morphism $M\leadsto M$. 
