...
laws do not apply to $T_{\text{Cont}}$. This significantly limits
the use of the continuation monad transformer. We will now verify
the remaining laws.

\paragraph{Monad construction law}

The transformed monad $T_{\text{Cont}}^{M}$ differs from the continuation
monad $\text{Cont}$ only in the type of the result ($R$), replacing
...
\[
\text{flift}:M^{A}\rightarrow T_{\text{Cont}}^{M,A}=M^{A}\rightarrow(A\rightarrow M^{R})\rightarrow M^{R}\quad,\quad\quad\text{flift}=\text{flatMap}_{M}\quad.
\]
