...
\subsection{Using type relations to derive lifts for monad stacks}
...
is described next. Another technique is an \textsf{``}MTL-style\textsf{''} monad stack
(see Section~\ref{subsec:Combining-monads-via-mtl-style} below).
...
Finally, we define a recursive relation instance: given a stack $K\varangle M$
and a monad $L$, we may lift $L\leadsto K\varangle M$ if there is
already a lift relation $L\leadsto K$: 
\begin{lstlisting}
...
\begin{lstlisting}
type Stack3[A] = EitherT[ReaderT[Option, *], A] // A stack with 3 monads.
...
\]
However, the implementation of the runner depends on the order of
monads in the stack and will have to be changed if the stack changes.
It would be convenient if the programmer could merely indicate which
monad\textsf{'}s effects to run, without writing the code of the runner by
hand. For instance, given the value \lstinline!resultStack3! computed
by the code shown above, we would like to write simply \lstinline!resultStack3.run[Reader](...)!
and obtain a value of type \lstinline!EitherT[Option, Int]!. However,
the 

To create the runners automatically, we need to define a single \lstinline!run!
method parameterized by the types of the stack and the monad whose
effects we need to run. It is convenient to implement \lstinline!run!
as an extension method on a monad stack:
\begin{lstlisting}
implicit class RunMethod[S[_]: Monad, A](s: S[A]) { // `S[_]` is a monad stack\textsf{'}s constructor.
  def run[M[_]: Monad](r: M ~> Id) = ???  // This should work as resultStack3.run[Reader](...)
}
\end{lstlisting}
So far it is not clear what the return type of \lstinline!run! should
be. The result of \lstinline!run! is a value of a monad stack \lstinline!S!
without the monad \lstinline!M!. will depend on :
\begin{lstlisting}
...
same as $\text{Cod}^{F,A}$, so the isomorphism is given by the identity
