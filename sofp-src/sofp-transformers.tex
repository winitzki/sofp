...
transformer} of\index{monad transformers} the monad $L$ is the type
...
that would take an \emph{arbitrary} runner $\theta_{\text{State}}:\text{State}^{S,A}\rightarrow A$
and convert it into a function of type $T_{\text{State}}^{M,A}\rightarrow M^{A}$
...
for the transformer $T_{L}^{\bullet,\bullet}$. So, if we already
have the type and an implementation for a monad\textsf{'}s transformer, we
do not need to define the monad\textsf{'}s type and methods separately. This
saves time when implementing monad code in libraries.
...
\subsection{Motivation for the laws of lift functions}
...
morphisms must hold also for the foreign lift function ($\text{flift}:M\leadsto T_{L}^{M}$).
...
\subsection{Motivation for the laws of runner functions}
...
Given a runner $\theta_{M}$, we may first run $M$\textsf{'}s effects by lifting
$\theta_{M}$ to $L$,
\[
...
base runner $\text{brun}_{L,K}(\phi):T_{L}^{M}\leadsto T_{K}^{M}$
...
\[
\text{flift}\bef\text{frun}_{L}(\theta_{M}):M^{A}\rightarrow L^{A}\quad.
\]
...
\[
m\triangleright\text{flift}\triangleright\text{frun}_{L}(\theta_{M})=m\triangleright\theta_{M}\triangleright\text{pu}_{L}\quad,\quad\quad\text{flift}\bef\text{frun}_{L}(\theta_{M})=\theta_{M}\bef\text{pu}_{L}\quad.
\]
...
\subsection{Category-theoretic properties of lifts and runners }
...
etc., to $T^{M}$, $T^{N}$, etc. Given such a $T$, we first define
the base monad as the image of the identity monad:
\[
L\triangleq T^{\text{Id}}\quad.
\]
The definition of $T$ already defines functions \lstinline!flift!,
...
\]
The functor laws and the monadic naturality laws will enforce all
the laws of monad transformers. 
...
consequence of monadic naturality, we will impose this law separately
...
$\text{Id}^{A}\triangleq A$.
\item \textbf{Lifting law}:\index{monad transformers!lifting law} There
...
are known, we need to show that the transformer for the monad $L^{\bullet}\triangleq G^{\bullet}\times H^{\bullet}$
is given by
\[
...
The monad methods of $T_{L}$ are defined via $\text{sw}$ as per
...
The identity law holds: setting $M=\text{Id}$ in the type of $T_{L}^{M}$
...
because the transformer $T_{L}^{M}=M\circ P$ is not a purely compositional
transformer: the monad $P$ also depends on $M$,
\[
P^{A}\triangleq A+T_{K}^{M,A}\quad.
\]
For the same reason, the monadic naturality of $\text{sw}_{M,P}$
is not simply the monadic naturality with respect to the monad type
$M$. Mapping the monad $M$ to $M^{\prime}$ forces us also to map
$P$ to $P^{\prime}\triangleq A+T_{K}^{M^{\prime},A}$. The runners
(\lstinline!frun! and \lstinline!brun!) are defined differently
...
(see Exercise~\ref{par:Exercise-mt-3-2-1}). If the transformer $T_{K}^{M}$
does not have a general base runner, the definition of $\text{brun}_{L}$
above will need to use any available base runner $\text{brun}_{K}$
instead of $\text{brun}_{K}((k\rightarrow\bbnum 0+k)\bef\theta)$.
...
that $\text{pu}_{M}$ is a monad morphism the identity monad ($\text{Id}$)
to a monad $M$. Show that there are no other such monad morphisms
$\phi$,
\[
...
Assume that there exists a monad morphism $\phi:\bbnum 1\leadsto M$
between the unit monad $L^{A}\triangleq\bbnum 1$ and some monad $M$,
and show that $M$ must be itself a unit monad ($M^{A}\triangleq\bbnum 1$).%
\begin{comment}
Solution:
...
(at least for some monads $M$) the required monad morphism $\text{flift}_{L}:M\leadsto P$
cannot be implemented.

\textbf{(b)} Given a runner $\theta_{K}:K\leadsto M$, where $M$
is an arbitrary target monad, implement a runner $\theta_{L}:L\leadsto M$
and show that $\theta_{L}$ is a monad morphism when $\theta_{K}$
is one.

\textbf{(c)} Given a runner $\theta_{L}:L\leadsto M$, where $M$
is an arbitrary target monad, implement a runner $\theta_{K}:K\leadsto M$
and show that $\theta_{K}$ is a monad morphism when $\theta_{L}$
is one.
...
Assume that $M$ is a given monad, and define $N$ as the free pointed
monad $N^{A}\triangleq A+M^{A}$.

\textbf{(a)} Given a monad morphism $\delta:M\leadsto\text{Opt}$
(from $M$ to the \lstinline!Option! monad), show that one can implement
a monad morphism $\phi:M\leadsto N$.

\textbf{(b)} Given a monad morphism $\phi:M\leadsto N$, show that
one can implement a monad morphism $\delta:M\leadsto\text{Opt}$.
...
morphism. Prove that any such $\varepsilon$ must be an identity function,
$\varepsilon=\text{id}^{:M^{A}\rightarrow M^{A}}$, or show an example
of such $\varepsilon$ not equal to identity.\footnote{The author of this book does not know the solution.}
