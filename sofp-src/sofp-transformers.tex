...
in $M$ but not in $L$ (this is reflected in the notation, as $L$
is a \emph{subscript} in $T_{L}^{M}$). So, changing the base monad
$L$ to another monad forces a complete rewrite of $T_{L}^{M}$\textsf{'}s
code.
...
We need to be able to lift values of types $K^{A}$, $L^{A}$, $M^{A}$,
$N^{A}$, $(K\varangle L)^{A}$, $(L\varangle M)^{A}$, etc., to the
stack type $(K\varangle L\varangle M\varangle N)^{A}$. Assume that
we have the base lifts, foreign lifts, and the runners of the relevant
...
For any such monad stack $P$, a certain composition of lift functions
will create a monad morphism $p:\text{State}^{S,\bullet}\leadsto P^{\bullet}$.
...
Table~\ref{tab:Known-operations-for-some-monads} shows a number
...
where we considered monad morphisms\index{monad morphisms} $\phi:M\leadsto N$
...
for all the runners? For intuition, recall the example with the continuation
...
(The functor $T_{L}^{M,\bullet}$ has methods $\text{pu}_{T}$ and
$\text{ftn}_{T}$ that satisfy the monad laws.)
...
so the monad construction law fails.
