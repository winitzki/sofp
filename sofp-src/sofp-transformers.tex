...
requires writing custom wrapper classes for \emph{each pair} of monads.
...
Similar code is required for $T_{\text{Read}}^{M}$, $T_{\text{Write}}^{M}$,
$T_{\text{State}}^{M}$, and all other monads. The advantage of the
transformer approach is that the code for $T_{L}^{M}$ needs to be
written once for every base monad $L$ (and not for every pair $L$,
$M$).
...
are far from obvious. The only justification for those choices is
that they work and obey the required laws, while no alternative transformers
...
The first task is to obtain the implementations of monad methods and
lifts for the transformers from Table~\ref{tab:Known-monad-transformers}.
...
the \textsf{``}big monad\textsf{''} \texttt{\textcolor{blue}{\footnotesize{}BigM}} 
...
$\text{lift}_{1}$ is a \textbf{monad morphism} between monads $M_{1}^{\bullet}$
and $\text{BigM}^{\bullet}$

example: monad \textsf{``}interpreters\textsf{''} $M^{A}\rightarrow N^{A}$ are monad
morphisms
...
{\footnotesize{}\vspace{-0.3cm}}To combine 3 or more monads, \textsf{``}stack
up\textsf{''} the transformers as $T_{L_{1}}^{T_{L_{2}}^{T_{L_{3}}^{M}},\bullet}$
...
Have monad morphisms between Cod, Sel, and Cont?

\section{Laws and structure}

\subsection{The laws of monad transformer lifts}

Our next goal is to derive the properties required for the new monad
...
of monad transformers, and indicates the laws that are violated by
them.
...
To avoid confusion, I use the name \textsf{``}swap\textsf{''} rather than \textsf{``}sequence\textsf{''}
...
\textsf{``}Composing monads\textsf{''} by M.~P.~Jones and L.~Duponcheel. They studied
