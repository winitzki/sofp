...
The specific choices of the transformer type constructors in Table~\ref{tab:Known-monad-transformers}
are far from obvious.\footnote{The difficulty of this task is illustrated by the history of the \lstinline!ListT!
transformer in the Haskell standard library. The type constructor
for \lstinline!ListT! was chosen incorrectly in 2002 and remained
in the library until it was corrected in 2007. See the discussion
in \texttt{\href{https://wiki.haskell.org/index.php?title=ListT_done_right}{https://wiki.haskell.org/index.php?title=ListT\_done\_right}}} The only justification for those choices is that they work and obey
the required laws, while no lawful alternative transformers are known.
We will derive and prove the required laws for these and other transformers
later in this chapter. For now, we accept the results of Table~\ref{tab:Known-monad-transformers}
as correct and turn to other tasks regarding the practical use of
monad transformers. The first task is to obtain the code of monad
methods and lifts for the transformers in Table~\ref{tab:Known-monad-transformers}.

The type constructors for monad transformers are conventionally named
\lstinline!ReaderT!, \lstinline!EitherT!, etc. 

\paragraph{The \lstinline!ReaderT! transformer}
...
dependency) and then run $M$'s effect:
\[
...
\paragraph{The \lstinline!ListT! transformer}
...
monads do not usually need runners ($\theta_{\text{List}}:\text{List}^{A}\rightarrow A$)
because the meaning of the monad's value is usually the entire set
or collection of useful results rather than a single value, and also
because the collection may be empty, which is incompatible with having
a fully parametric and lawful runner. However, the foreign monad $M$
may come with its runner $\theta_{M}$. In that case, we will need
to run the $M$'s effects if we are to extract values from a \lstinline!ListT!.

Intuitively, we may expect to be able to apply $M$'s runner ($\theta_{M}$)
...
are not available in advance but are evaluated on demand, by calling
$\theta_{M}$. Because of this, the total number of elements in \lstinline!ListT!
may not be known in advance, and may be even unlimited. An example
of an ``infinite'' collection uses the \lstinline!Reader! monad
as $M$:
\begin{lstlisting}
...
\paragraph{The \lstinline!StateT! transformer}
...
by running the \lstinline!State! monad's effects but keeping the
...
\paragraph{The \lstinline!ContT! transformer}
...
is not covariant in the parameter $M$. The base runner, $\text{brun}:((A\rightarrow M^{R})\rightarrow M^{R})\rightarrow M^{A}$,
cannot be implemented (even with non-purely functional code) because
there is no non-trivial way of constructing a value of type $M^{A}$
out of a value of type $M^{R}$.

\paragraph{The \lstinline!SelT! transformer}
...
\paragraph{The \lstinline!EvalT! transformer}
...
similar function for \lstinline!EvalT! plays the role of the ``base
runner'' and helps us implement \lstinline!flatMap!:
\begin{lstlisting}
...
To run \lstinline!Eval!'s effects but keep $M$'s effects intact,
...
to use the ``kind projector\index{``kind projector'' plugin}''
...
description (of type $E$) and produce no new state, ``canceling''
...
stack $T_{K}^{T_{L}^{T_{M}^{N}}}$ (where we used $M$'s transformer,
...
and an implementation for a monad $L$'s transformer $T_{L}$, we
do not need to define the monad $L$'s type and methods separately.
...
\lstinline!Future! monad and other ``task-like'' monads that perform
...
the ``lift relation''. Another technique is the ``MTL-style''
...
\paragraph{The non-degeneracy law of \lstinline!brun!}
...
$\text{Id}^{\bullet}\leadsto\text{Id}^{\bullet}$ in the category
of monads. To show that this natural transformation is necessarily
equal to an identity function, we need to prove that the identity
transformations are the only natural transformations between identity
functors in the category of monads. A natural transformation of type
$\text{Id}^{\bullet}\leadsto\text{Id}^{\bullet}$ implies, by definition,
a family of monad morphisms $\varepsilon^{M}:M\leadsto M$ that work
the same way for every monad $M$. One such morphism is the identity
function, $\varepsilon=\text{id}$. However, it is not clear (Problem~\ref{par:Problem-identity-natural-monad-morphism})
...
for any monads $L$ and $M$. This ``transformer'' is clearly useless:
...
laws. We could be given a fake implementation of a ``transformer''
...
\begin{align*}
\text{expect to equal }\text{brun}_{R}(\theta)\bef\phi:\quad & \text{frun}_{R}(\phi)\bef\text{brun}_{R}(\theta)=\gunderline{\text{frun}_{P}(\text{frun}_{Q}(\phi))\bef\text{brun}_{P}}\left(\text{blift}_{P}\bef\theta\right)\bef\text{brun}_{Q}\left(\text{flift}_{P}\bef\theta\right)\\
...
\paragraph{The difference between the operations \lstinline!swap! and \lstinline!sequence!}
...
``swap'' rather than ``sequence'' in the context of monad transformers.
Let us now find out what laws are required for the \texttt{}\lstinline!swap!
operation.\footnote{The \lstinline!swap! operation was used in the paper ``Composing
...
(the pass/fail monads, the ``rigid'' monads, and \lstinline!Writer!)
...
\begin{align*}
\text{expect this to equal }\text{pu}_{L\circ N}:\quad & \text{pu}_{L\circ M}\bef\text{frun}\,(\phi)\\
...
\begin{align*}
\text{expect this to equal }\text{ftn}_{T}\bef\phi^{\uparrow L}:\quad & \left(\text{frun}\,(\phi)\right)^{\uparrow M\uparrow L}\bef\text{frun}\,(\phi)\bef\text{ftn}_{L\circ N}\\
...
\begin{align*}
\text{expect this to equal }\text{ftn}_{T}\bef\phi:\quad & \left(\text{frun}\,(\phi)\right)^{\uparrow M\uparrow L}\bef\text{frun}\,(\phi)\bef\text{ftn}_{N\circ M}\\
...
A closely related monad is the \textbf{search} monad\index{monads!search monad},
$R^{A}\triangleq\left(A\rightarrow\bbnum 2\right)\rightarrow\bbnum 1+A$.
This (non-rigid) monad returns an empty value $1+\bbnum 0^{:A}$ if
no value satisfing the predicate is found. There is a monad morphism
from the selector monad to the search monad, implemented by checking
whether the value returned by the selector monad does actually satisfies
the predicate.
...
Define the search monad as $\text{Search}^{A}\triangleq\left(A\rightarrow\bbnum 2\right)\rightarrow\bbnum 1+A$.
The function defined by
\begin{lstlisting}
def finder[A](s: (A => Boolean) => A): (A => Boolean) => Option[A]
\end{lstlisting}
\[
\text{finder}:\text{Sel}^{\bbnum 2,A}\rightarrow\text{Search}^{A}\quad,\quad\quad\text{finder}\,(s^{:\text{Sel}^{\bbnum 2,A}\rightarrow\text{Search}^{A}})\triangleq p^{:A\rightarrow\bbnum 2}\rightarrow p(s(p))\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1\text{ (false)} & \text{id} & \bbnum 0\\
\bbnum 1\text{ (true)} & \bbnum 0 & s(p)
\end{array}
\]
is a monad morphism $\text{Sel}^{\bbnum 2,\bullet}\leadsto\text{Search}^{\bullet}$.
...
the \textbf{flipped Kleisli}\index{flipped@``flipped Kleisli'' technique}
...
the composition properties of rigid monad stacks: those properties
...
\paragraph{Monadic naturality of \lstinline!swap!}
...
\paragraph{Functor laws of \lstinline!frun!}
...
$T_{K}$, we define the base runner for $T_{L}$ through the special
runner $\theta_{P}:P^{A}\rightarrow M^{A}$ that we can implement
...
We denote $T^{A}\triangleq M^{A+T_{K}^{M,A}}\triangleq M^{A+N^{A}}\triangleq M^{P^{A}}$.
To verify the identity law for $\text{brun}_{L}$:
\begin{align*}
\text{expect to equal }\text{pu}_{M}:\quad & \text{pu}_{T}\bef\text{brun}_{L}=\text{pu}_{P}\bef\gunderline{\text{pu}_{M}\bef\theta_{P}^{\uparrow M}}\bef\text{ftn}_{M}=\text{pu}_{P}\bef\theta_{P}\bef\gunderline{\text{pu}_{M}\bef\text{ftn}_{M}}\\
\text{identity law of }M:\quad & =\text{pu}_{P}\bef\theta_{P}\\
\text{identity law of }\theta_{P}:\quad & =\text{pu}_{M}\quad.
\end{align*}

The composition law of \lstinline!brun! is an equation between functions
of type $M^{P^{M^{P^{A}}}}\rightarrow M^{A}$:
\begin{align}
 & \text{brun}_{L}\bef\text{brun}_{L}^{\uparrow M}\bef\text{ftn}_{M}\overset{?}{=}\text{ftn}_{T}\bef\text{brun}_{L}\quad,\nonumber \\
\text{or equivalently}:\quad & \theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}\overset{?}{=}\text{sw}^{\uparrow M}\bef\text{ftn}_{M}\bef\text{ftn}_{P}^{\uparrow M}\bef\theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\quad.\label{eq:monadic-naturality-brun-free-pointed-derivation1}
\end{align}
Rewrite the right-hand side of the last equation:
\begin{align*}
\text{right-hand side}:\quad & \text{sw}^{\uparrow M}\bef\text{ftn}_{M}\bef\text{ftn}_{P}^{\uparrow M}\bef\theta_{P}^{\uparrow M}\bef\text{ftn}_{M}=\text{sw}^{\uparrow M}\bef\text{ftn}_{M}\bef(\gunderline{\text{ftn}_{P}\bef\theta_{P}})^{\uparrow M}\bef\text{ftn}_{M}\\
\text{composition law of }\theta_{P}:\quad & =\text{sw}^{\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef(\theta_{P}}\bef\theta_{P}^{\uparrow M}\bef\text{ftn}_{M})^{\uparrow M}\bef\text{ftn}_{M}\\
\text{naturality of }\text{ftn}_{M}:\quad & =\text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}\quad.
\end{align*}
Then Eq.~(\ref{eq:monadic-naturality-brun-free-pointed-derivation1})
becomes
\begin{equation}
\theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}\overset{?}{=}\text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}\quad.\label{eq:monadic-naturality-brun-free-pointed-derivation2}
\end{equation}
The remaining difference between the two sides appears to be
\[
\theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\overset{?}{=}\text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}\quad.
\]
Since $M$ is an arbitrary foreign monad, we cannot have the code
for the lifting $^{\uparrow M}$. To proceed with the derivation,
we need to remove that lifting from the above equation. To achieve
that, we use the various naturality laws and the associativity law
of the monad $M$ to replace $\text{ftn}_{M}$ by $\text{ftn}_{M}^{\uparrow M}$
in the equation above. We rewrite the two sides of Eq.~(\ref{eq:monadic-naturality-brun-free-pointed-derivation2})
separately:
\begin{align*}
\text{left-hand side}:\quad & \theta_{P}^{\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}}\bef\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}=\theta_{P}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M\uparrow M}\bef\text{ftn}_{M}\bef\gunderline{\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}}\\
 & \quad=\theta_{P}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef\text{ftn}_{M}}\bef\text{ftn}_{M}=\theta_{P}^{\uparrow M}\bef\gunderline{\theta_{P}^{\uparrow M\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}}\bef\text{ftn}_{M}\bef\text{ftn}_{M}\\
 & \quad=\theta_{P}\bef\text{ftn}_{M}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}\bef\text{ftn}_{M}\quad,\\
\text{right-hand side}:\quad & \text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef\theta_{P}^{\uparrow M\uparrow M}}\bef\gunderline{\text{ftn}_{M}^{\uparrow M}\bef\text{ftn}_{M}}\\
 & \quad=\text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef\text{ftn}_{M}}\bef\text{ftn}_{M}\\
 & \quad=\text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\gunderline{\theta_{P}^{\uparrow M\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}}\bef\text{ftn}_{M}\bef\text{ftn}_{M}\\
 & \quad=\text{sw}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}\bef\theta_{P}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}\bef\text{ftn}_{M}\quad.
\end{align*}
The two sides will be now equal if we derive an equation between functions
of type $P^{M^{A}}\rightarrow M^{A}$: 
\[
\theta_{P}\bef\text{ftn}_{M}\overset{?}{=}\text{sw}\bef\theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\quad.
\]
The last equation can be seen as a weaker version of a monadic naturality
law of $\text{sw}_{M,P}$ with respect to the parameter $P$ (where
the monad $P$ is transformed into the identity monad by $\theta_{P}$).
...
\text{using definitions of }\text{brun}_{L}\text{ and }\text{frun}_{L}:\quad & \theta_{P}^{\uparrow M}\bef\text{ftn}_{M}\bef\phi\overset{?}{=}\,\begin{array}{|c||cc|}
 & A & T_{K}^{M^{\prime},A}\\
\hline A & \text{id} & \bbnum 0\\
T_{K}^{M,A} & \bbnum 0 & \text{frun}_{K}^{M,M^{\prime}}\!(\phi)
\end{array}^{\uparrow M}\bef\gunderline{\phi\bef\theta_{P^{\prime}}^{\uparrow M^{\prime}}}\bef\text{ftn}_{M^{\prime}}\\
\text{naturality of }\phi:\quad & \quad=\,\begin{array}{|c||cc|}
 & A & T_{K}^{M^{\prime},A}\\
\hline A & \text{id} & \bbnum 0\\
T_{K}^{M,A} & \bbnum 0 & \text{frun}_{K}^{M,M^{\prime}}\!(\phi)
...
The functor composition $T\triangleq M\circ L$ will be a monad if
\lstinline!swap! satisfies the four laws of Statement~\ref{sec:Statement-swap-laws-to-monad-transformer-first-law}
with the monads $L$ and $M$ exchanged:
\begin{align*}
...
Denote that map temporarily by $\psi$ and implement $\psi:L^{A}\rightarrow L^{\prime A}$
...
\end{equation}
Since the map $\phi^{\uparrow F}:F^{M^{A}}\leadsto F^{M^{\prime A}}$
is a natural transformation, Statement~\ref{subsec:Statement-free-monad-monadic-naturality}
...
To implement the base runner, we use the similarity between the structure
of the two disjunctive types, 
\[
...
\]
We need to show that $\text{brun}\,(\theta)$ is a monad morphism.
To verify the identity law:
\begin{align*}
\text{expect to equal }\text{pu}_{M}:\quad & \text{pu}_{T}\bef\text{brun}\,(\theta)=\text{pu}_{L}\bef\gunderline{\text{pu}_{M}\bef\xi^{\uparrow M}\bef\theta^{\uparrow M}}\bef\text{ftn}_{M}=\text{pu}_{L}\bef\xi\bef\theta\bef\gunderline{\text{pu}_{M}\bef\text{ftn}_{M}}\\
\text{identity law of }M:\quad & =\text{pu}_{L}\bef\xi\bef\theta=\,\begin{array}{||cc|}
\text{id} & \bbnum 0\end{array}\,\bef\,\begin{array}{||cc|}
\text{pu}_{M} & \bbnum 0\\
\bbnum 0 & (\overline{\text{brun}}\,(\theta)\bef(x^{:M^{A}}\rightarrow x+\bbnum 0^{:F^{K^{M^{A}}}}))^{\uparrow F}
\end{array}\,\bef\theta\\
\text{matrix composition}:\quad & =\,\begin{array}{||cc|}
\text{pu}_{M} & \bbnum 0\end{array}\,\bef\theta=\text{pu}_{M}\bef\gunderline{\text{pu}_{K}\bef\theta}=\text{pu}_{M}\bef\text{pu}_{\text{Id}}=\text{pu}_{M}\quad.
\end{align*}
In the last line, we used the identity law, $\text{pu}_{K}\bef\theta=\text{id}$,
of the monad morphism $\theta:K\leadsto\text{Id}$. We have also derived
the property 
\begin{equation}
\text{pu}_{L}\bef\xi\bef\theta=\text{pu}_{M}\quad.\label{eq:base-runner-laws-free-monad-derivation1}
\end{equation}
This property is similar to the identity law of a monad morphism $(\xi\bef\theta):L\leadsto M$.
Let us check whether the corresponding composition law holds for the
function $\xi\bef\theta$ as such as monad morphism

To verify the composition law for $\text{brun}\,(\theta)$, begin
by writing its two sides separately:
\begin{align*}
\text{left-hand side}:\quad & \text{ftn}_{T}\bef\text{brun}\,(\theta)=\text{sw}^{\uparrow M}\bef\text{ftn}_{M}\bef\text{ftn}_{L}^{\uparrow M}\bef\xi^{\uparrow M}\bef\theta^{\uparrow M}\bef\text{ftn}_{M}\quad,\\
\text{right-hand side}:\quad & (\text{brun}\,(\theta))^{\uparrow T}\bef\text{brun}\,(\theta)\bef\text{ftn}_{M}=(\xi^{\uparrow M}\bef\theta^{\uparrow M}\bef\text{ftn}_{M})^{\uparrow L\uparrow M}\bef\xi^{\uparrow M}\bef\theta^{\uparrow M}\bef\text{ftn}_{M}\bef\text{ftn}_{M}\quad.
\end{align*}
We can show that the two sides are equal only if we can somehow rewrite
the composition of \lstinline!swap! with $\xi$, which transforms
the monad $L$. So, we look for a law of \lstinline!swap! that involves
changing the monad parameter $L$.
...
of type $M^{A}\rightarrow M^{A}$:
\begin{align*}
\text{expect to equal }\text{id}:\quad & \text{flift}\bef\text{brun}\,(\theta)=\text{pu}_{L}^{\uparrow M}\bef\text{brun}\,(\theta)=\gunderline{\text{pu}_{L}^{\uparrow M}\bef\xi^{\uparrow M}}\bef\theta^{\uparrow M}\bef\text{ftn}_{M}\\
\text{use Eq.~(\ref{eq:base-runner-laws-free-monad-derivation1})}:\quad & =(\text{pu}_{M}\bef\gunderline{\text{pu}_{K}\bef\theta})^{\uparrow M}\bef\text{ftn}_{M}=\text{pu}_{M}^{\uparrow M}\bef\text{ftn}_{M}\\
\text{right identity law of }M:\quad & =\text{id}\quad.
\end{align*}
...
\subsection{Transformer for the list monad}
...
free pointed monads, and recursive monads are ``complete'' in the
sense that they support the most general forms of lifts and runners,
for which all laws hold. A \textbf{complete}\index{monad transformers!completeness}
monad transformer must fully conform to the laws of a pointed and
co-pointed functor in the category of monads (see Section~\ref{subsec:Category-theoretic-properties-of-lifts-and-runners-functors-in-category-of-monads}).
We now consider certain monads whose transformers are deficient in
one or another way, so that they are not, strictly speaking, lawful
pointed and co-pointed functors in the category of monads. Those deficiencies
lead to certain limitations in the practical use of the affected transformers.
...
equal to $M$'s \lstinline!pure! method: $\phi=\text{pu}_{M}$.%
\begin{comment}
Solution:
