...
the \lstinline!Either! monad is able to ``cancel'' the state-updating
...
to the value $m:M^{A}$. So, we derived a ``purity'' law:
\[
...
no value satisfying the ``predicate'' (a function of type $A\rightarrow\bbnum 1+P$)
...
functor $F$. We use the ``unrolling trick''\index{unrolling trick for recursive types}\index{recursive types!unrolling trick}
...
\subsection{Monadically natural lifting of MTL operations (Jaskelioff's \textquotedblleft\texttt{Monatron}\textquotedblright )\label{subsec:Monatron}}
...
shows that the identity monad is an initial object\index{category of monads!initial object}
...
is a terminal object in the category of monads.)\index{category of monads!terminal object}
...
The ``Day convolution'' operation ($\star$) was defined in Eq.~(\ref{eq:definition-of-monoidal-convolution})
