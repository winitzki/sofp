...
trait MTrans[T[_[_], _]] { // The type parameter T is a type constructor with 2 parameters.
...
The Day convolution is based on the fact that the type operations
$P+Q$ and $P\times Q$ are both monoidal, in a suitable sense. We
could generalize the Day convolution to use another monoidal operation,
such as $P+Q+Z\times P\times Q$ (where $Z$ is a fixed type), instead
of $P\times Q$. The result will be a different \textsf{``}monoidal convolution\textsf{''},
but it will also fail to combine monads.
...
\textbf{(3)} Consider the specially modified codensity monad:
\[
...
(this equivalence is due to the Yoneda identity). Although $\text{Cod}_{L}^{M,A}$
is a monad (Statement~), it cannot serve as a monad transformer because
the lift $M^{A}\rightarrow\text{Cod}_{L}^{M,A}$ does not exist for
some $L$ and $M$. A simple counterexample is found with $M^{A}\triangleq R\rightarrow A$
and $L^{A}\triangleq\bbnum 1+A$ similarly to the counterexample shown
in (1).
...
\textbf{(a)} The \textbf{combined} \index{codensity monad!combined}\textbf{codensity
monad} $\text{Cod}_{L}^{M,A}$ is defined by
\[
...
\]
Prove that the type constructor $\text{Cod}_{L}^{M,A}$ is a lawful
monad.

\textbf{(b)} Show that $L^{A}\triangleq(A\rightarrow X)\rightarrow M^{X}$
is \emph{not} a monad (here $X$ is a fixed type).

\begin{comment}
Solution

\textbf{(a)} Denote for brevity $\text{Cod}_{F}^{M,A}\triangleq C^{A}$
and dfine the flipped Kleisli method $\tilde{\text{pu}}_{C}$ by
\[
\tilde{\text{pu}}_{C}\triangleq\forall X.\,k^{:A\rightarrow F^{X}}\rightarrow k\bef\text{pu}_{M}^{\uparrow F}\quad.
\]

\textbf{(b)} Choose $M^{A}\triangleq\bbnum 1+A$. {*}{*}{*}
\end{comment}
