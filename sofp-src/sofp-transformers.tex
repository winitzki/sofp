...
\end{lstlisting}
After defining the ``lifts'', we may rewrite the previous code in
...
\end{lstlisting}
This code will work if we somehow create a new monad that ``unifies''
\lstinline!Seq!, \lstinline!Future!, and \lstinline!Try!. 

It turns out that there is no simple recipe for implementing a ``unified''
...
others have a ``preferred'' order (we will see proofs of these properties
...
The \lstinline!flatMap! method's type signature cannot be implemented
...
other monads; so we define the ``unified'' monad type as \lstinline!List[Try[A]]!.
...
because the ``correct'' value ($s_{2}$) is hidden within the scope
...
of \lstinline!flatMap! discards the second monad's effect. This will
...
supported monads. ``Monad transformers'' is a trick for organizing
the monad-combining code while avoiding the quadratic code size.

The main idea of that trick is to fix the first monad $L$ (the ``base''
monad) and to write code that combines $L$ with an \emph{arbitrary}
second monad $M$ (the ``foreign'' monad). The result is a new,
``transformed'' monad, denoted by $T_{L}^{M}$. It turns out that
...
methods for $T_{L}^{M}$ uses only $M$'s monad methods ($\text{pu}_{M}$
and $\text{flm}_{M}$) and will work in the same way for all foreign
monads $M$. The monad $T_{L}^{M}$ is called the ``$L$-transformer
applied to the monad $M$''. So, the \textbf{monad transformer} of\index{monad transformers}
...
to make explicit the usage of the \lstinline!List! monad's methods
...
\end{lstlisting}
The ``foreign lift'' and the ``base lift'' also use the methods
...
base monad $L$. The code for $T_{L}^{M}$'s methods is parametric
in $M$ but not in $L$ (this is reflected in the notation: $L$ is
a \emph{subscript} in $T_{L}^{M}$). Changing the base monad $L$
to another monad requires a complete rewrite of $T_{L}^{M}$'s code.
...
shows some types of known monad transformers. 
...
\noindent As we can see, monad transformers are created in different
ways: by composing inside the foreign monad (\lstinline!Either!,
...
way with the base monad (\lstinline!State!, \lstinline!Cont!, \lstinline!Eval!). 

Finding type constructors that work as lawful monad transformers is
not a simple task.\footnote{The difficulty of that task is illustrated by the history of the \lstinline!ListT!
...
are known. We will derive and prove the required laws for these and
other transformers later in this chapter. For now, we accept the results
of Table~\ref{tab:Known-monad-transformers} as correct and turn
to other tasks regarding the practical use of monad transformers.
...
\end{lstlisting}
The ``foreign lift'' and the ``base lift'' for \lstinline!ReaderT!
are implemented by
\begin{lstlisting}
...
$R$ (i.e., injects \lstinline!Reader!'s dependency value). In later
...
How can we combine $M$'s runner ($\theta_{M}$) with \lstinline!Reader!'s
...
\]
Alternatively, we can run \lstinline!Reader!'s effect first (injecting
the dependency) and then run $M$'s effect:
\[
...
type of $t$. However, we cannot immediately apply $M$'s \lstinline!flatten!
method to $t$ because $t$'s type is not of the form $M^{M^{X}}$
with some $X$. To bring it to that form, we use $M$'s \lstinline!map!
method:
\[
...
\]
Now the type is well adapted to using both $M$'s and \lstinline!Writer!'s
flatten methods:
\begin{align*}
...
\end{lstlisting}
The ``foreign lift'' and the ``base lift'' for \lstinline!WriterT!
...
\text{base lift}:\quad & \text{blift}:(R\rightarrow A)\rightarrow T_{\text{Writer}}^{M,A}\quad,\quad\quad\text{blift}\triangleq\text{pu}_{M}\quad.
\end{align*}
...
using the list concatenation function \lstinline!concat!; ``flattening''
...
We are now ready to write the code of \lstinline!ListT!'s \lstinline!flatMap!
method:
\begin{lstlisting}
final case class ListT[M[_]: Monad : Functor, A](value: M[Option[(A, ListT[M, A])]]) {
...
We can use $\theta_{M}$ to run the $M$'s effects and extract values
from a \lstinline!ListT!. This is accomplished by a runner (call
it \lstinline!runListT!) that applies $M$'s runner ($\theta_{M}$)
...
is an ``on-call''\index{on-call value} sequence whose elements
...
``infinite'' collection uses the \lstinline!Reader! monad as $M$:
\begin{lstlisting}
...
is passed via the \lstinline!Reader! monad's dependency injection.
...
The formula for that transformer's type, \lstinline!StateT[M, S, A] = S => M[(S, A)]!,
...
The \lstinline!State! monad's runner ($\theta_{\text{State}}$) substitutes
...
general ``base runner'' $\text{brun}\,(\theta_{\text{State}})$
...
that runs the \lstinline!State! monad's effects but keeps the effects
...
monad's effect, we need different code:
\begin{lstlisting}
...
For the continuation monad (\lstinline!Cont[R, A]!), the transformer's
type constructor \lstinline!ContT[M, R, A]! has a peculiar formula
where the foreign monad's constructor $M$ is applied to the result
...
Turning now to the code for lifts, we can easily write the ``foreign
lift'':
\begin{lstlisting}
...
\]
However, implementing the ``base lift'' is impossible: the required
type signature
\[
...
The absence of ``base lift'' for the continuation monad transformer
...
is not covariant in the parameter $M$. The base runner's type signature,
...
\end{lstlisting}
\begin{align*}
\text{foreign lift}:\quad & \text{flift}:M^{A}\rightarrow T_{\text{Sel}}^{M,A}\quad,\quad\quad m^{:M^{A}}\triangleright\text{flift}\triangleq\_^{:M^{A}\rightarrow Z}\rightarrow m=m\triangleright\text{pu}_{\text{Sel}}\quad,\\
\text{base lift}:\quad & \text{blift}:(\left(A\rightarrow Z\right)\rightarrow A)\rightarrow T_{\text{Sel}}^{M,A}\quad,\quad\quad\text{blift}\triangleq\text{pu}_{M}^{\uparrow\text{Sel}}\quad.
\end{align*}
...
\]
We see now that \lstinline!get! is \lstinline!Eval!'s runner. A
similar function (\lstinline!brun!) plays the role of the ``base
runner'' for \lstinline!EvalT!: it runs \lstinline!Eval!'s effect
(converting lazy values into eager ones) but keeps $M$'s effects
...
Given a runner for $M$, we can run $M$'s effects while keeping \lstinline!Eval!'s
effects intact:
\begin{lstlisting}
...
given foreign monad $M$. The result is a new, ``larger'' monad
$T_{L}^{M}$ that can describe the effects of both $L$ and $M$.
If we need to combine monad $T_{L}^{M}$ with a third monad (say,
$K$), we could use $K$'s transformer and obtain the monad $T_{K}^{T_{L}^{M}}$
...
and begin with $T_{L}^{M}$. Since the \lstinline!Writer!'s transformer
composes inside the foreign monad, we write
\[
T_{\text{Writer}}^{\text{State},A}=S\rightarrow A\times W\times S\quad.
\]
Now we apply the \lstinline!Reader!'s transformer, which composes
...
to use the ``kind projector\index{kind@``kind projector'' plugin}''
...
dependency (of type $R$) is used to produce an ``eager'' list of
...
represents a ``lazy'' (on-call) sequence because getting the value
...
monad will sometimes ``cancel'' an effect of the other.
...
description (of type $E$) and produce \emph{no} new state, ``canceling''
...
stack $T_{K}^{T_{L}^{T_{M}^{N}}}$ (where $T_{M}$ is $M$'s transformer).
...
set $M=\text{Eval}$ in the formula for \lstinline!State! monad's
transformer:
\[
...
\end{lstlisting}
Now we apply \lstinline!State!'s transformer to the foreign monad
...
is defined as the \lstinline!List! monad's transformer applied to
...
type TryM[A] = M[Try[A]]
\end{lstlisting}
The next step is to apply \lstinline!Option!'s transformer to the
...
\]
Intuitively, the identity monad has a ``trivial effect''. Combining
a trivial effect with another monad should not change that monad's
...
for a monad $L$'s transformer $T_{L}$, we do not need to define
the monad $L$'s type and methods separately. This saves time when
...
monad and other ``task-like'' monads that perform side effects with
...
\end{lstlisting}
The transformer's code is specific for the base monad $L$ but depends
...
\]
where the ``universal'' composition operation $\diamond$ should
...
co-density monad) have neither the \lstinline!frun! nor the \lstinline!blift!
...
can be lifted to the stack without need for $N$'s transformer:
\[
...
While we have solved the lifting problem for most monad stacks, it
is impractical for the programmer to have to write manually a composition
...
written similarly to this fragment:
...
changes in the lifts. Clearly, productive practical use of a deep
monad stack needs better techniques. We will now describe one such
technique, the ``lift relation''. Another technique is the ``MTL-style''
...
The \index{monad transformers!{}``lift relation'' technique}``lift
relation'' technique creates a type relation that describes all possible
...
The lift relation is implemented as a type relation for the monads'
...
\subsection{Constructing lifts via typeclasses for monad operations (``MTL-style'')\label{subsec:Combining-monads-via-mtl-style}}
...
``Lifting'' \textendash{} a monad morphism $\text{lift}_{L}^{M}:M^{A}\leadsto T_{L}^{M,A}$

``Base lifting'' \textendash{} a monad morphism $\text{blift}:L^{A}\leadsto T_{L}^{M,A}$
...
``Base runner'': lifts $L^{A}\leadsto A$ into a monad morphism
...
programmer's intuitions about monadic programs written using transformers.
...
If $L$'s \lstinline!pure! method is used before a source line, we
...
If $L$'s \lstinline!pure! method is used after a source line, we
...
\]
So, the first two laws are equivalent to a single\index{identity laws!of monad transformer's lift}
\textbf{identity law}, 
\[
...
$T$. This is the \textbf{composition law}\index{composition law!of monad transformer's lift}
...
A monad's runner ($\theta_{M}:M^{A}\rightarrow A$) extracts results
of type $A$ from monadic values of type $M^{A}$. The use of a runner
will agree with programmer's intuition if the runner obeys the laws
...
a runner ``executes'' or ``runs'' the effects of the monad $M$
...
Given a runner $\theta_{M}$, we may first run $M$'s effects by using
$\theta_{M}^{\uparrow L}$,
\[
\theta_{M}^{\uparrow L}:L^{M^{A}}\rightarrow L^{A}\quad.
\]
The result of type $A$ can be now extracted by using $L$'s runner.
So, we may write
\[
...
\text{naturality of }\theta_{M}:\quad & \quad=i\triangleright t\bef\theta_{M}\bef\pi_{1}\quad.
\end{align*}
...
run the effects of $M$ and represent the results using $N$'s effects.
...
possibly running some of $M$'s effects but keeping $L$'s effects
...
on $K$ non-parametrically, we cannot expect to have a ``universal''
...
continuation monad's runner (Section~\ref{subsec:The-continuation-monad})
...
to the value $m:M^{A}$. So, we derived the ``purity'' law:
\[
...
may view the functor $T_{L}^{\bullet}$ as the ``bare'' monad transformer,
...
\]
This is the ``purity'' law of \lstinline!flift! that we derived
earlier.
...
becomes clear from viewing \lstinline!frun! as a lifting (``\lstinline!map!'')
...
way for all foreign monads $M$. We call it a ``monadic'' naturality
...
so this function is equal to $L$'s runner ($\text{brun}_{L}^{\text{Id}}=\theta_{L}$).
...
The monadic naturality of \lstinline!frun! is equivalent to \lstinline!frun!'s
composition law:
\[
...
is a ``foreign runner''\footnote{This function is called \lstinline!hoist! in Haskell standard libraries
...
\begin{align*}
\text{monadic naturality of }\text{flift}:\quad & \text{flift}^{M}\bef\text{frun}\,(\phi^{:M\leadsto N})=\phi^{:M\leadsto N}\bef\text{flift}^{N}\quad,\\
\text{monadic naturality of }\text{brun}:\quad & \text{brun}^{M}\bef\phi^{:M\leadsto N}=\text{frun}\,(\phi^{:M\leadsto N})\bef\text{brun}^{N}\quad.
\end{align*}
\end{enumerate}
...
for any monads $L$ and $M$. This ``transformer'' is useless: it
...
morphism. So, the fake ``transformer'' actually satisfies many of
...
Other examples of incorrect ``transformers'' violating some of the
...
laws. A wrong implementation of a ``transformer'' could fail to
...
The name ``monoidal convolution'' comes from the fact that the type
operations $P+Q$ and $P\times Q$ are both monoidal, in a suitable
sense. We could generalize the monoidal convolution to use another
monoidal operation, such as $P+Q+Z\times P\times Q$ (where $Z$ is
a fixed type), instead of $P\times Q$. The result will be another
version of a ``monoidal convolution'', but it will also fail to
combine monads.
...
the notation, we talk about a ``monad $T_{P}^{M}$'' meaning the
...
\text{identity law for }\text{frun}_{Q}:\quad & =\text{frun}_{P}\big(\text{id}^{:T_{Q}^{M}\leadsto T_{Q}^{M}}\big)\\
\text{identity law for }\text{frun}_{P}:\quad & =\text{id}\quad.
\end{align*}
...
\text{composition law for }\text{frun}_{Q}:\quad & =\gunderline{\text{frun}_{P}}(\text{frun}_{Q}(\phi)\bef\text{frun}_{Q}(\chi))\\
\text{composition law for }\text{frun}_{P}:\quad & =\gunderline{\text{frun}_{P}(\text{frun}_{Q}(\phi))}\bef\gunderline{\text{frun}_{P}(\text{frun}_{Q}(\phi))}\\
...
\text{non-degeneracy for }\text{brun}_{P}:\quad & =\gunderline{\text{flift}_{Q}\bef\text{brun}_{Q}\left(\text{flift}_{P}\bef\theta\right)}\\
...
we have a function called \lstinline!swap! (short notation ``$\text{sw}$''),
...
\noindent In this way, the new monad's \texttt{}\lstinline!flatten!
...
``swap'' rather than ``sequence''. Let us now find out what laws
are required for the \texttt{}\lstinline!swap! operation.\footnote{The \lstinline!swap! operation was introduced in the paper ``Composing
monads'' (1993) by M.~P.~Jones and L.~Duponcheel; see \texttt{\href{http://web.cecs.pdx.edu/~mpj/pubs/composing.html}{http://web.cecs.pdx.edu/$\sim$mpj/pubs/composing.html}}}
...
\text{right identity law for }L:\quad & =\text{pu}_{M}^{\uparrow M\uparrow L}\bef\text{ftn}_{M}^{\uparrow L}=\gunderline{(\text{pu}_{M}^{\uparrow M}\bef\text{ftn}_{M})}^{\uparrow L}\\
...
\text{outer-interchange for }\text{sw}:\quad & =\text{sw}^{\uparrow L}\bef\text{ftn}_{L}\bef\gunderline{(\text{sw}^{\uparrow M}\bef\text{sw}\bef\text{ftn}_{M}^{\uparrow L})}^{\uparrow L}\bef\text{ftn}_{L}\bef\text{ftn}_{M}^{\uparrow L}\\
...
\text{naturality of }\text{ftn}_{L}:\quad & =\text{sw}^{\uparrow L}\bef\gunderline{\text{ftn}_{L}\bef(\text{sw}^{\uparrow M}\bef\text{sw})^{\uparrow L}}\bef\text{ftn}_{L}\bef\text{ftn}_{M}^{\uparrow L}\bef\text{ftn}_{M}^{\uparrow L}\\
\text{naturality of }\text{ftn}_{L}:\quad & =\text{sw}^{\uparrow L}\bef\gunderline{(\text{sw}^{\uparrow M}\bef\text{sw})^{\uparrow L\uparrow L}\bef\text{ftn}_{L}}\bef\text{ftn}_{L}\bef\text{ftn}_{M}^{\uparrow L}\bef\text{ftn}_{M}^{\uparrow L}\quad.
\end{align*}
...
\text{naturality of }\text{sw}:\quad & =(\text{sw}^{\uparrow L}\bef\text{ftn}_{L})^{\uparrow M\uparrow L}\bef\big(\text{sw}\bef\gunderline{\text{ftn}_{M}^{\uparrow M\uparrow L}}\big)^{\uparrow L}\gunderline{\bef\text{ftn}_{L}}\bef\text{ftn}_{M}^{\uparrow L}\\
...
(the \lstinline!Writer! monad, the pass/fail monads, and the ``rigid''
...
the additional ``compatibility laws''~(\ref{eq:ftn-first-compatibility-law})\textendash (\ref{eq:ftn-second-compatibility-law}),
...
\text{functor composition for }L:\quad & =\text{sw}\bef\gunderline{(\text{pu}_{M}^{\uparrow M}\bef\text{ftn}_{M})}^{\uparrow L}\\
...
\text{substitute }\text{sw}:\quad & =\gunderline{(\text{pu}_{M}^{\uparrow L\uparrow M}\bef\text{pu}_{L}\bef\text{ftn}_{T})^{\uparrow L}}\bef\text{ftn}_{L}\bef\text{ftn}_{M}^{\uparrow L}\nonumber \\
...
\end{equation}
This law expresses a certain kind of ``compatibility'' between the
monads $L$ and $T$.
...
\]
In the same way, the laws of the monad morphism will hold for ``$\text{id}$''
...
\text{use \ensuremath{\text{ftn}_{\text{Id}}}=\text{id} and \ensuremath{\text{sw}_{\text{Id},M}}=\text{id}}:\quad & =\text{ftn}_{M}\quad.
\end{align*}
...
\text{use \ensuremath{\text{ftn}_{\text{Id}}}=\text{id} and \ensuremath{\text{sw}_{L,\text{Id}}}=\text{id}}:\quad & =\text{ftn}_{L}\quad.
\end{align*}
...
\begin{align*}
\text{expect to equal }\text{pu}_{T}:\quad & \text{pu}_{M}\bef\text{flift}=\text{pu}_{M}\bef\text{pu}_{L}\\
\text{definition of }\text{pu}_{T}:\quad & \quad=\text{pu}_{T}\quad,\\
\text{expect to equal }\text{pu}_{T}:\quad & \text{pu}_{L}\bef\text{blift}=\text{pu}_{L}\bef\text{pu}_{M}^{\uparrow L}\\
\text{naturality of }\text{pu}_{L}:\quad & \quad=\text{pu}_{M}\bef\text{pu}_{L}=\text{pu}_{T}\quad.
\end{align*}
...
\text{naturality of }\text{ftn}_{L}:\quad & =\text{sw}_{L,M}^{\uparrow L}\bef\text{ftn}_{L}\bef\gunderline{(\phi^{\uparrow M}\bef\phi\bef\text{ftn}_{N})}^{\uparrow L}\\
...
\text{naturality of }\theta:\quad & =(\theta\bef\theta^{\uparrow L})^{\uparrow L}\bef\text{ftn}_{L}\quad.
\end{align*}
...
\text{composition law for }\theta:\quad & =\text{sw}_{L,M}^{\uparrow L}\bef\gunderline{\text{ftn}_{L}\bef\left(\theta\bef\theta\right)^{\uparrow L}}=\gunderline{\text{sw}_{L,M}^{\uparrow L}\bef\left(\theta\bef\theta\right)^{\uparrow L\uparrow L}}\bef\text{ftn}_{L}\\
...
\begin{align*}
\text{expect to equal }\text{id}:\quad & \text{flift}\bef\text{brun}\left(\theta\right)\\
\text{definitions of }\text{flift}\text{ and }\text{brun}:\quad & =\text{pu}_{M}^{\uparrow L}\bef\theta^{\uparrow L}\\
...
\text{define new }P,Q:\quad & \cong P+Q\times A\quad.
\end{align*}
...
\text{composition}:\quad & =a\rightarrow\gunderline{(\bbnum 0^{:P}+q\times a)\triangleright\text{sw}}\nonumber \\
...
\text{naturality of }\text{ftn}_{L}:\quad & =q\times l\rightarrow l\triangleright\text{ftn}_{L}\bef(x\rightarrow\bbnum 0^{:P}+q\times x)^{\uparrow L}\quad.
\end{align*}
...
So, this is the monad transformer for $M_{1}\circ M_{2}$, and it
is of the composed-inside kind.
...
and $M^{A}=Q\times\left(P+A\right)$, called ``linear''. No other
...
wider range of monads. Those monads are called ``rigid'' because
one of their general properties is having a single ``shape'' (Statement~\ref{subsec:Statement-rigid-functor-wrapped-unit-is-unit}
...
no value satisfing the ``predicate'' (a function of type $A\rightarrow\bbnum 1+P$)
...
Since the type constructor $R$ is itself a function type $H^{A}\rightarrow A$,
the type of the Kleisli morphism $A\rightarrow T^{B}$ is actually
$A\rightarrow T^{B}\triangleq A\rightarrow H^{M^{B}}\rightarrow M^{B}$.
...
to use the monad laws for $M$, it is helpful if we have the Kleisli
morphisms for $M$ of type $A\rightarrow M^{B}$ more easily available.
So, we flip the curried arguments of the Kleisli morphism type $A\rightarrow H^{M^{B}}\rightarrow M^{B}$
and instead consider the \textbf{flipped Kleisli}\index{flipped@``flipped Kleisli'' technique}
morphisms of type $H^{M^{B}}\rightarrow A\rightarrow M^{B}$, which
makes the type $A\rightarrow M^{B}$ a result type of the curried
function. Since the type $A\rightarrow H^{M^{B}}\rightarrow M^{B}$
...
technique makes the proof significantly shorter.
...
f\tilde{\diamond}_{T}g & =q\rightarrow\left(p\triangleright f\right)\diamond_{M}\left(q\triangleright g\right)\quad\text{where}\\
 & \quad p^{:H^{M^{B}}}=\left(\text{flm}_{M}\left(q\triangleright g\right)\right)^{\downarrow H}q\quad.
\end{align*}
This definition works by using the Kleisli product $\diamond_{M}$
on values $(p\triangleright f):A\rightarrow M^{B}$ and $(q\triangleright g):B\rightarrow M^{C}$.
To obtain a value $p:H^{M^{B}}$, we use the function $\text{flm}_{M}(q\triangleright g):M^{B}\rightarrow M^{C}$
to $H$-contramap $q:H^{M^{C}}$ into $p:H^{M^{B}}$.

Written as a single expression, the definition of $\tilde{\diamond}_{T}$
is
\begin{equation}
f\tilde{\diamond}_{T}g=q\rightarrow f\big(\left(\text{flm}_{M}\left(q\triangleright g\right)\right)^{\downarrow H}q\big)\diamond_{M}\left(q\triangleright g\right)\quad.\label{eq:def-flipped-kleisli}
\end{equation}
...
\]
This gives Eq.~(\ref{eq:rigid-monad-def-flm-t-via-flm-r}).
...
\text{definitions of }\text{pure}_{R^{M}}\text{ and }\text{frun}:\quad & =\text{pure}_{M}\bef\text{pure}_{R}\bef\phi^{\uparrow R}\\
...
\text{functor laws for }\text{frun}_{G},\,\text{frun}_{H}:\quad & \quad=\text{frun}_{G}(\phi\bef\chi)\boxtimes\text{frun}_{H}(\phi\bef\chi)=\text{frun}_{L}(\phi\bef\chi)\quad.
\end{align*}
...
The monad $L$'s transformer is defined by the type formula
\[
T_{L}^{M,A}\triangleq M^{A+T_{K}^{M,A}}\quad,
\]
where $T_{K}$ is the monad $K$'s transformer, which is assumed to
be known. 
...
that $P$'s monad methods are defined by
\[
...
\text{apply to }t:\quad & (\bbnum 0+t\triangleright\text{merge})\triangleright\text{sw}\overset{?}{=}\big(\bbnum 0+t\triangleright\text{ftn}_{M}^{\uparrow N}\triangleright\text{merge}\big)\triangleright\text{pu}_{M}\quad,\\
...
\noindent We know nothing about the transformer $T_{K}$'s \lstinline!flift!
...
\text{monad morphism law of }\text{frun}_{K}:\quad & =\text{flift}_{K}^{\uparrow N}\bef\text{ftn}_{N}\bef\text{frun}_{K}^{M,M^{\prime}}\!(\phi)\quad.
\end{align*}
...
\text{naturality of }\text{pu}_{M}:\quad & =\gunderline{\text{pu}_{P}\bef\psi}\bef\gunderline{\text{pu}_{M}\bef\phi}\\
...
\text{naturality of }\phi:\quad & =\gunderline{(\psi^{\uparrow M}\bef\phi)^{\uparrow P\uparrow M}\bef\psi^{\uparrow M}\bef\text{sw}_{M^{\prime},P^{\prime}}^{\uparrow M}}\bef\phi\bef\text{ftn}_{M^{\prime}}\bef\text{ftn}_{P^{\prime}}^{\uparrow M^{\prime}}\\
...
\text{naturality of }\text{sw}\text{, }\text{ftn}_{M}\text{, and }\phi:\quad & =\big(\text{sw}_{M,P}\bef\gunderline{\psi^{\uparrow P\uparrow M}\big)^{\uparrow M}\bef\text{ftn}_{M}}\bef\psi^{\uparrow M}\bef\text{ftn}_{P^{\prime}}^{\uparrow M}\bef\phi\\
\text{naturality of }\text{ftn}_{M}:\quad & =\text{sw}_{M,P}^{\uparrow P}\bef\text{ftn}_{M}\bef\big(\gunderline{\psi^{\uparrow P}\bef\psi\bef\text{ftn}_{P^{\prime}}}\big)^{\uparrow M}\bef\phi\\
...
To achieve that, we repeatedly use $\text{ftn}_{M}$'s laws of naturality
and associativity:
\begin{align*}
...
\begin{align*}
\text{expect to equal }\text{id}:\quad & \text{flift}_{L}\bef\text{brun}_{L}=\text{pu}_{P}^{\uparrow M}\bef\text{brun}_{L}=\gunderline{\text{pu}_{P}^{\uparrow M}\bef\theta_{P}^{\uparrow M}}\bef\text{ftn}_{M}=\text{pu}_{M}^{\uparrow M}\bef\text{ftn}_{M}\\
\text{right identity law of }M:\quad & =\text{id}\quad.
\end{align*}
...
\text{naturality of }\phi:\quad & =\phi\bef\text{pu}_{P}^{\uparrow M^{\prime}}\quad.
\end{align*}
...
\text{naturality of }\phi:\quad & =\,\begin{array}{|c||cc|}
 & A & T_{K}^{M^{\prime},A}\\
\hline A & \text{id} & \bbnum 0\\
T_{K}^{M,A} & \bbnum 0 & \text{frun}_{K}^{M,M^{\prime}}\!(\phi)
...
functor $F$. We use the ``unrolling trick''\index{unrolling trick for recursive types}
to express $T_{\text{Free}}$ through simpler types. We may write
$T_{\text{Free}}$ as an ``infinite'' type expression,
\[
...
\text{associativity of }M:\quad & =\big(\text{sw}^{\uparrow M}\bef\gunderline{\text{sw}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}}\bef\text{ftn}_{M}\big)^{\uparrow F}\bef(p\rightarrow\bbnum 0^{:A}+p)\bef\text{pu}_{M}\\
\text{naturality of }\text{ftn}_{M}:\quad & =\big(\text{sw}^{\uparrow M}\bef\text{ftn}_{M}\bef\text{sw}^{\uparrow M}\bef\text{ftn}_{M}\big)^{\uparrow F}\bef(p\rightarrow\bbnum 0^{:A}+p)\bef\text{pu}_{M}\quad.
\end{align*}
...
\begin{align*}
\text{expect to equal }\phi\bef\text{pu}_{L^{\prime}}^{\uparrow M^{\prime}}:\quad & \text{pu}_{L}^{\uparrow M}\bef\psi^{\uparrow M}\bef\phi=\phi\bef\big(\gunderline{\text{pu}_{L}\bef\psi}\big)^{\uparrow M^{\prime}}=\phi\bef\text{pu}_{L^{\prime}}^{\uparrow M^{\prime}}\quad.
\end{align*}
...
The \lstinline!List! monad's transformer, usually named \lstinline!ListT!,
...
an $M$-effect. The result is an ``effectful list'': obtaining each
...
and the transformer's implementation is significantly more complicated.
The reason is that the functor 
\[
L^{A}\triangleq\bbnum 1+A\times T_{\text{List}}^{M,A}=\bbnum 1+A\times M^{L^{A}}
\]
does not admit a monad instance that flattens the ``effectful lists''
...
\lstinline!prod! as shown in the following statement.\footnote{The function \lstinline!prod! was studied in the paper ``Composing
monads'' (1993) by M.~P.~Jones and L.~Duponcheel. However, that
paper did not correctly define the \lstinline!List! monad's transformer.}
...
\]
This function ``combines'' two values of type $M^{L^{A}}\triangleq T_{\text{List}}^{A}$
...
\end{equation}
The code uses the monad $M$'s \lstinline!flatMap! method to sequence
...
\text{naturality of }\phi:\quad & \quad=\phi\bef\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\big)^{\uparrow M^{\prime}}\bef\text{flm}_{M^{\prime}}(\text{prod})=\phi\bef\text{flm}_{M^{\prime}}\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\bef\text{prod}\big)\quad.
\end{align*}
The remaining difference is
\[
...
\text{naturality of }\phi_{1}:\quad & =\psi^{\uparrow M}(\phi_{1})\bef\psi^{\uparrow M}(\phi_{2})\bef\phi_{1}\bef\phi_{2}=\big(\psi(\phi_{1})\bef\psi(\phi_{2})\big)^{\uparrow M}\bef\phi_{1}\bef\phi_{2}\\
...
\end{array}\,\bigg)\\
\text{naturality of }\text{flm}_{M}:\quad & =\text{flm}_{M}\bigg(\text{pu}_{L}\bef\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M}(e_{R})\\
r\times t\rightarrow\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}\,(t)
\end{array}\,\bigg)\\
...
Begin with the left-hand side:
\begin{align*}
\text{expect to equal }\phi\bef\text{flift}^{M^{\prime}}:\quad & \text{flift}^{M}\bef\text{frun}\,(\phi)=\text{pu}_{L}^{\uparrow M}\bef\psi^{\uparrow M}\bef\phi\\
\text{naturality of }\phi:\quad & =\phi\bef(\text{pu}_{L}\bef\psi)^{\uparrow M}\\
...
\text{naturality of }\phi:\quad & =\phi(p)\triangleright\text{flm}_{M^{\prime}}(u\rightarrow\phi(q)\triangleright(v\rightarrow u\oplus_{R}v)^{\uparrow M^{\prime}})\\
...
\begin{align*}
\text{left-hand side}:\quad & \gunderline{\text{Nil}\triangleright\text{prod}}\bef\text{flm}_{M}(q\triangleright\text{ftn}_{T}\triangleright\xi)=\text{Nil}\triangleright\gunderline{\text{pu}_{M}\bef\text{flm}_{M}}(q\triangleright\text{ftn}_{T}\triangleright\xi)\\
...
\text{naturality of }\phi:\quad & =p\triangleright\gunderline{\text{flm}_{M}(q\triangleright\xi)\bef\phi}\bef\psi^{\uparrow M^{\prime}}\\
...
\text{naturality of }\text{flm}_{M^{\prime}}:\quad & =p\triangleright\phi\bef\text{flm}_{M^{\prime}}\big((q\triangleright\xi)\bef\phi\bef\psi^{\uparrow M^{\prime}}\big)\quad.
\end{align*}
...
\text{naturality of }\phi:\quad & =p\triangleright\phi\bef\gunderline{\psi^{\uparrow M^{\prime}}\bef\text{flm}_{M^{\prime}}}\big(q\triangleright\phi\bef\psi^{\uparrow M^{\prime}}\bef\xi\big)\\
...
\text{associativity law of }\text{flm}_{M}:\quad & \quad=\text{flm}_{M}(\overline{\text{prod}}\bef\text{flm}_{M}(\text{prod}))\quad,\\
...
\text{naturality of }\text{flm}_{M}:\quad & \quad=\text{flm}_{M}(\text{ftn}_{T}^{\overline{\uparrow L}}\bef\overline{\text{prod}})=\text{flm}_{M}(\text{ftn}_{T}^{\overline{\uparrow L}}\bef\overline{\text{prod}})\quad.
\end{align*}
It remains to show that
\[
...
\text{naturality of }\phi:\quad & \quad=\phi\bef\big((\psi^{\uparrow M}\bef\phi)^{\overline{\uparrow L}}\bef\psi\big)^{\uparrow M^{\prime}}\bef\text{flm}_{M^{\prime}}(\overline{\text{prod}})\\
\text{naturality of }\text{flm}_{M}:\quad & \quad=\phi\bef\text{flm}_{M^{\prime}}((\psi^{\uparrow M}\bef\phi)^{\overline{\uparrow L}}\bef\psi\bef\overline{\text{prod}})\quad,\\
...
\text{naturality of }\text{flm}_{M}:\quad & \quad=\text{flm}_{M}(\overline{\text{prod}}\bef\psi^{\uparrow M})\bef\phi\\
...
\text{apply }\text{flm}_{M^{\prime}}:\quad & \text{flm}_{M^{\prime}}\big(\overline{\text{prod}}_{M,L}\bef\psi^{\uparrow M}\bef\phi\big)=\text{flm}_{M^{\prime}}\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\bef\overline{\text{prod}}_{M^{\prime},L^{\prime}}\big)\quad.
\end{align*}
...
are ``complete'', as we have proved in the previous sections. We
...
The monad $M$'s effect could wrap several values, or no values at
all. The ``updated intermediate state'' is not well-defined in that
case.
...
\text{naturality of }\text{pu}_{M}:\quad & =s\rightarrow a\triangleright(a^{:A}\rightarrow a\times s)\bef\text{pu}_{M}=s\rightarrow\text{pu}_{M}(a\times s)=\text{pu}_{T}\quad.
\end{align*}
...
\text{naturality of }\diamond_{_{M}}:\quad & =a\triangleright f\diamond_{_{M}}(b\rightarrow\gunderline{b\times s)\bef\big(b\times s}\rightarrow b\triangleright g\bef(c\rightarrow c\times s)^{\uparrow M}\big)\\
...
\text{naturality of }\diamond_{_{M}}:\quad & =a\triangleright(f\diamond_{_{M}}g)\bef(c\rightarrow c\times s)^{\uparrow M}=\left(a\times s\right)\triangleright\text{uncu}\,\big((f\diamond_{_{M}}g)\bef\text{flift}\big)\quad.
\end{align*}
...
\text{naturality of }\text{pu}_{M}:\quad & =a\rightarrow(a\times s_{0})\triangleright\pi_{1}\bef\text{pu}_{M}=a\rightarrow a\triangleright\text{pu}_{M}=\text{pu}_{M}\quad.
\end{align*}
...
\text{right naturality of }\diamond_{_{M}}:\quad & =(a\rightarrow a\times s_{0})\bef\text{uncu}\,(f)\diamond_{_{M}}\big(\text{uncu}\,(g)\bef\pi_{1}^{\uparrow M}\big)\quad.
\end{align*}
...
\lstinline!State! monad's runner.
...
\text{naturality of }\phi:\quad & =s\rightarrow m\triangleright\phi\bef(a\rightarrow a\times s)^{\uparrow N}=m\triangleright\phi\triangleright\text{flift}^{N}\quad.
\end{align*}
...
\text{compute composition}:\quad & =p\rightarrow s_{0}\triangleright p\bef\gunderline{\phi\bef\pi_{1}^{\uparrow N}}\\
\text{naturality of }\phi:\quad & =\gunderline{p\rightarrow s_{0}\triangleright p\bef\pi_{1}^{\uparrow M}}\bef\phi=\text{brun}\,(s_{0})\bef\phi\quad.
\end{align*}


\subsection{The continuation monad transformer (\texttt{ContT})}

The transformer's type constructor \lstinline!ContT[R, M[_], *]!
is defined by
\[
...
The monad's methods work the same way for all result types, so $T_{\text{Cont}}^{M}$
...
\begin{align*}
\text{expect to equal }\tilde{\text{pu}}_{T}:\quad & \text{flip}\,(\text{pu}_{M}\bef\text{flift})=p\rightarrow\text{pu}_{M}\diamond_{_{M}}p=p\rightarrow p=\text{id}=\tilde{\text{pu}}_{T}\quad.
\end{align*}
...
The transformer for the generalized \lstinline!Search! monad is given
by the type formula
\[
T_{\text{Search}}^{M,A}\triangleq(A\rightarrow T_{L}^{M,Q})\rightarrow T_{L}^{M,A}\quad,
\]
where $T_{L}$ is the monad $L$'s transformer.
...
The \textbf{\index{co-density monad}co-density monad} on a type constructor
$F$ is defined by 
\[
...
\item If $F$ is itself a monad then we have monad morphisms $\text{inC}:F^{\bullet}\leadsto\text{Cod}^{F,\bullet}$
and $\text{outC}:\text{Cod}^{F,\bullet}\leadsto F^{\bullet}$ such
that $\text{inC}\,\bef\text{outC}=\text{id}$. However, the functions
\lstinline!inC! and \lstinline!outC! are not isomorphisms. For example,
when $F^{A}\triangleq R\rightarrow A$ (the \lstinline!Reader! monad),
we get (using the Yoneda identity):
\[
...
\]
The result is the \lstinline!State! monad, which includes the effects
...
cannot do). We find that, in general, $\text{Cod}^{F,\bullet}$ can
describe all $F$-effects and some additional effects. 
...
also does not have the required ``runner'' transformations $\text{frun}$
...
\text{Statement~\ref{subsec:Statement-base-runner-for-composed-monad}}:\quad & =\text{sw}_{R,M}\bef\gunderline{\text{sw}_{S,M}^{\uparrow R}\bef\theta_{R}}\bef\theta_{S}\\
...
\]
are monad morphisms. In other words, all ``runners'' $\theta$ for
...
we can ``run'' the \lstinline!Reader! monad into the identity monad.
...
\text{monad morphism identity law for }\phi_{a}:\quad & =a\rightarrow m\triangleright\gunderline{\phi_{a}}\\
\text{definition~(\ref{eq:runner-phi-def}) for }\phi_{a}:\quad & =a\rightarrow m(a)=m\quad.
\end{align*}
...
is one motivation for the name ``rigid'': the data structure $R^{A}$
must have a fixed ``shape'' and cannot be a disjunction of several
variants. 
...
\text{composition under }^{\uparrow R}:\quad & =a\rightarrow r_{1}\triangleright\big(1\rightarrow(\_^{:A}\rightarrow1)(a)\big)^{\uparrow R}=a\rightarrow r_{1}\triangleright(\gunderline{1\rightarrow1})^{\uparrow R}\\
...
The isomorphism is given by a ``split'' function,
\[
...
The next statements show some use cases for rigid functors. The ``$R$-valued
\lstinline!flatMap!'' is a generalization of \lstinline!flatMap!
...
equal to $M$'s \lstinline!pure! method: $\phi=\text{pu}_{M}$. (This
shows that the identity monad is the initial object in the category
of monads.)%
\begin{comment}
Solution:
...
argument. By the monad $M$'s left identity law, $\text{pu}_{M}\bef\text{ftn}_{M}=\text{id}^{:M^{A}\rightarrow M^{A}}$.
...
This function ``doubles'' the $M$-effect in a given monadic value
...
law must hold for any two monads $M$, $N$ and any monad morphism
$\phi^{:M\leadsto N}$:
\[
...
show that the monad $T\triangleq L\circ M$ satisfies two ``pure
compatibility'' laws,\index{pure compatibility laws@\texttt{pure} compatibility laws}
\begin{align*}
...
Define the ``search monad'' as $\text{Search}^{P,A}\triangleq\left(A\rightarrow\bbnum 1+P\right)\rightarrow\bbnum 1+A$.
The function 
\begin{lstlisting}
...
\end{array}\quad,
\]
transforms the selector monad into the search monad. Show that \lstinline!finder!
...
Show that the function $q:\text{Sel}^{P,\bullet}\leadsto\text{Cont}^{P,\bullet}$
defined below is a monad morphism:
\begin{lstlisting}
...
composition law.\index{flipped@``flipped Kleisli'' technique}
...
map to flatten ax is monad morphism so it's not just the function
or morphism but it's a monad morphism in other words it's a morphism
