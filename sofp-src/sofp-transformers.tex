...
\lstinline!ListT!:
\begin{lstlisting}
def concat[M[_]: Monad : Functor, A](t1: ListT[M, A], t2: ListT[M, A]): ListT[M, A] = ListT(for {
  first <- t1.value            // Run the first effect and get `first: Option[(A, ListT[M, A])]`.
  rest = first match {         // Compute `rest: M[Option[(A, ListT[M, A])]]`.
           case None => t2.value
           case Some((a, tail)) => Monad[M].pure(Some((a, concat(tail, t2))))  // Recursive call.
  y <- rest
} yield y)
\end{lstlisting}
...
final case class ListT[M[_]: Monad : Functor, A](value: M[Option[(A, ListT[M, A])]]) {
  ...
  def flatMap[B](f: A => ListT[M, B]): ListT[M, B] = ListT(
    t.flatMap { // Here we need a function of type Option[(A, ListT)] => M[Option[(A, ListT)]].
      ...
      case Some((head, tail)) => concat(f(head), tail).value  // Type is M[Option[(A, ListT)]].
  })
}
\end{lstlisting}
...
next element of the list requires running an $M$-effect.
...
\]
is not itself a monad, and so $T_{\text{List}}=M\circ L$ is not a
functor composition of two monads (unlike the other two transformers
just mentioned).

To see that $L^{A}$ is not a monad, consider the requirement of mapping
$L^{L^{A}}\rightarrow L^{A}$ via a natural transformation:
\[
...
of type $A\times M^{L^{A}}$. However, we cannot extract values of
type $A$ from $L^{A}$ or from the unknown monad $M$.

As a result, the \lstinline!flatten! metnod of $T_{\text{List}}$,
which we denote for brevity by $\text{ftn}_{T}$, cannot be defined
via a \lstinline!swap! function obeying the usual laws, as we have
done for the other monad transformers. Instead, we will define the
monad methods of $T_{\text{List}}$ via a helper function named \lstinline!prod!,
...
\[
\text{pu}_{L}\triangleq a^{:A}\rightarrow\bbnum 0^{:\bbnum 1}+a\times\text{pu}_{M}(\bbnum 1+\bbnum 0^{:A\times M^{L^{A}}})\quad.
\]
Special laws of \lstinline!prod!, to be proved below, will make the
monad laws hold for $T_{\text{List}}$. The required laws of \lstinline!prod!
(besides naturality) are two identity laws and an associativity law:
\begin{align*}
...
\end{array}\,\bigg)\quad.
\]
The monad $M$\textsf{'}s \lstinline!flatMap! method is used to sequence all
$M$-effects from the first list before all $M$-effects of the second
one. We will see that the \lstinline!comb! function is a monoidal
operation for values of type $T_{\text{List}}^{A}$. Using \lstinline!comb!,
we can define \lstinline!prod!:
\[
\text{prod}\triangleq\,\begin{array}{|c||c|}
 & M^{L^{A}}\\
\hline \bbnum 1 & (1\rightarrow1+\bbnum 0)\bef\text{pu}_{M}\\
M^{L^{A}}\times M^{L^{M^{L^{A}}}} & m\times p\rightarrow\text{app}\,(m)(p\triangleright\overline{\text{prod}}^{\uparrow M}\bef\text{ftn}_{M})
\end{array}\quad.
\]
