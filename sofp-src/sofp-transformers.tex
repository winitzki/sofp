...
\subsubsection{Example \label{subsec:Example-state-monad-composition-1-1}\ref{subsec:Example-state-monad-composition-1-1}}
...
\subsubsection{Example \label{subsec:Example-state-monad-composition-1}\ref{subsec:Example-state-monad-composition-1}}
...
of monad methods and lifts for the transformers in Table~\ref{tab:Known-monad-transformers}.
...
the identity monad has a \textsf{``}trivial effect\textsf{''}; combining it with another
monad should not change that monad\textsf{'}s effects. So, we may expect that
property to hold for every transformer:
\[
T_{L}^{\text{Id}}\cong L\quad,\quad\quad L\varangle\text{Id}\cong L\quad.
\]
We also \emph{define} the monad transformer for the identity monad
as equal to the foreign monad:
\[
...
With these definitions, we find that the monad combination operation
($\varangle$) is associative with unit element, resembling the binary
operation of a monoid. However, the monad $L\varangle M$ is defined
only if the monad $L$ has a transformer. It is unknown\footnote{For a discussion, see \texttt{\href{https://stackoverflow.com/questions/24515876/}{https://stackoverflow.com/questions/24515876/}}}
...
type constructors whose monad methods are lawful and are implemented
by purely functional code. Monad transformers will certainly fail
to exist for monads defined using low-level, non-purely functional
code that performs side effects. Examples of that kind are the \lstinline!Future!
monad and other \textsf{``}task-like\textsf{''} monads (often called \lstinline!Task!
and \lstinline!IO!) that perform side effects. A lawful transformer
must allow the foreign monad to cancel the effect of the base monad,
as we have seen in an example with \lstinline!State! and \lstinline!Either!.
However, certain side effects (picture them as \textsf{``}launching missiles\textsf{''})
cannot be undone or canceled.\footnote{See \texttt{\href{https://stackoverflow.com/questions/11792275/}{https://stackoverflow.com/questions/11792275/}}
for a discussion.} 

We will show later in this chapter that transformers exist for every
purely functional monad derived in this book (although some transformers
