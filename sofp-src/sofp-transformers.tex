...
designated processing thread (such as the GUI thread that drives the
user interface events). Special handling of updates cannot be implemented
through \lstinline!get! and \lstinline!set!; a new effectful operation
(\lstinline!update!) must be provided to the programmer.
...
The monadic naturality law of \lstinline!brun! is
\[
...
\subsection{Lifting monad operations via the codensity monad (Jaskelioff\textsf{'}s \texttt{Monatron})}

Section~\ref{subsec:Combining-monads-via-mtl-style} showed that
MTL-style programming requires custom code for lifting all monad operations
...
is based on an advanced method that lifts all known monad operations
to almost all monad stacks. This includes all monad stacks used in
practice, where a single monad with an incomplete or missing transformer
(such as \lstinline!Cont! or \lstinline!IO!) is always located at
the deep end of the stack. To explain how \texttt{Monatron} works,
this section will follow the ideas developed in M.~Jaskelioff\textsf{'}s paper.\footnote{See \texttt{\href{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}}}
...
we find that the types of existing operations in a monad $M$ fall
into four patterns: 

{*} $M^{Q}$ or $P\rightarrow M^{Q}$ where $P$ and $Q$ are fixed
types.

{*} $\forall A.\,M^{A}$ or $\forall A.\,F^{A}\rightarrow M^{A}$,
where $F$ is a functor that does not depend on $M$.

{*} $\forall A.\,F^{M^{A}}\rightarrow M^{A}$, where $F$ is a functor
that does not depend on $M$.

{*} Another type signature (e.g., the \lstinline!callcc! and \lstinline!filter!
operations).
...
out that all operations may be converted into a common type signature.
The following definition introduces some helpful names:

\subsubsection{Definition \label{subsec:Definition-simple-standard-operation-monatron-1}\ref{subsec:Definition-simple-standard-operation-monatron-1}}

Given a monad $M$ and a functor $F$:

\textbf{(a)} A \textbf{\textsf{``}fixed-type\textsf{''} operation} is a value of type
$P\rightarrow M^{Q}$, where $P$ and $Q$ are fixed types.

\textbf{(b)} A \textbf{\textsf{``}simple\textsf{''} operation} is a natural transformation
(\lstinline!sop!) with type signature
\[
\text{sop}:\forall A.\,F^{A}\rightarrow M^{A}\quad.
\]

\textbf{(c)} A \textbf{\textsf{``}standard\textsf{''} operation} is a natural transformation
(\lstinline!op!) with type signature
\[
...
The following statement shows that these classes of operations are
subsets of each other: the \textsf{``}fixed-type\textsf{''} operations are a subset
of \textsf{``}simple\textsf{''} operations, which are a subset of \textsf{``}standard\textsf{''} operations.

\subsubsection{Statement \label{subsec:Statement-simple-standard-operations-monatron-1}\ref{subsec:Statement-simple-standard-operations-monatron-1}}
...
\[
\text{sop}:P\times\left(Q\rightarrow A\right)\rightarrow M^{A}\quad,\quad\quad\text{sop}=p^{:P}\times q^{:Q\rightarrow A}\rightarrow p\triangleright\text{fop}\bef q^{\uparrow M}\quad,\quad\quad\text{fop}=p^{:P}\rightarrow\text{sop}^{Q}(p\times\text{id}^{Q\rightarrow Q})\quad.
\]

\textbf{(b)} Any \textsf{``}standard\textsf{''} operation $\text{op}:F^{M^{A}}\rightarrow M^{A}$
that satisfies the following compatibility law,
\begin{equation}
\text{op}\bef\text{ftn}_{M}=\text{ftn}_{M}^{\uparrow F}\bef\text{op}\quad,\label{eq:monatron-standard-operation-compatibility-law-1}
\end{equation}
is computationally equivalent to a \textsf{``}simple\textsf{''} operation \lstinline!sop!
defined by
\[
\text{sop}:F^{A}\rightarrow M^{A}\quad,\quad\quad\text{sop}\triangleq\text{pu}_{M}^{\uparrow F}\bef\text{op}\quad,\quad\quad\text{op}=\text{sop}\bef\text{ftn}_{M}.
\]
...
follows from the covariant Yoneda identity:
\[
\forall A.\,P\times\left(Q\rightarrow A\right)\rightarrow M^{A}\cong P\rightarrow\forall A.\,\left(Q\rightarrow A\right)\rightarrow M^{A}\cong P\rightarrow M^{Q}\quad.
\]
...
operation $\text{op}:F^{M^{A}}\rightarrow M^{A}$ satisfying Eq.~(\ref{eq:monatron-standard-operation-compatibility-law-1}),
...
{\color{greenunder}\text{use Eq.~(\ref{eq:monatron-standard-operation-compatibility-law-1})}:}\quad & =\gunderline{\text{pu}^{\uparrow F}\bef\text{ftn}_{M}^{\uparrow F}}\bef\text{op}=(\gunderline{\text{pu}\bef\text{ftn}_{M}})^{\uparrow F}=\text{op}\quad.
\end{align*}
...
The new operation is then equal to the initial \lstinline!sop!:
\begin{align*}
...
except \lstinline!callcc! and \lstinline!filter!, are equivalent
to some \textsf{``}standard\textsf{''} operations, and many of them are also \textsf{``}simple\textsf{''}.
For instance, \lstinline!List! monad\textsf{'}s \lstinline!append! operation
has the type signature of the \textsf{``}standard\textsf{''} form $F^{\text{List}^{A}}\rightarrow\text{List}^{A}$
with $F^{A}\triangleq A\times A$. However, \lstinline!append! satisfies
the compatibility law~(\ref{eq:monatron-standard-operation-compatibility-law-1}),
as shown by Exercise~\ref{subsec:Exercise-flatten-concat-distributive-law}.
So, by Statement~\ref{subsec:Statement-simple-standard-operations-monatron-1},
...
\end{align*}
Many \textsf{``}standard\textsf{''} operations satisfy the compatibility law, but
some do not (such as \lstinline!Either! monad\textsf{'}s \lstinline!handle!
operation).

Postponing the handling of \lstinline!callcc! and \lstinline!filter!,
we now focus on the methods for lifting \textsf{``}simple\textsf{''} and \textsf{``}standard\textsf{''}
operations to monad stacks.

\paragraph{Lifting \textsf{``}simple\textsf{''} operations}

We have seen in Section~\ref{subsec:Combining-monads-via-mtl-style}
that the \lstinline!State! monad\textsf{'}s \lstinline!get! and \lstinline!set!
operations are lifted to any monad stack by composing with a lift
function. This method applies equally well to any \textsf{``}simple\textsf{''} operation
$\text{sop}_{M}:F^{A}\rightarrow M^{A}$ of a monad $M$ and an arbitrary
monad stack $P$ that has a lift function $p:M^{A}\rightarrow P^{A}$.
The result is a simple operation $\text{sop}_{P}$ defined by
\begin{equation}
...
of \textsf{``}standard\textsf{''} operations, and also proves that the \textsf{``}standard\textsf{''}
version of $\text{sop}_{P}$ always satisfies a suitable monadic naturality
law.

\subsubsection{Statement \label{subsec:Statement-monatron-lifting-simple-operation-1}\ref{subsec:Statement-monatron-lifting-simple-operation-1}}
...
$P$, define the corresponding $P$-operation ($\text{sop}_{P}$)
by Eq.~(\ref{eq:monatron-lift-simple-operation-1}). Then the \textsf{``}standard\textsf{''}
operation $\text{op}_{P}\triangleq\text{sop}_{P}\bef\text{ftn}_{P}$
corresponding to $\text{sop}_{P}$ is expressed via $\text{op}_{M}\triangleq\text{sop}_{M}\bef\text{ftn}_{M}$
by
\[
\text{op}_{P}=\text{pu}_{M}^{\uparrow F}\bef\text{op}_{M}\bef p\bef\text{ftn}_{P}
\]
and satisfies the monadic naturality law:
\[
\text{op}_{M}\bef p=p^{\uparrow F}\bef\text{op}_{P}\quad.
\]
...
\begin{align*}
 & \gunderline{\text{op}_{M}}\bef p=\text{sop}_{M}\bef\gunderline{\text{ftn}_{M}\bef p}\\
...
The method of Statement~\ref{subsec:Statement-monatron-lifting-simple-operation-1}
allows us to lift a \textsf{``}simple\textsf{''} operation from a monad $M$ to any
monad stack $P$ containing $M$, as long as there exists a lift function
...
lift. For this method to work, one must place the continuation monad,
the search monad, the codensity monad, and task-like monads (such
as \lstinline!IO!) at the deep end of the stack, since those monads
do not support base lifts.

\paragraph{Lifting \textsf{``}standard\textsf{''} operations}

If a \textsf{``}standard\textsf{''} operation can be reduced to the form of a \textsf{``}simple\textsf{''}
operation, the lifting is just a composition with a lift function.
But some \textsf{``}standard\textsf{''} operations do not satisfy the compatibility
law~(\ref{eq:monatron-standard-operation-compatibility-law-1}),
so they need a different method of lifting.
...
\]
Its type signature $M^{A}\rightarrow M^{A}$ is of the \textsf{``}standard\textsf{''}
form $F^{M^{A}}\rightarrow M^{A}$ with $F^{A}\triangleq A$. It turns
out that \lstinline!clear! does not satisfy the compatibility law~(\ref{eq:monatron-standard-operation-compatibility-law-1})
...
\]
This operation appends to the log rather than erasing it. $\square$

We will first consider lifting a monad $M$\textsf{'}s \textsf{``}standard\textsf{''} operation
to a stack $P$ where the monad $M$ occurs at the end. So, we assume
that $P^{A}=T_{L}^{M,A}$ for some base monad (or stack) $L$.

A \textsf{``}standard\textsf{''} operation $\text{op}:F^{M^{A}}\rightarrow M^{A}$
is lifted to a stack $T_{L}^{M}$ in three steps: 
\begin{enumerate}
\item Convert $\text{op}$ into an equivalent \textsf{``}simple\textsf{''} operation $\text{cop}$
in the codensity monad $\text{Cod}^{M,\bullet}$.
\item Lift \lstinline!cop! to the stack $T_{L}^{\text{Cod}^{M,\bullet}}$
that contains the codensity monad instead of $M$.
\item Run the stack, replacing the codensity monad $\text{Cod}^{M,\bullet}$
...
turns out that the type of natural transformations $F^{A}\rightarrow\text{Cod}^{M,A}$
is \emph{equivalent} to the type of natural transformations $F^{M^{A}}\rightarrow M^{A}$.
To show this, we will prove a more general property that does not
depend on any monadic structures.

\subsubsection{Statement \label{subsec:Statement-kan-extension-property-1}\ref{subsec:Statement-kan-extension-property-1}}

For any three functors $F$, $K$, $L$, the set of natural transformations
of type $F\circ K\leadsto L$ is isomorphic to the set of natural
transformations of type
\[
F^{A}\rightarrow\forall R.\,(A\rightarrow K^{R})\rightarrow L^{R}\quad.
\]
...
\begin{align*}
 & \text{inK}:(\forall B.\,F^{K^{B}}\rightarrow L^{B})\rightarrow F^{A}\rightarrow\left(A\rightarrow K^{R}\right)\rightarrow L^{R}\quad,\quad\quad\text{inK}\,(\text{op}^{:\forall B.\,F^{K^{B}}\rightarrow L^{B}})\triangleq f^{:F^{A}}\rightarrow k^{:A\rightarrow K^{A}}\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op}^{B=R}\quad,\\
 & \text{outK}:\big(\forall R.\,F^{A}\rightarrow(A\rightarrow K^{R})\rightarrow L^{R}\big)\rightarrow F^{K^{A}}\rightarrow L^{A}\quad,\quad\quad\text{outK}\,(\text{cop}^{:F^{A}\rightarrow\forall R.\,(A\rightarrow K^{R})\rightarrow L^{R}})\triangleq g^{:F^{K^{A}}}\rightarrow(g\triangleright\text{cop})^{R=K^{A}}(\text{id}^{K^{A}})\quad.
\end{align*}
We now need to show that \lstinline!inK! and \lstinline!outK! are
inverses of each other. One direction:
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{id}:}\quad & \text{inK}\bef\text{outK}=(\text{op}\rightarrow f\rightarrow k\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op})\bef\big(\text{cop}\rightarrow g\rightarrow\text{cop}\,(g)(\text{id})\big)\\
 & =\text{op}\rightarrow g\rightarrow(\gunderline{f\rightarrow k\rightarrow}\,f\triangleright k^{\uparrow F}\triangleright\text{op})\gunderline{(g)(\text{id})}=\text{op}\rightarrow g\rightarrow g\triangleright\text{id}^{\uparrow F}\triangleright\text{op}\\
 & =\text{op}\rightarrow\gunderline{g\rightarrow g\,\triangleright}\,\text{op}=\text{op}\rightarrow\text{op}=\text{id}\quad.
\end{align*}
Proving the other direction requires using the naturality law of the
\lstinline!cop! functions:
\[
\text{cop}^{A,R}:F^{A}\rightarrow\left(A\rightarrow K^{R}\right)\rightarrow L^{R}\quad,\quad\quad(f^{:A\rightarrow B})^{\uparrow F}\bef\text{cop}^{B,R}=\text{cop}^{A,R}\bef\big(c^{:(A\rightarrow K^{R})\rightarrow L^{R}}\rightarrow k^{:B\rightarrow K^{R}}\rightarrow(f\bef k)\triangleright c\big)\quad.
\]
For convenience, we apply both sides of this naturality law to arbitrary
values $x^{:F^{A}}$and $y^{:B\rightarrow K^{R}}$:
\[
(x\triangleright f^{\uparrow F}\bef\text{cop})(y)=\text{cop}\,(x\triangleright f^{\uparrow F})(y)\overset{!}{=}\big(x\triangleright\text{cop}\bef(c\rightarrow k\rightarrow(f\bef k)\triangleright c)\big)(y)=\text{cop}\,(x)(f\bef y)\quad.
\]
We now rewrite $\text{outK}\bef\text{inK}$ to verify the other direction
of the isomorphism:
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{id}:}\quad & \text{outK}\bef\text{inK}=\big(\text{cop}\rightarrow g\rightarrow\text{cop}\,(g)(\text{id})\big)\bef(\text{op}\rightarrow f\rightarrow k\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op})\\
 & =\text{cop}\rightarrow f\rightarrow k\rightarrow f\triangleright k^{\uparrow F}\triangleright\big(g\rightarrow\text{cop}\,(g)(\text{id})\big)=\text{cop}\rightarrow f\rightarrow k\rightarrow\gunderline{\text{cop}\,(f\triangleright k^{\uparrow F})(\text{id})}\\
{\color{greenunder}\text{naturality law of }\text{cop}:}\quad & =\text{cop}\rightarrow f\rightarrow k\rightarrow\text{cop}(f)(k\bef\text{id})=\text{cop}\rightarrow f\rightarrow k\rightarrow\text{cop}\,(f)(k)=\text{cop}\rightarrow\text{cop}=\text{id}\quad.
\end{align*}
...
Note that the proof did not require the naturality law of the \lstinline!op!
functions,
\[
\text{op}^{A}:F^{K^{A}}\rightarrow L^{A}\quad,\quad\quad(f^{:A\rightarrow B})^{\uparrow K\uparrow F}\bef\text{op}^{B}=\text{op}^{A}\bef f^{\uparrow L}\quad.
\]
 Any \lstinline!op! function obtained via \lstinline!outK! will
satisfy this naturality law:
\begin{align*}
...
{\color{greenunder}\text{naturality law of }\text{cop}:}\quad & \quad=\text{cop}\,(x)(f^{\uparrow K}\bef\text{id})=\text{cop}\,(x)(f^{\uparrow K})\quad,\\
...
be a natural transformation with respect to $R$: 
\[
...
Using Statement~\ref{subsec:Statement-kan-extension-property-1}
with $K=L=M$, we find that any \textsf{``}standard\textsf{''} operation \lstinline!op!
in a monad $M$ is equivalent to a \textsf{``}simple\textsf{''} operation \lstinline!cop!
in the codensity monad $\text{Cod}^{M,\bullet}$. It is straightforward
to lift that operation to a stack $P^{\prime}\triangleq T_{L}^{\text{Cod}^{M,\bullet}}$
that contains the codensity monad instead of $M$. We use $T_{L}$\textsf{'}s
\lstinline!flift! function to transform between $P$ and $P^{\prime}$:
...
For any two monads $L$ and $M$, assume that a natural transformation
\lstinline!swap!$\,:M\circ L\leadsto L\circ M$ is given, the method
$\text{ftn}_{T}$ of the monad $T=L\circ M$ is \emph{defined} via
\lstinline!swap! by Eq.~(\ref{eq:define-flatten-via-swap}). Show
that the two interchange laws must hold for $\text{ftn}_{T}$:
\begin{align*}
