...
are called \textsf{``}rigid\textsf{''} in this book\footnote{There does not seem to exist any already accepted name for monads
of this kind.} because one of their general properties is having a single \textsf{``}data
shape\textsf{''} (Statement~\ref{subsec:Statement-rigid-functor-wrapped-unit-is-unit}
in Section~\ref{subsec:Rigid-functors}).\index{monads!rigid} 
...
The monad methods of $T_{\text{Search}}^{M}\triangleq T$ are written
...
\begin{align*}
 ...
 & f^{:H^{B}\rightarrow A\rightarrow T_{L}^{M,B}}\tilde{\diamond}_{_{T}}g^{:H^{C}\rightarrow A\rightarrow T_{L}^{M,C}}\triangleq k^{:H^{C}}\rightarrow f\big(k\triangleright\text{lift}_{T_{L},H}(g(k))\big)\diamond_{_{T_{L}}}g(k)\quad.
\end{align*}
...
We need to establish monad isomorphisms between the monads $(A\rightarrow L^{Q})\rightarrow L^{A}$
and $(A\rightarrow T_{L}^{\text{Id},Q})\rightarrow T_{L}^{\text{Id},A}$.
Denote for brevity $N^{A}\triangleq T_{L}^{\text{Id},A}$; our goal
is then to implement functions $p$ and $q$,
\[
...
that the identity laws hold for the transformer $T_{L}$. These laws
give us two monad morphisms $r^{A}:L^{A}\rightarrow N^{A}$ and $s^{A}:N^{A}\rightarrow L^{A}$
that satisfy $r\bef s=\text{id}$ and $s\bef r=\text{id}$. 
...
To implement $p$, we use typed holes:
\begin{align*}
 & p\triangleq u^{:(A\rightarrow L^{Q})\rightarrow L^{A}}\rightarrow v^{:A\rightarrow N^{Q}}\rightarrow\text{???}^{:N^{A}}=u\rightarrow v\rightarrow u(\text{???}^{:A\rightarrow L^{Q}})\triangleright r^{A}\\
 & =u\rightarrow v\rightarrow(\text{???}^{:A\rightarrow N^{Q}}\bef s^{Q})\triangleright u\bef r=u\rightarrow v\rightarrow(v\bef s)\triangleright u\bef r\quad.
\end{align*}
The code for $q$ is found by exchanging $r$ and $s$ in the code
of $p$:
\[
q\triangleq t\rightarrow w\rightarrow(w\bef r)\triangleright t\bef s\quad.
\]

To verify that $p\bef q=\text{id}$, write:
\begin{align*}
...
{\color{greenunder}\text{compute composition}:}\quad & =u\rightarrow w\rightarrow\gunderline{(w\bef r)\triangleright\big(v}\rightarrow(v\bef s)\triangleright u\bef r\big)\triangleright s\\
 & =u\rightarrow w\rightarrow(w\bef\gunderline{r\bef s})\triangleright u\triangleright\gunderline{r\triangleright s}=u\rightarrow(\gunderline{w\rightarrow w\,\triangleright}\,u)=u\rightarrow u=\text{id}\quad.
\end{align*}
...
\subsection{Example of a monad with two different transformers}

The selector monad, $\text{Sel}^{Q,A}=(A\rightarrow Q)\rightarrow A$,
is a rigid monad described in Section~\ref{subsec:Rigid-monad-construction-1-choice},
with a composed-outside transformer with the type $(M^{A}\rightarrow Q)\rightarrow M^{A}$.
...
with the type $(A\rightarrow M^{Q})\rightarrow M^{A}$. 

We have found an example of a monad that has two \emph{inequivalent}
transformers: the first transformer is covariant in $M$, the second
is not. Since the second transformer is incomplete (it lacks runners
and a base lift), it is preferable to use the rigid monad transformer.

\subsection{Transformer for the co-density monad (\texttt{CodT})}
...
Define the \textsf{``}search monad\textsf{''} as $\text{Search}^{P,A}\triangleq\left(A\rightarrow\bbnum 1+P\right)\rightarrow\bbnum 1+A$.
The function 
\begin{lstlisting}
