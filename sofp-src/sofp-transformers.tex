...
\[
\text{flift}:M^{A}\rightarrow L^{M^{A}}\quad,\quad\quad\text{flift}\triangleq\text{pu}_{L}\quad.
\]
To show that \lstinline!flift! is a monad morphism, begin with the
identity law:
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{pu}_{T}:}\quad & \text{pu}_{M}\bef\text{flift}=***\\
\end{align*}
