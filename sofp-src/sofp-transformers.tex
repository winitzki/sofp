...
for some base monad. We can recover $T$\textsf{'}s base monad $L$ as the
image of the identity monad:
\[
...
functors (in the category of monads). The composition of two or more
pointed functors is again a pointed functor.
...
(so, $F$ is not necessarily a functor).
...
(while \lstinline!outC! is \emph{not} a monad morphism).
...
where $S$ is limited to types of the form $F^{R}$. We do not need
...
two exceptions to that rule: monads with some inequivalent transformers.
...
The second example is the codensity monad ($\text{Cod}^{F,\bullet}$).
...
A base runner with type signature $\text{Cod}^{M,A}\rightarrow M^{A}$
is the function \lstinline!outC! from Statement~\ref{subsec:Statement-codensity-monad-inC-outC}.
The proof of that statement shows that \lstinline!outC! fails the
monad morphism composition law. $\square$

Exercise~\ref{par:Exercise-additional-16-1} shows that the \lstinline!List!
monad has an alternative monad transformer based on the codensity
monad.
