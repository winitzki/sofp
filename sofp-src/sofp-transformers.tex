...
codensity monad) have neither the \lstinline!frun! nor the \lstinline!blift!
...
$T^{M}$, $T^{N}$, etc. Given such a functor $T$, we \emph{define}
the base monad $L$ as the image of the identity monad:
\[
...
from the definition of $\text{Cod}_{F}^{M,A}$. For simplicity, set
$F^{A}\triangleq A$; show that for a fixed type $X$ and for some
$M^{\bullet}$, the type constructor $L^{A}\triangleq(A\rightarrow X)\rightarrow M^{X}$
is \emph{not} a monad.
