...
\subsubsection{Exercise \label{subsec:Statement-search-and-selector-monads}\ref{subsec:Statement-search-and-selector-monads}\index{exercise}}
...
The function $q:\text{Sel}^{P\{,\bullet}\leadsto\text{Cont}^{P,\bullet}$
...
if we restrict types to monoids: if $R$ is a monoid type, the standard
...
\end{array}\quad.
\]
We can similarly implement a base runner (\lstinline!brun!) with
the type signature $M^{L^{R}}\rightarrow M^{R}$. The code for \lstinline!brun!
aggregates all elements of the effectful list into a single value
of type $M^{R}$:
\[
...
defined by
\begin{equation}
p^{:M^{R}}\oplus_{M}q^{:M^{R}}\triangleq p\triangleright\text{flm}_{M}\big(u^{:R}\rightarrow q\triangleright(v^{:R}\rightarrow u\oplus_{R}v)^{\uparrow M}\big)\quad.\label{eq:definition-of-oplus-M-monad-monoid}
\end{equation}

We then require that the monad morphism laws of \lstinline!brun!
hold when applied to any monoid type $R$:
\begin{align*}
{\color{greenunder}\text{for all monoid types }R:}\quad & r^{:R}\triangleright\text{pu}_{T}\bef\text{brun}=r^{:R}\triangleright\text{pu}_{M}\quad,\\
 & p^{:T^{T^{R}}}\triangleright\text{ftn}_{T}\bef\text{brun }=p^{:T^{T^{R}}}\triangleright\text{brun}\bef\text{brun}^{\uparrow M}\bef\text{ftn}_{M}\quad.
\end{align*}
(Exercise~\ref{subsec:Exercise-traversables-10-1} then shows that
\lstinline!brun! is also a \emph{monoid} morphism $M^{L^{R}}\rightarrow M^{R}$.)
...
The base runner ($\text{brun}:T^{R}\rightarrow M^{R}$ where $R$
is a monoid type) must satisfy the monad morphism laws and the non-degeneracy
law. To verify the monad morphism identity law:
\begin{align*}
...
\end{array}\bef\text{brun}\\
 & \quad=\text{comb}\,(h)(t\triangleright\text{flm}_{M}(\overline{\text{prod}}))\bef\text{flm}_{M}\bigg(\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M}(e_{R})\\
r\times t\rightarrow\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}\,(t)
\end{array}\,\bigg)\\
 & \quad=***\\
...
\]
(finish the derivation of the composition law){*}{*}{*}
...
\end{array}\\
 & =\,\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M^{\prime}}(e_{R})\\
r\times t\rightarrow\text{pu}_{M^{\prime}}(r)\oplus_{M^{\prime}}\big(t\triangleright\gunderline{\overline{\psi}^{\uparrow M}\bef\phi\bef\overline{\text{brun}}{}^{M^{\prime}}}\big)
\end{array}\\
...
\]
This property is verified by this derivation:
\begin{align*}
