...
\]
To implement the base runner, we will first look for a runner of type
$\text{Free}^{F\circ M}\leadsto M$ given a runner $\theta:\text{Free}^{F}\leadsto\text{Id}$.
Using the similarity between the structure of the two disjunctive
types, 
\[
...
\]
we implement the base runner via the code
\begin{align*}
...
free pointed monads, and recursive monads are \textsf{``}complete\textsf{''} in the
