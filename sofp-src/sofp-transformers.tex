...
\]
We will now define a suitable \lstinline!swap! function and verify
its special laws. Then the results of Section~\ref{sec:Monad-transformers-that-use-composition}
ensure that $T$ is a lawful monad transformer. 

\subsubsection{Statement \label{subsec:Statement-choice-monad-swap}\ref{subsec:Statement-choice-monad-swap}}

Define the function \lstinline!swap! (short notation $\text{sw}_{R,M}$)
as
\begin{equation}
\text{sw}_{R,M}:M^{R^{A}}\rightarrow R^{M^{A}}\quad,\quad\quad\text{sw}_{R,M}\triangleq m^{:M^{R^{A}}}\rightarrow q^{:H^{M^{A}}}\rightarrow m\triangleright(r\rightarrow q\triangleright\text{pu}_{M}^{\downarrow H}\bef r)^{\uparrow M}\quad.\label{eq:rigid-monad-1-forward-formula-for-swap-1}
\end{equation}
The function \lstinline!swap! satisfies the four laws of Statement~\ref{sec:Statement-swap-laws-to-monad-transformer-first-law}
and the monadic naturality law of {*}{*}{*}.
...
and instead consider the \textbf{flipped Kleisli}\index{flipped@\textsf{``}flipped Kleisli\textsf{''} technique}
...
\]
where $T_{L}$ is the monad $L$\textsf{'}s transformer.
