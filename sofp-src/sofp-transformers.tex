...
erquivalent to \textsf{``}simple\textsf{''} operations because they satisfy the compatibility
law. 
...
Certain \textsf{``}standard\textsf{''} operations are not equivalent to \textsf{``}simple\textsf{''}
ones:
...
We will first consider lifting a \textsf{``}standard\textsf{''} operation $\text{op}:F^{M^{A}}\rightarrow M^{A}$
...
\begin{align*}
 & \text{inK}:(\forall B.\,F^{K^{B}}\rightarrow L^{B})\rightarrow F^{A}\rightarrow\left(A\rightarrow K^{R}\right)\rightarrow L^{R}\quad,\\
...
\[
\text{frun}_{L}(\text{inC}):T_{L}^{M}\leadsto T_{L}^{\text{Cod}^{M}}\quad,\quad\quad\text{frun}_{L}(\text{outC}):T_{L}^{\text{Cod}^{M}}\leadsto T_{L}^{M}\quad.
\]
One way of converting $\text{scop}_{P^{\prime}}$ into a $P$-operation
would be to compose with $\text{flift}_{L}(\text{outC})$, which creates
a \textsf{``}simple\textsf{''} operation $\text{sop}_{P}$ of type $F^{A}\rightarrow T_{L}^{M}$.
...
{\color{greenunder}\text{monadic naturality law of }\text{flift}_{L}:}\quad & =\text{scop}\bef\text{outC}\bef\text{flift}_{L}=(f\rightarrow k\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op})\bef\text{outC}\bef\text{flift}_{L}\\
 & =(f\rightarrow f\triangleright\text{pu}_{M}^{\uparrow F}\bef\text{op})\bef\text{flift}_{L}=\text{pu}_{M}^{\uparrow F}\bef\text{op}\bef\text{flift}_{L}\quad.
\end{align*}
...
of \lstinline!flift! with $\phi\triangleq\text{outC}$ even though
...
Then define the corresponding \textsf{``}standard\textsf{''} operation \lstinline!cop!
(in the codensity monad) via Statement~\ref{subsec:Statement-simple-standard-operations-monatron}:
\[
\text{cop}:F^{\text{Cod}^{M,A}}\rightarrow\text{Cod}^{M,A}\quad,\quad\quad\text{cop}\triangleq\text{pu}_{\text{Cod}}^{\uparrow F}\bef\text{scop}\quad.
\]
