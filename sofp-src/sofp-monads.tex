...
method, and \textbf{semimonads} are functors that additionally have
a \lstinline!.flatMap! method. This chapter begins by developing
an intuition for the behavior of the \lstinline!.flatMap! method.
...
study semimonads and monads in detail. For now, we note that the presence
of the \lstinline!pure! method is not required by the functor block
syntax.

If a functor has a \lstinline!.withFilter! method, the functor block
will support embedded \lstinline!if! conditions (see Section~\ref{sec:Practical-uses-of-filterable-functors}).
So, filterable semimonads allow us to use the full functionality of
functor blocks.
...
task may be used, for instance, to build and traverse a search tree
and to filter out incorrect solutions.
...
We will compute a \emph{sequence} of all permutations. First attempt:
\begin{lstlisting}
scala> for {
         x <- Seq("a", "b", "c")
         y <- Seq("a", "b", "c")
...
\end{lstlisting}
To obtain all permutations and nothing else, but we need to exclude
all repeated subsequences such as \lstinline!"aab"!. To achieve that,
we must make \lstinline!y! iterate over letters that do not include
the current value of \lstinline!x!. One solution is to exclude values
...
scala> for {
      x <- xs
      remainAfterX = xs.filter(_ != x)
      y <- remainAfterX
      remainAfterY = remainAfterX.filter(_ != y)
      z <- remainAfterY
} yield x + y + z
...
Compute the set of all subsets of \lstinline!Set("a", "b", "c")!. 
...
Compute all subsequences of length $3$ out of a given sequence.

\subparagraph{Solution}

{*}{*}{*}Another implementation is to use the standard method \lstinline!.tails!
defined on sequences:
\begin{lstlisting}
scala> Seq("a", "b", "c").tails.toList
res2: List[Seq[String]] = List(List(a, b, c), List(b, c), List(c), List()) 
\end{lstlisting}
...
to support arbitrary length $n$ instead of \lstinline!3!.
...
Transform Boolean formulas between CNF\footnote{\texttt{\href{https://en.wikipedia.org/wiki/Conjunctive_normal_form}{https://en.wikipedia.org/wiki/Conjunctive\_normal\_form}}}
and DNF\footnote{\texttt{\href{https://en.wikipedia.org/wiki/Disjunctive_normal_form}{https://en.wikipedia.org/wiki/Disjunctive\_normal\_form}}}.
...
\paragraph{{*} Motivation for categories and functors}
