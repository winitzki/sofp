...
\end{lstlisting}
This functor block needs to be written inside the yield value of the
iterations of the indices:
\begin{lstlisting}
scala> val matrix_T = for { index <- matrix.head.indices } // [0, 1, 2] are the column indices.
                      yield { for { row <- matrix}         // Iterate over rows of the matrix.
                              yield row(index)
                            }
...
The code shown in this example is for illustration only; for high
...
Implement the matrix-vector dot product (regardless of performance)
as a function
\begin{lstlisting}
def ???
\end{lstlisting}
