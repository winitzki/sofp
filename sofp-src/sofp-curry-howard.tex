...
denoted using the symbol $\vdash$ (``turnstile''):
\begin{equation}
...
in the type notation. %
\begin{comment}
Solution: $A\times\text{Int}+A\times\text{Char}+A\times\text{Float}$.
\end{comment}
...
Type expressions that also contain function types may be called \textbf{exponential-polynomial}\index{exponential-polynomial types}\index{types!exponential-polynomial}.\footnote{Polynomial types are often called ``algebraic data types\index{algebraic data type}''.}
...
``null'', or is a pointer to an invalid memory location.
