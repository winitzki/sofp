...
is equivalent to ``${\cal CH}(A)$ is true, assuming ${\cal CH}(X)$,
${\cal CH}(Y)$, ..., ${\cal CH}(Z)$ are true''. In mathematical
logic, a statement of this form is called a \textbf{sequent} and is
denoted using the symbol $\vdash$ (the ``turnstile''):\index{vdash (turnstyle) symbol@$\vdash$ (turnstyle) symbol}\index{turnstile (vdash) symbol@turnstile ($\vdash$) symbol}
\begin{equation}
...
an ``empty tuple''. This value can be \emph{always} computed since
...
algorithm'', see Appendix~\ref{app:CH-correspondence-LJT-algorithm}).
...
library for ``\index{code inference}code inference''. We will run
...
languages is clearly not Boolean. Peirce's law is a true theorem in
...
what it means to infer code ``guided by the types''. However, in
...
``is'' a proof of the logical formula~(\ref{eq:ch-proposition-example-2}),
...
A~ & x^{:A}\rightarrow x & \bbnum 0
\end{array}\\
\text{matrix multiplication}:\quad & =\,\,\begin{array}{|c||cc|}
 & A & B\\
...
\end{array}\\
\text{matrix multiplication}:\quad & =\,\,\begin{array}{||cc|}
...
\text{compute composition}:\quad & \quad=h^{:\bbnum 1\rightarrow A}\rightarrow1\rightarrow h(1)\\
...
\text{compute composition}:\quad & \quad=x^{:A}\rightarrow(1\rightarrow x)(1)\\
...
function whose body is ``empty''; or at least it does not need to
...
Type expressions that also contain function types may be called \textbf{exponential-polynomial}\index{exponential-polynomial type}\index{types!exponential-polynomial types}.\footnote{Polynomial types are often called ``algebraic data types\index{algebraic data types}''.}
...
will crash due to using a value that has a wrong type, or is ``null'',
...
The \index{void type}void type (Scala's \lstinline!Nothing!) corresponds
...
``value'' (which will never be actually computed) into a value of
...
a value ``should exist''; the real requirement is to \emph{compute}
a value of type $\left(A\rightarrow\bbnum 0\right)+A$ via a fully
parametric function. That function's code may not decide what to do
...
features of Scala (``reflection'', ``type tags'', and ``type
casts'') allow us to compare types as values and to determine what
