...
\]
The symbol $\forall$ means \textsf{``}for all\textsf{''} and is called the \index{universal quantifier}\textbf{universal
quantifier} in logic. 
...
code\index{\textsf{``}fully parametric!code}\textsf{''} and allows us to \emph{prove}
${\cal CH}$-propositions.
...
Begin with the left-most leaf \textsf{``}$\text{use arg}$\textsf{''}. This rule gives
the code $x^{:A}$:
\[
...
preceding proofs:
\begin{align*}
 & \text{Proof}\left((\alpha\Rightarrow\alpha)\Rightarrow\beta\vdash\beta\right)_{\text{given }f}\\
...
\begin{align*}
 ...
 & =f^{:\left(A\rightarrow A\right)\rightarrow B}\rightarrow\text{Proof}\left((\alpha\Rightarrow\alpha)\Rightarrow\beta\vdash\beta\right)_{\text{given }f}\\
...
an algorithm (the \textsf{``}\index{LJT algorithm}LJT algorithm\textsf{''}, see Appendix~\ref{app:The-LJT-algorithm}).
...
Fully parametric functions can be sometimes derived from their type
signatures alone. It is useful for a programmer to know that certain
...
derive an implementation by hand or to recognize when an implementation
...
that contain only products and sum types are called \textbf{polynomial}\index{polynomial type}\index{types!polynomial types}.
...
and translating it into a minimal \textsf{``}core\textsf{''} of a programming language.
Code in that language will be guaranteed \emph{never to crash} as
long as all types match. This mathematical guarantee (known as \index{type safety}\textbf{type
...
is used to denote functions that return no data. These functions are
...
statement was proved by Kurt G\"odel\index{Kurt@Kurt G\"odel} in
...
A fully parametric function needs to have the same code for all types
$A$. Since there are no values of type $\bbnum 0$, and the type
parameter $A$ could be, say, \lstinline!Int!, we cannot compute
a value of type $A\rightarrow\bbnum 0$.
...
is inconsistent with the actually given code of \lstinline!x!. 
