...
(the \textbf{numeric exponent}\index{exponent}, $\left|B\right|$
...
such derivations to code that appears to compute a certain value (since
the $\mathcal{CH}$-proposition appears to be $True$) although that
value is not actually available. In practice, such code \emph{crashes}
because the computed value has a wrong type, is \textsf{``}null\textsf{''}, or is
