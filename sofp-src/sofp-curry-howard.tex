...
\begin{center}
{\footnotesize{}}{\footnotesize{}\Tree[ .$\emptyset\vdash((\alpha\Longrightarrow\alpha)\Longrightarrow\beta)\Longrightarrow\beta $ [ .{rule ``$\text{create function}$''} [ .$(\alpha\Longrightarrow\alpha)\Longrightarrow\beta\vdash\beta$ [ .{rule ``$\text{use function}$''} [ .$(\alpha\Longrightarrow\alpha)\Longrightarrow\beta\vdash\alpha\Longrightarrow\alpha$ [ .{rule ``$\text{create function}$''} [ .$\Gamma_1,\alpha\vdash\alpha$ {axiom ``$\text{use arg}$''} ] ] ] [ .$(\alpha\Longrightarrow\alpha)\Longrightarrow\beta\vdash(\alpha\Longrightarrow\alpha)\Longrightarrow\beta$ {axiom ``$\text{use arg}$''} ] ] ] ] ]}
\par\end{center}%
...
output parts:\index{disjunctive type!matrix notation}\index{matrix notation}\begin{wrapfigure}{l}{0.5\columnwidth}%
\vspace{-0.5\baselineskip}
\begin{lstlisting}
def f1[A, B]: Either[A, B] => Either[B, A] = {
  case Left(a)    => Right(a)
