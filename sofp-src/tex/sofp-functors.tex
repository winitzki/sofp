...
Some type constructors are covariant or contravariant, as we have
seen. There is an important connection between covariance/contravariance
and the feature called \textsf{``}subtyping\textsf{''} that is present in Scala and
other programming languages. Let us begin by explaining how subtyping
works.

Ordinarily, it is a type error to apply a function of type $Q\rightarrow R$
to a value of a different type $P\neq\ensuremath{Q}$:
\begin{lstlisting}
def h(q: Q): R = ???
val p: P = ???
h(p)         // Type error: expected type Q but found P.
\end{lstlisting}
However, Scala will accept this kind of code when $P$ is a \textsf{``}subtype\textsf{''}
of $Q$ (which we will denote by $P\lesssim Q$). If $P\lesssim Q$
then it will \emph{not} be a type error to use values of type $P$
in places that ordinarily require the type $Q$. We will see shortly
how this works in Scala. But before that, let us look at some general
arguments that make the picture of subtyping simpler and more precise.

If $P\lesssim Q$, we are allowed to apply the identity function $\text{id}^{:Q\rightarrow Q}$
to a value $x$ of type $P$. Then the value $\text{id}^{:Q\rightarrow Q}(x)$
will have type $Q$. This uniquely defines a function of type $P\rightarrow Q$
for each pair of types $P$, $Q$ such that $P\lesssim Q$. The code
of that function is $c^{:P\rightarrow Q}\triangleq x^{:P}\rightarrow\text{id}^{:Q\rightarrow Q}(x)$
or, written more concisely, $c(x)=\text{id}\,(x)$. We call $c^{:P\rightarrow Q}$
the \textsf{``}subtype conversion function\textsf{''} for the types $P$ and $Q$.

For any $f^{:Q\rightarrow R}$ and $x^{:P}$, the expression $f(x)$
is accepted by the compiler if $P\lesssim Q$. However, since $f=\text{id}^{:Q\rightarrow Q}\bef f$,
we must have $f(x)=f(\text{id}\,(x))=f(c(x))$. So, subtyping is equivalent
to an automatic conversion of values of type $P$ into values of type
$Q$ via the designated conversion function $c^{:P\rightarrow Q}$.
We may imagine that the compiler inserts the function $c$ automatically
whenever needed.

Due to this property, we \emph{define} subtyping in terms of conversion
functions:

\subsubsection{Definition \label{subsec:Definition-subtyping}\ref{subsec:Definition-subtyping} }

A type $P$ is a \textbf{subtype}\index{types!subtyping}\index{subtyping}
of a type $Q$ (denoted $P\lesssim Q$) if the compiler automatically
inserts the fixed \index{subtyping!subtype conversion function|textit}
\textbf{subtype conversion} \textbf{function} $c$ of type $P\rightarrow Q$
whenever a value of type $P$ is used at a place that requires the
type $Q$. $\square$

Note that $c$ is just an identity function with a reassignment of
types; $c(x)$ does not actually transform the data in $x$. So, the
compiler will not need to insert any additional machine code. This
is the case in most programming languages that have subtyping. Let
us look at some examples of subtyping of that kind. 
...
Another example is  subtyping between \emph{function} types. Look
at these $P$, $Q$:
\begin{lstlisting}
...
type $A$. The corresponding conversion function is \lstinline!absurd!
$:\bbnum 0\rightarrow A$ (shown in Example~\ref{subsec:ch-Example-type-identity-0-to-A}).
The Scala compiler recognizes this automatically. Note that the type
$\bbnum 0$ has \emph{no} values, so the conversion function never
actually needs to be applied. 
...
\]
This gives us a conversion function from $L^{P}$ to $L^{Q}$. So,
it makes sense to define the subtyping relation between the types
$L^{P}$ and $L^{Q}$ as \lstinline!L[P] <: L[Q]! (in the type notation,
$L^{P}\lesssim L^{Q}$). If the code of $c$ is an identity  function
then $\text{fmap}_{L}(c)$\textsf{'}s code is also an identity function because
