...
how this works in Scala. But before that, let us show a general construction
that makes the picture of subtyping simpler and more precise.

If $P\lesssim Q$, we are allowed to apply the identity function $\text{id}^{:Q\rightarrow Q}$
to any value $x$ of type $P$. Then the value $\text{id}^{:Q\rightarrow Q}(x)$
...
It is valid to imagine that the compiler inserts the function $c$
automatically whenever needed.

Due to this property, we may \emph{define} subtyping in terms of conversion
functions:
