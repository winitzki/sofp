...
to its type parameter. There is an important connection between co(ntra)variance
and the feature called \textsf{``}subtyping\textsf{''} that is present in many programming
languages. Let us begin by explaining how subtyping works.
...
If $P\lesssim Q$ then we are allowed to apply the identity function
$\text{id}^{:Q\rightarrow Q}$ to a value $x$ of type $P$; the result
value, $\text{id}^{:Q\rightarrow Q}(x)$, will have type $Q$. This
uniquely defines a function of type $P\rightarrow Q$ for each pair
of types $P$, $Q$ such that $P\lesssim Q$. The code of that function
is $c^{:P\rightarrow Q}\triangleq x^{:P}\rightarrow\text{id}^{:Q\rightarrow Q}(x)$
or, written more concisely, $c(x)=\text{id}\,(x)$. We call $c$ the
\textsf{``}subtype conversion function\textsf{''}.

For any $f^{:Q\rightarrow R}$, the expression $f(x)$ is accepted
by the compiler and has type $R$. However, since $f=\text{id}^{:Q\rightarrow Q}\bef f$,
we must have $f(x)=f(\text{id}\,(x))=f(c(x))$. It follows that subtyping
automatically converts values of type $P$ via the designated conversion
function $c^{:P\rightarrow Q}$ into values of type $Q$ whenever
needed.
