...
of type $P$ instead of a value of type $Q$ in any expression, as
long as $P$ is a subtype of $Q$. Each programming language defines
in some way the subtypes of every given type. 
...
to \emph{define} subtyping through the existence of conversion functions:
...
does not lead to any decrease in performance. Let us look at some
examples of subtyping of that kind. 
...
a subtype conversion function $f:P\rightarrow Q$ into:
\[
...
the automatic subtyping features for \lstinline!Counted!. While doing
that, the Scala compiler will check that \lstinline!Counted! is in
fact covariant. It is a type error if a variance annotation specified
by the programmer does not match the actual covariance or contravariance
...
\subsection{Examples: implementing functors and contrafunctors\index{examples}}
...
The bullet symbol ($\bullet$) denotes the position of the free type
parameter. So, $F^{\bullet,B}$ is a type constructor with one type
parameter (and with $B$ fixed).
...
\end{equation}
where $S$ is a suitably chosen bifunctor called a \index{recursion scheme|textit}\textbf{recursion
scheme}. If a recursion scheme $S$ is given, the Scala code defining
$L$ can be written as:
\begin{lstlisting}
