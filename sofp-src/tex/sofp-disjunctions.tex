...
\subsubsection{Exercise \label{subsec:Exercise-disjunctive-3-1}\ref{subsec:Exercise-disjunctive-3-1}}

Define a disjunctive type \lstinline!RootOfLinear2! representing
all possibilities for the solutions of a system of two linear equations:
$a_{1}x+b_{1}y=c_{1}$, $a_{2}x+b_{2}y=c_{2}$ for arbitrary real
parameters. The possibilities are: no solutions; one solution; a linear
set of solutions; all $x$ and $y$ are solutions. Implement a solver
as a function \lstinline!solve2! with type signature: 
\begin{lstlisting}
def solve2(a1: Double, b1: Double, c1: Double, a2: Double, b2: Double, c2: Double): RootOfLinear2 = ???
\end{lstlisting}
What additional cases would be needed in the type \lstinline!RootOfLinear2!
if we wanted to describe numerical instability, overflow, and underflow
that could occur in the solutions?
