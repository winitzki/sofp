...
def countryCode(phone: Option[Long]): Option[Long] = phone match {
  case None      => None
  case Some(n)   =>
    val code = n / 10000000000L
    if (code != 0L) Some(code) else None 
}
\end{lstlisting}
...
To avoid that, we could try rewriting \lstinline!map! via the accumulator
...
We implement this sub-language in two stages. First, we create a data
structure (an AST) that represents an unevaluated expression\index{unevaluated expression}
...
\end{lstlisting}
This type does not include any tags telling us which of the values
is present. Without tags, we (and the compiler) will not know whether
...
while disjunctive types are usually not supported except in languages
designed for the FP paradigm. The reason why disjunctive types are
absent from C++ appears to be a decision by \index{Bjarne Stroustrup}B.~Stroustrup\footnote{See the 2025 talk by C.~Muratori (\texttt{\href{https://news.ycombinator.com/item?id=44596554}{https://news.ycombinator.com/item?id=44596554}})
at 0:48.} who perceived disjunctive types solely as a set of classes that inherit
from a common parent class. In that OOP-centric view, pattern matching
is just the ability to distinguish at run time between specific child
classes that implement a given interface. Stroustrup did not include
that ability in C++ because he viewed it as a violation of the interface
encapsulation principle: code working with an interface type should
not have access to implementation details.\footnote{The programming languages Simula, Ada, and Pascal support disjunctive
types but no other FP features. Languages such as C\#, Java, Go and
others continue the legacy of C++ and do not have direct support for
disjunctive types and pattern matching.} 

It is true that disjunctive types are represented in Scala by case
classes that inherit from a common \textsf{``}\lstinline!sealed trait!\textsf{''}.
But the OOP analogy stops here. The keyword \textsf{``}\lstinline!sealed!\textsf{''}
guarantees that no more inheritance is possible: values of the trait\textsf{'}s
type can be created only through the given set of case classes. In
that situation, the case classes are not implementations of a common
interface but a representation of the application\textsf{'}s data model. The
application code \emph{must} be able to distinguish between the case
classes. The implementation detail is the fact that Scala\textsf{'}s disjunctive
types are JVM classes inheriting from an interface.

Programming languages of the C family (C, C++, Objective C, Java,
C\#) support \textbf{enumeration} (\lstinline!enum!) types\index{enumeration type},
...
When reasoning about disjunctive types, it is inconvenient to write
out the type definitions and to use the tag names. Chapter~\ref{chap:5-Curry-Howard}
...
We find that tuple types are related to logical conjunctions in the
...
The functional programming community also calls them sum types,\index{sum type!see \textsf{``}disjunctive type\textsf{''}}
co-product types,\index{co-product type!see \textsf{``}disjunctive type\textsf{''}}
variants, disjoint unions,\index{disjoint union!see \textsf{``}disjunctive type\textsf{''}}
and tagged unions.\index{tagged union!see \textsf{``}disjunctive type\textsf{''}}\index{disjunctive type}
