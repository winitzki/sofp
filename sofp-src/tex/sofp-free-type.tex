...
on \lstinline!F! takes any other monad $M$ and a polymorphic function
of type $\forall T.\,F^{T}\rightarrow M^{T}$ and converts a free
monad value of type \lstinline!MonadDSL[F, A]! into a result value
of type \lstinline!M[A]!.
...
\subsection{Expected properties of a free typeclass construction}
...
effect constructor (say, $G$) such that we have a function $f:\forall T.\,F^{T}\rightarrow G^{T}$.
Then the universal runner can give us a transformation from \lstinline!Free4[F, A]!
to \lstinline!Free4[G, A]!. We just need to supply an effect runner,
which must be a function of type $\forall T.\,F^{T}\rightarrow\text{Free}_{4}^{G,T}$.
...
It turns out that \lstinline!run4M(runnerM)! always produces functions
(of type $\forall A.\,\text{Free}_{4}^{F,A}\rightarrow M^{A}$) that
\textsf{``}preserve the monad\textsf{'}s operations\textsf{''} in the sense that they satisfy
the laws of monad morphisms. (Those functions are not, strictly speaking,
monad morphisms because $\text{Free}_{4}^{F,A}$ does not itself obey
the monad laws.) To understand the significance of this property,
we need to understand in detail why the monad laws hold after applying
the runner. For that, it will help to replace the type of \lstinline!run4M!
by a simpler type signature. The type of \lstinline!run4M! is written
in the type notation as:
\[
\text{run}_{4}^{F,M,A}:(\forall T.\,F^{T}\rightarrow M^{T})\rightarrow\text{Free}_{4}^{F,A}\rightarrow M^{A}\quad.
\]
Since $F$ is a parameter, we may set $F=M$ in that type. Then the
effect runner (of type $\forall T.\,M^{T}\rightarrow M^{T}$) can
be just the identity function. Evaluating $\text{run}_{4}^{M,M,A}(\text{id})$,
we obtain a function of type $\text{Free}_{4}^{M,A}\rightarrow M^{A}$
that we call the \textsf{``}free evaluator\textsf{''} for \lstinline!Free4!. The
Scala code for the free evaluator of \lstinline!Free4! is a simplification
of the code of \lstinline!run4M!:
\begin{lstlisting}
...
The contravariant Yoneda identity for type constructors (Eq.~(\ref{eq:contravariant-yoneda-identity-for-type-constructors})
proved in Section~\ref{subsec:The-Yoneda-identities-for-type-constructors})
gives the type equivalence: 
\begin{align*}
...
\end{align*}
We call the function of type $\text{Free}_{4}^{M,A}\rightarrow M^{A}$
the \textsf{``}free evaluator\textsf{''}. 

Values of type $\text{Free}_{4}^{F,A}$ do not obey the monad laws
directly; however, the monad laws \emph{will} hold after applying
the runner, as long as $M$ is a lawful monad. To understand how that
happens, 

{*}{*}{*}we need to look in detail at the way the runner converts
a free monad program\textsf{'}s effects to the effects of a specific monad
$M$. 

{*}{*}{*}The code of \lstinline!run4M! converts the free monad\textsf{'}s
\lstinline!pure! to monad $M$\textsf{'}s \lstinline!pure!; the free monad\textsf{'}s
\lstinline!map! to monad $M$\textsf{'}s \lstinline!map!; and the free monad\textsf{'}s
\lstinline!flatMap! to the monad $M$\textsf{'}s \lstinline!flatMap!. Whenever
a value of type $\text{Free}_{4}^{F,A}$ uses the effect constructor
$F$ via the \lstinline!Op! case class, the given function of type
$F^{T}\rightarrow M^{T}$ is applied.

{*}{*}{*}

Violations of the monad laws are \textsf{``}hidden\textsf{''} when we map to a lawful
monad in an operation-preserving way.

{*}{*}{*} universal runner, E{[}S{]} => S instead of (T => S) => E{[}T{]}
=> S; typeclass-preserving maps; universal properties; covariance
in T; etc.?
...
def runner[T, P: HasDefault](run: T => P): Option[T] => P = {
  case Some(t)   => run(t)
  case None      => implicitly[HasDefault[P]].value // Use the default.
}
\end{lstlisting}
...
that expression tree. Given any semigroup \lstinline!S! and any function
of type \lstinline!T => S!, we can evaluate \lstinline!FSR[T]! into
\lstinline!S! like this:
\begin{lstlisting}
