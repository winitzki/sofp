...
and also a fixpoint of the pattern functor $F$. Section~\ref{subsec:Recursive-types-and-the-existence-of-their-values}
...
The \textbf{least fixpoint} of the type equation $T\cong F^{T}$ is
a type denoted by $\text{LFix}^{F}$ such that for any other fixpoint
$U$ there exists a fixpoint-preserving function of type $\text{LFix}^{F}\rightarrow U$.
The \textbf{greatest fixpoint} of the same type equation is a type
denoted by $\text{GFix}^{F}$ such that for any other fixpoint $U$
there exists a fixpoint-preserving function of type $U\rightarrow\text{GFix}^{F}$.
$\square$

The least fixpoint $\text{LFix}^{F}$ is a \textsf{``}smaller\textsf{''} type than
any other fixpoint $U$, in the sense that the map $\text{LFix}^{F}\rightarrow U$
is always injective: there exists another function of type $U\rightarrow\text{LFix}^{F}$
that recovers the values of type $\text{LFix}^{F}$ that were embedded
into the type $U$. Similarly, $\text{GFix}^{F}$ is \textsf{``}larger\textsf{''}
than any other fixpoint $U$ because the map $U\rightarrow\text{GFix}^{F}$
is always surjective. All this follows from the next statement.

\subsubsection{Statement \label{subsec:Statement-fixpoint-injective-surjective}\ref{subsec:Statement-fixpoint-injective-surjective}}

Any fixpoint-preserving function is injective. {*}{*}{*} Maybe we
can't prove this; or maybe we can only prove for LFix and GFix using
the uniqueness property of algebra morphisms. Not clear how to deal
with partial functions here.

In detail: Let $F$ be any covariant functor. Let $U$ and $V$ be
two fixpoint type satisfying the type equations $U\cong F^{U}$, $V\cong F^{V}$,
with the corresponding functions $\text{fix}_{U}$, $\text{unfix}_{U}$,
$\text{fix}_{V}$, $\text{unfix}_{V}$. Suppose $f:U\rightarrow V$
is a fixpoint-preserving function. Then $f$ is injective.

\subparagraph{Proof}

We need to define a function $g:V\rightarrow U$ such that:
\[
f\bef g=\text{id}\quad.
\]
There is no requirement that $g$ should be fixpoint-preserving; and
$g$ could even be a partial function; all we need is the law $f\bef g=\text{id}$.

To define $g$, we write a recursive code formula:
\[
g:V\rightarrow U\quad,\quad\quad g\triangleq v^{:V}\rightarrow\text{unfix}_{V}\bef\overline{g}^{\uparrow F}\bef\text{fix}_{U}\quad.
\]
{*}{*}{*}$\square$
