...
Using Eq.~(\ref{eq:existential-via-universal-Yoneda}), we can prove
important type equivalences similar to the Yoneda identities (Section~\ref{subsec:Yoneda-identities})
but involving existential types. Those type equivalences are known
as the co-Yoneda identities.

\subsubsection{Statement \label{subsec:Statement-contravariant-co-yoneda-identity-for-types}\ref{subsec:Statement-contravariant-co-yoneda-identity-for-types}
(contravariant co-Yoneda identity\index{co-Yoneda identity!contravariant})}

If $Z$ is a fixed type and $H$ is any contrafunctor, the type $H^{Z}$
is equivalent to the type $\exists A.\,(Z\rightarrow A)\times H^{A}$. 

\subparagraph{Proof}

We express the given existential type via Eq.~(\ref{eq:existential-via-universal-Yoneda}):
\begin{align*}
 & \exists A.\,(Z\rightarrow A)\times H^{A}\cong\forall D.\,(\forall A.\,((Z\rightarrow A)\times H^{A}\rightarrow D))\rightarrow D\\
{\color{greenunder}\text{convert to curried function}:}\quad & \cong\forall D.\,(\gunderline{\forall A.\,((Z\rightarrow A)\rightarrow H^{A}\rightarrow D)})\rightarrow D\\
{\color{greenunder}\text{covariant Yoneda identity}:}\quad & \forall D.\,(H^{Z}\rightarrow D)\rightarrow D\cong H^{Z}\quad.
\end{align*}


\subsubsection{Statement \label{subsec:Statement-covariant-co-yoneda-identity-for-types}\ref{subsec:Statement-covariant-co-yoneda-identity-for-types}
(covariant co-Yoneda identity)\index{co-Yoneda identity!covariant}}

If $Z$ is a fixed type and $H$ is any functor, the type $H^{Z}$
is equivalent to the type $\exists A.\,(A\rightarrow Z)\times H^{A}$. 

\subparagraph{Proof}

We express the given existential type via Eq.~(\ref{eq:existential-via-universal-Yoneda}):
\begin{align*}
 & \exists A.\,(A\rightarrow Z)\times H^{A}\cong\forall D.\,(\forall A.\,((A\rightarrow Z)\times H^{A}\rightarrow D))\rightarrow D\\
{\color{greenunder}\text{convert to curried function}:}\quad & \cong\forall D.\,(\gunderline{\forall A.\,((A\rightarrow Z)\rightarrow H^{A}\rightarrow D)})\rightarrow D\\
{\color{greenunder}\text{contravariant Yoneda identity}:}\quad & \forall D.\,(H^{Z}\rightarrow D)\rightarrow D\cong H^{Z}\quad.
\end{align*}
$\square$

One application of the co-Yoneda identities in this book is for demonstrating
that the free monad construction \lstinline!Free1! is equivalent
to \lstinline!Free2! when \lstinline!F! is a functor. Write the
type expressions of \lstinline!Free1! and \lstinline!Free2!:
\begin{align*}
...
\end{align*}
If $F$ is a functor, we may use the covariant co-Yoneda identity
(Statement~\ref{subsec:Statement-covariant-co-yoneda-identity-for-types})
setting $H=F$ and $Z=\text{Free}_{2}^{F,T}$. Then we obtain:
\[
\text{Free}_{2}^{F,T}\cong T+H^{Z}=F^{\text{Free}_{2}^{F,T}}\quad.
\]
This type expression is the same as the definition of $\text{Free}_{1}^{F,T}$.

We have already shown the equivalence of \lstinline!Free2!, \lstinline!Free3!,
and \lstinline!Free4! after running the free monad. It follows that
\lstinline!Free1! (which assumes that $F$ is a functor) is equivalent
to other free monad constructions.
...
some case classes in order to obtain the features of a monad. Are
there similar tricks for adding the features of an applicative functor,
