...
we obtain $\text{List}^{\bbnum 0}\cong\bbnum 1$. This is the \textsf{``}unit
monoid\textsf{''} where all operations always return the unit value. The unit
monoid is not only commutative but also obeys \emph{any} \emph{other}
monoid law that one could write.

For instance, some monoids obey the following laws:
\begin{align*}
{\color{greenunder}\text{idempotency law}:}\quad & x\oplus x=x\quad,\\
{\color{greenunder}\text{nilpotency law}:}\quad & x\oplus x=e\quad.
\end{align*}
The idempotency law is satisfied by the \lstinline!Boolean! monoids
that use the operation \lstinline!&&! or \lstinline!||!. The nilpotency
...
but the unit monoid satisfies both those laws trivially (as it does
...
laws will be largely the same for most other $T$. The following statement
...
Let $E$ be a free $P$-typeclass encoding.

\textbf{(a)} If types $T$, $U$ are such that a \emph{surjective}
function $f:T\rightarrow U$ exists then $E^{U}$ obeys all the $P$-typeclass
laws that $E^{T}$ obeys.

\textbf{(b)} If $E$ is a \emph{polynomial} functor and $E^{\text{Int}}$
...
\textbf{(b)} Given a type $T$, a law $l_{1}\times l_{2}$, and a
function $v^{:\text{Int}\rightarrow E^{T}}$, we need to prove that:
\begin{equation}
...
\end{equation}
with the assumption that, for any function $u^{:\text{Int}\rightarrow E^{\text{Int}}}$
we have:
\[
l_{1}\triangleright u^{\uparrow\text{FPR}}\bef\text{eval}_{\text{FPR}}^{E^{\text{Int}}}=l_{2}\triangleright u^{\uparrow\text{FPR}}\bef\text{eval}_{\text{FPR}}^{E^{\text{Int}}}\quad.
\]
We will prove Eq.~(\ref{eq:law-for-ET-derivation1}) if we find a
function $f:\text{Int}\rightarrow T$ such that $u\bef f^{\uparrow E}=v$
Because $E$ is a polynomial functor, any given value of type $E^{T}$
is a \textsf{``}container\textsf{''} storing at most a finite number of values of
type $T$. We treat the type $T$, the law $l_{1}\times l_{2}$, and
the function $v^{:\text{Int}\rightarrow E^{T}}$ as given. {*}{*}{*}
