...
\textbf{(a)} If Eq.~(\ref{eq:existential-via-universal}) holds then:
\begin{equation}
...
\]
where $B$ is considered to be a fixed type.
...
\]
The proof turns out to take significant work; see Statement~\ref{subsec:Statement-function-extension-rule}.
$\square$
...
typeclass. Suppose $T$ is a type that does not belong to the typeclass,
...
constructions. We will not prove that those properties hold for each
of the typeclasses, because Section~\ref{sec:Laws-of-free-constructions}
will show general proofs of those properties that apply all $P$-typeclasses
and all possible laws.
...
A functor $F$ may have several different fixpoints $T_{1}$, $T_{2}$,
etc. It means that each $T_{i}$ separately satisfies the fixpoint
equation $T\cong F^{T}$, but the $T_{i}$ are not isomorphic to each
other as types. An example is the fixpoint equation for the \lstinline!List!
data type:
\begin{equation}
...
\]
The standard data type \lstinline!List[A]! is a solution of this
fixpoint equation because the types $\text{List}^{A}$ and $\bbnum 1+A\times\text{List}^{A}$
are equivalent. But there is another solution that can be visualized
(non-rigorously) as the type of \textsf{``}infinitely\textsf{''} nested functions:
\[
\text{ListC}^{A}\triangleq\bbnum 1+\big(\bbnum 1\rightarrow A\times\text{ListC}^{A}\big)=\bbnum 1+(\bbnum 1\rightarrow A\times(\bbnum 1+(\bbnum 1\rightarrow A\times(...))))\quad.
\]
A value of type \lstinline!ListC[A]! is either empty or a function
that can be called to get each next value of type $A$. This is an
\textsf{``}on-call\textsf{''} list whose elements are computed only when needed. It
could happen that a value of type $\text{ListC}^{A}$ will \emph{never}
stop yielding new values of type $A$ if we keep requesting the next
elements of the list. Let us illustrate this possibility by Scala
code:
\begin{lstlisting}
sealed trait ListC[A]
final case class Empty[A]() extends ListC[A]
final case class Next[A](next: () => (A, ListC[A])) extends ListC[A]

def x: ListC[Int] = Next[Int](() => (123, x))
def y: ListC[Int] = Next[Int](() => (10, z))
def z: ListC[Int] = Next[Int](() => (20, y))
\end{lstlisting}
...
The types \lstinline!List[A]! and \lstinline!ListC[A]! are not equivalent:
\lstinline!List[A]! describes lists with finitely many elements but
\lstinline!ListC[A]! may also describe infinite sequences. This shows
...
In the current example, \lstinline!ListC[A]! is \textsf{``}larger\textsf{''} than
\lstinline!List[A]!. To see this formally, let us implement a function
\lstinline!toLC! that embeds values of type \lstinline!List[A]!
into the type \lstinline!ListC[A]! in the \textsf{``}natural\textsf{''} way. A reasonable
implementation is:
\begin{align*}
...
that converts \lstinline!ListC[A]! to \lstinline!List[A]!. The natural
...
\end{lstlisting}
We conclude that \lstinline!ListC[A]! is a \textsf{``}larger\textsf{''} type: it
cannot be embedded in \lstinline!List[A]!.
...
to check the compatibility law with \lstinline!fix!; the law for
\lstinline!unfix! follows automatically.
...
equation. As an example, let us verify that \lstinline!toLC! is fixpoint-preserving
with respect to the fixpoint equation~(\ref{eq:fixpoint-type-equation-for-oncall-list}).
We begin by writing the code for the two \lstinline!fix! functions:
\begin{align*}
 & \text{fix}_{1}:\bbnum 1+A\times\text{List}^{A}\rightarrow\text{List}^{A}\quad,\\
 & \text{fix}_{1}\triangleq\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\times\text{List}^{A}\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
A\times\text{List}^{A} & \bbnum 0 & h\times t\rightarrow h\times t
\end{array}\quad,\\
 & \text{fix}_{2}:\bbnum 1+A\times\text{LC}^{A}\rightarrow\text{LC}^{A}\quad,\\
 & \text{fix}_{2}\triangleq\,\begin{array}{|c||cc|}
 & \bbnum 1 & \bbnum 1\rightarrow A\times\text{LC}^{A}\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
