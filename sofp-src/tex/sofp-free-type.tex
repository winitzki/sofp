...
Functional programs sometimes implement a domain-specific language
(DSL)\index{domain-specific language (DSL)} that represents a certain
set of operations in a convenient and safe manner. Those languages
are usually small and are implemented within the host language (in
this book, that language is Scala). One design pattern for implementing
DSLs is known as the free monad. As a first motivation, the next subsection
will show how the free monad\index{free monad} emerges naturally
from elaborating a certain kind of DSL designed in the functional
programming paradigm.

Apart from the free monad, analogous \textsf{``}free typeclass\textsf{''} constructions
are known: free monoids, free functors, free applicative functors,
and so on. Those constructions are the focus of the present chapter.
...
technique, meaning that a DSL is implemented not as a separate new
language but as a library in an existing programming language (such
as Scala). This chapter only considers embedded DSLs and calls them
just \textsf{``}DSLs\textsf{''} for brevity.

We will now look at two examples: a DSL for file operations and a
DSL for complex numbers. Refining and refactoring these DSLs to make
them more powerful and safer to use, we will eventually obtain a construction
known as the \textsf{``}free monad\textsf{''}.
...
\begin{lstlisting}
val prgFile: PrgFile = Read(Path(Read(Path(Val("config.txt")))))
\end{lstlisting}
...
a \textsf{``}runner\textsf{''} function (\lstinline!runFile!):\index{runner!for free monads}
\begin{lstlisting}
...
final case class Complex(x: Double, y: Double) {
  ...
  def rotate(alpha: Double): Complex =
    this * Complex(math.cos(alpha), math.sin(alpha))
}
...
Instead of running a complex-number program directly, the DSL design
pattern tells us first to create a data structure that contains an
unevaluated expression tree describing \emph{what} needs to be computed.
Separate case classes are used for different operations as well as
for inserting a literal \lstinline!Complex! value (\lstinline!Val!):
\begin{lstlisting}
...
Although these DSLs are simple, we already get several benefits. By
...
The two DSLs defined in the previous section are \emph{not} type-safe.\index{type safety}
...
\lstinline!PrgFile[A]!, meaning a DSL program that will return a
value of type \lstinline!A! when we run it:
\begin{lstlisting}
...
\begin{lstlisting}
val prgFile: PrgFile[String] = Read(Path(Read(Path(Val("config.txt")))))
...
scala> runComplex(prgComplex3)    // The result will be incorrect!
...
is that new DSL programs will be able to compute values of any type,
not only of type \lstinline!Complex!. The new definitions of the
case classes are:
\begin{lstlisting}
...
Both of these limitations would be removed if the DSL could accept
\emph{Scala variables} and set them to values computed within the
DSL. We would then be able to add arbitrary Scala code for working
...
\begin{lstlisting}
val p: JPath = runFile(Path(Val("config.txt")))
val prg: PrgFile[String] = if (Files.exists(p)) Read(p) else Val("No file.")
\end{lstlisting}
here are two problems with this code. First, \lstinline!Read(p)!
...
in the code above. After this replacement, \lstinline!runFile(prg)!
will work. (The code of \lstinline!runFile! remains unchanged.)
...
should merely describe computations without executing them. The use
of a specific runner (\lstinline!runFile!) inside a DSL program will
also prevent us from being able to apply a different runner to the
entire DSL program later.
...
will have to call this function at the time when the entire DSL program
is run, at the step where we obtain a value for \lstinline!p!. To
make this work, we need to introduce a new DSL operation that replaces
the code \lstinline!val p = runFile(...)! in the example above. This
new operation will be called \lstinline!Bind!, meaning that it denotes
binding the value of a Scala variable to a result computed within
the DSL at the previous step. The above code that computes \lstinline!p!
and \lstinline!prg! is replaced by:
\begin{lstlisting}
...
\end{lstlisting}
The case class \lstinline!Bind! has two curried arguments: a value
of type \lstinline!PrgFile[JPath]! and a function of type \lstinline!JPath => PrgFile[String]!.
We write the definition of \lstinline!Bind! temporarily as:
\begin{lstlisting}
final case class Bind(pa: PrgFile[JPath])(f: JPath => PrgFile[String])
\end{lstlisting}
The function \lstinline!f! will need to be called by \lstinline!runFile!
when the DSL program is run. At that time, the result value obtained
by running \lstinline!prg2! will become available, and \lstinline!runFile!
will need to set the argument \lstinline!p! to that value when calling
\lstinline!f!. We will implement this logic in the new code for \lstinline!runFile!
(shown below).

In a different DSL program, we may need to use different types instead
of \lstinline!String! and \lstinline!JPath!. So, we replace those
types by type parameters \lstinline!A! and \lstinline!B!. The definition
of the case class \lstinline!Bind! becomes:
\begin{lstlisting}
final case class Bind[A, B](pa: PrgFile[B])(f: B => PrgFile[A]) extends PrgFile[A]
\end{lstlisting}

After adding \lstinline!Bind!-handling code to the runner, we get:
\begin{lstlisting}
sealed trait PrgFile[A]
final case class Val[A](a: A)             extends PrgFile[A]
...
arguments of the case class \lstinline!Bind[A, B]! are curried, the
first curried argument (of type \lstinline!PrgFile[B]!) will allow
the Scala compiler to determine the type parameter \lstinline!B!
automatically. As a syntax convention, \lstinline!Bind!\textsf{'}s second
curried argument may be written in a separate set of braces:
\begin{lstlisting}
val prg: PrgFile[String] = Bind(prg2){ p => if (Files.exists(p)) Read(Val(p)) else Val("No file.") }
\end{lstlisting}
Because the type parameter \lstinline!B! is already fixed, the argument
\lstinline!p! may be written without a type annotation such as \lstinline!(p: JPath) => ...!,
which makes the DSL easier to use. 

However, the underlying implementation becomes more complicated. Scala\textsf{'}s
pattern matching syntax is limited to the first curried argument.
...
that function may run arbitrary Scala code. That code is not limited
to DSL operations and may compute its result value (of type \lstinline!PrgFile[A]!)
using all features of Scala. But all that non-DSL Scala code is enclosed
inside a \lstinline!Bind! and will be evaluated only when the DSL
program is run:
\begin{lstlisting}
scala> runFile(prg)
...
The next observation is that the type signatures of \lstinline!Bind!
and \lstinline!Val! are similar to those of the standard \lstinline!flatMap!
and \lstinline!pure! methods of a monad. Because of that, we could
define those methods on the \lstinline!PrgFile! trait like this:
\begin{lstlisting}
sealed trait PrgFile[A] {
...
It is curious that \lstinline!flatMap!, \lstinline!map!, and \lstinline!pure!
run no DSL computations but merely create new case classes. However,
the type signatures are correct. Because \lstinline!map! and \lstinline!flatMap!
are now defined on the \lstinline!PrgFile! trait, we may write DSL
programs using Scala\textsf{'}s syntax of functor blocks (\lstinline!for!
/ \lstinline!yield!):
\begin{lstlisting}
...
\end{lstlisting}
Longer DSL programs may be composed easily from shorter ones:
\begin{lstlisting}
...
\end{lstlisting}
This simplifies the usage of the DSL since we may write, e.g., \lstinline!Read(path)!
instead of \lstinline!Read(Val(path))!. The code of \lstinline!runFile!
is also shortened:
\begin{lstlisting}
def runFile[A]: PrgFile[A] => A = {
  case Val(a)          => a
...
only when a \textsf{``}runner\textsf{''} is applied. After that, we will get the result
value of the entire DSL program.
...
\begin{lstlisting}
MonadDSL.pure(a).flatMap(f) == Bind(Val(a))(f) // This should be equal to f(a).
\end{lstlisting}
...
\end{lstlisting}
The law fails since the two data structures are not the same. But
when we apply a runner \lstinline!r! to both sides, we will get equal
results:
\begin{lstlisting}
...
This is an important benefit of the DSL design pattern.
...
be supplied separately. One may run the same \lstinline!MonadDSL!
...
is represented by its own case class.
...
we need  a case class from \lstinline!Free2! that can encode a value
of type \lstinline!F[A]! for some \lstinline!A!. The only suitable
case class is \lstinline!Bind!. We use the right identity law of
monads (\lstinline!x.flatMap(pure) == x!) to implement \lstinline!Suspend!
via \lstinline!Bind!:
\begin{lstlisting}
def free3toFree2[F[_], A]: Free3[F, A] => Free2[F, A] = {
  case Pure3(a)        => Return(a)
...
will be considered as a value of type \lstinline!MonadDSL[F, T]!,
...
class and in the parent trait. The type notation for \lstinline!Bind[F, T, A]!
alone is: 
\[
\text{Bind}^{F,T,A}\triangleq\text{MonadDSL}^{F,A}\times(A\rightarrow\text{MonadDSL}^{F,T})\quad.
\]
The use of \lstinline!Bind[F, T, A]! within the definition of \lstinline!MonadDSL!
is denoted by $\exists A.\,\text{Bind}^{F,T,A}$. Adding the type
notation for the case classes \lstinline!Val! and \lstinline!Op!,
we write the following expression for the type \lstinline!MonadDSL!:
\begin{align*}
...
The existential quantifier $\exists A$ binds the type parameter $A$
throughout the scope of the entire type expression that follows $\exists A$.
For instance:
\[
\exists A.\,P^{A}\rightarrow Q^{A}\quad\text{means the same as }\quad\exists A.\,(P^{A}\rightarrow Q^{A})\quad.
\]
...
with a specific (somehow chosen) type \lstinline!A!. However, once
a value \lstinline!q: MonadDSL[F, T]! is created, it is assumed that
no fully parametric code could somehow inspect \lstinline!q! and
determine the specific type \lstinline!A! that was used when creating
that value \lstinline!q!. No operations on \lstinline!q! can determine
whether the type \lstinline!A! inside \lstinline!q! is, say, \lstinline!Int!
or \lstinline!String!. (To be precise, it is the intended limitation
of fully parametric code that no such operations should be available.)
Fully parametric code can use the value \lstinline!q! but needs to
treat \lstinline!A! as an unknown, arbitrarily chosen type.

Scala supports two ways of defining types with existential quantifiers:
via case classes with an extra type parameter, or via a trait (or
a class) with a type member. The general case of an existential type
has the form $\exists A.\,P^{A}$, where $P$ is a type constructor.
A Scala implementation of $\exists A.\,P^{A}$ based on case classes
looks like this:
\begin{lstlisting}
sealed trait ExistsP
final case class ExistsP0[A](value: P[A]) // Assume that P is already defined.
\end{lstlisting}
An implementation based on type members is:
\begin{lstlisting}
trait ExistsPT {
  type A
  def value: P[A] // Assume that P is already defined.
}
\end{lstlisting}
...
type parameter \lstinline!A! is not visible within the outside scope:
\begin{lstlisting}
val x: ExistsP = ExistsP0[Int](...)
\end{lstlisting}

Creating a value of type \lstinline!ExistsPT! requires us to assign
...
While working with values \lstinline!y! of type \lstinline!ExistsPT!,
fully parametric code may extract the type member \lstinline!A! as
\lstinline!y.A! but must treat that type as an arbitrary and unknown
type parameter:
\begin{lstlisting}
scala> val z: y.A = 123
                    ^
...
Note that Scala type members are \emph{not} similar to types stored
...
time. In the example just shown, the value \lstinline!y! is considered
as given at run time (for instance, \lstinline!y! could be a function
argument). The Scala compiler does not and \emph{cannot} check that
the value \lstinline!123! agrees with the type \lstinline!y.A!,
where the value \lstinline!y! is given at run time.
...
as:
\begin{lstlisting}
type L[A, B, C] = ...  // Assume that L is already defined.
...
Then $\exists C.\,L^{A,B,C}$ is $\exists C.\,A\times C\times(C\rightarrow B)$.
...
the type $C$:
\begin{lstlisting}
val q: L[Int, Int] = L0[Int, Int, String](2, "abc", _.length)
\end{lstlisting}
...
type \lstinline!C! but does not inspect that type.
...
\begin{lstlisting}
scala> val x: r.C = "abc" // This should not know that actually r.C = String.
                    ^
error: type mismatch;
found   : String("abc")
required: r.C
...
properties of free monads and other constructions. With that notation,
the free monad constructions are written as:
\begin{align*}
...
\end{align*}
Generalizing from this example, we find that \textsf{``}code external to the
existential quantifier\textsf{''} means any function whose input has an existentially
quantified type. Consider such a function $f$ with type $(\exists C.\,F^{C})\rightarrow D$,
where $F^{C}$ is a type expression that depends on $C$, and $D$
is a type. The code of $f$ may use values of type $C$ as long as
the code does not depend on a specific type that $C$ stands for;
so, $f$ must treat $C$ as a \emph{type parameter}. This suggests
that $f$ must be a function of type $F^{C}\rightarrow D$ that works
in the same way for all types $C$. The types of such functions are
written as $\forall C.\,F^{C}\rightarrow D$. We conclude that the
following types should be equivalent (assuming fully parametric code):
\begin{equation}
\text{for any fixed \ensuremath{F} and }D:\quad(\exists C.\,F^{C})\rightarrow D\cong\forall C.\,(F^{C}\rightarrow D)\quad.\label{eq:existential-via-universal}
\end{equation}
...
(The type notation $\forall C.\,(F^{C}\rightarrow D)$ means the same
type as $\forall C.\,F^{C}\rightarrow D$.) In contrast, the type
$(\exists C.\,F^{C})\rightarrow D$ contains the existential quantifier
($\exists C$) \emph{inside} a function argument.
...
if in an infinite tuple: \lstinline!(f[Int], f[String], f[List[Int]], ...)!.
In this sense, the definition of \lstinline!f[A]! replaces an \textsf{``}infinite
product\textsf{''} ranging over all possible types:
\[
f:\forall A.\,F^{A}\quad,\quad\quad\forall A.\,F^{A}\overset{?}{=}F^{\text{Int}}\times F^{\text{String}}\times F^{\text{List}^{\text{Int}}}\times...
\]
This formula is not rigorous (the \textsf{``}infinite product\textsf{''} is not really
defined) and is meant here only as a heuristic inspiration. Importantly,
this formula does not express the requirement that the code of the
function \lstinline!f! must work in the same way for all types \lstinline!A!. 
...
\]
This type formula is also not rigorous and is shown only as a heuristic
inspiration; for instance, it does not describe the parametricity
restrictions on the code working with the type $\exists C.\,F^{C}$.
...
cases) a much smaller type than the product of types $F^{C}\rightarrow D$
for all possible $C$. Similarly, $\exists C.\,F^{C}$ is \emph{not}
equivalent to the co-product of $F^{C}$ for all possible $C$.

Another way to see how Eq.~(\ref{eq:existential-via-universal})
...
We will now show that the free monad construction \lstinline!Free1!
...
\end{align*}
To proceed, we use the so-called \textsf{``}covariant co-Yoneda identity\textsf{''}
(which we will prove as Statement~\ref{subsec:Statement-co-Yoneda-two-identities}a
...
is a functor) is type-equivalent to \lstinline!Free2!. Recall that
the equivalence of \lstinline!Free2!, \lstinline!Free3!, and \lstinline!Free4!
holds only after applying a runner to a free monad value; the free
monad encodings \lstinline!Free2!, \lstinline!Free3!, \lstinline!Free4!
are \emph{not} equivalent as types.
...
we find a recipe that can be generalized to many other typeclasses.
Let us now formulate this recipe more clearly.
...
So, for any other typeclass, we should translate each method into
a new case class that stores the method\textsf{'}s arguments but performs no
evaluation. In addition, we create an \lstinline!Op! case class for
wrapping values of a given type \lstinline!F!. The result is a \textsf{``}free
typeclass on \lstinline!F!\textsf{''} in the raw tree encoding.

This recipe works for typeclasses having operations in the form of
functions whose last return type is again of the same typeclass. For
instance, a semigroup \lstinline!S!\textsf{'}s \lstinline!combine! method
has the type signature \lstinline!S => S => S!, and the last return
type is again \lstinline!S!. A monad \lstinline!M!\textsf{'}s \lstinline!flatMap!
and \lstinline!pure! methods have type signatures of the form \lstinline!... => M[A]!,
where the last return type uses the same monad \lstinline!M!.

To illustrate the recipe, suppose a typeclass operation has type $A_{1}\rightarrow...\rightarrow A_{n}\rightarrow T$,
where $T$ is the type of the typeclass member and $A_{1}$, ...,
$A_{n}$ are some type expressions (possibly depending on $T$ as
well). Then we uncurry the function type to $A_{1}\times...\times A_{n}\rightarrow T$
and define a new case class holding data of type $A_{1}\times...\times A_{n}$
like this:
\begin{lstlisting}
sealed trait FreeTC[T]
final case class C1[T](a1: A1, ..., aN: AN) extends FreeTC[T]
\end{lstlisting}

In case of type constructors, suppose a typeclass operation has type
$A_{1}\rightarrow...\rightarrow A_{n}\rightarrow T^{A_{0}}$, where
$T$ is the type constructor that belongs to the typeclass. Then we
uncurry this type and define a case class like this:
\begin{lstlisting}
sealed trait FreeTC[T[_], A]
case class C1[T[_], A](a1: A1, ..., aN: AN) extends FreeTC[T, A0]
\end{lstlisting}
 In this way, we define a new case class for each typeclass operation.

The second step is to implement the universal runner that transforms
a free typeclass value into a value of an arbitrary other type belonging
to the same typeclass. For instance, the universal runner for the
free monad \lstinline!MonadDSL! takes any other monad $M$ and a
polymorphic function of type $\forall T.\,F^{T}\rightarrow M^{T}$
and converts a value of type \lstinline!MonadDSL[F, A]! into a result
value of type \lstinline!M[A]!. 
...
output type is a lawful typeclass member.) 

The third step is to try modifying the raw tree encoding so that some
of the typeclass laws will hold. In many cases, one can then find
a reduced encoding that uses a smaller number of case classes and
satisfies the chosen laws automatically. The code of the universal
runner needs to be revised to work with the reduced encoding.
...
of the form \lstinline!emap4(runner)! are \emph{not} themselves monad
morphisms: Those functions do satisfy the monad morphism laws, but
they are not maps between lawful monads, as $\text{Free}_{4}^{F,A}$
...
represented by \lstinline!Free4[M, A]! is converted into a value
of type $M^{A}$ by translating \lstinline!Free4!\textsf{'}s operations into
the corresponding operations of the monad $M$.
...
between them, there is a unique function (\lstinline!emap4(f)!) of
type $\text{Free}_{4}^{F,A}\rightarrow\text{Free}_{4}^{G,A}$ that
...
runner, a programmer would view that runner as incorrectly written.
...
We can now generalize the properties 1\textendash 4 to other typeclasses. 

Let $T$ be any type; denote a free typeclass instance constructed
on $T$ by $\text{Free}^{T}$ (in any encoding). We formulate the
expected properties in terms of the free evaluator (\lstinline!eval!)
...
\item For a given type $M$ that belongs to a typeclass, there is a unique
\textsf{``}free evaluator\textsf{''} function (\lstinline!eval!) of type $\text{Free}^{M}\rightarrow M$
...
\end{align*}
For a fixed $f^{:T\rightarrow M}$, the function $\text{run}\left(f\right)$
is the only function of type $\text{Free}^{T}\rightarrow M$ that
preserves the typeclass operations.
...
\end{enumerate}
It turns out that free instances exist for a range of typeclasses
that we will call $FM$\textbf{-typeclasses} below.\index{$FM$-typeclass}
The following sections will show various free typeclass constructions.
We will not prove separately for each of the free constructions that
the listed properties hold. The properties of all $FM$-typeclasses
are similar and can be treated abstractly, once the required techniques
are developed. Section~\ref{sec:Laws-of-free-constructions} will
show more general proofs that apply to all $FM$-typeclasses and to
all possible choices of typeclass laws.
...
as functions. The \lstinline!HasDefault! typeclass for a pointed
type \lstinline!P! has only one operation: obtaining a value \lstinline!default!
of type \lstinline!P!. This operation is equivalent to a function
of type $\bbnum 1\rightarrow P$. The free typeclass will model this
operation via a case class containing a value of a named unit type.
...
and \lstinline!flatten! express the basic properties of the free
...
semigroup program\textsf{''} (a value of type \lstinline!FSR[T]!) represents
...
and $1+(2+3)$ are different as trees (even though the results will
be equal after evaluation).
...
\begin{lstlisting}
def runnerFSR[S: Semigroup, T](runT: T => S): FSR[T] => S = {
  case Combine(left, right)   => runnerFSR(runT)(left) |+| runnerFSR(runT)(right)
...
aggregating tree-like data (Section~\ref{subsec:Aggregating-tree-like-data-bfs}).
...
property of free typeclass constructions that there is only one semigroup-preserving
...
still holds. But none of those alternative implementations would preserve
...
To simplify the code, we will store functions in a sequence of type
\lstinline!Vector[Any => Any]!. Type safety will be guaranteed when
constructing values of type \lstinline!SafeFunction!. The choice
of the \lstinline!Vector! container type allows us to implement efficient
...
\subsection{Church encodings of free typeclasses and the \textquotedblleft final
tagless\textquotedblright{} idiom}
...
\subsubsection{Exercise \label{subsec:Exercise-reduced-monoid-encoding-one-law-1}\ref{subsec:Exercise-reduced-monoid-encoding-one-law-1}\index{exercises}}

Implement a reduced encoding of the free monoid that satisfies:

\textbf{(a)} The right identity law and the associativity law, but
no other laws.

\textbf{(b)} Only the right identity law but no other monoid laws.

\subsubsection{Exercise \label{subsec:Exercise-reduced-monoid-encoding-one-law-2}\ref{subsec:Exercise-reduced-monoid-encoding-one-law-2}}

Consider two reduced encodings of the free monoid: one that satisfies
only the left identity law, another that satisfies only the right
identity law (see Exercise~\ref{subsec:Exercise-reduced-monoid-encoding-one-law-1}(b)
as a starting point). Find some examples of mappings between these
encodings and show that these mappings are not injective and do not
preserve the monoid operations.

\subsubsection{Exercise \label{subsec:Exercise-reduced-monoid-encoding-one-law-3}\ref{subsec:Exercise-reduced-monoid-encoding-one-law-3}}

Use a sorted list to implement a free monoid encoding for \emph{commutative}
monoids (also satisfying all other monoid laws). 

\subsubsection{Exercise \label{subsec:Exercise-reduced-monoid-encoding-one-law-4}\ref{subsec:Exercise-reduced-monoid-encoding-one-law-4}}

Find a free semigroup encoding that is commutative but not associative. 

\subsection{Slides (to be rewritten)}
...
A list of \emph{any} number of vector pairs $\sum_{i}\mathbf{u}_{i}\otimes\mathbf{v}_{i}$
