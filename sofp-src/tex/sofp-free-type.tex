...
It is sometimes convenient to use another, equivalent formula that
expresses $\exists C$ via $\forall C$:
...
is \emph{not} equivalent to $\exists C.\,F^{C}$.
...
\section{Expected properties of a free typeclass construction\label{sec:Expected-properties-of-free-typeclass}}
...
method, we get, for any $x^{:A}$:
\[
e^{A}\big(\text{pu}_{\text{Free}_{4}^{F,\bullet}}(x)\big)=\text{pu}_{\text{Free}_{4}^{G,\bullet}}(x)\quad.
\]
...
in Chapter~\ref{chap:Semimonads-and-monads}. So, the laws of monad
morphisms express the property of preserving the operations of the
\lstinline!Monad! typeclass. (Strictly speaking, the functions of
the form \lstinline!emap4(runner)! cannot be called monad morphisms:
$\text{Free}_{4}^{F,A}$ is not a monad as it fails to obey the monad
laws.) 
...
once we develop the necessary technique. For now, let us comment on
the practical significance of the items 1\textendash 4 listed above.

Programmers use the runner to convert a free monad program (a value
...
the monad operations, the programmer is justified to reason about
the free program in the same way as about a monad program whose effects
...
to transform one free monad into another whenever needed by using
the function \lstinline!emap4! . The function \lstinline!emap4!
is similar to \lstinline!fmap! for ordinary functors except that
the argument of \lstinline!emap4! is a function of type $\forall A.\,F^{A}\rightarrow G^{A}$
with the quantifier $\forall A$ \emph{inside}.
...
free encodings that violate some of the laws. The programmer may refactor
free monadic programs as if they obeyed the monad laws. All violations
of the monad laws will become invisible once a runner is applied.

This happens precisely because the runner maps a free monad\textsf{'}s operations
into operations of a specific, lawful monad. Two sides of a monad
law that is violated in \lstinline!Free4! are two expressions that
the runner maps into the corresponding two expressions involving $M$\textsf{'}s
operations; that is, two values of types $M^{A}$ for some type $A$.
But those values of type $M^{A}$ will be equal because $M$ obeys
the law. More examples illustrating this property are in Sections~\ref{subsec:Free-semigroup}\textendash \ref{subsec:Free-monoids}.

\section{Free constructions for other typeclasses}

The free monad was motivated by implementing a type-safe DSL that
supports user-defined bound variables. However, we may view the free
monad as just a trick that wraps an arbitrary type constructor in
some case classes in order to add the features of a monad. Are there
similar tricks for adding the features of an applicative functor,
a filterable functor, etc., to an arbitrary type constructor? 
...
So, we generalize this to any other typeclass: each of the methods
is translated to a new case class that stores the method\textsf{'}s arguments
and performs no evaluation. In addition, there is an \lstinline!Op!
case class for wrapping the custom operations. 
...
typeclass. (However, the laws will hold after applying a runner.) 

The third step is to impose some of the typeclass laws and to obtain
...
We can also generalize the properties listed in Section~\ref{sec:Expected-properties-of-free-typeclass}
to an arbitrary typeclass. Suppose $T$ is a type that does not belong
to the typeclass, and denote the free typeclass instance constructed
on $T$ by $\text{Free}^{T}$. We will formulate the expected properties
in terms of the free evaluator (\lstinline!eval!) as its type signature
is simpler than that of the universal runner:
\begin{enumerate}
...
\end{enumerate}
The following sections will show examples of various free typeclass
constructions. We will not prove that those properties hold, because
Section~\ref{sec:Laws-of-free-constructions} will show general proofs
that hold for all relevant typeclasses and any possible laws.
...
Let us implement the free evaluator for the free pointed class. The
free evaluator is a function of type \lstinline!FreeDefault[P] => P!,
...
\end{lstlisting}
So, we can view \lstinline!getOrElse! as a form of the free evaluator
for the free pointed typeclass.

What if the type $T$ is already pointed? The free pointed type on
$T$ is $\bbnum 1+T$, which is never equivalent to $T$. However,
the free evaluator can be applied with the type $T$ and gives a function
...
\end{lstlisting}
The associativity law is violated in the raw tree encoding:
\begin{lstlisting}
...
is to be expected because the unevaluated expression trees $(1+2)+3$
...
This shows that \lstinline!NEL[T]! is a \textsf{``}reduced encoding\textsf{''} of
the free semigroup on \lstinline!T!. This encoding produces a semigroup
that satisfies the associativity law automatically.
...
run operation could be implemented with tail recursion while \lstinline!FSR[T]!
is a binary tree whose traversal cannot be tail-recursive.

{*}{*}{*}Let us also consider how we could map \lstinline!FSR[T]!
to \lstinline!NEL[T]!. The code can be written as:{*}{*}{*}Actually
this is the only such function that preserves the semigroup operation.{*}{*}{*} 

{*}{*}{*}What if we interpret $\text{FS}^{X}$ into \emph{another}
free semigroup?
...
map} of the $F$-algebra. \index{functor algebra!structure map} All
$F$-algebras form a category whose objects are pairs $\left(M,p_{M}\right)$
...
or \textsf{``}the laws obeyed by $E^{T}$ for all $T$\textsf{''}.
...
obeys all the $P$-typeclass laws assured by $E$. The evaluator function
must satisfy:
\begin{align}
...
then $C$ must also obey that law. The type $C$ might obey \emph{more}
laws than assured by $E$, but any law assured by $E$ must also hold
...
to the typeclass. The usefulness of free $P$-typeclass constructions
...
That function is provided by the \textsf{``}universal runner\textsf{''} ($\text{run}_{E}^{T,C}$),
...
The \textsf{``}universal runner\textsf{''} also has a uniqueness property, which is
proved in the next statement.
...
\textbf{(a)} If $C$ is a $P$-algebra that obeys all the laws assured
by $E$ then any $P$-algebra morphism $g:E^{T}\rightarrow C$ can
be expressed as: 
\[
...
{\color{greenunder}\text{for the right identity law}:}\quad & f_{1}(\oplus,e,x,y,z)\triangleq x\oplus e\quad,\quad\quad f_{2}(\oplus,e,x,y,z)\triangleq x\quad;\\
{\color{greenunder}\text{for the commutativity law}:}\quad & f_{1}(\oplus,e,x,y,z)\triangleq x\oplus y\quad,\quad\quad f_{2}(\oplus,e,x,y,z)\triangleq y\oplus x\quad.
\end{align*}
...
\begin{align}
{\color{greenunder}p_{T}\text{ satisfies the law }l:}\quad & \text{for all }f^{:\text{Int}\rightarrow T}\quad:\quad\big(l^{T}(p_{T},f)\big)\triangleright\pi_{1}=\big(l^{T}(p_{T},f)\big)\triangleright\pi_{2}\quad.\label{eq:P-typeclass-law}
\end{align}
...
a type $T$:
\begin{align*}
{\color{greenunder}\text{law corresponding to }l_{1}\times l_{2}:}\quad & \forall v:\text{Int}\rightarrow T\quad:\quad l_{1}\triangleright v^{\uparrow\text{FPR}}\bef\text{eval}_{\text{FPR}}^{T}=l_{2}\triangleright v^{\uparrow\text{FPR}}\bef\text{eval}_{\text{FPR}}^{T}\quad.
\end{align*}
...
or as values of type LawET shown in Eq.~(\ref{eq:law-expression-tree-type}),
...
The law for $M$ has the form of Eq.~(\ref{eq:P-typeclass-law})
with $T=M$. Suppose a violation of the law for $M$ is found using
a specific $v:\text{Int}\rightarrow M$ as:
\begin{equation}
...
shows that no law violations will be observable after the runner is
applied. We have proved this explicitly for our free monad DSL in
Section~\ref{subsec:A-first-recipe-monadic-dsl}. Now we see that
this is a general property that applies to all $P$-typeclasses. It
is safe to use a free typeclass encoding (even if it violates some
laws) as long as its runner preserves the typeclass operations and
the target type is lawful. In the next section we will study such
\textsf{``}partially lawful\textsf{''} free typeclass encodings in more detail.
...
then we obtain $\text{List}^{\bbnum 0}\cong\bbnum 1$. This is the
\textsf{``}unit monoid\textsf{''} where all operations always return the unit value.
The unit monoid is not only commutative but also satisfies \emph{any}
\emph{other} possible law that we might formulate for the monoid operation.
For instance, we might write the following additional laws:
\begin{align*}
{\color{greenunder}\text{idempotency law}:}\quad & x\oplus x=x\quad,\\
{\color{greenunder}\text{nilpotency law}:}\quad & x\oplus x=e\quad.
\end{align*}
Most monoids are neither idempotent nor nilponent. The idempotency
law is satisfied by the \lstinline!Boolean! monoids that use the
operation \lstinline!&&! or \lstinline!||!. The nilpotency law is
satisfied by the \lstinline!Boolean! monoid with the \textsf{``}xor\textsf{''} operation.
The unit monoid satisfies both those laws trivially (as it does for
all other laws) because all values of the \lstinline!Unit! type are
equal.

This example suggests that free typeclass encodings $E^{T}$ might
obey additional laws for some specific $T$, but the set of laws will
be largely the same for most types $T$. To make this intuition precise,
...
Let $E$ be a free $P$-typeclass encoding and $T$, $U$ be some
fixed types.

\textbf{(a)} If a \emph{surjective} function $f:T\rightarrow U$ exists
then $E^{U}$ obeys all the laws that $E^{T}$ obeys.

\textbf{(b)} If $\text{FRP}$ is a \emph{polynomial} functor then
$E^{U}$ obeys all the laws that $E^{\text{Int}}$ obeys.
...
by using Definition~\ref{subsec:Definition-free-P-typeclass-encoding}
directly.
...
always produce lawful typeclass instances. {*}{*}{*}

{*}{*}{*}Constructions: product, function, recursion.

\subsection{Laws for the $P$-typeclass constructions\label{subsec:Laws-for-the-P-typeclass-constructions}}

In Section~\ref{subsec:P-typeclasses} we have {*}{*}{*} It turns
out that {*}{*}{*}
...
algebra whenever $A$ is, then any fixpoint of $S$ is also an $E$-monad
algebra.
...
of the form $T\cong F^{T}$, where the type constructor $F$ is a
\textbf{pattern functor} \index{recursion pattern functor} that specifies
how the type uses itself in its recursive definition. A solution of
the type equation $T\cong F^{T}$ is a type $T$ that is equivalent
to $F^{T}$ via two isomorphisms:
\[
...
\]
We can write that equation in the form $T\cong F^{T}$ where $F^{R}\triangleq\bbnum 1+A\times R$.
Considering $A$ to be a fixed type, we find $T=\text{List}^{A}$. 

Consider a modified pattern functor:
\[
...
First, we will guess the answer. Intuitively, we imagine the type
$\text{List}^{A}$ to be an \textsf{``}infinite disjunction\textsf{''} of the form:
\begin{align*}
