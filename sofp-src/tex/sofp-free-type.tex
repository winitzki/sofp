...
several typeclasses at once, using the Church encoding and the JOC
theorem.
...
intuition precise: it turns out to be sufficient to check the laws
...
Any polynomial functor $E$ is traversable (see Section~\ref{subsec:All-polynomial-functors-are-traversable}).
...
\]
Note that the second integer in the pair is equal to the value $n$.
...
is injective\index{injective function}. Intuitively, $E^{T}$ is
\textsf{``}smaller\textsf{''} than $\text{FPR}^{T}$.
...
the encoding $E$ will be \emph{equivalent} to \lstinline!FPR!. (We
do \emph{not} require $\text{in}_{E}^{T}$ to be a $P$-algebra morphism.
Then we are able to describe free $P$-typeclass encodings $E$ that
are different from $\text{FPR}$.)
...
typeclass constructions always produce lawful typeclass instances. 
...
to impose laws via monad algebras. As we have seen in the previous
section, a type $T$ is a lawful member of a $P$-typeclass if $T$
is an $E$-monad algebra, where the monad $E$ is the free $P$-typeclass
encoding that assures the laws of the typeclass. Using this formulation
of laws, it is sufficient to prove that a new derived type is again
an $E$-monad algebra.
