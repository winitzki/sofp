...
An example where it is impossible to implement tail recursion is the
\textsf{``}merge-sort\textsf{''} algorithm where the function body must contain two
recursive calls within a single expression. (It is impossible to rewrite
\emph{two} recursive calls as \emph{one} tail call.) 
...
\end{lstlisting}
An alternative, shorter implementation of the same function can be
written using the method \lstinline!groupBy! like this:
\begin{lstlisting}
def countWords(s: String): Map[String, Int] =
  s.split(" ").groupBy(w => w).map { case (w, xs) => (w, xs.length) }
\end{lstlisting}
This code creates a dictionary by a single function call to \lstinline!groupBy!.
But the resulting dictionary contains word lists instead of word counts,
so we use \lstinline!map! to compute the length of each word list:
\begin{lstlisting}
...
defined by $s_{0}=SD(n)$ and $s_{k}=SD(s_{k-1})$ for $k=1$, 2,
... Here $SD(x)$ is the sum of the decimal digits of the integer
$x$, e.g., $SD(123)=6$. Stop the sequence $s_{i}$ when the numbers
begin repeating. For example, $SD(99)=18$, $SD(18)=9$, $SD(9)=9$.
So, for $n=99$, the sequence $s_{i}$ must be computed as $\left[99,18,9\right]$.
...
Denote \lstinline!f(n) = n + revDigits(n)!, where the function \lstinline!revDigits!
was defined in Exercise~\ref{subsec:ch2revdigits-Exercise-seq-7}.
Using this \lstinline!f!, write a function \lstinline!findPalindrome(n: Long): Long!
that returns the first palindrome integer found in the sequence \lstinline!n!,
\lstinline!f(n)!, \lstinline!f(f(n))!, etc. (This sequence will
often contain a palindrome integer, although it is not mathematically
guaranteed.) A sample test:
\begin{lstlisting}
...
the digits $3$, $5$, or $7$. The required type signature and a
sample test:
\begin{lstlisting}
...
sum of decimal digits. The required type signature and a sample test:
\begin{lstlisting}
...
up to 10 thousand. The required type signature and a sample test:
\begin{lstlisting}
...
We could also set $y=f(x)$ and compute $y+y=20$, with the same result.
This property is called \index{referential transparency}\textbf{referential
...
using the \lstinline!toStream! method. This recovers the correctness
of value-based reasoning because streams behave as values:
\begin{lstlisting}
scala> val iter = (1 until 10).toIterator
iter: Iterator[Int] = non-empty iterator
