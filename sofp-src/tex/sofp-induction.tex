...
or \lstinline!reduce!. Often it is better to use the standard library
functions, but sometimes the code is more transparent when using explicit
recursion. So, let us consider each of these ways in turn.
...
additions to the left. The accumulator value tracks the result of
adding a certain number of left-grouped parentheses $\left(\left(0+1\right)...\right)+1$.
...
specific computations for the base case, the inductive step, and the
post-processing).
...
scala> revSentence("A quick brown fox") // Words are separated by 1 space.
...
So, it seems clear that we should avoidte code that generates such
errors. For instance, we may apply \lstinline!max! to a sequence
only if we know that it is non-empty.
...
that situation because reasoning about program correctness becomes
hard. In most cases, programs can be rewritten to avoid the possibility
