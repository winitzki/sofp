...
such errors. For instance, we may apply \lstinline!max! only to a
sequence known to be non-empty.
...
code will throw an exception and stop running. In functional programming,
we usually want to avoid that situation because reasoning about program
correctness becomes hard. In most cases, programs can be rewritten
to avoid the possibility of match failures. An example of an unsafe
pattern matching expression is:
\begin{lstlisting}[mathescape=false]
...
i.e., cannot be changed. (What would it mean to \textsf{``}change\textsf{''} the number
$10$?)
