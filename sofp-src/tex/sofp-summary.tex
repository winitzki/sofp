...
data structure to be analyzed is \textsf{``}functors\textsf{''}.
...
that a filter function must satisfy. To aid intuition, we also examine
a few examples of functors that are not filterable (do not support
...
It is perhaps surprising that a lawful \lstinline!zip! method can
...
not preserve the semigroup\textsf{'}s methods. The converse map (from the raw
...
\subsubsection{Exercise \label{par:Exercise-additional-17-1}\ref{par:Exercise-additional-17-1}}

A \lstinline!MonadPlus! type class is defined as an additional set
of operations for monads. A monad $M$ belongs to the \lstinline!MonadPlus!
typeclass if for any type $A$ the type $M^{A}$ is a monoid. This
requires having the operations $e_{M}^{A}:M^{A}$ and $\oplus_{M}^{A}:M^{A}\rightarrow M^{A}\rightarrow M^{A}$
that are parametric in $A$.

Show that all known monad transformers preserve \lstinline!MonadPlus!:
If $L$ is one of the known monads with a standard monad transformer
$L\varangle$ as described in this book, and if $M$ is a \lstinline!MonadPlus!
instance, then $L\varangle M$ is also a \lstinline!MonadPlus! instance.
(For each kind of monad transformer, show how to define $e_{L\varangle M}^{A}$
and $\oplus_{L\varangle M}^{A}$.)

It will follow that any monad stack $K\varangle L\varangle...\varangle N$
belongs to \lstinline!MonadPlus! if the last monad in the stack ($N$)
does.

As an example, consider the \lstinline!StateT! transformer ($L^{\bullet}\triangleq\text{State}^{S,\bullet}$):
\[
(\text{State}^{S}\varangle M)^{A}=S\rightarrow M^{A\times S}\quad.
\]
If $M$ belongs to \lstinline!MonadPlus! then we define $e_{L\varangle M}^{A}$
and $\oplus_{L\varangle M}^{A}$ by:
\begin{align*}
 & e_{L\varangle M}^{A}\triangleq s^{:S}\rightarrow e_{M}^{A\times S}\quad,\\
 & \big(l^{:(L\varangle M)^{A}}\big)\oplus_{L\varangle M}^{A}\big(r^{:(L\varangle M)^{A}}\big)\triangleq s^{:S}\rightarrow\big(l(s)\big)\oplus_{M}^{A\times S}\big(r(s)\big)\quad.
\end{align*}
Show that the monoid laws hold for these operations. 

Show that suitable definitions for $e_{L\varangle M}^{A}$ and $\oplus_{L\varangle M}^{A}$
can be found for all transformer constructions listed in Chapter~\ref{chap:monad-transformers}.
