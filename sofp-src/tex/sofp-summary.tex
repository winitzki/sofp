...
A monad $M$ belongs to the \lstinline!MonadPlus! typeclass if for
any type $A$ the type $M^{A}$ is a monoid. This requires having
additional operations $e_{M}^{A}:M^{A}$ and $\oplus_{M}^{A}:M^{A}\rightarrow M^{A}\rightarrow M^{A}$
