...
have suitable properties. The following statement gives an example:
...
\]
then the functor $F^{A}\triangleq S^{A,F^{A}}$ is filterable provided
that the filtering laws hold:
\begin{align*}
...
{\color{greenunder}\text{naturality law in }R:}\quad & (g^{:Q\rightarrow R})^{\uparrow S^{A,\bullet}}\bef\text{liftE}_{S}^{A,B,R}(f)=\text{liftE}_{S}^{A,B,Q}\bef g^{\uparrow U^{B,\bullet}}\quad,
\end{align*}
where we have defined the bifunctor $U$ for convenience:
\[
...
The lifting code for $U$ is:
\[
(g^{:Q\rightarrow R})^{\uparrow U^{B,\bullet}}=\,\begin{array}{|c||cc|}
 & R & S^{B,R}\\
\hline Q & g & \bbnum 0\\
...
laws with respect to $A$, and to be inverse to each other. Then the
code of $\text{liftOpt}_{F}$ is expressed through $\text{liftE}_{S}$
like this:
\begin{align*}
...
\begin{align*}
 ...
 & =\text{unfix}^{A}\bef\text{liftE}_{S}^{A,B,F^{A}}(f\bef\text{pu}_{\text{Opt}}^{B})\bef\,\begin{array}{|c||c|}
 & F^{B}\\
\hline F^{A} & \gunderline{\overline{\text{liftOpt}_{F}}(f)}\\
S^{B,F^{A}} & \big(\gunderline{\overline{\text{liftOpt}_{F}}(f)}\big)^{\uparrow S^{B,\bullet}}\bef\text{fix}^{B}
\end{array}\quad.
\end{align*}
The naturality-identity law of \lstinline!liftE! says that the value
$\text{liftE}_{S}^{A,B,F^{A}}(f\bef\text{pu}_{\text{Opt}}^{B})$ must
be always of type $\bbnum 0+S^{B,F^{A}}$. This allows us to simplify
the result:
\begin{align*}
 & \text{liftOpt}_{F}(f^{:A\rightarrow B}\bef\text{pu}_{\text{Opt}}^{B})=\text{unfix}^{A}\bef\gunderline{f^{\uparrow S^{\bullet,F^{A}}}\bef f^{\uparrow F\uparrow S^{B,\bullet}}\bef\text{fix}^{B}}\\
