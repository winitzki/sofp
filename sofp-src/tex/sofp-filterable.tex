...
Use \lstinline!deflate! to show that the functor $F^{A}\triangleq A+A\times A\times\text{String}$
...
based on a bifunctor $S$ as the pattern functor.
...
We denote by $F\triangleq$\lstinline!List! the recursive type constructor
defined by:
\[
...
\begin{align}
 ...
 & F^{B}\\
\hline \bbnum 1 & 1^{:\bbnum 1}\rightarrow r\triangleright\overline{\text{liftOpt}_{F}}(f)\\
...
\begin{align*}
{\color{greenunder}\text{expect the l.h.s.~of Eq.~(\ref{eq:comp-liftOpt-last-diff-derivation2-1})}:}\quad & (\gunderline{f\diamond_{_{\text{Opt}}}g})\bef\,\begin{array}{||c|}
p\\
q
...
pattern functor. Then the filtering logic is different from that used
...
but will instead return an empty list ($\bbnum 1+\bbnum 0^{:B\times F^{B}}$).
...
and all the following ones. This logic is implemented by the standard
\lstinline!takeWhile! method for \lstinline!List!.

The \textsf{``}greedy filter\textsf{''} logic is not necessarily the only possible
one. Some recursive data types, such as \lstinline!List!, support
other types of filtering logic, as long as their pattern functors
have suitable properties. For an example, see Problem~\ref{subsec:Statement-filtering-recursive-type-church}.
...
\paragraph{Intuition for naturality laws}
