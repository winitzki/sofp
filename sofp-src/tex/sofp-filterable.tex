...
just below with $P=G$ and $Q^{A}=\bbnum 1$) to rewrite $F$ equivalently
in a simpler form:
\[
...
This construction uses the \textsf{``}unrolling trick\textsf{''} in a form suitable
for \lstinline!List!-like recursive definitions. Let us formulate
and prove it:

\subsubsection{Statement \label{Statement-unrolling-trick}\ref{Statement-unrolling-trick}(unrolling
a \lstinline!List!)}
...
is general and works for any bifunctor $S$ that is filterable with
respect to one of its type parameters. To see what filtering logic
it represents, let us apply Statement~\ref{subsec:Statement-filterable-recursive-type-1}
