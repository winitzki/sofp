...
Begin by writing:{*}{*}{*}remove filt$_{\text{Opt}}$ everywhere!
\begin{lstlisting}
...
a list of length $1$ containing just that value. This logic of \lstinline!pure!,
as described, certainly does not depend on the choice of the type
\lstinline!A!: it creates a one-element list for any \lstinline!A!.
It is clear that \lstinline!pure[A]! works \textsf{``}in the same way\textsf{''}
for all \lstinline!A!. Let us pause to think how one could write
an equation for \lstinline!pure! that expresses that property. We
would like to require that, for any two types \lstinline!A! and \lstinline!B!,
the function \lstinline!pure[A]! should \textsf{``}work in the same way\textsf{''}
as \lstinline!pure[B]!. But what exactly should be equal for the
types \lstinline!A! and \lstinline!B!? The result of applying \lstinline!pure[A]!
is not of the same type as when applying \lstinline!pure[B]!; so,
the value \lstinline!pure[A](x)! could not possibly be \emph{equal}
\lstinline!pure[B](y)!. Instead, the relation between the values
\lstinline!pure[A](x)! and \lstinline!pure[B](y)! could be a function
that converts \lstinline!F[A]! into \lstinline!F[B]!. A natural
example of such a conversion is \lstinline!map(f)!, where \lstinline!f: A => B!
is some function. 
