...
type $A$ by $\bbnum 0$. (The resulting type $F^{\bbnum 0}$ is then
void because $F^{\bbnum 0}\cong\bbnum 0$.)
...
def filter[F[_]: Functor, A](p: A => Boolean): F[Option[A]] => F[Option[A]] =
    _.map(_.filter(p))    // Using the standard `filter` method of Option.
\end{lstlisting}
We can always convert a value of type $F^{A}$ back into a value of
type $F^{\bbnum 1+A}$ when $F$ is a functor:
\begin{lstlisting}
...
\[
\text{deflate}_{F}^{A}:F^{\bbnum 1+A}\rightarrow F^{A}\quad,
\]
...
\end{align*}
We will often need to use the function shown in the last line, so
let us call it $\psi(p)$ or even shorter, $\psi_{p}$, for convenience:
\begin{lstlisting}
