...
The next construction is for a functor defined via a filterable pattern
functor. Then the filtering logic is different from that used in Statement~\ref{subsec:Statement-filterable-recursive-type}.
...
(with $R$ fixed), the recursive functor $F$ defined by the type
equation $F^{A}\triangleq S^{A,F^{A}}$ is filterable.
...
have suitable properties. Here is an example:

\subsubsection{Statement \label{subsec:Statement-filtering-recursive-type-church}\ref{subsec:Statement-filtering-recursive-type-church}}

If a bifunctor $S$ supports a function we call \lstinline!liftE!:
\[
\text{liftE}_{S}^{A,B,R}:(A\rightarrow\bbnum 1+B)\rightarrow S^{A,R}\rightarrow R+S^{B,R}\quad,
\]
such that that the following laws hold:
\begin{align*}
...
{\color{greenunder}\text{composition law}:}\quad & \text{liftE}_{S}^{A,B,R}(f^{:A\rightarrow\bbnum 1+B})\diamond_{_{R}}\text{liftE}_{S}^{B,C,R}(g^{:B\rightarrow\bbnum 1+C})=\text{liftE}_{S}^{A,C,R}(f\diamond_{_{\text{Opt}}}g)\quad,\\
...
\end{align*}
then the functor $F^{A}\triangleq S^{A,F^{A}}$ is filterable. Here,
the bifunctor $U$ is defined by:
\[
U^{B,R}\triangleq R+S^{B,R}\quad,
\]
and the symbol $\diamond_{_{R}}$ denotes a special composition operation
analogous to $\diamond_{_{\text{Opt}}}$:
\begin{align*}
 & (\diamond_{_{R}}):\left(A\rightarrow R+B\right)\rightarrow\left(B\rightarrow R+C\right)\rightarrow A\rightarrow R+C\quad,\\
 & p^{:A\rightarrow R+B}\diamond_{_{R}}q^{:B\rightarrow R+C}\triangleq p\bef\,\begin{array}{|c||c|}
 & R+C\\
\hline R & r^{:R}\rightarrow r+\bbnum 0^{:C}\\
B & q
\end{array}\quad.
\end{align*}
The type $R$ is assumed to be fixed throughout this definition.

The operation $\diamond_{_{R}}$ is known as a \textsf{``}Kleisli composition\textsf{''},
and we will discuss such operations in Section~\ref{subsec:Generalizing-the-laws-of-liftings-kleisli-functions}
below.

\subparagraph{Proof}

The lifting codes for $F$ and $U$ are:
\[
...
\end{array}\quad.
\]
For convenience, we denote by \lstinline!fix! and \lstinline!unfix!
two functions that convert between equivalent types $F^{A}$ and $S^{A,F^{A}}$:
\[
\text{fix}^{A}:S^{A,F^{A}}\rightarrow F^{A}\quad,\quad\quad\text{unfix}^{A}:F^{A}\rightarrow S^{A,F^{A}}\quad.
\]
These functions are assumed to be known, to satisfy the naturality
laws with respect to $A$, and to be inverse to each other. We will
use these functions to show clearly the places in the proof where
values are transformed between the isomorphic types $F^{A}$ and $S^{A,F^{A}}$.

We also define a helper function called \lstinline!mergeE!:
\begin{align*}
...
With these definitions, the code of $\text{liftOpt}_{F}$ is expressed
through $\text{liftE}_{S}$ like this:
\begin{align*}
...
\end{align*}
An equivalent formulation of $\text{liftOpt}_{F}$ is found via the
naturality law of $\text{liftE}_{S}^{A,B,R}$ with respect to $R$,
where we need to set parameters as $Q=F^{A}$, $R=F^{B}$, and $g=\overline{\text{liftOpt}_{F}}(f)$:
\[
...
\end{align}
Now we can use the naturality law of $\text{liftE}_{S}^{A,B,R}$ with
respect to $R$, where we need to set parameters as $Q=F^{A}$, $R=F^{B}$,
and $g=\overline{\text{liftOpt}_{F}}(f\bef\text{pu}_{\text{Opt}}^{B})$:
\begin{align*}
...
\end{align*}
The assumed naturality-identity law of \lstinline!liftE! with $R=F^{B}$
gives:
\[
...
\end{align*}
Putting the simplified sub-expressions together, we rewrite Eq.~(\ref{eq:liftopt-recursive-derivation2})
as:
\begin{align*}
{\color{greenunder}\text{expect to equal }f^{\uparrow F}:}\quad & \text{liftOpt}_{F}(f^{:A\rightarrow B}\bef\text{pu}_{\text{Opt}}^{B})\\
...
{\color{greenunder}\text{inductive assumption}:}\quad & =\text{unfix}^{A}\bef\gunderline{f^{\uparrow F\uparrow S^{A,\bullet}}\bef f^{\uparrow S^{\bullet,F^{B}}}}\bef\text{fix}^{B}\\
{\color{greenunder}\text{definition of }f^{\uparrow F}:}\quad & =\text{unfix}^{A}\bef\gunderline{f^{\uparrow F}\bef\text{fix}^{B}}\\
{\color{greenunder}\text{naturality law of }\text{fix}:}\quad & =\gunderline{\text{unfix}^{A}\bef\text{fix}^{A}}\bef f^{\uparrow F}=\text{id}\bef f^{\uparrow F}=f^{\uparrow F}\quad.
\end{align*}
This is now equal to the right-hand side of Eq.~(\ref{eq:combined-naturality-identity-law-of-liftOpt}).
