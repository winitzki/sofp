...
The law says that, for any \lstinline!xs: F[A]! and \lstinline!f: A => B!,
we will get the same results if we first apply \lstinline!filter!
to \lstinline!xs! and then apply \lstinline!map(f)!; or if we first
apply \lstinline!map(f)!, which converts the container type from
\lstinline!F[A]! to \lstinline!F[B]!, and then apply \lstinline!filter!
to the resulting container of type \lstinline!F[B]!. Note that \lstinline!filter()!
may remove some data items from \lstinline!xs!, while \lstinline!map(f)!
never does (it just replaces all data of type \lstinline!A! by data
of type \lstinline!B!). So, the naturality law of \lstinline!filter!
means that the filtering operation removes data items according to
the same logic regardless of the data type (\lstinline!A!, \lstinline!B!,
etc.) Later we will see many more examples of naturality laws that
express this general requirement: a transformation between some containers
should work in the same way for all types of data stored in containers.
...
a new funciton of type $A\rightarrow\bbnum 1+C$. The operation $\diamond_{_{\text{Opt}}}$
