...
same type signature as the \lstinline!filter! method. The type signatures
of \lstinline!map! and \lstinline!withFilter! methods for a type
...
require considerable work to verify directly. In this section, we
will gradually derive a shorter formulation of the laws that is easier
to understand and to verify. Throughout this section, we fix a functor
$F$ and study the properties of $F$\textsf{'}s \lstinline!filter! method
and other functions that work with types of the form \lstinline!F[A]!.
...
operation in a way that is equivalent to the \lstinline!filter! method.
...
to each value stored in a functor is something every functor can do
via its \lstinline!map! method. So, that part of the filtering operation
is available to all functors and is not relevant to deciding whether
a given functor is filterable. The important part is how some of the
values are going to be removed from \lstinline!fa!.
...
stored in \lstinline!fa! and store all the results as values of type
\lstinline!Option[A]! computed as \lstinline!None! when \lstinline!p(x) == false!
and as \lstinline!Some(x)! when \lstinline!p(x) == true!. The code
could look like this:
\begin{lstlisting}
val stage1: F[Option[A]] = fa.map { x => if (p(x)) Some(x) else None }
\end{lstlisting}
To complete the filtering operation, we need to remove some values
from \lstinline!stage1! (at least the \lstinline!None! values should
be removed). Let us assume that the removal is done \emph{somehow}
by a function called \lstinline!deflate!:
\begin{lstlisting}
...
introduce the name \lstinline!psi! for the function that computes
\lstinline!stage1! shown above. 
\begin{lstlisting}
...
so let us call it $\psi(p)$ or even shorter ($\psi_{p}$) in the
code notation:
\begin{align*}
...
composition}.\label{kleisli-composition} It applies to functions
