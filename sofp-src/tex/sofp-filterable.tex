...
We can translate this expectation into the requirement that \lstinline!result1!
and \lstinline!result2! should be equal:
...
is not implementable, the functor $F$ is not filterable. $\square$
...
The laws for filterable contrafunctors are exactly such that $F^{A}\triangleq G^{A}\rightarrow H^{A}$
...
that an empty wrapper remains empty after any filtering: the function
$\text{filt}_{H}$ satisfies, for any $p^{:A\rightarrow\bbnum 2}$,
the equation:
\begin{equation}
...
Show that a \emph{polynomial} functor $F$ is filterable (in some
way) if and only if the type $F^{\bbnum 1}\rightarrow F^{\bbnum 0}$
is not void. Find an example of a non-filterable polynomial functor
$F$ that violates the condition $F^{\bbnum 1}\rightarrow F^{\bbnum 0}\not\cong\bbnum 0$.
...
\]
We can view this as a generalized \textsf{``}lifting\textsf{''} of functions with
...
programmers may accept the work of writing these proofs twice, a mathematician
...
Write the laws of identity and composition for the (categorical) functor
...
\end{itemize}
Performing these tasks requires certain symbolic derivation techniques
