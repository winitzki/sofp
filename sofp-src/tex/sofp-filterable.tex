...
\index{unrolling trick for recursive types}\index{recursive types!unrolling trick}The
unrolling trick says that, for any functors $P$ and $Q$:
\begin{equation}
...
\end{equation}
The functor $F$ given in this example is of that form with the functors
$P^{A}\triangleq\bbnum 1+A\times A$ and $Q^{A}\triangleq A\times\left(\bbnum 1+A\right)$. 

To prove Eq.~(\ref{eq:def-recursive-functor-unrolling}), write the
definition of the \lstinline!List! functor,
\begin{align*}
...
by induction on the number of \textsf{``}unrolled\textsf{''} functors $F$.
