...
type supporting a different number of values, including \emph{zero}
...
this schematic example of a functor block program that uses the filterable
...
function must satisfy. In the code notation, \lstinline!filter! is
$\text{filt}_{F}$:
\[
...
that works with an arbitrary function $f$:
\[
...
type $F^{A}\rightarrow G^{A}$ where $F,G$ are either functors or
