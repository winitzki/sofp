...
then derive the relevant mathematical laws. This will give us a precise
definition of \textsf{``}filterable\textsf{''} type constructors \textemdash{} those
that permit useful implementations of \lstinline!filter!.
...
the embedded \lstinline!if! keyword, the type constructor must in
addition support a method called \lstinline!withFilter! with the
same type signature as \lstinline!filter!. The type signatures of
\lstinline!map! and \lstinline!withFilter! methods for a type constructor
\lstinline!F! can be written as:
\begin{lstlisting}
class F[A] {
  def map[B](f: A => B): F[B] = ...
...
For brevity, we will view \lstinline!filter! as just an alias for
\lstinline!withFilter!.

\subsection{Examples and intuitions for the filtering operation}

Examples of  filterable functors defined in the Scala library are
\lstinline!List! and \lstinline!Option!:
\begin{lstlisting}
...
the resulting data structure will only store values that pass the
filtering predicate.
...
of values of type $T$ in different parts of the disjunctive type
(e.g., $T\times T$ for $2$ values and $T\times T\times T$ for $3$
values of $T$). 

So, we expect that a filterable functor should be a disjunctive type
supporting a different number of values, including \emph{zero} values.
When the filtering operation \lstinline!_.filter(p)! is applied,
some values will fail the predicate \lstinline!p! and will be removed
from the data structure. This example:
\begin{lstlisting}
...
We cannot decide that without knowing the mathematical laws that a
...
Computations in a functor block are easier to read than chains of
method calls: we can more quickly understand the logic of the code.
But we need to make sure that the program actually works in the way
that is consistent with our intuitive expectations. Consider this
schematic example of a functor block program that uses the filterable
...
val result = for {  // Some computations in the context of the `List` functor.
  ...
  if p2(y)       // ... impose condition p2 
  z = g(x, y)    // ... compute z
  if q(x, y, z)  // ... impose condition q
...
So, we expect that the code of \lstinline!filter! satisfies the following
law:
\begin{lstlisting}
xs.map(f).filter(p) == xs.filter(f andThen p).map(f)  // For all xs, f, p.
\end{lstlisting}
This is known as the \textsf{``}naturality law\textsf{''} of \lstinline!filter!.
Let us comment briefly on the meaning of that law (a more extensive
discussion is in Section~\ref{subsec:Naturality-laws-and-natural-transformations}).
The law says that, for any \lstinline!xs: F[A]!, \lstinline!f: A => B!,
and \lstinline!p: B => Boolean!, we will get the same results if
we first apply \lstinline!map(f)! (converting the container type
from \lstinline!F[A]! to \lstinline!F[B]!) and then apply \lstinline!filter(p)!
to the resulting container of type \lstinline!F[B]!; or if we first
apply \lstinline!filter(f andThen p)! to \lstinline!xs! and then
apply \lstinline!map(f)!. Note that \lstinline!filter()! may remove
some data items from \lstinline!xs!, while \lstinline!map(f)! never
does: a call to \lstinline!map(f)! will just replace all data of
type \lstinline!A! by data of type \lstinline!B!. So, the naturality
law of \lstinline!filter! means that the filtering operation removes
data items according to the same logic regardless of the data type
(\lstinline!A!, \lstinline!B!, etc.) Later we will see many examples
of naturality laws expressing this general requirement: a transformation
should work in the same way for all types of wrapped data.

For now, we return to examining the listing above. Lines 4\textendash 5
of the listing show two filtering operations, \textsf{``}\lstinline!if p1(y)!\textsf{''}
and \textsf{``}\lstinline!if p2(y)!\textsf{''}, applied one after another. We expect
that the first filtering operation keeps only values that satisfy
the condition \lstinline!p1!, and the second filtering operation
is applied to the results of the first one, additionally imposing
the condition \lstinline!p2!. So, we expect that the result of applying
these two filtering operations is to keep only values that satisfy
both conditions. This should be the same as filtering by the single
...
law:
\begin{lstlisting}
xs.filter(p1).filter(p2) == xs.filter(x => p1(x) && p2(x))  // For all xs,p1,p2.
\end{lstlisting}
This is known as the \textsf{``}composition law\textsf{''} of \lstinline!filter!.
...
xs.filter(_ => true) == xs
\end{lstlisting}
This is known as the \textsf{``}identity law\textsf{''} of \lstinline!filter!.
...
\begin{lstlisting}
def if_p[A, B](p: A => Boolean)(f: A => B): A => B =
  x => p(x) match { case true => f(x) }
\end{lstlisting}
...
to the co-product of two \textsf{``}named unit types\textsf{''}, $\bbnum 2\cong\bbnum 1+\bbnum 1$
...
The requirement \lstinline!xs.filter(p).map(f) == xs.filter(p).map(fp)!
is the \textsf{``}partial function law\textsf{''} of \lstinline!filter!.

We found four \textsf{``}laws\textsf{''} (i.e., equations) that we expect a reasonable
\lstinline!filter! function to satisfy. In the code notation, \lstinline!filter!
is written as $\text{filt}_{F}$:
\[
\text{filt}_{F}:(A\rightarrow\bbnum 2)\rightarrow F^{A}\rightarrow F^{A}\quad.
\]
The $4$ laws (the naturality, identity, composition, and partial
function laws) are formulated using arbitrary functions $f^{:A\rightarrow B}$,
...
with the extra business rule \textbf{(a)}, we get no errors:
\begin{lstlisting}
...
checkFilteringLaws[Orders, Boolean, Boolean]()      // Tests will fail! A specific failing case for the composition law:
...
\end{lstlisting}
The partial function law also fails for rule \textbf{(b)}:
\begin{lstlisting}
...
despite filtering with the predicate \textsf{``}\lstinline!x < 1000!\textsf{''}.
This happens because the code of \lstinline!filter! does not remove
the value \lstinline!x = 2000! from the data structure in case there
is another value that passes the predicate.

The laws of filtering are a rigorous formulation of our intuitions
...
partial function law whenever the data structure $F^{A}$ stores a
value of type $A$ that does not pass the filter. However, the identity
function is the \emph{only} possible implementation of \lstinline!filter!
for certain functors $F$, e.g., the identity functor $F^{A}\triangleq A$
or the \textsf{``}reader\textsf{''} functor $F^{A}\triangleq Z\rightarrow A$. So,
...
an identity function of type \lstinline!Option[A] => Option[A]!.
...
(the co-product and the \lstinline!Unit! type) without using any
externally defined values. For this reason, it is allowed to use the
\lstinline!Boolean! type in fully parametric functions.

\subsection{Examples: Programming with filterable functors\index{examples} }
...
which may expire. When credentials expire for one server, it may copy
valid credentials from the other server. If no server has valid credentials,
the cluster is down. Is this setup described by a filterable functor?

\subparagraph{Solution}

Assume that credentials have type $A$ and that expiration is checked
by a predicate $p:A\rightarrow\bbnum 2$. We need to describe two
possibilities: either both servers have valid credentials, or the
cluster is down. The corresponding data type is $F^{A}\triangleq\bbnum 1+A\times A$.
Can we implement a lawful \lstinline!filter! function for $F$? Begin
writing code for \lstinline!filter!:
\begin{lstlisting}[numbers=left]
type F[A] = Option[(A, A)]
...
Let us recall the required business logic. If \lstinline!p(a1) == false!
then the first server\textsf{'}s credentials expired. In that case, if \lstinline!p(a2) == true!,
the first server will copy the second server\textsf{'}s valid credentials (\lstinline!a2!).
...
This derivation illustrates why fully parametric functions such as
...
It turns out that fully parametric functions always satisfy a suitably
formulated naturality law. We will see many more examples of naturality
laws in this book, and we will find that the naturality law of a method
$q$ often has the form $f^{\uparrow}\bef q=\text{<modified }q\text{>}\bef f^{\uparrow}$,
...
\end{lstlisting}
The filtering laws will still hold for these alternative implementations.
We omit the proofs.
...
Once a non-authenticated request comes in, no further requests are
...
This filtering function also removes other values that may or may
not fail the predicate, but the filtering laws do not forbid removing
...
\textbf{(b)} $F^{A}\triangleq\text{Int}+\text{Int}\times A+\text{Int}\times A\times A+\text{Int}\times A\times A\times A$.

\textbf{(c)} A non-empty list ($F^{A}=\text{NEList}^{A}$) defined
recursively as $F^{A}\triangleq A+A\times F^{A}$. 
...
def filter[T](p: T => Boolean): F[T] => F[T] = {
  ...
  case F(t, Some((t1, t2)))   =>
    F(t.filter(p), if (p(t1) && p(t2)) Some((t1, t2)) else None)
}
\end{lstlisting}
...
def filter[A](p: A => Boolean)(fa: F[A]): F[A] =
  fa.copy(johns = filterJohn(p)(johns))
\end{lstlisting}
...
\textbf{(c)} A Scala definition of \lstinline!NEList! is:
\begin{lstlisting}
...
But $Z$ is an unknown type, and we cannot create values of type $Z$
from scratch. We conclude that $F^{Z,A}$ is not filterable with respect
to $A$.
\begin{lstlisting}
...
Confucius\index{Confucius} gave a wise proverb on each of the 7 days
of a week. Sometimes the wise proverbs were hard to remember. If Confucius
...
two of the tests \emph{failed} that week?

\textbf{(b)} All data sets for a given week are retained if at least
two of the tests \emph{passed} that week?
...
$A$, where $Q$ is defined by the Scala code:\texttt{\textcolor{blue}{\footnotesize{}}}
\begin{lstlisting}
...
\textbf{(d)} The functor \lstinline!F[A] = Option[Tree2[A]]!, where
\lstinline!Tree2! was defined in Section~\ref{subsec:Binary-trees}.

\textbf{(e)} The functor \lstinline!Tree22! defined recursively by:
\[
...
filterable? Implement a \lstinline!filter! function for tree-like
functors\index{perfect-shaped tree} $R$ defined by:

\textbf{(a)} $R^{A}\triangleq\bbnum 1+A+R^{A\times A}\quad$.
...
require considerable work if we want to verify them directly for a
given implementation. In this section, we will derive shorter formulations
of those laws that are easier to understand and to verify. Throughout
this section, we fix a functor $F$ and study the properties of $F$\textsf{'}s
\lstinline!filter! method and other functions that work with types
of the form \lstinline!F[A]!.
...
The first step towards making the filtering laws simpler is to pass
from \lstinline!filter! to a function we will call \lstinline!deflate!
with type signature \lstinline!F[Option[A]] => F[A]! (in the type
notation, $F^{\bbnum 1+A}\rightarrow F^{A}$). It turns out that a
function with that type signature describes the logic of the filtering
operation in a way that is simpler but equivalent to the \lstinline!filter!
method.

To introduce \lstinline!deflate!, begin by considering a heuristic
picture of a filter operation \lstinline!fa.filter(p)! that works
with a value \lstinline!fa: F[A]! and a predicate \lstinline!p: A => Boolean!.
The partial function law of \lstinline!filter! says that, at least,
all stored values that fail the predicate $p$ should be removed.
To implement that requirement, the \lstinline!filter! function will
need to apply $p$ to the stored values. Applying a given function
to each stored value is something every functor can do via its \lstinline!map!
method. So, that part of the filtering operation is available to all
functors and is not relevant to deciding whether a given functor is
filterable. The important part is how some of the values are going
to be removed from \lstinline!fa!.

To separate out that part of the functionality of \lstinline!filter!,
let us \textsf{``}refactor\textsf{''} the filtering computation \lstinline!fa.filter(p)!
into two stages. First, we apply \lstinline!p! to all values \lstinline!x!
stored in \lstinline!fa!. However, we do not simply compute \lstinline!fa.map(p)!.
Instead, for each \lstinline!x! we compute a value of type \lstinline!Option[A]!,
which is \lstinline!None! when \lstinline!p(x) == false! and \lstinline!Some(x)!
when \lstinline!p(x) == true!. Begin writing the code:
\begin{lstlisting}
val stage1: F[Option[A]] = fa.map { x => if (p(x)) Some(x) else None }
\end{lstlisting}
After this stage, we have already replaced undesirable values \lstinline!x!
by \lstinline!None!. To complete the filtering operation, we need
to remove some values from \lstinline!stage1! (at least the \lstinline!None!
values should be removed). Let us assume that the removal is done
\emph{somehow} by a function called \lstinline!deflate!:
\begin{lstlisting}
val stage2: F[A] = deflate(stage1) // Assuming `deflate` is given:

def deflate[A]: F[Option[A]] => F[A] = ???
\end{lstlisting}
The code notation for the type signature of \lstinline!deflate! is:
\[
\text{deflate}_{F}:F^{\bbnum 1+A}\rightarrow F^{A}\quad.
\]

In this way, we have expressed the filtering operation through \lstinline!map!
and \lstinline!deflate!. To make that code more convenient, let us
introduce the name \lstinline!psi! (and the code notation $\psi$)
for the function that computes \lstinline!stage1! shown above. 
\begin{lstlisting}
def psi[A](p: A => Boolean): A => Option[A] = if (p(x)) Some(x) else None

val stage1: F[Option[A]] = fa.map(psi(p))
\end{lstlisting}
We will often need to use the function \lstinline!psi! in derivations,
so we denote it by $\psi(p)$ or (even shorter) by $\psi_{p}$ in
the code notation:
\begin{align*}
 & \psi^{A}:(A\rightarrow\bbnum 2)\rightarrow A\rightarrow\bbnum 1+A\quad,\\
 & \psi_{p}\triangleq\psi(p^{:A\rightarrow\bbnum 2})\triangleq x\rightarrow p(x)\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1\text{ (false)} & \text{id} & \bbnum 0\\
\bbnum 1\text{ (true)} & \bbnum 0 & \_\rightarrow x
\end{array}\quad.
\end{align*}
...
function \lstinline!deflate! exists. In other words, if we know nothing
about a functor $F$ and someone gives us just a function \lstinline!deflate!,
we can then implement \lstinline!filter! for $F$. 

It is also possible to derive the code for \lstinline!deflate! if
a \lstinline!filter! function is given. To implement \lstinline!deflate!,
we use \lstinline!filter! to remove the empty \lstinline!Option!
...
\end{lstlisting}
\begin{align}
 & \text{deflate}_{F}:\xymatrix{\xyScaleX{7.0pc}F^{\bbnum 1+A}\ar[r]\sp(0.5){\text{filt}_{F}(\text{nonEmpty)}} & F^{\bbnum 1+A}\ar[r]\sp(0.5){\text{get}^{\uparrow F}} & F^{A}}
\nonumber \\
 & \text{deflate}_{F}^{:F^{\bbnum 1+A}\rightarrow F^{A}}=\text{filt}_{F}(\text{nonEmpty})\bef\text{get}^{\uparrow F}\quad.\label{eq:def-deflate-via-filter}
\end{align}
...
values. As long as $F$\textsf{'}s \lstinline!filter! obeys its partial function
law, \lstinline!deflate! is a \emph{total} function when defined
...
and convert the result to $F^{A}$. We could define the \lstinline!Filterable!
...
} // Could override this default implementation of `filt` for performance.

implicit class FilterableSyntax[F[_]: Filterable, A](fa: F[Option[A]]) {
  def deflate: F[A] = implicitly[Filterable[F]].deflate(fa)
  def filter(p: A => Boolean): F[A] = implicitly[Filterable[F]].filt(p)(fa)
}
\end{lstlisting}


\subsubsection{Example \label{subsec:Example-ff-deflate-1-1}\ref{subsec:Example-ff-deflate-1-1}\index{examples}}
...
\end{lstlisting}
This code implements a \textsf{``}greedy filter\textsf{''}:\index{greedy filter}
if just one of the values in the pair $A\times A$ fails the predicate,
the filter removes all data and returns the \textsf{``}empty\textsf{''} value ($\bbnum 0^{:A\times A}+\text{id}^{:Z\rightarrow Z}$).
$\square$

For some functors, one cannot implement a fully parametric \lstinline!deflate!:

\subsubsection{Example \label{subsec:Example-ff-deflate-2}\ref{subsec:Example-ff-deflate-2}}

Show that the functor $F^{A}\triangleq A+A\times A\times\text{String}$
is \emph{not} filterable.
...
These examples show that \lstinline!deflate! is easier to reason
about than \lstinline!filter!.

Note that the type signature opposite to \lstinline!deflate! (that
is, $F^{A}\rightarrow F^{\bbnum 1+A}$) can be always implemented
for any functor $F$. Here is a function called \lstinline!inflate!:
\[
\text{inflate}_{F}:F^{A}\rightarrow F^{\bbnum 1+A}\quad,\quad\quad\text{inflate}_{F}\triangleq(a^{:A}\rightarrow\bbnum 0^{:\bbnum 1}+a)^{\uparrow F}\quad.
\]
Because of the identity law of \lstinline!deflate!, the following
equation always holds (we will show this in Example~\ref{subsec:Example-identity-law-of-deflate}
below):
\[
\text{inflate}_{F}\bef\text{deflate}_{F}=\text{id}^{:F^{A}\rightarrow F^{A}}\quad.
\]
...
are inverses of each other as long as certain laws hold.
...
as long as that obeys the partial function law.

\subparagraph{Proof}

We need to show that $\text{filt}_{F}(p)$ is the same as \lstinline!filter!$^{\prime}(p)$
for any $p^{:A\rightarrow\bbnum 2}$:
\begin{align}
...
\[
\text{nonEmpty}\triangleq(\_^{:A}\rightarrow1)^{\uparrow\text{Opt}}\quad,\quad\quad\psi_{p}(x)\triangleq x\triangleright p\bef(\_\rightarrow x)^{\uparrow\text{Opt}}\quad.
\]
...
{\color{greenunder}\text{expect to equal }x\triangleright p:}\quad & x^{:A}\triangleright\psi_{p}\bef\text{nonEmpty}\nonumber \\
 & =x\triangleright p\bef(\_\rightarrow x)^{\uparrow\text{Opt}}\bef(\_^{:A}\rightarrow1)^{\uparrow\text{Opt}}\nonumber \\
{\color{greenunder}\text{composition under }^{\uparrow\text{Opt}}:}\quad & =x\triangleright p\bef(\gunderline{\_\rightarrow1})^{\uparrow\text{Opt}}\nonumber \\
...
\end{align}
We simplify the expression $\psi_{p}\bef\text{get}$ to the partial
function $\text{id}_{|p}$ by writing the code matrices for those
functions:
\begin{align*}
...
{\color{greenunder}\text{matrix composition}:}\quad & =p(x)\triangleright\,\,\begin{array}{|c||c|}
 & A\\
\hline \bbnum 1\,(\text{false}) & \bbnum 0\\
\bbnum 1\,(\text{true}) & \_\rightarrow x
\end{array}\nonumber \\
...
\end{lstlisting}
We can now finish the derivation in Eq.~(\ref{eq:filter-prime-derivation-1}):
\begin{align*}
...
as long as the naturality law~(\ref{eq:naturality-law-of-deflate})
holds.
...
the form:
\begin{align*}
{\color{greenunder}\text{for any }f^{:A\rightarrow B}:}\quad & \text{deflate}\bef f^{\uparrow F}=(\text{???})^{\uparrow F}\bef\text{deflate}\quad.
\end{align*}
\[
...
looking to prove that \lstinline!deflate!$^{\prime}=$ \lstinline!deflate!:
\begin{align}
...
defined via \lstinline!deflate!, as long as the law~(\ref{eq:naturality-law-of-deflate})
holds.
...
into an equation illustrated by the diagram: 
\[
...
{\color{greenunder}\text{matrix composition}:}\quad & =p(x)\triangleright\,\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1\,(\text{false}) & \text{id} & \bbnum 0\\
\bbnum 1\,(\text{true}) & \bbnum 0 & \_\rightarrow f(x)
\end{array}\quad.
\end{align*}
...
are equivalent, it remains to prove the converse statement:
...
\begin{equation}
x^{:A}\triangleright\psi_{p}\triangleq x^{:A}\triangleright p^{:A\rightarrow\text{Opt}^{\bbnum 1}}\bef(\_^{:\bbnum 1}\rightarrow x)^{\uparrow\text{Opt}}\quad.\label{eq:def-of-psi}
\end{equation}
...
{\color{greenunder}\text{use Eq.~(\ref{eq:def-of-psi})}:}\quad & \quad=x\triangleright f\triangleright p\bef(\_\rightarrow x\triangleright f)^{\uparrow\text{Opt}}\\
{\color{greenunder}\triangleright\text{-notation}:}\quad & \quad=x\triangleright f\bef p\bef(\_\rightarrow x\triangleright f)^{\uparrow\text{Opt}}\quad.\\
...
{\color{greenunder}\text{composition law of }^{\uparrow\text{Opt}}:}\quad & \quad=x\triangleright f\bef p\bef\big(\gunderline{(\_\rightarrow x)\bef f}\big)^{\uparrow\text{Opt}}\\
{\color{greenunder}\text{compute composition}:}\quad & \quad=x\triangleright f\bef p\bef(\gunderline{\_\rightarrow x\triangleright f})^{\uparrow\text{Opt}}\quad.
\end{align*}
...
Begin by writing:
\begin{lstlisting}
psi(p) == { x => if (p(x)) Some(x) else None }
       == { x => if (p1(x) && p2(x)) Some(x) else None }
\end{lstlisting}
We would like to transform this code into some combination of the
functions $\psi_{p_{1}}$ and $\psi_{p_{2}}$:
\begin{lstlisting}
psi(p1) == { x => if (p1(x)) Some(x) else None }
psi(p2) == { x => if (p2(x)) Some(x) else None }
\end{lstlisting}
Consider an \lstinline!x: A! to which we apply \lstinline!psi(p1)!.
The result is a value \lstinline!psi(p1)(x)! of type \lstinline!Option[A]!.
We may apply \lstinline!map(psi(p2))! to that value and obtain:
\begin{lstlisting}
psi(p1)(x).map(psi(p2)) == if (p1(x)) {
  Some(if (p2(x)) Some(x) else None)
} else None // The type is Option[Option[A]].
\end{lstlisting}
Replace \lstinline!map! by \lstinline!flatMap! to convert the result
to the required type \lstinline!OptionA]!:
\begin{lstlisting}
psi(p1)(x).flatMap(psi(p2))
  == if (p1(x)) {
                  if (p2(x)) Some(x) else None
     } else None // The type is now Option[A].
\end{lstlisting}
 Now we notice that the code returns \lstinline!Some(x)! only if
\emph{both} \lstinline!p1(x)! and \lstinline!p2(x)! return \lstinline!true!.
We conclude that this code is equivalent to \lstinline!psi(p)(x)!:
\begin{lstlisting}
psi(p1)(x).flatMap(psi(p2)) == if (p1(x) && p2(x)) Some(x) else None
  == psi(p)(x)
\end{lstlisting}
In the code notation, \lstinline!psi(p1)(x).flatMap(psi(p2))! looks
like this:
\[
x\triangleright\psi_{p_{1}}\triangleright\text{flm}_{\text{Opt}}(\psi_{p_{2}})=x\triangleright\psi_{p_{1}}\bef\text{flm}_{\text{Opt}}(\psi_{p_{2}})\quad.
\]
...
We see that $\psi_{p}$ is expressed through a specific combination
of the functions $\psi_{p_{1}}$ and $\psi_{p_{2}}$. We will denote
that combination by the symbol $\diamond_{_{\text{Opt}}}$:
\[
\psi_{p}=\psi_{p_{1}}\diamond_{_{\text{Opt}}}\psi_{p_{2}}\triangleq x^{:A}\rightarrow x\triangleright\psi_{p_{1}}\triangleright\text{flm}_{\text{Opt}}(\psi_{p_{2}})=\psi_{p_{1}}\bef\text{flm}_{\text{Opt}}(\psi_{p_{2}})\quad.
\]
The operation $\diamond_{_{\text{Opt}}}$ is a special kind of composition
called \index{Kleisli composition!for Option@for \texttt{Option}}\textbf{Kleisli
composition}.\label{kleisli-composition} It combines two functions
of types $A\rightarrow\bbnum 1+B$ and $B\rightarrow\bbnum 1+C$ into
a new function of type $A\rightarrow\bbnum 1+C$. The operation $\diamond_{_{\text{Opt}}}$
is implemented via this Scala code:
\begin{lstlisting}
...
without having to prove laws? To build up intuition, it helps to perform
structural analysis that systematically looks for type constructions
creating new filterable functors out of existing ones while preserving
the laws. The results are summarized in Table~\ref{tab:Constructions-of-filterable-functors}.
...
\hline 
\hline 
{\footnotesize{}fixed type} & {\footnotesize{}$F^{A}\triangleq Z$} & {\footnotesize{}$Z$ is any fixed type}\tabularnewline
\hline 
{\footnotesize{}product} & {\footnotesize{}$F^{A}\triangleq P^{A}\times Q^{A}$} & {\footnotesize{}$P$ and $Q$ are filterable functors}\tabularnewline
\hline 
{\footnotesize{}co-product} & {\footnotesize{}$F^{A}\triangleq P^{A}+Q^{A}$} & {\footnotesize{}$P$ and $Q$ are filterable functors}\tabularnewline
\hline 
 & {\footnotesize{}$F^{A}\triangleq\bbnum 1+A\times G^{A}$} & {\footnotesize{}$G$ is a filterable functor }\tabularnewline
\hline 
{\footnotesize{}function type} & {\footnotesize{}$F^{A}\triangleq L^{A}\rightarrow H^{A}$} & {\footnotesize{}$H$ is a filterable functor, $L$ is a filterable
contrafunctor}\tabularnewline
\hline 
{\footnotesize{}composition} & {\footnotesize{}$F^{A}\triangleq G^{H^{A}}$} & {\footnotesize{}$H$ is a filterable functor, $G$ is any functor}\tabularnewline
\hline 
{\footnotesize{}composition} & {\footnotesize{}$F^{A}\triangleq G^{H^{A}}$} & {\footnotesize{}$H$ is a filterable contrafunctor, $G$ is any contrafunctor}\tabularnewline
\hline 
{\footnotesize{}recursive type} & {\footnotesize{}$F^{A}\triangleq G^{A}+A\times F^{A}$} & {\footnotesize{}$G$ is a filterable functor}\tabularnewline
\hline 
{\footnotesize{}recursive type} & {\footnotesize{}$F^{A}\triangleq S^{A,F^{A}}$} & {\footnotesize{}Bifunctor $S^{A,R}$ is filterable w.r.t.~$A$}\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
\caption{Constructions of filterable functors.\label{tab:Constructions-of-filterable-functors}}
\end{table}

To begin, we note that \lstinline!Option!, \lstinline!Either!, \lstinline!Try!,
\lstinline!List!, \lstinline!Seq!, \lstinline!Set!, and \lstinline!Map!
are filterable. Let us now go through all type constructions available
...
two laws~(\ref{eq:combined-naturality-identity-law-of-liftOpt}),
...
functor is filterable because we can define $\text{liftOpt}\,(f)\triangleq\text{id}^{:Z\rightarrow Z}$
...
based on a pattern bifunctor.

We begin by showing a proof of the filtering laws for \lstinline!List!.

\subsubsection{Statement \label{subsec:Statement-list-is-filterable}\ref{subsec:Statement-list-is-filterable}}

The standard \lstinline!List! functor is filterable.

\subparagraph{Proof}

We denote by $F\triangleq\,$\lstinline!List! the recursive type
constructor defined by:
\[
F^{A}\triangleq\bbnum 1+A\times F^{A}\quad.
\]

We first need to implement the function $\text{liftOpt}_{F}$ for
it:
\begin{lstlisting}
def liftOpt[A, B](f: A => Option[B]): List[A] => List[B] = {
   case Nil            => Nil
   case head :: tail   => f(head) match { // Does `head` pass the filtering predicate?
      case None      => liftOpt(f)(tail)        // No. Drop `head` and filter `tail` recursively.
      case Some(b)   => b :: liftOpt(f)(tail))  // Yes. Keep `head` and filter `tail` recursively.
   }
}
\end{lstlisting}
\begin{align*}
 ...
 & F^{B}\\
\hline \bbnum 1 & \_\rightarrow1+\bbnum 0^{:B\times F^{B}}\\
...
B & b^{:B}\rightarrow\bbnum 0^{:\bbnum 1}+b\times\overline{\text{liftOpt}_{F}}(f)(r)
\end{array}
\end{array}\quad.
\end{align*}
...
column instead of columns split by the disjunctive type $F^{A}=\bbnum 1+A\times F^{A}$.
...
The disjunctive type $F^{A}$ has two cases, $\bbnum 1+\bbnum 0^{:A\times F^{A}}$
and $\bbnum 0^{:\bbnum 1}+A\times F^{A}$. When applied to a value
$1+\bbnum 0$, the function $\text{liftOpt}_{F}$ is an identity function,
so both laws are satisfied. It remains to verify the laws when applied
to a value $\bbnum 0^{:\bbnum 1}+a^{:A}\times r^{:F^{A}}$.

To prepare for the calculations, write the result of applying $\text{liftOpt}_{F}$
to a value $\bbnum 0+a\times r$:
\begin{align}
 & (\bbnum 0^{:\bbnum 1}+a^{:A}\times r^{:F^{A}})\triangleright\text{liftOpt}_{F}(f^{:A\rightarrow\bbnum 1+B})\nonumber \\
 & =f(a)\triangleright\,\begin{array}{|c||c|}
 & F^{B}\\
\hline \bbnum 1 & \_\rightarrow r\triangleright\overline{\text{liftOpt}_{F}}(f)\\
B & b^{:B}\rightarrow\bbnum 0^{:\bbnum 1}+\left(b\times r\right)\triangleright\overline{\text{liftOpt}_{F}}(f)
\end{array}\quad.\label{eq:expression-liftOpt-derivation2-1}
\end{align}
...
{\color{greenunder}\text{use Eq.~(\ref{eq:expression-liftOpt-derivation2-1})}:}\quad & =\gunderline{a\triangleright(f\bef\text{pu}_{\text{Opt}})}\bef\,\begin{array}{||c|}
...
to $\bbnum 0+a\times r$:
\[
(\bbnum 0^{:\bbnum 1}+a^{:A}\times r^{:F^{A}})\triangleright(f^{:A\rightarrow B})^{\uparrow F}=\bbnum 0^{:\bbnum 1}+f(a)\times f^{\uparrow F}(r)=\bbnum 0+f(a)\times r\triangleright f^{\uparrow F}\quad.
\]
...
{\color{greenunder}\text{use Eq.~(\ref{eq:expression-liftOpt-derivation2-1})}:}\quad & =f(a)\triangleright\,\begin{array}{||c|}
...
\_\rightarrow r\triangleright\overline{\text{liftOpt}_{F}}(f)\,\triangleright\,\text{liftOpt}_{F}(g)\\
...
\end{array}\nonumber \\
 & \quad{\color{greenunder}\text{use Eq.~(\ref{eq:expression-liftOpt-derivation2-1})}:}\quad\nonumber \\
 & =f(a)\triangleright\,\begin{array}{||c|}
\_\rightarrow r\triangleright\overline{\text{liftOpt}_{F}}(f)\bef\text{liftOpt}_{F}(g)\\
b^{:B}\rightarrow g(b)\triangleright\,\begin{array}{||c|}
\_\rightarrow r\triangleright\overline{\text{liftOpt}_{F}}(f)\bef\overline{\text{liftOpt}_{F}}(g)\\
...
\end{array}
\end{array}\quad.\label{eq:lhs-comp-law-liftOpt-derivation2-1}
\end{align}
...
{\color{greenunder}\text{use Eq.~(\ref{eq:expression-liftOpt-derivation2-1})}:}\quad & =a\triangleright(f\diamond_{_{\text{Opt}}}g)\triangleright\,\begin{array}{||c|}
\_\rightarrow r\triangleright\overline{\text{liftOpt}_{F}}(f\diamond_{_{\text{Opt}}}g)\\
c^{:C}\rightarrow\bbnum 0+\left(c\times r\right)\triangleright\overline{\text{liftOpt}_{F}}(f\diamond_{_{\text{Opt}}}g)
\end{array}\quad.
\end{align*}
The only remaining difference compared with the last expression in
Eq.~(\ref{eq:lhs-comp-law-liftOpt-derivation2-1}) is:
\begin{equation}
f\bef\,\begin{array}{||c|}
p\\
b^{:B}\rightarrow g(b)\triangleright\,\begin{array}{||c|}
...
p\\
q
\end{array}\quad,\label{eq:comp-liftOpt-last-diff-derivation2-1}
\end{equation}
...
are some fixed functions. We can show that Eq.~(\ref{eq:comp-liftOpt-last-diff-derivation2-1})
holds for arbitrary functions $p$ and $q$ having suitable types.
Start from the right-hand side:
\begin{align*}
 & (\gunderline{f\diamond_{_{\text{Opt}}}g})\bef\,\begin{array}{||c|}
p\\
q
...
\end{array}\quad.
\end{align*}
This proves Eq.~(\ref{eq:comp-liftOpt-last-diff-derivation2-1})
and concludes the proof of Statement~\ref{subsec:Statement-filterable-recursive-type}.

This implementation preserves all values $x^{:A}$ except those that
fail the filtering predicate (i.e., when $f(x)=1+\bbnum 0$). This
is the standard filtering logic of the \lstinline!filter! method
for sequences. $\square$
...
Use the unrolling trick (Statement~\ref{Statement-unrolling-trick}
below with $P=G$ and $Q^{A}=\bbnum 1$) to rewrite $F$ equivalently
in a simpler form:
\[
F^{A}\cong G^{A}\times\text{List}^{A}\quad.
\]
This functor is filterable as a product of two filterable functors.
For reference, we show the code for the type constructor $F$ and
the function $\text{liftOpt}_{F}$:
\begin{lstlisting}
...
\end{align*}
The overline denotes recursive uses of $\text{liftOpt}_{F}$ within
its definition. $\square$

The next construction is for a recursive functor defined via a filterable
pattern bifunctor. The filtering logic is different from that used
in Statement~\ref{subsec:Statement-filterable-recursive-type}. The
assumption is that the pattern bifunctor $S^{A,R}$ is filterable
with respect to $A$. This means we have a function \lstinline!deflate!
with this type signature:
\[
\text{deflate}_{S}:S^{\bbnum 1+A,R}\rightarrow S^{A,R}\quad.
\]
To get some intuition about how this function might work, consider
the definition of \lstinline!List! via a pattern functor:
\[
\text{List}^{A}=S^{A,\text{List}^{A}}\quad,\quad\quad S^{A,R}\triangleq\bbnum 1+A\times R\quad.
\]
The \lstinline!deflate! function for $S$ will have this type:
\begin{align*}
 & \text{deflate}_{S}:\bbnum 1+(\bbnum 1+A)\times R\rightarrow\bbnum 1+A\times R\quad,\\
{\color{greenunder}\text{or equivalently}:}\quad & \text{deflate}_{S}:\bbnum 1+R+A\times R\rightarrow\bbnum 1+A\times R\quad.
\end{align*}
There is only one lawful implementation of this function:
\[
\text{deflate}_{S}\triangleq\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\times R\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
R & \_^{:R}\rightarrow1 & \bbnum 0\\
A\times R & \bbnum 0 & \text{id}
\end{array}\quad.
\]
When the value of type $A$ fails the filtering predicate, the function
returns $1+\bbnum 0$. So, filtering drops all values stored in the
remainder of the list (represented by a value of type $R$) as soon
as any list element fails the predicate. This \textsf{``}greedy\textsf{''}\index{greedy filter}
filtering logic corresponds to the standard method \lstinline!takeWhile!
for \lstinline!List!. 

The following statement generalizes this logic to arbitrary recursive
functors:
...
(with $R$ fixed) then the recursive functor $F$ defined by $F^{A}\triangleq S^{A,F^{A}}$
is filterable.

\subparagraph{Proof}

To make the proof shorter, we formulate the laws via \lstinline!liftOpt!
rather than via \lstinline!deflate!. We follow the proof of Statement~\ref{subsec:functor-Statement-functor-recursive}.
Define $\text{liftOpt}_{F}$ recursively:
\begin{align*}
...
{\color{greenunder}\text{or equivalently}:}\quad & =\text{liftOpt}_{S}(f)\bef\big(\overline{\text{liftOpt}_{F}}(f)\big)^{\uparrow S^{B,\bullet}}\quad,
\end{align*}
where $\text{liftOpt}_{S}:(A\rightarrow\bbnum 1+B)\rightarrow S^{A,R}\rightarrow S^{B,R}$
is assumed to obey the filtering laws. The lifting to $F$ is implemented
recursively by:
\[
...
\]
\[
\xymatrix{\xyScaleY{2.5pc}\xyScaleX{6.5pc}S^{B,R}\ar[r]\sp(0.5){~\text{liftOpt}_{S}(g^{:B\rightarrow\bbnum 1+C})}\ar[d]\sb(0.4){\big(p^{:R\rightarrow R^{\prime}}\big)^{\uparrow S^{B,\bullet}}} & S^{C,R}\ar[d]\sp(0.4){\big(p^{:R\rightarrow R^{\prime}}\big)^{\uparrow S^{C,\bullet}}}\\
...
The construction in Statement~\ref{subsec:Statement-filterable-recursive-type-1}
is general and works for all bifunctor $S$ that are filterable with
respect to one of type parameters. To see what filtering logic we
would obtain, let us apply Statement~\ref{subsec:Statement-filterable-recursive-type-1}
to $S^{A,R}\triangleq\bbnum 1+A\times R$, which is the pattern bifunctor
that gives a recursive definition of the standard \lstinline!List!
functor:
\[
\text{List}^{A}\cong S^{A,\text{List}^{A}}=\bbnum 1+A\times\text{List}^{A}\quad.
\]
The function $\text{liftOpt}_{S}$ is defined by:
\begin{align*}
 & \text{liftOpt}_{S}:(A\rightarrow\bbnum 1+B)\rightarrow\bbnum 1+A\times R\rightarrow\bbnum 1+B\times R\quad,\\
 & \text{liftOpt}_{S}(f^{:A\rightarrow\bbnum 1+B})\triangleq\,\begin{array}{|c||c|}
 & \bbnum 1+B\times R\\
\hline \bbnum 1 & \_\rightarrow1+\bbnum 0^{:B\times R}\\
A\times R & a^{:A}\times r^{:R}\rightarrow f(a)\triangleright\begin{array}{|c||c|}
 & \bbnum 1+B\times R\\
\hline \bbnum 1 & \_\rightarrow1+\bbnum 0^{:B\times R}\\
B & b^{:B}\rightarrow\bbnum 0^{:\bbnum 1}+b\times r
\end{array}
\end{array}\quad.
\end{align*}
Then we write the code for $\text{liftOpt}_{F}$ as:
\begin{align*}
 & \text{liftOpt}_{F}=\text{liftOpt}_{S}(f)\bef\big(\overline{\text{liftOpt}_{F}}(f)\big)^{\uparrow S^{B,\bullet}}\\
 & =\,\begin{array}{|c||c|}
 & \bbnum 1+B\times F^{B}\\
\hline \bbnum 1 & \_\rightarrow1+\bbnum 0^{:B\times F^{B}}\\
A\times F^{A} & a^{:A}\times r^{:F^{A}}\rightarrow f(a)\triangleright\begin{array}{|c||c|}
 & \bbnum 1+B\times F^{B}\\
\hline \bbnum 1 & \_\rightarrow1+\bbnum 0^{:B\times F^{B}}\\
B & b^{:B}\rightarrow\bbnum 0+b\times\overline{\text{liftOpt}_{F}}(f)(r)
\end{array}
\end{array}\quad.
\end{align*}
If at any stage in this function we get $f(a)=1+\bbnum 0$ (that is,
the predicate returns \textsf{``}$False$\textsf{''}) then the code will no longer
perform a recursive call to $\overline{\text{liftOpt}_{F}}(f)(r)$
but will instead return an empty list ($1+\bbnum 0^{:B\times F^{B}}$).
The tail of the original list ($r$) will be ignored. This corresponds
to the \textsf{``}greedy filter\textsf{''}\index{greedy filter} logic: as soon as
some data item fails the predicate, the filter removes that data item
and all the following ones. 

The \textsf{``}greedy filter\textsf{''} logic is not necessarily the only possible
one. Some recursive data types support other types of filtering logic,
as long as their pattern functors have suitable properties. For an
example, see Problem~\ref{subsec:Statement-filtering-recursive-type-church}.

The construction in Statement~\ref{subsec:Statement-filterable-recursive-type}
used the \textsf{``}unrolling trick\textsf{''} in a form suitable for \lstinline!List!-like
recursive definitions. Let us formulate and prove it:

\subsubsection{Statement \label{Statement-unrolling-trick}\ref{Statement-unrolling-trick}
(unrolling a \lstinline!List!)}

For any functors $P$ and $Q$:\index{unrolling trick for recursive types}\index{recursive types!unrolling trick}
\begin{equation}
\text{if }F\text{ is defined by}\quad F^{A}\triangleq P^{A}+Q^{A}\times F^{A}\quad\text{then}\quad F^{A}\cong P^{A}\times\text{List}^{Q^{A}}\quad.\label{eq:def-recursive-functor-unrolling}
\end{equation}


\subparagraph{Proof}

Write the definition of the \lstinline!List! functor,
\begin{align*}
...
\end{align*}
and \textsf{``}unroll\textsf{''} the definition of $F$ from Eq.~(\ref{eq:def-recursive-functor-unrolling})
$n$ times like this:
\begin{align*}
...
that is, $F^{A}\cong P^{A}\times\text{List}^{Q^{A}}$, then follows
by induction on the number of \textsf{``}unrolled\textsf{''} functors $F$. $\square$
...
\subsubsection{Example \label{subsec:Example-first-filterable-contrafunctor}\ref{subsec:Example-first-filterable-contrafunctor}\index{examples}}
...
would lose information. If \lstinline!p(a) == true! then $d$ may
return \lstinline!c(a)!. If \lstinline!p(a) == false!, the function
$d$ must return \lstinline!None!. So, the code of $d$ must be:
\begin{lstlisting}
...
functions \emph{before} filtering in contrafunctors.\index{partial function law!reverse order in contrafunctors}
...
lawful filterable contrafunctors. The results are summarized in Table~\ref{tab:Constructions-of-filterable-contrafunctors}.
...
\hline 
\hline 
{\footnotesize{}fixed type} & {\footnotesize{}$C^{A}\triangleq Z$} & {\footnotesize{}$Z$ is any fixed type}\tabularnewline
\hline 
{\footnotesize{}product} & {\footnotesize{}$C^{A}\triangleq P^{A}\times Q^{A}$} & {\footnotesize{}$P$ and $Q$ are filterable contrafunctors}\tabularnewline
\hline 
{\footnotesize{}co-product} & {\footnotesize{}$C^{A}\triangleq P^{A}+Q^{A}$} & {\footnotesize{}$P$ and $Q$ are filterable contrafunctors}\tabularnewline
\hline 
 & {\footnotesize{}$C^{A}\triangleq A\rightarrow\bbnum 1+H^{A}$} & {\footnotesize{}$H$ is a filterable contrafunctor}\tabularnewline
\hline 
{\footnotesize{}function type} & {\footnotesize{}$C^{A}\triangleq L^{A}\rightarrow H^{A}$} & {\footnotesize{}$L$ is a filterable functor, $H$ is a filterable
contrafunctor}\tabularnewline
\hline 
{\footnotesize{}composition} & {\footnotesize{}$C^{A}\triangleq G^{H^{A}}$} & {\footnotesize{}$H$ is a filterable functor,  $G$ is any contrafunctor}\tabularnewline
\hline 
{\footnotesize{}composition} & {\footnotesize{}$C^{A}\triangleq G^{H^{A}}$} & {\footnotesize{}$H$ is a filterable contrafunctor, $G$ is any functor}\tabularnewline
\hline 
{\footnotesize{}recursive type} & {\footnotesize{}$C^{A}\triangleq S^{A,C^{A}}$} & {\footnotesize{}Profunctor $S^{A,R}$ is filterable w.r.t.~$A$}\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
...
Constant contrafunctors $C^{A}\triangleq Z$ (which are at the same
time constant functors) are filterable, as we have seen in Section~\ref{subsec:Constructions-of-filterable-functors}.
...
To define a contrafunctor via type recursion, we need to use a pattern
functor $S^{A,R}$ that is contravariant in $A$ and covariant in
$R$. In other words, $S^{A,R}$ must be a profunctor\index{profunctor}\index{recursion pattern functor!being a profunctor}
...
$R$ fixed), then the contrafunctor $F$ defined recursively by $F^{A}\triangleq S^{A,F^{A}}$
is filterable.

\subparagraph{Proof}

The  contrafunctor $F$ is implemented by wrapping $S$ in a case
class:
\begin{lstlisting}
type S[A, R] = ...
...
with respect to the type parameter $A$ of $S^{A,R}$. That law is:
\[
...
Show that the functor $F^{A}\triangleq G^{A}\rightarrow A$ is \emph{not}
filterable (for any contrafunctor $G$).

\subparagraph{Solution}

Try implementing \lstinline!deflate!$~:F^{\bbnum 1+A}\rightarrow F^{A}$,
...
type $G^{A}$ does not wrap any values of $A$. So, the only hope
of filling the typed hole $\text{???}^{:A}$ is to apply the function
$p$ to an argument of type $G^{\bbnum 1+A}$. Even if we are able
to map $G^{A}\rightarrow G^{\bbnum 1+A}$ (e.g., if $G$ is filterable),
the result of applying $p$ will be a value of type $\bbnum 1+A$.
We cannot compute a value of type $A$ out of that. So, the type signature
of \lstinline!deflate! for $F$ is not implementable. We conclude
that $F$ is not filterable.
...
So, $F=R_{1}\circ L$ is filterable. $\square$
...
Suppose $G$ is a filterable functor and $K$ is any functor (not
necessarily filterable) such that $H^{A}\triangleq\bbnum 1+K^{A}$
is filterable. Prove that the functor $F\triangleq G\circ K$ is filterable.
...
if we convert the argument from type $K^{A}$ to type $\bbnum 1+K^{A}$
...
\]
 Now we are ready to write the code for $\text{liftOpt}_{F}$:
\[
...
A suitable version of the unrolling trick is shown in Statement~\ref{Statement-unrolling-trick}.
The functor $F$ given in this example is of the form used in Statement~\ref{Statement-unrolling-trick}
if we set $P^{A}\triangleq\bbnum 1+A\times A$ and $Q^{A}\triangleq A\times\left(\bbnum 1+A\right)$. 

We can now use the functor product construction for $F$ if we show
that the functor $\text{List}\circ Q$ is filterable. This does not
follow by functor composition because $Q^{A}\triangleq A\times\left(\bbnum 1+A\right)$
...
setting $G^{A}\triangleq\bbnum 1+A$, that the functor $H$ defined
by:
\[
...
\]
The only way to fill the type hole $\text{???}^{:Z}$ is applying
...
Show that the function type $F^{\bbnum 1}\rightarrow F^{\bbnum 0}$
is \emph{not} void when $F$ is a filterable functor.
...
gives a necessary but \emph{not} a sufficient condition for a functor
$F$ to be filterable.
...
$\bbnum 1+G^{A}$ and $\bbnum 1+H^{A}$ are both filterable (even
when $G$ and $H$ are by themselves not filterable).
...
of compositions that involve lifted functions. For instance, Example~\ref{subsec:filt-solved-example-5-1}
required such an interchange within this expression:
\[
\text{get}^{\downarrow C}\bef\text{filt}_{C}(\text{nonEmpty})\bef\psi_{p}^{\downarrow C}\quad.
\]
We could make progress with the solution only because we managed to
move $\psi_{p}^{\downarrow C}$ to the left of $\text{filt}_{C}$
in this expression. We were able to interchange the order of composition
due to the law~(\ref{eq:naturality-for-filter-for-contrafunctors})
that holds for an arbitrary function $f$:
\[
\text{filt}_{C}(p)\bef f^{\downarrow C}=f^{\downarrow C}\bef\text{filt}_{C}(f\bef p)\quad.
\]

In this and previous chapters, we discovered a number of laws of a
similar form: interchanging the order of composition with an arbitrary
lifted function ($f^{\uparrow F}$ or $f^{\downarrow C}$). Let us
write down some of those laws and the type signatures of functions
...
Is there a common pattern and a common intuition for these laws?

\paragraph{Motivation for naturality laws}

All these equations describe properties of different functions and
involve different types. But at an abstract level, they express the
same property: that a function with a type parameter works in the
same way for all types. As an example, write Eq.~(\ref{eq:naturality-law-of-pure})
with full type annotations:
\[
\text{for all types }A,B,\text{ for all }x^{:F^{A}},f^{:A\rightarrow B}:\quad x\triangleright\text{pu}_{F}^{A}\triangleright f^{\uparrow F}=x\triangleright f\triangleright\text{pu}_{F}^{B}\quad.
\]
In Scala:
\begin{lstlisting}
(pure[A](x)).map(f) == pure[B](f(x))  // For all x: F[A] and f: A => B.
\end{lstlisting}
To be definite, let us set \lstinline!F = List!. The function \lstinline!pure[A]!
for \lstinline!List! takes a value of type \lstinline!A! and returns
a list of length $1$ containing just that value. This logic does
not depend on the choice of the type \lstinline!A!: the function
\lstinline!pure[A]! creates a one-element list for any \lstinline!A!.
In this sense, we say that \lstinline!pure[A]! works \textsf{``}in the same
way\textsf{''} for all \lstinline!A!. 

Let us pause to think how one could write an \emph{equation} for \lstinline!pure!
that expresses that property. We would like to require that, for any
two types \lstinline!A! and \lstinline!B!, the function \lstinline!pure[A]!
should \textsf{``}work in the same way\textsf{''} as \lstinline!pure[B]!. An equation
must state that some expression on the left-hand side must be equal
to another expression on the right-hand side. What exactly should
be \emph{equal} for the types \lstinline!A! and \lstinline!B!? The
result of evaluating \lstinline!pure[A](x)! is of type \lstinline!List[A]!,
while \lstinline!pure[B](y)! will have type \lstinline!List[B]!.
So, the value \lstinline!pure[A](x)! could not possibly be \emph{equal}
to \lstinline!pure[B](y)!. We know that \lstinline!pure[A](x)! of
type \lstinline!List[A]! is built up from \lstinline!x: A! \textsf{``}in
the same way\textsf{''} as \lstinline!pure[B](y)! of type \lstinline!List[B]!
is built up from \lstinline!y: B!. So, if we replace \lstinline!x!
by \lstinline!y! (say, by computing \lstinline!y = f(x)! with some
\lstinline!f: A => B!) and then apply \lstinline!pure!, we should
obtain the same result as when we first apply \lstinline!pure! to
\lstinline!x!, get a value of type \lstinline!List[A]!, and then
replace all values of type \lstinline!A! in that list by values of
type \lstinline!B! using the function \lstinline!f!. 

In this way, we arrive at the formulation of \lstinline!pure!\textsf{'}s naturality
law as an equation: for all \lstinline!x: F[A]! and all functions
\lstinline!f: A => B!, applying \lstinline!map(f)! to \lstinline!pure(x)!
must be equal to applying \lstinline!pure! to \lstinline!f(x)!.
This is the law we write as $\text{pu}_{F}(x)\triangleright f^{\uparrow F}=\text{pu}_{F}(f(x))$
or equivalently in the way we have written it above.

Naturality laws generalize this intuition to arbitrary functions with
type signatures of the form \lstinline!F[A] => G[A]! (or, in the
type notation, $F^{A}\rightarrow G^{A}$). Such functions are called
\textsf{``}natural transformations\textsf{''}.

Now we turn to a more practical question: how to guess or derive the
form of a naturality law for a given function. 
...
This pattern covers functions with type signatures of the form $\forall A.\,F^{A}\rightarrow G^{A}$,
where $F$ and $G$ must be both functors or both contrafunctors.
Functions with those types, i.e., fully parametric functions $t^{A}:F^{A}\rightarrow G^{A}$,
are called \textbf{natural transformations} between $F$ and $G$.\index{natural transformation}
In Scala, such functions look like this:
\begin{lstlisting}
def t[A](fa: F[A]): G[A] = ...
\end{lstlisting}
...
Applying \lstinline!headOption! to \lstinline!List[Int]!, \lstinline!List[String]!,
etc., will always follow the same logic. This property is formulated
mathematically as the requirement that we may first transform a list
with a lifted function $(f^{:A\rightarrow B})^{\uparrow\text{List}}$
...
to the \lstinline!List! functor); the results must be equal. We write
...
The naturality law only expresses the property that a function works
in the same way for all data types. This is true for \lstinline!headOption!,
for \lstinline!lastOption!, and for many other functions.
...
we have already seen. All those naturality laws are captured by the
...
Note that in the equation $f^{\uparrow F}\bef t=t\bef f^{\uparrow G}$,
the two different applications of $t$ may have different type parameters.
Written with type annotations, the naturality law becomes:
\[
\text{for all }f^{:A\rightarrow B}:\quad f^{\uparrow F}\bef t^{B}=t^{A}\bef f^{\uparrow G}\quad.
\]
The law expresses the property that the transformation $t^{A}$ (in
Scala, \lstinline!t[A]!) will follow the same logic regardless of
the type assigned to the type parameter $A$. 
...
except that the order of type parameters must be swapped:
\[
...
for $\text{cpu}_{D}$ (where we need to set $C^{A}\triangleq\bbnum 1$)
...
is realized by a type signature $C^{A}\rightarrow F^{A}\rightarrow G^{A}$
...
amount of work, since every such function has one naturality law per
type parameter. Until now, we have been systematically deriving and
checking all naturality laws; but we will not keep checking those
laws in the rest of the book.

Although naturality laws hold automatically, it is important to be
...
For example, the function type $(A\rightarrow A)\rightarrow A$ is
not of the form $F^{A}\rightarrow G^{A}$ where $F$ and $G$ are
either functors or contrafunctors. So, the \textsf{``}the natural transformation
recipe\textsf{''} will not be able to produce a naturality law for functions
of that type. A more general procedure is needed, based on the parametricity
theorem; this will be shown in Section~\ref{sec:Naturality-laws-for-fully-parametric-functions}.
...
\end{align*}
This motivates the name \textsf{``}associativity law\textsf{''} for Eq.~(\ref{eq:associativity-law-of-flatMap-for-Option}).
$\square$
...
The close analogy between ordinary functions and Kleisli functions
means that any proofs of properties of ordinary liftings can be mechanically
...
laws must hold:
\[
f\bef_{_{\mathcal{C}}}\text{id}_{\mathcal{C}}=f=\text{id}_{\mathcal{C}}\bef_{_{\mathcal{C}}}f\quad,\quad\quad\big(f\bef_{_{\mathcal{C}}}g\big)\bef_{_{\mathcal{C}}}h=f\bef_{_{\mathcal{C}}}\big(g\bef_{_{\mathcal{C}}}h\big)\quad.
\]
...
is a mapping of each object $A$ in ${\cal C}$ to the corresponding
object $\mathcal{F}(A)$ in $\mathcal{D}$, as well as a mapping of
...
This gives us assurance that we have correctly formulated the relevant
laws. 

The choice of typeclass laws is not self-evident. For example, Section~\ref{subsec:Motivation-for-and-derivation-of-laws-of-filtering}
deduced the four laws of the \lstinline!filter! function from heuristic
...
The two laws of the categorical functor are general and appear time
and again in different areas of mathematics. This gives us confidence
that those laws are correctly chosen and will be useful in a wide
range of contexts. Proving that the four laws of \lstinline!filter!
...
is likely to prove useful in many applications.
...
and so see the syntax of the kind projector podían was designed to
