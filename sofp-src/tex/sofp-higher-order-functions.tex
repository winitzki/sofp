...
An ordinary, or \textsf{``}\textbf{uncurried}\textsf{''}\index{uncurried function}
...
a single argument of type \lstinline!String => String!. When an argument\textsf{'}s
type is a function type, e.g., \lstinline!String => String!, it \emph{must}
be enclosed in parentheses, as in \lstinline!(String => String) => Unit!.
...
It takes time to get used to reading this syntax. The type \lstinline!A => B => C => D!
does \emph{not} mean a function that takes a value of type \lstinline!A!
and returns a value of type \lstinline!B!. It is better to think
about it as a function taking arguments of types \lstinline!A!, \lstinline!B!,
\lstinline!C! and returning a value of type \lstinline!D!.
...
\index{curried function!saturated application}\textbf{saturated}
application. A saturated application returns a value that is not of
a function type. So, it cannot be applied to more arguments.
...
This standard mathematical convention is also adopted in functional
programming. A variable defined in a function scope (i.e., a bound
variable) will shadow any outside definitions of a variable with the
same name.
...
type signature. As an example, consider a fully parametric function
that performs partial applications\index{partial application} for
arbitrary other functions. A possible type signature is:
\begin{lstlisting}
