...
is denoted using the symbol $\vdash$ (called the \textsf{``}\textbf{turnstile}\textsf{''}):\index{$\triangleright$@$\vdash$ (turnstile) symbol}\index{turnstile (vdash) symbol@turnstile ($\vdash$) symbol}
\begin{align}
...
{\color{greenunder}\text{the function }\text{\texttt{bad2}}:}\quad & {\cal CH}(\text{\texttt{A => B}})\vdash{\cal CH}(\text{\texttt{A}})
\end{align*}
...
tuples (also called \index{product type}\textbf{product types}),
disjunctive types (also called \index{co-product type!see \textsf{``}disjunctive type\textsf{''}}\textbf{co-product
types}), function types, parameterized types, and recursive types. 

We now define a shorter notation for those types. This notation is
...
aliases}\index{type alias}) can be used to make code shorter. Type
...
\textbf{Curry-Howard correspondence}\index{Curry-Howard correspondence}.
...
\subsection{Examples: Type notation\index{examples}}
...
as the \index{Curry-Howard correspondence}Curry-Howard correspondence)
...
A simple example of a true logical formula is \textsf{``}if $\alpha$ is true
...
\subsection{Short notation for fully parametric code\label{subsec:Short-notation-for-eight-code-constructions}}
...
In fact, we can enumerate all the allowed language constructs that
may be used by fully parametric code. There are \emph{eight} code
constructs\index{eight code constructs}\index{fully parametric!code constructions}
as illustrated here:
...
argument: $f(x)$ and $x\triangleright f$ (the \index{pipe notation}\textbf{pipe
notation}).
...
To motivate this unconventional notation, consider the type inferred
...
The type notation for pattern matching is also unconventional because
...
the parts of the disjunctive type of the output.\index{pattern matching!in matrix notation}
...
constructs\index{eight code constructs}. That code combination can
be translated into a combination of logic rules, which will produce
...
correspondence}\index{Curry-Howard correspondence}.
...
Actually, the premises for the sequent $\Gamma,\alpha\vdash\alpha$
...
\]
The \textbf{turnstile}\index{$\triangleright$@$\vdash$ (turnstile) symbol}\index{turnstile (vdash) symbol@turnstile ($\vdash$) symbol}
...
functions. Let us look at two simple examples where we will perform
such derivations step by step.
...
give an algorithm (called the \textsf{``}\index{LJT algorithm}\index{algorithm LJT}LJT
algorithm\textsf{''}, see Section~\ref{app:The-LJT-algorithm} below). That
algorithm either finds that the given formula cannot be proved, or
...
The LJT algorithm\index{algorithm LJT} solves an important problem:
namely, that the logic rules in Table~\ref{tab:Proof-rules-of-constructive-and-boolean}
...
leading to an infinite loop. There is no guaranteed method of avoiding
those infinite loops; so, the LJ algorithm is not guaranteed to terminate.
This problem is solved by a modification of the LJ algorithm, called
\index{LJT algorithm}\index{algorithm LJT}LJT, first formulated
in 1992.\footnote{An often cited paper by R.~Dyckhoff\index{Roy Dyckhoff} is \texttt{\href{https://philpapers.org/rec/DYCCSC}{https://philpapers.org/rec/DYCCSC}}.
...
not always terminate, it is simpler to understand and to apply by
hand. Then we will show how to modify the LJ algorithm in order to
obtain the always-terminating LJT algorithm.

\subsubsection*{The LJ algorithm}

Figure~\ref{fig:Rules-of-the-LJ-algorithm} shows the \index{LJ algorithm}\index{algorithm LJ}LJ
algorithm\textsf{'}s axioms and derivation rules. Each rule says that the bottom
sequent will be proved if proofs are given for sequent(s) at the top.
For each possible sub-expression (conjunction $X\wedge Y$, disjunction
...
\begin{align*}
\frac{}{\Gamma,X\vdash{\color{red}X}}~(\text{Id})\qquad & \qquad\frac{}{\Gamma\vdash{\color{red}\top}}~(\text{True})\\
\frac{\Gamma,A\Rightarrow B\vdash A\quad\Gamma,B\vdash C}{\Gamma,{\color{red}A\Rightarrow B}\vdash C}~(\text{Left}\Rightarrow)\qquad & \qquad\frac{\Gamma,A\vdash B}{\Gamma\vdash{\color{red}A\Rightarrow B}}~(\text{Right}\Rightarrow)\\
\frac{\Gamma,A_{i}\vdash C}{\Gamma,{\color{red}A_{1}\wedge A_{2}}\vdash C}~(\text{Left}\wedge_{i})\qquad & \qquad\frac{\Gamma\vdash A\quad\quad\Gamma\vdash B}{\Gamma\vdash{\color{red}A\wedge B}}~(\text{Right}\wedge)\\
\frac{\Gamma,A\vdash C\quad\Gamma,B\vdash C}{\Gamma,{\color{red}A\vee B}\vdash C}~(\text{Left}\vee)\qquad & \qquad\frac{\Gamma\vdash A_{i}}{\Gamma\vdash{\color{red}A_{1}\vee A_{2}}}~(\text{Right}\vee_{i})
\end{align*}
%
\end{minipage}}
\par\end{centering}
...
{\small{}
\begin{align*}
\frac{}{\Gamma,A\vdash{\color{red}A}}~(\text{Id})\quad & \quad\text{Proof}\,(\Gamma,A\vdash A)_{\text{given }p^{:\Gamma},x^{:A}}=x\\
\frac{}{\Gamma\vdash{\color{red}\top}}~(\text{True})\quad & \quad\text{Proof}\,(\Gamma\vdash\top)_{\text{given }p^{:\Gamma}}=1\\
\frac{\Gamma,A\Rightarrow B\vdash A\quad\Gamma,B\vdash C}{\Gamma,{\color{red}A\Rightarrow B}\vdash C}~(\text{Left}\Rightarrow)\quad & \quad\text{Proof}\,(\Gamma,A\Rightarrow B\vdash C)_{\text{given }p^{:\Gamma},q^{:A\rightarrow B}}\\
...
\frac{\Gamma,A\vdash B}{\Gamma\vdash{\color{red}A\Rightarrow B}}~(\text{Right}\Rightarrow)\quad & \quad\text{Proof}\,(\Gamma\vdash A\Rightarrow B)_{\text{given }p^{:\Gamma}}\\
 & \quad\quad=x^{:A}\rightarrow\text{Proof}\,(\Gamma,A\vdash B)_{\text{given }p^{:\Gamma},x^{:A}}\\
\frac{\Gamma,A\vdash C}{\Gamma,{\color{red}A\wedge B}\vdash C}~(\text{Left}\wedge_{1})\quad & \quad\text{Proof}\,(\Gamma,A\wedge B\vdash C)_{\text{given }p^{:\Gamma},(a^{:A}\times b^{:B})}\\
...
\index{algorithm LJT|textit}\index{LJT algorithm|textit}As we have
seen, the LJ algorithm enters a loop if the rule \textsf{``}($\text{Left}\Rightarrow$)\textsf{''}
...
{\small{}
\begin{align*}
\frac{\Gamma,A,B\vdash D}{\Gamma,A,{\color{red}A\Rightarrow B}\vdash D}~(\text{Left}\Rightarrow_{A})\quad & \quad\text{Proof}\,(\Gamma,A,A\Rightarrow B\vdash D)_{\text{given }p^{:\Gamma},x^{:A},q^{:A\rightarrow B}}\\
 & \quad\quad=\text{Proof}\,(\Gamma,A,B\vdash D)_{\text{given }p,x,q(x)}\\
\frac{\Gamma,A\Rightarrow B\Rightarrow C\vdash D}{\Gamma,{\color{red}(A\wedge B)\Rightarrow C}\vdash D}~(\text{Left}\Rightarrow_{\wedge})\quad & \quad\text{Proof}\,(\Gamma,(A\wedge B)\Rightarrow C\vdash D)_{\text{given }p^{:\Gamma},q^{:A\times B\rightarrow C}}\\
...
\frac{\Gamma,A\Rightarrow C,B\Rightarrow C\vdash D}{\Gamma,{\color{red}(A\vee B)\Rightarrow C}\vdash D}~(\text{Left}\Rightarrow_{\vee})\quad & \quad\text{Proof}\,(\Gamma,(A\vee B)\Rightarrow C\vdash D)_{\text{given }p^{:\Gamma},q^{:A+B\rightarrow C}}\\
...
\frac{\Gamma,B\Rightarrow C\vdash A\Rightarrow B\quad\Gamma,C\vdash D}{\Gamma,{\color{red}(A\Rightarrow B)\Rightarrow C}\vdash D}~(\text{Left}\Rightarrow_{\Rightarrow})\quad & \quad\text{Proof}\,(\Gamma,(A\Rightarrow B)\Rightarrow C\vdash D)_{\text{given }p^{:\Gamma},q^{:\left(A\rightarrow B\right)\rightarrow C}}\\
...
measure on sequents.} This guarantees that the proof search will terminate either with
...
Logical formulas that have no proof correspond to type signatures
for which there is no fully parametric implementation. It is a remarkable
feature of functional programming that some type signatures \emph{cannot}
be implemented just because of the structure of types. As a simple
example, consider the type signature of the function \lstinline!bad2!
that we saw before:
\begin{lstlisting}
def bad2[A, B]: (A => B) => A = ???
\end{lstlisting}
In the type notation, this is written as $\forall A.\,\forall B.\,(A\rightarrow B)\rightarrow A$.
A fully parametric implementation of this type signature corresponds
to a proof of the sequent $\emptyset\vdash(\alpha\Rightarrow\beta)\Rightarrow\alpha$.
If we try to use algorithm LJ for that sequent, we find that only
one rule matches the goal: the rule \textsf{``}(Right$\Rightarrow$)\textsf{''}, which
gives us the sequent $\alpha\Rightarrow\beta\vdash\alpha$. That sequent
matches the rule \textsf{``}(Left$\Rightarrow$)\textsf{''}, which in turn tells us
to prove two further sequents ($\alpha\Rightarrow\beta\vdash\alpha$
and $\beta\vdash\alpha$). But the second of those sequents ($\beta\vdash\alpha$)
cannot be proved. Indeed, the propositions $\alpha$ and $\beta$
are arbitrary and unrelated, so we cannot use the axioms $\Gamma,X\vdash X$
or $\Gamma\vdash\top$. There are no other rules that match $\beta\vdash\alpha$.
We find that the LJ algorithm arrives at a stage where no more rules
apply but we did not finish proving our sequent. If the LJ algorithm
fails to find a proof then there \emph{is} no proof. We conclude that
the sequent $\emptyset\vdash(\alpha\Rightarrow\beta)\Rightarrow\alpha$
cannot be derived. This is a rigorous proof that the function \lstinline!bad2!
cannot be implemented via fully parametric code.

In this way, the LJ and LJT algorithms provide a method of proving
that a type signature cannot be implemented. When such proofs become
difficult, other techniques can help us arrive at that conclusion
more easily. One such technique is to use the Boolean logic, which
we consider next.
...
types in a functional language. \emph{Not all} true Boolean formulas
correspond to implementable function types. An example is the function
\lstinline!bad3! shown in Section~\ref{subsec:Motivation-and-outlook}:
\begin{lstlisting}
...
logic even though the corresponding type signature cannot be implemented.

Despite this, the Boolean logic is sometimes useful: if a proposition
is \emph{not} true in Boolean logic then the corresponding type certainly
cannot be implemented. For instance, the function \lstinline!bad2!
shown above corresponds to the Boolean formula $(\alpha\Rightarrow\beta)\Rightarrow\alpha$.
It is easy to see that this formula is not true in Boolean logic:
if we substitute $\alpha=False$ and $\beta=True$ then we obtain
$False$ as the truth value of $(\alpha\Rightarrow\beta)\Rightarrow\alpha$.
It follows immediately that $(\alpha\Rightarrow\beta)\Rightarrow\alpha$
...
\subsubsection{Example \label{subsec:ch-Example-0-times-A}\ref{subsec:ch-Example-0-times-A}\index{examples}}
...
parts:\index{disjunctive type!matrix notation}\index{matrix notation}
\begin{lstlisting}
def f1[A, B]: Either[A, B] => Either[B, A] = {
  case Left(a)    => Right(a)
  case Right(b)   => Left(b)
...
\end{array}\quad.
\]
\index{pattern matching!in matrix notation} The matrix element in
...
But this introduces an arbitrary limit on the total size of data,
...
type \lstinline!Either[A, B]!, is a \index{tagged union!see \textsf{``}disjunctive type\textsf{''}}tagged
...
\subsubsection{Example \label{subsec:ch-Example-cardinality-option-either}\ref{subsec:ch-Example-cardinality-option-either}\index{examples}}
...
\subsubsection{Example \label{subsec:ch-Example-type-identity-f}\ref{subsec:ch-Example-type-identity-f}\index{examples}}
...
The function arrow ($\rightarrow$) binds weaker than the pipe\index{pipe notation!operator precedence}
...
to view the recursive type \lstinline!List! heuristically as an \textsf{``}infinite
disjunction\textsf{''} describing lists of zero, one, two, etc., elements.
...
operation. That operation is usually defined as:
\[
\neg\alpha\triangleq(\alpha\Rightarrow False)\quad.
\]
The logical negation plays an important role in Boolean logic because,
in particular, the implication can be expressed through negation:
$(\alpha\Rightarrow\beta)$ is equivalent to $(\neg\alpha\vee\beta)$.
But this equivalence does not hold in the constructive logic. The
type corresponding to the proposition $\alpha\Rightarrow False$ is
the function type $A\rightarrow\bbnum 0$ or, in Scala, \lstinline!A => Nothing!.
Such types are rarely needed in practice.
...
\subsubsection{Example \label{subsec:Example-boolean-implication-not-constructive-1}\ref{subsec:Example-boolean-implication-not-constructive-1}}
...
Show that the Boolean identity $\left(\alpha\Rightarrow\beta\right)=\neg\alpha\vee\beta$
does not hold in constructive logic by arguing that there is no fully
...
some more examples of formulas that are \emph{not} true in Boolean
logic:
\[
...
\]
It follows that these formulas are also not true in the constructive
logic.
...
I here shown in red so these are the interesting parts of the sequence
...
this premise which I have here shown in the red in red and so for
...
that takes a G of this type shown here in red and return we need to
return a D so how do we get a deal we apply F to a function of type
A to B to C so we create that function out of G X of type a going
