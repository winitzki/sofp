...
So, the issue is to be able to implement the following type signature
...
\end{lstlisting}
But the type signature of \lstinline!q! is not the required type
signature of \lstinline!bad3!.
...
parametric code. For that reason, constructive logic gives correct
...
logic that enable us to obtain proofs of ${\cal CH}$-propositions
and to derive program code from those proofs. While that theory (known
...
def switch: Either[Int, String] => Either[String, Int] = {
...
The code notation writes the function \lstinline!switch! as the following
\textsf{``}function matrix\textsf{''}:
\[
\text{switch}^{:\text{Int}+\text{String}\rightarrow\text{String}+\text{Int}}\triangleq\,\begin{array}{|c||cc|}
 & \text{String} & \text{Int}\\
...
corresponding to the return type $\text{String}+\bbnum 0$. 
...
the first column. The second column contains a $\bbnum 0$ in row~2.
...
this section, we will limit our consideration to types with finite
cardinalities.
...
operation ($\triangleright$), so the code notation $x\rightarrow y\triangleright z$
...
not hold in the constructive logic.
...
or in Scala:
\begin{lstlisting}
def impl[A, B](f: A => B): Either[A => Nothing, B]
\end{lstlisting}


\subparagraph{Solution}

The result type of \lstinline!impl! is a disjunctive type, but the
argument type is not. The code of \lstinline!impl! cannot pattern-match
on its argument \lstinline!f! to make a decision about returning
a \lstinline!Left! or a \lstinline!Right!. So, \lstinline!impl!
...
is to return a value of type $A\rightarrow\bbnum 0$. But those values
do not exist unless $A=\bbnum 0$ (see Example~\ref{subsec:ch-Example-type-identity-A-0}).
...
So, the function \lstinline!impl! is not implementable via fully
