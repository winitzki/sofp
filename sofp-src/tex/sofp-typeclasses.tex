...
\index{Russell O\textsf{'}Connor}Assume that a functor $F$ admits a function
...
that a data type may support. Scala supports two other features for
...
$T$ and possibly other (fixed) types. We call $\text{Sig}$ the \textsf{``}method
signature\textsf{''} of the typeclass.
...
value denoted by $\text{Sig}_{\text{Monoid}}^{T}\triangleq T\times(T\times T\rightarrow T)$.
So, we say that the \lstinline!Monoid! typeclass has the method signature
$\text{Sig}_{\text{Monoid}}$.

An object-oriented interface is defined in Scala via the \lstinline!trait!
keyword. As an example, here is an interface describing data types
that can be tested for \textsf{``}being small enough\textsf{''} or for \textsf{``}being empty\textsf{''}:
\begin{lstlisting}
...
not do that. It will be convenient to assume that each method in an
OOI definition has an additional first argument denoted by Scala\textsf{'}s
\lstinline!this! keyword.

Then the type of an interface is seen as a tuple of function types
whose first argument is the type of a class that will later implement
that interface. That type is unknown within the scope of an interface
definition, and so we have to denote that type by a type parameter
$T$. All functions in an interface will have types of the form $T\times(...)\rightarrow(...)$,
with a first argument of type $T$. Using curried function types equivalent
to those type signatures, we can make $T$ a curried first argument.
Then we obtain a description of an OOI type as a tuple of types of
the form $T\rightarrow U_{1}$, $T\rightarrow U_{2}$, ..., $T\rightarrow U_{n}$,
where $U_{1}$, ..., $U_{n}$ are some type expressions that might
also use $T$. Note that $U_{i}$ correspond to the type expressions
as seen in the Scala interface definition, where the first argument
of type $T$ is not written out.

Using the standard type equivalence such as:
\[
...
\]
(We omitted the method names and only wrote the types.) Then we curry
the argument $T$ and simplify this type to:
\[
...
Note that the types $\bbnum 2$ and $\text{Int}\rightarrow\bbnum 2$
exactly correspond to the type signatures of the methods in the Scala
syntax, where the first argument (of type $T$) is not written.
...
value is the typeclass evidence). Similarly, an OOI may be described
as \textsf{``}an unknown type $T$ that has a value of type $T\rightarrow U^{T}$
...
the form $\text{Sig}^{T}=T\rightarrow U^{T}$.
...
which must produce a value of type $T$ without any previously given
values of type $T$. In some object-oriented languages, such methods
are implemented via a special feature called \textsf{``}static methods\textsf{''}.
Static methods cannot be included in an interface definition. Typeclasses
need no special features to support methods with arbitrary type signatures.

We now turn to algebraic data types\index{algebraic data types} (ADTs).
...
With the standard type equivalence such as:
\[
...
As an example to clarify this description, consider $\text{List}^{\text{Int}}$
as an ADT:
\begin{lstlisting}
...
if the Scala syntax does not show them. So, we view the type of \lstinline!NilInt!
as the function type $\bbnum 1\rightarrow\text{ListInt}$ and the
type of \lstinline!ConsInt! as the type $\text{Int}\times\text{ListInt}\rightarrow\text{ListInt}$.
...
We have achieved a description that presents every ADT as a special
case of a typeclass whose method signature has the form $\text{Sig}^{T}=P^{T}\rightarrow T$.
...
cases of typeclasses, if we only consider the type signatures of the
required methods.

At this level of abstraction, we can also show that the inheritance
of typeclasses is similar to the object-oriented inheritance of interfaces.
...
to taking products of method signatures. Object-oriented inheritance
of interfaces turns out to be exactly of the same nature. To see that,
consider a first OOI with method signature $T\rightarrow U^{T}$ that
inherits from a second OOI with method signature $T\rightarrow V^{T}$.
The first OOI may not change any type signatures that it inherited;
it may only add new methods. Recall that the interface declarations
$U^{T}$ and $V^{T}$ involve the type signatures without the hidden
first argument. If we denote by $W^{T}$ the type signatures added
by the first OOI then it follows that $U^{T}=V^{T}\times W^{T}$.
So, we find:
\[
...
or equivalently $(P^{T}\rightarrow T)\times(Q^{T}\rightarrow T)$.
So, the ADTs expressed as typeclasses with signatures $P^{T}\rightarrow T$
have the same inheritance behavior based on products of method signatures.
...
via the object-oriented inheritance mechanism. If a class $A$ inherits
an interface $B$ (or if an interface $A$ inherits an interface $B$)
then the compiler will automatically consider $A$ as a subtype of
$B$. Subtyping and inheritance are transitive: if $A$ inherits from
$B$ and $B$ inherits from $C$ then $A$ is considered automatically
to inherit also from $C$ and to be a subtype of $C$. The compiler
will construct an inheritance tree automatically. However, an inheritance
tree cannot be changed. Once it is declared that $A$ inherits from
$B$, $C$, etc.~(this needs to be declared together with the definition
of $A$), then the programmer cannot declare in another scope that
$A$ should inherit additionally from any other interface. Typeclasses
are more flexible in this respect: evidence values for typeclass membership
can be defined in any scope, not necessarily together with the declaration
of a type. Typeclasses may be implemented via OOIs, which will enable
features of inheritance and subtyping. This will make typeclass inheritance
behave like object-oriented interface inheritance, with all its benefits
and shortcomings. Typeclass inheritance may be also implemented without
using any object-oriented inheritance.
...
values and implicit functions. These features are known as \textsf{``}typeclass
derivation\textsf{''} (this works independently of and in parallel with typeclass
inheritance). The programmer can tell the compiler how to compute
(via custom code) a typeclass evidence value for, say, any pair type
\lstinline!(A, B)! whenever evidence values for \lstinline!A! and
\lstinline!B! are available. The compiler will then automatically
use that information and accept that the pair \lstinline!(A, B)!
belongs to the typeclass whenever \lstinline!A! and \lstinline!B!
do. Similarly, typeclass evidence could be derived for \lstinline!Either[A, B]!
under appropriate conditions. In this way, typeclass membership can
be decided by the compiler for a wide range of types, which reduces
boilerplate code. There are no comparable facilities for \textsf{``}interface
derivation\textsf{''} for OOIs. 
\item The compiler\textsf{'}s treatment of ADTs in Scala and in other functional
languages is radically different from the treatment of typeclasses
or interfaces. An ADT is never interpreted as just any type $T$ that
happens to have methods of type $P^{T}\rightarrow T$ (what this book
calls a $P$-typeclass). Instead, the compiler defines an ADT as the
\emph{unique} type generated by the given constructors while using
no other information. Rigorous definitions and the necessary theory
