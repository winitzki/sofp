...
\lstinline!Functor! instances for $F$ and $G$. This is called\index{typeclass!derivation}
...
Implementing typeclass derivation requires us to know which type constructions
(product types, co-product types, etc.) will produce new instances
of a given typeclass. So, the next step is to apply \index{structural analysis}
...
Existing libraries for typeclass derivation (such as \lstinline!kittens!,\footnote{\texttt{\href{https://github.com/typelevel/kittens}{https://github.com/typelevel/kittens}}}
...
required for implementing typeclass derivation libraries are beyond
the scope of this book.) Also, typeclass instances are not always
derived uniquely, as we have seen, for example, in the co-product
constructions for monoids and for pointed functors.
...
that can be tested for \textsf{``}being small enough\textsf{''} and for \textsf{``}being empty\textsf{''}:
\begin{lstlisting}
...
exactly correspond to the type signatures of the methods \lstinline!isEmpty!
and \lstinline!isSmallerThan! in the Scala syntax, where the first
argument (of type $T$) is not written.
...
But static methods cannot be included in an interface definition.
On the other hand, typeclasses need no special features to support
methods with arbitrary type signatures.
...
the type signatures without the hidden first argument. If we denote
by $W^{T}$ the type signatures \emph{added} by the first OOI then
it follows that $U^{T}=V^{T}\times W^{T}$. So, we find:
\[
...
$B$. Subtyping and inheritance are \textbf{transitive}: if $A$ inherits
from $B$ and $B$ inherits from $C$ then $A$ is considered automatically
...
tree cannot be changed. Once it is declared that $A$ inherits, say,
from interfaces $B$ and $C$ (this needs to be declared together
with the definition of $A$), then the programmer cannot declare in
another scope that $A$ should inherit additionally from any other
interface. Typeclasses are more flexible in this respect: evidence
values for typeclass membership can be defined in any scope, not necessarily
...
inheritance, with all its benefits and shortcomings. Typeclass inheritance
may be also implemented without using any object-oriented inheritance.
...
under appropriate conditions. Then the compiler will compute the typeclass
evidence values automatically for \emph{any} suitable types \lstinline!A!,
\lstinline!B!. In this way, typeclass membership can be provided
by the compiler for a wide range of types, which reduces boilerplate
code. But there are no comparable facilities for object-oriented \textsf{``}interface
derivation\textsf{''}. 
