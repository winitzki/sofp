...
\section{Typeclass derivation via structural analysis\label{sec:Typeclass-derivation-via-structural-analysis}}
...
Compiler support for typeclass derivation makes typeclasses a powerful
design pattern. Without such support, a typeclass is little more than
a collection of implicit global constants that are substituted into
functions when types match.
...
This motivates the definition of \index{$FM$-typeclass|textit} an
$FM$-\textbf{typeclass}: it is a typeclass whose instance values
have type $P^{T}\rightarrow T$. The functor $P$ is called the \index{structure functor of a typeclass}\index{typeclass!structure functor}\textbf{structure
functor} of the typeclass. (The name \textsf{``}FM\textsf{''} refers to \textsf{``}functors\textsf{''}
and \textsf{``}monads\textsf{''} and will be justified later.)

We can implement this definition by the Scala code:
\begin{lstlisting}
final case class FMTypeclass[P[_]: Functor, A](methods: P[A] => A)
\end{lstlisting}
The code allows us to declare any $FM$-typeclass. For example, the
...
and the recursive-type constructions work for all $FM$-typeclasses. 

\paragraph{Products}

Consider an arbitrary $FM$-typeclass \lstinline!TC! defined via
a structure functor \lstinline!P!. If two types $A$, $B$ have an
...
arbitrary $FM$-typeclasses: given evidence values of types $P^{A}\rightarrow A$
and $P^{B}\rightarrow B$, we cannot always find a suitable value
of type $P^{A+B}\rightarrow A+B$. Later we will see that some $FM$-typeclasses,
...
The function-type construction creates an $FM$-typeclass instance
for the type $E\rightarrow A$ if the type $A$ belongs to the $FM$-typeclass.
...
where the functor $S$ \textsf{``}preserves\textsf{''} $FM$-typeclass instances:
...
as well as for other important $FM$-typeclasses such as filterable
...
Some typeclasses \emph{cannot} be viewed as $FM$-typeclasses. The
...
We may call those typeclasses \textbf{co-}$P$\textbf{-typeclasses}.\index{co-$FM$-typeclasses}
...
detailed description of co-$FM$-typeclasses is beyond the scope of
this book.
...
(This book calls them \textsf{``}$FM$-typeclasses\textsf{''}.)
