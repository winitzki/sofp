...
At the same time, the function body itself must have the same code
for all types \lstinline!T!.

So, it seems we can implement \lstinline!average[T]! only if we have
the information about the necessary methods for the given type \lstinline!T!.
The solution is to pass that information to the function as an extra
argument. What type should that argument have? It could be just a
tuple containing all the necessary methods, or it could be a class
with methods.

To be specific, suppose the required methods are \lstinline!add!
and \lstinline!intdiv! with these types:
\begin{lstlisting}
add: (T, T) => T
intdiv: (T, Int) => T
\end{lstlisting}
With these methods we will be certainly able to implement \lstinline!average[T]!.
So, let us define a case class that encapsulates these methods. The
case class will need to have \lstinline!T! as a type parameter:
...
Now the function \lstinline!average[T]! can be called only with types
\lstinline!T! for which we can provide a value of type \lstinline!Frac[T]!,
which must contain the methods \lstinline!add! and \lstinline!intdiv!.
...
\lstinline!intdiv!) are available. Those functions are called the
\textbf{methods} of the typeclass. We say that the types \lstinline!Double!
and \lstinline!BigDecimal! are \textbf{instances} of the typeclass
\lstinline!Frac!, or \textsf{``}belong to the typeclass \lstinline!Frac!\textsf{''}.
A function argument of type \lstinline!Frac[T]! is called an \textbf{evidence
parameter}\index{typeclass!evidence parameter} because it \textsf{``}provides
evidence\textsf{''} that the type \lstinline!T! actually belongs to the typeclass
\lstinline!Frac!. To \textsf{``}create a \lstinline!Frac! typeclass instance
for type \lstinline!T!\textsf{''} means to write code that computes a value
of type \lstinline!Frac[T]!. That value is called a \textsf{``}\lstinline!Frac!
typeclass evidence value\textsf{''} or a \textsf{``}\lstinline!Frac! typeclass instance
value\textsf{''} for \lstinline!T!.\index{typeclass!instance value}
...
with typeclass evidence \lstinline!ev!.
...
functions with typeclass constraints. First, let us write the code
using an implicit argument to pass the typeclass instance:
\begin{lstlisting}
...
If the types $A$ and $B$ are known to belong to the \lstinline!Extractor!
typeclass, can we add the pair $A\times B$ to that typeclass? If
we can extract a value of type $Z$ from each of two values $a^{:A}$
and $b^{:B}$, we can certainly extract a value of type $Z$ from
the product $a\times b$ by choosing to extract only from $a$ or
only from $b$. So, it appears that we have two possibilities for
implementing the typeclass for $A\times B$. Reasoning more rigorously,
...
We need to investigate whether we can choose $C$ such that $C\rightarrow A$
or $A\rightarrow C$ will have an \lstinline!Extractor! instance
whenever we have an instance for $A$. To compute evidence values
for $C\rightarrow A$ or $A\rightarrow C$, we need to find functions
with the types:
\[
...
applicative functors, do not support the co-product construction (the
co-product of two monads is not always a monad; the co-product of
two applicative functors is not always an applicative functor).
...
yet prove that the laws will hold for the new typeclass instances.
We will prove that in Section~\ref{subsec:Laws-for-the-P-typeclass-constructions}.
...
(The typeclass laws for the new typeclass instances will hold automatically,
as we will show in Section~\ref{subsec:Laws-for-the-P-typeclass-constructions}.)
