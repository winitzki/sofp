...
of type $Q$, which we do not have. (We assume that nothing is known
about the types $P$ and $Q$.) Similarly, we cannot return a value
of type $Q\rightarrow A\times B$.

We find that a function of type~(\ref{eq:type-signature-of-g-zip})
cannot be implemented for the functor $F^{A}\triangleq(P\rightarrow A)+(Q\rightarrow A)$
without additional information about the types $P$ and $Q$. Functors
$F$ for which the type signature~(\ref{eq:type-signature-of-g-zip})
