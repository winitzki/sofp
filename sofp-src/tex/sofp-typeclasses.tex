...
As an example, consider the functor \lstinline!F! defined by \lstinline!F[A] = Option[(A, A)]!:
\begin{lstlisting}
type F[A] = Option[(A, A)]
\end{lstlisting}
...
\subsubsection{Example \label{subsec:Example-inflate-function}\ref{subsec:Example-inflate-function}\index{examples}}
...
Use the \lstinline!map! method of \lstinline!F! to replace all stored
values:
\begin{lstlisting}
...
\subsubsection{Example \label{subsec:Example-inflate-function-1}\ref{subsec:Example-inflate-function-1}\index{examples}}
...
$T$ and possibly other (fixed) types. We call $\text{Sig}$ the \index{method signature}\textbf{method
...
OOI definition has an additional \emph{first} argument (corresponding
to Scala\textsf{'}s \lstinline!this! keyword).

In this way, the type of an interface is seen as a tuple of some function
...
class is defined. After that definition, however, one cannot add or
remove inherited interfaces. For example, once it is declared that
...
and shortcomings. As we have seen, typeclasses and their inheritance
may be also implemented without using any object-oriented interfaces
or inheritance.
...
whenever evidence values for \lstinline!A! and \lstinline!B! are
available. The compiler will then automatically use that information
and accept that, for any types \lstinline!A! and \lstinline!B!,
the pair \lstinline!(A, B)! belongs to the typeclass whenever \lstinline!A!
and \lstinline!B! do. Similarly, typeclass evidence could be derived
for \lstinline!Either[A, B]! under appropriate conditions. Then the
compiler will compute and use the new typeclass evidence values automatically
for \emph{all} suitable types \lstinline!A!, \lstinline!B!. Certain
advanced features of Scala (beyond the scope of this book) allow programmers
to extend typeclass derivation from pairs \lstinline!(A, B)! to all
case classes, and from the disjunctive type \lstinline!Either[A, B]!
to all disjunctive types. In this way, typeclass membership can be
provided by the compiler for a wide range of types, which reduces
boilerplate code. But there are no comparable facilities for object-oriented
...
and interfaces. An ADT is never interpreted as just any type $T$
that happens to have methods of type $P^{T}\rightarrow T$. Instead,
the compiler interprets an ADT as the \emph{unique} type generated
by the given constructors while using no other information or constraints.
Rigorous definitions and the necessary theory will be developed in
