...
Scala methods in an OOI may use an automatically defined value denoted
by the keyword \lstinline!this!. In some other object-oriented languages,
method signatures have an explicit first argument denoted by a special
keyword (such as \lstinline!this! or \lstinline!self!). When we
analyze OOIs as types, we must make all argument types explicit, even
though the Scala syntax does not do that. For this reason, we will
view the types of methods in an OOI definition as having an additional
first argument (corresponding to Scala\textsf{'}s \lstinline!this! keyword).
