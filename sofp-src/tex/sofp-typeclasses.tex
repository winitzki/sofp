...
functions (TVF). The syntax for TVFs supported in Scala 3 shows this
more clearly:
\begin{lstlisting}
...
type quantifiers such as $\forall(A,B)$. In Scala 2, we need to represent
such \textsf{``}nested\textsf{''} type quantifiers by writing a \lstinline!trait!
with a \lstinline!def! method:\index{typeclass!Functor@\texttt{Functor}}
\begin{lstlisting}
trait Functor[F[_]] {
  def map[A, B](fa: F[A])(f: A => B): F[B]
}
