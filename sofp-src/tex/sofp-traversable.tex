...
The way from \lstinline!foldLeft! to \lstinline!foldMap! \index{foldMap function@\texttt{foldMap} function}begins
...
the arbitrary monoid $M$ by an arbitrary \emph{applicative} functor
...
\subsubsection{Example \label{subsec:Example-traverse-for-1+a*a}\ref{subsec:Example-traverse-for-1+a*a}\index{examples}}
...
two values of type $F^{B}$. We can merge them into a single value
...
\begin{lstlisting}
def toList[A]: T2[A] => List[A] = foldMap[A, List[A]](a => List(a))
\end{lstlisting}
...
(An initial empty list is added because the sub-trees begin one level
deeper.) The standard \lstinline!zip! operation would not work correctly
here: \lstinline!zip! would truncate the longer list. Instead, we
need to keep the longer list\textsf{'}s elements. So, let us implement a special
helper function (\lstinline!listMerge!) for merging the nested lists
in this way:
\begin{lstlisting}
def listMerge[A](l: List[List[A]], r: List[List[A]]): List[List[A]] =
  (l, r) match {
...
implemented a breadth-first folding for trees of type \lstinline!T2!.
...
descriptors, that is, values of type \lstinline!TD[A]! that can be
obtained by applying \lstinline!t2ToTd! to some trees of type \lstinline!T2[A]!.
The function composition \lstinline!t2ToTD andThen tdToT2! should
...
\begin{lstlisting}
def removeLeft[A]: List[Either[A, A]] => List[A] =
  _.collect { case Left(x) => x }
...
\begin{lstlisting}
def removeRight[A]: List[Either[A, A]] => List[A] =
  _.collect { case Right(x) => x }
...
which we here denote by \lstinline!travList!. For the functor \lstinline!Either[A, A]!,
we will also need a \lstinline!traverse! function, which we call
\lstinline!travEither!:
\begin{lstlisting}
...
\end{lstlisting}
Using \lstinline!travList! and \lstinline!travEither!, we can implement
\lstinline!travTD! like this:
\begin{lstlisting}
...
value \lstinline!init!. We could have defined \lstinline!scanLeft!
differently, such that the initial value is omitted and the length
of the output list is equal to that of the input list. In fact, we
\emph{need} to omit that initial element when we generalize \lstinline!scanLeft!
...
def accum[A, Z](a: A, f: (A, Z) => Z): State[Z, Z] = State { z =>
  val newZ = f(a, z)  // Compute the new internal state using `f`.
  (newZ, newZ)        // Store the new internal state, and also return it as a result value.
}
...
arbitrary applicative functor. However, some computations are still
not expressible via \lstinline!traverse! because those computations
require information that \lstinline!traverse! \emph{cannot} have.
...
converts trees of type \lstinline!T2! into that \LaTeX{} format:
\begin{lstlisting}
...
\subsection{Recursion schemes. I. Folding \label{subsec:Recursion-schemes.-folding}}
...
the effort required for implementing various traversing operations
...
pattern} \textbf{functor}\index{recursion pattern functor|textit}
of the type \lstinline!T2!. A pattern functor describes the places
where a recursive type refers to itself in its definition. The recursive
uses correspond to occurrences of the type parameter $R$ in $S^{A,R}$.
The name \textsf{``}pattern functor\textsf{''} emphasizes that it must be covariant
with respect to the type parameter $R$ that describes recursive type
usages. In general, a pattern functor may have other type parameters
with respect to which it is not necessarily covariant. In this case,
$S$ is a bifunctor because $S$ is covariant with respect to both
its type parameters. 
...
respect to the type parameter $R$ of $S^{A,R}$. The custom string
manipulations specific to \lstinline!printLaTeXSubtree! are described
by the second matrix in the code above. We may express that via a
function \lstinline!toLaTeX! of type $S^{A,Z}\rightarrow Z$:
\[
\text{toLaTeX}:A+Z\times Z\rightarrow Z\quad,\quad\text{toLaTeX}\triangleq\,\begin{array}{|c||c|}
 & Z\\
\hline A & \text{toString}\\
Z\times Z & l\times r\rightarrow\text{"[ "}+l+\text{" "}+r+\text{" ]"}
\end{array}\quad.
...
corresponding Scala code is:
\begin{lstlisting}
def fmapR[A, R, T](f: R => T): S[A, R] => S[A, T] =
  _.map { case (r1, r2) => (f(r1), f(r2)) }
...
in the pattern functor $S$ and in the result type $Z$ (which is
\emph{not} required to be a monoid). Different pattern functors $S$
may be used to define lists, trees, and other recursive data types.
The same code of $\text{fold}_{S}$ will work for all those data types,
as long as we have the pattern functor $S$ and the corresponding
lifting function (\lstinline!fmapR! in the Scala code shown above,
or $^{\uparrow S^{A,\bullet}}$ in the code notation).
...
Scala\textsf{'}s standard \lstinline!List! functor may be defined via a pattern
functor $S$ by:
\[
...
A non-empty list type (\lstinline!NEL!) and the corresponding pattern
functor $S^{A,R}$ are defined by:
\[
...
A rose tree (\lstinline!TreeN!) and the corresponding pattern functor
$S^{A,R}$ are defined by:
\[
...
\]
For rose trees, the pattern functor $S$ itself uses recursively defined
types (non-empty lists, \lstinline!NEL!). This is not a problem:
$S$ is still polynomial, which guarantees that any value of type
$S^{A,R}$ contains a finite number of elements of types $A$ and
$R$. So, any value of type \lstinline!TreeN[A]! will contain a finite
number of values of type $A$, assuring that the folding operation
will terminate.
...
recursive class \lstinline!Fix! that takes  $S$ as a type parameter:
\begin{lstlisting}
...
def treeNToLaTeX[A](t: Fix[S3, A]): String =
  "\\Tree" + fold[A, String, S3](toLaTeX3)(t)
...
where $\text{fold}_{S}$ enters an infinite loop is the pattern functor
$S^{A,R}\triangleq A+(\bbnum 1\rightarrow R)$. This $S$ is non-polynomial
due to the function type $\bbnum 1\rightarrow R$, which delays the
evaluation of a value of type $R$. This allows us to implement a
well-defined, finite value \lstinline!x: L[A]! which refers to itself
under the delayed evaluation:
\begin{lstlisting}
...
It seems that we need to restrict pattern functors $S$ to \emph{polynomial}
...
The method $\text{fold}_{S}$ is an example of a \textbf{recursion
scheme}\index{recursion scheme}: a higher-order function that manipulates
recursive types and works in the same way for all pattern functors
$S$. This makes $\text{fold}_{S}$ powerful but hard to use because
data structures need to be defined via the \lstinline!Fix! constructor.
Instead of working with the general function $\text{fold}_{S}$, it
is more convenient to implement and use a specialized code of $\text{fold}_{S}$
adapted to the data structure at hand. The general implementation
of $\text{fold}_{S}$ in Eq.~(\ref{eq:fold-via-recursion-scheme-1})
can be translated mechanically (in principle, even automatically using
macros or code generators) into code specialized for a given data
type and pattern functor.
...
An example of an aggregation operation that cannot be expressed via
\lstinline!traverse! is the task of determining the maximum branching
...
Another example of an operation not expressible via \lstinline!traverse!
...
via $\text{fold}_{S}$ specialized to the binary tree\textsf{'}s pattern functor
$S^{A,R}\triangleq A+R\times R$ that defines the  type constructor
\lstinline!T2!. The code for the specialized version of $\text{fold}_{S}$
is:
\begin{lstlisting}
type S[A, R] = Either[A, (R, R)]         // Pattern functor for T2[A].
def foldT2[A, Z](f: S[A, Z] => Z): T2[A] => Z = {
  case Leaf(a)        => f(Left(a))
  case Branch(l, r)   => f(Right((foldT2(f)(l), foldT2(f)(r))))
}
\end{lstlisting}

The function call \lstinline!foldT2(f)! will be able to convert any
tree into a value of type \lstinline!Z! if we supply a function \lstinline!f!
of type $A+Z\times Z\rightarrow Z$. It appears that we need the type
\lstinline!Z! to contain a new depth-indexed tree. The function \lstinline!f!
will be applied recursively to leaves and branches of a tree. The
depth-indexed index can be computed correctly only if the type \lstinline!Z!
somehow keeps track of the current depth in the tree. So, we cannot
just set \lstinline!Z = T2[(A, Int)]!. We need to use a \lstinline!State!
monad whose internal state (of type \lstinline!Int!) represents the
current depth and is updated automatically. Let us define a suitable
\lstinline!State! monad that we will denote by \lstinline!St! for
brevity:
\begin{lstlisting}
final case class St[A](run: Int => (A, Int))  { // A State monad with internal state of type Int.
...
\end{lstlisting}
The folding result type \lstinline!Z! will be defined as \lstinline!Z = St[T2[(A, Int)]]!.
The wrapped value (of type \lstinline!T2[(A, Int)]!) inside the \lstinline!State!
monad will be the final decorated tree. To extract that tree, we will
need to build a \lstinline!State! monad program and run it with the
initial depth value \lstinline!0!. So, we expect the code of \lstinline!zipWithDepth!
to look like this:
\begin{lstlisting}
def zipWithDepth[A](tree: T2[A]): T2[(A, Int)] =
  foldT2[A, St[T2[(A, Int)]]](f).run(0)._1
\end{lstlisting}

It remains to implement a suitable function \lstinline!f! to which
we will apply \lstinline!foldT2!. The value of the internal state
represents the current depth of the tree element. We need to increment
that depth whenever we find a branch and then traverse the two subtrees
starting from the same depth value. The final code is:
\begin{lstlisting}
def zipWithDepth[A](tree: T2[A]): T2[(A, Int)] = foldT2[A, St[T2[(A, Int)]]] {
...
Note that the the actually used type of \lstinline!foldT2! is somewhat
similar to the type of \lstinline!traverse!:
\begin{align*}
 & \text{foldT2}:\big(S^{A,F^{L^{B}}}\rightarrow F^{L^{B}}\big)\rightarrow L^{A}\rightarrow F^{L^{B}}\quad,\\
 & \text{trav}_{L}:(A\rightarrow F^{B})\rightarrow L^{A}\rightarrow F^{L^{B}}\quad.
\end{align*}
(Here $L=\text{T2}$ and $S$ is its pattern functor.) We apply \lstinline!foldT2!
to a function $f:S^{A,F^{L^{B}}}\rightarrow F^{L^{B}}$ whose $F$-effect
is \emph{not} equivalent to an applicative functor\textsf{'}s effect: The function
$f$ assigns the \lstinline!State! monad\textsf{'}s internal state to a previously
computed value, which applicative effects cannot do (as their effects
cannot depend on a previous value). This example shows that the recursion
scheme $\text{fold}_{S}$ is more powerful than the plain traversal,
$\text{trav}_{L}(f^{:A\rightarrow F^{B}})$, which may only use applicative
functors $F$. Because of that extra power, \lstinline!zipWithDepth!
can be expressed via $\text{fold}_{S}$.

\subsection{Recursion schemes. II. Unfolding \label{subsec:Recursion-schemes.-II.unfolding}}

A \textsf{``}folding\textsf{''} operation is a recursion scheme that converts a collection
to a single value. The opposite recursion scheme is \textsf{``}unfolding\textsf{''}:
converting a single value somehow into a new collection. By reversing
the direction of certain function arrows in the type signature of
$\text{fold}_{S}$, we can define a function called \lstinline!unfold!
parameterized by a pattern functor $S$ and a function of type $Z\rightarrow S^{A,Z}$:
\begin{equation}
...
constructor $L$ whose pattern functor $S$ is given. 

To get more intuition, we look at some examples using \lstinline!unfold!
with lists and binary trees.

\subsubsection{Example \label{subsec:Example-unfold-list}\ref{subsec:Example-unfold-list}\index{examples}}
...
The pattern functor for \lstinline!List! is $S^{A,R}\triangleq\bbnum 1+A\times R$.
...
we expect to be~{\tiny{}\Tree[ [ 0 1 ] [ 2 3 ] ]}:\vspace{0.6\baselineskip}
\begin{lstlisting}
...
fail because data structures in a computer cannot have infinite size.
...
As an example, consider the pattern functor $S^{A,R}\triangleq A+(\bbnum 1\rightarrow R\times R)$.
...
write a pattern functor and implement a specialized version of \lstinline!unfold!
...
With these definitions, we can now formulate and prove an equivalence
between \lstinline!foldLeft!, \lstinline!foldMap!, and \lstinline!reduceE!.
...
{\color{greenunder}\text{use Eq.~(\ref{eq:identity-law-of-inMF})}:}\quad & \quad=p\triangleright\text{reduceE}\triangleright f\quad.
\end{align*}
The two sides are now equal. This proves that the equivalence between
\lstinline!reduceE! and \lstinline!foldFn! holds also in the other
direction. $\square$
...
\[
A\triangleq N\quad,\quad B\triangleq M\quad,\quad x^{:L^{N\rightarrow N}}\triangleq p\triangleright f^{\uparrow L}\triangleright\text{inMF}^{\uparrow L}\quad,\quad y^{:L^{M\rightarrow M}}\triangleq p\triangleright\text{inMF}^{\uparrow L}\quad.
\]
In order to use Eq.~(\ref{eq:strong-dinaturality-law-of-phi-with-f}),
...
is \lstinline!reduceE! used with the monoidal type \lstinline!List[A]!
...
\[
\text{pu}_{\text{List}}\bef\text{reduceList}\overset{?}{=}\text{id}^{:M\rightarrow M}\quad.
\]
...
is equal to an identity function:
\begin{equation}
\text{pu}_{\text{List}}^{\uparrow\text{List}}\bef\text{reduceList}\overset{?}{=}\text{id}^{:\text{List}^{M}\rightarrow\text{List}^{M}}\quad.\label{eq:identity-for-reduceList-and-pure}
\end{equation}
...
\]
We can now simplify the code and obtain an identity function:
\begin{align*}
...
expect this property to hold. But the main purpose of \lstinline!toList[A]!
is to extract values of type $A$ out of $L^{A}$ and store them in
a list. Naturality laws do not describe that purpose.
...
holds only when \lstinline!toList! extracts \emph{every} value of
type \lstinline!A! stored in \lstinline!p!. Unfortunately, it seems
to be impossible to express that property in the form of an equation
...
law for \lstinline!toList! stating that \lstinline!toList(p)! should
extract some known values contained in \lstinline!p!? If $L$ were
a pointed functor\index{pointed functor} (see Section~\ref{subsec:Pointed-functors-motivation-equivalence}),
...
\subsection{All polynomial functors are foldable\label{subsec:All-polynomial-functors-are-foldable}}
...
\begin{lstlisting}
def toList_M[A]: ((K[A], L[A])) => List[A] =
  { case (p, q) => toList_K(p) ++ toList_L(q) }
\end{lstlisting}
\[
...
in $p$ are listed \emph{before} the values stored in $q$. We could
equally well write $\text{toList}_{L}(q)\,\pplus\,\text{toList}_{K}(p)$
...
typeclass may use different traversal orders, as we have seen in the
...
We will implement \lstinline!toList! for a functor $L$ defined recursively
by $L^{A}\triangleq S^{A,L^{A}}$, where the pattern functor $S$
is a bifunctor that is itself foldable. A bifunctor $S$ is \textbf{foldable}\index{foldable bifunctor}
if there is a \lstinline!toList! function with this type signature:
\[
\text{toList}_{S}:S^{A,A}\rightarrow\text{List}^{A}\quad.
\]
Assuming that $\text{toList}_{S}$ is available, we define $\text{toList}_{L}$
using $\text{toList}_{S}$ and the bifunctor $S$\textsf{'}s liftings (denoted
by \lstinline!mapS1! and \lstinline!mapS2!):
\begin{lstlisting}
def toList_L[A]: S[A, L[A]] => List[A] =
  _.mapS2(toList_L).mapS1(List(_)).toList_S.flatten
\end{lstlisting}
\[
...
\subsubsection{Example \label{subsec:Example-some-applicative-morphisms}\ref{subsec:Example-some-applicative-morphisms}\index{examples}}
...
between $\text{Id}$ (the identity functor) and $F$.

\subparagraph{Solution}

The identity functor\textsf{'}s applicative methods are identity functions:
\[
\text{wu}_{\text{Id}}=1^{:\bbnum 1}\quad,\quad\quad\text{pu}_{\text{Id}}(x)=x\quad,\quad\quad\text{zip}_{\text{Id}}(p\times q)=p\times q\quad.
\]
...
to combine the three $F$-effects together. One of the wrapped values
of type \lstinline!B! is discarded by the code \lstinline!{ case ((x, y), z) => (y, z) }!.
...
more than once. To get a heuristic explanation, suppose that \lstinline!traverse!
...
In general, the results will not be the same when $F$-effects and
...
Here $L^{A}\triangleq S^{A,L^{A}}$, with a pattern functor given
by the bifunctor $S$. In order to obtain the traversable property
of $L$, we will need to assume that $S$ is traversable with respect
to \emph{both} its type parameters in a special way, which we call
\textsf{``}bitraversable\textsf{''}. It is not sufficient if $S$ is traversable with
...
are free to use any polynomial bifunctor $S$ as a pattern functor.
...
number of parts in the disjunctive type. Each part of the disjunctive
type is written as $Z_{k}\times A\times...\times A$ and contains
a finite number $k$ of values of type $A$ and a value of a fixed
type $Z_{k}$. The computation \lstinline!traverse(f)!, where $f$
is a function of type $A\rightarrow F^{B}$, iterates over the $k$
values of type $A$ and merges the $F$-effects in the natural order
using $F$\textsf{'}s \lstinline!zip! operation. This creates a value of type
$F^{B\times...\times B}$. The constant value of type $Z_{k}$ is
then lifted into $F$ to get the final value of type $F^{Z_{k}\times B\times B\times...\times B}$.
...
Here $S^{A,B}\triangleq T^{A,B,S^{A,B}}$, with a pattern functor
...
require a 4-traversable 4-functor as a pattern functor. To prove that
all polynomial 4-functors are 4-traversable, we will need to use 5-traversable
5-functors, and so on. Note that the proofs in this section differ
from the proofs in Section~\ref{subsec:All-polynomial-functors-are-traversable}
...
by using a suitable pattern functor $S^{A,R}$.
...
\subsection{Recursion schemes and structure-preserving maps}

Sections~\ref{subsec:Recursion-schemes.-folding} and~\ref{subsec:Recursion-schemes.-II.unfolding}
show the general \textsf{``}folding\textsf{''} and \textsf{``}unfolding\textsf{''} functions that
work in the same way for all recursive types. It turns out that those
functions are adapted to the recursive structure of the types in a
way that can be expressed by mathematical laws.

What are those laws? The \textsf{``}folding\textsf{''} function has the type signature:
\[
\text{fold}_{S}:(S^{A,Z}\rightarrow Z)\rightarrow L^{A}\rightarrow Z\quad.
\]
Given $f:S^{A,Z}\rightarrow Z$, the function $\text{fold}_{S}(f)$
transforms a value of type $L^{A}$ into a value of type $Z$. Here,
the type $L^{A}$ is defined recursively by $L^{A}\triangleq S^{A,L^{A}}$,
while the type $Z$ is arbitrary. 

Although $L^{A}$ is a data structure with a type parameter, the code
of $\text{fold}_{S}$ always keeps $A$ fixed. So, we may simplify
our considerations by removing the type parameter $A$. Then we can
replace the pattern functor $S^{A,R}$ by just $S^{R}$ (where $S$
is a functor) and the type $L^{A}$ by a type $T$ defined by the
recursive equation $T\triangleq S^{T}$. The type of $\text{fold}_{S}$
is simplified to:
\[
\text{fold}_{S}:(S^{Z}\rightarrow Z)\rightarrow T\rightarrow Z\quad.
\]

At first sight, the types $T$ and $Z$ have nothing in common. However,
we need to keep in mind that $T$ is equivalent to $S^{T}$ by definition
of the recursive type $T$. The equivalent types $T$ and $S^{T}$
are related by a pair of isomorphisms that we may call \lstinline!fix!
and \lstinline!unfix!:
\[
\text{fix}:S^{T}\rightarrow T\quad,\quad\text{unfix}:T\rightarrow S^{T}\quad,\quad\text{fix}\bef\text{unfix}=\text{id}\quad,\quad\text{unfix}\bef\text{fix}=\text{id}\quad.
\]
We consider the functions \lstinline!fix! and \lstinline!unfix!
as given. The type signature of $\text{fold}_{S}$ requires a function
of type $S^{Z}\rightarrow Z$, which is similar to the type of \lstinline!fix!.
So, inside the function body of $\text{fold}_{S}(f)$ we have types
$T$ and $Z$ for which functions of types $S^{T}\rightarrow T$ and
$S^{Z}\rightarrow Z$ are available.

It is useful at this point to borrow some terminology from category
theory where an object $Z$ with a morphism of type $S^{Z}\rightarrow Z$
has a special name. The object $Z$ is called a \textbf{functor algebra}
on the functor $S$, and\index{functor algebra} the morphism of type
$S^{Z}\rightarrow Z$ is called the algebra\textsf{'}s \textbf{structure map}.\index{functor algebra!structure map}\footnote{In the mathematical literature, a functor algebra on a functor $F$
is commonly referred to as an $F$\textbf{-algebra}\index{$F$-algebra!see functor algebra}.
That terminology is inconvenient because the relevant functor is not
always called $F$. This book will use the terms \textsf{``}functor algebra\textsf{''}
or \textsf{``}$F$-functor algebra\textsf{''}.}

From a programmer\textsf{'}s viewpoint, an intuitive picture is that the type
$Z$ must be able somehow to absorb information from the type $S^{Z}$.
For instance, if $S$ is a pattern functor for a binary tree (such
as, $S^{R}=A+R\times R$ for \lstinline!T2!) then the type $S^{Z}\rightarrow Z$
is equivalent to the product type $(A\rightarrow Z)\times(Z\times Z\rightarrow Z)$.
So, $Z$ must support an operation that maps a tree leaf (of some
fixed type $A$) into a value of type $Z$, and also an operation
that maps two tree branches (two values of type $Z$) into another
value of type $Z$. The types of those operations follow automatically
from the pattern functor $S$. 

If both types $T$ and $Z$ are functor algebras on $S$ then both
$T$ and $Z$ support the same operations. It is then natural to expect
that a useful map between values of types $T$ and $Z$ should preserve
the structure of those operations. The mathematical formulation of
that requirement is called the law of \textsf{``}\index{functor algebra!morphism law}functor
algebra morphisms\textsf{''}.

\subsubsection{Definition \label{subsec:Definition-functor-algebra}\ref{subsec:Definition-functor-algebra}}

Given a functor $S$ and two $S$-functor algebras $X,$$Y$ with
given structure maps $p_{X}:S^{X}\rightarrow X$ and $p_{Y}:S^{Y}\rightarrow Y$,
a function $f:X\rightarrow Y$ is called a \textbf{functor algebra
morphism}\index{functor algebra!morphism} if the following law holds:
\[
\xymatrix{\xyScaleY{1.4pc}\xyScaleX{3.0pc}S^{X}\ar[r]\sp(0.5){\ p_{X}}\ar[d]\sp(0.45){\,f^{\uparrow S}} & X\ar[d]\sp(0.45){\,f}\\
S^{Y}\ar[r]\sp(0.5){~p_{Y}} & Y
}
\]
\begin{equation}
p_{X}\bef f=f^{\uparrow S}\bef p_{Y}\quad.\label{eq:p-algebra-morphism-law-1}
\end{equation}
$\square$

Suppose we have a function $p_{Z}:S^{Z}\rightarrow Z$, which makes
a type $Z$ into an $S$-functor algebra. Then it turns out that $\text{fold}_{S}(p_{Z}):T\rightarrow Z$
is a functor algebra morphism between $T$ and $Z$. In other words,
the function $\text{fold}_{S}(p_{Z})$ preserves the operations of
the $S$-functor algebras.

\subsubsection{Statement \label{subsec:Statement-catamorphism}\ref{subsec:Statement-catamorphism}}

Consider a pattern functor $S$ and a recursive type $T$ defined
by $T\triangleq S^{T}$. The functions \lstinline!fix! $:S^{T}\rightarrow T$
and \lstinline!unfix! $:T\rightarrow S^{T}$ are assumed to be given.
For any type $Z$ and any function $g:S^{Z}\rightarrow Z$, define
$f:T\rightarrow Z$ by $f\triangleq\text{fold}_{S}(g)$, where $\text{fold}_{S}$
is defined in Section~\ref{subsec:Recursion-schemes.-folding}. Then
$f$ is a functor algebra morphism: it obeys Eq.~(\ref{eq:p-algebra-morphism-law-1})
with $X=T$, $p_{X}=$ \lstinline!fix!, $Y=Z$, and $p_{Y}=g$. 

\subparagraph{Proof}

The functor algebra morphism law~(\ref{eq:p-algebra-morphism-law-1})
of $f$ is:
\[
\text{fix}\bef f\overset{?}{=}f^{\uparrow S}\bef g\quad.
\]
Write the code of $f\triangleq\text{fold}_{S}(g)$:
\[
f:T\rightarrow Z\quad,\quad\quad f\triangleq\text{unfix}\bef\overline{f}^{\uparrow S}\bef g\quad.
\]
Pre-compose \lstinline!fix! to both sides of the equation for $f$:
\begin{align*}
 & \text{fix}\bef f=\gunderline{\text{fix}\bef\text{unfix}}\bef\overline{f}^{\uparrow S}\bef g\\
{\color{greenunder}\text{recall that }\text{fix}\bef\text{unfix}=\text{id}:}\quad & =\overline{f}^{\uparrow S}\bef g\quad.
\end{align*}
We have obtained the required law. $\square$

A similar property holds for the function \lstinline!unfold!. A type
$Z$ with a function $g:Z\rightarrow S^{Z}$ is called an\index{functor coalgebra}
$S$-\textbf{functor coalgebra}. We define $f:Z\rightarrow T$ by
$f\triangleq\text{unfold}_{S}(g)$. Then we will have:
\[
f\bef\text{unfix}=g\bef f^{\uparrow S}\quad.
\]
This property is called the \index{functor coalgebra morphism law}\textsf{``}\textbf{functor
coalgebra morphism} law\textsf{''} in category theory. Intuitively, it means
that the function $f$, in a certain precise sense, preserves the
operations of $S$-functor coalgebras. 

The proof is analogous to that of Statement~\ref{subsec:Statement-catamorphism}.
Write the code of $f\triangleq\text{unfold}_{S}(g)$:
\[
f\triangleq g\bef f^{\uparrow S}\bef\text{fix}\quad.
\]
It remains to compose \lstinline!unfix! with both sides of the equation
above:
\[
f\bef\text{unfix}=g\bef f^{\uparrow S}\bef\gunderline{\text{fix}\bef\text{unfix}}=g\bef f^{\uparrow S}\quad.
\]
This proves the functor coalgebra morphism law for $f$. $\square$

Note that these properties of $\text{fold}_{S}(g)$ and $\text{unfold}_{S}(g)$
hold only under the assumption that the resulting functions terminate.
The proofs do \emph{not} show that those functions must terminate.
As we have seen, functions obtained via \lstinline!fold! and \lstinline!unfold!
will in some cases loop forever. The laws only hold when the functions
do not generate infinite loops.

In the functional programming literature, the function $\text{fold}_{S}$
is called a \textbf{catamorphism}\index{catamorphism}, and $\text{unfold}_{S}$
is called an \index{anamorphism}\textbf{anamorphism}.
...
The monad laws hold for $\text{CFState}$ because those laws hold
for the code of $\text{pu}_{\text{State}}$ and $\text{ftn}_{\text{State}}$.
...
it has a left inverse (\lstinline!toCF!):
\[
...
because, by assumption, $S$ is not a void type.
...
\end{align*}
The sub-expression ($\_^{:\text{Int}}\rightarrow a$) is a constant
function. So, $g\bef\pi_{1}^{\uparrow\text{State}}$ can be expressed
as a function $h$ of type $A\rightarrow\text{CFState}^{\text{Int},A}$
...
advanced techniques motivated by dependent type theory to prove the
second property of \lstinline!zipWithIndex! for arbitrary polynomial
functors $L$ without assuming any new laws of \lstinline!traverse!.
The techniques of Bird et al.~are beyond the scope of this book,
as the required theory is complicated but has limited practical use.
Because all polynomial functors are traversable (and no other functors
are), it is not as important to be able to characterize \lstinline!traverse!
solely via laws. In practice, there is no uncertainty about how to
implement a law-abiding \lstinline!traverse! correctly, for any given
polynomial functor and a given traversal order. 
...
there exist values $a_{1}$, ..., $a_{n}$ of type $A$ such that:
\[
...
\]
Now we may define a function $t_{p}:\text{Int}\rightarrow A$ such
that:
\[
t_{p}(i)\triangleq a_{i}\quad,\quad\quad i=1,2,...,n\quad.
\]
For other arguments $i$, the values $t_{p}(i)$ may be defined arbitrarily
as: 
\[
t_{p}(i)\triangleq a_{0}\quad,\quad\quad i\neq1,2,...,n\quad,
\]
where $a_{0}$ is any chosen value of type $A$ (by assumption, the
type $A$ is not void). Then we use the naturality law of \lstinline!make!
and prove the required property:
\begin{align*}
...
\end{align*}
Then the property of $\text{zwi}_{L}$ from part \textbf{(a)} is rewritten
as:
\[
...
So, the code of $\text{seq}_{L}^{F,A}$ \emph{must be} this:
\[
...
returns when applied to nontrivial $F$-effects. 
...
a pattern functor. We have seen one example of a recursive type constructor
that cannot be defined in that way: a perfect-shaped tree (see Section~\ref{subsec:Perfect-shaped-trees}
and Example~\ref{subsec:Example-traversal-perfect-shaped-tree}).
The recursive type equation for a perfect-shaped binary tree\index{perfect-shaped tree}
($\text{PT}$) is:
\begin{equation}
...
a pattern functor, we need to introduce an additional functor $P$
and write:
\[
...
than once. We will not study such type equations in this book, as
it is not clear how much use they have in practice.
...
The type \lstinline!Sq[A]! is equivalent to an \textsf{``}infinite\textsf{''} disjunctive
type representing square matrices of every size ($1\times1$, $2\times2$,
and so on). To define an infinite disjunctive type, we normally use
recursion at type level. In a mathematical sense, this recursion will
be induction on the size of the matrix. So, let us introduce the size
of the matrix as an extra \emph{type parameter} $N$. It will be convenient
to define \lstinline!SqSize[N, A]! as the type of matrices of size
\emph{at least} $N$. We intend $N$ to be equivalent to one of the
types $\bbnum 1$, $\bbnum 2$, etc. 
