...
those operations are mathematically unique. We will now prove that
there is only one \textsf{``}folding\textsf{''} function and only one \textsf{``}unfolding\textsf{''}
function obeying appropriate laws. 

What are those laws? The \textsf{``}folding\textsf{''} function had the type signature:
\[
...
the type $L^{A}$ is defined recursively via the equation $L^{A}\triangleq S^{A,L^{A}}$
while the type $Z$ is arbitrary. 

It is actually not so important that $L^{A}$ is a data structure
with a type parameter, because the code of $\text{fold}_{S}$ always
keeps $A$ fixed. So, we may simplify our considerations by eliminating
the type parameter $A$. We replace the recursion scheme $S^{A,R}$
by just $S^{R}$ (where $S$ is a functor) and the type $L^{A}$ by
a type $T$ defined by the recursive equation $T\triangleq S^{T}$.
Then the type of $\text{fold}_{S}$ is simplified to:
\[
...
That terminology is rather inconvenient because the relevant functor
is not always called $F$.}
...
So, both types $T$ and $Z$ have the property of being functor algebras
on $S$. In other words, both types $T$ and $Z$ have operations
induced by the recursion scheme $S$. It is then natural to expect
...
preserves the operations induced by the recursion scheme $S$.
...
with $X=T$, $p_{X}=\text{fix}$, $Y=Z$, $p_{Y}=g$. 
...
\]
This proves the functor coalgebra morphism law for $f$.
...
The proofs do not show that those functions must terminate. As we
have seen, functions obtained via \lstinline!fold! and \lstinline!unfold!
...
\]
Now we may find a function $t_{p}:\text{Int}\rightarrow A$ such that:
\[
t_{p}(i)\triangleq a_{i}\quad,\quad\quad i=1,2,...,n\quad.
\]
For other arguments $i$, the value $t_{p}(i)$ may be defined arbitrarily
as: 
\[
