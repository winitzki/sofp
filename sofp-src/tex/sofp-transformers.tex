...
\subsubsection{Example \label{subsec:Example-either-reader-not-compose}\ref{subsec:Example-either-reader-not-compose}\index{examples}}
...
$L$\textsf{'}s\textbf{ transformer} \index{monad transformers}is the type
...
This example defines a type constructor denoted by \lstinline!TryT!,
...
Transformers for some monads are obtained via functor composition;
...
Its type is \lstinline!StateT[M, S, A] = S => M[(S, A)]!, which puts
...
as making a \index{monads!stack of}\textbf{stack} of monads.
...
\subsubsection{Example \label{subsec:Example-reader-state-transformed}\ref{subsec:Example-reader-state-transformed}\index{examples}}
...
To combine $K\varangle L\varangle M$ with a new monad $N$, we write
...
\subsubsection{Example \label{subsec:Example-monad-stack}\ref{subsec:Example-monad-stack}\index{examples}}
...
The first two laws are now easier to interpret: the function ($\text{pu}_{L}\bef\text{blift}$)
...
a\textbf{ base runner}, $\text{brun}_{L}:T\leadsto M$, and\index{monad transformers!foreign runner}
...
In the first case, we lift a value $m:M^{A}$ to the stack $L\varangle M$
...
of the category of monads\index{monads!category of}.
...
law\index{monadic naturality law!of foreign runner (frun)@of foreign runner (\texttt{frun})}
...
of monads, $L\varangle$ is an \emph{endofunctor}\index{endofunctor!in the category of monads}
...
are known as \textsf{``}pointed\textsf{''}\index{pointed functor!in the category of monads};
...
would joke\index{jokes} similarly to the joke in footnote~\ref{fn:A-monad-is-a-monoid-in-category-of-endofunctors-big-deal}
...
$M$.\index{monad transformers!monad laws} (The functor $T\triangleq L\varangle M$
must have methods $\text{pu}_{T}$ and $\text{ftn}_{T}$ that satisfy
the monad laws.)
\item \textbf{Identity laws}:\index{monad transformers!identity laws} $L\varangle\text{Id}\cong L$
...
corresponding \textbf{functor laws}\index{monad transformers!functor laws of runner}:
\[
...
$\phi\triangleq\text{pu}_{M}:\text{Id}\leadsto M$, we obtain a \index{monad transformers!base lift}\textbf{base
...
So, we will not need to check this nondegeneracy law.}\index{nondegeneracy law!of base runner (brun)@of base runner (\texttt{brun})}
\[
...
We try setting $T^{A}=L^{A}\times M^{A}$. We know from Chapter~\ref{chap:Semimonads-and-monads}
...
The following type looks like a general monad transformer:
\[
...
\textbf{(2)} The composition of $\text{Cod}^{L,\bullet}$ and $M$
...
Define $T^{A}$ as the following type expression:
\[
...
Theoretically, $T^{A}$ has all the properties that a monad transformer
...
called \textbf{stacking}\index{monad transformers!stacking} the monads. 
...
\lstinline!A!). The resulting monad is called a \textbf{stack}\index{monads!stack of}
...
We need to show that for any monad morphism $\theta:P\varangle Q\leadsto\text{Id}$
...
We have found the isomorphism maps between $L\varangle\text{Id}$
...
Expanding the definition of the type constructor $M$, we can write
...
\subsubsection{Definition \label{subsec:Definition-rigid-monad}\ref{subsec:Definition-rigid-monad}}
...
To argue that the third law holds, apply the left-hand side to $m$
and $q$:
\begin{align}
...
With $M=\text{Id}$, we have $L\varangle\text{Id}=(G\varangle\text{Id})\times(H\varangle\text{Id})$.
...
To verify the monad transformer laws, begin by observing that $A+(K\varangle M)^{A}$
...
and are followed by $\text{ftn}_{N}$:
\begin{align*}
...
\]
Its monad transformer is also defined recursively:
\[
(\text{Free}^{F,\bullet}\varangle M)^{A}\triangleq M^{A+F^{(\text{Free}^{F,\bullet}\varangle M)^{A}}}\quad.
\]
For brevity, we write $\text{Free}\varangle M$ instead of $\text{Free}^{F,\bullet}\varangle M$,
omitting the superscript $F$ as the functor $F$ will be fixed throughout
the derivations. When the foreign monad $M$ is also fixed, we will
denote the transformed monad $\text{Free}\varangle M$ simply by $T$.

\subsubsection{Statement \label{subsec:Statement-free-monad-transformer}\ref{subsec:Statement-free-monad-transformer}}

\lstinline!Free!$\,\varangle$ is a lawful monad transformer.
...
we use the \textsf{``}unrolling trick\textsf{''}\index{unrolling trick for recursive types}\index{recursive types!unrolling trick}
(Statement~\ref{subsec:Statement-unrolling-trick} with $R^{X}=M^{X}$,
$S^{X}=A+F^{X}$, $U=T^{A}$, and $V=L^{A}$) to express $T$ as a
composition of two monads:
\[
(\text{Free}\varangle M)^{A}=T^{A}=M^{L^{A}}\quad,\quad\quad T=M\circ L=M\circ\text{Free}^{F\circ M}\quad.
\]
...
laws that involve changing the foreign monad. But in the proof of
the monad laws of $T$, the foreign monad $M$ is fixed. 
...
To prove that the composition $M\circ L$ is a lawful monad, we use
Statement~\ref{sec:Statement-swap-laws-to-monad-transformer-first-law}
...
Setting $M=\text{Id}$ in the type of $\text{Free}\varangle M$, we
get the isomorphism $\text{Id}\circ\text{Free}^{F\circ\text{Id}}\cong\text{Free}^{F}$.

\paragraph{Lifting laws}

The function \lstinline!flift! for $\text{Free}\varangle$ is defined
by:
\[
...
Since the transformer $\text{Free}\varangle M=M\circ L$ is not purely
compositional (the monad $L$ itself depends on $M$), mapping the
monad $M$ to $M^{\prime}$ via a monad morphism $\phi:M\leadsto M^{\prime}$
requires us also to map $L$ to $L^{\prime}\triangleq\text{Free}^{F\circ M^{\prime}}$.
...
is a monad morphism. The foreign runner $\text{frun}^{M,M^{\prime}}(\phi):T\leadsto T^{\prime}$
(where $T^{\prime}\triangleq\text{Free}\varangle M^{\prime}$) is
then implemented by:
\[
...
\begin{align*}
 & \quad{\color{greenunder}\text{left-hand side}:}\quad\\
 & \text{sw}_{M,L}\bef\psi^{\uparrow M}\bef\phi=\,\begin{array}{|c||c|}
...
\end{array}\quad,\\
 & \quad{\color{greenunder}\text{right-hand side}:}\quad\\
 & \phi^{\uparrow L}\bef\psi\bef\text{sw}_{M^{\prime},L^{\prime}}\\
 & =\,\begin{array}{|c||cc|}
...
{\color{greenunder}\text{right-hand side}:}\quad & (\text{frun}\,(\phi))^{\uparrow T}\bef\text{frun}\,(\phi)\bef\text{ftn}_{T^{\prime}}\\
 & \quad=(\psi^{\uparrow M}\bef\phi)^{\uparrow L\uparrow M}\bef\psi^{\uparrow M}\bef\phi\bef\text{sw}_{M^{\prime},L^{\prime}}^{\uparrow M^{\prime}}\bef\text{ftn}_{M^{\prime}}\bef\text{ftn}_{L^{\prime}}^{\uparrow M^{\prime}}\quad.
\end{align*}
...
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{frun}\,(\phi_{1}\bef\phi_{2}):}\quad & \text{frun}\,(\phi_{1})\bef\text{frun}(\phi_{2})\\
 & =\psi_{1}^{\uparrow Q}\bef\gunderline{\phi_{1}\bef\psi_{2}^{\uparrow R}}\bef\phi_{2}\\
{\color{greenunder}\text{naturality of }\phi_{1}:}\quad & =\gunderline{\psi_{1}^{\uparrow Q}\bef\psi_{2}^{\uparrow Q}}\bef\phi_{1}\bef\phi_{2}\\
 & =(\psi_{1}\bef\psi_{2})^{\uparrow Q}\bef(\phi_{1}\bef\phi_{2})\quad.
\end{align*}
...
\begin{align*}
 & \phi^{\uparrow L}\bef\phi\bef\text{ftn}_{M}\\
 & =\,\begin{array}{|c||cc|}
 & M^{A} & F^{M^{L^{M^{A}}}}\\
\hline A & \text{pu}_{M} & \bbnum 0\\
...
F^{M^{L^{A}}} & \overline{\phi}^{\uparrow M\uparrow F}\bef\text{ftn}_{M}^{\uparrow F}\bef r
\end{array}\\
 & =\overline{\text{sw}}^{\uparrow M\uparrow F}\bef\text{ftn}_{M}^{\uparrow F}\bef\overline{\phi}^{\uparrow M\uparrow F}\bef\text{ftn}_{M}^{\uparrow F}\bef r\\
 & =\big(\overline{\text{sw}}^{\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef\overline{\phi}^{\uparrow M}}\bef\text{ftn}_{M}\big)^{\uparrow F}\bef r\\
 & =\big(\overline{\text{sw}}^{\uparrow M}\bef\phi^{\uparrow M\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef\text{ftn}_{M}}\big)^{\uparrow F}\bef r\\
 & =\big(\gunderline{\overline{\text{sw}}^{\uparrow M}\bef\phi^{\uparrow M\uparrow M}\bef\text{ftn}_{M}^{\uparrow M}}\bef\text{ftn}_{M}\big)^{\uparrow F}\bef r\\
...
{\color{greenunder}\text{composition law of }\phi:}\quad & =\gunderline{\overline{\xi}^{\uparrow M\uparrow F}\bef\theta^{\uparrow M\uparrow F}\bef\big(\phi^{\uparrow M}}\bef\phi\bef\text{ftn}_{M^{\prime}}\big)^{\uparrow F}\bef r\\
 & =(\gunderline{\overline{\xi}\bef\theta\bef\phi})^{\uparrow M\uparrow F}\bef\phi^{\uparrow F}\bef\text{ftn}_{M^{\prime}}^{\uparrow F}\bef r\\
{\color{greenunder}\text{inductive assumption}:}\quad & =\big((\overline{\psi}\bef\gunderline{\xi\bef\theta)^{\uparrow M}\bef\phi}\bef\text{ftn}_{M^{\prime}}\big)^{\uparrow F}\bef r\\
 & =\big((\overline{\psi}^{\uparrow M}\bef\phi\bef(\xi\bef\theta)^{\uparrow M^{\prime}}\bef\text{ftn}_{M^{\prime}}\big)^{\uparrow F}\bef r\quad.
\end{align*}
...
The definition of $\text{Free}^{F}\varangle M$ is: 
\[
(\text{Free}^{F}\varangle M)^{A}=M^{A+F^{(\text{Free}^{F}\varangle M)^{A}}}\quad.
\]
Denote for brevity $U\triangleq\text{Free}^{F}\varangle\text{Free}^{G}$
and set $M=\text{Free}^{G}$ in the above formula:
\begin{align}
{\color{greenunder}\text{recursive definition of }U^{A}:}\quad & U^{A}\triangleq(\text{Free}^{F}\varangle\text{Free}^{G})^{A}=\text{Free}^{G,A+F^{U^{A}}}\quad.\label{eq:fixpoint-definition-of-free-t}
\end{align}
The type $U^{A}$ is defined via (the \emph{least} \emph{fixpoint}
of) this equation. Denote by $V^{A}\triangleq\text{Free}^{F+G,A}$
the combined free monad:
\begin{align}
{\color{greenunder}\text{recursive definition of }V^{A}:}\quad & \text{Free}^{F+G,A}\triangleq V^{A}=A+F^{V^{A}}+G^{V^{A}}\quad.\label{eq:fixpoint-definition-of-free-f+g}
\end{align}
...
We find that any fixpoint of Eq.~(\ref{eq:fixpoint-definition-of-free-t})
...
The method $\text{ftn}_{U}$ is defined as in the proof of Statement~\ref{subsec:Statement-free-monad-transformer}:
\[
...
To make progress, let us write $\text{ftn}_{U}$ as a code matrix.
...
\end{array}\quad.
\end{align*}
Here we used the lifting formula to obtain $\text{pu}_{L}^{\uparrow M}$.
...
The last function in the definition of $\text{ftn}_{U}$ is $\text{ftn}_{L}^{\uparrow M}$:
\[
...
The \lstinline!flatten! method for $V$ is that of the free monad
$\text{Free}^{F+G}$:
\[
...
\end{array}\quad.
\]
Now we write the two sides separately:
\begin{align*}
...
\[
(\text{List}\varangle M)^{A}\triangleq M^{\bbnum 1+A\times(\text{List}\varangle M)^{A}}\quad,
\]
...
Despite a superficial similarity between $\text{List}\varangle$ and
the transformers for the free monad and the free pointed monad, proofs
of the laws for $\text{List}\varangle$ require a different technique,
and the transformer\textsf{'}s implementation is more complicated. The reason
is that the functor $L$ defined by:
\[
L^{A}\triangleq\bbnum 1+A\times(\text{List}\varangle M)^{A}=\bbnum 1+A\times M^{L^{A}}\quad,
\]
...
So, we \emph{cannot} define $\text{List}\varangle M=M\circ L$ as
a functor composition of two monads. As a result, the \lstinline!flatten!
method of $\text{List}\varangle$, which we will denote for brevity
by $\text{ftn}_{T}$, cannot be defined via a \lstinline!swap! function,
as we have done for the other monad transformers just mentioned. Instead,
the \lstinline!flatten! method must be defined via a special helper
function named \lstinline!prod! as shown in the following statement.\footnote{The paper \textsf{``}Composing monads\textsf{''} (\texttt{\href{http://web.cecs.pdx.edu/~mpj/pubs/RR-1004.pdf}{http://web.cecs.pdx.edu/$\sim$mpj/pubs/RR-1004.pdf}})
...
The type constructor $\text{List}\varangle M$ satisfies all the monad
transformer laws.
...
\]
Denote $T^{A}\triangleq(\text{List}\varangle M)^{A}$. The monad methods
of $T$ are then defined by:
\[
...
\]
This function \textsf{``}combines\textsf{''} two values of type $M^{L^{A}}$ into
...
of type $T^{A}$; that property will be used in the later derivations.
The function \lstinline!prod! is defined recursively via \lstinline!comb!:
\begin{equation}
...
We will now verify that these laws make the monad laws for $T$ hold:

\paragraph{Monad laws}

need to be verified using the monad methods $\text{pu}_{T}$ and $\text{ftn}_{T}$
we defined above. To verify the left identity law of $T$:
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{id}:}\quad & \text{pu}_{T}\bef\text{ftn}_{T}=\text{pu}_{L}\bef\gunderline{\text{pu}_{M}\bef\text{prod}^{\uparrow M}}\bef\text{ftn}_{M}\\
 & =\gunderline{\text{pu}_{L}\bef\text{prod}}\bef\text{pu}_{M}\bef\text{ftn}_{M}\\
...
{\color{greenunder}\text{associativity law of }\text{prod}:}\quad & =(\text{prod}\bef\text{prod}^{\uparrow M}\bef\gunderline{\text{ftn}_{M})^{\uparrow M}\bef\text{ftn}_{M}}\\
 & =\text{prod}^{\uparrow M}\bef\gunderline{\text{prod}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}}\bef\text{ftn}_{M}\\
...
When $M=\text{Id}$, the type constructor $\text{List}\varangle M$
is equivalent to \lstinline!List! since $J^{A}\triangleq(\text{List}\varangle\text{Id})^{A}$
is then defined by the recursive equation:
\[
J^{A}=\text{Id}^{\bbnum 1+A\times J^{A}}\cong\bbnum 1+A\times J^{A}\quad.
\]
...
The transformer $\text{List}\varangle M=M\circ L$ is not purely compositional
($L$ itself depends on $M$), so mapping the monad $M$ to $M^{\prime}$
...
\end{equation}
Denote $T^{\prime}\triangleq\text{List}\varangle M^{\prime}$. The
foreign runner ($\text{frun}^{M,M^{\prime}}(\phi):T\leadsto T^{\prime}$)
...
{\color{greenunder}\text{identity law of }\phi:}\quad & =\text{pu}_{L}\bef\psi\bef\text{pu}_{M^{\prime}}\\
 & =\,\begin{array}{||cc|}
...
{\color{greenunder}\text{right-hand side}:}\quad & \big(\text{frun}\,(\phi)\big)^{\uparrow T}\bef\text{frun}\,(\phi)\bef\text{ftn}_{T^{\prime}}\\
 & \quad=\gunderline{(\psi^{\uparrow M}\bef\phi)^{\uparrow L\uparrow M}\bef\psi^{\uparrow M}\bef\phi}\bef\text{flm}_{M^{\prime}}(\text{prod})
\end{align*}
\begin{align*}
{\color{greenunder}\text{naturality law of }\phi:}\quad & \quad=\phi\bef\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\big)^{\uparrow M^{\prime}}\bef\text{flm}_{M^{\prime}}(\text{prod})\\
 & \quad=\phi\bef\text{flm}_{M^{\prime}}\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\bef\text{prod}\big)\quad.
\end{align*}
The remaining difference is:
\[
...
a given value of type $(\text{\text{List}}\varangle M)^{A}$. However,
for a monoid type $R$ with binary operation $\oplus_{R}$ and empty
element $e_{R}$, the type signature $\text{List}^{R}\rightarrow R$
is implemented by the standard \lstinline!reduce! operation: 
\[
...
transformer $\text{List}\varangle M$ if we restrict its usage to
\emph{monoid} types $A=R$. The function \lstinline!brun! with the
...
\[
\text{brun}:M^{L^{R}}\rightarrow M^{R}\quad,\quad\text{brun}\triangleq\text{flm}_{M}\bigg(\,\begin{array}{|c||c|}
 & M^{R}\\
...
law:
\begin{align*}
 & \quad{\color{greenunder}\text{expect to equal }\text{id}:}\quad\\
 & \text{flift}\bef\text{brun}=\text{pu}_{L}^{\uparrow M}\bef\text{flm}_{M}\bigg(\,\begin{array}{||c|}
...
\end{array}\,\bigg)\\
 ...
 & =\text{flm}_{M}\bigg(\gunderline{\text{pu}_{L}}\bef\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M}(e_{R})\\
r\times t\rightarrow\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}\,(t)
\end{array}\,\bigg)\\
 & \quad{\color{greenunder}\text{definition of }\text{pu}_{L}:}\quad\\
 & =\text{flm}_{M}\bigg(\,\begin{array}{||cc|}
...
\end{array}\,\bigg)\\
 ...
 & \quad{\color{greenunder}\text{use Eq.~(\ref{eq:listt-brun-derivation1})}:}\quad\\
 & =\text{flm}_{M}(a\rightarrow\gunderline{\text{pu}_{M}(a)\oplus\text{pu}_{M}(e_{R})})=\text{flm}_{M}(\gunderline{a\rightarrow\text{pu}_{M}(a)})=\text{flm}_{M}(\text{pu}_{M})=\text{id}\quad.
\end{align*}
...
\begin{align*}
 ...
 & \text{pu}_{T}\bef\text{brun}=\text{pu}_{L}\bef\gunderline{\text{pu}_{M}\bef\text{flm}_{M}}\bigg(\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M}(e_{R})\\
r\times t\rightarrow\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}\,(t)
\end{array}\,\bigg)\\
 & \quad{\color{greenunder}\text{identity law of }\text{flm}_{M}:}\quad\\
 & =\,\begin{array}{||cc|}
...
\end{array}\\
 ...
 & =a^{:R}\rightarrow\text{pu}_{M}(a)\oplus_{M}\gunderline{\overline{\text{brun}}\,(\text{Nil}\triangleright\text{pu}_{M})}\\
 & \quad{\color{greenunder}\text{use Eq.~(\ref{eq:listt-brun-derivation1})}:}\quad\\
 & =a\rightarrow\text{pu}_{M}(a)\,\gunderline{\oplus_{M}\,\text{pu}_{M}(e_{R})}=a\rightarrow\text{pu}_{M}(a)=\text{pu}_{M}\quad.
\end{align*}
...
1\rightarrow\text{pu}_{M}(e_{R})\\
r\times t\rightarrow\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}\,(t)
\end{array}\,\bigg)\bef\phi\\
 & =\phi\bef\text{flm}_{M^{\prime}}\bigg(\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M}(e_{R})\\
r\times t\rightarrow\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}\,(t)
...
\end{array}\,\bigg)\\
 & \quad{\color{greenunder}\text{naturality law of }\text{flm}_{M^{\prime}}:}\quad\\
 & =\phi\bef\text{flm}_{M^{\prime}}\bigg(\psi\bef\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M^{\prime}}(e_{R})\\
r\times t\rightarrow\text{pu}_{M^{\prime}}(r)\oplus_{M^{\prime}}\overline{\text{brun}}\,(t)
\end{array}\,\bigg)\quad.
\end{align*}
...
\end{array}\quad.\label{eq:phi-psi-comparison-derivation1}
\end{equation}
This equality is between functions whose input has type $\bbnum 1+A\times M^{L^{A}}$.
It will be simpler to proceed if we apply both sides of Eq.~(\ref{eq:phi-psi-comparison-derivation1})
to values of types $\bbnum 1+\bbnum 0^{:A\times M^{L^{A}}}$ and $\bbnum 0^{:\bbnum 1}+A\times M^{L^{A}}$
separately.

When the input is $1+\bbnum 0$, we have $(1+\bbnum 0)\triangleright\psi=1$,
and so Eq.~(\ref{eq:phi-psi-comparison-derivation1}) is reduced
to:
\[
\text{pu}_{M}(e_{R})\bef\phi\overset{?}{=}1\triangleright(1\rightarrow\text{pu}_{M^{\prime}}(e_{R}))=\text{pu}_{M^{\prime}}(e_{R})\quad.
\]
The equality holds by $\phi$\textsf{'}s identity law of monad morphisms.

When the input is $\bbnum 0+r^{:A}\times t^{:M^{L^{A}}}$, we reduce
Eq.~(\ref{eq:phi-psi-comparison-derivation1}) to:
\begin{align*}
 & \big(\text{pu}_{M}(r)\oplus_{M}\overline{\text{brun}}{}^{M}(t)\big)\triangleright\phi\\
 & \overset{?}{=}\big(\bbnum 0+r^{:A}\times t^{:M^{L^{A}}}\big)\triangleright\psi\triangleright\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M^{\prime}}(e_{R})\\
r\times t\rightarrow\text{pu}_{M^{\prime}}(r)\oplus_{M^{\prime}}\overline{\text{brun}}{}^{M^{\prime}}(t)
\end{array}\quad.
\end{align*}
...
\begin{align*}
 & \big(\bbnum 0+r^{:A}\times t^{:M^{L^{A}}}\big)\triangleright\psi\\
 & =\big(\bbnum 0+r^{:A}\times t^{:M^{L^{A}}}\big)\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\times M^{\prime L^{\prime A}}\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
A\times M^{L^{A}} & \bbnum 0 & \text{id}\boxtimes(\overline{\psi}^{\uparrow M}\bef\phi)
\end{array}\\
 & =\bbnum 0+r\times(t\triangleright\gunderline{\overline{\psi}^{\uparrow M}\bef\phi})\\
 & \quad{\color{greenunder}\text{definition of }\text{frun}:}\quad\\
 & =\bbnum 0+r\times(t\triangleright\overline{\text{frun}}\,(\phi))\quad.
\end{align*}
So, the right-hand side of Eq.~(\ref{eq:phi-psi-comparison-derivation1})
becomes:
\begin{align*}
 & \big(\bbnum 0+r\times(t\triangleright\overline{\text{frun}}\,(\phi))\big)\triangleright\,\begin{array}{||c|}
1\rightarrow\text{pu}_{M^{\prime}}(e_{R})\\
r\times t\rightarrow\text{pu}_{M^{\prime}}(r)\oplus_{M^{\prime}}\overline{\text{brun}}{}^{M^{\prime}}(t)
\end{array}\\
 & =\text{pu}_{M^{\prime}}(r)\oplus_{M^{\prime}}\big(t\triangleright\gunderline{\text{frun}\,(\phi)\bef\overline{\text{brun}}{}^{M^{\prime}}}\big)\\
{\color{greenunder}\text{inductive assumption}:}\quad & =\text{pu}_{M^{\prime}}(r)\oplus_{M^{\prime}}\big(t\triangleright\text{brun}^{M}\bef\phi\big)\quad.
\end{align*}
The remaining difference in Eq.~(\ref{eq:phi-psi-comparison-derivation1})
is:
\[
...
\[
\phi(p^{:M^{R}}\oplus_{M}q^{:M^{R}})\overset{?}{=}\phi(p)\oplus_{M^{\prime}}\phi(q)\quad.
\]
So, we write:
\begin{align*}
...
satisfies:
\begin{align*}
{\color{greenunder}\text{left identity law}:}\quad & \text{comb}\,(\text{Nil}\triangleright\text{pu}_{M})(m^{:M^{L^{A}}})=m\quad,\\
{\color{greenunder}\text{right identity law}:}\quad & \text{comb}\,(m^{:M^{L^{A}}})(\text{Nil}\triangleright\text{pu}_{M})=m\quad,\\
...
\begin{align*}
 & \quad{\color{greenunder}\text{expect to equal }m:}\quad\\
 & \text{comb}\,(\text{Nil}\triangleright\text{pu}_{M})(m^{:M^{L^{A}}})\\
...
\end{array}\,\bigg)\\
 & \quad{\color{greenunder}\text{identity law of }M:}\quad\\
 & =\text{Nil}\triangleright\,\begin{array}{|c||c|}
 & M^{L^{A}}\\
...
\begin{align*}
 & \quad{\color{greenunder}\text{expect to equal }m:}\quad\\
 & \text{comb}\,(m^{:M^{L^{A}}})(\text{Nil}\triangleright\text{pu}_{M})\\
...
{\color{greenunder}\text{and }\xi\text{ is defined by}:}\quad & n\triangleright\xi\triangleq\,\begin{array}{|c||c|}
 & M^{L^{A}}\\
\hline \bbnum 1 & 1\rightarrow n\\
A\times M^{L^{A}} & a\times p\rightarrow\text{pu}_{M}\big(\bbnum 0^{:\bbnum 1}+a\times\overline{\text{comb}}\,(p)(n)\big)
\end{array}\quad.\nonumber 
...
The right-hand side:
\begin{align*}
 & \text{comb}\,\big(\text{comb}\,(x)(y)\big)(z)=\text{comb}\,(x)(y)\triangleright(z\triangleright\xi)^{\uparrow M}\bef\text{ftn}_{M}\\
 & =x\triangleright(y\triangleright\xi)^{\uparrow M}\bef\gunderline{\text{ftn}_{M}\bef(z\triangleright\xi)^{\uparrow M}}\bef\text{ftn}_{M}\\
...
\begin{align*}
 & \big(\text{comb}\,(p)(q)\big)\triangleright\text{ftn}_{T}=p\triangleright\text{flm}_{M}(q\triangleright\xi)\triangleright\text{ftn}_{T}\\
 & =p\triangleright\text{flm}_{M}(q\triangleright\xi)\bef\text{flm}_{M}(\text{prod})\\
...
\end{array}\,\bef\text{flm}_{M}(q\triangleright\text{ftn}_{T}\triangleright\xi)\\
 & =\big(\text{comb}\,(h)(t\triangleright\text{ftn}_{T})\big)\triangleright\text{flm}_{M}(q\triangleright\text{ftn}_{T}\triangleright\xi)\quad.
\end{align*}
...
\begin{align*}
 ...
 & \quad{\color{greenunder}\text{definition of }\xi:}\quad\\
 & =(\bbnum 0+h\times t)\triangleright\,\begin{array}{||c|}
...
\end{align*}
\begin{align*}
 & =\big(\bbnum 0+h\times\overline{\text{comb}}\,(t)(q)\big)\triangleright\text{prod}\\
...
{\color{greenunder}\text{inductive assumption}:}\quad & =\gunderline{\text{comb}\,(h)\big(\text{comb}}\,(t\triangleright\text{ftn}_{T})(q\triangleright\text{ftn}_{T})\big)\\
...
{\color{greenunder}\text{definition~(\ref{eq:definition-of-comb-for-effectful-list})}:}\quad & =\big(\text{comb}\,(h)((t\triangleright\text{ftn}_{T}))\big)\triangleright\text{flm}_{M}(q\triangleright\text{ftn}_{T}\triangleright\xi)\quad.
\end{align*}
...
\begin{align*}
 & \text{comb}\,(p\triangleright\psi^{\uparrow M}\bef\phi)(q\triangleright\psi^{\uparrow M}\bef\phi)\\
 & =p\triangleright\gunderline{\psi^{\uparrow M}\bef\phi}\bef\text{flm}_{M^{\prime}}\big(q\triangleright\gunderline{\psi^{\uparrow M}\bef\phi}\bef\xi\big)\\
...
\end{align*}
To show that the law holds, it remains to prove the following equality:
\[
...
\begin{align*}
 & (q\triangleright\xi)\bef\phi\bef\psi^{\uparrow M^{\prime}}=\,\begin{array}{||c|}
...
\end{array}\quad,\\
 & \psi\bef(q\triangleright\phi\triangleright\psi^{\uparrow M^{\prime}}\triangleright\xi)\\
 & \quad=\,\begin{array}{||cc|}
\text{id} & \bbnum 0\\
...
$\psi$, we find:
\begin{align*}
 & \big(\bbnum 0+a\times\overline{\text{comb}}\,(p)(q)\big)\triangleright\psi\\
 & =\big(\bbnum 0+a\times\overline{\text{comb}}\,(p)(q)\big)\triangleright\,\begin{array}{||cc|}
\text{id} & \bbnum 0\\
\bbnum 0 & \text{id}\boxtimes(\phi\bef\overline{\psi}^{\uparrow M^{\prime}})
\end{array}\\
...
satisfies the laws given in Eqs.~(\ref{eq:identity-laws-of-prod-for-effectful-list})\textendash (\ref{eq:monadic-naturality-law-of-prod-for-list-transformer}).

\subparagraph{Proof}

To verify the left identity law of \lstinline!prod!, which is $\text{pu}_{L}\bef\text{prod}=\text{id}$,
note that $\text{Nil}\triangleright\text{prod}=\text{Nil}\triangleright\text{pu}_{M}$
and write:
\begin{align*}
 & \text{pu}_{L}\bef\text{prod}\\
 & =\,\begin{array}{||cc|}
\bbnum 0 & a\rightarrow a\times(\text{Nil}\triangleright\text{pu}_{M})\end{array}\,\bef\,\begin{array}{||c|}
1\rightarrow\text{Nil}\triangleright\text{pu}_{M}\\
...
\begin{align*}
 ...
 & \quad{\color{greenunder}\text{inductive assumption}:}\quad\\
 & =\text{comb}\,(a\triangleright\text{pu}_{T})(m\triangleright\gunderline{\text{pu}_{M}\bef\text{ftn}_{M}})=\text{comb}\,(a\triangleright\text{pu}_{T})(m)\\
 & \quad{\color{greenunder}\text{definition of }\text{comb}:}\quad\\
 & =a\triangleright\text{pu}_{L}\bef\gunderline{\text{pu}_{M}\bef\text{flm}_{M}}\bigg(\,\begin{array}{||c|}
1\rightarrow m\\
a\times p\rightarrow\text{pu}_{M}\big(\bbnum 0+a\times\overline{\text{comb}}\,(p)(m)\big)
\end{array}\,\bigg)\\
 & \quad{\color{greenunder}\text{identity law of }M:}\quad\\
 & =\gunderline{a\triangleright\text{pu}_{L}}\triangleright\,\begin{array}{||c|}
1\rightarrow m\\
a\times p\rightarrow\text{pu}_{M}\big(\bbnum 0+a\times\overline{\text{comb}}\,(p)(m)\big)
\end{array}\\
 & \quad{\color{greenunder}\text{definition of }\text{pu}_{L}:}\quad\\
...
a\times p\rightarrow\text{pu}_{M}\big(\bbnum 0+a\times\overline{\text{comb}}\,(p)(m)\big)
\end{array}\\
 & \quad{\color{greenunder}\text{apply matrix function}:}\quad\\
 & =\text{pu}_{M}\big(\bbnum 0+a\times\overline{\text{comb}}\,(p)(\text{Nil}\triangleright\text{pu}_{M})\big)=\text{pu}_{M}(\bbnum 0+a\times m)\quad.
\end{align*}
...
\begin{align*}
 & \psi\bef\text{prod}_{M^{\prime},L^{\prime}}\\
 & =\,\begin{array}{||cc|}
\text{id} & \bbnum 0\\
...
both sides above:
\begin{align*}
{\color{greenunder}\text{left-hand side}:}\quad & (\psi^{\uparrow M}\bef\phi)^{\overline{\uparrow L}\uparrow M}\bef\psi^{\uparrow M}\bef\phi\bef\gunderline{\overline{\text{ftn}}_{T^{\prime}}}\\
 & \quad=\gunderline{(\psi^{\uparrow M}\bef\phi)^{\overline{\uparrow L}\uparrow M}\bef\psi^{\uparrow M}\bef\phi}\bef\text{flm}_{M^{\prime}}(\overline{\text{prod}})\\
...
\begin{align*}
{\color{greenunder}\text{inductive hypothesis}:}\quad & \overline{\text{prod}}_{M,L}\bef\psi^{\uparrow M}\bef\phi\overset{!}{=}(\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\bef\overline{\text{prod}}_{M^{\prime},L^{\prime}}\quad,\\
{\color{greenunder}\text{apply }\text{flm}_{M^{\prime}}:}\quad & \text{flm}_{M^{\prime}}\big(\overline{\text{prod}}_{M,L}\bef\psi^{\uparrow M}\bef\phi\big)\overset{!}{=}\text{flm}_{M^{\prime}}\big((\psi^{\uparrow M}\bef\phi)^{\uparrow L}\bef\psi\bef\overline{\text{prod}}_{M^{\prime},L^{\prime}}\big)\quad.
\end{align*}
...
monad is:
\[
(\text{State}\varangle M)^{A}\triangleq S\rightarrow M^{A\times S}\quad.
\]
...
In the next derivations, we fix a foreign monad $M$ and denote $T\triangleq\text{State}\varangle M$.

\paragraph{Monad laws}

We need to show that $T$ is a lawful monad, assuming that $M$ is
one. The monad methods for $T$ are defined by Eqs.~(\ref{eq:pure-for-stateT-transformer})\textendash (\ref{eq:flatmap-for-stateT-transformer}).
...
The type signature for a $T$-Kleisli function is:
\[
A\rightarrow T^{B}=A\rightarrow S\rightarrow M^{B\times S}\quad.
\]
...
\]
The proper $T$-Kleisli composition ($\diamond_{_{T}}$) is then defined
by uncurrying:
\[
...
\]
We can now show that all the monad laws for $T$ hold due to $M$\textsf{'}s
monad laws:
\begin{align*}
...
When $M=\text{Id}$, the type of $(\text{State}\varangle M)^{A}$
becomes equal to $\text{State}^{A}$, and the monad methods become
equal to those of the \lstinline!State! monad.
...
\end{equation}
We need to show that \lstinline!flift! is a monad morphism $M\leadsto T$.
The identity law:
\begin{align*}
...
of $\diamond_{_{T}}$:
\begin{align*}
 & \text{uncu}\,(f^{:A\rightarrow M^{B}}\bef\text{flift})=\text{uncu}\,(a^{:A}\rightarrow f(a)\triangleright\text{flift})\\
 & =\text{uncu}\,\big(a^{:A}\rightarrow s^{:S}\rightarrow a\triangleright f\triangleright(b^{:B}\rightarrow b\times s)^{\uparrow M}\big)\\
 & =a\times s\rightarrow a\triangleright f\bef(b\rightarrow b\times s)^{\uparrow M}\quad.
\end{align*}
Using this formula, we get:
\begin{align}
 & \text{uncu}\,(f^{:A\rightarrow M^{B}}\bef\text{flift})\,\tilde{\diamond}_{_{T}}\,\text{uncu}\,(g^{:B\rightarrow M^{C}}\bef\text{flift})\nonumber \\
 & =\text{uncu}\,(f^{:A\rightarrow M^{B}}\bef\text{flift})\diamond_{_{M}}\text{uncu}\,(g^{:B\rightarrow M^{C}}\bef\text{flift})\nonumber \\
...
{\color{greenunder}\text{naturality law of }\diamond_{_{M}}:}\quad & =a\triangleright(f\diamond_{_{M}}g)\bef(c\rightarrow c\times s)^{\uparrow M}\\
 & =\left(a\times s\right)\triangleright\text{uncu}\,\big((f\diamond_{_{M}}g)\bef\text{flift}\big)\quad.
\end{align*}
...
monad morphism of type $\text{State}\varangle M\leadsto\text{State}\varangle N$.
For convenience, denote $U\triangleq\text{State}\varangle N$. To
verify the identity law:
\begin{align*}
 & \text{pu}_{T}\bef\text{frun}\,(\phi^{:M\leadsto N})\\
 & =(a^{:A}\rightarrow s^{:S}\rightarrow(a\times s)\triangleright\text{pu}_{M})\bef(p\rightarrow p\bef\phi)\\
...
{\color{greenunder}\text{identity law of }\phi:}\quad & =a\rightarrow s\rightarrow(a\times s)\triangleright\text{pu}_{N}=\text{pu}_{U}\quad.
\end{align*}
...
is a monad morphism of type $T\leadsto U$. We need to show that:
\[
\big(f^{:A\rightarrow T^{B}}\bef r\big)\diamond_{_{U}}(g^{:B\rightarrow T^{C}}\bef r)\overset{?}{=}(f\diamond_{_{T}}g)\bef r\quad.
\]
Express this through uncurried functions and the operations $\diamond_{_{T}}$
and $\tilde{\diamond}_{_{U}}$:
\[
\text{uncu}\,\big(f^{:A\rightarrow T^{B}}\bef r\big)\,\tilde{\diamond}_{_{U}}\,\text{uncu}\,(g^{:B\rightarrow T^{C}}\bef r)\overset{?}{=}\text{uncu}\,\big((f\diamond_{_{T}}g)\bef r\big)\quad.
\]
Uncurrying a function composition such as $f\bef r$ works like this:
\begin{align}
 & \text{uncu}\,\big(f^{:A\rightarrow S\rightarrow M^{B\times S}}\bef r^{:T^{B}\leadsto U^{B}}\big)=a^{:A}\times s^{:S}\rightarrow s\triangleright(\gunderline{a\triangleright f\bef r})\nonumber \\
 & =a^{:A}\times s^{:S}\rightarrow s\triangleright(f(a)\triangleright\gunderline r)\nonumber \\
...
\[
\text{uncu}\,\big((f\diamond_{_{T}}g)\bef r\big)=\text{uncu}\,(f\diamond_{_{T}}g)\bef\phi=\big(\text{uncu}\left(f\right)\diamond_{_{M}}\text{uncu}\left(g\right)\big)\bef\phi\quad.
\]
We can now proceed to simplify the left-hand side of the composition
law:
\begin{align*}
 & \gunderline{\text{uncu}\,\big(f^{:A\rightarrow T^{B}}\bef r\big)}\,\tilde{\diamond}_{_{U}}\,\gunderline{\text{uncu}\,(g^{:B\rightarrow T^{C}}\bef r)}\\
{\color{greenunder}\text{use Eq.~(\ref{eq:uncurry-state-composition-derivation1})}:}\quad & =\big(\text{uncu}\left(f\right)\bef\phi\big)\,\gunderline{\tilde{\diamond}_{_{U}}}\,\big(\text{uncu}\left(g\right)\bef\phi\big)\\
{\color{greenunder}\text{definition of }\tilde{\diamond}_{_{U}}:}\quad & =\big(\text{uncu}\left(f\right)\bef\phi\big)\diamond_{_{N}}\big(\text{uncu}\left(g\right)\bef\phi\big)\\
{\color{greenunder}\text{composition law of }\phi:}\quad & =\big(\text{uncu}\left(f\right)\diamond_{_{M}}\text{uncu}\left(g\right)\big)\bef\phi=\text{uncu}\,\big((f\diamond_{_{T}}g)\bef r\big)\quad.
\end{align*}
This is now equal to the right-hand side of the composition law.
...
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{pu}_{M}:}\quad & \text{pu}_{P}\bef\text{brun}\,(s_{0})\\
 & =\big(a\rightarrow s\rightarrow(a\times s)\triangleright\text{pu}_{M}\big)\bef(p\rightarrow p(s_{0})\triangleright\pi_{1}^{\uparrow M})\\
...
\begin{align*}
 & \big(f\bef\text{brun}\,(s_{0})\big)\diamond_{_{M}}\big(g\bef\text{brun}\,(s_{0})\big)\\
 & =\big((a\rightarrow a\times s_{0})\bef\text{uncu}\,(f)\bef\gunderline{\pi_{1}^{\uparrow M}\big)\,\diamond_{_{M}}}\big(g\bef\text{brun}\,(s_{0})\big)\\
...
arbitrary $m^{:M^{A}}$:
\begin{align*}
{\color{greenunder}\text{expect to equal }m\triangleright\phi\triangleright\text{flift}^{N}:}\quad & m\triangleright\text{flift}^{M}\triangleright\text{frun}\left(\phi\right)\\
 & =m\triangleright\text{flift}^{M}\triangleright(p\rightarrow p\bef\phi)=(m\triangleright\text{flift}^{M})\bef\phi\\
{\color{greenunder}\text{use Eq.~(\ref{eq:state-transformer-def-of-flift})}:}\quad & =\big(s\rightarrow m\triangleright(a\rightarrow a\times s)^{\uparrow M}\big)\bef\phi\\
 & =s\rightarrow m\triangleright\gunderline{(a\rightarrow a\times s)^{\uparrow M}\bef\phi}\\
...
{\color{greenunder}\text{compute composition}:}\quad & =p\rightarrow s_{0}\triangleright p\bef\gunderline{\phi\bef\pi_{1}^{\uparrow N}}=\gunderline{p\rightarrow s_{0}\triangleright p\bef\pi_{1}^{\uparrow M}}\bef\phi\\
 & =\text{brun}\,(s_{0})\bef\phi\quad.
\end{align*}
...
is defined by:
\[
(\text{Cont}\varangle M)^{A}\triangleq(A\rightarrow M^{R})\rightarrow M^{R}\quad.
\]
Unlike most other monad transformers, $\text{Cont}\varangle$ is not
covariant in the foreign monad $M$: the type $(\text{Cont}\varangle M)^{A}$
contains $M$ both in covariant and contravariant positions. (In the
language of categories, $\text{Cont}\varangle$ is not a functor in
the category of monads.) As a result, we cannot define the foreign
runner or the base lift functions. This limits the usability of the
...
The transformed monad $\text{Cont}\varangle M$ differs from the base
...
The monad\textsf{'}s methods work the same way for all result types, so $\text{Cont}\varangle M$
is a monad for all monads $M$. (This would remain true even if the
type constructor $M$ were not a functor!)

\paragraph{Identity laws}

If we set $M=\text{Id}$, the type of $(\text{Cont}\varangle M)^{A}$
is equivalent to the base monad, $\left(A\rightarrow R\right)\rightarrow R$.

\paragraph{Lifting laws}

The foreign lift function (\lstinline!flift!) is equal to $M$\textsf{'}s
\lstinline!flatMap!:
\begin{align*}
 & \text{flift}:M^{A}\rightarrow(A\rightarrow M^{R})\rightarrow M^{R}\quad,\\
 & \text{flift}\triangleq\text{flatMap}_{M}=m^{:M^{A}}\rightarrow f^{:A\rightarrow M^{R}}\rightarrow m\triangleright\text{flm}_{M}(f)\quad.
\end{align*}
Denote $T\triangleq\text{Cont}\varangle M$ with a fixed $M$. To
verify that \lstinline!flift! is a monad morphism, we use the flipped
Kleisli formulation for the monad $T$. In that formulation, Kleisli
functions of type $A\rightarrow T^{B}=A\rightarrow(B\rightarrow M^{R})\rightarrow M^{R}$
...
\begin{align}
\text{flip}\,(f^{:A\rightarrow M^{B}}\bef\text{flift}) & =p^{:B\rightarrow M^{R}}\rightarrow a^{:A}\rightarrow f(a)\triangleright\text{flm}_{M}(p)\nonumber \\
 & =p\rightarrow f\bef\text{flm}_{M}(p)=p\rightarrow f\diamond_{_{M}}p\quad.\label{eq:continuation-transformer-flip-derivation2}
\end{align}

The identity law of \lstinline!flift! is now easy to verify. The
flipped \lstinline!pure! method is:
\[
...
where we defined:
\[
\text{lift}_{L\varangle M,H}\triangleq f^{:A\rightarrow(L\varangle M)^{B}}\rightarrow h^{:B\rightarrow(L\varangle M)^{Q}}\rightarrow f\diamond_{_{L\varangle M}}h\quad.
\]
...
\begin{align*}
 & p:\big((A\rightarrow L^{Q})\rightarrow L^{A}\big)\rightarrow(A\rightarrow N^{Q})\rightarrow N^{A}\quad,\\
 & q:\big((A\rightarrow N^{Q})\rightarrow N^{A}\big)\rightarrow(A\rightarrow L^{Q})\rightarrow L^{A}\quad,
\end{align*}
...
\begin{align*}
{\color{greenunder}\text{expect to equal }\text{id}:}\quad & p\bef q=\big(u\rightarrow\gunderline{v\rightarrow(v\bef s)\triangleright u\bef r}\big)\bef\big(\gunderline t\rightarrow w\rightarrow(w\bef r)\triangleright\gunderline t\bef s\big)\\
...
{\color{greenunder}\text{apply function}:}\quad & =u\rightarrow w\rightarrow(w\bef\,\gunderline{r\bef s})\triangleright u\bef\,\gunderline{r\bef s}=u\rightarrow(\gunderline{w\rightarrow w\,\triangleright}\,u)\\
 & =u\rightarrow u=\text{id}\quad.
\end{align*}
...
and $\text{lift}_{N,H}$:
\begin{align*}
 & \text{lift}_{L,H}:(A\rightarrow L^{B})\rightarrow(B\rightarrow L^{Q})\rightarrow A\rightarrow L^{Q}\quad,\\
{\color{greenunder}\text{and so}:}\quad & \quad g^{:B\rightarrow L^{Q}}\triangleright\text{lift}_{L,H}(f^{:A\rightarrow L^{B}})=f\diamond_{_{L}}g\quad,\\
 & \text{lift}_{N,H}:(A\rightarrow N^{B})\rightarrow(B\rightarrow N^{Q})\rightarrow A\rightarrow N^{Q}\quad,\\
{\color{greenunder}\text{and so}:}\quad & \quad g^{:B\rightarrow N^{Q}}\triangleright\text{lift}_{N,H}(f^{:A\rightarrow N^{B}})=f\diamond_{_{N}}g\quad.
\end{align*}
...
\begin{align*}
{\color{greenunder}\text{expect }(p\diamond_{_{M}}q)\bef\text{flift}:}\quad & \big(p^{:A\rightarrow M^{B}}\bef\text{flift}\big)\diamond_{_{T}}\big(q^{:B\rightarrow M^{C}}\bef\text{flift}\big)\\
 & =\text{flip}\,\big(\gunderline{\text{flip}\,(p\bef\text{flift})}\tilde{\diamond}_{_{T}}\gunderline{\text{flip}\,(q\bef\text{flift})}\big)\\
 & =\text{flip}\,\big((\_\rightarrow p\bef\text{flift}_{L})\,\tilde{\diamond}_{_{T}}(\_\rightarrow q\bef\text{flift}_{L})\big)\\
 & =\text{flip}\,\big(\_\rightarrow\gunderline{(p\bef\text{flift}_{L})\diamond_{_{L\varangle M}}(q\bef\text{flift}_{L})}\big)\\
...
monad}\footnote{See \texttt{\href{https://begriffs.com/posts/2016-02-04-difference-lists-and-codennsity.html}{https://begriffs.com/posts/2016-02-04-difference-lists-and-codennsity.html}}
...
\begin{align*}
 & \text{inC}:F^{A}\rightarrow\forall R.\,(A\rightarrow F^{R})\rightarrow F^{R}\quad,\\
 & \text{inC}^{R,A}\triangleq f^{:F^{A}}\rightarrow k^{:A\rightarrow F^{R}}\rightarrow f\triangleright\text{flm}_{F}(k)\quad,\\
 & \text{outC}:\big(\forall R.\,(A\rightarrow F^{R})\rightarrow F^{R}\big)\rightarrow F^{A}\quad,\\
 & \text{outC}^{A}\triangleq d^{:\forall R.\,(A\rightarrow F^{R})\rightarrow F^{R}}\rightarrow d^{A}(\text{pu}_{F}^{:A\rightarrow F^{A}})\quad.
\end{align*}
...
\begin{align*}
 & f^{:\forall R.\,A\rightarrow(B\rightarrow F^{R})\rightarrow F^{R}}\diamond_{_{\text{Cod}}}g^{:\forall S.\,B\rightarrow(C\rightarrow F^{S})\rightarrow F^{S}}\triangleq\\
 & \quad\quad\forall T.\,a^{:A}\rightarrow k^{:C\rightarrow F^{T}}\rightarrow f^{T}(a)\big(b^{:B}\rightarrow g^{T}(b)(k)\big)\quad.
\end{align*}
...
{\color{greenunder}\text{right-hand side}:}\quad & a^{:A}\triangleright\big((f\bef\text{inC})\diamond_{_{\text{Cod}}}\!(g\bef\text{inC})\big)\\
 & \quad=k\rightarrow(f\bef\text{inC})^{T}(a)\big(b\rightarrow(g\bef\text{inC})^{T}(b)(k)\big)\quad.
\end{align*}
...
\begin{align*}
 & k\rightarrow(f\bef\text{inC})^{T}(a)\big(\gunderline{b\rightarrow b\,\triangleright\,}g\bef\text{flm}_{F}(k)\big)\\
 & =k\rightarrow\gunderline{(f\bef\text{inC})(a)}(g\bef\text{flm}_{F}(k))\\
...
results:
\begin{align*}
{\color{greenunder}\text{left-hand side}:}\quad & a\triangleright(f\diamond_{_{\text{Cod}}}g)\bef\text{outC}\\
 & \quad=\big(\forall T.\,k\rightarrow f^{T}(a)\big(b\rightarrow g^{T}(b)(k)\big)\big)\bef(d\rightarrow d(\text{pu}_{F}))\\
 & \quad=\big(\forall T.\,k\rightarrow f^{T}(a)\big(b\rightarrow g^{T}(b)(k)\big)\big)(\text{pu}_{F})\\
 & \quad=f(a)(b\rightarrow g(b)(\text{pu}_{F}))\quad.\\
{\color{greenunder}\text{right-hand side}:}\quad & a\triangleright\big((f\bef\text{outC})\diamond_{_{F}}\!(g\bef\text{outC})\big)=a\triangleright(f\bef\text{outC})\bef\text{flm}_{F}(g\bef\text{outC})\\
 & \quad=a\triangleright f\triangleright\text{outC}\triangleright\text{flm}_{F}(g\bef\text{outC})\\
 & \quad=f(a)(\text{pu}_{F})\triangleright\text{flm}_{F}\big(b\rightarrow g(b)(\text{pu}_{F})\big)\quad.
\end{align*}
...
\[
s:\bbnum 1+P\rightarrow(\bbnum 1+P)\times(\bbnum 1+P)\quad,\quad\quad s\triangleq r^{:\bbnum 1+P}\rightarrow r\times(1+\bbnum 0)\quad.
\]
...
\[
(\text{Cod}^{F}\varangle M)^{A}\triangleq\forall R.\,(A\rightarrow M^{F^{R}})\rightarrow M^{F^{R}}\quad.
\]
This assumes a lawful foreign monad $M$, while $F$ may be any type
constructor. The transformer $\text{Cod}\varangle$ does not have
the base lift, foreign runner, or base runner methods.
...
where we replace $F$ by $M\circ F$.
...
\begin{align*}
 & \text{flift}:M^{A}\rightarrow\forall R.\,(A\rightarrow M^{F^{R}})\rightarrow M^{F^{R}}\quad,\\
 & \text{flift}\triangleq\forall R.\,m^{:M^{A}}\rightarrow k^{:A\rightarrow M^{F^{R}}}\rightarrow m\triangleright\text{flm}_{M}(k)\quad.
\end{align*}
...
\[
\text{inC}:M^{A}\rightarrow\forall S.\,(A\rightarrow M^{S})\rightarrow M^{S}\quad,\quad\text{inC}\triangleq\forall S.\,m^{:M^{A}}\rightarrow k^{:A\rightarrow M^{S}}\rightarrow m\triangleright\text{flm}_{M}(k)\quad,
\]
...
\[
T_{1}^{A}=(\text{Sel}\varangle M)^{A}\triangleq(M^{A}\rightarrow Q)\rightarrow M^{A}\quad.
\]
...
Even when $\text{Cod}^{F,\bullet}$ is equivalent to another known
...
(which we usually denote by $G\varangle$) because $\text{Cod}\varangle M$
is not covariant in the foreign monad $M$, while many \textsf{``}standard\textsf{''}
transformers are. For instance, the standard transformers for \lstinline!Id!,
...
\[
\sigma=\Delta\bef\left(\pi_{2}\boxtimes\pi_{1}\right)\quad.
\]
The functions $\Delta$, $\pi_{1}$, and $\pi_{2}$ are monad morphisms
according to Statements~\ref{subsec:Statement-M-to-M-times-M-is-monadic-morphism}
and~\ref{subsec:Statement-projection-is-monadic-morphism}. The function
$\pi_{1}\boxtimes\pi_{2}$ is a monad morphism by Statement~\ref{subsec:Statement-pair-product-of-monadic-morphisms}.
...
We show that this is an identity function to finish the proof:
\begin{align*}
...
\begin{align*}
 & \text{flm2}_{M}:(A\rightarrow M^{B}\times M^{B})\rightarrow M^{A}\rightarrow M^{B}\times M^{B}\quad,\\
 & \text{flm2}_{M}\triangleq f\rightarrow\Delta\bef\big(\text{flm}_{M}(f\bef\pi_{1})\boxtimes\text{flm}_{M}(f\bef\pi_{2})\big)\quad.
\end{align*}
...
To see that, let us compare Eqs.~(\ref{eq:fuseOut-def}) and~(\ref{eq:runner-phi-def})
...
\subsubsection{Example \label{subsec:Example-fo-fi-not-id}\ref{subsec:Example-fo-fi-not-id}\index{examples}}
...
{\color{greenunder}\text{compose under }^{\uparrow R}:}\quad & =a\rightarrow\text{wu}_{R}\triangleright\big(1\rightarrow(\_^{:A}\rightarrow1)(a)\big)^{\uparrow R}=a\rightarrow\text{wu}_{R}\triangleright(\gunderline{1\rightarrow1})^{\uparrow R}\\
...
as:
\begin{align*}
 & \text{split}:\left(A\rightarrow B\times C\right)\rightarrow\left(A\rightarrow B\right)\times\left(A\rightarrow C\right)\quad,\\
 & \text{split}\triangleq f^{:A\rightarrow B\times C}\rightarrow(f\bef\pi_{1})\times(f\bef\pi_{2})\quad.
\end{align*}
...
\begin{align*}
 & \text{merge}:\left(A\rightarrow B\right)\times\left(A\rightarrow C\right)\rightarrow A\rightarrow B\times C\quad,\\
 & \text{merge}\triangleq f^{:A\rightarrow B}\times g^{:A\rightarrow C}\rightarrow a^{:A}\rightarrow f(a)\times g(a)\quad.
\end{align*}
...
based on the methods and results explained in a paper by \index{Mauro Jaskelioff}M.~Jaskelioff,\footnote{See \texttt{\href{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}{https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.219.5365}}}
...
A \textbf{\textsf{``}simple\textsf{''} operation} is a natural transformation of type
$\forall A.\,F^{A}\rightarrow M^{A}$.

A \textbf{\textsf{``}standard\textsf{''} operation} is a natural transformation of
type $\forall A.\,F^{M^{A}}\rightarrow M^{A}$.\\
$\square$
...
The next statement shows that \textsf{``}fixed-type\textsf{''} operations are a subset
of \textsf{``}simple\textsf{''} operations, while \textsf{``}simple\textsf{''} operations are a subset
of \textsf{``}standard\textsf{''} operations.
...
\begin{align*}
 & \text{sop}:P\times\left(Q\rightarrow A\right)\rightarrow M^{A}\quad,\quad\quad\text{sop}=p^{:P}\times q^{:Q\rightarrow A}\rightarrow p\triangleright\text{fop}\triangleright q^{\uparrow M}\quad,\\
 & \text{fop}:P\rightarrow M^{Q}\quad,\quad\quad\text{fop}=p^{:P}\rightarrow\text{sop}^{Q}(p\times\text{id}^{Q\rightarrow Q})\quad.
\end{align*}
...
\lstinline!sappend!:
\begin{align*}
 & \text{sappend}:A\times A\rightarrow\text{List}^{A}\quad,\\
 & \text{sappend}\left(a_{1}\times a_{2}\right)\triangleq[a_{1},a_{2}]=\text{pu}_{\text{List}}(a_{1})\,\pplus\,\text{pu}_{\text{List}}(a_{2})\quad,\\
...
\begin{align*}
 & \text{op}:M^{A}\rightarrow M^{A}\quad,\\
 & \text{op}\triangleq\text{sop}\bef\text{ftn}_{M}=(a\times w\rightarrow a\times w\times w_{0})\bef\text{ftn}_{M}=a\times w\rightarrow a\times(w\oplus w_{0})\quad.
\end{align*}
...
and the corresponding \textsf{``}standard\textsf{''} operations $\text{op}_{M}\triangleq\text{sop}_{M}\bef\text{ftn}_{M}$
and $\text{op}_{P}\triangleq\text{sop}_{P}\bef\text{ftn}_{P}$. Then
$\text{op}_{P}$ is expressed via $\text{op}_{M}$ by:
\begin{equation}
...
{\color{greenunder}\text{naturality law of }\text{sop}_{M}:}\quad & =p^{\uparrow F}\bef\gunderline{\text{sop}_{M}\bef p}\bef\text{ftn}_{P}=p^{\uparrow F}\bef\gunderline{\text{sop}_{P}\bef\text{ftn}_{P}}\\
 & =p^{\uparrow F}\bef\text{op}_{P}\quad.
\end{align*}
$\square$
...
\begin{align*}
 & \forall(A,R).\,F^{A}\rightarrow(A\rightarrow K^{R})\rightarrow L^{R}\cong\forall(A,R).\,(A\rightarrow K^{R})\rightarrow F^{A}\rightarrow L^{R}\\
 & \quad\quad=\forall R.\,\big(\forall A.\,(A\rightarrow K^{R})\rightarrow H^{A,R}\big)\quad,
\end{align*}
where we defined $H^{A,R}\triangleq F^{A}\rightarrow L^{R}$. The
type constructor $H^{A,R}$ is contravariant in $A$. Since we assume
the naturality laws to hold, we are dealing with functions of type
$\forall A.\,(A\rightarrow K^{R})\rightarrow H^{A,R}$ that are natural
transformations between contrafunctors. So, we may use the contravariant
Yoneda identity (Statement~\ref{subsec:Statement-contravariant-yoneda-identity-for-types})
...
\]
It remains to substitute the definition of $H$ and to rename $R$
to $A$:
\[
...
\begin{align*}
 & \text{frun}_{L}(\text{inC}):L\varangle M\leadsto L\varangle\text{Cod}^{M}\cong P\leadsto P^{\prime}\quad,\\
 & \text{frun}_{L}(\text{outC}):L\varangle\text{Cod}^{M}\leadsto L\varangle M\cong P^{\prime}\leadsto P\quad.
\end{align*}
One way of converting $\text{scop}_{P^{\prime}}$ into a $P$-operation
would be to compose with the function $\text{flift}_{L}(\text{outC})$,
which creates a \textsf{``}simple\textsf{''} operation $\text{sop}_{P}$ of type $F^{A}\rightarrow P^{A}$.
...
as:
\begin{align*}
 & \text{sop}_{P}\triangleq\text{scop}_{P^{\prime}}\bef\text{flift}_{L}(\text{outC})\\
 & =\text{scop}\bef\gunderline{\text{flift}_{L}\bef\text{frun}_{L}}(\text{outC})\\
{\color{greenunder}\text{monadic naturality law of }\text{flift}_{L}:}\quad & =\text{scop}\bef\text{outC}\bef\text{flift}_{L}\\
 & =(f\rightarrow k\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op})\bef\text{outC}\bef\text{flift}_{L}\\
...
a correct lifting of \lstinline!op!. For a correct lifting, the equivalence
between \lstinline!op! and \lstinline!scop! must be expressed differently:
...
substituting \lstinline!cop! and \lstinline!scop!:
\begin{align*}
...
\end{align*}
\begin{align*}
 & \quad{\color{greenunder}\text{use Eq.~(\ref{eq:monatron-scop-via-op}) and }^{\uparrow\text{Cod}}:}\quad\\
 & =(f\rightarrow k\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op})\bef(c\rightarrow k\rightarrow c(\text{inC}\bef k))\bef\text{ftn}_{\text{Cod}}\bef\text{outC}
\end{align*}
\begin{align*}
...
towards $\text{ftn}_{P^{\prime}}$:
\begin{align*}
{\color{greenunder}\text{expect to equal }(\text{op}\bef\text{flift}_{L}):}\quad & \text{flift}_{L}^{\uparrow F}\bef\text{op}_{P}\\
 & =\text{flift}_{L}^{\uparrow F}\bef(\text{frun}_{L}(\text{inC}))^{\uparrow F}\bef\text{cop}_{P^{\prime}}\bef\text{frun}_{L}(\text{outC})\\
...
Begin by substituting $\text{op}=\text{sop}\bef\text{ftn}_{M}$ into
Eq.~(\ref{eq:monatron-scop-via-op}) and rewriting \lstinline!scop!
in terms of \lstinline!inC!:
\begin{align*}
 & \text{scop}\triangleq f^{:F^{A}}\rightarrow k^{:A\rightarrow M^{R}}\rightarrow f\triangleright k^{\uparrow F}\triangleright\text{op}\\
 & =f\rightarrow k\rightarrow f\triangleright\gunderline{k^{\uparrow F}\bef\text{sop}}\bef\text{ftn}_{M}\\
{\color{greenunder}\text{naturality law of }\text{sop}:}\quad & =f\rightarrow k\rightarrow f\triangleright\text{sop}\gunderline{\bef k^{\uparrow M}\bef\text{ftn}_{M}}\\
 & =f\rightarrow k\rightarrow f\triangleright\text{sop}\bef\text{flm}_{M}(k)\\
...
\end{align*}
Now we rewrite the right-hand side of Eq.~(\ref{eq:monatron-op-P-via-cop})
:
\begin{align*}
...
{\color{greenunder}\text{definition of }\text{scop}_{P^{\prime}}:}\quad & =\text{scop}\bef\text{flift}_{L}^{\text{Cod}}\bef(\text{frun}_{L}(\text{inC}))^{\uparrow P^{\prime}}\bef\text{ftn}_{P^{\prime}}\bef\text{frun}_{L}(\text{outC})
\end{align*}
\begin{align*}
 & \quad{\color{greenunder}\text{substitute }\text{scop}=\text{sop}\bef\text{inC}:}\quad\\
 & =\text{sop}\bef\gunderline{\text{inC}\bef\text{flift}_{L}^{\text{Cod}}}\bef(\text{frun}_{L}(\text{inC}))^{\uparrow P^{\prime}}\bef\text{ftn}_{P^{\prime}}\bef\text{frun}_{L}(\text{outC})\\
 & \quad{\color{greenunder}\text{monadic naturality law of }\text{flift}_{L}:}\quad\\
 & =\text{sop}\bef\text{flift}_{L}\bef\gunderline{\text{frun}_{L}(\text{inC})\bef(\text{frun}_{L}(\text{inC}))^{\uparrow P^{\prime}}\bef\text{ftn}_{P^{\prime}}}\bef\text{frun}_{L}(\text{outC})\\
 & \quad{\color{greenunder}\text{composition law of }\text{frun}_{L}(\text{inC}):}\quad\\
 & =\text{sop}\bef\text{flift}_{L}\bef\text{ftn}_{P}\bef\gunderline{\text{frun}_{L}(\text{inC})\bef\text{frun}_{L}(\text{outC})}\\
 & \quad{\color{greenunder}\text{composition law of }\text{frun}_{L}:}\quad\\
 & =\text{sop}\bef\text{flift}_{L}\bef\text{ftn}_{P}\bef\text{frun}_{L}(\gunderline{\text{inC}\bef\text{outC}})\\
 & \quad{\color{greenunder}\text{use }\text{inC}\bef\text{outC}=\text{id}:}\quad\\
 & =\gunderline{\text{sop}\bef\text{flift}_{L}}\bef\text{ftn}_{P}=\text{sop}_{P}\bef\text{ftn}_{P}\quad.
\end{align*}
$\square$
...
\end{align*}
\begin{align*}
{\color{greenunder}\text{naturality law of }\text{op}:}\quad & \quad=\text{pu}_{M}^{\uparrow F}\bef\text{op}\bef\gunderline{\text{pu}_{N}^{\uparrow M}\bef\text{pu}_{N}}\bef\text{ftn}_{N\circ M}\\
 & \quad=\text{pu}_{M}^{\uparrow F}\bef\text{op}\bef\gunderline{\text{blift}_{M}^{\uparrow M}\bef\text{blift}_{M}\bef\text{ftn}_{N\circ M}}\\
...
\subsubsection{Example \label{subsec:Statement-monatron-lifting-writer-clear-to-base}\ref{subsec:Statement-monatron-lifting-writer-clear-to-base}\index{examples}}
...
operation:
\begin{align*}
 & \text{handle}:\left(E\rightarrow E+A\right)\times\left(E+A\right)\rightarrow E+A\quad,\\
 & \text{handle}\triangleq h^{:E\rightarrow E+A}\times m^{:E+A}\rightarrow m\triangleright\,\begin{array}{|c||c|}
 & E+A\\
\hline E & h\\
...
\begin{align*}
 & \text{handle}_{Q}:(E\rightarrow Q^{A})\times Q^{A}\rightarrow Q^{A}\quad,\\
 & \text{handle}_{Q}\triangleq h^{:E\rightarrow N^{E+A}}\times n^{:N^{E+A}}\rightarrow n\triangleright\text{flm}_{N}\bigg(\,\begin{array}{|c||c|}
 & N^{E+A}\\
\hline E & h\\
A & \text{pu}_{Q}
\end{array}\,\bigg)\quad.
...
To verify the monadic naturality law ($\text{pu}_{N}^{\uparrow F}\bef\text{handle}_{Q}=\text{handle}\bef\text{pu}_{N}$)
using the definitions $\text{blift}_{\text{Either}}\triangleq\text{pu}_{N}$
and $F^{B}\triangleq\left(E\rightarrow B\right)\times B$:
\begin{align*}
 & \text{pu}_{N}^{\uparrow F}\bef\text{handle}_{Q}=\big(h^{:E\rightarrow E+A}\times m^{:E+A}\rightarrow(h\bef\text{pu}_{N})\times(m\triangleright\text{pu}_{N})\big)\bef\text{handle}_{Q}\\
...
and:
\begin{align*}
 & \forall A.\,F^{A}\rightarrow\forall R.\,(A\rightarrow K^{R})\rightarrow L^{R}\\
 & =\forall A.\,A\times W\rightarrow\forall R.\,(A\rightarrow W\rightarrow R)\rightarrow R\times W\quad.
\end{align*}
Transforming the last type via the covariant Yoneda identity, we obtain
the type of \lstinline!collect!:
\begin{align*}
 & \forall A.\,A\times W\rightarrow\forall R.\,(\gunderline{A\rightarrow W\rightarrow}\,R)\rightarrow R\times W\\
 & \cong\forall A.\,A\times W\rightarrow\gunderline{\forall R.\,}(A\times W\rightarrow\gunderline{R)\rightarrow R}\times W\\
...
{\color{greenunder}\text{associativity law of }\text{ftn}_{M}:}\quad & =\gunderline{\text{sop}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}}\bef\text{ftn}_{M}=\text{ftn}_{M}\bef\text{sop}^{\uparrow M}\bef\text{ftn}_{M}\\
 & =\text{ftn}_{M}\bef\text{op}\quad.
\end{align*}
$\square$
...
\end{array}\quad,\\
 & \quad\quad\text{deflate}=\text{flm}_{\text{List}}(\text{defl})\quad.
\end{align*}
...
\begin{align*}
 ...
 & \text{outS}:\big(((A\rightarrow R)\rightarrow R)\rightarrow R\big)\rightarrow A\rightarrow R\quad,\\
 & \text{outS}\triangleq s^{:((A\rightarrow R)\rightarrow R)\rightarrow R}\rightarrow a^{:A}\rightarrow s(k^{:A\rightarrow R}\rightarrow k(a))\quad.
\end{align*}
To prove the injective and surjective properties, we will show that
$\text{inS}\bef\text{outS}=\text{id}$:
\begin{align*}
{\color{greenunder}\text{expect to equal }k:}\quad & k^{:A\rightarrow R}\triangleright\text{inS}\triangleright\text{outS}\\
 & =(c\rightarrow c(k))\triangleright\text{outS}=a\rightarrow(c\rightarrow c(k))(k\rightarrow k(a))\\
...
appears wrapped under a functor $G$ defined by $G^{X}\triangleq(X\rightarrow\text{Cont}^{R,A})\rightarrow\text{Cont}^{R,A}$(here
the types $A$ and $R$ are considered as fixed). Write a function
transforming the type of \lstinline!cc! into the type of \lstinline!callcc!
as:
\[
...
\begin{align*}
 & \text{callcc}\\
 & =h^{:(A\rightarrow\text{Cont}^{R,B})\rightarrow\text{Cont}^{R,A}}\rightarrow\text{cc}\,\big(s^{:\text{Cont}^{R,A\rightarrow R}}\rightarrow h(a^{:A}\rightarrow\_^{:B\rightarrow R}\rightarrow s(k^{:A\rightarrow R}\rightarrow k(a)))\big)\quad.
\end{align*}
...
such as \lstinline!reset! and \lstinline!shift!:\footnote{See the blog posts \texttt{\href{https://blog.bruchez.name/2011/09/continuations-in-scala.html}{https://blog.bruchez.name/2011/09/continuations-in-scala.html}},
\texttt{\href{https://dcsobral.blogspot.com/2009/07/delimited-continuations-explained-in.html}{https://dcsobral.blogspot.com/2009/07/delimited-continuations-explained-in.html}},
and \texttt{\href{https://stackoverflow.com/questions/43695653/cont-monad-shift}{https://stackoverflow.com/questions/43695653/cont-monad-shift}}
for discussions.}
\begin{align*}
...
This exercise asks to prove that a monad stack containing two similar
...
such that it is a left inverse of \lstinline!inS!: 
\[
\text{inS}\bef\text{outS}=\text{id}\quad.
\]
...
and $V\triangleq M\circ T$. We have the \lstinline!swap! functions:
\begin{align*}
\text{sw}_{L,T} & :T\circ L\leadsto L\circ T\quad,\\
\text{sw}_{M,T} & :T\circ M\leadsto M\circ T\quad,
\end{align*}
expressed using the already given methods \lstinline!flift! and \lstinline!blift!
of $T$ as:
\begin{align*}
...
\end{lstlisting}
\begin{align*}
 & \text{finder}:\text{Sel}^{\bbnum 1+P,A}\rightarrow\text{Search}^{P,A}\quad,\\
 & \text{finder}\,(s^{:\text{Sel}^{\bbnum 1+P,A}})\triangleq p^{:A\rightarrow\bbnum 1+P}\rightarrow p(s(p))\triangleright\,\begin{array}{|c||cc|}
 & \bbnum 1 & A\\
\hline \bbnum 1 & \text{id} & \bbnum 0\\
P & \bbnum 0 & s(p)
\end{array}\quad,
...
empty sub-list, such that for example: 
\[
\text{ftn}_{L}(\left[\left[1,2\right],\left[3,4\right],\left[\right],\left[5,6\right]\right])=\left[1,2,3,4\right]\quad.
\]
\index{monads!List monad with invalid truncation@\texttt{List} monad with invalid truncation}(Compare
with Exercise~\ref{subsec:Exercise-1-monads-3-1} where the \lstinline!flatten!
method returns \lstinline!Nil! whenever there are any empty sub-lists.)\index{monads!List monad with empty sub-lists@\texttt{List} monad with empty sub-lists}
...
for a fixed type $X$ and for some functor $L$, the functor $P^{A}\triangleq(A\rightarrow X)\rightarrow L^{X}$
is \emph{not} a monad.
