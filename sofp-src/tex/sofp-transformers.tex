...
Suppose we are given a pointed and co-pointed endofunctor $T$ in
the category of monads. We may apply $T$ to the identity monad; the
result is another monad that we may call $L\triangleq T^{\text{Id}}$.
Then we interpret $T$ as a monad transformer for the base monad $L$.
The definition of $T$ as a pointed and co-pointed functor already
...
that works parametrically in the monad $M$.
...
a \textbf{nondegeneracy law}:\footnote{If the monad morphism laws hold for both \lstinline!flift! and \lstinline!brun!
...
that the only such monad morphism is an identity function. So, we
do not need a separate check for the nondegeneracy law of base runners.}\index{nondegeneracy law!of base runner (brun)@of base runner (\texttt{brun})}
\[
...
laws are $L\varangle M\triangleq L$ (no lifting law) and $L\varangle M\triangleq M$
(no identity law).
...
If we could ignore all the law restrictions, we would rewrite $T^{A}$
as:
\begin{align*}
 & T^{A}=\forall N^{\bullet}.\,(\forall B.\,\gunderline{L^{B}}\rightarrow F^{B})\times(\forall B.\,\gunderline{M^{B}}\rightarrow F^{B})\rightarrow F^{A}\\
...
reduces to the functor co-product ($L^{A}+M^{A}$), which will in
many cases fail to be a monad.
...
We need to demonstrate a monad morphism $\text{flift}_{R}:M\leadsto P\varangle(Q\varangle M)$
for any monad $M$. The lifting law for $Q\varangle$ gives a monad
...
will guarantee that $(M\varangle L)^{A}$ is a lawful monad transformer.
...
that its monad transformer is of the composed-outside kind. Since
$R_{1}$ composes outside, it follows that the monad $R_{1}\circ R_{2}$
is the same as $R_{1}$\textsf{'}s transformer applied to $R_{2}$. By Statement~\ref{subsec:Stacking-two-monads},
the stacking of monad transformers $R_{1}\varangle$and $R_{2}\varangle$
is again a lawful monad transformer.{*}{*}{*}make this logic simpler
and clearer Since the transformers for $R_{1}$ and $R_{2}$ are of
the composed-outside kind, $T_{R_{1}}^{M}=R_{1}\circ M$ and $T_{R_{2}}^{M}=R_{2}\circ M$,
