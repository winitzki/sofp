...
$A$. This book\textsf{'}s code notation $\text{Try}\varangle$ corresponds
to the name \lstinline!TryT!.) We write $(\text{Try}\varangle M)^{A}$
to denote the transformed monad applied to the type parameter $A$. 
...
$L\varangle$ for another base monad $L$? 
...
in $M$ but \emph{not} in $L$. Changing the base monad $L$ to another
monad requires a complete rewrite of $L\varangle M$\textsf{'}s code. 
...
\hline 
{\small{}}\lstinline!List! & {\small{}$L^{A}\triangleq\bbnum 1+A\times L^{A}$} & {\small{}$(L\varangle M)^{A}\triangleq M^{\bbnum 1+A\times T_{L}^{M,A}}$}\tabularnewline
\hline 
...
Show that $\text{Reader}\varangle\text{State}\cong\text{State}\varangle\text{Reader}$.
...
Show that $\text{List}\varangle\text{Reader}\not\cong\text{Reader}\varangle\text{List}$.
...
Show that $\text{Either}\varangle\text{State}\not\cong\text{State}\varangle\text{Either}$.
...
should somehow work with arbitrary Kleisli functions using the given
monads or even sub-stacks. We also expect to be able to run the effects
...
\]
Since the stack type is $P^{A}=(\text{State}\varangle\text{Writer})^{A}=S\rightarrow W\times A\times S$,
...
by $T\triangleq L\varangle M$ for brevity. Assume that we have defined
...
Now consider how we would run a monad stack $L\varangle\text{Cont}$.
We may use the foreign runner with $\theta_{\text{Cont}}$ as:
\[
\text{frun}_{L}(\theta_{\text{Cont}}):L\varangle\text{Cont}\leadsto L\quad.
\]
We could also use those runners to map $L\varangle\text{Cont}\leadsto L\varangle\text{Future}$
and then $L\varangle\text{Future}\leadsto L\varangle\text{Id}\cong L$:
\[
...
should be the same as when running $L\varangle\text{Cont}$ directly
into $L$:
\[
...
This functor maps a monad $M$ to the stack $L\varangle M$; it is
...
\[
\xymatrix{\xyScaleY{1.6pc}\xyScaleX{2.1pc}M^{A}\ar[d]\sb(0.45){\phi}\ar[r]\sp(0.3){\text{flift}} & (L\varangle M)^{A}\ar[d]\sp(0.45){\text{frun}_{L}(\phi)}\\
N^{A}\ar[r]\sp(0.3){\text{flift}} & (L\varangle N)^{A}
}
\]
...
by $^{\uparrow L\varangle}$, we rewrite the law~(\ref{eq:monadic-naturality-law-of-flift})
as:
\[
...
the monad $L$\textsf{'}s transformer has a base runner, $\text{brun}_{L}:L\varangle M\leadsto M$.
...
{*}{*}{*}Since the category of monad stacks is a sub-category of the
category of monads, $L\varangle$ is an \emph{endofunctor}\index{endofunctor!in the category of monads}
...
defined parametrically in the monad $M$. The base runner should also
satisfy a \textbf{nondegeneracy law}:\footnote{If the monad morphism laws hold for both \lstinline!flift! and \lstinline!brun!
then the composition $\text{flift}_{L}^{M}\bef\text{brun}_{L}^{M}$
is a monad morphism of type $M\leadsto M$ that is parametric in $M$
(working in the same way for all monads $M$). We will prove in Statement~\ref{subsec:Statement-identity-monad-morphism}(b)
that the only such morphism is an identity function. So, the nondegeneracy
law of base runners is a consequence of the monad morphism laws and
does not need to be checked separately.}\index{nondegeneracy law!of base runner (brun)@of base runner (\texttt{brun})}
\[
...
Define $T_{L}^{M,A}$ as the following type expression:
\[
T_{L}^{M,A}\triangleq\forall F^{:\text{Monad}}.\,(\forall B.\,L^{B}\rightarrow F^{B})_{\text{monad morphism}}\times(\forall B.\,M^{B}\rightarrow F^{B})_{\text{monad morphism}}\rightarrow F^{A}\quad.
\]
...
the natural transformations of types $L^{B}\rightarrow F^{B}$ and
$M^{B}\rightarrow F^{B}$ are required to be monad morphisms. These
restrictions on types cannot be correctly represented as a type signature
in Scala (or in most other languages). We could try writing a Scala
type signature for $T_{L}^{M,A}$:
\begin{lstlisting}
...
If we could ignore all the law restrictions, we would rewrite $T_{L}^{M,A}$
as:
\begin{align*}
 & \forall N^{\bullet}.\,(\forall B.\,\gunderline{L^{B}}\rightarrow F^{B})\times(\forall B.\,\gunderline{M^{B}}\rightarrow F^{B})\rightarrow F^{A}\\
{\color{greenunder}\text{merge two }\forall B\text{ quantifiers into one}:}\quad & \cong\forall N^{\bullet}.\,(\forall B.\,(L^{B}+M^{B})\rightarrow F^{B})\rightarrow F^{A}\\
...
a monad. The typeclass restrictions in the definition of $T_{L}^{M,A}$
...
explored in the 2002 paper by C.~Lüth and N.~Ghani (see \texttt{\href{https://dl.acm.org/doi/10.1145/583852.581492}{https://dl.acm.org/doi/10.1145/583852.581492}}).}) that cannot be simplified via Yoneda identities.

Theoretically, $T_{L}^{M,A}$ has all the properties that a monad
transformer needs. It is a monad because it has the structure $R\rightarrow F^{A}$,
where $R$ is a type without parameters and $F$ is required to be
a monad; we recognize the \lstinline!Reader! transformer here.

Note that $T_{L}^{M,A}$ is completely symmetric and fully parametric
with respect to the monads $L$ and $M$. Also, the type $T_{L}^{M,A}$
is covariant in its type parameters $L$, $M$. So, one can lift a
monad morphism $M^{A}\rightarrow N^{A}$ to a monad morphism $T_{L}^{M,A}\rightarrow T_{L}^{N,A}$
and similarly for $L$. This gives the foreign runner and the generalized
base runner.

One can prove that the identity law holds ($T_{L}^{\text{Id},A}\cong L^{A}$)
...
\]
To implement $\text{flift}_{T}$, we just specialize the given monad
morphism of type $\forall B.\,M^{B}\rightarrow F^{B}$ by setting
the type parameter $B=A$ and obtain a monad morphism of type $M^{A}\rightarrow F^{A}$,
which is then lifted to a monad morphism $M^{A}\rightarrow(R\rightarrow F^{A})$.
The base lift of type $L^{A}\rightarrow T_{L}^{M,A}$ is defined similarly.

The problem with $T_{L}^{M,A}$ is that its proper definition and
use requires advanced features of the type system that most programming
languages do not support. Without restricting the arguments of types
$L^{B}\rightarrow F^{B}$ and $M^{B}\rightarrow F^{B}$ to monad morphisms,
the transformer reduces to the functor co-product ($L^{A}+M^{A}$),
which will in many cases fail to be a monad.
...
The monad stack notation means that $\text{Free}^{F}\varangle\text{Free}^{G}=T_{\text{Free}^{F}}^{M,\bullet}$
