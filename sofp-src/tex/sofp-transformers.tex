...
that works parametrically in the foreign monad $M$. (The foreign
monad $M$ is the first type parameter in the transformer.) We can
use $T_{\text{Try}}^{M,A}$ as a monad with respect to the type parameter
$A$. We will later implement transformers for other monads, such
as $T_{\text{Reader}}^{M}$, $T_{\text{Writer}}^{M}$, $T_{\text{State}}^{M}$,
etc. The advantage of the transformer approach is that the code for
$T_{L}^{M}$ is written once for every base monad $L$, not for every
pair of monads $L$, $M$.
...
\lstinline!Success! subtypes). The code for $T_{L}^{M}$\textsf{'}s methods
to be parametric in $M$ but not in $L$. This is shown by the notation
$T_{L}^{M}$ where $L$ is a \emph{subscript}. Changing the base monad
$L$ to another monad will require a complete rewrite of $T_{L}^{M}$\textsf{'}s
code. It is not obvious how to implement a monad transformer $T_{L}^{M}$
for an arbitrary given base monad $L$. 
...
for more details.} whether \lstinline!brunE! is a lawful monad morphism of type $T_{\text{List}}^{M}\leadsto M$,
or even whether it is a monoid morphism $T_{\text{List}}^{M,A}\rightarrow M^{A}$.
...
Show that $T_{\text{State}}^{\text{Reader},A}$ is equivalent to $T_{\text{Reader}}^{\text{State},A}$.
...
\end{align*}
The code for $T_{\text{State}}^{\text{Reader},A}$ is found by using
