...
\textbf{codensity monad} $\text{Cod}_{F}^{M,A}$ is defined by:
\[
...
methods for $\text{Cod}_{F}^{M,A}$ are defined via the flipped Kleisli
composition:
\[
...
removed from the definition of $\text{Cod}_{F}^{M,A}$ even if $F^{A}\triangleq A$:
for a fixed type $X$ and for some monad $M$, the functor $P^{A}\triangleq(A\rightarrow X)\rightarrow M^{X}$
is not a monad.
