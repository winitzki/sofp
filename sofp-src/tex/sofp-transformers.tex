...
\]
We will prove in Appendix~\ref{app:Proofs-of-naturality-parametricity}
(Example~\ref{subsec:Example-simplify-quantifier-1-1}) that the
type $\forall A.\,(H^{A}\rightarrow A)\rightarrow A$ is equivalent
to $H^{\bbnum 1}$. That is, each fully parametric implementation
...
We can see that the function $\theta$ works by taking a given value
of type $H^{\bbnum 1}$, converting it to $H^{A}$, and substituting
that value of type $H^{A}$ into the given argument of type $H^{A}\rightarrow A$.
