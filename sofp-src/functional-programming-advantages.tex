...
\textcolor{blue}{\footnotesize{}def~utfChars(fileName:~String):~Try{[}Int{]}~=~tryOpenFile()~match~\{}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~case~Success(file)~=>~tryRead(fileName)~match~\{}{\footnotesize\par}
...
\item We can write code like this (avoiding nesting and repetition):
\end{itemize}
\begin{lyxcode}
...
\textcolor{blue}{\footnotesize{}def~utfChars(fileName:~String):~Try{[}Int{]}~=~for~\{}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~file~~~~<-~tryOpenFile(fileName)}{\footnotesize\par}
