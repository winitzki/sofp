...
\item Can we derive the code of a function from its type signature?\\
...
\item A sequent $X,Y\vdash Z$ corresponds to an \emph{expression} of type
\texttt{\textcolor{blue}{\footnotesize{}Z}} that uses some previously
defined values \texttt{\textcolor{blue}{\footnotesize{}x:X}} and \texttt{\textcolor{blue}{\footnotesize{}y:Y}} 
\begin{itemize}
...
\begin{itemize}
\item For brevity, write $A$ instead of $\mathcal{CH}(A)$ and use short
type notation
\item Axioms:
\begin{itemize}
\item $\emptyset\vdash\top$
\item $A\vdash A$ 
\item $A,B\vdash(A\times B)$
\item $(A\times B)\vdash A$
\item $(A\times B)\vdash B$
\item $A,(A\Rightarrow B)\vdash B$
\item $A\vdash(A+B)$
\item $B\vdash(A+B)$
\item $(A+B),(A\Rightarrow C),(B\Rightarrow C)\vdash C$
\end{itemize}
...
\item Cannot compute an \texttt{\textcolor{blue}{\footnotesize{}x:A}} because
...
\begin{itemize}
\item ...because most other papers on this subject are incomprehensible
to non-specialists, or describe algorithms that are too complicated
\end{itemize}
\end{itemize}
\end{itemize}
\end{frame}
