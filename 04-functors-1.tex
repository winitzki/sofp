...
as we build up the type constructor
...
\begin{itemize}
\item All our functors here will work with respect to the type parameter
$A$
\item Building blocks: creating functors from scratch
\begin{itemize}
\item Constant functors $\text{Const}^{C,A}\equiv C$ with $\text{fmap}(f)=id$,
and are at the same time contrafunctors with $\text{contrafmap}(f)=id$
\item Identity functor $\text{Id}^{A}=A$ with $\text{fmap}(f)=f$ (not
a contrafunctor!)
\end{itemize}
\item Operations: creating new functors out of previous ones
\begin{itemize}
\item We have already seen how this works in examples
...
\end{itemize}
\item $F^{G^{A}}$ (composition) \textendash{} \texttt{\textcolor{blue}{\footnotesize{}fmap}}
is built by composing the two \texttt{\textcolor{blue}{\footnotesize{}fmap}}s
\begin{itemize}
\item Check that the functor laws still hold after each operation
\end{itemize}
\item Similar constructions hold for contrafunctors, \emph{mutatis mutandis}
\end{itemize}
\end{frame}

\begin{frame}{Worked example II: Checking the functor laws}
...
\begin{itemize}
\item Hence we get $\text{fmap}_{F+G}(id)=(p+q)\Rightarrow id(p)+id(q)=p+q$
\end{itemize}
\item Composition law: 
\begin{align*}
 & (\text{fmap}_{F+G}(f_{1})\circ\text{fmap}_{F+G}(f_{2}))(p+q)\\
=\  & \text{fmap}_{F+G}(f_{1})\left(\text{fmap}_{F}(f_{2})(p)+\text{fmap}_{G}(f_{2})(q)\right)\\
=\  & (\text{fmap}_{F}(f_{1})\circ\text{fmap}_{F}(f_{1}))(p)+\left(\text{fmap}_{G}(f_{1})\circ\text{fmap}_{G}(f_{2})\right)(q)\\
=\  & \text{fmap}_{F}(f_{1}\circ f_{2})(p)+\text{fmap}_{G}(f_{1}\circ f_{2})(q)\\
=\  & \text{fmap}_{F+G}(f_{1}\circ f_{2})(p+q)
\end{align*}
 \textendash{} note how $\text{fmap}_{F+G}(f)$ works on each side
of $\left(p+q\right)$ separately
...
\item Check that the \texttt{\textcolor{blue}{\footnotesize{}fmap}} laws
hold for $F^{A}\Rightarrow G^{A}$ if they hold for a functor $G^{A}$,
and the contrafunctor laws hold for $F^{A}$
...
nor a contrafunctor when both $F^{A}$ and $G^{A}$ are functors (an
example of suitable $F^{A}$ and $G^{A}$ will be sufficient)
\end{enumerate}
\end{frame}
