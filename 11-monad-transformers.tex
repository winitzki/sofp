...
\begin{frame}{Monad transformers IV: The zoology}
...
\end{itemize}
\item ``Outside'' transformers: base monad is outside, $T_{L}^{M,A}=L^{M^{A}}$ormers: base monad is outside, $T_{L}^{M,A}=L^{M^{A}}$
\begin{itemize}
\item Examples: \texttt{\textcolor{blue}{\footnotesize{}ReaderT}} 
\item More generally: rigid monads
\end{itemize}
\item Constructions: product, contrafunctor projection
