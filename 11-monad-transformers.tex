...
\begin{itemize}
\item A \textbf{rigid functor} has the method \texttt{\textcolor{blue}{\footnotesize{}fuseIn}}$:\left(A\Rightarrow R^{B}\right)\Rightarrow R^{A\Rightarrow B}$
\end{itemize}
\end{itemize}
...
\[
\xymatrix{\xyScaleY{2pc}\xyScaleX{4pc}M^{R^{A}}\ar[r]\sb(0.5){\text{fmap}_{M}\left(\text{fmap}_{R}\left(\text{pure}_{M}\right)\right)} & M^{R^{M^{A}}}\ar[r]\sb(0.5){\text{pure}_{R}} & R^{M^{R^{M^{A}}}}=T^{T^{A}}\ar[r]\sb(0.5){\text{ftn}_{T}} & T^{A}\equiv R^{M^{A}}}
\]
}{\footnotesize\par}
\begin{itemize}
\item {\footnotesize{}\vspace{-0.2cm}}Compatibility law: $\text{flm}_{R}g^{:A\Rightarrow R^{M^{B}}}=\text{pure}_{M}^{\uparrow R}\bef\text{flm}_{T}g^{:A\Rightarrow R^{M^{B}}}$
\end{itemize}
...
\begin{itemize}
\item $\text{seq}=\text{pure}_{M}^{\uparrow R\uparrow M}\bef\text{pure}_{R}\bef\text{ftn}_{T}$
as $M^{R^{\bullet}}\leadsto M^{R^{M^{\bullet}}}\leadsto R^{M^{R^{M^{\bullet}}}}\leadsto R^{M^{\bullet}}$
\end{itemize}
...
from the $\text{flm}$-based law $\text{lift}_{1}\bef\text{flm}_{\text{BigM}}\left(q\bef\text{lift}_{1}\right)=\text{flm}_{M_{1}}q\bef\text{lift}_{1}$ft}_{1}\right)=\text{flm}_{M_{1}}q\bef\text{lift}_{1}$.
