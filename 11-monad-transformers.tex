...
\item {\footnotesize{}\vspace{-0.2cm}}{\small{}Alternative formulation:
...
makes $R^{A\Rightarrow M^{B}}$ into a category}{\footnotesize\par}
\begin{itemize}
...
$\text{fi}\left(\text{pure}_{R}\right)\bef\left(q^{:A\Rightarrow A}\Rightarrow q\bef\text{pure}_{M}\right)^{\uparrow R}=\text{pure}_{R}\left(\text{id}\right)\bef\left(q\Rightarrow q\bef\text{pure}_{M}\right)^{\uparrow R}=\text{pure}_{R}\left(\text{id}\bef\text{pure}_{M}\right)=\text{pure}_{R}\left(\text{pure}_{M}\right):R^{A\Rightarrow M^{A}}$.
Compose ext{pure}_{M}\right):R^{A\Rightarrow M^{A}}$.
Compose $\text{fi}\left(\text{pure}_{T}\right)$ with a morphism $r:R^{A\Rightarrow M^{B}}$
and get $\diamond_{M}^{\uparrow R}\left(\text{pure}_{R}(\text{pure}_{M})\bowtie_{R}r\right)=\diamond_{M}^{\uparrow R}\big(\left(f\Rightarrow\text{pure}_{M}\times f\right)^{\uparrow R}r\big)=\left(f\Rightarrow\text{pure}_{M}\diamond_{M}f\right)^{\uparrow R}r=\left(\text{id}\right)^{\uparrow R}r=r$,
