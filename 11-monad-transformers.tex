...
The $M$}\textbf{\footnotesize{}-naturality}{\footnotesize{} of $\text{sw}$
means that $\text{sw}\bef\phi=\phi^{\uparrow L}\bef\text{sw}$, so
we get $\phi^{\uparrow L\uparrow M}\bef\big(\text{sw}^{\uparrow M}\bef\phi\big)\bef\text{ftn}_{L}^{\uparrow N\uparrow N}\bef\text{ftn}_{N}=\phi^{\uparrow L\uparrow M}\bef\phi\bef\big(\text{sw}^{\uparrow N}\bef\text{ftn}_{L}^{\uparrow N\uparrow N}\bef\text{ftn}_{N}\big)=\phi^{\uparrow L\uparrow M}\bef\phi\bef\text{ftn}_{N^{L}}$}{\footnotesize\par}
...
$\text{Cont}^{A}$, $\text{State}^{A}$text{List}^{A}$,
...
\begin{itemize}
\item {\footnotesize{}\vspace{-0.2cm}}Some monads involve a higher-order
function, e.g.:
\begin{itemize}
...
\]
\item {\footnotesize{}\vspace{-0.1cm}}\emph{Flip} \emph{the} \emph{arguments}
of the Kleisli functions: for example, instead of $A\Rightarrow R^{B}\triangleq A\Rightarrow\left(B\Rightarrow P^{Q}\right)\Rightarrow P^{B}$,
