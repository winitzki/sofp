...
{\footnotesize{}\vspace{-0.2cm}\hspace{-0.0cm}r law:
...
\begin{itemize}
\item $\text{lift}_{1}$ is a \textbf{monadic morphism} between monads $M_{1}$
and $\text{BigM}$
\end{itemize}
\end{itemize}
The (functor) naturality law follows: 
\[
...
\item But we can fix $M_{1}$ (base monad) and to let $M_{2}$ (foreign
monad) vary
...
Definition:
\begin{itemize}
\item Monad transformer for a base monad $L$ is a type constructor $T_{L}^{M,A}$
\end{itemize}
\end{frame}
