...
\begin{itemize}
\item ``Effect'' $\triangleq$ what else happens in {\footnotesize{}$A\Rightarrow M^{B}$}
...
others are not
\end{itemize}
\item It is not known whether all monads $L$ have a transformer (?)
\end{itemize}
...
{\footnotesize{}
\[
\xymatrix{\xyScaleY{2pc}\xyScaleX{5pc}T^{T^{A}}\triangleq M^{L^{M^{L^{A}}}}\ar[r]\sb(0.5){\text{fmap}_{M}\big(\text{seq}^{L^{A}}\big)} & M^{M^{L^{L^{A}}}}\ar[r]\sb(0.5){\text{fmap}_{M}(\text{fmap}_{M}\text{ftn}_{L})} & M^{M^{L^{A}}}\ar[r]\sb(0.5){\text{ftn}_{M}} & M^{L^{A}}\triangleq T^{A}}
\]
}{\footnotesize\par}
\begin{itemize}
...
($M$-identity) and other properties
\begin{itemize}
\item See example code that verifies these properties for $L^{A}\triangleq E+W\times A$
...
monad $M^{\bullet}$, transformer $T_{L}^{M,\bullet}\triangleq T^{\bullet}\triangleq M^{L^{\bullet}}$
\begin{itemize}
...
\item $\text{lift}$'s identity law: $\text{pure}_{M}\bef\text{lift}=\text{pure}_{T}$
...
\begin{itemize}
\item {\footnotesize{}\vspace{-0.15cm}}Given a monadic morphism $\phi:M^{\bullet}\leadsto N^{\bullet}$,
we need to show that $\text{mrun}\,\phi\triangleq\phi:M^{L^{\bullet}}\leadsto N^{L^{\bullet}}$
is also a monadic morphism
\item {\footnotesize{}$\phi$'s laws are $\text{pure}_{M}\bef\phi=\text{pure}_{N}$
and $\text{ftn}_{M}\bef\phi=\phi^{\uparrow M}\bef\phi\bef\text{ftn}_{N}$
and $f^{\uparrow M}\bef\phi=\phi\bef f^{\uparrow N}$}{\footnotesize\par}
\item {\footnotesize{}Identity law: $\text{pure}_{L}\bef\text{pure}_{M}\bef\phi=\text{pure}_{L}\bef\text{pure}_{N}$
-- follows from $\phi$'s law}{\footnotesize\par}
\item {\footnotesize{}\vspace{-0.1cm}Composition law: $\text{ftn}_{M^{L}}\bef\phi=\phi^{\uparrow L\uparrow M}\bef\phi\bef\text{ftn}_{N^{L}}$
as functions $T^{T^{\bullet}}\Rightarrow N^{L^{\bullet}}$. Proof:
By def.~of $\text{ftn}_{M^{L}}$ and $\text{ftn}_{N^{L}}$ and by
the composition and naturality laws for $\phi$, get $\text{seq}^{\uparrow M}\bef\text{ftn}_{L}^{\uparrow M\uparrow M}\bef\left(\text{ftn}_{M}\bef\phi\right)=\text{seq}^{\uparrow M}\bef\big(\text{ftn}_{L}^{\uparrow M\uparrow M}\bef\phi^{\uparrow M}\big)\bef\phi\bef\text{ftn}_{N}=$
$\text{seq}^{\uparrow M}\bef\phi^{\uparrow M}\bef\text{ftn}_{L}^{\uparrow N\uparrow M}\bef\phi\bef\text{ftn}_{N}=\big(\text{seq}\bef\phi\big)^{\uparrow M}\bef\big(\text{ftn}_{L}^{\uparrow N\uparrow M}\bef\phi\big)\bef\text{ftn}_{N}$.
The $M^{\bullet}$}\textbf{\footnotesize{}-naturality}{\footnotesize{}
of $\text{seq}$ means that $\text{seq}\bef\phi=\phi^{\uparrow L}\bef\text{seq}$,
so we get $\phi^{\uparrow L\uparrow M}\bef\big(\text{seq}^{\uparrow M}\bef\phi\big)\bef\text{ftn}_{L}^{\uparrow N\uparrow N}\bef\text{ftn}_{N}=\phi^{\uparrow L\uparrow M}\bef\phi\bef\big(\text{seq}^{\uparrow N}\bef\text{ftn}_{L}^{\uparrow N\uparrow N}\bef\text{ftn}_{N}\big)=\phi^{\uparrow L\uparrow M}\bef\phi\bef\text{ftn}_{N^{L}}$}{\footnotesize\par}
\item {\footnotesize{}\vspace{-0.1cm}}Given a monadic morphism $\phi:L^{A}\leadsto A$,
we need to show that $\text{brun}\,\phi\triangleq\phi^{\uparrow M}:M^{L^{A}}\Rightarrow M^{A}$
is also a monadic morphism
...
also $\text{seq}\bef\phi^{\uparrow M}=\phi$}{\footnotesize\par}
...
as functions $T^{T^{\bullet}}\Rightarrow M^{\bullet}$. Proof: $\text{seq}^{\uparrow M}\bef\big(\text{ftn}_{L}^{\uparrow M\uparrow M}\bef\text{ftn}_{M}\big)\bef\phi^{\uparrow M}=\text{seq}^{\uparrow M}\bef\text{ftn}_{M}\bef\big(\text{ftn}_{L}\bef\phi\big)^{\uparrow M}=$
$\text{seq}^{\uparrow M}\bef\big(\text{ftn}_{M}\bef\phi^{\uparrow M}\big)\bef\phi^{\uparrow M}=\big(\text{seq}^{\uparrow M}\bef\phi^{\uparrow M\uparrow M}\big)\bef\big(\text{ftn}_{M}\bef\phi^{\uparrow M}\big)=\phi^{\uparrow M}\bef\phi^{\uparrow M\uparrow M}\bef\text{ftn}_{M}$}{\footnotesize\par}
\end{itemize}
\end{frame}
...
}}$=\text{seq}$ with $M^{A}\triangleq Z\Rightarrow A$
...
{\footnotesize{}$T_{R}^{M,\bullet}\triangleq T^{\bullet}\triangleq R^{M^{\bullet}}$llet}\triangleq T^{\bullet}\triangleq R^{M^{\bullet}}$}{\footnotesize\par}
\begin{itemize}
...
\item Choosing $M^{A}\triangleq Z\Rightarrow A$, we get $\text{seq}=\text{fi}:\left(Z\Rightarrow R^{A}\right)\Rightarrow R^{Z\Rightarrow A}$
\begin{itemize}
...
\begin{itemize}
\item $R^{A}\triangleq H^{A}\Rightarrow A$,~ $R^{A}\triangleq\left(A\Rightarrow P^{Q}\right)\Rightarrow P^{A}$,~
$R^{A}\triangleq\forall B.\left(A\Rightarrow F^{B}\right)\Rightarrow F^{B}$
\end{itemize}
...
functions: for example, instead of $A\Rightarrow R^{B}\triangleq A\Rightarrow\left(B\Rightarrow P^{Q}\right)\Rightarrow P^{B}$,
...
\end{enumerate}
\item Use ``compatibility laws'': for any monad $M$, and denoting $T^{\bullet}\triangleq R^{M^{\bullet}}${\footnotesize{},\vspace{-0.0cm}
\[
