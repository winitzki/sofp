...
somehow combines all the needed effects:

{\footnotesize{}\vspace{-0.35cm}\hspace{-0.35cm}}\texttt{\textcolor{blue}{\footnotesize{}}}%
\begin{minipage}[t]{0.49\columnwidth}%
\begin{lyxcode}
\textcolor{darkgray}{\footnotesize{}//~This~could~be~valid~Scala.}{\footnotesize\par}
...
\begin{minipage}[t]{0.49\columnwidth}%
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}~}\textcolor{darkgray}{\footnotesize{}//~If~we~define~the~various}{\footnotesize\par}

\textcolor{darkgray}{\footnotesize{}~//~required~``lifting''~functions:}{\footnotesize\par}
...
\textcolor{blue}{\footnotesize{}def~lift$_{2}${[}A{]}:~Future{[}A{]}~$\Rightarrow$~Future{[}Option{[}A{]}{]}~=~\_.map(Some.apply)}{\footnotesize\par}
\end{lyxcode}
\begin{itemize}
...
\end{itemize}
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}def~lift$_{1}${[}A{]}:~Try{[}A{]}~$\Rightarrow$~List{[}Try{[}A{]}{]}~=~x~$\Rightarrow$~List(x)}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}def~lift$_{2}${[}A{]}:~List{[}A{]}~$\Rightarrow$~List{[}Try{[}A{]}{]}~=~\_.map(x~$\Rightarrow$~Success(x))}{\footnotesize\par}
\end{lyxcode}
{\footnotesize{}\vspace{-0.1cm}ize\par}
\end{lyxcode}
{\footnotesize{}\vspace{-0.1cm}}Remains to be understood:
\begin{itemize}
\item How to build a ``big monad'' out of ``smaller'' ones, providing
liftings
\begin{itemize}
\item Is this always possible? Are there alternatives?
\end{itemize}
\item What are the required laws for liftings
\item Ways of reducing the complexity of code; make liftings implicit
\end{itemize}
\end{frame}

\begin{frame}{Exercises}
\begin{enumerate}
\item {\small{}Show that $M_{1}^{A}+M_{2}^{A}$ is not a monad when $M_{1}^{A}\equiv1+A$
and $M_{2}^{A}\equiv Z\Rightarrow A$.}{\small\par}
\end{enumerate}
\end{frame}
