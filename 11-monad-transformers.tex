...
\begin{itemize}
\item {\footnotesize{}\vspace{-0.2cm}}``Nested'' associativity law: $\text{flm}_{R}g^{:A\Rightarrow R^{M^{B}}}=\text{pure}_{M}^{\uparrow P}\bef\text{flm}_{T}g^{:A\Rightarrow R^{M^{B}}}$
\end{itemize}
...
\begin{itemize}
\item $\text{seq}m=\text{pure}_{M}^{\uparrow R\uparrow M}\bef\text{pure}_{R}\bef\text{ftn}_{T}$
...
{\footnotesize{}\vspace{-0.15cm}Proofs for the }{\footnotesize\par}
\end{frame}

\begin{frame}{Rigid monads: Open questions}

\begin{itemize}
\item {\footnotesize{}\vspace{-0.25cm}}What properties of $\text{fi}:\left(A\Rightarrow R^{B}\right)\Rightarrow R^{A\Rightarrow B}$
define rigid monads?
\begin{itemize}
...
general; $L^{\bullet}\times M^{\bullet}$ot a monad in
