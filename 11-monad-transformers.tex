...
\item This is needed for lifting a ``runner'' ll\par}
...
\begin{itemize}
\item Pointed reader $R_{Z}^{A}\equiv A+\left(Z\Rightarrow A\right)$; selector
$S_{P,Q}^{A}\equiv\left(A\Rightarrow P^{Q}\right)\Rightarrow P^{A}$
\end{itemize}
\item Monad constructions: making a transformer for new monads
\begin{itemize}
\item Product monads $M^{A}\times N^{A}$ -- have the ``product transformer''
\item Consumer choice monads $H^{A}\Rightarrow A$ -- have the ``outside''
transformer
\item Free pointed monads $A+M^{A}$ -- have no transformer
\end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Rigid monads, their laws and structure}

\begin{itemize}
\item A \textbf{rigid monad} has the method \texttt{\textcolor{blue}{\footnotesize{}fuseIn}}$:\left(A\Rightarrow R^{B}\right)\Rightarrow R^{A\Rightarrow B}$
\begin{itemize}
\item Any functor has the method \texttt{\textcolor{blue}{\footnotesize{}fuseOut}}$:R^{A\Rightarrow B}\Rightarrow A\Rightarrow R^{B}$
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item free monad over $L^{M^{\bullet}}$: $F^{A}\equiv A+L^{M^{F^{A}}}$
-- \texttt{\textcolor{blue}{\footnotesize{}lift}} violates identity
law
...
\end{itemize}
\item ``Monoidal'' convolution: $F^{A}\equiv\exists P\exists Q.\left(P\times Q\Rightarrow A\right)\times L^{P}\times M^{Q}$
\begin{itemize}
\item combines functors via $L^{A}\cong\exists P.L^{P}\times\left(P\Rightarrow A\right)$
and same for $M^{\bullet}$
\item not a monad for some $L^{\bullet}$, $M^{\bullet}$
\end{itemize}
