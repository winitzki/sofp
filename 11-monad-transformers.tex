...
\begin{itemize}
\item Express $\text{fmap}$ as $\text{fmap}_{M}f\equiv f^{\uparrow M}=\text{flm}_{M}\left(f\bef\text{pure}_{M}\right)$
for both monads
...
\item Composition law: $\text{blift}\bef\text{blift}^{\uparrow T}\bef\text{ftn}_{T}=\text{ftn}_{L}\bef\text{blift}$w T}\bef\text{ftn}_{T}=\text{ftn}_{L}\bef\text{blift}$.
...
monad $M^{\bullet}$, transformer $T_{L}^{M,\bullet}\equiv T^{\bullet}\equiv M^{L^{\bullet}}$
\begin{itemize}
\item Given a monadic morphism $\phi:M^{\bullet}\leadsto N^{\bullet}$,
we need to show that $\text{mrun}\,\phi\equiv\phi:M^{L^{\bullet}}\leadsto N^{L^{\bullet}}$
...
\item {\footnotesize{}Identity law for $\text{mrun}$: $\text{pure}_{L}\bef\text{pure}_{M}\bef\phi=\text{pure}_{L}\bef\text{pure}_{N}$
-- follows from $\phi$'s law}{\footnotesize\par}
\item {\footnotesize{}Composition law: $\text{ftn}$}{\footnotesize\par}
\end{itemize}
\end{frame}
