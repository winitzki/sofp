...
\end{itemize}
\item So $\text{MyTC}^{Z}$ is a PTTF defined only for $Z\in\left\{ \text{Int};\,\text{String}\right\} $
\item This \textbf{type domain} is checked and enforced \emph{at compile
time}!
...
Compare (value-to-value vs.\ type-to-value functions):
\begin{itemize}
\item the \textbf{type} of a function's argument is the set of admissible
values
\begin{itemize}
\item the function can be applied safely if its argument is of the correct
type
\end{itemize}
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}def~f(x:~Option{[}Int{]})~=~...}{\footnotesize \par}
\end{lyxcode}
\item the \textbf{type domain} of a PTVF is the set of admissible types
\begin{itemize}
\item ``type domain'' is also called the \textbf{kind}
...
\begin{itemize}
\item suggested notation: $(*:\text{MyTypeClass})$
\end{itemize}
\item another available kind is the \textbf{type constructor} kind, e.g.:
\texttt{\textcolor{blue}{\footnotesize{}F{[}\_{]}}}{\footnotesize \par}
\begin{itemize}
\item note: the \texttt{\textcolor{blue}{\footnotesize{}F}} in \texttt{\textcolor{blue}{\footnotesize{}F{[}T{]}}}
is a different \textbf{kind} of type than \texttt{\textcolor{blue}{\footnotesize{}T}}{\footnotesize \par}
\item example: \texttt{\textcolor{blue}{\footnotesize{}type Ap{[}F{[}\_{]},
T{]} = F{[}T{]}}} will not compile with wrong kinds
\begin{itemize}
\item suggested notation: $\text{Ap}:(*\rightarrow*,\,*)\rightarrow*$
\end{itemize}
\end{itemize}
\end{itemize}
See test code
...
Available syntax for Scala functions:
\begin{enumerate}
...
or equivalently, \texttt{\textcolor{blue}{\footnotesize{}x func y}}{\footnotesize \par}
\begin{itemize}
\item this is similar to \texttt{\textcolor{blue}{\footnotesize{}func(x)(y)}}
but is implemented differently
\end{itemize}
\end{enumerate}
It is often convenient to use PTVFs syntactically as methods
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}def~func{[}T:~MyTypeClass{]}(t:~T)(args...)~=~...}{\footnotesize \par}

\textcolor{blue}{\footnotesize{}val~t:~T~=~...}{\footnotesize \par}

\textcolor{blue}{\footnotesize{}func(t)(args)~}\textrm{\textcolor{gray}{\footnotesize{}//~that's~how~we~have~to~call~func()}}{\footnotesize \par}

\textrm{\textcolor{gray}{\footnotesize{}//~but~instead~we~want~to~be~able~to~write~}}\textcolor{gray}{\footnotesize{}t.func(args)}{\footnotesize \par}
\end{lyxcode}
To implement this in Scala:
\begin{itemize}
