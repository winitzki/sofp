...
with the short type notation {\footnotesize{}$\text{Boolean}\Rightarrow\left(T+\text{Int}\times T+(\text{String}\Rightarrow T)\right)$}{\footnotesize \par}
...
in logic
\item Denote $\text{Reader}^{E,T}\equiv E\Rightarrow T$ and implement functions
with types $A\Rightarrow\text{Reader}^{E,A}$ and $\text{Reader}^{E,A}\Rightarrow(A\Rightarrow B)\Rightarrow\text{Reader}^{E,B}$
...
}with no ``information loss'', that is, \texttt{\textcolor{blue}{\footnotesize{}map{[}A,A{]}}}
...
\end{itemize}
\item Denoting $\text{State}^{S,T}\equiv S\Rightarrow T\times S$, implement
the functions:
\begin{itemize}
...
\item $\text{flatMap}^{S,A,B}:\text{State}^{S,A}\Rightarrow\left(A\Rightarrow\text{State}^{S,B}\right)\Rightarrow\text{State}^{S,B}$
\end{itemize}
...
\begin{itemize}
\item Implement the functions \texttt{\textcolor{blue}{\footnotesize{}map}}
and \texttt{\textcolor{blue}{\footnotesize{}concat}} for \texttt{\textcolor{blue}{\footnotesize{}NEList}},
...
\begin{enumerate}
\item Define a parameterized type \texttt{\textcolor{blue}{\footnotesize{}MyTU{[}T,U{]}}}
with the short type notation {\footnotesize{}$T\times U+\text{Int}\times T+\text{String}\times U$}{\footnotesize \par}
...
into an equivalent type of the form $A\times(...)$
\item Define type $\text{OptEither}^{A,B}=1+A+B$ and implement\texttt{\textcolor{blue}{\footnotesize{}
map}} and \texttt{\textcolor{blue}{\footnotesize{}flatMap}} for it,
preferring $B$ over $A$. Show that the result is the same when using
the equivalent type $(1+A)+B$, i.e. \texttt{\textcolor{blue}{\footnotesize{}Either{[}Option{[}A{]},B{]}}}{\footnotesize \par}
...
$A\times Z\Rightarrow(A\Rightarrow B)\Rightarrow B\times Z$}{\footnotesize \par}
\item {\footnotesize{}$\text{flatMap}^{E,A,B}:\text{Reader}^{E,A}\Rightarrow\left(A\Rightarrow\text{Reader}^{E,B}\right)\Rightarrow\text{Reader}^{E,B}$}{\footnotesize \par}
...
\item {\footnotesize{}$\text{flatMap}^{Z,A,B}:\text{Density}^{Z,A}\Rightarrow\left(A\Rightarrow\text{Density}^{Z,B}\right)\Rightarrow\text{Density}^{Z,B}$}{\footnotesize \par}
\end{enumerate}
