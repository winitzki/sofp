...
\texttt{\textcolor{blue}{\footnotesize{}  List(y0, y1, y2, y3, y4,
y5, y6)}} 
...
Can use eager (\texttt{\textcolor{blue}{\footnotesize{}Seq}}) or lazy
...
{\footnotesize{}$\text{flm}\,(f^{A\Rightarrow B}\circ g^{B\Rightarrow S^{C}})=\text{fmap}\,f\circ\text{flm}\,g$}
...
{\footnotesize{}$\text{flm}\,\big(f^{A\Rightarrow S^{B}}\circ\text{fmap}\,g^{B\Rightarrow C}\big)=\text{flm}\,f\circ\text{fmap}\,g$}
...
{\footnotesize{}$\text{fmap}\big(\text{fmap}\,f^{A\Rightarrow B}\big)\circ\text{ftn}^{\left[B\right]}=\text{ftn}^{\left[A\right]}\circ\text{fmap}\,f$}
...
{\footnotesize{}$\text{fmap}\,(\text{ftn}^{\left[A\right]})\circ\text{ftn}^{\left[A\right]}=\text{ftn}^{[S^{A}]}\circ\text{ftn}^{\left[A\right]}$}
...
{\footnotesize{}$\text{flm}\left(f\circ g\right)=f^{\uparrow}\circ\text{flm}\,g$}
...
{\footnotesize{}$\text{flm}\left(f\circ g^{\uparrow}\right)=\text{flm}\,f\circ g^{\uparrow}$}
...
{\footnotesize{}$\text{flm}\left(f\circ\text{flm}\,g\right)=\text{flm}\,f\circ\text{flm}\,g$
...
\subsection{Checking the associativity law for standard monads}
...
Right identity: $\text{flm}\left(\text{pure}\right)=\text{fmap}\left(\text{pure}\right)\circ\text{ftn}=\text{id}^{S^{A}\Rightarrow S^{A}}$
\[
\xymatrix{\xyScaleY{0.2pc}\xyScaleX{3pc} & S^{S^{A}}\ar[rd]\sp(0.5){\ \text{ftn}^{[A]}\ }\\
...
Use $\text{flm}$ to define \textbf{Kleisli composition}: $f^{A\Rightarrow S^{B}}\diamond g^{B\Rightarrow S^{C}}\equiv f\circ\text{flm}\,g$

Define \textbf{Kleisli identity} $\text{id}_{\diamond}$ of type $A\Rightarrow S^{A}$
as $\text{id}_{\diamond}\equiv\text{pure}$

Composition law: $\text{flm}\left(f\diamond g\right)=\text{flm}\,f\circ\text{flm}\,g$
...
Follows from the $\text{flm}$ law: $f\circ\text{flm}\left(g\circ\text{flm}h\right)=f\circ\text{flm}\,g\circ\text{flm}\,h$
...
Left naturality: {\small{}$f^{A\Rightarrow B}\circ g^{B\Rightarrow S^{C}}=\left(f\circ\text{pure}\right)\diamond g$}{\small\par}

Right naturality: {\small{}$f^{A\Rightarrow S^{B}}\circ\text{fmap}\,g^{B\Rightarrow C}=f\diamond\left(g\circ\text{pure}\right)$}{\small\par}
...
Use ``left naturality'', get: $\left(f\circ g\right)\diamond h=\left(f\circ pure\right)\diamond g\diamond h=f\circ\left(g\diamond h\right)$

Naturality for \texttt{\textcolor{blue}{\footnotesize{}pure}}: $\text{pure}\circ\text{fmap}\,f=\text{pure}\diamond\left(f\circ\text{pure}\right)=f\circ\text{pure}$
...
Any type $Z$ is a semigroup with operation $z_{1}\circledast z_{2}=z_{1}$
(or $z_{2}$)
...
Any totally ordered type is a monoid, with $\circledast$ defined
as $\max$ or $\min$

$S_{1}\times S_{2}$ is a semigroup (monoid) if $S_{1}$, $S_{2}$
are semigroups (monoids)

$S_{1}+S_{2}$ is a semigroup (monoid) if $S_{1}$, $S_{2}$ are semigroups
(monoids)
...
\subsection{Exercises II (continued)}
