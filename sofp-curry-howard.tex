...
summarizes the type notation\footnote{See also Appendix~\ref{chap:Appendix-Notations} for more detail
about the type notation.} and also shows how the type notation is directly translated into
logic formulas for the propositions of the form ${\cal CH}(...)$.

The precedence\index{type notation!operator precedence} of operators
in the type notation is chosen to write frequently used type expressions
with fewer parentheses. Let us now summarize the rules of operator
precedence.

The type product operators ($\times$) group stronger than the disjunctive
...
The function type arrows ($\Rightarrow$) group weaker than the operators
...
The type quantifiers group weaker than all other operators, so that
we can write types such as $\forall A.\,A\Rightarrow A\Rightarrow A$
without parentheses. Type quantifiers are most often placed outside
a type expression. When this is not the case, parentheses will be
necessary, e.g.~$\left(\forall A.\,A\Rightarrow A\right)\Rightarrow\bbnum 1$.
...
\hline 
\lstinline!Unit! {\small{}or a ``named''} \lstinline!Unit! {\small{}type} & $\bbnum 1$ & {\small{}always} \emph{True}\tabularnewline
\hline 
...
\end{align*}
The type \lstinline!Either[A, B]! is denoted as $A+B$ by definition
of the disjunctive type operator ($+$). The type \lstinline!Option[A]!
has two disjoint cases, \lstinline!None! and \lstinline!Some[A]!.
The value \lstinline!None! is a ``\index{unit type!named}named
...
tuples \lstinline!(String, String)!, \lstinline!(String)!, and \lstinline!(Long)!.
...
with three parts. To implement \lstinline!F! in Scala, we need to
choose names for each of the disjoint parts, which will become case
classes. For the purposes of this example, let us choose names \lstinline!F1!,
\lstinline!F2!, and \lstinline!F3!. Each of these case classes needs
to have the same type parameter \lstinline!A!. So the code will be
\begin{lstlisting}
sealed trait F[A]
...
\begin{tabular}{|c|c|c|}
\hline 
{\small{}$\alpha$} & {\small{}$\beta$} & \textbf{\small{}$\alpha\Rightarrow\beta$}\tabularnewline
\hline 
\hline 
{\small{}$True$} & {\small{}$True$} & {\small{}$True$}\tabularnewline
\hline 
{\small{}$True$} & {\small{}$False$} & {\small{}$False$}\tabularnewline
\hline 
{\small{}$False$} & {\small{}$True$} & {\small{}$True$}\tabularnewline
\hline 
{\small{}$False$} & {\small{}$False$} & {\small{}$True$}\tabularnewline
\hline 
...
of logical formulas that are true in Boolean logic. Each logical formula
is first given in terms of ${\cal CH}$-propositions (for brevity,
we denote $\alpha\triangleq{\cal CH}(A)$, $\beta\triangleq{\cal CH}(B)$,
etc.) and then into a Scala type signature of a function that can
be implemented.
...
into type formulas and then into Scala code, these formulas yield
type signatures that \emph{cannot} be implemented as fully parametric
functions.
...
$x$ of type $A$. This means we cannot build a function of type $A\Rightarrow B$
...
let us derive the suitable logical axioms and derivation rules systematically.
...
in the mathematical sense. So, we will now write the proof rules that
will define \emph{the} logic appropriate for reasoning about ${\cal CH}$-propositions
for fully parametric functions. (As we have already seen, the required
logic is somewhat different from Boolean logic.)
...
similar to Eq.~(\ref{eq:ch-example-sequent-2}), i.e.~has a function
type (in logic, an implication, $p\Rightarrow q$) in the goal. We
have only one rule that can prove a sequent of the form $\Gamma\vdash(p\Rightarrow q$);
this is the rule ``$\text{create function}$''. That rule requires
us to already have a proof of the sequent $(\Gamma,p)\vdash q$. So,
we use this rule with $\Gamma=\emptyset$, $p=(\alpha\Rightarrow\alpha)\Rightarrow\beta$,
and $q=\beta$: 
\[
...
as the set containing the single premise $(\alpha\Rightarrow\alpha)\Rightarrow\beta$. 
...
by assuming another sequent containing a function type. We would be
...
$\Gamma_{1}$ already contains $(\alpha\Rightarrow\alpha)\Rightarrow\beta$.
...
theorem in Boolean logic, but it is not true in the constructive logic
(i.e.~it cannot be derived using the proof rules of Section~\ref{subsec:The-rules-of-proof}).
If we try to implement a fully parametric function \lstinline!g[A, B]!
with the corresponding type signature shown above, we will find that
we cannot write code for \lstinline!g! that compiles without type
errors. This failure is because no such code exists, \textendash{}
not because we are not clever enough. The LJT algorithm can \emph{prove}
...
\subsection{Identity in logic does not correspond to type equivalence}
...
(denoted as $P\cong Q$) \index{type equivalence}when there exist
functions $f_{1}^{:P\Rightarrow Q}$ and $f_{2}^{:Q\Rightarrow P}$
that are inverses for each other: Compositions of $f_{1}$ and $f_{2}$
in both directions must be equal to an identity function. We can write
...
$Q$. This is clearly the same as to say that the data types $P$
and $Q$ ``carry equivalent information''.
...
or a tuple \lstinline!(Right(b), c)! respectively. Then we apply
\lstinline!f1!, which will map these tuples back to \lstinline!Left((a, c))!
or \lstinline!Right((b, c))! respectively. It follows that applying
$f_{2}$ and then $f_{1}$ will always recover the initial value $y$.
In other words, $f_{2}\bef f_{1}=\text{id}$.
...
the tuple pattern \lstinline!(Left(a), c)! is converted into \lstinline!Left((a, c))!,
and the pattern \lstinline!(Right(b), c)! into \lstinline!Right((b, c))!,
...
is transformed into the pattern \lstinline!(a, (b, c))! and vice
versa. So, the types $\left(A\times B\right)\times C$ and $A\times\left(B\times C\right)$
...
identity that does \emph{not} correspond to a type equivalence is
\begin{equation}
...
We can verify this intuition rigorously by proving that any two fully
parametric functions with type signatures $g_{1}:\bbnum 1+A\Rightarrow\bbnum 1$
and $g_{2}:\bbnum 1\Rightarrow\bbnum 1+A$ cannot satisfy $g_{1}\bef g_{2}=\text{id}$.
To verify this, we notice that $g_{2}:\bbnum 1\Rightarrow\bbnum 1+A$
...
identity function with that type signature because the identity function
will not always return \lstinline!None!.
...
(this is always true in Boolean logic, but not always true in the
constructive logic we are using now). The CH correspondence maps Eq.~(\ref{eq:ch-example-distributive-2})
...
because we will have discarded one of the given values of type \lstinline!C!.
After evaluating $g_{3}$, we will not be able to restore \emph{both}
\lstinline!c1! and \lstinline!c2! \textendash{} no matter what code
we write for $g_{4}$. So, the composition $g_{3}\bef g_{4}$ cannot
possibly equal the identity function.
...
It is not guaranteed, however, that these functions will be inverses
of each other, i.e.~that there is no information loss in the type
conversions $P\Rightarrow Q$ or $Q\Rightarrow P$. So, the type equivalence
$P\cong Q$ does not follow from the logical identity ${\cal CH}(P)={\cal CH}(Q)$.
...
computed value $x^{:X}$ should satisfy any additional properties
or laws.
...
\]
while the logical identity in Eq.~(\ref{eq:ch-example-distributive-2}),
...
corresponding type equivalences. In all rows, the quantifiers such
as $\forall\alpha$ or $\forall(A,B)$ are implied when necessary.

Because we chose the type notation to be similar to ordinary arithmetic
notation, it is easy to check whether a possible type equivalence
would be a valid arithmetic identity. In all cases, valid arithmetic
identities correspond to type equivalences, and failures to obtain
a type equivalence correspond to incorrect arithmetic identities.
As far as type equivalence goes, types such as $A+B$ and $A\times B$
behave similarly to arithmetic expressions such as $10+20$ and $10\times20$
and not similarly to logical formulas such as $\alpha\vee\beta$ and
$\alpha\wedge\beta$.
...
\begin{tabular}{|c|c|}
\hline 
\textbf{Logical identity} & \textbf{Type equivalence (if holds)}\tabularnewline
\hline 
\hline 
...
At this point, let us consider the 3rd and 4th lines of Table~\ref{tab:Logical-identities-with-disjunction-and-conjunction},
...
We would need to create values to fill both parts of the tuple. We
could create a value of type \lstinline!Int!, but we can never get
a value of type \lstinline!Nothing!. So, regardless of the type \lstinline!A!,
it is impossible to create any values of type \lstinline!(Nothing, A)!.
In other words, the set of values of the type \lstinline!(Nothing, A)!
is empty; but that is the definition of the type \lstinline!Nothing!.
The types \lstinline!(Nothing, A)! (denoted by $\bbnum 0\times A$)
and \lstinline!Nothing! (denoted by $\bbnum 0$) are both void and
...
\end{lstlisting}
For the same reason, there is only one implementation of the inverse
function,
\begin{lstlisting}
...
and is not similar to the \lstinline!Unit! type, $\bbnum 1$, whose
value we denote by $1$.
...
\end{lstlisting}
Now let us write a proof in a code notation. The codes of $f_{1}$
and $f_{2}$ are
\begin{align*}
...
It is clear from the code that the functions \lstinline!f1! and \lstinline!f2!
are inverses for each other (in other words, the function \lstinline!f1!
is its own inverse). To verify that rigorously, we need show that
\lstinline!f1 andThen f2! is equal to an identity function. The function
\lstinline!f1 andThen f2! applies \lstinline!f2! to the result of
\lstinline!f1!. The code of \lstinline!f1! contains two \lstinline!case ...!
lines, each returning its own result. So, we need to apply \lstinline!f2!
...
Verify the type equivalences $\left(A+B\right)+C\cong A+\left(B+C\right)$
and $\left(A\times B\right)\times C\cong A\times\left(B\times C\right)$.
These equivalences permit us to write the type notation as $A+B+C$
and $A\times B\times C$ without the parentheses.
...
the \textbf{cardinality} of type $A$; this is the same as the cardinality\index{cardinality}
of the set of all values of type $A$. Since any computer's memory
is limited, and since we may assume that we are already working with
the largest possible computer, then there will be \emph{finitely}
many different values of a given type $A$ that can be represented
by the computer. So, we may assume that $\left|A\right|$ is always
a finite integer value. This assumption will simplify our reasoning.
We will not actually need to compute the precise number of, say, all
the different possible strings; it is sufficient to know that the
set of all strings is finite, so that we can denote its cardinality
...
the sets of values of type $P$ and values of type $Q$.

We conclude that types can be equivalent when a logical identity \emph{and}
...
it is helpful to do so in a given application. For example, the types
\lstinline!Option[Option[A]]! and \lstinline!Either[Boolean, A]!
...
and $\bbnum 2+A$ respectively (note that the type Boolean is denoted
by $\bbnum 2$ since it has only two distinct values).
...
or to \lstinline!Left(true)!, and adjust the code \lstinline!f2!
accordingly; the type equivalence would still hold. So, formally speaking,
these types \emph{are} equivalent; but in practice, there is no ``natural''
choice of the conversion functions \lstinline!f1! and \lstinline!f2!
that will work correctly in all applications. because the meaning
of these data types is application-dependent. The type equivalence
is accidental\index{type equivalence!accidental}.
...
equivalent? Check whether the corresponding logic identity and the
arithmetic identity hold.
...
that the type notation is not misleading us here.
...
\end{lstlisting}
It is clear from the code that these functions are inverses of each
other. This confirms the type equivalence.
...
Assume that the number of all distinct values of type \lstinline!A!
is $\left|A\right|$. Any possible value of type \lstinline!Option[A]!
...
about type equivalences. The correct answer was found immediately
...
have? A function \lstinline!f:A=>B! needs to select a value of type
$B$ for each possible value of type $A$. Therefore, the number of
different functions \lstinline!f:A=>B! is $\left|B\right|^{\left|A\right|}$
(the arithmetic exponent, $\left|B\right|$ to the power $\left|A\right|$).
...
\]
In practice, many of these functions will map integers to integers
in a complicated (and practically useless) way and will be impossible
to implement on a realistic computer because their code will be much
longer than the available memory. So, the actual number of implementable
functions of type $A\Rightarrow B$ is much smaller than $\left|B\right|^{\left|A\right|}$.
...
no type equivalence available for the type expression $A\Rightarrow B+C$.
Because such type expressions cannot be transformed into an equivalent
formula with simpler parts, the presence of such type expressions
makes type reasoning more complicated.
...
\end{tabular}
\par\end{centering}
\caption{Logical identities and arithmetic identities for function types. In
the last column, we defined $a\triangleq\left|A\right|$, $b\triangleq\left|B\right|$,
and $c\triangleq\left|C\right|$ for brevity.\label{tab:Logical-identities-with-function-types}}
\end{table}

We will now prove some of these type identities. We will also develop
better notation for reasoning about code.
...
\lstinline!x:A!. Substituting the Scala code, we get
\begin{lstlisting}
f1(f2(x)) == f1(_ => x) = (_ => x)(()) = x
\end{lstlisting}
Now compute \lstinline!f2(f1(x))! for arbitrary \lstinline!h: Unit => A!:
\begin{lstlisting}
f2(f1(h)) == f2(h(())) == { _ => h(()) }
\end{lstlisting}
...
$A$; that is, a value computed on demand every time.
...
``not implemented''. Trying to evaluate this symbol will produce
an error:
\begin{lstlisting}
scala> ???
...
argument, this error will never happen. Therefore, one can formally
view the result value (which will never be computed or returned) as
having any desirable type, e.g.~the type $A$.
...
and $g^{:\bbnum 0\Rightarrow A}$. We could show that $f$ and $g$
are \emph{not} the same function only if we could find a value $x$
such that $f(x)\neq g(x)$. However, there are \emph{no} values of
type $\bbnum 0$, and so we will never be able to find the required
value $x$. It follows that any two functions $f$ and $g$ of type
$\bbnum 0\Rightarrow A$ are equal. In other words, there exists only
\emph{one} distinct value of type $\bbnum 0\Rightarrow A$ (i.e.~the
cardinality of the type $\bbnum 0\Rightarrow A$ is $1$). So, the
type $\bbnum 0\Rightarrow A$ is equivalent to the type $\bbnum 1$.

\subsubsection{Example \label{subsec:ch-Example-type-identity-3}\ref{subsec:ch-Example-type-identity-3}}
...
is itself $\bbnum 0$ (which might be the case since $A$ is a type
parameter) but $\left|A\Rightarrow\bbnum 0\right|=0$ for all other
types $A$. We conclude that the type $A\Rightarrow\bbnum 0$ is not
equivalent to $\bbnum 0$ or $\bbnum 1$ for all $A$; it is equivalent
to $\bbnum 0$ only for non-void types $A$.
...
to the \lstinline!Unit! type, $\bbnum 1$.
...
In a similar way, we produce a function of type \lstinline!B => C!.
The code is
\begin{lstlisting}
def f1[A,B,C]: (Either[A, B] => C) => (A => C, B => C) =
...
Rather than To compute the composition $f_{1}\bef f_{2}$, we write
(omitting types)
\begin{align*}
...
row will be selected automatically if we use the rules of matrix multiplication
with a \emph{row} vector placed to the \emph{left} of the matrix,
and if we continue to use the convention of omitting any terms containing
$\bbnum 0$:
\begin{align*}
 & \begin{array}{|cc|}
...
from a function when the argument is to the \emph{left} of the function.
We need to obtain $p(a)$ and $q(b)$ in the places where we have
$a\triangleright p$ and $b\triangleright q$. So, let us \emph{define}
the symbol $\triangleright$ (pronounced ``pipe'') as a function
application where the argument is to the left of the function, $x\triangleright f\triangleq f(x)$.
In Scala, this operation is available as \lstinline!x.pipe(f)! as
of Scala 2.13.

We can then denote values of disjunctive types, such as $a+\bbnum 0$,
by row vectors:
\begin{equation}
...
\end{align*}
Then we can complete the proof of $f_{1}\bef f_{2}=\text{id}$:
\begin{align*}
...
g
\end{array})\\
{\color{greenunder}\text{matrix notation}:}\quad & =f\times g\Rightarrow\left(a\Rightarrow a\triangleright f\right)\times\left(b\Rightarrow b\triangleright g\right)\\
{\color{greenunder}\text{definition of }\triangleright:}\quad & =f\times g\Rightarrow\left(a\Rightarrow f(a)\right)\times\left(b\Rightarrow g(b)\right)\\
...
{\color{greenunder}\text{simplify function }a\Rightarrow h(a):}\quad & =\left(h\Rightarrow h\right)=\text{id}\quad.
\end{align*}
...
\begin{itemize}
\item Decide type equivalence using the rules of school mathematics.
\item Simplify type expressions before writing code.
\end{itemize}
\item Convert a fully parametric type signature into a logical formula.
\begin{itemize}
\item Decide whether the type signature can be implemented in code.
\item If so, derive or generate the code using the CH correspondence.
\end{itemize}
\end{itemize}
What tasks cannot be performed with these tools?
\begin{itemize}
\item Automatically generate code for a recursive function. (We usually
\emph{can} derive recursive code by hand.)
...
but cannot automatically generate code from type signatures.
...
\lstinline!.map! method for \lstinline!Array!. That methods has
...
Write this type in the type notation and simplify to an equivalent
type.
...
type \lstinline!Option[Boolean]! has one more value:
\[
...
So, the type notation it is consistent with the cardinality $3$ of
that type,
\[
\left|\bbnum 1+\text{Bool}\right|=\left|\bbnum 1+\bbnum 2\right|=1+2=3\quad.
\]

The type of functions \lstinline!Option[Boolean] => Boolean! is denoted
...
\]
Finally, the outer \lstinline!Option[]! type constructor adds $1$
...
Transform the type \lstinline!P = (Either[A, B], Either[C, D])! into
...
\]
We can use the usual rules of school mathematics to expand brackets
in this type expression and to obtain an equivalent type:
\[
...
all this information in a single value of type $A$. 
...
rules. For the first pair type signatures, we find
\begin{lstlisting}
...
we try to implement fully parametric functions with the corresponding
type signatures. For the first type signature:
\begin{lstlisting}
def f1[A,B,C]: (((A, B)) => C) => Either[A => C, B => C] = k => ???
\end{lstlisting}
We are required to return either a \lstinline!Left(g)! with \lstinline!g: A => C!,
or a \lstinline!Right(h)! with \lstinline!h: B => C!. Our only given
...
a function of type \lstinline!B => C!.
...
is to find the truth values for the variables $\alpha$, $\beta$,
and $\gamma$ for which both sides are $True$ or $False$. The left-hand
side, $\left(\alpha\wedge\beta\right)\Rightarrow\gamma$, can be $False$
only if $\alpha\wedge\beta=True$ (that is, both $\alpha$ and $\beta$
are $True$) and $\gamma=False$; for all other truth values of $\alpha$,
$\beta$, and $\gamma$, the formula $\left(\alpha\wedge\beta\right)\Rightarrow\gamma$
is $True$. Let us then determine when the right-hand side, $(\alpha\Rightarrow\gamma)\vee(\beta\Rightarrow\gamma)$,
can be $False$. This can happen only if both parts of the disjunction
are $False$; this means $\alpha=True$, $\beta=True$, and $\gamma=False$.
We find that the two sides of the identity~(\ref{eq:ch-example-identity-boolean-not-constructive})
...
\end{lstlisting}
The argument of type $E$ must be discarded: there is no way we could
have used it while computing a value of type \lstinline!A! given
an \lstinline!x:A!.

The second type signature has three type parameters, and has a typical
type signature of the standard function \lstinline!map!:
\begin{lstlisting}
def map[E, A, B]: (E => A) => (A => B) => E => B = ???
\end{lstlisting}
We note that the two curried arguments are functions of types $E\Rightarrow A$
and $A\Rightarrow B$. The forward composition of these functions
is a function of type $E\Rightarrow B$, or $\text{Read}^{E,B}$,
which is exactly what we are required to return. So the code can be
written as
...
We are required to compute a value of type $B$ given \emph{three}
curried arguments $r^{:E\Rightarrow A}$, $f^{:A\Rightarrow B}$,
and $e^{:E}$. The only way of getting a value of type $B$ is to
apply $f^{:A\Rightarrow B}$ to some value of type $A$. The only
way of getting an $A$ is to apply $r$ to a value of type $E$. We
have exactly one value of type $E$, namely $e^{:E}$. So the code
must be 
\[
...
we need to compute a value of type $T$. The only way of getting a
$T$ is to apply $r$ to some value of type $A$. However, we do not
have any values of type $A$. We have a function $f^{:A\Rightarrow B}$
that consumes values of type $A$, and we cannot use $f$ to produce
any values of type $A$. So we find ourselves unable to implement
this function.

In order to verify that it is unimplementable, we need to prove that
the logical formula
\begin{equation}
...
Define a type $P^{A}\triangleq\bbnum 1+A+A$ and implement the function
\[
\text{map}^{A,B}:P^{A}\Rightarrow(A\Rightarrow B)\Rightarrow P^{B}\quad.
\]
As a check that the function \lstinline!map! does not lose information,
verify the law \lstinline!map(p)(x => x) == p! for any \lstinline!p:P[A]!.
...
\lstinline!x!. If we return \lstinline!P3(...)!, we will satisfy
the type signature and preserve the information about \lstinline!x!,
but we will lose the information that the input value was a \lstinline!P2!
rather than a \lstinline!P3!. So we should return \lstinline!P2(...)!
...
So the final code of \lstinline!map! is
\begin{lstlisting}
def map[A, B]: P[A] => (A => B) => P[B] = p => f => p match {
  case P1() => P1()         // No choice here.
...
For the type constructor \lstinline!Either[L, R]! (i.e.~$L+R$),
...
We begin writing the code of \lstinline!map! as
\[
...
The plugin will print the known type information about the typed hole
in the code.
...
the pair that $t(s)$ returns. We can write an explicit destructuring
function like this:
\[
...
$s$, so we can rewrite this code as a shorter formula, 
\[
...
of type $S$. We have only one given value of type $S$, so there
is no choice here:
\[
...
\end{lstlisting}
As before, we choose not to discard the computed value \lstinline!s2!
...
the equivalent type $(\bbnum 1+T)+A$, i.e. \lstinline!Either[Option[T], A]!.
...
$A+Z\Rightarrow(A\Rightarrow B)\Rightarrow B+Z$

$A+Z\Rightarrow B+Z\Rightarrow(A\Rightarrow B\Rightarrow C)\Rightarrow C+Z$

$\text{flatMap}^{E,A,B}:\text{Read}^{E,A}\Rightarrow(A\Rightarrow\text{Read}^{E,B})\Rightarrow\text{Read}^{E,B}$

$\text{State}^{S,A}\Rightarrow\left(S\times A\Rightarrow S\times B\right)\Rightarrow\text{State}^{S,B}$
...
$\text{map}^{Z,A,B}:\text{Search}^{Z,A}\Rightarrow\left(A\Rightarrow B\right)\Rightarrow\text{Search}^{Z,B}$

$\text{flatMap}^{Z,A,B}:\text{Search}^{Z,A}\Rightarrow(A\Rightarrow\text{Search}^{Z,B})\Rightarrow\text{Search}^{Z,B}$

\section{Discussion}

\subsection{Using the Curry-Howard correspondence for writing code}

This chapter shows how the CH correspondence performs two kinds of
practically important tasks of reasoning about types: checking whether
a type signature can be implemented as a fully parametric function,
and determining whether two types are equivalent. The first task is
accomplished by mapping type expressions into formulas in the constructive
logic and by applying the proof rules of that logic. The second task
is accomplished by mapping type expressions into \emph{arithmetic}
...
is impossible. Exercises in this chapter will help build up the required
technique. The type notation introduced in this book is designed for
recognizing patterns in type expressions and for reasoning about them
more easily.

Throughout this chapter, we were mostly concerned about fully parametric
functions. The reason is that the CH correspondence becomes informative
only with parameterized types and with fully parametric functions.
For concrete types, e.g.\ \lstinline!Int!, we can always produce
\emph{some} value even with no previous data, so the proposition $\mathcal{CH}(\text{Int})$
...
are able to compute any value of type $A$. The validity of ${\cal CH}$-propositions
...
We have shown that a type equivalence corresponds to each arithmetic
identity such as $\left(a+b\right)+c=a+\left(b+c\right)$, $\left(a\times b\right)\times c=a\times(b\times c)$,
$1\times a=a$, $\left(a+b\right)\times c=a\times c+b\times c$, etc.
So, having an arithmetic identity always corresponds to a type equivalence.
We are allowed to transform types as if they were arithmetic expressions,
e.g.~to rewrite
\[
\left(A+B\right)\times C+D\cong D+A\times C+B\times C\quad.
\]
The type notation makes this reasoning easier and more intuitive for
people familiar with arithmetic. As a result, we can freely use the
standard rules of school mathematics for deriving type equivalences
or for simplifying type expressions. It is useful for programmers
to be able to reason about types and transform type expressions to
equivalent simpler types before starting to write code. 

These results apply to any type expressions built up using product
...
may be called ``exponential-polynomial''\index{exponential-polynomial type}
types.

This ``arithmetic CH correspondence'' allows us to perform type
manipulations corresponding to any arithmetic transformations that
involve addition, multiplication, and powers. There is no types corresponding
to subtraction or division, so equations such as 
\begin{align*}
\left(1-t\right)\times\left(1+t\right) & =1-t\times t\quad,\\
\frac{t+t\times t}{t} & =1+t\quad,
\end{align*}
do not yield type equivalences. However, consider this standard formula,
\[
\frac{1}{1-t}=1+t+t^{2}+t^{3}+...+t^{n}+...\quad.
\]
At first sight, this formula appears to involve subtraction, division,
and an infinite series. However, this formula can be rewritten as
\begin{equation}
...
These constructions are the foundational, in that all types and all
code is expressed via these constructions. A programming language
that does not support some of these constructions cannot be considered
...
A functional programming language supporting all the standard type
and code constructions \textendash{} for instance, OCaml, Haskell,
F\#, Scala, Swift, etc., \textendash{} will be mapped into constructive
...
languages are mapped into incomplete logics where some theorems will
not be provable. Languages such as Python, JavaScript, Ruby, Clojure
have no type checking and so are mapped to inconsistent logics where
any proposition can be derived (even propositions normally considered
$False$). 

The deficiency of the logics is translated into inability of these
languages to easily express certain computations with data, such as
handling data that belongs to a disjoint domain. The inconsistency
of a logic means that a program might ``derive $False$ from $True$'',
i.e.~the program thinks it has computed a value that is not actually
there. In practice, this usually means that a program crashes because
some value is missing, is ``null'', or is represented by an ``invalid
pointer''.

So, the CH correspondence gives a mathematically justified procedure
for designing the type systems in programming languages. The procedure
contains these steps:
\begin{itemize}
\item Choose a complete logic that is free of inconsistencies.
\item For each logical operation, provide a type construction.
\item For each proof rule, provide a code construction.
\end{itemize}
Mathematicians have studied different logics such as modal logic,
temporal logic, linear logic, and so on. For each of these logics,
mathematicians have determined the minimal complete sets of logical
operations, axioms, and proof rules that do not lead to inconsistency.
This allows programming language designers to pick and choose logics
and to routinely translate them into minimal programming languages
whose programs are guaranteed to have correct types. This mathematical
guarantee is a powerful help for programmers since it automatically
...
are quite limited. One use case is for a branch of a \lstinline!match!
/ \lstinline!case! expression that does not return a value (e.g.~because
...
then map to a value of any other type (through the function \lstinline!absurd[A]: Nothing => A!,
see Example~\ref{subsec:ch-Example-type-identity-0-to-A}). 
...
both branches must return values of the same type. So, the Scala compiler
...
to convert that type to \lstinline!Double!. In this way, types can
be made to match in the definition of the value \lstinline!x!. 
...
Finally, none of our examples involved the logical \textbf{negation}\index{negation}
...
in constructive logic. However, any theorem of constructive logic
is also a theorem of Boolean logic.

The reason is that each of the rules of constructive logic (Section~\ref{subsec:The-rules-of-proof})
preserves Boolean truth values. To verify that a formula is true in
Boolean logic, we only need to check that the truth value of the formula
is $True$ for all possible truth values of its variables. A sequent
such as $\alpha,\beta\vdash\gamma$ is true in Boolean logic if and
only if $\gamma=True$ given that $\alpha=\beta=True$. We can check
that all proof rules of Section~\ref{subsec:The-rules-of-proof}
have this property: if the premises are true in the Boolean sense,
the goal is also true.

It is much easier to check Boolean truth than to find a proof tree
in constructive logic. So, if we find that a formula is not a theorem
in Boolean logic, we know it is also not a theorem in constructive
logic. This gives us a quick way of proving that some type signatures
are not implementable as fully parametric functions. Examples of formulas
that are not true in Boolean logic are
\begin{align*}
 & \forall A.\,A\quad,\\
 & \forall(A,B).\,A\Rightarrow B\quad,\\
 & \forall(A,B).\,(A\Rightarrow B)\Rightarrow B\quad,
\end{align*}
as well as formulas shown in Table~\ref{tab:Logical-formulas-not-Boolean-theorems}
(Section~\ref{subsec:ch-Motivation-and-first-examples}).
