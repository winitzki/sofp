...
\subsection{Types and propositional logic}
...
Need to verify that $f_{1}\circ f_{2}=id$ and $f_{2}\circ f_{1}=id$
...
Implement {\footnotesize{}$map^{A,B}:1+A\Rightarrow(A\Rightarrow B)\Rightarrow1+B$
...
Implement \texttt{\textcolor{blue}{\footnotesize{}map}} and \texttt{\textcolor{blue}{\footnotesize{}concat}}
...
Define type {\footnotesize{}$\text{OptEither}^{A,B}=1+A+B$ }and implement\texttt{\textcolor{blue}{\footnotesize{}
...
Define recursive type {\footnotesize{}$\text{Tr3}{}^{A}\equiv1+A\times A\times A\times\text{Tr3}^{A}$};
