...
def g3[A,B,C]: ((Either[A, C], Either[B, C])) => Either[(A, B), C] = {
  ...
  case (Right(c1), Right(c2))  => Right(c1)    // Must ignore c1 or c2 here!
...
because we will have ignored one of the given values of type \lstinline!C!.
...
\end{lstlisting}
Implementing \lstinline!f2! is straightforward; we can just ignore
...
\subsubsection{Example \label{subsec:ch-Example-type-identity-1}\ref{subsec:ch-Example-type-identity-1}}
...
However, Example~\ref{subsec:ch-Example-type-identity-1} shows that
$\bbnum 0\Rightarrow\bbnum 0$ has cardinality $1$. So, the cardinality
$\left|A\Rightarrow\bbnum 0\right|=1$ if the type $A$ is itself
$\bbnum 0$ (which might be the case since $A$ is a type parameter)
but $\left|A\Rightarrow\bbnum 0\right|=0$ for all other types $A$.
We conclude that the type $A\Rightarrow\bbnum 0$ is not equivalent
to $\bbnum 0$ or $\bbnum 1$ for all $A$; it is equivalent to $\bbnum 0$
only for non-void types $A$.
...
is known about that type. So the code of $f$ \emph{must} ignore its
argument and return the fixed value \lstinline!()! of type \lstinline!Unit!.
...
\begin{itemize}
\item Decide type equivalence using school-level algebra.
...
\end{itemize}
To see an example of type constructors with properties, consider a
...
requires \index{first-order logic}\textbf{first-order logic}, that
is, a logic whose proof rules can handle quantified types such as
$\forall(A,B)$\emph{ inside} premises. However, first-order logic
is \textbf{undecidable\index{undecidable logic@\textbf{undecidable logic}}}:
...
\[
\left|\text{Opt}^{\text{Bool}}\right|=3\quad.
\]
In the type notation, \lstinline!Boolean! is denoted by $\bbnum 2$,
and the type \lstinline!Option[Boolean]! by $\bbnum 1+\bbnum 2$.
So, the type notation it is consistent with the cardinality $3$ of
that type,
\[
\left|\bbnum 1+\bbnum 2\right|=1+2=3\quad.
\]
...
\]
We can use the rules of school algebra to expand brackets in this
...
\lstinline!a1! and ignores \lstinline!a2! (or vice versa).
...
{\color{greenunder}\text{use de Morgan's law}:}\quad & =\neg\alpha\vee\neg\beta\vee\neg\gamma\quad;\\
 & (\alpha\gunderline{\Rightarrow}\gamma)\vee(\beta\gunderline{\Rightarrow}\gamma)\\
...
$False$. In this sense, constructive logic does not assume that every
proposition is either $True$ or $False$. This is not intuitive,
...
\end{lstlisting}
The argument of type $E$ must be ignored: there is no way we could
...
logic. By the CH correspondence, we conclude that the function \lstinline!m!
cannot be implemented.

\subsubsection{Example \label{subsec:ch-solvedExample-7}\ref{subsec:ch-solvedExample-7}}

Implement $\text{map}^{A,B}:\bbnum 1+A+A\Rightarrow(A\Rightarrow B)\Rightarrow\bbnum 1+B+B$
with no ``information loss''; verify that \lstinline!map(y)(x => x) == y!.

\subparagraph{Solution}

Begin by defining a Scala type for the notation $\bbnum 1+A+A$:
\begin{lstlisting}
sealed trait P[A]
final case class P1[A]() extends P[A]
final case class P2[A](x: A) extends P[A]
...
in the implementation, we will aim to preserve information as much
as possible.
\begin{lstlisting}
...
only way of getting a value of $B$ is to apply $f$ to $x$. So,
we return \lstinline!P2(f(x))!.
...
\end{lstlisting}
A matrix notation for this function is
\[
\text{map}^{A,B}=p^{:\bbnum 1+A+A}\Rightarrow f^{:A\Rightarrow B}\Rightarrow p\triangleright\begin{array}{|c||ccc|}
 & \bbnum 1 & B & B\\
...
by preferring \lstinline!R! over \lstinline!L!.

\subparagraph{Solution}

\subsubsection{Example \label{subsec:ch-solvedExample-9}\ref{subsec:ch-solvedExample-9}{*}}

Denoting $\text{State}^{S,A}\equiv S\Rightarrow A\times S$, implement
the functions:
...
It is assumed that all functions must be fuly parametric.
...
Show that $A\Rightarrow(B+C)\neq(A\Rightarrow B)\times(A\Rightarrow C)$
in logic.
...
Define type $\text{OptE}^{A,B}\triangleq\bbnum 1+A+B$ and implement
\lstinline!map! and \lstinline!flatMap! for it, without information
loss, preferring $B$ over $A$. Get the same result using the equivalent
type $(\bbnum 1+A)+B$, i.e. \lstinline!Either[Option[A], B]!. The
required type signatures are
\begin{align*}
\text{map}^{A,B,C} & :\text{OptE}^{A,B}\Rightarrow\left(B\Rightarrow C\right)\Rightarrow\text{OptE}^{A,C}\quad,\\
\text{flatMap}^{A,B,C} & :\text{OptE}^{A,B}\Rightarrow(B\Rightarrow\text{OptE}^{A,C})\Rightarrow\text{OptE}^{A,C}\quad.
\end{align*}
...
\subsection{Using ``school algebra'' to reason about types}

To reason about equivalent types, we can use the rules of school-level
algebra (that is, the rules of arithmetic).{*}{*}{*}

Recursive type: ``list of \texttt{\textcolor{blue}{\footnotesize{}A}}'',
type notation: $\text{List}^{A}$
...
are extremely limited. The only use case is for if or case clauses
that do not return a value; they are considered formally to return
a value of type \lstinline!Nothing!, which can then generate a value
of any other type. In this way, types can match in the expression
\lstinline!if (x=0) 123 else throw new Exception("...")!. However,
we will not use exceptions in this book.

We did not talk about the logical negation; it is defined as $\neg A\equiv A\Rightarrow False$
and its practical use is as limited as that of $False$.
