...
\chapter{The formal logic of types III. The Curry-Howard correspondence\label{chap:3-3-The-formal-logic-curry-howard}}
...
systematic derivation algorithm. This chapter will describe the main
results and applications of this theory to functional programming.
Technical details about the algorithm are found in Appendix~\ref{app:The-Curry-Howard-correspondence}.
...
values of type \lstinline!A! and without calling a function that
would return a value of type \lstinline!A!. There are no values of
type \lstinline!A! given as arguments; the given function \lstinline!f: A => B!
returns values of type \lstinline!B! and not \lstinline!A!. The
code of \lstinline!fmap! must do pattern matching on \lstinline!oa:Option[A]!,
which yields two cases, and the value x must be computed in both cases:
\begin{lstlisting}
def fmap[A, B](f: A => B): Option[A] => Option[B] = {
  case None       => 
    val x: A = ??? // Cannot compute x here!
...
Can we prove rigorously that the implementation of \lstinline!before!
is unique, and that no values of type \lstinline!C => A! can be computed
within the body of \lstinline!before!? Or would some clever trick
(that we do not know about) produce a value of that type? So far,
we only have an intuition about whether a value of certain type can
be computed. To make our arguments rigorous, we need to translate
the statement such as ``the body of a fully parametric function can
compute a value of type \lstinline!T! from given arguments of types
\lstinline!X!, \lstinline!Y!, ..., \lstinline!Z!'' into a mathematical
formula.

{*}{*}{*}This formula will be either provable or not.

The considerations will be simpler if we assume that the function
is \emph{returning} a value of type \lstinline!T! (rather than computing
it as an intermediate result). Being able to return a \lstinline!t:T!
is equivalent to being able to compute a \lstinline!t:T! in the body
of the function. So, we can now formulate the statement

{*}{*}{*}We will denote this statement by ${\cal CH}(X\Rightarrow Y)$.

Let's denote this proposition by ${\cal CH}(T)$ \textendash{} ``$\mathcal{C}$ode
$\mathcal{H}$as a value of type T''

Correspondence between types and propositions, for a given program:
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline 
\textbf{Type} & \textbf{Proposition} & \textbf{Logic notation} & \textbf{Type notation}\tabularnewline
\hline 
\hline 
\texttt{\textcolor{blue}{\footnotesize{}A}} & ${\cal CH}(A)$ & $A$ & $A$\tabularnewline
\hline 
...
\hline 
\texttt{\textcolor{blue}{\footnotesize{}A $\Rightarrow$ B}} & ${\cal CH}(A)$ \emph{implies} ${\cal CH}(B)$ & $A\Rightarrow B$ & $A\Rightarrow B$\tabularnewline
\hline 
\texttt{\textcolor{blue}{\footnotesize{}Unit}} & \emph{True} & $T$ & $\bbnum 1$\tabularnewline
\hline 
\texttt{\textcolor{blue}{\footnotesize{}Nothing}} & \emph{False} & $F$ & $\bbnum 0$\tabularnewline
\hline 
\end{tabular}
\par\end{center}

Short notation helps reason about logic and types

Type parameter {[}T{]} in a function type means $\forall T$

Example: def dupl{[}A{]}:\ A $\Rightarrow$ (A, A). The type of this
function corresponds to the (valid) proposition $\forall A:A\Rightarrow A\times A$


\section{Slides}

\subsection{Working with the CH correspondence I}

Convert Scala types to short notation and back

Example 1: A disjunction type

\texttt{\textcolor{blue}{\footnotesize{}sealed trait UserAction}}{\footnotesize\par}

\texttt{\textcolor{blue}{\footnotesize{}case class SetName(first: String,
last: String) extends UserAction}}{\footnotesize\par}

\texttt{\textcolor{blue}{\footnotesize{}case class SetEmail(email: String)
extends UserAction}}{\footnotesize\par}

\texttt{\textcolor{blue}{\footnotesize{}case class SetUserId(id: Long)
extends UserAction}}{\footnotesize\par}
...
\texttt{\textcolor{blue}{\footnotesize{}sealed trait Either3{[}A,
B, C{]}}}{\footnotesize\par}

\texttt{\textcolor{blue}{\footnotesize{}case class Left{[}A, B, C{]}(x: A)
extends Either3{[}A, B, C{]}}}{\footnotesize\par}

\texttt{\textcolor{blue}{\footnotesize{}case class Middle{[}A, B,
C{]}(x: B) extends Either3{[}A, B, C{]}}}{\footnotesize\par}

\texttt{\textcolor{blue}{\footnotesize{}case class Right{[}A, B, C{]}(x: C)
extends Either3{[}A, B, C{]}}}{\footnotesize\par}
...
Define type \texttt{\textcolor{blue}{\footnotesize{}MyTU{[}T,U{]}}}
