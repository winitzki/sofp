...
\section{Values computable by fully parametric functions}
...
of the function'' means being able to compute \lstinline!x! in \emph{all}
cases, not just within one part of the \lstinline!match! expression.
...
Can we prove rigorously that no values of type \lstinline!C => A!
can be computed within the body of \lstinline!before!? Could some
clever trick produce a value of that type? So far, we only gave informal
...
a value of type $A$'', and here by ``the code'' we mean the body
of a given fully parametric function. Actually, the notation ${\cal CH}(A)$
...
the possibility of computing a value of a given type $A$. So, a precise
formulation of the proposition ${\cal CH}(A)$ is 
\begin{align}
{\color{greenunder}{\cal CH}\text{-proposition}:}\quad & \text{a fully parametric function having arguments of types}\nonumber \\
 & X,Y,...,Z\text{ can compute a value of type }A\quad.\label{eq:CH-proposition-def}
\end{align}
...
will be true. So, proposition~(\ref{eq:CH-proposition-def}) is equivalent
to ``${\cal CH}(A)$ assuming ${\cal CH}(X)$, ${\cal CH}(Y)$, ...,
${\cal CH}(Z)$''. In mathematical logic, a statement of this form
is called a \textbf{sequent}\index{sequent!example} and is denoted
by
\[
{\cal CH}(X),{\cal CH}(Y),...,{\cal CH}(Z)\vdash{\cal CH}(A)\quad.
\]
...
means proving sequents of this form.
...
\end{align*}
Derivations in formal logic are called ``proofs''. So, in this section
we gave informal arguments towards proving the first two sequents
...
\subsection{Type notation for the standard code constructions}
...
We begin with the \lstinline!Unit! type. That type has only a single
value \lstinline!()!, and this value can be \emph{always} computed
since it does not depend on having any previous data:
\begin{lstlisting}
def f[...]: ... = {
  ...
...
The special Scala type \lstinline!Nothing! has no values, so the
proposition ${\cal CH}($\lstinline!Nothing!$)$ is always false.
The type \lstinline!Nothing! is denoted by $\bbnum 0$ in the type
notation.
...
The next available type construction is a tuple. To compute a value
of a tuple type \lstinline!(A, B)! requires to compute a value of
type \lstinline!A! \emph{and} a value of type \lstinline!B!. This
is expressed by the logic formula ${\cal CH}($\lstinline!(A, B)!$)={\cal CH}(A)\wedge{\cal CH}(B)$.
...
A disjunctive type generally consists of several case classes. Having
a value of a disjunctive type means to have a value of (at least)
one of those case classes. An example of translating this relationship
into a formula was shown by Eq.~(\ref{eq:curry-howard-example-disjunction}).
For the standard disjunctive type \lstinline!Either[A, B]!, we have
the logical formula ${\cal CH}($\lstinline!Either[A, B]!$)={\cal CH}(A)\vee{\cal CH}(B)$.
...
Finally, consider a function with type parameters, e.g.
\begin{lstlisting}
...
\[
\forall A.\forall B.{\cal CH}\left(A\Rightarrow(A\Rightarrow B)\Rightarrow B\right)\quad.
\]
The code notation for this parameterized function is 
\[
f^{A,B}:A\Rightarrow\left(A\Rightarrow B\right)\Rightarrow B\quad,
\]
and its type can be written as
\[
^{A,B}\Rightarrow A\Rightarrow\left(A\Rightarrow B\right)\Rightarrow B\quad,
\]
...
\[
f:\big({}^{A,B}\Rightarrow F^{A,B}\big)=\big(^{A,B}\Rightarrow x^{:A}\Rightarrow g^{:A\Rightarrow B}\Rightarrow g(x)\big)\quad.
\]
...
The seven kinds of type constructions we just considered (the unit
type, the \lstinline!Nothing! type, the primitive type, the tuple
type, the disjunctive type, the function type, and the parameterized
type) are all the standard type constructions available in functional
languages.

Case classes and disjunctive types add \emph{names} to the types and
to their parts. However, those names are useful only for the programmer's
convenience and do not affect the computational properties of types.
So, no names are used in the type notation.
...
\hline 
\lstinline!Nothing! & $\bbnum 0$ & {\small{}always} \emph{False}\tabularnewline
\hline 
{\small{}parameterized method} \lstinline!def f[A]: F[A]! & $f^{A}:F^{A}$ & $\forall A.\,{\cal CH}(F^{A})$\tabularnewline
\hline 
{\small{}parameterized value (Scala 3)} \lstinline![A] => F[A]! & $^{A}\Rightarrow F^{A}$ & $\forall A.\,{\cal CH}(F^{A})$\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
\caption{The correspondence between type notation and ${\cal CH}$-propositions.\label{tab:ch-correspondence-type-notation-CH-propositions}}
...
Let us use the letter $\Delta$ to denote the function \lstinline!delta!.
In the type notation, the type of $\Delta$ is denoted by 
\[
...
so that parentheses $A\Rightarrow\left(A\times A\right)$ may be omitted.
...
The type notation
\[
F^{A}\triangleq1+\text{Int}\times A\times A+\text{Int}\times\left(\text{Int}\Rightarrow A\right)
\]
defines a type constructor \lstinline!F! that can be implemented
in Scala as
\begin{lstlisting}
sealed trait F[A]
...
the type notation as $\text{MuTU}^{T,U}\triangleq1+T\times U+\text{Int}\times(T\Rightarrow T)+\text{String}\times U$.
...
notation as $\text{OptEither}^{A,B}\triangleq1+A+B$.
...
\subsection{Working with the CH correspondence II}

Any correct proposition can be implemented in code
\begin{center}
\begin{tabular}{|c|c|}
\hline 
...
Incorrect propositions \emph{cannot be implemented} in code 

Examples of incorrect propositions:\\
...
\begin{lstlisting}
def f[A, B]: ((((A => B) => A) => A) => B) => B = ???
\end{lstlisting}

Constructive propositional logic has a decision algorithm

See code examples using the \href{https://github.com/Chymyst/curryhoward}{curryhoward}
library, and Appendix~???

\subsection{Working with the CH correspondence III}
...
Some standard identities in logic ($\forall A\forall B\forall C$
is assumed):
...
\subsection{Working with the CH correspondence IV}
...
Provide a type constructor for each basic operation (e.g.~``\emph{or}'',
``\emph{and}'')

\subsection{Working with the CH correspondence V}
...
\section{Some slides may be repeated}
...
\texttt{\textcolor{blue}{\footnotesize{}def f1{[}A{]}(pair: (A, Unit)): A
= pair.\_1}}{\footnotesize\par}

\texttt{\textcolor{blue}{\footnotesize{}def f2{[}A{]}(x: A): (A, Unit)
= (x, ())}}{\footnotesize\par}

Verify that both their compositions equal \texttt{\textcolor{blue}{\footnotesize{}identity}} 
...
\subsection{Type isomorphisms IV}
...
Have identities $a^{c}\times b^{c}=\left(a\times b\right)^{c}$, $a^{b+c}=a^{b}\times a^{c}$,
$a^{b\times c}=\left(a^{b}\right)^{c}$

If the set size (\textbf{cardinality}) differs, $A$ and $B$ cannot
be equivalent 
...
Logic identities only signify \emph{equal implementability} of types
...
\subsection{Making practical use of the CH correspondence I}
...
Let the computer \emph{generate} the code from type when possible
