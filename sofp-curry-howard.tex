...
and determining whether two types are equivalent. The first tasks
is accomplished by mapping type expressions into formulas in the constructive
...
technique. The type notation introduced in this book is designed to
enable the programmer to recognize patterns in type expressions and
to reason about them more easily.
...
such as $\left(a+b\right)+c=a+\left(b+c\right)$, $\left(a\times b\right)\times c=a\times(b\times c)$,
...
\]
We have introduced the type notation to make this reasoning easier
and more intuitive for people familiar with arithmetic. As a result,
we can freely use the standard rules of school mathematics for reasoning
about type equivalence or for simplifying any type expressions built
up using products, disjunctive types (also called ``sum'' types
because they correspond to arithmetic sums), and function types (also
called ``exponential'' types because they correspond to arithmetic
powers). These type expressions may be called ``exponential-polynomial''\index{exponential-polynomial type}
types.
...
involve addition, multiplication, and powers. We can use simplify
any type expression There is no analog for subtraction or division,
so equations such as 
\begin{align*}
\left(1-t\right)\times\left(1+t\right) & =1-t\times t\quad,\\
\frac{t+t\times t}{t} & =1+t\quad,
\end{align*}
do not correspond to any type equivalences. However, consider this
...
The functional programming paradigm is based on the seven standard
type constructions (Section~\ref{subsec:Type-notation-and-standard-type-constructions})
...
system of any programming language (functional or not) can be mapped
into a \emph{certain} \emph{logic}, i.e.~a system of logical propositions
and proof rules.
...
types and with fully parametric functions. For concrete types, e.g.\ \texttt{\textcolor{blue}{\footnotesize{}Int}},
we can always produce \emph{some} value even with no previous data,
so $\mathcal{CH}(\text{Int})$ is always true. If we permit functions
that are not fully parametric, we will not be able to reason about
type signatures. It is useful to know that $\forall A.\,A\Rightarrow A$
is always an identity function, but this reasoning is correct only
under the assumption of parametricity.
...
this type signature is insufficient to specify the code. Only a fully
parametric type signature, such as $A\Rightarrow\left(A\Rightarrow B\right)\Rightarrow B$,
...
Scala's \index{void type}void type \lstinline!Nothing! correspond
