...
\subsection{Type notation for the standard type constructions}
...
\begin{equation}
\forall(A,B,C).\,\left(A+B\right)\times C=A\times C+B\times C\quad?\label{eq:ch-example-distributive-1}
\end{equation}


\subsection{Equivalences of types vs.~identities in logic}

The CH correspondence maps Eq.~(\ref{eq:ch-example-distributive-1})
into the logic formula
\[
\left(A\vee B\right)\wedge C=\left(A\wedge C\right)\vee\left(B\vee C\right)\quad.
\]
...
the distributive law means that the two formulas hold,
\begin{align}
 & \left(A\vee B\right)\wedge C\Rightarrow\left(A\wedge C\right)\vee\left(B\vee C\right)\quad,\label{eq:ch-example-distributive-1a}\\
 & \left(A\wedge C\right)\vee\left(B\vee C\right)\Rightarrow\left(A\vee B\right)\wedge C\quad.\label{eq:ch-example-distributive-1b}
\end{align}
The CH correspondence maps these two logical formulas to fully parametric
...
Let us verify that these conditions hold for the functions \lstinline!f1!
and \lstinline!f2! defined above. {*}{*}{*}

Another example of a logical identity is
\[
...
of types? This turns out to be \emph{not} so. {*}{*}{*}
