...
\item Get $F^{1+A}$ from $F^{A}$ using $\text{fmap}\,(\text{Some}^{A\Rightarrow1+A}):F^{A}\Rightarrow F^{1+A}$ 
\end{itemize}
...
$\Rightarrow$ Seq{[}T{]}}} 
\end{itemize}
\item How to express \texttt{\textcolor{blue}{\footnotesize{}filter}} through
\texttt{\textcolor{blue}{\footnotesize{}flatten}}? (see example code)
\begin{itemize}
\item Types are transformed as $F^{A}\Rightarrow F^{1+A}\Rightarrow F^{1+A}\Rightarrow F^{A}$
\item $\text{filter}\left(p\right)=\text{fmap}\left(\text{bop}\,(p)\right)\circ\text{flatten}$,
where we defined \texttt{\textcolor{blue}{\footnotesize{}bop}} as
\end{itemize}
\end{itemize}
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}def~bop{[}T{]}(p:~T~$\Rightarrow$~Boolean):~T~$\Rightarrow$~Option{[}T{]}~=~...}{\footnotesize \par}
\end{lyxcode}
\begin{itemize}
\item Note: the $\text{Boolean}$ type is isomorphic to $1+1$ i.e.~\texttt{\textcolor{blue}{\footnotesize{}Option{[}Unit{]}}}{\footnotesize \par}
\end{itemize}
...
\begin{itemize}
\item Denote {\footnotesize{}$\psi^{A\Rightarrow1+A}\equiv\text{bop}\left(p^{A\Rightarrow1+1}\right)=x^{A}\Rightarrow\text{fmap}^{\text{Opt}}\left(\_\Rightarrow x\right)(p(x))$}{\footnotesize \par}
\begin{itemize}
\item Have property: {\footnotesize{}$f^{T\Rightarrow A}\circ\text{bop}\left(p^{A\Rightarrow1+1}\right)=\text{bop}\left(f\circ p\right)\circ\text{fmap}^{\text{Opt}}\,f$}{\footnotesize \par}
\end{itemize}
...
and {\footnotesize{}$\psi^{A\Rightarrow1+A}\equiv\text{bop}\left(p\right)$}{\footnotesize \par}
\begin{itemize}
...
\item Derive old law 2: need to work with{\footnotesize{} $\psi\equiv\text{bop}\left(p\right):A\Rightarrow1+A$ }{\footnotesize \par}
\begin{itemize}
