...
laws hold
\item $F^{A}\equiv1+A\times A$ (``collapsible product'')
\end{itemize}
...
\end{itemize}
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}def~optB{[}T{]}(p:~T~$\Rightarrow$~Boolean):~T~$\Rightarrow$~Option{[}T{]}~=~...}{\footnotesize \par}
\end{lyxcode}
\begin{itemize}
...
\begin{itemize}
\item {\footnotesize{}$\text{filter}(p^{A\Rightarrow\text{Boolean}})=\text{inflate}^{C^{A}\Rightarrow C^{1+A}}\circ\text{contrafmap}(\text{optB}\,(p))$}{\footnotesize \par}
